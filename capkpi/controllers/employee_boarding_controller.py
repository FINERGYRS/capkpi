...
from finergy.utils import add_days, flt, unique
...
class EmployeeBoardingController(Document):
		...
		if self.doctype == "Employee Separation":
			...
			if self.employee:
				...
				if not self.holiday_list:
					finergy.throw(_("Please set the Holiday List."), finergy.MandatoryError)
				else:
	...
	def on_cancel(self):
		# delete task project
		project = self.project
		for task in finergy.get_all("Task", filters={"project": project}):
			finergy.delete_doc("Task", task.name, force=1)
		finergy.delete_doc("Project", project, force=1)
		self.db_set("project", "")
		for activity in self.activities:
...
def update_employee_boarding_status(project):
	employee_onboarding = finergy.db.exists("Employee Onboarding", {"project": project.name})
	employee_separation = finergy.db.exists("Employee Separation", {"project": project.name})
	...
	if employee_onboarding:
		finergy.db.set_value("Employee Onboarding", employee_onboarding, "boarding_status", status)
	elif employee_separation:
		finergy.db.set_value("Employee Separation", employee_separation, "boarding_status", status)
