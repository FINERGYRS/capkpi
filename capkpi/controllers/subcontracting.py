...
class Subcontracting:
	...
	def __update_consumed_qty_in_po(self, itemwise_consumed_qty):
		...
		for row in finergy.get_all(
			...
			finergy.db.set_value("Purchase Order Item Supplied", row.name, "consumed_qty", consumed_qty)
	...
	def __validate_batch_no(self, row, key):
		if row.get("batch_no") and row.get("batch_no") not in self.__transferred_items.get(key).get(
			...
			msg = f'The Batch No {finergy.bold(row.get("batch_no"))} has not supplied against the Purchase Order {link}'
