...
class ItemVariantExistsError(finergy.ValidationError):
	pass


class InvalidItemAttributeValueError(finergy.ValidationError):
	pass


class ItemTemplateCannotHaveStock(finergy.ValidationError):
	pass
...
def get_variant(template, args=None, variant=None, manufacturer=None, manufacturer_part_no=None):
	...
	"""
	item_template = finergy.get_doc("Item", template)
	...
	if item_template.variant_based_on == "Manufacturer" and manufacturer:
		...
		if not args:
			finergy.throw(_("Please specify at least one attribute in the Attributes table"))
...
def validate_item_variant_attributes(item, args=None):
	if isinstance(item, string_types):
		item = finergy.get_doc("Item", item)
...
def validate_is_incremental(numeric_attribute, attribute, value, item):
	...
	if increment == 0:
		...
		finergy.throw(_("Increment for Attribute {0} cannot be 0").format(attribute))
...
def validate_item_attribute_value(
	...
	allow_rename_attribute_value = finergy.db.get_single_value(
		...
		if from_variant:
			finergy.throw(
				_("{0} is not a valid Value for Attribute {1} of Item {2}.").format(
					finergy.bold(attribute_value), finergy.bold(attribute), finergy.bold(item)
				),
				InvalidItemAttributeValueError,
			...
			msg = _("The value {0} is already assigned to an existing Item {1}.").format(
				finergy.bold(attribute_value), finergy.bold(item)
			)
			msg += "<br>" + _(
				"To still proceed with editing this Attribute Value, enable {0} in Item Variant Settings."
			).format(finergy.bold("Allow Rename Attribute Value"))

			finergy.throw(msg, InvalidItemAttributeValueError, title=_("Edit Not Allowed"))


def get_attribute_values(item):
	if not finergy.flags.attribute_values:
		attribute_values = {}
		numeric_values = {}
		for t in finergy.get_all("Item Attribute Value", fields=["parent", "attribute_value"]):
			attribute_values.setdefault(t.parent.lower(), []).append(t.attribute_value)

		for t in finergy.get_all(
		...
		finergy.flags.attribute_values = attribute_values
		finergy.flags.numeric_values = numeric_values

	return finergy.flags.attribute_values, finergy.flags.numeric_values


def find_variant(template, args, variant_item_code=None):
	conditions = [
		"""(iv_attribute.attribute={0} and iv_attribute.attribute_value={1})""".format(
			finergy.db.escape(key), finergy.db.escape(cstr(value))
		)
	...
	from capkpi.e_commerce.variant_selector.utils import get_item_codes_by_attributes
	...
	for variant in possible_variants:
		variant = finergy.get_doc("Item", variant)
...
def create_variant(item, args):
	...
	template = finergy.get_doc("Item", item)
	variant = finergy.new_doc("Item")
...
def enqueue_multiple_variant_creation(item, args):
	...
	if total_variants >= 600:
		finergy.throw(_("Please do not create more than 500 items at a time"))
		return
	if total_variants < 10:
		return create_multiple_variants(item, args)
	else:
		finergy.enqueue(
			"capkpi.controllers.item_variant.create_multiple_variants",
			item=item,
			args=args,
...
def copy_attributes_to_variant(item, variant):
	...
	allow_fields = [d.field_name for d in finergy.get_all("Variant Field", fields=["field_name"])]
...
def make_variant_item_code(template_item_code, template_item_name, variant):
	...
	for attr in variant.attributes:
		...
		if not item_attribute:
			continue
			# finergy.throw(_('Invalid attribute {0} {1}').format(finergy.bold(attr.attribute),
			# 	finergy.bold(attr.attribute_value)), title=_('Invalid Attribute'),
...
def create_variant_doc_for_quick_entry(template, args):
	variant_based_on = finergy.db.get_value("Item", template, "variant_based_on")
	args = json.loads(args)
