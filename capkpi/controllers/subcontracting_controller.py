...
from finergy.utils import cint, cstr, flt, get_link_to_form
...
class SubcontractingController(StockController):
	...
	def validate_items(self):
		for item in self.items:
			if not finergy.get_value("Item", item.item_code, "is_sub_contracted_item"):
				msg = f"Item {item.item_name} must be a subcontracted item."
				finergy.throw(_(msg))
			if item.bom:
				bom = finergy.get_doc("BOM", item.bom)
	...
	def __get_backflush_based_on(self):
		self.backflush_based_on = finergy.db.get_single_value(
			"Buying Settings", "backflush_raw_materials_of_subcontract_based_on"
		)

	def initialized_fields(self):
		self.available_materials = finergy._dict()
		self.__transferred_items = finergy._dict()
		self.alternative_item_details = finergy._dict()
	...
	def __get_transferred_items(self):
		...
		return finergy.get_all("Stock Entry", fields=fields, filters=filters)
	...
	def __get_received_items(self, doctype):
		...
		return finergy.get_all(f"{doctype}", fields=fields, filters=filters)
	...
	def __get_materials_from_bom(self, item_code, bom_no, exploded_item=0):
		...
		return (
			finergy.get_all("BOM", fields=fields, filters=filters, order_by=f"`tab{doctype}`.`idx`") or []
		)
	...
	def __get_qty_based_on_material_transfer(self, item_row, transfer_item):
		...
		if self.qty_to_be_received:
			...
			if transfer_item.serial_no or finergy.get_cached_value(
				"UOM", transfer_item.item_details.stock_uom, "must_be_whole_number"
			):
				return finergy.utils.ceil(qty)
	...
	def __validate_batch_no(self, row, key):
		if row.get("batch_no") and row.get("batch_no") not in self.__transferred_items.get(key).get(
			...
			msg = f'The Batch No {finergy.bold(row.get("batch_no"))} has not supplied against the {self.subcontract_data.order_doctype} {link}'
			finergy.throw(_(msg), title=_("Incorrect Batch Consumed"))
	...
	def __validate_serial_no(self, row, key):
		if row.get("serial_no"):
			...
			if incorrect_sn:
				...
				finergy.throw(_(msg), title=_("Incorrect Serial Number Consumed"))
	...
	def update_ordered_and_reserved_qty(self):
		...
		for sco, sco_item_rows in sco_map.items():
			if sco and sco_item_rows:
				sco_doc = finergy.get_doc("Subcontracting Order", sco)
		...
		for item in self.get("supplied_items"):
			if item.reference_name == item_row_id:
				if (
					...
					and finergy.get_cached_value("Item", item.rm_item_code, "is_stock_item")
				):
...
def make_rm_stock_entry(subcontract_order, rm_items, order_doctype="Subcontracting Order"):
	...
	if subcontract_order:
		subcontract_order = finergy.get_doc(order_doctype, subcontract_order)
...
def get_materials_from_supplier(
	...
	doc = finergy.get_cached_doc(order_doctype, subcontract_order)
