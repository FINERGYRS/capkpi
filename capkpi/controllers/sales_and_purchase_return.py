...
from finergy.utils import flt, format_datetime, get_datetime

import capkpi
from capkpi.stock.utils import get_incoming_rate


class StockOverReturnError(finergy.ValidationError):
	pass
...
def validate_return_against(doc):
	if not finergy.db.exists(doc.doctype, doc.return_against):
		finergy.throw(
			_("Invalid {0}: {1}").format(doc.meta.get_label("return_against"), doc.return_against)
		)
	else:
		ref_doc = finergy.get_doc(doc.doctype, doc.return_against)
...
def validate_returned_items(doc):
	...
	for d in doc.get("items"):
		if d.item_code and (flt(d.qty) < 0 or flt(d.get("received_qty")) < 0):
			if d.item_code not in valid_items:
				...
				ref = valid_items.get(d.item_code, finergy._dict())
				...
				if ref.rate and doc.doctype in ("Delivery Note", "Sales Invoice") and flt(d.rate) > ref.rate:
					...
					if not d.serial_no:
						finergy.throw(_("Row # {0}: Serial No is mandatory").format(d.idx))
					else:
				...
				if (
					...
					and finergy.db.get_value("Item", d.item_code, "is_stock_item")
				):
					finergy.throw(_("Warehouse is mandatory"))
	...
	if not items_returned:
		finergy.throw(_("Atleast one item should be entered with negative quantity in return document"))
...
def validate_quantity(doc, args, ref, valid_items, already_returned_items):
	...
	company_currency = capkpi.get_company_currency(doc.company)
	stock_qty_precision = get_field_precision(
		finergy.get_meta(doc.doctype + " Item").get_field("stock_qty"), company_currency
	)
	...
	for column in fields:
		...
		if reference_qty:
			if flt(args.get(column)) > 0:
				finergy.throw(_("{0} must be negative in return document").format(label))
...
def get_returned_qty_map_for_row(return_against, party, row_name, doctype):
	...
	reference_field = "dn_detail" if doctype == "Delivery Note" else finergy.scrub(child_doctype)
...
def make_return_doc(doctype: str, source_name: str, target_doc=None):
	...
	company = finergy.db.get_value("Delivery Note", source_name, "company")
	default_warehouse_for_sales_return = finergy.db.get_value(
	...
	def set_missing_values(source, target):
		...
		if doctype == "Sales Invoice" or doctype == "POS Invoice":
			...
			if not doc.select_print_heading:
				doc.select_print_heading = finergy.db.get_value("Print Heading", _("Credit Note"))

		elif doctype == "Purchase Invoice":
			# look for Print Heading "Debit Note"
			doc.select_print_heading = finergy.db.get_value("Print Heading", _("Debit Note"))
...
def get_rate_for_return(
	...
	if not return_against:
		return_against = finergy.get_cached_value(voucher_type, voucher_no, "return_against")
	...
	rate = flt(finergy.db.get_value("Stock Ledger Entry", filters, select_field))
	if not (rate and return_against) and voucher_type in ["Sales Invoice", "Delivery Note"]:
		rate = finergy.db.get_value(f"{voucher_type} Item", voucher_detail_no, "incoming_rate")
...
def get_filters(
	...
	if item_row:
		...
		reference_voucher_detail_no = finergy.db.get_value(
...
def get_returned_serial_nos(child_doc, parent_doc):
	...
	return_ref_field = finergy.scrub(child_doc.doctype)
	...
	for row in finergy.get_all(parent_doc.doctype, fields=fields, filters=filters):
