...
from finergy.utils import nowdate, unique

import capkpi
from capkpi.stock.get_item_details import _get_item_tax_template
...
def customer_query(doctype, txt, searchfield, start, page_len, filters):
	doctype = "Customer"
	conditions = []
	cust_master_name = finergy.defaults.get_user_default("cust_master_name")
	...
	searchfields = finergy.get_meta(doctype).get_search_fields()
...
def supplier_query(doctype, txt, searchfield, start, page_len, filters):
	doctype = "Supplier"
	supp_master_name = finergy.defaults.get_user_default("supp_master_name")
...
def item_query(doctype, txt, searchfield, start, page_len, filters, as_dict=False):
	...
	meta = finergy.get_meta(doctype, cached=True)
	...
	if filters and isinstance(filters, dict):
		if filters.get("customer") or filters.get("supplier"):
			...
			item_rules_list = finergy.get_all(
	...
	description_cond = ""
	if finergy.db.count(doctype, cache=True) < 50000:
...
def get_project_name(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters and filters.get("customer"):
		cond = """(`tabProject`.customer = %s or
			...
			finergy.db.escape(filters.get("customer"))
		)
...
def get_account_list(doctype, txt, searchfield, start, page_len, filters):
	...
	return finergy.desk.reportview.execute(
		doctype,
		filters=filter_list,
...
def get_blanket_orders(doctype, txt, searchfield, start, page_len, filters):
	return finergy.db.sql(
		...
		where
			...
			item_code=finergy.db.escape(filters.get("item")),
			blanket_order_type=filters.get("blanket_order_type"),
			company=finergy.db.escape(filters.get("company")),
		)
	)
...
def warehouse_query(doctype, txt, searchfield, start, page_len, filters):
	...
	query = """select `tabWarehouse`.name,
		...
		start=start,
		page_len=page_len,
		txt=finergy.db.escape("%{0}%".format(txt)),
	)

	return finergy.db.sql(query)
...
def get_batch_numbers(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters and filters.get("item"):
		query += " and item = {item}".format(item=finergy.db.escape(filters.get("item")))

	return finergy.db.sql(query, filters)
...
def item_manufacturer_query(doctype, txt, searchfield, start, page_len, filters):
	...
	item_manufacturers = finergy.get_all(
		"Item Manufacturer",
...
def get_purchase_receipts(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters and filters.get("item_code"):
		query += " and pritem.item_code = {item_code}".format(
			item_code=finergy.db.escape(filters.get("item_code"))
		)
...
def get_healthcare_service_units(doctype, txt, searchfield, start, page_len, filters):
	query = """
		...
		where
			...
			and company = {company}
			and name like {txt}""".format(
		company=finergy.db.escape(filters.get("company")), txt=finergy.db.escape("%{0}%".format(txt))
	)
	...
	if filters and filters.get("inpatient_record"):
		...
		if service_unit:
			query += " and (allow_appointments = 1 or name = {service_unit})".format(
				service_unit=finergy.db.escape(service_unit)
			)
		else:
...
def get_tax_template(doctype, txt, searchfield, start, page_len, filters):
	...
	item_doc = finergy.get_cached_doc("Item", filters.get("item_code"))
