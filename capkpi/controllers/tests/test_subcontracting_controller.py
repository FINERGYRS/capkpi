...
class TestSubcontractingController(FinergyTestCase):
	def setUp(self):
	...
	def test_incorrect_serial_no_components_based_on_material_transfer(self):
		...
		self.assertRaises(finergy.ValidationError, scr1.save)
		scr1.delete()
	...
	for row in scr_doc.get("supplied_items"):
		if row.rm_item_code not in supplied_items:
			supplied_items.setdefault(
				row.rm_item_code, finergy._dict({"qty": 0, "serial_no": [], "batch_no": defaultdict(float)})
			)
...
def make_stock_in_entry(**args):
	...
	for row in args.rm_items:
		...
		if row.item_code not in items:
			items.setdefault(
				row.item_code, finergy._dict({"qty": 0, "serial_no": [], "batch_no": defaultdict(float)})
			)
...
def make_subcontracted_items():
	...
	for item, properties in sub_contracted_items.items():
		if not finergy.db.exists("Item", item):
...
def make_bom_for_subcontracted_items():
	...
	for item_code, raw_materials in boms.items():
		if not finergy.db.exists("BOM", {"item": item_code}):
...
def get_subcontracting_order(**args):
	...
	if args.get("po_name"):
		po = finergy.get_doc("Purchase Order", args.get("po_name"))
