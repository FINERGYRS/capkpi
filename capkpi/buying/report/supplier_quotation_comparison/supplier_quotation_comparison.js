...
finergy.query_reports["Supplier Quotation Comparison"] = {
	filters: [
		{
			fieldtype: "Link",
			label: __("Company"),
		...
		},
		{
			...
			fieldtype: "Link",
			get_query: () => {
				let quote = finergy.query_report.get_filter_value('supplier_quotation');
				if (quote != "") {
					return {
						query: "capkpi.stock.doctype.quality_inspection.quality_inspection.item_query",
						filters: {
		...
		},
		{
			...
			get_data: function(txt) {
				return finergy.db.get_link_options('Supplier', txt);
			}
		},
		{
			fieldtype: "MultiSelectList",
			...
			default: "",
			get_data: function(txt) {
				return finergy.db.get_link_options('Supplier Quotation', txt, {'docstatus': ["<", 2]});
			}
		},
		{
			fieldtype: "Link",
	...
	formatter: (value, row, column, data, default_formatter) => {
		...
		if(column.fieldname === "valid_till" && data.valid_till){
			if(finergy.datetime.get_diff(data.valid_till, finergy.datetime.nowdate()) <= 1){
				value = `<div style="color:red">${value}</div>`;
			}
			else if (finergy.datetime.get_diff(data.valid_till, finergy.datetime.nowdate()) <= 7){
	...
	onload: (report) => {
		...
		report.page.add_inner_button(__("Select Default Supplier"), () => {
			let reporter = finergy.query_reports["Supplier Quotation Comparison"];
		...
		dialog.set_primary_action(__("Set Default Supplier"), () => {
			...
			if(values) {
				...
				finergy.call({
					...
					freeze: true,
					callback: (r) => {
						finergy.msgprint(__("Successfully Set Supplier"));
						dialog.hide();
					}
				});
			}
