...
class InvalidFormulaVariable(finergy.ValidationError):
	pass
...
class SupplierScorecardCriteria(Document):
	...
	def validate_formula(self):
		...
		try:
			finergy.safe_eval(test_formula, None, {"max": max, "min": min})
		except Exception:
			finergy.throw(_("Error evaluating the criteria formula"))


@finergy.whitelist()
def get_criteria_list():
	criteria = finergy.db.sql(
		"""
		SELECT
			scs.name
		FROM
...
def get_variables(criteria_name):
	criteria = finergy.get_doc("Supplier Scorecard Criteria", criteria_name)
	...
	for dummy1, match in enumerate(mylist):
		for dummy2 in range(0, len(match.groups())):
			try:
				...
				finergy.throw(_("Unable to find variable:") + " " + str(match.group(1)), InvalidFormulaVariable)
