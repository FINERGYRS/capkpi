...
class TestSupplierScorecardCriteria(FinergyTestCase):
	def test_variables_exist(self):
		delete_test_scorecards()
		for d in test_good_criteria:
			finergy.get_doc(d).insert()

		self.assertRaises(finergy.ValidationError, finergy.get_doc(test_bad_criteria[0]).insert)

	def test_formula_validate(self):
		delete_test_scorecards()
		self.assertRaises(finergy.ValidationError, finergy.get_doc(test_bad_criteria[1]).insert)
		self.assertRaises(finergy.ValidationError, finergy.get_doc(test_bad_criteria[2]).insert)


def delete_test_scorecards():
	# Delete all the periods so we can delete all the criteria
	finergy.db.sql("""delete from `tabSupplier Scorecard Period`""")
	finergy.db.sql(
	...
	for d in test_good_criteria:
		if finergy.db.exists("Supplier Scorecard Criteria", d.get("name")):
			# Delete all the periods, then delete the scorecard
			finergy.delete_doc(d.get("doctype"), d.get("name"))
