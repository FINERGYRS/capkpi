...
finergy.ui.form.on("Request for Quotation",{
	setup: function(frm) {
	...
	refresh: function(frm, cdt, cdn) {
		if (frm.doc.docstatus === 1) {
			...
			frm.add_custom_button(__("Send Emails to Suppliers"), function() {
				finergy.call({
					method: 'capkpi.buying.doctype.request_for_quotation.request_for_quotation.send_supplier_emails',
					freeze: true,
					args: {
			...
			frm.add_custom_button(__('Download PDF'), () => {
				...
				finergy.prompt(fields, data => {
					var child = locals[cdt][cdn]

					var w = window.open(
						finergy.urllib.get_full_url("/api/method/capkpi.buying.doctype.request_for_quotation.request_for_quotation.get_pdf?"
	...
	make_supplier_quotation: function(frm) {
		...
		var dialog = new finergy.ui.Dialog({
			...
			primary_action: (args) => {
				...
				return finergy.call({
					type: "GET",
					method: "capkpi.buying.doctype.request_for_quotation.request_for_quotation.make_supplier_quotation_from_rfq",
					args: {
...
finergy.ui.form.on("Request for Quotation Item", {
	items_add(frm, cdt, cdn) {
		if (frm.doc.schedule_date) {
			finergy.model.set_value(cdt, cdn, 'schedule_date', frm.doc.schedule_date);
		}
	}
});
finergy.ui.form.on("Request for Quotation Supplier",{
	supplier: function(frm, cdt, cdn) {
		...
		finergy.call({
			...
			callback: function(r){
				if(r.message){
					finergy.model.set_value(cdt, cdn, 'contact', r.message.contact_person)
					finergy.model.set_value(cdt, cdn, 'email_id', r.message.contact_email)
				}
			}
		})
	},
...
capkpi.buying.RequestforQuotationController = class RequestforQuotationController extends capkpi.buying.BuyingController {
	refresh() {
		var me = this;
		super.refresh();
		if (this.frm.doc.docstatus===0) {
			...
			this.frm.add_custom_button(__('Possible Supplier'), function() {
				...
				var dialog = new finergy.ui.Dialog({
					...
					primary_action: (args) => {
						...
						capkpi.utils.map_current_doc({
							method: "capkpi.buying.doctype.request_for_quotation.request_for_quotation.get_item_from_material_requests_based_on_supplier",
			...
			this.frm.add_custom_button(__('Link to Material Requests'),
				function() {
					capkpi.buying.link_to_mrs(me.frm);
				}, __("Tools"));
		...
		var dialog = new finergy.ui.Dialog({
			...
			fields: [
				{
					...
					onchange() {
						if(dialog.get_value('search_type') == 'Tag'){
							finergy.call({
								method: 'capkpi.buying.doctype.request_for_quotation.request_for_quotation.get_supplier_tag',
							}).then(r => {
			...
			primary_action : (args) => {
				...
				if (args.search_type === "Tag" && args.tag) {
					return finergy.call({
						type: "GET",
						method: "finergy.desk.doctype.tag.tag.get_tagged_docs",
						args: {
							"doctype": "Supplier",
							"tag": args.tag
						},
...
extend_cscript(cur_frm.cscript, new capkpi.buying.RequestforQuotationController({frm: cur_frm}));
