...
finergy.ui.form.on("Supplier", {
	setup: function (frm) {
		...
		frm.set_query("supplier_primary_contact", function(doc) {
			return {
				query: "capkpi.buying.doctype.supplier.supplier.get_supplier_primary_contact",
				filters: {
					"supplier": doc.name
				}
			};
	...
	refresh: function (frm) {
		finergy.dynamic_link = { doc: frm.doc, fieldname: 'name', doctype: 'Supplier' }

		if (finergy.defaults.get_default("supp_master_name") != "Naming Series") {
		...
		if (frm.doc.__islocal) {
			...
			frm.add_custom_button(__('Accounts Payable'), function () {
				finergy.set_route('query-report', 'Accounts Payable', { supplier: frm.doc.name });
			}, __("View"));

			frm.add_custom_button(__('Bank Account'), function () {
				capkpi.utils.make_bank_account(frm.doc.doctype, frm.doc.name);
			}, __('Create'));
