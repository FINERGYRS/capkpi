...
class Supplier(TransactionBase):
	...
	def autoname(self):
		supp_master_name = finergy.defaults.get_global_default("supp_master_name")
	...
	def validate(self):
		...
		if finergy.defaults.get_global_default("supp_master_name") == "Naming Series":
			if not self.naming_series:
	...
	def get_supplier_group_details(self):
		doc = finergy.get_doc("Supplier Group", self.supplier_group)
		self.payment_terms = ""
		self.accounts = []
	...
	def validate_internal_supplier(self):
		...
		internal_supplier = finergy.db.get_value(
			"Supplier",
			{
	...
	def create_primary_contact(self):
		from capkpi.selling.doctype.customer.customer import make_contact
	...
	def create_primary_address(self):
		...
		from capkpi.selling.doctype.customer.customer import make_address
	...
	def after_rename(self, olddn, newdn, merge=False):
		if finergy.defaults.get_global_default("supp_master_name") == "Supplier Name":
			finergy.db.set(self, "supplier_name", newdn)

	def create_onboarding_docs(self, args):
		company = finergy.defaults.get_defaults().get("company") or finergy.db.get_single_value(
			...
			if supplier:
				try:
					...
					if args.get("supplier_email_" + str(i)):
						from capkpi.selling.doctype.customer.customer import create_contact
