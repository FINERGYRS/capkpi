...
from finergy.utils import add_days, add_years, get_last_day, getdate, nowdate

from capkpi.buying.doctype.supplier_scorecard_period.supplier_scorecard_period import (
...
class SupplierScorecard(Document):
	...
	def calculate_total_score(self):
		...
		for scp in scorecards:
			my_sc = finergy.get_doc("Supplier Scorecard Period", scp.name)
		...
		for standing in self.standings:
			if (not standing.min_grade or (standing.min_grade <= self.supplier_score)) and (
				...
				for fieldname in ("prevent_pos", "prevent_rfqs", "warn_rfqs", "warn_pos"):
					...
					finergy.db.set_value("Supplier", self.supplier, fieldname, self.get(fieldname))


@finergy.whitelist()
def get_timeline_data(doctype, name):
	# Get a list of all the associated scorecards
	scs = finergy.get_doc(doctype, name)
	out = {}
	timeline_data = {}
	scorecards = finergy.db.sql(
		"""
	...
	for sc in scorecards:
		start_date, end_date, total_score = finergy.db.get_value(
...
def refresh_scorecards():
	...
	for sc in scorecards:
		...
		if make_all_scorecards(sc.name) > 0:
			...
			finergy.get_doc("Supplier Scorecard", sc.name).save()


@finergy.whitelist()
def make_all_scorecards(docname):

	sc = finergy.get_doc("Supplier Scorecard", docname)
	supplier = finergy.get_doc("Supplier", sc.supplier)
