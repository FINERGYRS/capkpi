...
class VariablePathNotFound(finergy.ValidationError):
	pass
	...
	def validate_path_exists(self):
		if "." in self.path:
			try:
				...
				finergy.throw(_("Could not find path for " + self.path), VariablePathNotFound)
...
def get_item_workdays(scorecard):
	"""Gets the number of days in this period"""
	supplier = finergy.get_doc("Supplier", scorecard.supplier)
	total_item_days = finergy.db.sql(
		"""
			SELECT
...
def get_cost_of_on_time_shipments(scorecard):
	...
	total_delivered_on_time_costs = finergy.db.sql(
		"""
			SELECT
...
def get_total_days_late(scorecard):
	...
	supplier = finergy.get_doc("Supplier", scorecard.supplier)
	total_delivered_late_days = finergy.db.sql(
		"""
			SELECT
	...
	total_missed_late_days = finergy.db.sql(
		"""
			SELECT
...
def get_on_time_shipments(scorecard):
	...
	# Look up all PO Items with delivery dates between our dates
	total_items_delivered_on_time = finergy.db.sql(
		"""
			SELECT
				COUNT(pr_item.qty)
			FROM
