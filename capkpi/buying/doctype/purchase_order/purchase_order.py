		...
		if self.is_subcontracted and not self.is_old_subcontracting_flow:
			for item in self.items:
				if not item.fg_item:
					...
					if not finergy.get_value("Item", item.fg_item, "is_sub_contracted_item"):
						finergy.throw(
							_(
								"Row #{0}: Finished Good Item {1} must be a sub-contracted item for service item {2}"
							).format(item.idx, item.fg_item, item.item_code)
						)
					elif not finergy.get_value("Item", item.fg_item, "default_bom"):
						finergy.throw(
...
def get_mapped_subcontracting_order(source_name, target_doc=None):
	...
	if target_doc.set_warehouse:
		...
		source_doc = finergy.get_doc("Purchase Order", source_name)
