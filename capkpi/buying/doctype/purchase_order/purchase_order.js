...
finergy.ui.form.on("Purchase Order", {
	setup: function(frm) {
	...
	get_materials_from_supplier: function(frm) {
		...
		if (po_details && po_details.length) {
			frm.add_custom_button(__('Return of Components'), () => {
				frm.call({
					method: 'capkpi.controllers.subcontracting_controller.get_materials_from_supplier',
					freeze: true,
					...
					callback: function(r) {
						if (r && r.message) {
							const doc = finergy.model.sync(r.message);
	...
	onload: function(frm) {
		...
		if (!frm.doc.transaction_date){
			frm.set_value('transaction_date', finergy.datetime.get_today())
		}
...
finergy.ui.form.on("Purchase Order Item", {
	schedule_date: function(frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if (row.schedule_date) {
			if(!frm.doc.schedule_date) {
				capkpi.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "schedule_date");
			} else {
				set_schedule_date(frm);
			}
		}
...
capkpi.buying.PurchaseOrderController = class PurchaseOrderController extends capkpi.buying.BuyingController {
	setup() {
				...
				if(this.frm.doc.status !== 'Closed' && flt(this.frm.doc.per_received) < 100 && flt(this.frm.doc.per_billed) < 100) {
					...
					if (!(this.frm.doc.is_subcontracted && !this.frm.doc.is_old_subcontracting_flow)) {
						this.frm.add_custom_button(__('Update Items'), () => {
							capkpi.utils.update_child_items({
								frm: this.frm,
								child_docname: "items",
	...
	get_items_from_open_material_requests() {
		capkpi.utils.map_current_doc({
			method: "capkpi.stock.doctype.material_request.material_request.make_purchase_order_based_on_supplier",
			args: {
			...
			get_query_method: "capkpi.stock.doctype.material_request.material_request.get_material_requests_based_on_supplier"
		});
	}
			...
			if(me.values) {
				me.values.sub_con_rm_items.map((row,i) => {
					if (!row.item_code || !row.rm_item_code || !row.warehouse || !row.qty || row.qty === 0) {
						let row_id = i+1;
						finergy.throw(__("Item Code, warehouse and quantity are required on row {0}", [row_id]));
					}
				})
	...
	_make_rm_stock_entry(rm_items) {
		finergy.call({
			method:"capkpi.controllers.subcontracting_controller.make_rm_stock_entry",
			args: {
	...
	make_inter_company_order(frm) {
		finergy.model.open_mapped_doc({
			method: "capkpi.buying.doctype.purchase_order.purchase_order.make_inter_company_sales_order",
			frm: frm
		});
	}
	...
	make_purchase_invoice() {
		finergy.model.open_mapped_doc({
			method: "capkpi.buying.doctype.purchase_order.purchase_order.make_purchase_invoice",
			frm: cur_frm
		})
	}
	...
	make_subcontracting_order() {
		finergy.model.open_mapped_doc({
			method: "capkpi.buying.doctype.purchase_order.purchase_order.make_subcontracting_order",
			frm: cur_frm,
	...
	add_from_mappers() {
		...
		this.frm.add_custom_button(__('Material Request'),
			function() {
				capkpi.utils.map_current_doc({
					method: "capkpi.stock.doctype.material_request.material_request.make_purchase_order",
					source_doctype: "Material Request",
					target: me.frm,
					setters: {
		...
		this.frm.add_custom_button(__('Supplier Quotation'),
			function() {
				capkpi.utils.map_current_doc({
					method: "capkpi.buying.doctype.supplier_quotation.supplier_quotation.make_purchase_order",
					...
					while (i < item_length) {
						...
						(r.message[0] || []).forEach(function(d) {
							if (d.qty > 0 && qty > 0 && me.frm.doc.items[i].item_code == d.item_code && !me.frm.doc.items[i].material_request_item)
							{
								...
								finergy.msgprint("Assigning " + d.mr_name + " to " + d.item_code + " (row " + me.frm.doc.items[i].idx + ")");
								if (qty > 0) {
									finergy.msgprint("Splitting " + qty + " units of " + d.item_code);
									var new_row = finergy.model.add_child(me.frm.doc, me.frm.doc.items[i].doctype, "items");
									item_length++;
	...
	hold_purchase_order(){
		...
		var d = new finergy.ui.Dialog({
			...
			primary_action: function() {
				...
				finergy.call({
					method: "finergy.desk.form.utils.add_comment",
					args: {
...
extend_cscript(cur_frm.cscript, new capkpi.buying.PurchaseOrderController({frm: cur_frm}));

cur_frm.cscript.update_status= function(label, status){
	finergy.call({
		method: "capkpi.buying.doctype.purchase_order.purchase_order.update_status",
...
function set_schedule_date(frm) {
	if(frm.doc.schedule_date){
		capkpi.utils.copy_value_in_all_rows(frm.doc, frm.doc.doctype, frm.doc.name, "items", "schedule_date");
	}
}

finergy.provide("capkpi.buying");

finergy.ui.form.on("Purchase Order", "is_subcontracted", function(frm) {
	if (frm.doc.is_old_subcontracting_flow) {
