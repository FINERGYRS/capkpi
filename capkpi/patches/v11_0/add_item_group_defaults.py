...
def execute():
	...
	finergy.reload_doc("setup", "doctype", "item_group")

	companies = finergy.get_all("Company")
	item_groups = finergy.db.sql(
	...
	if len(companies) == 1:
		for item_group in item_groups:
			doc = finergy.get_doc("Item Group", item_group.get("name"))
			item_group_defaults = []
		...
		for item_group in item_groups:
			...
			for d in [
				...
				if item_group.get(d[0]):
					company = finergy.get_value(d[1], item_group.get(d[0]), "company", cache=True)
