...
def execute():
	finergy.reload_doc("Payroll", "doctype", "Salary Structure")
	finergy.reload_doc("Payroll", "doctype", "Salary Structure Assignment")
	finergy.db.sql(
		"""
	...
	if finergy.db.table_exists("Salary Structure Employee"):
		ss_details = finergy.db.sql(
			"""
		...
		cols = ""
		if "base" in finergy.db.get_table_columns("Salary Structure"):
			cols = ", base, variable"
	...
	all_companies = finergy.db.get_all("Company", fields=["name", "default_currency"])
	...
	for d in ss_details:
		try:
			...
			if joining_date and getdate(from_date) < joining_date:
				...
				continue
			company_currency = finergy.db.get_value("Company", d.company, "default_currency")

			s = finergy.new_doc("Salary Structure Assignment")
	...
	finergy.db.sql("update `tabSalary Structure` set docstatus=1")
