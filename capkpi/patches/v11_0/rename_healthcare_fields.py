...
def execute():
	for dt, field_list in field_rename_map.items():
		if finergy.db.exists("DocType", dt):
			finergy.reload_doc(get_doctype_module(dt), "doctype", scrub(dt))
			for field in field_list:
				if finergy.db.has_column(dt, field[0]):
					rename_field(dt, field[0], field[1])

	if finergy.db.exists("DocType", "Lab Prescription"):
		if finergy.db.has_column("Lab Prescription", "parentfield"):
			finergy.db.sql(
				"""
	...
	if finergy.db.exists("DocType", "Lab Test Groups"):
		if finergy.db.has_column("Lab Test Groups", "parentfield"):
			finergy.db.sql(
				"""
