...
def execute():
	finergy.reload_doc("assets", "doctype", "asset_finance_book")
	finergy.reload_doc("assets", "doctype", "depreciation_schedule")
	finergy.reload_doc("assets", "doctype", "asset_category")
	finergy.reload_doc("assets", "doctype", "asset")
	finergy.reload_doc("assets", "doctype", "asset_movement")
	finergy.reload_doc("assets", "doctype", "asset_category_account")

	if finergy.db.has_column("Asset", "warehouse"):
		finergy.db.sql(
			""" update `tabAsset` ast, `tabWarehouse` wh
			set ast.location = wh.warehouse_name where ast.warehouse = wh.name"""
		)

		for d in finergy.get_all("Asset"):
			doc = finergy.get_doc("Asset", d.name)
		...
		for category in finergy.get_all("Asset Category"):
			asset_category_doc = finergy.get_doc("Asset Category", category)
