...
def execute():
	...
	for doctype in ["department", "leave_period", "staffing_plan", "job_opening"]:
		finergy.reload_doc("hr", "doctype", doctype)
	finergy.reload_doc("Payroll", "doctype", "payroll_entry")

	companies = finergy.db.get_all("Company", fields=["name", "abbr"])
	departments = finergy.db.get_all("Department")
	comp_dict = {}
	...
	for department in departments:
		...
		for company in companies:
			copy_doc = finergy.copy_doc(department_doc)
...
def update_instructors(comp_dict):
	when_then = []
	emp_details = finergy.get_all("Employee", fields=["name", "company"])
