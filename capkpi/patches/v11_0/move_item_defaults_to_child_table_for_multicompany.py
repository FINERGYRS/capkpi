...
def execute():
	...
	"""
	if not finergy.db.has_column("Item", "default_warehouse"):
		return
	...
	if len(companies) == 1 and not finergy.get_all("Item Default", limit=1):
		try:
			finergy.db.sql(
				"""
		...
		for item_data in item_details:
			for d in [
				...
				if item_data.get(d[0]):
					company = finergy.get_value(d[1], item_data.get(d[0]), "company", cache=True)
