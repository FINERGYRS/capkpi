...
def execute():
	...
	if not company:
		return
	finergy.reload_doc("Payroll", "doctype", "Employee Tax Exemption Declaration")
	finergy.reload_doc("Payroll", "doctype", "Employee Tax Exemption Proof Submission")
	finergy.reload_doc("hr", "doctype", "Employee Grade")
	finergy.reload_doc("hr", "doctype", "Leave Policy")

	finergy.reload_doc("accounts", "doctype", "Bank Account")
	finergy.reload_doc("accounts", "doctype", "Tax Withholding Category")
	finergy.reload_doc("accounts", "doctype", "Allowed To Transact With")
	finergy.reload_doc("accounts", "doctype", "Finance Book")
	finergy.reload_doc("accounts", "doctype", "Loyalty Program")

	finergy.reload_doc("stock", "doctype", "Item Barcode")

	make_custom_fields()

	finergy.reload_doc("accounts", "doctype", "sales_taxes_and_charges")
	finergy.reload_doc("accounts", "doctype", "purchase_taxes_and_charges")
	finergy.reload_doc("accounts", "doctype", "sales_taxes_and_charges_template")
	finergy.reload_doc("accounts", "doctype", "purchase_taxes_and_charges_template")

	# set is_inter_state in Taxes And Charges Templates
	if finergy.db.has_column(
		"Sales Taxes and Charges Template", "is_inter_state"
	) and finergy.db.has_column("Purchase Taxes and Charges Template", "is_inter_state"):
