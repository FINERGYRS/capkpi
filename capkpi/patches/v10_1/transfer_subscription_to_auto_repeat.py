...
def execute():
	finergy.reload_doc("automation", "doctype", "auto_repeat")
	...
	for module, doctypes in doctypes_to_rename.items():
		for doctype in doctypes:
			...
			if finergy.db.has_column(doctype, "subscription"):
				rename_field(doctype, "subscription", "auto_repeat")

	subscriptions = finergy.db.sql("select * from `tabSubscription`", as_dict=1)

	for doc in subscriptions:
		doc["doctype"] = "Auto Repeat"
		auto_repeat = finergy.get_doc(doc)
		auto_repeat.db_insert()

	finergy.db.sql("delete from `tabSubscription`")
	finergy.db.commit()
...
def drop_columns_from_subscription():
	...
	for field in [
		...
		if field in finergy.db.get_table_columns("Subscription"):
			fields_to_drop["Subscription"].append(field)

	finergy.model.delete_fields(fields_to_drop, delete=1)
