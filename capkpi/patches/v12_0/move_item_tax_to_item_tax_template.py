...
def execute():
	if "tax_type" not in finergy.db.get_table_columns("Item Tax"):
		return
	old_item_taxes = {}
	item_tax_templates = {}

	finergy.reload_doc("accounts", "doctype", "item_tax_template_detail", force=1)
	finergy.reload_doc("accounts", "doctype", "item_tax_template", force=1)
	existing_templates = finergy.db.sql(
	...
	finergy.reload_doc("stock", "doctype", "item", force=1)
	finergy.reload_doc("stock", "doctype", "item_tax", force=1)
	finergy.reload_doc("selling", "doctype", "quotation_item", force=1)
	finergy.reload_doc("selling", "doctype", "sales_order_item", force=1)
	finergy.reload_doc("stock", "doctype", "delivery_note_item", force=1)
	finergy.reload_doc("accounts", "doctype", "sales_invoice_item", force=1)
	finergy.reload_doc("buying", "doctype", "supplier_quotation_item", force=1)
	finergy.reload_doc("buying", "doctype", "purchase_order_item", force=1)
	finergy.reload_doc("stock", "doctype", "purchase_receipt_item", force=1)
	finergy.reload_doc("accounts", "doctype", "purchase_invoice_item", force=1)
	finergy.reload_doc("accounts", "doctype", "accounts_settings", force=1)
	...
	for item_code in old_item_taxes.keys():
		...
		finergy.db.sql("delete from `tabItem Tax` where parent=%s and parenttype='Item'", item_code)
	...
	for dt in doctypes:
		for d in finergy.db.sql(
			...
			finergy.db.set_value(dt + " Item", d.name, "item_tax_template", item_tax_template_name)
...
def get_item_tax_template(
	...
	item_tax_template = finergy.new_doc("Item Tax Template")
	...
	for tax_type, tax_rate in iteritems(item_tax_map):
		...
		if account_details:
			...
			if account_details.account_type not in (
				...
				finergy.db.set_value("Account", account_details.name, "account_type", "Chargeable")
		else:
			...
			if not account_name:
				...
				parent_account = finergy.get_value(
				...
				tax_type = finergy.db.get_value("Account", filters)
				if not tax_type:
					account = finergy.new_doc("Account")
					account.update(filters)
					try:
						account.insert()
						tax_type = account.name
					except finergy.DuplicateEntryError:
						tax_type = finergy.db.get_value(
							"Account", {"account_name": account_name, "company": company}, "name"
						)

		account_type = finergy.get_cached_value("Account", tax_type, "account_type")
...
def get_company(company_abbr, parenttype=None, parent=None):
	if parenttype and parent:
		company = finergy.get_cached_value(parenttype, parent, "company")
	else:
		company = finergy.db.get_value("Company", filters={"abbr": company_abbr})
