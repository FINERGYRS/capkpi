...
def execute():
	finergy.reload_doc("support", "doctype", "issue_priority")
	finergy.reload_doc("support", "doctype", "service_level_priority")
...
def set_issue_priority():
	...
	for priority in finergy.get_meta("Issue").get_field("priority").options.split("\n"):
		if priority and not finergy.db.exists("Issue Priority", priority):
			finergy.get_doc({"doctype": "Issue Priority", "name": priority}).insert(ignore_permissions=True)


def set_priority_for_issue():
	# Sets priority for Issues as Select field is changed to Link field.
	issue_priority = finergy.get_list("Issue", fields=["name", "priority"])
	finergy.reload_doc("support", "doctype", "issue")

	for issue in issue_priority:
		finergy.db.set_value("Issue", issue.name, "priority", issue.priority)
...
def set_priorities_service_level():
	...
	try:
		service_level_priorities = finergy.get_list(
			"Service Level",
			fields=[
				"name",
				"priority",
		...
		finergy.reload_doc("support", "doctype", "service_level")
		finergy.reload_doc("support", "doctype", "support_settings")
		finergy.db.set_value("Support Settings", None, "track_service_level_agreement", 1)

		for service_level in service_level_priorities:
			if service_level:
				doc = finergy.get_doc("Service Level", service_level.name)
...
def set_priorities_service_level_agreement():
	...
	try:
		service_level_agreement_priorities = finergy.get_list(
