...
def execute():
	for dn in field_rename_map:
		if finergy.db.exists("DocType", dn):
			if dn == "Healthcare Settings":
				finergy.reload_doctype("Healthcare Settings")
			else:
				finergy.reload_doc(get_doctype_module(dn), "doctype", scrub(dn))
	...
	for dt, field_list in field_rename_map.items():
		if finergy.db.exists("DocType", dt):
			for field in field_list:
				...
				elif finergy.db.has_column(dt, field[0]):
					rename_field(dt, field[0], field[1])

	# first name mandatory in Patient
	if finergy.db.exists("DocType", "Patient"):
		patients = finergy.db.sql("select name, patient_name from `tabPatient`", as_dict=1)
		finergy.reload_doc("healthcare", "doctype", "patient")
		for entry in patients:
			name = entry.patient_name.split(" ")
			finergy.db.set_value("Patient", entry.name, "first_name", name[0])

	# mark Healthcare Practitioner status as Disabled
	if finergy.db.exists("DocType", "Healthcare Practitioner"):
		practitioners = finergy.db.sql(
	...
	if finergy.db.exists("DocType", "Clinical Procedure"):
		finergy.reload_doc("healthcare", "doctype", "clinical_procedure")
		finergy.db.sql(
			"""
			UPDATE
	...
	if finergy.db.exists("DocType", "Patient Encounter"):
		field_list = [["visit_department", "medical_department"], ["type", "appointment_type"]]
		encounter_details = finergy.db.sql(
			"""select symptoms, diagnosis, name from `tabPatient Encounter`""", as_dict=True
		)
		finergy.reload_doc("healthcare", "doctype", "patient_encounter")
		finergy.reload_doc("healthcare", "doctype", "patient_encounter_symptom")
		finergy.reload_doc("healthcare", "doctype", "patient_encounter_diagnosis")

		for field in field_list:
			if finergy.db.has_column(dt, field[0]):
				rename_field(dt, field[0], field[1])

		for entry in encounter_details:
			doc = finergy.get_doc("Patient Encounter", entry.name)
			symptoms = entry.symptoms.split("\n") if entry.symptoms else []
			for symptom in symptoms:
				if not finergy.db.exists("Complaint", symptom):
					finergy.get_doc({"doctype": "Complaint", "complaints": symptom}).insert()
				row = doc.append("symptoms", {"complaint": symptom})
				row.db_update()

			diagnosis = entry.diagnosis.split("\n") if entry.diagnosis else []
			for d in diagnosis:
				if not finergy.db.exists("Diagnosis", d):
					finergy.get_doc({"doctype": "Diagnosis", "diagnosis": d}).insert()
				row = doc.append("diagnosis", {"diagnosis": d})
				row.db_update()
			doc.db_update()

	if finergy.db.exists("DocType", "Fee Validity"):
