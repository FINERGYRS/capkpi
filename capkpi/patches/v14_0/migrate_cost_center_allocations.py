...
def get_existing_cost_center_allocations():
	if not finergy.db.exists("DocType", "Distributed Cost Center"):
		return

	par = finergy.qb.DocType("Cost Center")
	child = finergy.qb.DocType("Distributed Cost Center")
	...
	cc_allocations = finergy._dict()
	for d in records:
		cc_allocations.setdefault(d.name, finergy._dict()).setdefault(
