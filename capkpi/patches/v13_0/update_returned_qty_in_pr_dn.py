...
def execute():
	...
	finergy.reload_doc("stock", "doctype", "purchase_receipt_item")
	finergy.reload_doc("stock", "doctype", "delivery_note")
	finergy.reload_doc("stock", "doctype", "delivery_note_item")
	finergy.reload_doc("stock", "doctype", "stock_settings")

	def update_from_return_docs(doctype):
		for return_doc in finergy.get_all(
			doctype, filters={"is_return": 1, "docstatus": 1, "return_against": ("!=", "")}
		):
			# Update original receipt/delivery document from return
			return_doc = finergy.get_cached_doc(doctype, return_doc.name)
			try:
			...
			return_against = finergy.get_doc(doctype, return_doc.return_against)
