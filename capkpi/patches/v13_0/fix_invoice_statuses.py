...
def execute():
	...
	if not finergy.db.exists("DocType", "Party Specific Item"):
		return
	...
	for doctype in ("Purchase Invoice", "Sales Invoice"):
		...
		invoices_to_update = finergy.get_all(
			doctype,
			fields=fields,
			filters={
				"docstatus": 1,
		...
		payment_schedule_items = finergy.get_all(
			"Payment Schedule",
		...
		for status, docs in status_map.items():
			finergy.db.set_value(doctype, {"name": ("in", docs)}, "status", status, update_modified=False)
