...
def execute():
	if not (
		finergy.db.table_exists("Payroll Period") and finergy.db.table_exists("Taxable Salary Slab")
	):
		return
	...
	standard_tax_exemption_amount_exists = finergy.db.has_column(
	...
	for company in finergy.get_all("Company"):
		payroll_periods = finergy.db.sql(
			"""
			SELECT
				{0}
			FROM
	...
	if not finergy.db.table_exists("Employee Tax Exemption Proof Submission"):
		return

	migrated = []
	proofs = finergy.get_all(
	...
	for proof in proofs:
		if proof.income_from_other_sources:
			employee_other_income = finergy.new_doc("Employee Other Income")
	...
	if not finergy.db.table_exists("Employee Tax Exemption Declaration"):
		return

	declerations = finergy.get_all(
