...
def execute():
	finergy.reload_doc("accounts", "doctype", "pos_payment_method")
	pos_profiles = finergy.get_all("POS Profile")

	for pos_profile in pos_profiles:
		payments = finergy.db.sql(
			"""
		...
		if payments:
			for payment_mode in payments:
				pos_payment_method = finergy.new_doc("POS Payment Method")
		...
		finergy.db.sql("""delete from `tabSales Invoice Payment` where parent=%s""", pos_profile.name)
