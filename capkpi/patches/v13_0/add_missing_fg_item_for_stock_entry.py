...
def execute():
	if not finergy.db.has_column("Work Order", "has_batch_no"):
		return

	finergy.reload_doc("manufacturing", "doctype", "manufacturing_settings")
	if cint(
	...
	work_orders = [d.name for d in finergy.get_all("Work Order", filters=filters, fields=fields)]
	...
	stock_entries = finergy.db.sql_list(
		"""
		SELECT
			se.name
		FROM
	...
	for stock_entry in stock_entries:
		doc = finergy.get_doc("Stock Entry", stock_entry)
...
def set_expense_account(doc):
	for row in doc.items:
		if row.is_finished_item and not row.expense_account:
			row.expense_account = finergy.get_cached_value(
