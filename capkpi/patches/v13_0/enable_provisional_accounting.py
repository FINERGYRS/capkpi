...
def execute():
	if not finergy.get_meta("Company").has_field("enable_perpetual_inventory_for_non_stock_items"):
		return

	finergy.reload_doc("setup", "doctype", "company")

	company = finergy.qb.DocType("Company")

	finergy.qb.update(company).set(
