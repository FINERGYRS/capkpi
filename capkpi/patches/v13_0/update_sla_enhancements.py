...
def execute():
	...
	if finergy.db.exists("DocType", "Service Level Agreement"):
		sla_details = finergy.db.get_all("Service Level Agreement", fields=["name", "service_level"])
		priorities = finergy.db.get_all(
		...
		finergy.reload_doc("support", "doctype", "pause_sla_on_status")
		finergy.reload_doc("support", "doctype", "service_level_priority")
		finergy.reload_doc("support", "doctype", "service_day")

		for entry in sla_details:
			values = finergy.db.get_value(
		...
		if finergy.db.exists("DocType", "Service Level"):
			service_levels = finergy.db.get_all(
				"Service Level", filters={"service_level": ("not in", sl)}, fields=["*"]
			)
			for entry in service_levels:
				sla = finergy.new_doc("Service Level Agreement")
				...
				if priority_list:
					sla = finergy.get_doc("Service Level Agreement", sla.name)
	...
	finergy.delete_doc_if_exists("DocType", "Service Level")
