...
def execute():
	...
	if finergy.db.exists("DocType", "Sales Invoice"):
		finergy.reload_doc("accounts", "doctype", "sales_invoice", force=True)

		# rename_field method assumes that the field already exists or the doc is synced
		if not finergy.db.has_column("Sales Invoice", "ksa_einv_qr"):
		...
		if finergy.db.has_column("Sales Invoice", "qr_code"):
			rename_field("Sales Invoice", "qr_code", "ksa_einv_qr")
			finergy.delete_doc_if_exists("Custom Field", "Sales Invoice-qr_code")
