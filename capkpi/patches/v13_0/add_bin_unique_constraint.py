...
def delete_broken_bins():
	# delete useless bins
	finergy.db.sql("delete from `tabBin` where item_code is null or warehouse is null")


def delete_and_patch_duplicate_bins():

	duplicate_bins = finergy.db.sql(
		"""
		SELECT
	...
	for duplicate_bin in duplicate_bins:
		...
		existing_bins = finergy.get_list(
			"Bin",
		...
		for broken_bin in existing_bins:
			finergy.delete_doc("Bin", broken_bin.name)
