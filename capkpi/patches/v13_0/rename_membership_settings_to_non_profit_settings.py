...
def execute():
	if finergy.db.exists("DocType", "Membership Settings"):
		finergy.rename_doc("DocType", "Membership Settings", "Non Profit Settings")
		finergy.reload_doctype("Non Profit Settings", force=True)

	if finergy.db.exists("DocType", "Non Profit Settings"):
		rename_fields_map = {
		...
		finergy.delete_doc_if_exists("DocType", "Membership Settings")
