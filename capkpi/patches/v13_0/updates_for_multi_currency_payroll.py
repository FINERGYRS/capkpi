...
def execute():
	...
	finergy.reload_doc("Accounts", "doctype", "Salary Component Account")
	if finergy.db.has_column("Salary Component Account", "default_account"):
	...
	for item in doctype_list:
		finergy.reload_doc(item["module"], "doctype", item["doctype"])
	...
	finergy.db.sql("update `tabEmployee Advance` set exchange_rate=1")

	# get all companies and it's currency
	all_companies = finergy.db.get_all(
	...
	for d in all_companies:
		...
		if not default_payroll_payable_account:
			default_payroll_payable_account = finergy.db.get_value(
				"Account",
				{
