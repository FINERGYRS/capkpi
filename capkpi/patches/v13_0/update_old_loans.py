...
def execute():
	...
	finergy.reload_doc("loan_management", "doctype", "repayment_schedule")
	finergy.reload_doc("loan_management", "doctype", "process_loan_interest_accrual")
	finergy.reload_doc("loan_management", "doctype", "loan_repayment")
	finergy.reload_doc("loan_management", "doctype", "loan_repayment_detail")
	finergy.reload_doc("loan_management", "doctype", "loan_interest_accrual")
	finergy.reload_doc("accounts", "doctype", "gl_entry")
	finergy.reload_doc("accounts", "doctype", "journal_entry_account")

	updated_loan_types = []
	loans_to_close = []

	# Update old loan status as closed
	if finergy.db.has_column("Repayment Schedule", "paid"):
		loans_list = finergy.db.sql(
	...
	for loan in loans:
		...
		loan_type_company = finergy.db.get_value("Loan Type", loan.loan_type, "company")
		loan_type = loan.loan_type

		group_income_account = finergy.get_value(
			"Account",
			{
		...
		if loan_type_company and loan_type_company != loan.company:
			...
			loan_type_name = finergy.get_value(
				"Loan Type",
				{
					"company": loan.company,
			...
			loan_type_doc = finergy.get_doc("Loan Type", loan.loan_type)
		...
		if loan_type in updated_loan_types:
			...
			if finergy.db.has_column("Repayment Schedule", "paid"):
				total_principal, total_interest = finergy.db.get_value(
					"Repayment Schedule",
...
def create_loan_type(loan, loan_type_name, penalty_account):
	loan_type_doc = finergy.new_doc("Loan Type")
