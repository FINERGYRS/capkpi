...
def execute():
	finergy.reload_doc("e_commerce", "doctype", "website_item")
	finergy.reload_doc("e_commerce", "doctype", "website_item_tabbed_section")
	finergy.reload_doc("e_commerce", "doctype", "website_offer")
	finergy.reload_doc("e_commerce", "doctype", "recommended_items")
	finergy.reload_doc("e_commerce", "doctype", "e_commerce_settings")
	...
	item_table_fields = finergy.db.sql("desc `tabItem`", as_dict=1)  # nosemgrep
	...
	items = finergy.db.get_all("Item", fields=item_fields, or_filters=or_filters)
	total_count = len(items)

	for count, item in enumerate(items, start=1):
		if finergy.db.exists("Website Item", {"item_code": item.item_code}):
			continue
		...
		finergy.utils.update_progress_bar("Creating Website Items", count, total_count)
