...
def execute():
	...
	"""
	finergy.reload_doc("e_commerce", "web_template", "item_card_group")

	blocks = finergy.db.get_all(
		"Web Page Block",
	...
	for block in blocks:
		...
		for field in fields:
			...
			if finergy.db.exists("Website Item", {"item_code": item}):
				website_item = finergy.db.get_value("Website Item", {"item_code": item})
			else:
