...
def execute():
	...
	def move_table_multiselect_data(docfield):
		...
		for row in table_multiselect_data:
			...
			web_item = finergy.db.get_value("Website Item", {"item_code": row.parent})
			web_item_doc = finergy.get_doc("Website Item", web_item)

			child_doc = finergy.new_doc(docfield.options, web_item_doc, field)
