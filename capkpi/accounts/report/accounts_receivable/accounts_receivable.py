# Copyright (c) 2015, Finergy Reporting Solutions SAS
...
import finergy
from finergy import _, qb, scrub
from finergy.query_builder import Criterion
from finergy.query_builder.functions import Date
from finergy.utils import cint, cstr, flt, getdate, nowdate

from capkpi.accounts.doctype.accounting_dimension.accounting_dimension import (
	get_accounting_dimensions,
	get_dimension_with_children,
)
from capkpi.accounts.utils import get_currency_precision
...
class ReceivablePayableReport(object):
	...
	def run(self, args):
		...
		self.set_defaults()
		self.party_naming_by = finergy.db.get_value(
	...
	def set_defaults(self):
		if not self.filters.get("company"):
			self.filters.company = finergy.db.get_single_value("Global Defaults", "default_company")
		self.company_currency = finergy.get_cached_value(
	...
	def init_voucher_balance(self):
		...
		for ple in self.ple_entries:
			...
			if not key in self.voucher_balance:
				self.voucher_balance[key] = finergy._dict(
	...
	def build_delivery_note_map(self):
		if self.invoices and self.filters.show_delivery_notes:
			self.delivery_notes = finergy._dict()

			# delivery note link inside sales invoice
			si_against_dn = finergy.db.sql(
				"""
	...
	def get_invoice_details(self):
		self.invoice_details = finergy._dict()
		if self.party_type == "Customer":
			si_list = finergy.db.sql(
				"""
	...
	def get_payment_terms(self, row):
		...
		payment_terms_details = finergy.db.sql(
			"""
			select
		...
		original_row = finergy._dict(row)
		row.payment_terms = []
		...
		for d in payment_terms_details:
			term = finergy._dict(original_row)
		...
		for key in ("paid", "credit_note"):
			if row[key] > 0:
				if not additional_row:
					additional_row = finergy._dict(row)
	...
	def get_future_payments(self):
		if self.filters.show_future_payments:
			self.future_payments = finergy._dict()
	...
	def get_return_entries(self):
		...
		self.return_entries = finergy._dict(
			finergy.get_all(doctype, filters, ["name", "return_against"], as_list=1)
		)
	...
	def get_sales_invoices_or_customers_based_on_sales_person(self):
		if self.filters.get("sales_person"):
			lft, rgt = finergy.db.get_value("Sales Person", self.filters.get("sales_person"), ["lft", "rgt"])
			...
			self.sales_person_records = finergy._dict()
			for d in records:
	...
	def get_cost_center_conditions(self, conditions):
		lft, rgt = finergy.db.get_value("Cost Center", self.filters.cost_center, ["lft", "rgt"])
		cost_center_list = [
			center.name
			for center in finergy.get_list("Cost Center", filters={"lft": (">=", lft), "rgt": ("<=", rgt)})
		]
	...
	def get_hierarchical_filters(self, doctype, key):
		lft, rgt = finergy.db.get_value(doctype, self.filters.get(key), ["lft", "rgt"])
	...
	def get_party_details(self, party):
		if not party in self.party_details:
			if self.party_type == "Customer":
				self.party_details[party] = finergy.db.get_value(
					"Customer",
					party,
