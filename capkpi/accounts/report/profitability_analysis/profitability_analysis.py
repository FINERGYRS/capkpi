...
def get_accounts_data(based_on, company):
	if based_on == "cost_center":
		...
		return finergy.get_all("Project", fields=["name"], filters={"company": company}, order_by="name")
	else:
		filters = {}
		doctype = finergy.unscrub(based_on)
		has_company = finergy.db.has_column(doctype, "company")

		if has_company:
			filters.update({"company": company})

		return finergy.get_all(doctype, fields=["name"], filters=filters, order_by="name")
...
def prepare_data(accounts, filters, total_row, parent_children_map, based_on):
	...
	for d in accounts:
		...
		if based_on == "cost_center":
			cost_center_doc = finergy.get_doc("Cost Center", d.name)
			if not cost_center_doc.enable_distributed_cost_center:
				DCC_allocation = finergy.db.sql(
