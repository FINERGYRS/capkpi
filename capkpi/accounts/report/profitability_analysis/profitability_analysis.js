...
finergy.require("assets/capkpi/js/financial_statements.js", function() {
	finergy.query_reports["Profitability Analysis"] = {
		"filters": [
			{
				"fieldname": "company",
				"label": __("Company"),
		...
		"formatter": function(value, row, column, data, default_formatter) {
			if (column.fieldname=="account") {
				...
				column.link_onclick =
					"finergy.query_reports['Profitability Analysis'].open_profit_and_loss_statement(" + JSON.stringify(data) + ")";
				column.is_tree = true;
			}
		...
		"open_profit_and_loss_statement": function(data) {
			...
			if(data.based_on == 'cost_center'){
				finergy.route_options["cost_center"] = data.account
			} else {
				finergy.route_options["project"] = data.account
			}

			finergy.set_route("query-report", "Profit and Loss Statement");
		},
		"tree": true,
		"name_field": "account",
	...
	capkpi.dimension_filters.forEach((dimension) => {
		finergy.query_reports["Profitability Analysis"].filters[1].options.push(dimension["document_type"]);
	});
