...
import capkpi
from capkpi.accounts.report.balance_sheet.balance_sheet import (
	get_chart_data,
...
from capkpi.accounts.report.cash_flow.cash_flow import (
	add_total_row_account,
	get_account_type_based_gl_data,
	get_cash_flow_accounts,
)
from capkpi.accounts.report.cash_flow.cash_flow import get_report_summary as get_cash_flow_summary
from capkpi.accounts.report.financial_statements import (
	filter_out_zero_value_rows,
	get_fiscal_year_data,
	sort_accounts,
)
from capkpi.accounts.report.profit_and_loss_statement.profit_and_loss_statement import (
...
from capkpi.accounts.report.utils import convert, convert_to_presentation_currency
...
def execute(filters=None):
	...
	if filters.get("report") == "Balance Sheet":
		...
		if cint(finergy.db.get_single_value("Accounts Settings", "use_custom_cash_flow")):
			from capkpi.accounts.report.cash_flow.custom_cash_flow import execute as execute_custom
...
def get_company_currency(filters=None):
	return filters.get("presentation_currency") or finergy.get_cached_value(
			...
			if d:
				...
				for company in companies:
					...
					if (
						...
						parent_company_currency = capkpi.get_company_currency(d.company)
						child_company_currency = capkpi.get_company_currency(entry.company)
...
def get_subsidiary_companies(company):
	lft, rgt = finergy.get_cached_value("Company", company, ["lft", "rgt"])
...
def get_accounts(root_type, companies):
	...
	for company in companies:
		for account in finergy.get_all(
			"Account",
			fields=[
				"name",
				"is_group",
...
def set_gl_entries_by_account(
	...
	company_lft, company_rgt = finergy.get_cached_value(
		"Company", filters.get("company"), ["lft", "rgt"]
	)

	additional_conditions = get_additional_conditions(from_date, ignore_closing_entries, filters)
	companies = finergy.db.sql(
	...
	for d in companies:
		gl_entries = finergy.db.sql(
			...
			),
			{
				...
				"company_fb": finergy.db.get_value("Company", d.name, "default_finance_book"),
			},
			as_dict=True,
		)
