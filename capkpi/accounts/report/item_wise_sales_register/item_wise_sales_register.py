...
from capkpi.accounts.report.sales_register.sales_register import get_mode_of_payments
from capkpi.selling.report.item_wise_sales_history.item_wise_sales_history import (
...
def _execute(filters=None, additional_table_columns=None, additional_query_columns=None):
	...
	for d in item_list:
		...
		for tax in tax_columns:
			...
			row.update(
				{
					finergy.scrub(tax + " Rate"): item_tax.get("tax_rate", 0),
					finergy.scrub(tax + " Amount"): item_tax.get("tax_amount", 0),
				}
			)
...
def get_group_by_conditions(filters, doctype):
	if filters.get("group_by") == "Invoice":
		...
		return "ORDER BY `tab{0} Item`.{1}".format(doctype, finergy.scrub(filters.get("group_by")))
	elif filters.get("group_by") in ("Customer", "Customer Group", "Territory", "Supplier"):
		return "ORDER BY `tab{0}`.{1}".format(doctype, finergy.scrub(filters.get("group_by")))
...
def get_delivery_notes_against_sales_order(item_list):
	so_dn_map = finergy._dict()
	so_item_rows = list(set([d.so_detail for d in item_list]))

	if so_item_rows:
		delivery_notes = finergy.db.sql(
			"""
...
def get_tax_accounts(
				...
				for item_code, tax_data in item_wise_tax_detail.items():
					...
					for d in item_row_map.get(parent, {}).get(item_code, []):
						...
						if item_tax_amount:
							...
							itemised_tax.setdefault(d.name, {})[description] = finergy._dict(
	...
	for desc in tax_columns:
		columns.append(
			{
				"label": _(desc + " Rate"),
				"fieldname": finergy.scrub(desc + " Rate"),
				"fieldtype": "Float",
				"width": 100,
			}
		)
		...
		columns.append(
			{
				"label": _(desc + " Amount"),
				"fieldname": finergy.scrub(desc + " Amount"),
				"fieldtype": "Currency",
				"options": "currency",
				"width": 100,
			}
...
def get_display_value(filters, group_by_field, item):
	if filters.get("group_by") == "Item":
		...
		party = finergy.scrub(filters.get("group_by"))
...
def get_group_by_and_display_fields(filters):
	if filters.get("group_by") == "Item":
		...
		group_by_field = finergy.scrub(filters.get("group_by"))
		subtotal_display_field = "item_code"
...
def add_sub_total_row(item, total_row_map, group_by_value, tax_columns):
	...
	for tax in tax_columns:
		total_row.setdefault(finergy.scrub(tax + " Amount"), 0.0)
		total_row[finergy.scrub(tax + " Amount")] += flt(item[finergy.scrub(tax + " Amount")])
