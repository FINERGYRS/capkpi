...
from finergy.utils import cint, flt, formatdate
...
def execute(filters=None):
	...
	filters.currency = finergy.get_cached_value("Company", filters.company, "default_currency")

	gross_profit_data = GrossProfitGenerator(filters)

	data = []

	group_wise_columns = finergy._dict(
		{
			"invoice": [
				"invoice_or_item",
				"customer",
...
class GrossProfitGenerator(object):
	def __init__(self, filters=None):
		...
		self.filters = finergy._dict(filters)
	...
	def process(self):
		...
		self.currency_precision = cint(finergy.db.get_default("currency_precision")) or 3
		self.float_precision = cint(finergy.db.get_default("float_precision")) or 2
		...
		for row in reversed(self.si_list):
			...
			product_bundles = []
			if row.update_stock:
				product_bundles = self.product_bundles.get(row.parenttype, {}).get(row.parent, finergy._dict())
			elif row.dn_detail:
				product_bundles = self.product_bundles.get("Delivery Note", {}).get(
					row.delivery_note, finergy._dict()
				)
	...
	def get_returned_invoice_items(self):
		returned_invoices = finergy.db.sql(
			"""
			select
		...
		self.returned_invoices = finergy._dict()
		for inv in returned_invoices:
			self.returned_invoices.setdefault(inv.return_against, finergy._dict()).setdefault(
				inv.item_code, []
			).append(inv)
	...
	def get_last_purchase_rate(self, item_code, row):
		purchase_invoice = finergy.qb.DocType("Purchase Invoice")
		purchase_invoice_item = finergy.qb.DocType("Purchase Invoice Item")

		query = (
			finergy.qb.from_(purchase_invoice_item)
		...
		query.orderby(purchase_invoice.posting_date, order=finergy.qb.desc)
		query.limit(1)
		...
		for index, row in enumerate(self.si_list):
			if parents_index < len(parents) and row.parent == parents[parents_index]:
				...
				if not row.indent:
					...
					if finergy.db.exists("Product Bundle", row.item_code):
		...
		return finergy._dict(
			{
				...
				"base_net_amount": finergy.db.get_value("Sales Invoice", row.parent, "base_net_total"),
			}
		)
	...
	def load_product_bundle(self):
		...
		for d in finergy.db.sql(
			...
			self.product_bundles.setdefault(d.parenttype, finergy._dict()).setdefault(
				d.parent, finergy._dict()
			).setdefault(d.parent_item, []).append(d)

	def load_non_stock_items(self):
		self.non_stock_items = finergy.db.sql_list(
