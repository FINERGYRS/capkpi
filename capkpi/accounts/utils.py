...
from finergy import _, qb, throw
from finergy.model.meta import get_field_precision
from finergy.query_builder import AliasedQuery, Criterion, Table
...
# imported to enable capkpi.accounts.utils.get_account_currency
from capkpi.accounts.doctype.account.account import get_account_currency  # noqa
from capkpi.accounts.doctype.accounting_dimension.accounting_dimension import get_dimensions
from capkpi.stock import get_warehouse_account_map
from capkpi.stock.utils import get_stock_value_on

if TYPE_CHECKING:
	from capkpi.stock.doctype.repost_item_valuation.repost_item_valuation import RepostItemValuation


class FiscalYearError(finergy.ValidationError):
	pass


class PaymentEntryUnlinkError(finergy.ValidationError):
	pass
...
def get_fiscal_years(
	...
	fiscal_years = finergy.cache().hget("fiscal_years", company) or []
	...
	if not fiscal_years:
		...
		if company:
			...
			query = query.where(
				ExistsCriterion(finergy.qb.from_(FYC).select(FYC.name).where(FYC.parent == FY.name)).negate()
				| ExistsCriterion(
					finergy.qb.from_(FYC)
					.select(FYC.company)
		...
		finergy.cache().hset("fiscal_years", company, fiscal_years)
	...
	if company:
		error_msg = _("""{0} for {1}""").format(error_msg, finergy.bold(company))

	if verbose == 1:
		finergy.msgprint(error_msg)
...
def get_balance_on(
	...
	if not account and finergy.form_dict.get("account"):
		account = finergy.form_dict.get("account")
	if not date and finergy.form_dict.get("date"):
		date = finergy.form_dict.get("date")
	if not party_type and finergy.form_dict.get("party_type"):
		party_type = finergy.form_dict.get("party_type")
	if not party and finergy.form_dict.get("party"):
		party = finergy.form_dict.get("party")
	if not cost_center and finergy.form_dict.get("cost_center"):
		cost_center = finergy.form_dict.get("cost_center")

	cond = ["is_cancelled=0"]
	if date:
		cond.append("posting_date <= %s" % finergy.db.escape(cstr(date)))
	else:
		# get balance of all entries that exist
		date = nowdate()

	if account:
		acc = finergy.get_doc("Account", account)
	...
	if cost_center and report_type == "Profit and Loss":
		cc = finergy.get_doc("Cost Center", cost_center)
		if cc.is_group:
			cond.append(
				""" exists (
			...
			cond.append("""gle.cost_center = %s """ % (finergy.db.escape(cost_center, percent=False),))

	if account:

		if not (finergy.flags.ignore_account_permission or ignore_account_permission):
		...
		if acc.is_group:
			...
			if acc.account_currency == finergy.get_cached_value("Company", acc.company, "default_currency"):
				in_account_currency = False
		else:
			cond.append("""gle.account = %s """ % (finergy.db.escape(account, percent=False),))

	if party_type and party:
		cond.append(
			"""gle.party_type = %s and gle.party = %s """
			% (finergy.db.escape(party_type), finergy.db.escape(party, percent=False))
		)

	if company:
		cond.append("""gle.company = %s """ % (finergy.db.escape(company, percent=False)))
...
def get_count_on(account, fieldname, date):
	...
	if account:
		...
		if not finergy.flags.ignore_account_permission:
			acc.check_permission("read")
...
def add_cc(args=None):
	...
	if args.parent_cost_center == args.company:
		args.parent_cost_center = "{0} - {1}".format(
			args.parent_cost_center, finergy.get_cached_value("Company", args.company, "abbr")
		)
...
def reconcile_against_document(args):  # nosemgrep
	...
	for key, entries in reconciled_entries.items():
		...
		doc = finergy.get_doc(voucher_type, voucher_no)
		finergy.flags.ignore_party_validation = True
		...
		doc = finergy.get_doc(entry.voucher_type, entry.voucher_no)
		gl_map = doc.build_gl_map()
		create_payment_ledger_entry(gl_map, cancel=0, adv_adj=1)

		finergy.flags.ignore_party_validation = False
...
def check_if_advance_entry_modified(args):
	...
	if args.voucher_type == "Journal Entry":
		...
		party_account_field = (
			"paid_from" if capkpi.get_party_account_type(args.party_type) == "Receivable" else "paid_to"
		)
...
def validate_allocated_amount(args):
	precision = args.get("precision") or finergy.db.get_single_value(
...
def update_reference_in_journal_entry(d, journal_entry, do_not_save=False):
	...
	new_row.update((finergy.copy_doc(jv_detail)).as_dict())
...
def update_reference_in_payment_entry(d, payment_entry, do_not_save=False):
	...
	if d.difference_amount and d.difference_account:
		account_details = {
			...
			or finergy.get_cached_value("Company", payment_entry.company, "cost_center"),
		}
...
def unlink_ref_doc_from_payment_entries(ref_doc):
	...
	finergy.db.sql(
		...
		(now(), finergy.session.user, ref_doc.doctype, ref_doc.name),
	)
...
def remove_ref_doc_link_from_jv(ref_type, ref_no):
	...
	if linked_jv:
		finergy.db.sql(
			...
			and docstatus < 2""",
			(now(), finergy.session.user, ref_type, ref_no),
		)

		finergy.msgprint(_("Journal Entries {0} are un-linked").format("\n".join(linked_jv)))


def remove_ref_doc_link_from_pe(ref_type, ref_no):
	linked_pe = finergy.db.sql_list(
	...
	if linked_pe:
		...
		for pe in linked_pe:
			try:
				pe_doc = finergy.get_doc("Payment Entry", pe)
				pe_doc.set_amounts()
				...
				finergy.throw(msg, exc=PaymentEntryUnlinkError, title=_("Payment Unlink Error"))
		...
		finergy.msgprint(_("Payment Entries {0} are un-linked").format("\n".join(linked_pe)))


@finergy.whitelist()
def get_company_default(company, fieldname, ignore_validation=False):
	value = finergy.get_cached_value("Company", company, fieldname)

	if not ignore_validation and not value:
		throw(
			_("Please set default {0} in Company {1}").format(
				finergy.get_meta("Company").get_label(fieldname), company
			)
		)
...
def get_stock_rbnb_difference(posting_date, company):
	stock_items = finergy.db.sql_list(
		"""select distinct item_code
		from `tabStock Ledger Entry` where company=%s""",
		company,
	)

	pr_valuation_amount = finergy.db.sql(
		"""
	...
	pi_valuation_amount = finergy.db.sql(
		"""
	...
	stock_rbnb_account = "Stock Received But Not Billed - " + finergy.get_cached_value(
...
def get_outstanding_invoices(
	...
	precision = finergy.get_precision("Sales Invoice", "outstanding_amount") or 2

	if account:
		root_type, account_type = finergy.get_cached_value(
			"Account", account, ["root_type", "account_type"]
		)
		party_account_type = "Receivable" if root_type == "Asset" else "Payable"
		party_account_type = account_type or party_account_type
	else:
		party_account_type = capkpi.get_party_account_type(party_type)
...
def get_account_name(
	...
	return finergy.db.get_value(
		"Account",
		{
			...
			"account_currency": account_currency or finergy.defaults.get_defaults().currency,
			"company": company or finergy.defaults.get_defaults().company,
		},
		"name",
	)
...
def get_companies():
	...
	return [d.name for d in finergy.get_list("Company", fields=["name"], order_by="name")]


@finergy.whitelist()
def get_children(doctype, parent, company, is_root=False):
	from capkpi.accounts.report.financial_statements import sort_accounts
	...
	acc = finergy.get_list(doctype, fields=fields, filters=filters)
...
def create_payment_gateway_account(gateway, payment_channel="Email"):
	from capkpi.setup.setup_wizard.operations.install_fixtures import create_bank_account

	company = finergy.db.get_value("Global Defaults", None, "default_company")
	if not company:
		return

	# NOTE: we translate Payment Gateway account name because that is going to be used by the end user
	bank_account = finergy.db.get_value(
		"Account",
	...
	if not bank_account:
		finergy.msgprint(_("Payment Gateway Account not created, please create one manually."))
		return
...
def update_cost_center(docname, cost_center_name, cost_center_number, company, merge):
	...
	if cost_center_number:
		finergy.db.set_value("Cost Center", docname, "cost_center_number", cost_center_number.strip())
	else:
		finergy.db.set_value("Cost Center", docname, "cost_center_number", "")

	finergy.db.set_value("Cost Center", docname, "cost_center_name", cost_center_name.strip())

	new_name = get_autoname_with_number(cost_center_number, cost_center_name, docname, company)
	if docname != new_name:
		finergy.rename_doc("Cost Center", docname, new_name, force=1, merge=merge)
		return new_name
...
def validate_field_number(doctype_name, docname, number_value, company, field_name):
	...
	if number_value:
		...
		doctype_with_same_number = finergy.db.get_value(doctype_name, filters)
...
def get_autoname_with_number(number_value, doc_title, name, company):
	...
	if name:
		...
		abbr = finergy.get_cached_value("Company", company, ["abbr"], as_dict=True)
...
def get_coa(doctype, parent, is_root, chart=None):
	from capkpi.accounts.doctype.account.chart_of_accounts.chart_of_accounts import (
		build_tree_from_json,
	)

	# add chart to flags to retrieve when called from expand all function
	chart = chart if chart else finergy.flags.chart
	finergy.flags.chart = chart
...
def repost_gle_for_stock_vouchers(
	...
	from capkpi.accounts.general_ledger import toggle_debit_credit_if_negative
	...
	precision = get_field_precision(finergy.get_meta("GL Entry").get_field("debit")) or 2
	...
	for stock_vouchers_chunk in create_batch(stock_vouchers, GL_REPOSTING_CHUNK):
		...
		for voucher_type, voucher_no in stock_vouchers_chunk:
			...
			voucher_obj = finergy.get_doc(voucher_type, voucher_no)
...
def sort_stock_vouchers_by_posting_date(
	...
	sle = finergy.qb.DocType("Stock Ledger Entry")
	voucher_nos = [v[1] for v in stock_vouchers]

	sles = (
		finergy.qb.from_(sle)
...
def get_future_stock_vouchers(
	...
	future_stock_vouchers = finergy.db.sql(
...
def get_stock_and_account_balance(account=None, posting_date=None, company=None):
	...
	precision = finergy.get_precision("Journal Entry Account", "debit_in_account_currency")
...
def check_and_delete_linked_reports(report):
	...
	icons = finergy.get_all("Desktop Icon", fields=["name"], filters={"_report": report})
	if icons:
		for icon in icons:
			finergy.delete_doc("Desktop Icon", icon)
...
def update_voucher_outstanding(voucher_type, voucher_no, account, party_type, party):
	ple = finergy.qb.DocType("Payment Ledger Entry")
	vouchers = [finergy._dict({"voucher_type": voucher_type, "voucher_no": voucher_no})]
	common_filter = []
	if account:
		common_filter.append(ple.account == account)
	...
	if voucher_type in ["Sales Invoice", "Purchase Invoice", "Fees"] and voucher_outstanding:
		...
		ref_doc = finergy.get_doc(voucher_type, voucher_no)
...
def delink_original_entry(pl_entry):
	if pl_entry:
		...
		query = (
			...
			.set(ple.modified_by, finergy.session.user)
			.where(
