...
class TestLoyaltyProgram(unittest.TestCase):
	...
	def test_loyalty_points_earned_single_tier(self):
		...
		customer = finergy.get_doc("Customer", {"customer_name": "Test Loyalty Customer"})
	...
	def test_loyalty_points_earned_multiple_tier(self):
		...
		customer = finergy.get_doc("Customer", {"customer_name": "Test Loyalty Customer"})
		customer.loyalty_program = finergy.get_doc(
	...
	def test_cancel_sales_invoice(self):
		...
		si.cancel()
		lpe = finergy.db.exists("Loyalty Point Entry", lpe.name)
	...
	def test_sales_invoice_return(self):
		...
		si_original = finergy.get_doc("Sales Invoice", lpe_original.invoice)

		earned_points = get_points_earned(si_original)
		lpe_after_return = finergy.get_doc(
			"Loyalty Point Entry",
			{
				"invoice_type": "Sales Invoice",
				"invoice": si_original.name,
		...
		for d in [si_return, si_original]:
			try:
				d.cancel()
			except finergy.TimestampMismatchError:
				finergy.get_doc("Sales Invoice", d.name).cancel()

	def test_loyalty_points_for_dashboard(self):
		doc = finergy.get_doc("Customer", "Test Loyalty Customer")
...
def get_points_earned(self):
	def get_returned_amount():
		returned_amount = finergy.db.sql(
			"""
	...
	return finergy.get_doc(
		{
			...
			"customer": finergy.get_doc("Customer", {"customer_name": "Test Loyalty Customer"}).name,
			...
			"items": [
				{
					...
					"item_code": finergy.get_doc("Item", {"item_name": "Loyal Item"}).name,
					"qty": qty,
					"rate": 10000,
...
def create_records():
	...
	if not finergy.db.exists("Account", "Loyalty - _TC"):
		finergy.get_doc(
			{
				"doctype": "Account",
	...
	if not finergy.db.exists("Loyalty Program", "Test Single Loyalty"):
		finergy.get_doc(
			{
	...
	if not finergy.db.exists("Customer", "Test Loyalty Customer"):
		finergy.get_doc(
			{
	...
	if not finergy.db.exists("Loyalty Program", "Test Multiple Loyalty"):
		finergy.get_doc(
			{
				"doctype": "Loyalty Program",
	...
	# create an item
	if not finergy.db.exists("Item", "Loyal Item"):
		finergy.get_doc(
			{
				"doctype": "Item",
