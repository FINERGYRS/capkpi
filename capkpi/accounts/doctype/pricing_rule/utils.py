...
from finergy.utils import cint, flt, fmt_money, get_link_to_form, getdate, today

from capkpi.setup.doctype.item_group.item_group import get_child_item_groups
from capkpi.stock.doctype.warehouse.warehouse import get_child_warehouses
from capkpi.stock.get_item_details import get_conversion_factor


class MultiplePricingRuleConflict(finergy.ValidationError):
	pass
...
def get_pricing_rules(args, doc=None):
	...
	if not finergy.db.exists("Pricing Rule", {"disable": 0, args.transaction_type: 1}):
		return
	...
	if doc:
		for pricing_rule in pricing_rules:
			if pricing_rule.condition:
				try:
					if finergy.safe_eval(pricing_rule.condition, None, doc.as_dict()):
...
def _get_pricing_rules(apply_on, args, values):
	apply_on_field = finergy.scrub(apply_on)
	...
	if apply_on_field in ["item_code", "brand"]:
		...
		if apply_on_field == "item_code":
			if "variant_of" not in args:
				args.variant_of = finergy.get_cached_value("Item", args.item_code, "variant_of")
...
def _get_tree_conditions(args, parenttype, table, allow_blank=True):
	field = finergy.scrub(parenttype)
	condition = ""
	if args.get(field):
		if not finergy.flags.tree_conditions:
			finergy.flags.tree_conditions = {}
		key = (parenttype, args.get(field))
		if key in finergy.flags.tree_conditions:
			return finergy.flags.tree_conditions[key]

		try:
			lft, rgt = finergy.db.get_value(parenttype, args.get(field), ["lft", "rgt"])
		except TypeError:
			finergy.throw(_("Invalid {0}").format(args.get(field)))

		parent_groups = finergy.db.sql_list(
		...
		if parenttype in ["Customer Group", "Item Group", "Territory"]:
			parent_field = "parent_{0}".format(finergy.scrub(parenttype))
			root_name = finergy.db.get_list(
				parenttype,
		...
		if parent_groups:
			...
			condition = "ifnull({table}.{field}, '') in ({parent_groups})".format(
				table=table, field=field, parent_groups=", ".join(finergy.db.escape(d) for d in parent_groups)
			)

			finergy.flags.tree_conditions[key] = condition
	return condition
...
def filter_pricing_rules(args, pricing_rules, doc=None):
	...
	if pricing_rules:
		...
		if pricing_rules[0].apply_rule_on_other:
			field = finergy.scrub(pricing_rules[0].apply_rule_on_other)

			if field and pricing_rules[0].get("other_" + field) != args.get(field):
				return

		pr_doc = finergy.get_cached_doc("Pricing Rule", pricing_rules[0].name)
		...
		if pricing_rules[0].mixed_conditions and doc:
			...
			items = [args.get(finergy.scrub(pr_doc.get("apply_on")))]
...
def get_qty_and_rate_for_mixed_conditions(doc, pr_doc, args):
	...
	apply_on = finergy.scrub(pr_doc.get("apply_on"))
...
def get_qty_and_rate_for_other_item(doc, pr_doc, pricing_rules):
	...
	for row in doc.items:
		if row.get(finergy.scrub(pr_doc.apply_rule_on_other)) in items:
...
def get_qty_amount_data_for_cumulative(pr_doc, doc, items=None):
	...
	date_field = (
		"transaction_date" if finergy.get_meta(doctype).has_field("transaction_date") else "posting_date"
	)
			...
			if d.price_or_product_discount == "Price":
				...
				for field in ["additional_discount_percentage", "discount_amount"]:
					...
					if (
						...
						finergy.msgprint(_("User has not applied rule on the invoice {0}").format(doc.name))
					else:
						if not d.coupon_code_based:
							doc.set(field, d.get(pr_field))
						elif doc.get("coupon_code"):
							# coupon code based pricing rule
							coupon_code_pricing_rule = finergy.db.get_value(
				...
				item_details = finergy._dict({"parenttype": doc.doctype, "free_item_data": []})
...
def get_product_discount_rule(pricing_rule, item_details, args=None, doc=None):
	...
	item_data = finergy.get_cached_value(
...
def get_pricing_rule_items(pr_doc):
	...
	if pr_doc.apply_rule_on_other:
		apply_on = finergy.scrub(pr_doc.apply_rule_on_other)
...
def validate_coupon_code(coupon_name):
	coupon = finergy.get_doc("Coupon Code", coupon_name)

	if coupon.valid_from:
		if coupon.valid_from > getdate(today()):
			finergy.throw(_("Sorry, this coupon code's validity has not started"))
	elif coupon.valid_upto:
		if coupon.valid_upto < getdate(today()):
			finergy.throw(_("Sorry, this coupon code's validity has expired"))
	elif coupon.used >= coupon.maximum_use:
		finergy.throw(_("Sorry, this coupon code is no longer valid"))
