...
import finergy
from finergy import MandatoryError

from capkpi.accounts.doctype.sales_invoice.test_sales_invoice import create_sales_invoice
from capkpi.healthcare.doctype.lab_test_template.lab_test_template import make_item_price
...
class TestPricingRule(unittest.TestCase):
	...
	def test_pricing_rule_for_discount(self):
		...
		finergy.get_doc(test_record.copy()).insert()
		...
		prule = finergy.get_doc(test_record.copy())
		prule.priority = 1
		...
		finergy.db.sql("update `tabPricing Rule` set priority=NULL where campaign='_Test Campaign'")
		from capkpi.accounts.doctype.pricing_rule.utils import MultiplePricingRuleConflict
	...
	def test_pricing_rule_for_variants(self):
		...
		if not finergy.db.exists("Item", "Test Variant PRT"):
			finergy.get_doc(
				{
					"doctype": "Item",
	...
	def test_pricing_rule_for_stock_qty(self):
		...
		if not finergy.db.get_value("UOM Conversion Detail", {"parent": "_Test Item", "uom": "box"}):
		...
		so.submit()
		so = finergy.get_doc("Sales Order", so.name)
	...
	def test_pricing_rule_with_margin_and_discount(self):
		finergy.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule")
		make_pricing_rule(
	...
	def test_cumulative_pricing_rule(self):
		finergy.delete_doc_if_exists("Pricing Rule", "_Test Cumulative Pricing Rule")
		test_record = {
			"doctype": "Pricing Rule",
			...
			"company": "_Test Company",
			"valid_from": finergy.utils.nowdate(),
			"valid_upto": finergy.utils.nowdate(),
		}
		finergy.get_doc(test_record.copy()).insert()
		...
		args = finergy._dict(
			{
				...
				"customer": "_Test Customer",
				"name": None,
				"transaction_date": finergy.utils.nowdate(),
			}
		)
		details = get_item_details(args)
	...
	def test_multiple_pricing_rules(self):
		...
		finergy.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule 1")
		finergy.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule 2")
	...
	def test_item_price_with_pricing_rule(self):
		...
		rule = finergy.get_doc(pricing_rule_record)
		rule.insert()
		...
		si.delete()
		rule.delete()
		finergy.get_doc("Item Price", {"item_code": "Water Flask"}).delete()
		item.delete()
	...
	def test_pricing_rule_for_different_currency(self):
		...
		rule1 = finergy.get_doc(pricing_rule_record)
		rule1.currency = "USD"
	...
	def test_multiple_pricing_rules_with_min_qty(self):
		...
		finergy.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule with Min Qty - 1")
		finergy.delete_doc_if_exists("Pricing Rule", "_Test Pricing Rule with Min Qty - 2")
...
def setup_pricing_rule_data():
	if not finergy.db.exists("Campaign", "_Test Campaign"):
		finergy.get_doc(
...
def delete_existing_pricing_rules():
	for doctype in [
		...
		finergy.db.sql("delete from `tab{0}`".format(doctype))
