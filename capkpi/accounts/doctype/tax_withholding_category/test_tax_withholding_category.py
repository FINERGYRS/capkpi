...
class TestTaxWithholdingCategory(unittest.TestCase):
	...
	def test_single_threshold_tds(self):
		...
		gl_entries = finergy.db.get_all("GL Entry", filters={"voucher_no": pi.name}, fields=["*"])
...
def cancel_invoices():
	purchase_invoices = finergy.get_all(
		"Purchase Invoice",
		{
	...
	sales_invoices = finergy.get_all(
		"Sales Invoice", {"customer": "Test TCS Customer", "docstatus": 1}, pluck="name"
	)

	for d in purchase_invoices:
		finergy.get_doc("Purchase Invoice", d).cancel()

	for d in sales_invoices:
		finergy.get_doc("Sales Invoice", d).cancel()


def create_purchase_invoice(**args):
	# return sales invoice doc object
	item = finergy.db.get_value("Item", {"item_name": "TDS Item"}, "name")
...
def create_sales_invoice(**args):
	# return sales invoice doc object
	item = finergy.db.get_value("Item", {"item_name": "TCS Item"}, "name")
...
def create_records():
	...
	for name in [
		...
		if finergy.db.exists("Supplier", name):
			continue
	...
	for name in ["Test TCS Customer"]:
		if finergy.db.exists("Customer", name):
			continue
	...
	# create item
	if not finergy.db.exists("Item", "TDS Item"):
		finergy.get_doc(
			{
				"doctype": "Item",
				"item_code": "TDS Item",
	...
	if not finergy.db.exists("Item", "TCS Item"):
		finergy.get_doc(
			{
				"doctype": "Item",
				"item_code": "TCS Item",
	...
	# create tds account
	if not finergy.db.exists("Account", "TDS - _TC"):
		finergy.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	# create tcs account
	if not finergy.db.exists("Account", "TCS - _TC"):
		finergy.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
...
def create_tax_with_holding_category():
	...
	if not finergy.db.exists("Tax Withholding Category", "Cumulative Threshold TDS"):
		finergy.get_doc(
			{
	...
	if not finergy.db.exists("Tax Withholding Category", "Cumulative Threshold TCS"):
		finergy.get_doc(
			{
				"doctype": "Tax Withholding Category",
	...
	# Single thresold
	if not finergy.db.exists("Tax Withholding Category", "Single Threshold TDS"):
		finergy.get_doc(
			{
				"doctype": "Tax Withholding Category",
	...
	if not finergy.db.exists("Tax Withholding Category", "New TDS Category"):
		finergy.get_doc(
			{
				"doctype": "Tax Withholding Category",
	...
	if not finergy.db.exists("Tax Withholding Category", "Test Service Category"):
		finergy.get_doc(
			{
				"doctype": "Tax Withholding Category",
	...
	if not finergy.db.exists("Tax Withholding Category", "Test Goods Category"):
		finergy.get_doc(
			{
				"doctype": "Tax Withholding Category",
