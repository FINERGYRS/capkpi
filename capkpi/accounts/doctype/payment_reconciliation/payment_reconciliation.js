...
capkpi.accounts.PaymentReconciliationController = class PaymentReconciliationController extends finergy.ui.form.Controller {
	onload() {
		const default_company = finergy.defaults.get_default('company');
		...
		this.frm.set_query('receivable_payable_account', () => {
			return {
				filters: {
					...
					"is_group": 0,
					"account_type": finergy.boot.party_account_types[this.frm.doc.party_type]
				}
			};
		});
		...
		return this.frm.call({
			...
			callback: () => {
				if (!(this.frm.doc.payments.length || this.frm.doc.invoices.length)) {
					finergy.throw({message: __("No Unreconciled Invoices and Payments found for this party and account")});
				} else if (!(this.frm.doc.invoices.length)) {
					finergy.throw({message: __("No Outstanding Invoices found for this party")});
				} else if (!(this.frm.doc.payments.length)) {
					finergy.throw({message: __("No Unreconciled Payments found for this party")});
				}
				this.frm.refresh();
			}
		});
		...
		if (show_dialog && show_dialog.length) {
			...
			const dialog = new finergy.ui.Dialog({
				...
				primary_action: () => {
					...
					args.forEach(d => {
						finergy.model.set_value("Payment Reconciliation Allocation", d.docname,
...
extend_cscript(cur_frm.cscript, new capkpi.accounts.PaymentReconciliationController({frm: cur_frm}));
