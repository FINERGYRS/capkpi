...
class TestPaymentReconciliation(unittest.TestCase):
	...
	def test_payment_reconciliation(self):
		payment_reco = finergy.get_doc("Payment Reconciliation")
		...
		payment_entry_doc = finergy.get_doc("Payment Entry", payment_entry)
		self.assertEqual(payment_entry_doc.get("references")[0].reference_name, invoice)


def make_customer():
	if not finergy.db.get_value("Customer", "_Test Payment Reco Customer"):
		finergy.get_doc(
			{
				"doctype": "Customer",
