...
import finergy
from finergy import qb
from finergy.tests.utils import FinergyTestCase
from finergy.utils import add_days, nowdate

from capkpi.accounts.doctype.payment_entry.test_payment_entry import create_payment_entry
...
class TestPaymentReconciliation(FinergyTestCase):
	def setUp(self):
		self.create_company()
		self.create_item()
	...
	def create_company(self):
		company = None
		if finergy.db.exists("Company", "_Test Payment Reconciliation"):
			company = finergy.get_doc("Company", "_Test Payment Reconciliation")
		else:
		...
		# create bank account
		if finergy.db.exists("Account", "HDFC - _PR"):
			self.bank = "HDFC - _PR"
		else:
			bank_acc = finergy.get_doc(
				{
					"doctype": "Account",
					"account_name": "HDFC",
	...
	def create_customer(self):
		if finergy.db.exists("Customer", "_Test PR Customer"):
		...
		if finergy.db.exists("Customer", "_Test PR Customer 2"):
		...
		if finergy.db.exists("Customer", "_Test PR Customer 3"):
	...
	def create_account(self):
		...
		if not finergy.db.get_value(
			...
			acc = finergy.get_doc("Account", name)
	...
	def create_payment_reconciliation(self):
		pr = finergy.new_doc("Payment Reconciliation")
	...
	def test_filter_posting_date_case2(self):
		...
		pr.allocate_entries(finergy._dict({"invoices": invoices, "payments": payments}))
		pr.reconcile()
