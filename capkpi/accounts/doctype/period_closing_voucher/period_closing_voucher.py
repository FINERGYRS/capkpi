...
class PeriodClosingVoucher(AccountsController):
	...
	def validate_account_head(self):
		closing_account_type = finergy.db.get_value("Account", self.closing_account_head, "root_type")
		...
		if account_currency != company_currency:
			finergy.throw(_("Currency of the Closing Account must be {0}").format(company_currency))

	def validate_posting_date(self):
		from capkpi.accounts.utils import get_fiscal_year, validate_fiscal_year
	...
	def make_gl_entries(self):
		...
		if gl_entries:
			if len(gl_entries) > 5000:
				finergy.enqueue(process_gl_entries, gl_entries=gl_entries, queue="long")
				finergy.msgprint(
...
def make_reverse_gl_entries(voucher_type, voucher_no):
	...
	try:
		...
		finergy.db.set_value("Period Closing Voucher", voucher_no, "gle_processing_status", "Completed")
	except Exception as e:
		finergy.db.rollback()
		finergy.log_error(e)
		finergy.db.set_value("Period Closing Voucher", voucher_no, "gle_processing_status", "Failed")
