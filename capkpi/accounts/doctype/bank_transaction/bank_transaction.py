...
class BankTransaction(StatusUpdater):
	...
	def update_allocations(self):
		...
		if allocated_amount:
			finergy.db.set_value(self.doctype, self.name, "allocated_amount", flt(allocated_amount))
			...
			finergy.db.set_value(self.doctype, self.name, "allocated_amount", 0)
			finergy.db.set_value(
		...
		if amount == self.allocated_amount:
			finergy.db.set_value(self.doctype, self.name, "status", "Reconciled")
	...
	def clear_simple_entry(self, payment_entry, for_cancel=False):
		if payment_entry.payment_document == "Payment Entry":
			if (
				finergy.db.get_value("Payment Entry", payment_entry.payment_entry, "payment_type")
				== "Internal Transfer"
			):
...
def get_doctypes_for_bank_reconciliation():
	...
	return finergy.get_hooks("bank_reconciliation_doctypes")


def get_reconciled_bank_transactions(payment_entry):
	reconciled_bank_transactions = finergy.get_all(
...
def get_paid_amount(payment_entry, currency, bank_account):
	if payment_entry.payment_document in ["Payment Entry", "Sales Invoice", "Purchase Invoice"]:
		...
		if payment_entry.payment_document == "Payment Entry":
			doc = finergy.get_doc("Payment Entry", payment_entry.payment_entry)
...
def unclear_reference_payment(doctype, docname):
	if finergy.db.exists(doctype, docname):
		...
		if doctype == "Sales Invoice":
			...
			finergy.db.set_value(doc.payment_document, doc.payment_entry, "clearance_date", None)
