					...
					account = finergy.get_doc(
						{
							...
							or finergy.db.get_value("Company", company, "default_currency"),
							"tax_rate": child.get("tax_rate"),
						}
					)

					if root_account or finergy.local.flags.allow_unverified_charts:
...
def get_chart(chart_template, existing_company=None):
	...
	if existing_company:
		...
		from capkpi.accounts.doctype.account.chart_of_accounts.verified import (
		...
		for folder in folders:
			...
			for fname in os.listdir(path):
				fname = finergy.as_unicode(fname)
...
def get_charts_for_country(country, with_standard=False):
	...
	def _get_chart_name(content):
		if content:
			...
			) or finergy.local.flags.allow_unverified_charts:
				charts.append(content["name"])

	country_code = finergy.db.get_value("Country", country, "code")
	if country_code:
		folders = ("verified",)
		if finergy.local.flags.allow_unverified_charts:
			folders = ("verified", "unverified")
