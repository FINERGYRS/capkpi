...
finergy.ui.form.on('Shareholder', {
	refresh: function(frm) {
		finergy.dynamic_link = { doc: frm.doc, fieldname: 'name', doctype: 'Shareholder' };
		...
		if (frm.doc.folio_no != undefined){
			frm.add_custom_button(__("Share Balance"), function(){
				...
				finergy.set_route("query-report", "Share Balance");
			});
			frm.add_custom_button(__("Share Ledger"), function(){
				finergy.route_options = {
					"shareholder": frm.doc.name,
				};
				finergy.set_route("query-report", "Share Ledger");
			});
