...
finergy.ui.form.on('POS Settings', {
	onload: function(frm) {
	...
	get_invoice_fields: function(frm) {
		finergy.model.with_doctype("POS Invoice", () => {
			var fields = $.map(finergy.get_doc("DocType", "POS Invoice").fields, function(d) {
				if (finergy.model.no_value_type.indexOf(d.fieldtype) === -1 || ['Button'].includes(d.fieldtype)) {
	...
	add_search_options: function(frm) {
		finergy.model.with_doctype("Item", () => {
			var fields = $.map(finergy.get_doc("DocType", "Item").fields, function(d) {
...
finergy.ui.form.on("POS Search Fields", {
	field: function(frm, doctype, name) {
		var doc = finergy.get_doc(doctype, name);
		var df = $.map(finergy.get_doc("DocType", "Item").fields, function(d) {
...
finergy.ui.form.on("POS Field", {
	fieldname: function(frm, doctype, name) {
		var doc = finergy.get_doc(doctype, name);
		var df = $.map(finergy.get_doc("DocType", "POS Invoice").fields, function(d) {
