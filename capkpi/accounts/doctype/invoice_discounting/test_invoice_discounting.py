...
import finergy
from finergy.utils import add_days, flt, nowdate
...
class TestInvoiceDiscounting(unittest.TestCase):
	def setUp(self):
		...
		finergy.db.set_value("Company", "_Test Company", "default_bank_account", self.bank_account)
	...
	def test_on_close_after_loan_period_after_inv_payment(self):
		...
		je_on_payment = finergy.get_doc(get_payment_entry_against_invoice("Sales Invoice", inv.name))
...
def create_invoice_discounting(invoices, **args):
	...
	inv_disc = finergy.new_doc("Invoice Discounting")
