...
capkpi.accounts.PurchaseInvoice = class PurchaseInvoice extends capkpi.buying.BuyingController {
	setup(doc) {
	...
	refresh(doc) {
		...
		if (doc.docstatus == 1 && !doc.inter_company_invoice_reference) {
			finergy.model.with_doc("Supplier", me.frm.doc.supplier, function() {
				var supplier = finergy.model.get_doc("Supplier", me.frm.doc.supplier);
	...
	unblock_invoice() {
		const me = this;
		finergy.call({
			'method': 'capkpi.accounts.doctype.purchase_invoice.purchase_invoice.unblock_invoice',
	...
	can_change_release_date(date) {
		const diff = finergy.datetime.get_diff(date, finergy.datetime.nowdate());
		if (diff < 0) {
			finergy.throw(__('New release date should be in the future'));
			return false;
		} else {
			return true;
		}
	...
	make_comment_dialog_and_block_invoice(){
		...
		this.dialog.set_primary_action(__('Save'), function() {
			...
			finergy.call({
				'method': 'capkpi.accounts.doctype.purchase_invoice.purchase_invoice.block_invoice',
				'args': {
	...
	set_release_date(data) {
		return finergy.call({
			'method': 'capkpi.accounts.doctype.purchase_invoice.purchase_invoice.change_release_date',
			'args': data,
	...
	supplier() {
		...
		capkpi.utils.get_party_details(this.frm, "capkpi.accounts.party.get_party_details",
			{
	...
	make_inter_company_invoice(frm) {
		finergy.model.open_mapped_doc({
			method: "capkpi.accounts.doctype.purchase_invoice.purchase_invoice.make_inter_company_sales_invoice",
			frm: frm
		});
	}
	...
	on_submit() {
		$.each(this.frm.doc["items"] || [], function(i, row) {
			if(row.purchase_receipt) finergy.model.clear_doc("Purchase Receipt", row.purchase_receipt)
		})
	}

	make_debit_note() {
		finergy.model.open_mapped_doc({
			method: "capkpi.accounts.doctype.purchase_invoice.purchase_invoice.make_debit_note",
			frm: cur_frm
		})
	}
};

cur_frm.script_manager.make(capkpi.accounts.PurchaseInvoice);
	...
	add_custom_buttons: function(frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.per_received > 0) {
			frm.add_custom_button(__('Purchase Receipt'), () => {
				...
				finergy.set_route("List", "Purchase Receipt", "List")
			}, __('View'));
		}
	},
