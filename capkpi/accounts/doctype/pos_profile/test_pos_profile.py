...
from capkpi.accounts.doctype.pos_profile.pos_profile import get_child_nodes
from capkpi.stock.get_item_details import get_pos_profile
...
class TestPOSProfile(unittest.TestCase):
	def test_pos_profile(self):
		...
		if pos_profile:
			doc = finergy.get_doc("POS Profile", pos_profile.get("name"))
...
def make_pos_profile(**args):
	finergy.db.sql("delete from `tabPOS Payment Method`")
	finergy.db.sql("delete from `tabPOS Profile`")

	args = finergy._dict(args)

	pos_profile = finergy.get_doc(
		{
			...
			"customer_group": finergy.db.get_value("Customer Group", {"is_group": 0}, "name"),
	...
	if not finergy.db.get_value("Mode of Payment Account", {"company": company, "parent": "Cash"}):
	...
	if not finergy.db.exists("POS Profile", args.name or "_Test POS Profile"):
		pos_profile.insert()
