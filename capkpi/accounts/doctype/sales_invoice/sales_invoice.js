...
{% include 'capkpi/selling/sales_common.js' %};
finergy.provide("capkpi.accounts");


capkpi.accounts.SalesInvoiceController = capkpi.selling.SellingController.extend({
	setup: function(doc) {
	...
	company: function() {
		capkpi.accounts.dimensions.update_dimension(this.frm, this.frm.doctype);
		let me = this;
		if (this.frm.doc.company) {
			finergy.call({
				method:
					"capkpi.accounts.party.get_party_account",
				args: {
					party_type: 'Customer',
	...
	onload: function() {
		...
		capkpi.queries.setup_warehouse_query(this.frm);
	},
	...
	refresh: function(doc, dt, dn) {
		...
		if(doc.docstatus==1 && !doc.is_return) {
			...
			if (doc.outstanding_amount>0) {
				...
				if (doc.due_date < finergy.datetime.get_today()) {
			...
			if(!doc.auto_repeat) {
				cur_frm.add_custom_button(__('Subscription'), function() {
					capkpi.utils.make_subscription(doc.doctype, doc.name)
				}, __('Create'))
			}
		}
	...
	make_maintenance_schedule: function() {
		finergy.model.open_mapped_doc({
			method: "capkpi.accounts.doctype.sales_invoice.sales_invoice.make_maintenance_schedule",
			frm: cur_frm
		})
	},
	...
	on_submit: function(doc, dt, dn) {
		...
		if (finergy.get_route()[0] != 'Form') {
			return
		}

		$.each(doc["items"], function(i, row) {
			if(row.delivery_note) finergy.model.clear_doc("Delivery Note", row.delivery_note)
		})
	},
		...
		this.$sales_order_btn = this.frm.add_custom_button(__('Sales Order'),
			function() {
				capkpi.utils.map_current_doc({
					method: "capkpi.selling.doctype.sales_order.sales_order.make_sales_invoice",
	...
	quotation_btn: function() {
		...
		this.$quotation_btn = this.frm.add_custom_button(__('Quotation'),
			function() {
				capkpi.utils.map_current_doc({
					method: "capkpi.selling.doctype.quotation.quotation.make_sales_invoice",
	...
	delivery_note_btn: function() {
		...
		this.$delivery_note_btn = this.frm.add_custom_button(__('Delivery Note'),
			function() {
				capkpi.utils.map_current_doc({
					method: "capkpi.stock.doctype.delivery_note.delivery_note.make_sales_invoice",
					...
					get_query: function() {
						...
						return {
							query: "capkpi.controllers.queries.get_delivery_notes_to_be_billed",
							filters: filters
						};
					}
				});
	...
	customer: function() {
		...
		capkpi.utils.get_party_details(this.frm,
			"capkpi.accounts.party.get_party_details", {
		...
		if(this.frm.doc.customer) {
			finergy.call({
				"method": "capkpi.accounts.doctype.sales_invoice.sales_invoice.get_loyalty_programs",
				"args": {
	...
	make_inter_company_invoice: function() {
		finergy.model.open_mapped_doc({
			method: "capkpi.accounts.doctype.sales_invoice.sales_invoice.make_inter_company_purchase_invoice",
			frm: me.frm
		});
	},
	...
	write_off_outstanding_amount_automatically() {
		if (cint(this.frm.doc.write_off_outstanding_amount_automatically)) {
			finergy.model.round_floats_in(this.frm.doc, ["grand_total", "paid_amount"]);
	...
	make_sales_return: function() {
		finergy.model.open_mapped_doc({
			method: "capkpi.accounts.doctype.sales_invoice.sales_invoice.make_sales_return",
			frm: cur_frm
		})
	},
	...
	asset: function(frm, cdt, cdn) {
		...
		if(row.asset) {
			finergy.call({
				method: capkpi.assets.doctype.asset.depreciation.get_disposal_account_and_cost_center,
				args: {
					"company": frm.doc.company
				},
				callback: function(r, rt) {
					finergy.model.set_value(cdt, cdn, "income_account", r.message[0]);
					finergy.model.set_value(cdt, cdn, "cost_center", r.message[1]);
				}
			})
		}
	},
	...
	set_pos_data: function() {
		if(this.frm.doc.is_pos) {
			...
			if(!this.frm.doc.company) {
				...
				finergy.msgprint(__("Please specify Company to proceed"));
			} else {
				var me = this;
				return this.frm.call({
					...
					callback: function(r) {
						if(!r.exc) {
							...
							finergy.model.set_default_values(me.frm.doc);
							me.set_dynamic_labels();
	...
	currency() {
		...
		if (this.frm.doc.timesheets) {
			this.frm.doc.timesheets.forEach((d) => {
				let row = finergy.get_doc(d.doctype, d.name)
...
$.extend(cur_frm.cscript, new capkpi.accounts.SalesInvoiceController({frm: cur_frm}));

cur_frm.cscript['Make Delivery Note'] = function() {
	finergy.model.open_mapped_doc({
		method: "capkpi.accounts.doctype.sales_invoice.sales_invoice.make_delivery_note",
		frm: cur_frm
	})
}
...
cur_frm.set_query("income_account", "items", function(doc) {
	return{
		query: "capkpi.controllers.queries.get_income_account",
...
cur_frm.cscript.income_account = function(doc, cdt, cdn) {
	capkpi.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "income_account");
}

cur_frm.cscript.expense_account = function(doc, cdt, cdn) {
	capkpi.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "expense_account");
}

cur_frm.cscript.cost_center = function(doc, cdt, cdn) {
	capkpi.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "cost_center");
}
...
finergy.ui.form.on('Sales Invoice', {
	setup: function(frm){
		...
		frm.fields_dict["timesheets"].grid.get_field("time_sheet").get_query = function(doc, cdt, cdn){
			return{
				query: "capkpi.projects.doctype.timesheet.timesheet.get_timesheet",
		...
		frm.fields_dict['items'].grid.get_field('expense_account').get_query = function(doc) {
			if (capkpi.is_perpetual_inventory_enabled(doc.company)) {
				return {
					filters: {
						'report_type': 'Profit and Loss',
						'company': doc.company,
		...
		frm.set_query('pos_profile', function(doc) {
			if(!doc.company) {
				finergy.throw(_('Please set Company'));
			}

			return {
				query: 'capkpi.accounts.doctype.pos_profile.pos_profile.pos_profile_query',
				filters: {
					company: doc.company
				}
			};
	...
	company: function(frm){
		if (frm.doc.company) {
			finergy.call({
				method: "capkpi.setup.doctype.company.company.get_default_company_address",
	...
	loyalty_points: function(frm) {
		if (frm.redemption_conversion_factor) {
			...
			finergy.call({
				method: "capkpi.accounts.doctype.loyalty_program.loyalty_program.get_redeemption_factor",
				args: {
	...
	hide_fields: function(frm) {
		...
		if(cint(doc.is_pos) == 1) {
			...
			for (var i in parent_fields) {
				var docfield = finergy.meta.docfield_map[doc.doctype][parent_fields[i]];
				if(!docfield.hidden) unhide_field(parent_fields[i]);
			}
		}

		// India related fields
		if (finergy.boot.sysdefaults.country == 'India') unhide_field(['c_form_applicable', 'c_form_no']);
	...
	get_loyalty_details: function(frm) {
		if (frm.doc.customer && frm.doc.redeem_loyalty_points) {
			finergy.call({
				method: "capkpi.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details",
				args: {
					"customer": frm.doc.customer,
					"loyalty_program": frm.doc.loyalty_program,
	...
	set_loyalty_points: function(frm) {
		if (frm.redemption_conversion_factor) {
			...
			if (frm.doc.grand_total && (remaining_amount < loyalty_amount)) {
				...
				finergy.throw(__("You can only redeem max {0} points in this order.",[redeemable_points]));
			}
	...
	patient: function(frm) {
		if (finergy.boot.active_domains.includes("Healthcare")){
			if(frm.doc.patient){
				finergy.call({
					method: "finergy.client.get_value",
					args:{
	...
	async get_timesheet_data(frm, kwargs) {
		return finergy.call({
			method: "capkpi.projects.doctype.timesheet.timesheet.get_projectwise_timesheet_data",
			args: kwargs
		}).then(r => {
	...
	create_invoice_discounting: function(frm) {
		finergy.model.open_mapped_doc({
			method: "capkpi.accounts.doctype.sales_invoice.sales_invoice.create_invoice_discounting",
			frm: frm
		});
	},
	...
	create_dunning: function(frm) {
		finergy.model.open_mapped_doc({
			method: "capkpi.accounts.doctype.sales_invoice.sales_invoice.create_dunning",
			frm: frm
		});
	}
});
...
finergy.ui.form.on("Sales Invoice Timesheet", {
...
var set_timesheet_detail_rate = function(cdt, cdn, currency, timelog) {
	finergy.call({
		method: "capkpi.projects.doctype.timesheet.timesheet.get_timesheet_detail_rate",
		args: {
			timelog: timelog,
			currency: currency
		},
		callback: function(r) {
			if (!r.exc && r.message) {
				finergy.model.set_value(cdt, cdn, 'billing_amount', r.message);
			}
		}
	});
}
...
var get_healthcare_services_to_invoice = function(frm) {
	...
	dialog.fields_dict["patient"].df.onchange = () => {
		...
		if(patient && patient!=selected_patient){
			...
			var method = "capkpi.healthcare.utils.get_healthcare_services_to_invoice";
...
var make_list_row= function(columns, invoice_healthcare_services, result={}) {
	...
	columns.forEach(function(column) {
		contents += `<div class="list-item__content ellipsis">
			${
				head ? `<span class="ellipsis">${__(finergy.model.unscrub(column))}</span>`
...
var set_primary_action= function(frm, dialog, $results, invoice_healthcare_services) {
	...
	dialog.set_primary_action(__('Add'), function() {
		...
		if(checked_values.length > 0){
			...
			if(invoice_healthcare_services){
				finergy.msgprint(__("Please select Healthcare Service"));
			}
			else{
				finergy.msgprint(__("Please select Drug"));
			}
		}
	});
};
...
var get_drugs_to_invoice = function(frm) {
	...
	dialog.fields_dict["encounter"].df.onchange = () => {
		...
		if(encounter && encounter!=selected_encounter){
			...
			var method = "capkpi.healthcare.utils.get_drugs_to_invoice";
...
var add_to_item_line = function(frm, checked_values, invoice_healthcare_services){
	if(invoice_healthcare_services){
		...
		for(let i=0; i<checked_values.length; i++){
			var si_item = finergy.model.add_child(frm.doc, 'Sales Invoice Item', 'items');
			finergy.model.set_value(si_item.doctype, si_item.name, 'item_code', checked_values[i]['item']);
			finergy.model.set_value(si_item.doctype, si_item.name, 'qty', 1);
			if(checked_values[i]['qty'] > 1){
				finergy.model.set_value(si_item.doctype, si_item.name, 'qty', parseFloat(checked_values[i]['qty']));
			}
		}
		frm.refresh_fields();
	}
