...
def create_opening_entry(pos_profile, user):
	entry = finergy.new_doc("POS Opening Entry")
	entry.pos_profile = pos_profile.name
	entry.user = user
	entry.company = pos_profile.company
	entry.period_start_date = finergy.utils.get_datetime()

	balance_details = []
	for d in pos_profile.payments:
		balance_details.append(finergy._dict({"mode_of_payment": d.mode_of_payment}))
