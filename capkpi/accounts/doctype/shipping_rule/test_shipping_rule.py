...
from capkpi.accounts.doctype.shipping_rule.shipping_rule import (
	FromGreaterThanToError,
	ManyBlankToValuesError,
	OverlappingConditionError,
)

test_records = finergy.get_test_records("Shipping Rule")


class TestShippingRule(unittest.TestCase):
	def test_from_greater_than_to(self):
		shipping_rule = finergy.copy_doc(test_records[0])
...
def create_shipping_rule(shipping_rule_type, shipping_rule_name):
	...
	if finergy.db.exists("Shipping Rule", shipping_rule_name):
		return finergy.get_doc("Shipping Rule", shipping_rule_name)

	sr = finergy.new_doc("Shipping Rule")
