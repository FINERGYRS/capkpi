...
class OverlappingConditionError(finergy.ValidationError):
	pass


class FromGreaterThanToError(finergy.ValidationError):
	pass


class ManyBlankToValuesError(finergy.ValidationError):
	pass
	...
	def add_shipping_rule_to_tax_table(self, doc, shipping_amount):
		...
		if self.shipping_rule_type == "Selling":
			...
			if not doc.meta.get_field("taxes").options == "Sales Taxes and Charges":
				finergy.throw(_("Shipping rule only applicable for Selling"))
			...
			if not doc.meta.get_field("taxes").options == "Purchase Taxes and Charges":
				finergy.throw(_("Shipping rule only applicable for Buying"))
