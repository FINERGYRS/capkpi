...
from finergy.utils import add_days, format_date, getdate


class MainCostCenterCantBeChild(finergy.ValidationError):
	pass


class InvalidMainCostCenter(finergy.ValidationError):
	pass


class InvalidChildCostCenter(finergy.ValidationError):
	pass


class WrongPercentageAllocation(finergy.ValidationError):
	pass


class InvalidDateError(finergy.ValidationError):
	pass
...
class CostCenterAllocation(Document):
	...
	def validate_from_date_based_on_existing_gle(self):
		...
		last_gle_date = finergy.db.get_value(
			"GL Entry",
	...
	def validate_backdated_allocation(self):
		...
		future_allocation = finergy.db.get_value(
		...
		if future_allocation:
			finergy.msgprint(
				_(
					...
					finergy.bold(future_allocation.name),
					finergy.bold(format_date(future_allocation.valid_from)),
					finergy.bold(format_date(add_days(future_allocation.valid_from, -1))),
				),
				title=_("Warning!"),
				indicator="orange",
				alert=1,
	...
	def validate_child_cost_centers(self):
		...
		main_cost_centers = [
			d.main_cost_center
			for d in finergy.get_all("Cost Center Allocation", {"docstatus": 1}, "main_cost_center")
		]
