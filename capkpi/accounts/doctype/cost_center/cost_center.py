...
class CostCenter(NestedSet):
	...
	def validate_distributed_cost_center(self):
		if cint(self.enable_distributed_cost_center):
			if not self.distributed_cost_center:
				finergy.throw(_("Please enter distributed cost center"))
			...
			if not self.get("__islocal"):
				if (
					not cint(
						finergy.get_cached_value("Cost Center", {"name": self.name}, "enable_distributed_cost_center")
					)
				...
				if next((True for x in self.distributed_cost_center if x.cost_center == x.parent), False):
					finergy.throw(_("Parent Cost Center cannot be added in Distributed Cost Center"))
	...
	def validate_mandatory(self):
		if self.cost_center_name != self.company and not self.parent_cost_center:
			finergy.throw(_("Please enter parent cost center"))
		elif self.cost_center_name == self.company and self.parent_cost_center:
			finergy.throw(_("Root cannot have a parent cost center"))

	def validate_parent_cost_center(self):
		if self.parent_cost_center:
			if not finergy.db.get_value("Cost Center", self.parent_cost_center, "is_group"):
				finergy.throw(
					_("{0} is not a group node. Please select a group node as parent cost center").format(
						finergy.bold(self.parent_cost_center)
					)
				)
	...
	def convert_group_to_ledger(self):
		if self.check_if_child_exists():
			finergy.throw(_("Cannot convert Cost Center to ledger as it has child nodes"))
		elif self.check_gle_exists():
			finergy.throw(_("Cost Center with existing transactions can not be converted to ledger"))
		else:
			self.is_group = 0
			self.save()
			return 1
	...
	def convert_ledger_to_group(self):
		...
		if self.check_gle_exists():
			finergy.throw(_("Cost Center with existing transactions can not be converted to group"))
		self.is_group = 1
		self.save()
		return 1

	def check_gle_exists(self):
		return finergy.db.get_value("GL Entry", {"cost_center": self.name})
	...
	def check_if_part_of_distributed_cost_center(self):
		return finergy.db.get_value("Distributed Cost Center", {"cost_center": self.name})

	def before_rename(self, olddn, newdn, merge=False):
		# Add company abbr if not provided
		from capkpi.setup.doctype.company.company import get_name_with_abbr
	...
	def after_rename(self, olddn, newdn, merge=False):
		...
		if not merge:
			new_cost_center = finergy.db.get_value(
...
def on_doctype_update():
	finergy.db.add_index("Cost Center", ["lft", "rgt"])
