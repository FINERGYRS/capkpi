...
class TestCostCenter(unittest.TestCase):
	...
	def test_validate_distributed_cost_center(self):
		...
		if not finergy.db.get_value("Cost Center", {"name": "_Test Cost Center - _TC"}):
			finergy.get_doc(test_records[0]).insert()

		if not finergy.db.get_value("Cost Center", {"name": "_Test Cost Center 2 - _TC"}):
			finergy.get_doc(test_records[1]).insert()

		invalid_distributed_cost_center = finergy.get_doc(
			{
				"company": "_Test Company",
		...
		self.assertRaises(finergy.ValidationError, invalid_distributed_cost_center.save)
