...
finergy.ui.form.on('Cost Center', {
	onload: function(frm) {
	...
	refresh: function(frm) {
		...
		if(!frm.doc.__islocal) {
			frm.add_custom_button(__('Chart of Cost Centers'),
				function() { finergy.set_route("Tree", "Cost Center"); });

			frm.add_custom_button(__('Budget'),
				function() { finergy.set_route("List", "Budget", {'cost_center': frm.doc.name}); });
		}
	},
	update_cost_center_number: function(frm) {
		var d = new finergy.ui.Dialog({
			...
			primary_action: function() {
				...
				finergy.dom.freeze();
				finergy.call({
					method: "capkpi.accounts.utils.update_cost_center",
					args: {
					...
					callback: function(r) {
						...
						if(!r.exc) {
							if(r.message) {
								finergy.set_route("Form", "Cost Center", r.message);
							} else {
	...
	parent_cost_center(frm) {
		if(!frm.doc.company) {
			finergy.msgprint(__('Please enter company name first'));
		}
	},
