...
finergy.provide("capkpi.cheque_print");

finergy.ui.form.on('Cheque Print Template', {
	refresh: function(frm) {
		if(!frm.doc.__islocal) {
			frm.add_custom_button(frm.doc.has_print_format?__("Update Print Format"):__("Create Print Format"),
				function() {
					capkpi.cheque_print.view_cheque_print(frm);
			...
			var template = '<div style="position: relative; overflow-x: scroll;">\
				<div id="cheque_preview" style="width: {{ cheque_width }}cm; \
					...
					<span style="top: {{ date_dist_from_top_edge }}cm;\
						...
						position: absolute;"> {{ finergy.datetime.obj_to_user() }} </span>\
			...
			$(finergy.render(template, frm.doc)).appendTo(frm.fields_dict.cheque_print_preview.wrapper)
...
capkpi.cheque_print.view_cheque_print = function(frm) {
	finergy.call({
		method: "capkpi.accounts.doctype.cheque_print_template.cheque_print_template.create_or_update_cheque_print_format",
		args:{
		...
		callback: function(r) {
			if (!r.exe && !frm.doc.has_print_format) {
				...
				finergy.msgprint(__("Print settings updated in respective print format"))
			}
		}
	})
}
