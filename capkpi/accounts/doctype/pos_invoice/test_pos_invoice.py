...
from capkpi.accounts.doctype.pos_invoice.pos_invoice import make_sales_return
...
class TestPOSInvoice(unittest.TestCase):
	...
	def tearDown(self):
		...
		if finergy.db.get_single_value("Selling Settings", "validate_selling_price"):
			finergy.db.set_value("Selling Settings", None, "validate_selling_price", 0)
	...
	def test_change_naming_series(self):
		...
		self.assertRaises(finergy.CannotChangeConstantError, inv.save)
	...
	def test_without_payment(self):
		...
		inv.payments = []
		self.assertRaises(finergy.ValidationError, inv.insert)
	...
	def test_serialized_item_transaction(self):
		...
		pos2.insert()
		self.assertRaises(finergy.ValidationError, pos2.submit)
	...
	def test_invalid_serial_no_validation(self):
		...
		self.assertRaises(finergy.ValidationError, pos.submit)

	def test_loyalty_points(self):
		from capkpi.accounts.doctype.loyalty_program.loyalty_program import (
			get_loyalty_program_details_with_points,
		)
		from capkpi.accounts.doctype.loyalty_program.test_loyalty_program import create_records
	...
	def test_merging_into_sales_invoice_with_discount(self):
		from capkpi.accounts.doctype.pos_closing_entry.test_pos_closing_entry import (
			init_user_and_profile,
		)
		from capkpi.accounts.doctype.pos_invoice_merge_log.pos_invoice_merge_log import (
			consolidate_pos_invoices,
		)

		finergy.db.sql("delete from `tabPOS Invoice`")
		...
		rounded_total = finergy.db.get_value(
	...
	def test_merging_with_validate_selling_price(self):
		...
		if not finergy.db.get_single_value("Selling Settings", "validate_selling_price"):
			finergy.db.set_value("Selling Settings", "Selling Settings", "validate_selling_price", 1)
		...
		self.assertRaises(finergy.ValidationError, pos_inv.submit)
	...
	def test_pos_batch_item_qty_validation(self):
		...
		item = finergy.get_doc("Item", "_BATCH ITEM")
		batch = finergy.get_doc("Batch", "TestBatch 01")
		batch.submit()
		...
		self.assertRaises(finergy.ValidationError, pos_inv2.submit)
	...
	def test_ignore_pricing_rule(self):
		from capkpi.accounts.doctype.pricing_rule.test_pricing_rule import make_pricing_rule
	...
	def test_delivered_serial_no_case(self):
		from capkpi.accounts.doctype.pos_invoice_merge_log.test_pos_invoice_merge_log import (
			init_user_and_profile,
		)
		from capkpi.stock.doctype.delivery_note.test_delivery_note import create_delivery_note
		from capkpi.stock.doctype.serial_no.test_serial_no import get_serial_nos
		from capkpi.stock.doctype.stock_entry.test_stock_entry import make_serialized_item

		finergy.db.savepoint("before_test_delivered_serial_no_case")
		try:
			se = make_serialized_item()
			...
			delivery_document_no = finergy.db.get_value("Serial No", serial_no, "delivery_document_no")
			...
			finergy.db.rollback(save_point="before_test_delivered_serial_no_case")
			finergy.set_user("Administrator")
	...
	def test_returned_serial_no_case(self):
		...
		from capkpi.stock.doctype.serial_no.serial_no import get_pos_reserved_serial_nos
		from capkpi.stock.doctype.serial_no.test_serial_no import get_serial_nos
		from capkpi.stock.doctype.stock_entry.test_stock_entry import make_serialized_item

		finergy.db.savepoint("before_test_returned_serial_no_case")
		try:
			se = make_serialized_item()
			serial_no = get_serial_nos(se.get("items")[0].serial_no)[0]
			...
			finergy.db.rollback(save_point="before_test_returned_serial_no_case")
			finergy.set_user("Administrator")
...
def create_pos_invoice(**args):
	...
	pos_inv = finergy.new_doc("POS Invoice")
	pos_inv.update(args)
	...
	pos_inv.posting_date = args.posting_date or finergy.utils.nowdate()
...
def make_batch_item(item_name):
	...
	if not finergy.db.exists(item_name):
