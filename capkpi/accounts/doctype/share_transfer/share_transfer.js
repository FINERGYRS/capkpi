...
finergy.provide("capkpi.share_transfer");

finergy.ui.form.on('Share Transfer', {
	refresh: function(frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.equity_or_liability_account && frm.doc.asset_account) {
			frm.add_custom_button(__('Create Journal Entry'), function () {
				capkpi.share_transfer.make_jv(frm);
			});
		}
	...
	no_of_shares: (frm) => {
		if (frm.doc.rate != undefined || frm.doc.rate != null){
			capkpi.share_transfer.update_amount(frm);
		}
	},
	rate: (frm) => {
	...
	company: async function(frm) {
		if (frm.doc.company) {
			let currency = (await finergy.db.get_value("Company", frm.doc.company, "default_currency")).message.default_currency;
...
capkpi.share_transfer.update_amount = function(frm) {
	frm.doc.amount = frm.doc.no_of_shares * frm.doc.rate;
	frm.refresh_field("amount");
};

capkpi.share_transfer.make_jv = function (frm) {
	...
	finergy.call({
		...
		method: "capkpi.accounts.doctype.share_transfer.share_transfer.make_jv_entry",
		callback: function (r) {
