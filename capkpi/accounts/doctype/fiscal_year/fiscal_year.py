...
from finergy.utils import add_days, add_years, cstr, getdate


class FiscalYearIncorrectDate(finergy.ValidationError):
	pass
...
class FiscalYear(Document):
	...
	def set_as_default(self):
		finergy.db.set_value("Global Defaults", None, "current_fiscal_year", self.name)
		global_defaults = finergy.get_doc("Global Defaults")
	...
	def validate(self):
		...
		if not self.is_new():
			year_start_end_dates = finergy.db.sql(
	...
	def on_update(self):
		...
		finergy.cache().delete_value("fiscal_years")
	...
	def validate_overlap(self):
		existing_fiscal_years = finergy.db.sql(
		...
		if existing_fiscal_years:
			for existing in existing_fiscal_years:
				company_for_existing = finergy.db.sql_list(
...
def auto_create_fiscal_year():
	for d in finergy.db.sql(
		...
		try:
			current_fy = finergy.get_doc("Fiscal Year", d[0])

			new_fy = finergy.copy_doc(current_fy, ignore_no_copy=False)
...
def get_from_and_to_date(fiscal_year):
	...
	return finergy.db.get_value("Fiscal Year", fiscal_year, fields, as_dict=1)
