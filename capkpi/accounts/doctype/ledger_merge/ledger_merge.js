...
finergy.ui.form.on('Ledger Merge', {
	setup: function(frm) {
		finergy.realtime.on('ledger_merge_refresh', ({ ledger_merge }) => {
			if (ledger_merge !== frm.doc.name) return;
			finergy.model.clear_doc(frm.doc.doctype, frm.doc.name);
			finergy.model.with_doc(frm.doc.doctype, frm.doc.name).then(() => {
				frm.refresh();
			});
		});

		finergy.realtime.on('ledger_merge_progress', data => {
		...
		frm.set_query("account", function(doc) {
			if (!doc.company) finergy.throw(__('Please set Company'));
			if (!doc.root_type) finergy.throw(__('Please set Root Type'));
			return {
				filters: {
		...
		frm.set_query('account', 'merge_accounts', function(doc) {
			if (!doc.company) finergy.throw(__('Please set Company'));
			if (!doc.root_type) finergy.throw(__('Please set Root Type'));
			if (!doc.account) finergy.throw(__('Please set Account'));
			let acc = [doc.account];
...
finergy.ui.form.on('Ledger Merge Accounts', {
