...
import finergy
from finergy import ValidationError, _, qb, scrub, throw
from finergy.utils import cint, comma_or, flt, getdate, nowdate

import capkpi
from capkpi.accounts.doctype.bank_account.bank_account import (
	get_bank_account_details,
	get_party_bank_account,
)
from capkpi.accounts.doctype.invoice_discounting.invoice_discounting import (
	get_party_account_based_on_invoice_discounting,
)
from capkpi.accounts.doctype.journal_entry.journal_entry import get_default_bank_cash_account
from capkpi.accounts.doctype.tax_withholding_category.tax_withholding_category import (
	get_party_tax_withholding_details,
)
from capkpi.accounts.general_ledger import make_gl_entries, process_gl_map
from capkpi.accounts.party import get_party_account
from capkpi.accounts.utils import get_account_currency, get_balance_on, get_outstanding_invoices
...
class PaymentEntry(AccountsController):
	...
	def on_submit(self):
		if self.difference_amount:
			finergy.throw(_("Difference Amount must be zero"))
	...
	def set_payment_req_status(self):
		from capkpi.accounts.doctype.payment_request.payment_request import update_payment_req_status
	...
	def delink_advance_entry_references(self):
		for reference in self.references:
			if reference.reference_doctype in ("Sales Invoice", "Purchase Invoice"):
				doc = finergy.get_doc(reference.reference_doctype, reference.reference_name)
	...
	def set_missing_values(self):
		if self.payment_type == "Internal Transfer":
			...
			if not self.party_type:
				finergy.throw(_("Party Type is mandatory"))

			if not self.party:
				finergy.throw(_("Party is mandatory"))

			_party_name = "title" if self.party_type == "Shareholder" else self.party_type.lower() + "_name"

			if finergy.db.has_column(self.party_type, _party_name):
				self.party_name = finergy.db.get_value(self.party_type, self.party, _party_name)
			else:
				self.party_name = finergy.db.get_value(self.party_type, self.party, "name")
	...
	def validate_payment_type(self):
		if self.payment_type not in ("Receive", "Pay", "Internal Transfer"):
			finergy.throw(_("Payment Type must be one of Receive, Pay and Internal Transfer"))

	def validate_party_details(self):
		if self.party:
			if not finergy.db.exists(self.party_type, self.party):
				finergy.throw(_("Invalid {0}: {1}").format(self.party_type, self.party))

			if self.party_account and self.party_type in ("Customer", "Supplier"):
				self.validate_account_type(
					self.party_account, [capkpi.get_party_account_type(self.party_type)]
				)
	...
	def validate_account_type(self, account, account_types):
		account_type = finergy.db.get_value("Account", account, "account_type")
		# if account_type not in account_types:
		# 	finergy.throw(_("Account Type for {0} must be {1}").format(account, comma_or(account_types)))
	...
	def validate_mandatory(self):
		for field in ("paid_amount", "received_amount", "source_exchange_rate", "target_exchange_rate"):
			if not self.get(field):
				finergy.throw(_("{0} is mandatory").format(self.meta.get_label(field)))
	...
	def validate_reference_documents(self):
		...
		for d in self.get("references"):
			...
			if d.reference_doctype not in valid_reference_doctypes:
				...
				if not finergy.db.exists(d.reference_doctype, d.reference_name):
					finergy.throw(_("{0} {1} does not exist").format(d.reference_doctype, d.reference_name))
				else:
					ref_doc = finergy.get_doc(d.reference_doctype, d.reference_name)
					...
					if d.reference_doctype in finergy.get_hooks("invoice_doctypes"):
					...
					if ref_doc.docstatus != 1:
						finergy.throw(_("{0} {1} must be submitted").format(d.reference_doctype, d.reference_name))
	...
	def validate_paid_invoices(self):
		...
		for d in self.get("references"):
			...
			if d.reference_doctype in ("Sales Invoice", "Purchase Invoice"):
				outstanding_amount, is_return = finergy.get_cached_value(
		...
		for k, v in no_oustanding_refs.items():
			finergy.msgprint(
				_(
					...
					_(k),
					finergy.bold(", ".join(d.reference_name for d in v)),
					finergy.bold(_("negative outstanding amount")),
				)
				+ "<br><br>"
	...
	def update_payment_schedule(self, cancel=0):
		...
		for ref in self.get("references"):
			if ref.payment_term and ref.reference_name:
				...
				if not invoice_paid_amount_map.get(key):
					payment_schedule = finergy.get_all(
						"Payment Schedule",
		...
		for idx, (key, allocated_amount) in enumerate(invoice_payment_amount_map.items(), 1):
			if not invoice_paid_amount_map.get(key):
				finergy.throw(_("Payment term {0} not used in {1}").format(key[0], key[1]))
	...
	def validate_received_amount(self):
		if self.paid_from_account_currency == self.paid_to_account_currency:
			if self.paid_amount < self.received_amount:
				finergy.throw(_("Received Amount cannot be greater than Paid Amount"))
	...
	def set_title(self):
		if finergy.flags.in_import and self.title:
	...
	def validate_transaction_reference(self):
		...
		bank_account_type = finergy.db.get_value("Account", bank_account, "account_type")

		if bank_account_type == "Bank":
			if not self.reference_no or not self.reference_date:
				finergy.throw(_("Reference No and Reference Date is mandatory for Bank transaction"))
	...
	def add_party_gl_entries(self, gl_entries):
		if self.party_account:
			...
			dr_or_cr = (
				"credit" if capkpi.get_party_account_type(self.party_type) == "Receivable" else "debit"
			)

			for d in self.get("references"):
				cost_center = self.cost_center
				if d.reference_doctype == "Sales Invoice" and not cost_center:
					cost_center = finergy.db.get_value(d.reference_doctype, d.reference_name, "cost_center")
	...
	def add_tax_gl_entries(self, gl_entries):
		for d in self.get("taxes"):
			...
			if account_currency != self.company_currency:
				finergy.throw(_("Currency for {0} must be {1}").format(d.account_head, self.company_currency))
	...
	def add_deductions_gl_entries(self, gl_entries):
		for d in self.get("deductions"):
			if d.amount:
				...
				if account_currency != self.company_currency:
					finergy.throw(_("Currency for {0} must be {1}").format(d.account, self.company_currency))
	...
	def update_advance_paid(self):
		if self.payment_type in ("Receive", "Pay") and self.party:
			for d in self.get("references"):
				if d.allocated_amount and d.reference_doctype in finergy.get_hooks("advance_payment_doctypes"):
					finergy.get_doc(d.reference_doctype, d.reference_name).set_total_advance_paid()
	...
	def calculate_deductions(self, tax_details):
		return {
			...
			"cost_center": finergy.get_cached_value("Company", self.company, "cost_center"),
...
def get_outstanding_reference_documents(args):
	...
	company_currency = finergy.get_cached_value("Company", args.get("company"), "default_currency")

	# Get positive outstanding sales /purchase invoices
	condition = ""
	if args.get("voucher_type") and args.get("voucher_no"):
		condition = " and voucher_type={0} and voucher_no={1}".format(
			finergy.db.escape(args["voucher_type"]), finergy.db.escape(args["voucher_no"])
		)
	...
	if args.get("company"):
		condition += " and company = {0}".format(finergy.db.escape(args.get("company")))
	...
	for d in outstanding_invoices:
		...
		if party_account_currency != company_currency:
			if d.voucher_type in finergy.get_hooks("invoice_doctypes"):
				d["exchange_rate"] = finergy.db.get_value(d.voucher_type, d.voucher_no, "conversion_rate")
			elif d.voucher_type == "Journal Entry":
				d["exchange_rate"] = get_exchange_rate(
					party_account_currency, company_currency, d.posting_date
				)
		if d.voucher_type in ("Purchase Invoice"):
			d["bill_no"] = finergy.db.get_value(d.voucher_type, d.voucher_no, "bill_no")
	...
	if not data:
		finergy.msgprint(
			...
			).format(_(args.get("party_type")).lower(), finergy.bold(args.get("party")))
		)
...
def split_invoices_based_on_payment_terms(outstanding_invoices):
	...
	for idx, d in enumerate(outstanding_invoices):
		if d.voucher_type in ["Sales Invoice", "Purchase Invoice"]:
			payment_term_template = finergy.db.get_value(
				d.voucher_type, d.voucher_no, "payment_terms_template"
			)
			if payment_term_template:
				allocate_payment_based_on_payment_terms = finergy.db.get_value(
...
def get_orders_to_be_billed(
	...
	# Add cost center condition
	if voucher_type:
		doc = finergy.get_doc({"doctype": voucher_type})
		condition = ""
...
def get_party_details(company, party_type, party, date, cost_center=None):
	bank_account = ""
	if not finergy.db.exists(party_type, party):
		finergy.throw(_("Invalid {0}: {1}").format(party_type, party))
	...
	party_name = finergy.db.get_value(party_type, party, _party_name)
...
def get_account_details(account, date, cost_center=None):
	finergy.has_permission("Payment Entry", throw=True)

	# to check if the passed account is accessible under reference doctype Payment Entry
	account_list = finergy.get_list(
	...
	if not account_list:
		finergy.throw(_("Account: {0} is not permitted under Payment Entry").format(account))
...
def get_company_defaults(company):
	...
	ret = finergy.get_cached_value("Company", company, fields, as_dict=1)
...
def get_reference_details(reference_doctype, reference_name, party_account_currency):
	...
	ref_doc = finergy.get_doc(reference_doctype, reference_name)
	company_currency = ref_doc.get("company_currency") or capkpi.get_company_currency(
		ref_doc.company
	)
...
def get_payment_entry(
	...
	if dt in ("Sales Order", "Purchase Order") and flt(doc.per_billed, 2) > 0:
		finergy.throw(_("Can only make payment against unbilled {0}").format(dt))
	...
	pe = finergy.new_doc("Payment Entry")
	...
	if doc.doctype == "Purchase Invoice" and doc.invoice_is_blocked():
		finergy.msgprint(_("{0} is on hold till {1}").format(doc.name, doc.release_date))
	else:
		if doc.doctype in ("Sales Invoice", "Purchase Invoice") and finergy.get_value(
	...
	if party_account and bank:
		...
		if discount_amount:
			pe.set_gain_or_loss(
				account_details={
					"account": finergy.get_cached_value("Company", pe.company, "default_discount_account"),
					"cost_center": pe.cost_center
					or finergy.get_cached_value("Company", pe.company, "cost_center"),
...
def apply_early_payment_discount(paid_amount, received_amount, doc):
	...
	if doc.doctype in eligible_for_payments and has_payment_schedule:
		...
		if total_discount:
			money = finergy.utils.fmt_money(total_discount, currency=doc.get("currency"))
			finergy.msgprint(_("Discount of {} applied as per Payment Term").format(money), alert=1)
