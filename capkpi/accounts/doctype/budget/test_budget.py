...
import finergy
from finergy.utils import now_datetime, nowdate

from capkpi.accounts.doctype.budget.budget import BudgetError, get_actual_expense
...
class TestBudget(unittest.TestCase):
	def test_monthly_budget_crossed_ignore(self):
		...
		self.assertTrue(
			finergy.db.get_value("GL Entry", {"voucher_type": "Journal Entry", "voucher_no": jv.name})
		)
	...
	def test_exception_approver_role(self):
		...
		finergy.db.set_value("Company", budget.company, "exception_budget_approver_role", "Accounts User")

		jv.submit()
		self.assertEqual(finergy.db.get_value("Journal Entry", jv.name, "docstatus"), 1)
		jv.cancel()

		finergy.db.set_value("Company", budget.company, "exception_budget_approver_role", "")
	...
	def test_monthly_budget_crossed_for_mr(self):
		...
		finergy.db.set_value("Budget", budget.name, "fiscal_year", fiscal_year)
	...
	def test_monthly_budget_crossed_stop2(self):
		...
		project = finergy.get_value("Project", {"project_name": "_Test Project"})
	...
	def test_monthly_budget_against_parent_group_cost_center(self):
		...
		if not finergy.db.exists("Cost Center", cost_center):
			finergy.get_doc(
				{
					"doctype": "Cost Center",
...
def set_total_expense_zero(posting_date, budget_against_field=None, budget_against_CC=None):
	if budget_against_field == "project":
		budget_against = finergy.db.get_value("Project", {"project_name": "_Test Project"})
	else:
...
def make_budget(**args):
	...
	if budget_against == "Project":
		...
		budget_list = finergy.get_all("Budget", fields=["name"], filters={"name": ("like", project_name)})
	else:
		cost_center_name = "{0}%".format(cost_center or "_Test Cost Center - _TC/" + fiscal_year)
		budget_list = finergy.get_all(
			"Budget", fields=["name"], filters={"name": ("like", cost_center_name)}
		)
	for d in budget_list:
		finergy.db.sql("delete from `tabBudget` where name = %(name)s", d)
		finergy.db.sql("delete from `tabBudget Account` where parent = %(name)s", d)

	budget = finergy.new_doc("Budget")

	if budget_against == "Project":
		budget.project = finergy.get_value("Project", {"project_name": "_Test Project"})
	else:
		budget.cost_center = cost_center or "_Test Cost Center - _TC"

	monthly_distribution = finergy.get_doc("Monthly Distribution", "_Test Distribution")
