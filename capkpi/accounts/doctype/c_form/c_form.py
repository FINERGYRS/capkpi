...
class CForm(Document):
		...
		for d in self.get("invoices"):
			if d.invoice_no:
				...
				if inv and inv[0][0] != "Yes":
					finergy.throw(_("C-form is not applicable for Invoice: {0}").format(d.invoice_no))
	...
	def before_cancel(self):
		...
		finergy.db.sql("""update `tabSales Invoice` set c_form_no=null where c_form_no=%s""", self.name)
	...
	def set_cform_in_sales_invoices(self):
		...
		if inv:
			...
			finergy.throw(_("Please enter atleast 1 invoice in the table"))

	def set_total_invoiced_amount(self):
		total = sum(flt(d.grand_total) for d in self.get("invoices"))
		finergy.db.set(self, "total_invoiced_amount", total)

	@finergy.whitelist()
	def get_invoice_details(self, invoice_no):
		"""Pull details from invoices for referrence"""
		if invoice_no:
			inv = finergy.db.get_value(
				"Sales Invoice",
				invoice_no,
