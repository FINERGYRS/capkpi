...
from finergy.utils import cint, cstr, flt, formatdate, getdate, now
...
from capkpi.accounts.utils import create_payment_ledger_entry


class ClosedAccountingPeriod(finergy.ValidationError):
	pass
...
def validate_disabled_accounts(gl_map):
	...
	Account = finergy.qb.DocType("Account")

	disabled_accounts = (
		finergy.qb.from_(Account)
	...
	if disabled_accounts:
		account_list = "<br>"
		account_list += ", ".join([finergy.bold(d.name) for d in disabled_accounts])
		finergy.throw(
...
def validate_accounting_period(gl_map):
	accounting_periods = finergy.db.sql(
		""" SELECT
			ap.name as name
		FROM
	...
	if accounting_periods:
		finergy.throw(
			...
			).format(finergy.bold(accounting_periods[0].name)),
...
def get_cost_center_allocation_data(company, posting_date):
	par = finergy.qb.DocType("Cost Center Allocation")
	child = finergy.qb.DocType("Cost Center Allocation Percentage")

	records = (
		finergy.qb.from_(par)
		.inner_join(child)
		...
		.orderby(par.valid_from, order=finergy.qb.desc)
	).run(as_dict=True)

	cc_allocation = finergy._dict()
	for d in records:
		cc_allocation.setdefault(d.main_cost_center, finergy._dict()).setdefault(
...
def merge_similar_entries(gl_map, precision=None):
	...
	company = gl_map[0].company if gl_map else capkpi.get_default_company()
...
def validate_cwip_accounts(gl_map):
	...
	cwip_enabled = any(
		...
		for ac in finergy.db.get_all("Asset Category", "enable_cwip_accounting")
	)
	if cwip_enabled:
		cwip_accounts = [
			d[0]
...
def process_debit_credit_difference(gl_map):
	precision = get_field_precision(
		finergy.get_meta("GL Entry").get_field("debit"),
		currency=finergy.get_cached_value("Company", gl_map[0].company, "default_currency"),
	)
...
def update_accounting_dimensions(round_off_gle):
	...
	meta = finergy.get_meta(round_off_gle["voucher_type"])
	...
	if dimensions and has_all_dimensions:
		dimension_values = finergy.db.get_value(
...
def get_round_off_account_and_cost_center(company, voucher_type, voucher_no):
	round_off_account, round_off_cost_center = finergy.get_cached_value(
		"Company", company, ["round_off_account", "round_off_cost_center"]
	) or [None, None]

	meta = finergy.get_meta(voucher_type)

	# Give first preference to parent cost center for round off GLE
	if meta.has_field("cost_center"):
		parent_cost_center = finergy.db.get_value(voucher_type, voucher_no, "cost_center")
		if parent_cost_center:
			round_off_cost_center = parent_cost_center

	if not round_off_account:
		finergy.throw(_("Please mention Round Off Account in Company"))

	if not round_off_cost_center:
		finergy.throw(_("Please mention Round Off Cost Center in Company"))
...
def make_reverse_gl_entries(
	...
	if not gl_entries:
		gl_entry = finergy.qb.DocType("GL Entry")
		gl_entries = (
			finergy.qb.from_(gl_entry)
			.select("*")
	...
	if not adv_adj:
		...
		if acc_frozen_upto:
			...
			if getdate(posting_date) <= getdate(acc_frozen_upto) and (
				frozen_accounts_modifier not in finergy.get_roles() or finergy.session.user == "Administrator"
			):
				finergy.throw(
