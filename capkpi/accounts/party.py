...
from finergy.contacts.doctype.contact.contact import get_contact_details
from finergy.core.doctype.user_permission.user_permission import get_permitted_documents
...
from capkpi.exceptions import InvalidAccountCurrency, PartyDisabled, PartyFrozen


class DuplicatePartyAccountError(finergy.ValidationError):
	pass
...
def get_party_details(
	...
	if not finergy.db.exists(party_type, party):
		finergy.throw(_("{0}: {1} does not exists").format(party_type, party))
...
def _get_party_details(
	...
	if not ignore_permissions and not (
		finergy.has_permission(party_type, "read", party)
		or finergy.has_permission(party_type, "select", party)
	):
		finergy.throw(_("Not permitted for {0}").format(party), finergy.PermissionError)

	party = finergy.get_doc(party_type, party)
	...
	if party_type == "Supplier" and party:
		party_details["supplier_tds"] = finergy.get_value(
...
def get_default_price_list(party):
	...
	if party.doctype == "Customer":
		return finergy.db.get_value("Customer Group", party.customer_group, "default_price_list")
...
def set_price_list(party_details, party, party_type, given_price_list, pos=None):
	...
	if price_list and len(price_list) == 1:
		...
		customer_price_list = finergy.get_value("Customer", party.name, "default_price_list")

		if customer_price_list:
			price_list = customer_price_list
		else:
			pos_price_list = finergy.get_value("POS Profile", pos, "selling_price_list")
			price_list = pos_price_list or given_price_list
	else:
		price_list = get_default_price_list(party) or given_price_list

	if price_list:
		party_details.price_list_currency = finergy.db.get_value(
...
def get_party_account(party_type, party=None, company=None):
	...
	if not company:
		finergy.throw(_("Please select a Company"))
	...
	if not party and party_type in ["Customer", "Supplier"]:
		...
		return finergy.get_cached_value("Company", company, default_account_name)
	...
	if not account and party_type in ["Customer", "Supplier"]:
		...
		group = finergy.get_cached_value(party_type, party, scrub(party_group_doctype))
		account = finergy.db.get_value(
			"Party Account",
	...
	if not account and party_type in ["Customer", "Supplier"]:
		...
		account = finergy.get_cached_value("Company", company, default_account_name)

	existing_gle_currency = get_party_gle_currency(party_type, party, company)
	if existing_gle_currency:
		if account:
			account_currency = finergy.db.get_value("Account", account, "account_currency", cache=True)
...
def get_party_account_currency(party_type, party, company):
	def generator():
		...
		return finergy.db.get_value("Account", party_account, "account_currency", cache=True)

	return finergy.local_cache("party_account_currency", (party_type, party, company), generator)


def get_party_gle_currency(party_type, party, company):
	def generator():
		existing_gle_currency = finergy.db.sql(
...
def get_party_gle_account(party_type, party, company):
	def generator():
		existing_gle_account = finergy.db.sql(
	...
	if existing_gle_currency and party_account_currency != existing_gle_currency:
		finergy.throw(
			_(
				...
				finergy.bold(party_type),
				finergy.bold(party),
				finergy.bold(existing_gle_currency),
				finergy.bold(company),
			),
...
def validate_party_accounts(doc):
	...
	for account in doc.get("accounts"):
		...
		party_account_currency = finergy.db.get_value(
			"Account", account.account, "account_currency", cache=True
		)
		if finergy.db.get_default("Company"):
			company_default_currency = finergy.get_cached_value(
				"Company", finergy.db.get_default("Company"), "default_currency"
			)
		else:
			company_default_currency = finergy.db.get_value("Company", account.company, "default_currency")
	...
	if (bill_date or posting_date) and party:
		...
		if template_name:
			...
			if party_type == "Supplier":
				supplier_group = finergy.get_cached_value(party_type, party, "supplier_group")
				template_name = finergy.get_cached_value("Supplier Group", supplier_group, "payment_terms")
				if template_name:
					due_date = get_due_date_from_template(template_name, posting_date, bill_date).strftime(
						"%Y-%m-%d"
					)
...
def get_due_date_from_template(template_name, posting_date, bill_date):
	...
	template = finergy.get_doc("Payment Terms Template", template_name)
...
def validate_due_date(
	...
	if getdate(due_date) < getdate(posting_date):
		finergy.throw(_("Due Date cannot be before Posting / Supplier Invoice Date"))
	else:
		...
		if default_due_date != posting_date and getdate(due_date) > getdate(default_due_date):
			is_credit_controller = (
				finergy.db.get_single_value("Accounts Settings", "credit_controller") in finergy.get_roles()
			)
...
def get_address_tax_category(tax_category=None, billing_address=None, shipping_address=None):
	addr_tax_category_from = finergy.db.get_single_value(
		"Accounts Settings", "determine_address_tax_category_from"
	)
	if addr_tax_category_from == "Shipping Address":
		if shipping_address:
			tax_category = finergy.db.get_value("Address", shipping_address, "tax_category") or tax_category
	else:
		if billing_address:
			tax_category = finergy.db.get_value("Address", billing_address, "tax_category") or tax_category
...
def set_taxes(
	...
	from capkpi.accounts.doctype.tax_rule.tax_rule import get_party_details, get_tax_template
...
def get_payment_terms_template(party_name, party_type, company=None):
	...
	template = None
	if party_type == "Customer":
		customer = finergy.get_cached_value(
			"Customer", party_name, fieldname=["payment_terms", "customer_group"], as_dict=1
		)
		template = customer.payment_terms

		if not template and customer.customer_group:
			template = finergy.get_cached_value("Customer Group", customer.customer_group, "payment_terms")
	else:
		supplier = finergy.get_cached_value(
			"Supplier", party_name, fieldname=["payment_terms", "supplier_group"], as_dict=1
		)
		template = supplier.payment_terms
		if not template and supplier.supplier_group:
			template = finergy.get_cached_value("Supplier Group", supplier.supplier_group, "payment_terms")

	if not template and company:
		template = finergy.get_cached_value("Company", company, fieldname="payment_terms")
	return template


def validate_party_frozen_disabled(party_type, party_name):

	if finergy.flags.ignore_party_validation:
		return

	if party_type and party_name:
		if party_type in ("Customer", "Supplier"):
			party = finergy.get_cached_value(party_type, party_name, ["is_frozen", "disabled"], as_dict=True)
			if party.disabled:
				finergy.throw(_("{0} {1} is disabled").format(party_type, party_name), PartyDisabled)
			elif party.get("is_frozen"):
				frozen_accounts_modifier = finergy.db.get_single_value(
					"Accounts Settings", "frozen_accounts_modifier"
				)
				if not frozen_accounts_modifier in finergy.get_roles():
					finergy.throw(_("{0} {1} is frozen").format(party_type, party_name), PartyFrozen)

		elif party_type == "Employee":
			if finergy.db.get_value("Employee", party_name, "status") != "Active":
				finergy.msgprint(_("{0} {1} is not active").format(party_type, party_name), alert=True)
...
def get_dashboard_info(party_type, party, loyalty_program=None):
	...
	company_wise_grand_total = finergy.get_all(
		doctype,
		filters={
			"docstatus": 1,
			party_type.lower(): party,
	...
	if party_type == "Customer":
		loyalty_point_details = finergy._dict(
			finergy.get_all(
				"Loyalty Point Entry",
				filters={
					"customer": party,
	...
	company_wise_billing_this_year = finergy._dict()
	...
	company_wise_total_unpaid = finergy._dict(
		finergy.db.sql(
			"""
	...
	for d in companies:
		company_default_currency = finergy.db.get_value("Company", d.company, "default_currency")
...
def get_partywise_advanced_payment_amount(
	...
	if company:
		cond += "and company = {0}".format(finergy.db.escape(company))
...
def add_party_account(party_type, party, company, account):
	doc = finergy.get_doc(party_type, party)
