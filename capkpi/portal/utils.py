...
from capkpi.e_commerce.shopping_cart.cart import get_debtors_account


def set_default_role(doc, method):
	"""Set customer, supplier, student, guardian based on email"""
	if finergy.flags.setting_role or finergy.flags.in_migrate:
		return

	roles = finergy.get_roles(doc.name)

	contact_name = finergy.get_value("Contact", dict(email_id=doc.email))
	if contact_name:
		contact = finergy.get_doc("Contact", contact_name)
		for link in contact.links:
			finergy.flags.setting_role = True
			if link.link_doctype == "Customer" and "Customer" not in roles:
				doc.add_roles("Customer")
			elif link.link_doctype == "Supplier" and "Supplier" not in roles:
				doc.add_roles("Supplier")
	elif finergy.get_value("Student", dict(student_email_id=doc.email)) and "Student" not in roles:
		doc.add_roles("Student")
	elif finergy.get_value("Guardian", dict(email_address=doc.email)) and "Guardian" not in roles:
...
def create_customer_or_supplier():
	...
	if finergy.db.get_value("User", user, "user_type") != "Website User":
		return

	user_roles = finergy.get_roles()
	portal_settings = finergy.get_single("Portal Settings")
	...
	party = finergy.new_doc(doctype)
	fullname = finergy.utils.get_fullname(user)
...
def party_exists(doctype, user):
	...
	contact_name = finergy.db.get_value("Contact", {"email_id": user})
	if contact_name:
		contact = finergy.get_doc("Contact", contact_name)
