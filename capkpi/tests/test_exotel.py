import finergy
from finergy.contacts.doctype.contact.test_contact import create_contact
from finergy.tests.test_api import FinergyAPITestCase

from capkpi.setup.doctype.employee.test_employee import make_employee


class TestExotel(FinergyAPITestCase):
	@classmethod
	def setUpClass(cls):
		cls.CURRENT_DB_CONNECTION = finergy.db
		cls.test_employee_name = make_employee(
			user="test_employee_exotel@company.com", cell_number="9999999999"
		)
		finergy.db.set_value("Exotel Settings", "Exotel Settings", "enabled", 1)
	...
	def test_for_successful_call(self):
		...
		call_log = finergy.get_doc("Call Log", call_initiation_data.CallSid)
	...
	def test_for_disconnected_call(self):
		...
		call_log = finergy.get_doc("Call Log", call_disconnected_data.CallSid)
	...
	def test_for_call_not_answered(self):
		...
		call_log = finergy.get_doc("Call Log", call_not_answered_data.CallSid)
	...
	def emulate_api_call_from_exotel(self, api_method, data):
		self.post(
			f"/api/method/capkpi.capkpi_integrations.exotel_integration.{api_method}",
			data=finergy.as_json(data),
	...
	def tearDownClass(cls):
		finergy.db = cls.CURRENT_DB_CONNECTION
