...
def create_employee_records():
	...
	finergy.db.sql("DELETE FROM tabEmployee WHERE company='Test Org Chart'")
...
def create_company():
	company = finergy.db.exists("Company", "Test Org Chart")
	if not company:
		company = (
			finergy.get_doc(
				{
...
def create_employee(first_name, designation, reports_to=None):
	employee = finergy.db.exists("Employee", {"first_name": first_name, "designation": designation})
	if not employee:
		employee = (
			finergy.get_doc(
				{
...
def create_missing_designation():
	if not finergy.db.exists("Designation", "CTO"):
		finergy.get_doc({"doctype": "Designation", "designation_name": "CTO"}).insert()
