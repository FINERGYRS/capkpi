...
import finergy
from finergy.core.doctype.report.report import get_report_module_dotted_path
...
def create_test_contact_and_address():
	...
	finergy.db.sql("delete from tabAddress")
	finergy.db.sql("delete from `tabDynamic Link`")
...
def execute_script_report(
	...
	report_execute_fn = finergy.get_attr(
		get_report_module_dotted_path(module, report_name) + ".execute"
	)
	report_filters = finergy._dict(default_filters).copy().update(filters)
