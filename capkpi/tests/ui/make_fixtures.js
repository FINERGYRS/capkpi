...
QUnit.test('Make fixtures', assert => {
	...
	let tasks = [];
	Object.keys(finergy.test_data).forEach(function(doctype) {
		tasks.push(function() {
			return finergy.tests.setup_doctype(doctype, finergy.test_data[doctype]);
		});
	});
	finergy.run_serially(tasks).then(() => done());
});
