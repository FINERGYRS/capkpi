...
from capkpi.capkpi_integrations.connectors.woocommerce_connection import order


class TestWoocommerce(unittest.TestCase):
	def setUp(self):

		woo_settings = finergy.get_doc("Woocommerce Settings")
	...
	def test_sales_order_for_woocommerce(self):
		finergy.flags.woocomm_test_order_data = {
			"id": 75,
			"parent_id": 0,
			"number": "74",
		...
		self.assertTrue(finergy.get_value("Customer", {"woocommerce_email": "tony@gmail.com"}))
		self.assertTrue(finergy.get_value("Item", {"woocommerce_id": 56}))
		self.assertTrue(finergy.get_value("Sales Order", {"woocommerce_id": 75}))
		finergy.flags.woocomm_test_order_data = {}
...
def emulate_request():
	...
	# Build URL
	port = finergy.get_site_config().webserver_port or "8000"

	if os.environ.get("CI"):
		host = "localhost"
	else:
		host = finergy.local.site

	url = "http://{site}:{port}/api/method/capkpi.capkpi_integrations.connectors.woocommerce_connection.order".format(
		site=host, port=port
	)
