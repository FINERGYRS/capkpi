...
finergy.ready(function() {
	...
	finergy.call({
		type: "POST",
		method: "capkpi.e_commerce.shopping_cart.product_info.get_product_info_for_website",
		args: {
	...
	$("#item-add-to-cart button").on("click", function() {
		finergy.provide('capkpi.shopping_cart');

		capkpi.shopping_cart.update_cart({
			item_code: get_item_code(),
	...
	$("[itemscope] .item-view-attribute .form-control").on("change", function() {
		try {
			...
			if (!item_code) {
				finergy.msgprint(__("Cannot find a matching Item. Please select some other value for {0}.", [attribute]))
				throw e;
			}
		}
