finergy.ready(function() {
	...
	$(".file-size").each(function() {
		$(this).text(finergy.form.formatters.FileSize($(this).text()));
	});
	...
	var reload_items = function(item_status, item, $btn) {
		$.ajax({
			...
			dataType: "json",
			data: {
				cmd: "capkpi.templates.pages.projects.get_"+ item +"_html",
				project: '{{ doc.name }}',
	...
	var close_item = function(item, item_name){
		...
		finergy.call({
			btn: this,
			type: "POST",
			method: "capkpi.templates.pages.projects.set_"+ item +"_status",
			args: args,
			callback: function(r) {
				if(r.exc) {
