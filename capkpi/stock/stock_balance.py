...
import finergy
from finergy.utils import cstr, flt, now, nowdate, nowtime

from capkpi.controllers.stock_controller import create_repost_item_valuation_entry
from capkpi.stock.utils import update_bin
...
def repost(only_actual=False, allow_negative_stock=False, allow_zero_rate=False, only_bin=False):
	...
	item_warehouses = finergy.db.sql(
		"""
...
def set_stock_balance_as_per_serial_no(
	...
	for d in bin:
		...
		sle_doc = finergy.get_doc(sle_dict)
...
def reset_serial_no_status_and_warehouse(serial_nos=None):
	if not serial_nos:
		serial_nos = finergy.db.sql_list("""select name from `tabSerial No` where docstatus = 0""")
		for serial_no in serial_nos:
			try:
				sr = finergy.get_doc("Serial No", serial_no)
