...
def get_warehouse_account_map(company=None):
	company_warehouse_account_map = company and finergy.flags.setdefault(
		"warehouse_account_map", {}
	).get(company)
	warehouse_account_map = finergy.flags.warehouse_account_map

	if not warehouse_account_map or not company_warehouse_account_map or finergy.flags.in_test:
		warehouse_account = finergy._dict()

		filters = {}
		if company:
			filters["company"] = company
			finergy.flags.setdefault("warehouse_account_map", {}).setdefault(company, {})
		...
		for d in finergy.get_all(
			...
			if d.account:
				d.account_currency = finergy.db.get_value("Account", d.account, "account_currency", cache=True)
				warehouse_account.setdefault(d.name, d)
		if company:
			finergy.flags.warehouse_account_map[company] = warehouse_account
		else:
			finergy.flags.warehouse_account_map = warehouse_account

	return finergy.flags.warehouse_account_map.get(company) or finergy.flags.warehouse_account_map
...
def get_company_default_inventory_account(company):
	return finergy.get_cached_value("Company", company, "default_inventory_account")
