...
from capkpi.stock.report.stock_ledger.stock_ledger import get_item_group_condition
...
def get_conditions(filters):
	...
	if filters.get("to_date"):
		conditions += " and sle.posting_date <= %s" % finergy.db.escape(filters.get("to_date"))
	else:
		finergy.throw(_("'To Date' is required"))

	if filters.get("company"):
		conditions += " and sle.company = %s" % finergy.db.escape(filters.get("company"))

	if filters.get("warehouse"):
		warehouse_details = finergy.db.get_value(
...
def get_stock_ledger_entries(filters, items):
	...
	if items:
		item_conditions_sql = " and sle.item_code in ({})".format(
			", ".join(finergy.db.escape(i, percent=False) for i in items)
		)
...
def get_item_details(items, sle, filters):
	...
	if filters.get("include_uom"):
		...
		cf_join = (
			...
			% finergy.db.escape(filters.get("include_uom"))
		)
...
def get_item_reorder_details(items):
	...
	if items:
		item_reorder_details = finergy.db.sql(
			"""
...
def get_variants_attributes():
	...
	return [i.name for i in finergy.get_all("Item Attribute")]


def get_variant_values_for(items):
	"""Returns variant values for items."""
	attribute_map = {}
	for attr in finergy.db.sql(
