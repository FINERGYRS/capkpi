...
from finergy.utils import cint, date_diff, flt, getdate
from finergy.utils.nestedset import get_descendants_of
from pypika.terms import ExistsCriterion

import capkpi
from capkpi.stock.doctype.inventory_dimension.inventory_dimension import get_inventory_dimensions
from capkpi.stock.report.stock_ageing.stock_ageing import FIFOSlots, get_average_age
from capkpi.stock.utils import add_additional_uom_columns, is_reposting_item_valuation_in_progress
...
def execute(filters: Optional[StockBalanceFilter] = None):
	...
	if filters.get("company"):
		company_currency = capkpi.get_company_currency(filters.get("company"))
	else:
		company_currency = finergy.db.get_single_value("Global Defaults", "default_currency")
...
def apply_conditions(query, filters):
	...
	warehouse_table = finergy.qb.DocType("Warehouse")

	if not filters.get("from_date"):
		finergy.throw(_("'From Date' is required"))

	if to_date := filters.get("to_date"):
		query = query.where(sle.posting_date <= to_date)
	else:
		finergy.throw(_("'To Date' is required"))
	...
	if warehouse := filters.get("warehouse"):
		...
		chilren_subquery = (
			finergy.qb.from_(warehouse_table)
...
def get_item_warehouse_map(filters: StockBalanceFilter, sle: List[SLEntry]):
	...
	float_precision = cint(finergy.db.get_default("float_precision")) or 3
	...
	for d in sle:
		...
		if group_by_key not in iwb_map:
			iwb_map[group_by_key] = finergy._dict(
				{
					"opening_qty": 0.0,
					"opening_val": 0.0,
					"in_qty": 0.0,
		...
		if d.posting_date < from_date or (
			...
			and finergy.db.get_value("Stock Reconciliation", d.voucher_no, "purpose") == "Opening Stock"
		):
...
def get_items(filters: StockBalanceFilter) -> List[str]:
	...
	if item_code := filters.get("item_code"):
		...
		return finergy.get_all("Item", filters=item_filters, pluck="name", order_by=None)
...
def get_item_details(items: List[str], sle: List[SLEntry], filters: StockBalanceFilter):
	...
	item_table = finergy.qb.DocType("Item")

	query = (
		finergy.qb.from_(item_table)
		.select(
			item_table.name,
	...
	if uom := filters.get("include_uom"):
		uom_conv_detail = finergy.qb.DocType("UOM Conversion Detail")
		query = (
...
def get_item_reorder_details(items):
	item_reorder_details = finergy._dict()

	if items:
		item_reorder_details = finergy.get_all(
			"Item Reorder",
...
def get_variants_attributes() -> List[str]:
	...
	return finergy.get_all("Item Attribute", pluck="name")
...
def get_variant_values_for(items):
	...
	attribute_info = finergy.get_all(
