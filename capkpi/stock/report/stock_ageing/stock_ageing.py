...
from finergy.utils import cint, date_diff, flt
...
def format_report_data(filters: Filters, item_details: Dict, to_date: str) -> List[Dict]:
	...
	precision = cint(finergy.db.get_single_value("System Settings", "float_precision", cache=True))
...
class FIFOSlots:
		...
		for key, row in wh_wise_data.items():
			...
			if not item_aggregated_data.get(item):
				item_aggregated_data.setdefault(
					item,
					{"details": finergy._dict(), "fifo_queue": [], "qty_after_transaction": 0.0, "total_qty": 0.0},
				)
	...
	def __get_item_query(self) -> str:
		...
		item = finergy.qb.from_("Item").select(
	...
	def __get_warehouse_conditions(self, sle, sle_query) -> str:
		warehouse = finergy.qb.DocType("Warehouse")
		lft, rgt = finergy.db.get_value("Warehouse", self.filters.get("warehouse"), ["lft", "rgt"])

		warehouse_results = (
			finergy.qb.from_(warehouse)
			.select("name")
