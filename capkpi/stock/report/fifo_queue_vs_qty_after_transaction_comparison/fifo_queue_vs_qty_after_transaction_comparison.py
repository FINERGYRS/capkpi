...
def get_data(filters):
	if not any([filters.warehouse, filters.item_code, filters.item_group]):
		finergy.throw(_("Any one of following filters required: warehouse, Item Code, Item Group"))
...
def get_stock_ledger_entries(filters):
	...
	if filters.item_code:
		...
		sle_filters["item_code"] = (
			"in",
			finergy.get_all("Item", filters=item_group_filter, pluck="name", order_by=None),
		)
...
def find_first_bad_queue(sles):
	...
	for _item_wh, sles in item_warehouse_sles.items():
		for idx, sle in enumerate(sles):
			...
			if sle.batch_no:
				sle.use_batchwise_valuation = finergy.db.get_value(
