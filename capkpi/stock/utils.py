...
def update_bin(args, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	from capkpi.stock.doctype.bin.bin import update_stock
	...
	if is_stock_item:
		...
		finergy.msgprint(_("Item {0} ignored since it is not a stock item").format(args.get("item_code")))


@finergy.whitelist()
def get_incoming_rate(args, raise_error_if_no_rate=True):
	"""Get Incoming Rate based on valuation method"""
	from capkpi.stock.stock_ledger import get_previous_sle, get_valuation_rate
...
def get_valuation_method(item_code):
	...
	if not val_method:
		val_method = finergy.db.get_value("Stock Settings", None, "valuation_method") or "FIFO"
	return val_method
