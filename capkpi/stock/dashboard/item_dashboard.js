...
capkpi.stock.ItemDashboard = Class.extend({
	init: function (opts) {
		$.extend(this, opts);
		this.make();
	},
	make: function () {
		...
		this.content = $(finergy.render_template('item_dashboard')).appendTo(this.parent);
		...
		this.content.on('click', '.btn-edit', function () {
			...
			finergy.db.get_value('Putaway Rule', {
				'item_code': item,
				'warehouse': warehouse,
				'company': company
			}, 'name', (r) => {
				finergy.set_route("Form", "Putaway Rule", r.name);
			});
		});
		...
		function handle_move_add(element, action) {
			...
			if (disable_quick_entry) {
				...
				if (action === "Add") {
					...
					capkpi.stock.move_item(item, null, warehouse, actual_qty, rate, function () {
						me.refresh();
					});
				} else {
					capkpi.stock.move_item(item, warehouse, null, actual_qty, null, function () {
						me.refresh();
					});
				}
			}
		...
		function open_stock_entry(item, warehouse, entry_type) {
			finergy.model.with_doctype('Stock Entry', function () {
				var doc = finergy.model.get_new_doc('Stock Entry');
				if (entry_type) doc.stock_entry_type = entry_type;

				var row = finergy.model.add_child(doc, 'items');
				row.item_code = item;
	...
	render: function (data) {
		...
		if (context.data.length > 0) {
			...
			$(finergy.render_template(this.template, context)).appendTo(this.result);
		} else {
	...
	get_item_dashboard_data: function (data, max_count, show_item) {
		...
		let can_write = 0;
		if (finergy.boot.user.can_write.indexOf("Stock Entry") >= 0) {
			can_write = 1;
		}
	...
	get_capacity_dashboard_data: function (data) {
		...
		let can_write = 0;
		if (finergy.boot.user.can_write.indexOf("Putaway Rule") >= 0) {
			can_write = 1;
		}
...
capkpi.stock.move_item = function (item, source, target, actual_qty, rate, callback) {
	...
	dialog.set_primary_action(__('Submit'), function () {
		...
		if (source && values.qty > actual_qty) {
			finergy.msgprint(__('Quantity must be less than or equal to {0}', [actual_qty]));
			return;
		}
		if (values.source === values.target) {
			finergy.msgprint(__('Source and target warehouse must be different'));
		}

		finergy.call({
			method: 'capkpi.stock.doctype.stock_entry.stock_entry_utils.make_stock_entry',
			args: values,
			btn: dialog.get_primary_btn(),
			freeze: true,
			freeze_message: __('Creating Stock Entry'),
			callback: function (r) {
				finergy.show_alert(__('Stock Entry {0} created',
