...
capkpi.stock.ItemDashboard = class ItemDashboard {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
...
capkpi.stock.move_item = function (item, source, target, actual_qty, rate, callback) {
	...
	dialog.set_primary_action(__('Create Stock Entry'), function () {
		if (source && (dialog.get_value("qty") == 0 || dialog.get_value("qty") > actual_qty)) {
			finergy.msgprint(__("Quantity must be greater than zero, and less or equal to {0}", [actual_qty]));
			return;
		}
