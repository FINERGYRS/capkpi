...
from finergy.utils import add_days, cint, flt, nowdate
...
def reorder_item():
	...
	if not (finergy.db.a_row_exists("Company") and finergy.db.a_row_exists("Fiscal Year")):
		return

	if cint(finergy.db.get_value("Stock Settings", None, "auto_indent")):
		return _reorder_item()


def _reorder_item():
	material_requests = {"Purchase": {}, "Transfer": {}, "Material Issue": {}, "Manufacture": {}}
	warehouse_company = finergy._dict(
		finergy.db.sql(
	...
	default_company = (
		capkpi.get_default_company() or finergy.db.sql("""select name from tabCompany limit 1""")[0][0]
	)

	items_to_consider = finergy.db.sql_list(
...
def get_item_warehouse_projected_qty(items_to_consider):
	...
	for item_code, warehouse, projected_qty in finergy.db.sql(
		...
		warehouse_doc = finergy.get_doc("Warehouse", warehouse)
		...
		while warehouse_doc.parent_warehouse:
			...
			warehouse_doc = finergy.get_doc("Warehouse", warehouse_doc.parent_warehouse)
...
def create_material_request(material_requests):
	...
	def _log_exception(mr):
		if finergy.local.message_log:
			exceptions_list.extend(finergy.local.message_log)
			finergy.local.message_log = []
		else:
			exceptions_list.append(finergy.get_traceback())
	...
	for request_type in material_requests:
		for company in material_requests[request_type]:
			try:
				...
				for d in items:
					d = finergy._dict(d)
					item = finergy.get_doc("Item", d.item_code)
					uom = item.stock_uom
					...
					must_be_whole_number = finergy.db.get_value("UOM", uom, "must_be_whole_number", cache=True)
	...
	if mr_list:
		if getattr(finergy.local, "reorder_email_notify", None) is None:
			finergy.local.reorder_email_notify = cint(
				finergy.db.get_value("Stock Settings", None, "reorder_email_notify")
			)

		if finergy.local.reorder_email_notify:
...
def send_email_notification(mr_list):
	...
	email_list = finergy.db.sql_list(
	...
	msg = finergy.render_template("templates/emails/reorder_item.html", {"mr_list": mr_list})

	finergy.sendmail(recipients=email_list, subject=_("Auto Material Requests Generated"), message=msg)


def notify_errors(exceptions_list):
	subject = _("[Important] [CapKPI] Auto Reorder Errors")
	content = (
