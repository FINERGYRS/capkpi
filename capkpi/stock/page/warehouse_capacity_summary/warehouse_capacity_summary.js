finergy.pages['warehouse-capacity-summary'].on_page_load = function(wrapper) {
	...
	page.sort_selector = new finergy.ui.SortSelector({
	...
	finergy.require('assets/js/item-dashboard.min.js', function() {
		$(finergy.render_template('warehouse_capacity_summary_header')).appendTo(page.main);

		page.capacity_dashboard = new capkpi.stock.ItemDashboard({
			page_name: "warehouse-capacity-summary",
			page_length: 10,
			parent: page.main,
			sort_by: 'stock_capacity',
			sort_order: 'desc',
			method: 'capkpi.stock.dashboard.warehouse_capacity_dashboard.get_data',
		...
		let setup_click = function(doctype) {
			page.main.on('click', 'a[data-type="'+ doctype.toLowerCase() +'"]', function() {
				...
				if (field.get_value()===name) {
					finergy.set_route('Form', doctype, name);
				} else {
