...
from finergy.query_builder.functions import CombineDatetime, Sum
from finergy.utils import cint, cstr, flt, get_link_to_form, getdate, now, nowdate

import capkpi
from capkpi.stock.doctype.bin.bin import update_qty as update_bin_qty
from capkpi.stock.utils import (
	get_incoming_outgoing_rate_for_cancel,
	get_or_make_bin,
	get_valuation_method,
)
from capkpi.stock.valuation import FIFOValuation, LIFOValuation, round_off_if_near_zero


class NegativeStockError(finergy.ValidationError):
	pass


class SerialNoExistsInFutureTransaction(finergy.ValidationError):
	pass
...
def make_sl_entries(sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	args:
	        ...
	        such cases certain validations need to be ignored (like negative
	                        stock)
	"""
	from capkpi.controllers.stock_controller import future_sle_exists
	...
	if sl_entries:
		...
		for sle in sl_entries:
			...
			is_stock_item = finergy.get_cached_value("Item", args.get("item_code"), "is_stock_item")
			if is_stock_item:
...
def validate_serial_no(sle):
	...
	for sn in get_serial_nos(sle.serial_no):
		...
		for row in get_stock_ledger_entries(args, ">"):
			voucher_type = finergy.bold(row.voucher_type)
			voucher_no = finergy.bold(get_link_to_form(row.voucher_type, row.voucher_no))
			vouchers.append(f"{voucher_type} {voucher_no}")

		if vouchers:
			serial_no = finergy.bold(sn)
			msg = (
			...
			finergy.throw(_(msg), title=_(title), exc=SerialNoExistsInFutureTransaction)


def validate_cancellation(args):
	if args[0].get("is_cancelled"):
		repost_entry = finergy.db.get_value(
		...
		if repost_entry:
			...
			if repost_entry.status == "Queued":
				doc = finergy.get_doc("Repost Item Valuation", repost_entry.name)
				doc.status = "Skipped"
...
def set_as_cancel(voucher_type, voucher_no):
	finergy.db.sql(
		...
		(now(), finergy.session.user, voucher_type, voucher_no),
	)


def make_entry(args, allow_negative_stock=False, via_landed_cost_voucher=False):
	args["doctype"] = "Stock Ledger Entry"
	sle = finergy.get_doc(args)
...
def validate_item_warehouse(args):
	for field in ["item_code", "warehouse", "posting_date", "posting_time"]:
		if not args.get(field):
			validation_msg = f"The field {finergy.unscrub(args.get(field))} is required for the reposting"
			finergy.throw(_(validation_msg))
	...
	doc.db_set(
		{
			...
			"current_index": index,
			"affected_transactions": finergy.as_json(affected_transactions),
		}
	)
...
def get_items_to_be_repost(voucher_type=None, voucher_no=None, doc=None):
	...
	if not items_to_be_repost and voucher_type and voucher_no:
		items_to_be_repost = finergy.db.get_all(
			"Stock Ledger Entry",
...
def get_distinct_item_warehouse(args=None, doc=None):
	...
	if doc and doc.distinct_item_and_warehouse:
		...
		distinct_item_warehouses = {
			finergy.safe_eval(k): finergy._dict(v) for k, v in distinct_item_warehouses.items()
		}
	else:
		for i, d in enumerate(args):
			distinct_item_warehouses.setdefault(
				(d.item_code, d.warehouse), finergy._dict({"reposting_status": False, "sle": d, "args_idx": i})
			)
...
def get_affected_transactions(doc) -> Set[Tuple[str, str]]:
	...
	transactions = finergy.parse_json(doc.affected_transactions)
...
class update_entries_after(object):
	...
	def __init__(
		...
		self.args = finergy._dict(args)
		if self.args.sle_id:
			self.args["name"] = self.args.sle_id

		self.company = finergy.get_cached_value("Warehouse", self.args.warehouse, "company")
		self.set_precision()
		self.valuation_method = get_valuation_method(self.item_code)

		self.new_items_found = False
		self.distinct_item_warehouses = args.get("distinct_item_warehouses", finergy._dict())
	...
	def set_precision(self):
		self.flt_precision = cint(finergy.db.get_default("float_precision")) or 2
		self.currency_precision = get_field_precision(
			finergy.get_meta("Stock Ledger Entry").get_field("stock_value")
		)
	...
	def initialize_previous_data(self, args):
		...
		"""
		self.data.setdefault(args.warehouse, finergy._dict())
	...
	def get_future_entries_to_fix(self):
		...
		args = self.data[self.args.warehouse].previous_sle or finergy._dict(
	...
	def update_distinct_item_warehouses(self, dependant_sle):
		...
		val = finergy._dict({"sle": dependant_sle})
	...
	def process_sle(self, sle):
		...
		elif sle.batch_no and finergy.db.get_value(
	...
	def get_incoming_outgoing_rate_from_transaction(self, sle):
		...
		if sle.voucher_type == "Stock Entry":
			...
			rate = finergy.db.get_value("Stock Entry Detail", sle.voucher_detail_no, "valuation_rate")
		# Sales and Purchase Return
			...
			if finergy.get_cached_value(sle.voucher_type, sle.voucher_no, "is_return"):
				from capkpi.controllers.sales_and_purchase_return import (
				...
				# check in item table
				item_code, incoming_rate = finergy.db.get_value(
	...
	def update_rate_on_stock_entry(self, sle, outgoing_rate):
		finergy.db.set_value("Stock Entry Detail", sle.voucher_detail_no, "basic_rate", outgoing_rate)
	...
	def recalculate_amounts_in_stock_entry(self, voucher_no):
		stock_entry = finergy.get_doc("Stock Entry", voucher_no, for_update=True)
	...
	def update_rate_on_delivery_and_sales_return(self, sle, outgoing_rate):
		...
		item_code = finergy.db.get_value(sle.voucher_type + " Item", sle.voucher_detail_no, "item_code")
		if item_code == sle.item_code:
	...
	def update_rate_on_purchase_receipt(self, sle, outgoing_rate):
		if finergy.db.exists(sle.voucher_type + " Item", sle.voucher_detail_no):
			finergy.db.set_value(
		...
		if finergy.get_cached_value(sle.voucher_type, sle.voucher_no, "is_subcontracted"):
			doc = finergy.get_doc(sle.voucher_type, sle.voucher_no)
	...
	def update_rate_on_subcontracting_receipt(self, sle, outgoing_rate):
		if finergy.db.exists(sle.voucher_type + " Item", sle.voucher_detail_no):
			finergy.db.set_value(sle.voucher_type + " Item", sle.voucher_detail_no, "rate", outgoing_rate)
		else:
			finergy.db.set_value(
	...
	def get_incoming_value_for_serial_nos(self, sle, serial_nos):
		...
		all_serial_nos = finergy.get_all(
	...
	def check_if_allow_zero_valuation_rate(self, voucher_type, voucher_detail_no):
		...
		if ref_item_dt:
			return finergy.db.get_value(ref_item_dt, voucher_detail_no, "allow_zero_valuation_rate")
		else:
			return 0
	...
	def get_fallback_rate(self, sle) -> float:
		...
		return get_valuation_rate(
			...
			currency=capkpi.get_company_currency(sle.company),
			company=sle.company,
			batch_no=sle.batch_no,
		)
	...
	def get_sle_before_datetime(self, args):
		...
		sle = sle[0] if sle else finergy._dict()
		return sle
	...
	def raise_exceptions(self):
		...
		for warehouse, exceptions in self.exceptions.items():
			...
			) in finergy.local.flags.currently_saving:

				msg = _("{0} units of {1} needed in {2} to complete this transaction.").format(
					abs(deficiency),
					finergy.get_desk_link("Item", exceptions[0]["item_code"]),
					finergy.get_desk_link("Warehouse", warehouse),
				)
			else:
				msg = _(
					...
					finergy.get_desk_link(exceptions[0]["voucher_type"], exceptions[0]["voucher_no"]),
				)
		...
		if msg_list:
			message = "\n\n".join(msg_list)
			if self.verbose:
				finergy.throw(message, NegativeStockError, title=_("Insufficient Stock"))
			else:
	...
	def update_bin(self):
		...
		for warehouse, data in self.data.items():
			...
			finergy.db.set_value("Bin", bin_name, updated_values)
...
def get_previous_sle_of_current_voucher(args, exclude_current_voucher=False):
	...
	return sle[0] if sle else finergy._dict()
...
def get_stock_ledger_entries(
	...
	if check_serial_no and previous_sle.get("serial_no"):
		# conditions += " and serial_no like {}".format(finergy.db.escape('%{0}%'.format(previous_sle.get("serial_no"))))
...
def get_valuation_rate(
	...
	if not company:
		company = finergy.get_cached_value("Warehouse", warehouse, "company")

	last_valuation_rate = None

	# Get moving average rate of a specific batch number
	if warehouse and batch_no and finergy.db.get_value("Batch", batch_no, "use_batchwise_valuation"):
		last_valuation_rate = finergy.db.sql(
			"""
	...
	if not valuation_rate:
		...
		valuation_rate = finergy.db.get_value("Item", item_code, "standard_rate")

		if not valuation_rate:
			# try in price list
			valuation_rate = finergy.db.get_value(
	...
	if (
		...
		and cint(capkpi.is_perpetual_inventory_enabled(company))
	):
		...
		finergy.throw(msg=msg, title=_("Valuation Rate Missing"))
...
def validate_negative_qty_in_future_sle(args, allow_negative_stock=False):
	...
	if is_negative_with_precision(neg_sle):
		message = _(
			...
			finergy.get_desk_link("Item", args.item_code),
			finergy.get_desk_link("Warehouse", args.warehouse),
			neg_sle[0]["posting_date"],
			neg_sle[0]["posting_time"],
			finergy.get_desk_link(neg_sle[0]["voucher_type"], neg_sle[0]["voucher_no"]),
		)
	...
	if is_negative_with_precision(neg_batch_sle, is_batch=True):
		message = _(
			...
			finergy.get_desk_link("Batch", args.batch_no),
			finergy.get_desk_link("Warehouse", args.warehouse),
			neg_batch_sle[0]["posting_date"],
			neg_batch_sle[0]["posting_time"],
			finergy.get_desk_link(neg_batch_sle[0]["voucher_type"], neg_batch_sle[0]["voucher_no"]),
		)
		finergy.throw(message, NegativeStockError, title=_("Insufficient Stock for Batch"))
...
def is_negative_with_precision(neg_sle, is_batch=False):
	...
	precision = cint(finergy.db.get_default("float_precision")) or 2
...
def is_negative_stock_allowed(*, item_code: Optional[str] = None) -> bool:
	if cint(finergy.db.get_single_value("Stock Settings", "allow_negative_stock", cache=True)):
		return True
	if item_code and cint(finergy.db.get_value("Item", item_code, "allow_negative_stock", cache=True)):
		return True
	return False
