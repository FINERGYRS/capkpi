...
from capkpi.stock import get_warehouse_account
...
class Warehouse(NestedSet):
	...
	def autoname(self):
		if self.company:
			suffix = " - " + finergy.get_cached_value("Company", self.company, "abbr")
	...
	def onload(self):
		...
		if self.company and cint(
			finergy.db.get_value("Company", self.company, "enable_perpetual_inventory")
		):
	...
	def on_trash(self):
		# delete bin
		bins = finergy.get_all("Bin", fields="*", filters={"warehouse": self.name})
		for d in bins:
			if (
				d["actual_qty"]
				or d["reserved_qty"]
		...
		finergy.db.delete("Bin", filters={"warehouse": self.name})
		self.update_nsm_model()
	...
	def warn_about_multiple_warehouse_account(self):
		...
		def get_accounts_where_value_is_booked(name):
			...
			gle = finergy.qb.DocType("GL Entry")
			ac = finergy.qb.DocType("Account")
		...
		old_wh_account = finergy.db.get_value("Warehouse", self.name, "account")
		...
		if self.account != old_wh_account:
			...
			account_str = "<br>" + ", ".join(finergy.bold(ac) for ac in accounts)
			reason = "<br><br>" + _(
	...
	def check_if_sle_exists(self):
		return finergy.db.exists("Stock Ledger Entry", {"warehouse": self.name})

	def check_if_child_exists(self):
		return finergy.db.exists("Warehouse", {"parent_warehouse": self.name})
	...
	def convert_to_ledger(self):
		if self.check_if_child_exists():
			finergy.throw(_("Warehouses with child nodes cannot be converted to ledger"))
	...
	def unlink_from_items(self):
		finergy.db.set_value("Item Default", {"default_warehouse": self.name}, "default_warehouse", None)
...
def get_children(doctype, parent=None, company=None, is_root=False):
	...
	warehouses = finergy.get_list(doctype, fields=fields, filters=filters, order_by="name")
...
def convert_to_group_or_ledger(docname=None):
	...
	return finergy.get_doc("Warehouse", docname).convert_to_group_or_ledger()
...
def get_warehouses_based_on_account(account, company=None):
	warehouses = []
	for d in finergy.get_all("Warehouse", fields=["name", "is_group"], filters={"account": account}):
		if d.is_group:
	...
	if (
		not warehouses
		and company
		and finergy.get_cached_value("Company", company, "default_inventory_account") == account
	):
		warehouses = [d.name for d in finergy.get_all("Warehouse", filters={"is_group": 0})]

	if not warehouses:
		finergy.throw(_("Warehouse not found against the account {0}").format(account))
