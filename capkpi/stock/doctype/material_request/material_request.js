...
finergy.ui.form.on('Material Request', {
	setup: function(frm) {
	...
	set_from_warehouse: function(frm) {
		if (frm.doc.material_request_type == "Material Transfer"
			...
			frm.doc.items.forEach(d => {
				finergy.model.set_value(d.doctype, d.name,
	...
	make_custom_buttons: function(frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.status != 'Stopped') {
			let precision = finergy.defaults.get_default("float_precision");
	...
	update_status: function(frm, stop_status) {
		finergy.call({
			method: 'capkpi.stock.doctype.material_request.material_request.update_status',
		...
		frm.call({
			...
			args: {
				args: {
					...
					buying_price_list: finergy.defaults.get_default('buying_price_list'),
					...
					callback: function(r) {
						if (!r.message) {
							...
							$.each(r.message, function(i, item) {
								var d = finergy.model.add_child(cur_frm.doc, "Material Request Item", "items");
		...
		finergy.prompt(
			{
				...
				get_query: () => {
					return{
						query: "capkpi.stock.doctype.material_request.material_request.get_default_supplier_query",
	...
	make_request_for_quotation: function(frm) {
		finergy.model.open_mapped_doc({
			method: "capkpi.stock.doctype.material_request.material_request.make_request_for_quotation",
			frm: frm,
			run_link_triggers: true
		});
	},
	...
	make_supplier_quotation: function(frm) {
		finergy.model.open_mapped_doc({
			method: "capkpi.stock.doctype.material_request.material_request.make_supplier_quotation",
			frm: frm
		});
	},
	...
	raise_work_orders: function(frm) {
		finergy.call({
			method:"capkpi.stock.doctype.material_request.material_request.raise_work_orders",
			args: {
...
finergy.ui.form.on("Material Request Item", {
	qty: function (frm, doctype, name) {
		var d = locals[doctype][name];
		if (flt(d.qty) < flt(d.min_order_qty)) {
			finergy.msgprint(__("Warning: Material Requested Qty is less than Minimum Order Qty"));
		}
...
capkpi.buying.MaterialRequestController = class MaterialRequestController extends capkpi.buying.BuyingController {
	tc_name() {
		this.get_terms();
	}
...
extend_cscript(cur_frm.cscript, new capkpi.buying.MaterialRequestController({frm: cur_frm}));
