...
from capkpi.stock.utils import get_stock_balance


class OpeningEntryAccountError(finergy.ValidationError):
	pass


class EmptyStockReconciliationItemsError(finergy.ValidationError):
	pass
...
class StockReconciliation(StockController):
	...
	def validate(self):
		if not self.expense_account:
			self.expense_account = finergy.get_cached_value(
				"Company", self.company, "stock_adjustment_account"
			)
		if not self.cost_center:
			self.cost_center = finergy.get_cached_value("Company", self.company, "cost_center")
	...
	def remove_items_with_no_change(self):
		...
		if not items:
			...
			for i, item in enumerate(self.items):
				item.idx = i + 1
			finergy.msgprint(_("Removed items with no change in quantity or value."))
	...
	def validate_data(self):
		...
		for row_num, row in enumerate(self.items):
			...
			# validate warehouse
			if not finergy.db.get_value("Warehouse", row.warehouse):
			...
			if row.qty and row.valuation_rate in ["", None]:
				...
				if not row.valuation_rate:
					...
					buying_rate = finergy.db.get_value(
						"Item Price",
					...
					if buying_rate:
						...
						row.valuation_rate = finergy.get_value("Item", row.item_code, "valuation_rate")
		...
		if self.validation_messages:
			...
			raise finergy.ValidationError(self.validation_messages)
	...
	def validate_item(self, item_code, row):
		...
		try:
			...
			if item.has_batch_no and not row.batch_no and not item.create_new_batch:
				raise finergy.ValidationError(_("Batch no is required for batched item {0}").format(item_code))
	...
	def update_stock_ledger(self):
		...
		for row in self.items:
			item = finergy.get_doc("Item", row.item_code)
			...
			if item.has_serial_no or item.has_batch_no:
				...
				if row.serial_no or row.batch_no:
					finergy.throw(
						...
						).format(row.idx, finergy.bold(row.item_code))
					)
	...
	def update_valuation_rate_for_serial_nos(self, row, serial_nos):
		...
		for d in serial_nos:
			finergy.db.set_value("Serial No", d, "purchase_rate", valuation_rate)
		...
		data = finergy._dict(
			{
				...
				"stock_uom": finergy.db.get_value("Item", row.item_code, "stock_uom"),
	...
	def validate_expense_account(self):
		...
		if not self.expense_account:
			finergy.throw(_("Please enter Expense Account"))
		elif self.purpose == "Opening Stock" or not finergy.db.sql(
			"""select name from `tabStock Ledger Entry` limit 1"""
		):
			if finergy.db.get_value("Account", self.expense_account, "report_type") == "Profit and Loss":
				finergy.throw(
					_(
	...
	def set_zero_value_for_customer_provided_items(self):
		...
		for d in self.get("items"):
			is_customer_item = finergy.db.get_value("Item", d.item_code, "is_customer_provided_item")
...
def get_items(
	...
	items = [finergy._dict({"item_code": item_code, "warehouse": warehouse})]
...
def get_itemwise_batch(warehouse, posting_date, company, item_code=None):
	from capkpi.stock.report.batch_wise_balance_history.batch_wise_balance_history import execute
...
def get_stock_balance_for(
	...
	finergy.has_permission("Stock Reconciliation", "write", throw=True)

	item_dict = finergy.db.get_value("Item", item_code, ["has_serial_no", "has_batch_no"], as_dict=1)

	if not item_dict:
		# In cases of data upload to Items table
		msg = _("Item {} does not exist.").format(item_code)
		finergy.throw(msg, title=_("Missing"))
