...
# CapKPI - web based ERP (http://capkpi.com)
# For license information, please see license.txt


import finergy
from finergy.tests.utils import FinergyTestCase, change_settings
from finergy.utils import add_days, cstr, flt, nowdate, nowtime, random_string

from capkpi.accounts.utils import get_stock_and_account_balance
...
from capkpi.stock.stock_ledger import get_previous_sle, update_entries_after
from capkpi.stock.tests.test_utils import StockTestMixin
from capkpi.stock.utils import get_incoming_rate, get_stock_value_on, get_valuation_method


class TestStockReconciliation(FinergyTestCase, StockTestMixin):
	@classmethod
	def setUpClass(cls):
	...
	def tearDown(self):
		...
		finergy.flags.pop("dont_execute_stock_reposts", None)
	...
	def test_stock_reco_for_serialized_item(self):
		...
		for d in to_delete_records:
			stock_doc = finergy.get_doc("Stock Reconciliation", d)
			stock_doc.cancel()
	...
	def test_stock_reco_for_serial_and_batch_item(self):
		...
		self.assertEqual(finergy.db.get_value("Serial No", serial_nos[0], "batch_no"), batch_no)

		sr.cancel()

		self.assertEqual(finergy.db.get_value("Serial No", serial_nos[0], "status"), "Inactive")
	...
	def test_stock_reco_for_serial_and_batch_item_with_future_dependent_entry(self):
		...
		self.assertEqual(finergy.db.get_value("Serial No", reco_serial_no, "batch_no"), batch_no)
		self.assertEqual(finergy.db.get_value("Serial No", reco_serial_no, "status"), "Active")

		# Check if Serial No from Stock Entry is Unlinked and Inactive
		self.assertEqual(finergy.db.get_value("Serial No", serial_no_2, "batch_no"), None)
		self.assertEqual(finergy.db.get_value("Serial No", serial_no_2, "status"), "Inactive")
	...
	def test_backdated_stock_reco_qty_reposting(self):
		...
		def assertBalance(doc, qty_after_transaction):
			sle_balance = finergy.db.get_value(
	...
	def test_backdated_stock_reco_cancellation_future_negative_stock(self):
		...
		dn_balance = finergy.db.get_value(
		...
		repost_exists = bool(finergy.db.exists("Repost Item Valuation", {"voucher_no": sr.name}))
	...
	def test_intermediate_sr_bin_update(self):
		...
		old_bin_qty = finergy.db.get_value(
		...
		new_bin_qty = finergy.db.get_value(
			"Bin", {"item_code": item_code, "warehouse": warehouse}, "actual_qty"
		)
	...
	def test_valid_batch(self):
		...
		self.assertRaises(finergy.ValidationError, sr.submit)
...
def create_batch_item_with_batch(item_name, batch_id):
	...
	if not finergy.db.exists("Batch", batch_id):
		b = finergy.new_doc("Batch")
		b.item = item_name
		b.batch_id = batch_id
		b.save()
...
def create_stock_reconciliation(**args):
	...
	sr = finergy.new_doc("Stock Reconciliation")
	...
	sr.expense_account = args.expense_account or (
		"Stock Adjustment - _TC" if finergy.get_all("Stock Ledger Entry") else "Temporary Opening - _TC"
	)
	sr.cost_center = (
		args.cost_center
		or finergy.get_cached_value("Company", sr.company, "cost_center")
...
def set_valuation_method(item_code, valuation_method):
	...
	finergy.db.set_value("Item", item_code, "valuation_method", valuation_method)

	for warehouse in finergy.get_all(
