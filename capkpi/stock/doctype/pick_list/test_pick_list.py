...
from capkpi.selling.doctype.sales_order.sales_order import create_pick_list
...
from capkpi.stock.doctype.packed_item.test_packed_item import create_product_bundle
from capkpi.stock.doctype.pick_list.pick_list import create_delivery_note
...
class TestPickList(FinergyTestCase):
	...
	def test_pick_list_shows_serial_no_for_serialized_item(self):
		...
		stock_reconciliation = finergy.get_doc(
			{
				"doctype": "Stock Reconciliation",
	...
	def test_pick_list_shows_batch_no_for_batched_item(self):
		...
		item = finergy.db.exists("Item", {"item_name": "Batched Item"})
		if not item:
			...
			item = finergy.get_doc("Item", {"item_name": "Batched Item"})
	...
	def test_pick_list_for_items_from_multiple_sales_orders(self):
		...
		sales_order = finergy.get_doc(
			{
				"doctype": "Sales Order",
				"customer": "_Test Customer",
				"company": "_Test Company",
				"items": [{"item_code": "_Test Item", "qty": 10, "delivery_date": finergy.utils.today()}],
			}
		)
		sales_order.submit()
				...
				"items": [
					{
						...
						"conversion_factor": 5,
						"stock_qty": 5,
						"delivery_date": finergy.utils.today(),
					},
					{
						"item_code": "_Test Item",
						"qty": 1,
	...
	def test_multiple_dn_creation(self):
		...
		for dn in finergy.get_all(
			"Delivery Note",
			filters={"pick_list": pick_list.name, "customer": "_Test Customer"},
			fields={"name"},
		):
			for dn_item in finergy.get_doc("Delivery Note", dn.name).get("items"):
