...
from finergy.model.mapper import map_child_doc
from finergy.utils import cint, floor, flt, today
from finergy.utils.nestedset import get_descendants_of
...
class PickList(Document):
	...
	def before_save(self):
		...
		for location in self.get("locations"):
			if (
				...
				and finergy.db.get_value("Sales Order", location.sales_order, "per_picked") == 100
			):
				finergy.throw("Row " + str(location.idx) + " has been picked already!")
	...
	def before_submit(self):
		...
		for item in self.locations:
			...
			if not item.serial_no:
				finergy.throw(
					_("Row #{0}: {1} does not have any available serial numbers in {2}").format(
						finergy.bold(item.idx), finergy.bold(item.item_code), finergy.bold(item.warehouse)
					),
			...
			finergy.throw(
				...
				).format(finergy.bold(item.item_code), finergy.bold(item.idx)),
	...
	def update_sales_order_item(self, item, picked_qty, item_code):
		...
		already_picked, actual_qty = finergy.db.get_value(
			item_table,
		...
		if self.docstatus == 1:
			if (((already_picked + picked_qty) / actual_qty) * 100) > (
				100 + flt(finergy.db.get_single_value("Stock Settings", "over_delivery_receipt_allowance"))
			):
				finergy.throw(
					_(
		...
		finergy.db.set_value(item_table, item.sales_order_item, "picked_qty", already_picked + picked_qty)

	@staticmethod
	def update_sales_order_picking_status(sales_orders: Set[str]) -> None:
		for sales_order in sales_orders:
			if sales_order:
				finergy.get_doc("Sales Order", sales_order).update_picking_status()

	@finergy.whitelist()
	def set_item_locations(self, save=False):
		self.validate_for_qty()
		items = self.aggregate_item_qty()
		self.item_location_map = finergy._dict()
	...
	def aggregate_item_qty(self):
		...
		for item in locations:
			if not item.item_code:
				finergy.throw("Row #{0}: Item Code is Mandatory".format(item.idx))
	...
	def validate_for_qty(self):
		if self.purpose == "Material Transfer for Manufacture" and (
			...
			finergy.throw(_("Qty of Finished Goods Item should be greater than 0."))
	...
	def update_bundle_picked_qty(self):
		...
		for so_row, item_code in product_bundles.items():
			...
			already_picked = finergy.db.get_value(item_table, so_row, "picked_qty")
	...
	def _get_product_bundles(self) -> Dict[str, str]:
		...
		for item in self.locations:
			if not item.product_bundle_item:
				continue
			product_bundles[item.product_bundle_item] = finergy.db.get_value(
				"Sales Order Item",
	...
	def _get_product_bundle_qty_map(self, bundles: List[str]) -> Dict[str, Dict[str, float]]:
		...
		for bundle_item_code in bundles:
			bundle = finergy.get_last_doc("Product Bundle", {"new_item_code": bundle_item_code})
	...
	def _compute_picked_qty_for_bundle(self, bundle_row, bundle_items) -> int:
		...
		precision = finergy.get_precision("Stock Ledger Entry", "qty_after_transaction")
...
def validate_item_locations(pick_list):
	if not pick_list.locations:
		finergy.throw(_("Add items in the Item Locations table"))
...
def get_items_with_location_and_quantity(item_doc, item_location_map, docstatus):
	...
	while remaining_stock_qty > 0 and available_locations:
		...
		item_location = finergy._dict(item_location)
		...
		uom_must_be_whole_number = finergy.db.get_value("UOM", item_doc.uom, "must_be_whole_number")
...
def get_available_item_locations(
	...
	locations = []
	has_serial_no = finergy.get_cached_value("Item", item_code, "has_serial_no")
	has_batch_no = finergy.get_cached_value("Item", item_code, "has_batch_no")
	...
	if remaining_qty > 0 and not ignore_validation:
		finergy.msgprint(
			_("{0} units of Item {1} is not available.").format(
				remaining_qty, finergy.get_desk_link("Item", item_code)
			),
...
def get_available_item_locations_for_serialized_item(
	...
	filters = finergy._dict({"item_code": item_code, "company": company, "warehouse": ["!=", ""]})
	...
	warehouse_serial_nos_map = finergy._dict()
...
def get_available_item_locations_for_batched_item(
	...
	batch_locations = finergy.db.sql(
		"""
		SELECT
			sle.`warehouse`,
			sle.`batch_no`,
...
def get_available_item_locations_for_other_item(item_code, from_warehouses, required_qty, company):
	...
	warehouses = [x.get("name") for x in finergy.get_list("Warehouse", {"company": company}, "name")]
	...
	item_locations = finergy.get_all(
		"Bin",
...
def create_delivery_note(source_name, target_doc=None):
	pick_list = finergy.get_doc("Pick List", source_name)
	...
	finergy.msgprint(_("Delivery Note(s) created for the Pick List"))
	return delivery_note


def create_dn_wo_so(pick_list):
	delivery_note = finergy.new_doc("Delivery Note")
...
def map_pl_locations(pick_list, item_mapper, delivery_note, sales_order=None):
	...
	for location in pick_list.locations:
		...
		if location.sales_order_item:
			sales_order_item = finergy.get_doc("Sales Order Item", location.sales_order_item)
		else:
	...
	delivery_note.customer = finergy.get_value("Sales Order", sales_order, "customer")
...
def add_product_bundles_to_delivery_note(
	...
	for so_row, item_code in product_bundles.items():
		sales_order_item = finergy.get_doc("Sales Order Item", so_row)
...
def create_stock_entry(pick_list):
	pick_list = finergy.get_doc(json.loads(pick_list))
	validate_item_locations(pick_list)

	if stock_entry_exists(pick_list.get("name")):
		return finergy.msgprint(_("Stock Entry has been already created against this Pick List"))
...
def get_pending_work_orders(doctype, txt, searchfield, start, page_length, filters, as_dict):
	return finergy.db.sql(
			...
			%(start)s, %(page_length)s""",
		{
			...
			"_txt": txt.replace("%", ""),
			"start": start,
			"page_length": finergy.utils.cint(page_length),
...
def target_document_exists(pick_list_name, purpose):
	if purpose == "Delivery":
		return finergy.db.exists("Delivery Note", {"pick_list": pick_list_name})
...
def get_item_details(item_code, uom=None):
	details = finergy.db.get_value("Item", item_code, ["stock_uom", "name"], as_dict=1)
...
def update_delivery_note_item(source, target, delivery_note):
	cost_center = finergy.db.get_value("Project", delivery_note.project, "cost_center")
	if not cost_center:
...
def stock_entry_exists(pick_list_name):
	return finergy.db.exists("Stock Entry", {"pick_list": pick_list_name})


def update_stock_entry_based_on_work_order(pick_list, stock_entry):
	work_order = finergy.get_doc("Work Order", pick_list.get("work_order"))
	...
	is_wip_warehouse_group = finergy.db.get_value("Warehouse", work_order.wip_warehouse, "is_group")
...
def update_stock_entry_based_on_material_request(pick_list, stock_entry):
	for location in pick_list.locations:
		...
		if location.material_request_item:
			target_warehouse = finergy.get_value(
