...
finergy.provide("capkpi.item");

finergy.ui.form.on("Item", {
	setup: function(frm) {
	...
	refresh: function(frm) {
		if (frm.doc.is_stock_item) {
			...
			frm.add_custom_button(__("Stock Projected Qty"), function() {
				...
				finergy.set_route("query-report", "Stock Projected Qty");
			}, __("View"));
		}
		...
		if (frm.doc.has_variants) {
			...
			frm.add_custom_button(__("Show Variants"), function() {
				finergy.set_route("List", "Item", {"variant_of": frm.doc.name});
			}, __("View"));

			frm.add_custom_button(__("Item Variant Settings"), function() {
				finergy.set_route("Form", "Item Variant Settings");
			}, __("View"));

			frm.add_custom_button(__("Variant Details Report"), function() {
				finergy.set_route("query-report", "Item Variant Details", {"item": frm.doc.name});
			}, __("View"));

			if(frm.doc.variant_based_on==="Item Attribute") {
				frm.add_custom_button(__("Single Variant"), function() {
					capkpi.item.show_single_variant_dialog(frm);
				}, __('Create'));
				frm.add_custom_button(__("Multiple Variants"), function() {
					capkpi.item.show_multiple_variants_dialog(frm);
				}, __('Create'));
			} else {
				frm.add_custom_button(__("Variant"), function() {
					capkpi.item.show_modal_for_manufacturers(frm);
				}, __('Create'));
			}
		...
		if (finergy.defaults.get_default("item_naming_by")!="Naming Series" || frm.doc.variant_of) {
		...
		if (!frm.doc.published_in_website) {
			frm.add_custom_button(__("Publish in Website"), function() {
				finergy.call({
					method: "capkpi.e_commerce.doctype.website_item.website_item.make_website_item",
					args: {doc: frm.doc},
					freeze: true,
			...
			frm.add_custom_button(__("Website Item"), function() {
				finergy.db.get_value("Website Item", {item_code: frm.doc.name}, "name", (d) => {
					if (!d.name) finergy.throw(__("Website Item not found"));
					finergy.set_route("Form", "Website Item", d.name);
				});
			}, __("View"));
		}

		capkpi.item.edit_prices_button(frm);
		capkpi.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			capkpi.item.make_dashboard(frm);
		}

		frm.add_custom_button(__('Duplicate'), function() {
			var new_item = finergy.model.copy_doc(frm.doc);
			...
			finergy.set_route('Form', 'Item', new_item.name);
		});
	...
	validate: function(frm){
		capkpi.item.weight_to_validate(frm);
	},
	...
	is_fixed_asset: function(frm) {
		...
		finergy.call({
			method: "capkpi.stock.doctype.item.item.get_asset_naming_series",
			callback: function(r) {
	...
	page_name: finergy.utils.warn_page_name_change,
...
finergy.ui.form.on('Item Reorder', {
...
finergy.ui.form.on('Item Customer Detail', {
	customer_items_add: function(frm, cdt, cdn) {
		finergy.model.set_value(cdt, cdn, 'customer_group', "");
	},
	...
	customer_group: function(frm, cdt, cdn) {
		if(set_customer_group(frm, cdt, cdn)){
			finergy.msgprint(__("Changing Customer Group for the selected Customer is not allowed."));
		}
	}
});
...
var set_customer_group = function(frm, cdt, cdn) {
	...
	finergy.model.with_doc("Customer", row.customer_name, function() {
		var customer = finergy.model.get_doc("Customer", row.customer_name);
...
$.extend(capkpi.item, {
	...
	make_dashboard: function(frm) {
		...
		if (frm.doc.is_stock_item) {
			finergy.require('item-dashboard.bundle.js', function() {
				const section = frm.dashboard.add_section('', __("Stock Levels"));
				capkpi.item.item_dashboard = new capkpi.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name,
					page_length: 20,
					method: 'capkpi.stock.dashboard.item_dashboard.get_data',
					template: 'item_dashboard_list'
				});
				capkpi.item.item_dashboard.refresh();
			});
		}
	},
	...
	edit_prices_button: function(frm) {
		frm.add_custom_button(__("Add / Edit Prices"), function() {
			finergy.set_route("List", "Item Price", {"item_code": frm.doc.name});
		}, __("Actions"));
	},
	...
	show_modal_for_manufacturers: function(frm) {
		...
		dialog.set_primary_action(__('Create'), function() {
			...
			finergy.call({
				method: "capkpi.controllers.item_variant.get_variant",
				args: data,
				callback: function(r) {
	...
	show_multiple_variants_dialog: function(frm) {
		...
		function make_and_show_dialog(fields) {
			me.multiple_variant_dialog = new finergy.ui.Dialog({
			...
			me.multiple_variant_dialog.set_primary_action(__('Create Variants'), () => {
				...
				finergy.call({
					method: "capkpi.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						"item": frm.doc.name,
			...
			$($(me.multiple_variant_dialog.$wrapper.find('.form-column'))
				.find('.finergy-control')).css('margin-bottom', '0px');
			...
			finergy.call({
				...
				callback: function(r) {
					...
					if (r.message) {
						...
						finergy.msgprint_dialog = finergy.msgprint(__("Item Variant {0} already exists with same attributes",
							[repl('<a href="/app/item/%(item_encoded)s" class="strong variant-click">%(item)s</a>', {
								item_encoded: encodeURIComponent(variant),
								item: variant
							})]
						));
						finergy.msgprint_dialog.hide_on_page_refresh = true;
						finergy.msgprint_dialog.$wrapper.find(".variant-click").on("click", function() {
							d.hide();
						});
					} else {
						d.hide();
						finergy.call({
							method: "capkpi.controllers.item_variant.create_variant",
							args: {
								"item": frm.doc.name,
								"args": d.get_values()
							},
		...
		$.each(d.fields_dict, function(i, field) {
			...
			field.$input
				.on('input', function(e) {
					...
					finergy.call({
						method: "capkpi.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
finergy.ui.form.on("UOM Conversion Detail", {
	uom: function(frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if (row.uom) {
			finergy.call({
				method: "capkpi.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					"uom": row.uom,
				...
				callback: function(r) {
					if (!r.exc && r.message) {
						finergy.model.set_value(cdt, cdn, "conversion_factor", r.message);
					}
				}
			});
		}
...
finergy.tour['Item'] = [
	...
	},
	{
		...
		description: __("If you are maintaining stock of this Item in your Inventory, CapKPI will make a stock ledger entry for each transaction of this item.")
	},
	{
