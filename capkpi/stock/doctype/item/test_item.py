...
from capkpi.stock.doctype.item.item import (
	DataValidationError,
	InvalidBarcode,
...
def make_item(item_code=None, properties=None):
	if not item_code:
		item_code = finergy.generate_hash(length=16)

	if finergy.db.exists("Item", item_code):
		return finergy.get_doc("Item", item_code)
...
class TestItem(FinergyTestCase):
	def setUp(self):
		super().setUp()
	...
	def get_item(self, idx):
		...
		if not finergy.db.exists("Item", item_code):
			item = finergy.copy_doc(test_records[idx])
			item.insert()
		else:
	...
	def test_get_item_details(self):
		...
		finergy.db.sql("""delete from `tabItem Price`""")
		...
		currency = finergy.get_cached_value("Company", company, "default_currency")
	...
	def test_item_defaults(self):
		finergy.delete_doc_if_exists("Item", "Test Item With Defaults", force=1)
		make_item(
	...
	def test_item_default_validations(self):
		...
		with self.assertRaises(finergy.ValidationError) as ve:
			make_item(
				"Bad Item defaults",
				{
					"item_group": "_Test Item Group",
	...
	def test_item_attribute_change_after_variant(self):
		finergy.delete_doc_if_exists("Item", "_Test Variant Item-L", force=1)

		variant = create_variant("_Test Variant Item", {"Test Size": "Large"})
		variant.save()

		attribute = finergy.get_doc("Item Attribute", "Test Size")
	...
	def test_copy_fields_from_template_to_variants(self):
		finergy.delete_doc_if_exists("Item", "_Test Variant Item-XL", force=1)
		...
		if not finergy.db.get_value(
			...
			item_attribute = finergy.get_doc("Item Attribute", "Test Size")
		...
		variant = finergy.get_doc("Item", "_Test Variant Item-XL")
	...
	def test_make_item_variant_with_numeric_values(self):
		# cleanup
		for d in finergy.db.get_all("Item", filters={"variant_of": "_Test Numeric Template Item"}):
			finergy.delete_doc_if_exists("Item", d.name)

		finergy.delete_doc_if_exists("Item", "_Test Numeric Template Item")
		finergy.delete_doc_if_exists("Item Attribute", "Test Item Length")
	...
	def test_item_merging(self):
		old = create_item(finergy.generate_hash(length=20)).name
		new = create_item(finergy.generate_hash(length=20)).name
		...
		finergy.rename_doc("Item", old, new, merge=True)

		self.assertFalse(finergy.db.exists("Item", old))

		self.assertTrue(
			finergy.db.get_value("Bin", {"item_code": new, "warehouse": "_Test Warehouse - _TC"})
		)
		self.assertTrue(
			finergy.db.get_value("Bin", {"item_code": new, "warehouse": "_Test Warehouse 1 - _TC"})
		)
	...
	def test_item_merging_with_product_bundle(self):
		...
		with self.assertRaises(DataValidationError):
			finergy.rename_doc("Item", "Test Item Bundle Item 1", "Test Item Bundle Item 2", merge=True)

		bundle1.delete()
		finergy.rename_doc("Item", "Test Item Bundle Item 1", "Test Item Bundle Item 2", merge=True)

		self.assertFalse(finergy.db.exists("Item", "Test Item Bundle Item 1"))

	def test_uom_conversion_factor(self):
		if finergy.db.exists("Item", "Test Item UOM"):
			finergy.delete_doc("Item", "Test Item UOM")
	...
	def test_item_variant_by_manufacturer(self):
		...
		if finergy.db.exists("Item", "_Test Variant Mfg"):
			finergy.delete_doc("Item", "_Test Variant Mfg")
		if finergy.db.exists("Item", "_Test Variant Mfg-1"):
			finergy.delete_doc("Item", "_Test Variant Mfg-1")
		if finergy.db.exists("Manufacturer", "MSG1"):
			finergy.delete_doc("Manufacturer", "MSG1")
		...
		manufacturer = finergy.get_doc(dict(doctype="Manufacturer", short_name="MSG1")).insert()
	...
	def test_stock_exists_against_template_item(self):
		stock_item = finergy.get_all("Stock Ledger Entry", fields=["item_code"], limit=1)
		if stock_item:
			item_code = stock_item[0].item_code

			item_doc = finergy.get_doc("Item", item_code)
			item_doc.has_variants = 1
			self.assertRaises(StockExistsForTemplate, item_doc.save)

	def test_add_item_barcode(self):
		# Clean up
		finergy.db.sql("""delete from `tabItem Barcode`""")
		item_code = "Test Item Barcode"
		if finergy.db.exists("Item", item_code):
			finergy.delete_doc("Item", item_code)
		...
		self.assertRaises(finergy.UniqueValidationError, item_doc.save)
	...
	def test_index_creation(self):
		...
		indices = finergy.db.sql("show index from tabItem", as_dict=1)
	...
	def test_check_stock_uom_with_bin(self):
		...
		item = finergy.get_doc("Item", "_Test Item")
		item.stock_uom = "Gram"
		self.assertRaises(finergy.ValidationError, item.save)

	def test_check_stock_uom_with_bin_no_sle(self):
		from capkpi.stock.stock_balance import update_bin_qty
		...
		try:
			item.save()
		except finergy.ValidationError as e:
	...
	def test_erasure_of_old_conversions(self):
		...
		item.stock_uom = "Gram"
		item.append("uoms", finergy._dict(uom="Box", conversion_factor=2))
		item.save()
		item.reload()
		item.stock_uom = "Nos"
		item.save()
	...
	def test_validate_stock_item(self):
		self.assertRaises(finergy.ValidationError, validate_is_stock_item, "_Test Non Stock Item")
	...
	def test_item_dashboard(self):
		from capkpi.stock.dashboard.item_dashboard import get_data
...
def set_item_variant_settings(fields):
	doc = finergy.get_doc("Item Variant Settings")
	doc.set("fields", fields)
	doc.save()


def make_item_variant():
	if not finergy.db.exists("Item", "_Test Variant Item-S"):
...
test_records = finergy.get_test_records("Item")
