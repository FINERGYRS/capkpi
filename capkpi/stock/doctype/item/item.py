...
from finergy.utils.html_utils import clean_html
...
from capkpi.setup.doctype.item_group.item_group import invalidate_cache_for
from capkpi.stock.doctype.item_default.item_default import ItemDefault


class DuplicateReorderRows(finergy.ValidationError):
	pass


class StockExistsForTemplate(finergy.ValidationError):
	pass


class InvalidBarcode(finergy.ValidationError):
	pass


class DataValidationError(finergy.ValidationError):
	pass
...
class Item(Document):
	...
	def autoname(self):
		if finergy.db.get_default("item_naming_by") == "Naming Series":
			if self.variant_of:
				if not self.item_code:
					template_item_name = finergy.db.get_value("Item", self.variant_of, "item_name")
	...
	def after_insert(self):
		...
		if self.standard_rate:
			for default in self.item_defaults or [finergy._dict()]:
	...
	def validate(self):
		...
		if not self.is_new():
			self.old_item_group = finergy.db.get_value(self.doctype, self.name, "item_group")
	...
	def validate_description(self):
		...
		if cint(finergy.db.get_single_value("Stock Settings", "clean_description_html")):
			self.description = clean_html(self.description)

	def validate_customer_provided_part(self):
		if self.is_customer_provided_item:
			if self.is_purchase_item:
				finergy.throw(_('"Customer Provided Item" cannot be Purchase Item also'))
			if self.valuation_rate:
				finergy.throw(_('"Customer Provided Item" cannot have Valuation Rate'))
			self.default_material_request_type = "Customer Provided"

	def add_price(self, price_list=None):
		"""Add a new price"""
		if not price_list:
			price_list = finergy.db.get_single_value(
				"Selling Settings", "selling_price_list"
			) or finergy.db.get_value("Price List", _("Standard Selling"))
		if price_list:
			item_price = finergy.get_doc(
				{
					"doctype": "Item Price",
					...
					"uom": self.stock_uom,
					"brand": self.brand,
					"currency": capkpi.get_default_currency(),
	...
	def set_opening_stock(self):
		...
		if not self.valuation_rate and not self.is_customer_provided_item:
			finergy.throw(_("Valuation Rate is mandatory if Opening Stock entered"))

		from capkpi.stock.doctype.stock_entry.stock_entry_utils import make_stock_entry

		# default warehouse, or Stores
		for default in self.item_defaults or [
			finergy._dict({"company": finergy.defaults.get_defaults().company})
		]:
			default_warehouse = default.default_warehouse or finergy.db.get_single_value(
				"Stock Settings", "default_warehouse"
			)
			if default_warehouse:
				warehouse_company = finergy.db.get_value("Warehouse", default_warehouse, "company")

			if not default_warehouse or warehouse_company != default.company:
				default_warehouse = finergy.db.get_value(
	...
	def validate_fixed_asset(self):
		if self.is_fixed_asset:
			if self.is_stock_item:
				finergy.throw(_("Fixed Asset Item must be a non-stock item."))

			if not self.asset_category:
				finergy.throw(_("Asset Category is mandatory for Fixed Asset item"))

			if self.stock_ledger_created():
				finergy.throw(_("Cannot be a fixed asset item as Stock Ledger is created."))

		if not self.is_fixed_asset:
			asset = finergy.db.get_all("Asset", filters={"item_code": self.name, "docstatus": 1}, limit=1)
			if asset:
				finergy.throw(
	...
	def validate_retain_sample(self):
		if self.retain_sample and not finergy.db.get_single_value(
			"Stock Settings", "sample_retention_warehouse"
		):
			finergy.throw(_("Please select Sample Retention Warehouse in Stock Settings first"))
	...
	def update_website_item(self):
		...
		web_item = finergy.db.exists("Website Item", {"item_code": self.item_code})
		...
		if web_item:
			...
			web_item_doc = finergy.get_doc("Website Item", web_item)
	...
	def validate_item_tax_net_rate_range(self):
		for tax in self.get("taxes"):
			if flt(tax.maximum_net_rate) < flt(tax.minimum_net_rate):
				finergy.throw(_("Row #{0}: Maximum Net Rate cannot be greater than Minimum Net Rate"))

	def update_template_tables(self):
		template = finergy.get_doc("Item", self.variant_of)
	...
	def validate_item_type(self):
		if self.has_serial_no == 1 and self.is_stock_item == 0 and not self.is_fixed_asset:
			finergy.throw(_("'Has Serial No' can not be 'Yes' for non-stock item"))
			...
			if series and "#" in series and "." not in series:
				finergy.throw(
					_("Invalid naming series (. missing) for {0}").format(
						finergy.bold(self.meta.get_field(field).label)
					)
				)

	def check_for_active_boms(self):
		if self.default_bom:
			bom_item = finergy.db.get_value("BOM", self.default_bom, "item")
		...
		for d in self.get("taxes"):
			if d.item_tax_template:
				if d.item_tax_template in check_list:
					finergy.throw(_("{0} entered twice in Item Tax").format(d.item_tax_template))
				else:
	...
	def validate_barcode(self):
		...
		if len(self.barcodes) > 0:
			for item_barcode in self.barcodes:
				options = finergy.get_meta("Item Barcode").get_options("barcode_type").split("\n")
				if item_barcode.barcode:
					duplicate = finergy.db.sql(
	...
	def validate_warehouse_for_reorder(self):
		...
		for d in self.get("reorder_levels"):
			...
			if d.warehouse_reorder_level and not d.warehouse_reorder_qty:
				finergy.throw(_("Row #{0}: Please set reorder quantity").format(d.idx))
	...
	def on_trash(self):
		finergy.db.sql("""delete from tabBin where item_code=%s""", self.name)
		finergy.db.sql("delete from `tabItem Price` where item_code=%s", self.name)
		for variant_of in finergy.get_all("Item", filters={"variant_of": self.name}):
			finergy.delete_doc("Item", variant_of.name)

	def before_rename(self, old_name, new_name, merge=False):
		if self.item_name == old_name:
			finergy.db.set_value("Item", old_name, "item_name", new_name)
	...
	def after_rename(self, old_name, new_name, merge):
		...
		finergy.db.set_value("Item", new_name, "item_code", new_name)
	...
	def validate_duplicate_item_in_stock_reconciliation(self, old_name, new_name):
		...
		msg = _("The items {0} and {1} are present in the following {2} :").format(
			finergy.bold(old_name), finergy.bold(new_name), document
		)
		...
		).format(finergy.bold(old_name))

		finergy.throw(_(msg), title=_("Cannot Merge"), exc=DataValidationError)

	def validate_properties_before_merge(self, new_name):
		# Validate properties before merging
		if not finergy.db.exists("Item", new_name):
			finergy.throw(_("Item {0} does not exist").format(new_name))

		field_list = ["stock_uom", "is_stock_item", "has_serial_no", "has_batch_no"]
		new_properties = [cstr(d) for d in finergy.db.get_value("Item", new_name, field_list)]
		...
		if new_properties != [cstr(self.get(field)) for field in field_list]:
			...
			finergy.throw(msg, title=_("Cannot Merge"), exc=DataValidationError)

	def validate_duplicate_product_bundles_before_merge(self, old_name, new_name):
		"Block merge if both old and new items have product bundles."
		old_bundle = finergy.get_value("Product Bundle", filters={"new_item_code": old_name})
		new_bundle = finergy.get_value("Product Bundle", filters={"new_item_code": new_name})

		if old_bundle and new_bundle:
			bundle_link = get_link_to_form("Product Bundle", old_bundle)
			old_name, new_name = finergy.bold(old_name), finergy.bold(new_name)
	...
	def validate_duplicate_website_item_before_merge(self, old_name, new_name):
		...
		msg = f"Please delete linked Website Item {finergy.bold(web_item_link)} before merging {old_name} into {new_name}"
		finergy.throw(_(msg), title=_("Cannot Merge"), exc=DataValidationError)

	def set_last_purchase_rate(self, new_name):
		last_purchase_rate = get_last_purchase_details(new_name).get("base_net_rate", 0)
		finergy.db.set_value("Item", new_name, "last_purchase_rate", last_purchase_rate)

	def recalculate_bin_qty(self, new_name):
		from capkpi.stock.stock_balance import repost_stock

		existing_allow_negative_stock = finergy.db.get_value(
			"Stock Settings", None, "allow_negative_stock"
		)
		finergy.db.set_value("Stock Settings", None, "allow_negative_stock", 1)

		repost_stock_for_warehouses = finergy.get_all(
			"Stock Ledger Entry",
			"warehouse",
		...
		finergy.db.delete("Bin", {"item_code": new_name})
	...
	def validate_item_defaults(self):
		...
		if len(companies) != len(self.item_defaults):
			finergy.throw(_("Cannot set multiple Item Defaults for a company."))
	...
	def update_defaults_from_item_group(self):
		...
		item_defaults = finergy.db.get_values(
			"Item Default",
		...
		if item_defaults:
			...
			defaults = finergy.defaults.get_defaults() or {}

			# To check default warehouse is belong to the default company
			if (
				defaults.get("default_warehouse")
				and defaults.company
				and finergy.db.exists(
	...
	def update_variants(self):
		if self.flags.dont_update_variants or finergy.db.get_single_value(
			"Item Variant Settings", "do_not_update_variants"
		):
			return
		if self.has_variants:
			variants = finergy.db.get_all("Item", fields=["item_code"], filters={"variant_of": self.name})
			if variants:
				if len(variants) <= 30:
					update_variants(variants, self, publish_progress=False)
					finergy.msgprint(_("Item Variants updated"))
				else:
					finergy.enqueue(
						"capkpi.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
	...
	def validate_has_variants(self):
		if not self.has_variants and finergy.db.get_value("Item", self.name, "has_variants"):
			if finergy.db.exists("Item", {"variant_of": self.name}):
				finergy.throw(_("Item has variants."))
	...
	def validate_attributes_in_variants(self):
		...
		items = [item["name"] for item in finergy.get_all("Item", {"variant_of": self.name})]
		...
		item_attributes = finergy.get_all(
		...
		for docname, attr_list in not_included.items():
			link = "<a href='/app/Form/Item/{0}'>{0}</a>".format(finergy.bold(_(docname)))
		...
		finergy.throw(message, title=_("Variant Attribute Error"), is_minimizable=True, wide=True)
	...
	def validate_variant_based_on_change(self):
		if not self.is_new() and (
			self.variant_of or (self.has_variants and finergy.get_all("Item", {"variant_of": self.name}))
		):
			if self.variant_based_on != finergy.db.get_value("Item", self.name, "variant_based_on"):
				finergy.throw(_("Variant Based On cannot be changed"))

	def validate_uom(self):
		if not self.is_new():
			check_stock_uom_with_bin(self.name, self.stock_uom)
		if self.has_variants:
			for d in finergy.db.get_all("Item", filters={"variant_of": self.name}):
				check_stock_uom_with_bin(d.name, self.stock_uom)
		if self.variant_of:
			template_uom = finergy.db.get_value("Item", self.variant_of, "stock_uom")
	...
	def validate_attributes(self):
		...
		if self.variant_based_on == "Item Attribute":
			...
			if not self.attributes:
				finergy.throw(_("Attribute table is mandatory"))
	...
	def cant_change(self):
		...
		values = finergy.db.get_value("Item", self.name, restricted_fields, as_dict=True)
		if not values:
			return

		if not values.get("valuation_method") and self.get("valuation_method"):
			values["valuation_method"] = (
				finergy.db.get_single_value("Stock Settings", "valuation_method") or "FIFO"
			)
		...
		if linked_doc := self._get_linked_submitted_documents(changed_fields):
			changed_field_labels = [finergy.bold(self.meta.get_label(f)) for f in changed_fields]
			msg = _(
			...
			if linked_doc and isinstance(linked_doc, dict):
				...
				msg += _("Example of a linked document: {0}").format(
					finergy.get_desk_link(linked_doc.doctype, linked_doc.docname)
				)

			finergy.throw(msg, title=_("Linked with submitted documents"))
	...
	def _get_linked_submitted_documents(self, changed_fields: List[str]) -> Optional[Dict[str, str]]:
		...
		for doctype in linked_doctypes:
			...
			if doctype in (
				...
				if linked_doc := finergy.db.get_value(
					"Stock Ledger Entry",
			...
			elif linked_doc := finergy.db.get_value(
				doctype,
				filters,
	...
	def validate_auto_reorder_enabled_in_stock_settings(self):
		if self.reorder_levels:
			enabled = finergy.db.get_single_value("Stock Settings", "auto_indent")
			if not enabled:
				finergy.msgprint(
...
def validate_end_of_life(item_code, end_of_life=None, disabled=None):
	if (not end_of_life) or (disabled is None):
		end_of_life, disabled = finergy.db.get_value("Item", item_code, ["end_of_life", "disabled"])
	...
	if disabled:
		finergy.throw(_("Item {0} is disabled").format(item_code))


def validate_is_stock_item(item_code, is_stock_item=None):
	if not is_stock_item:
		is_stock_item = finergy.db.get_value("Item", item_code, "is_stock_item")

	if is_stock_item != 1:
		finergy.throw(_("Item {0} is not a stock Item").format(item_code))


def validate_cancelled_item(item_code, docstatus=None):
	if docstatus is None:
		docstatus = finergy.db.get_value("Item", item_code, "docstatus")

	if docstatus == 2:
		finergy.throw(_("Item {0} is cancelled").format(item_code))
...
def get_last_purchase_details(item_code, doc_name=None, conversion_rate=1.0):
	...
	last_purchase_order = finergy.db.sql(
		"""\
	...
	last_purchase_receipt = finergy.db.sql(
		"""\
...
def invalidate_item_variants_cache_for_website(doc):
	...
	from capkpi.e_commerce.variant_selector.item_variants_cache import ItemVariantsCacheManager

	item_code = None
	is_web_item = doc.get("published_in_website") or doc.get("published")
	if doc.has_variants and is_web_item:
		item_code = doc.item_code
	elif doc.variant_of and finergy.db.get_value("Item", doc.variant_of, "published_in_website"):
...
def check_stock_uom_with_bin(item, stock_uom):
	if stock_uom == finergy.db.get_value("Item", item, "stock_uom"):
		return

	ref_uom = finergy.db.get_value("Stock Ledger Entry", {"item_code": item}, "stock_uom")
	...
	finergy.db.sql("""update `tabBin` set stock_uom=%s where item_code=%s""", (stock_uom, item))


def get_item_defaults(item_code, company):
	item = finergy.get_cached_doc("Item", item_code)
	...
	for d in item.item_defaults:
		if d.company == company:
			if not d.get(fieldname):
				finergy.db.set_value(d.doctype, d.name, fieldname, value)
			return
...
def get_item_details(item_code, company=None):
	out = finergy._dict()
	if company:
		out = get_item_defaults(item_code, company) or finergy._dict()

	doc = finergy.get_cached_doc("Item", item_code)
...
def get_uom_conv_factor(uom, stock_uom):
	...
	intermediate_match = finergy.db.sql(
		"""
...
def get_item_attribute(parent, attribute_value=""):
	...
	if not finergy.has_permission("Item"):
		finergy.throw(_("No Permission"))
...
def update_variants(variants, template, publish_progress=True):
	...
	for count, d in enumerate(variants, start=1):
		variant = finergy.get_doc("Item", d)
		copy_attributes_to_variant(template, variant)
		variant.save()
		if publish_progress:
			finergy.publish_progress(count / total * 100, title=_("Updating Variants..."))
	...
	for item_default in item_defaults:
		for doctype, field in [
			...
			if item_default.get(field):
				company = finergy.db.get_value(doctype, item_default.get(field), "company", cache=True)
				if company and company != item_default.company:
					finergy.throw(
						_("Row #{}: {} {} doesn't belong to Company {}. Please select valid {}.").format(
							item_default.idx,
							doctype,
							finergy.bold(item_default.get(field)),
							finergy.bold(item_default.company),
							finergy.bold(finergy.unscrub(field)),
						),
...
def get_asset_naming_series():
	from capkpi.assets.doctype.asset.asset import get_asset_naming_series
