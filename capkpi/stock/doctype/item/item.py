...
class Item(Document):
	...
	def cant_change(self):
		...
		values = finergy.db.get_value("Item", self.name, fields, as_dict=True)
				...
				if cstr(self.get(field)) != cstr(values.get(field)):
					if self.check_if_linked_document_exists(field):
						finergy.throw(
							...
							).format(self.name, finergy.bold(self.meta.get_label(field)))
						)
	...
	def check_if_linked_document_exists(self, field):
		...
		for doctype in linked_doctypes:
			...
			elif finergy.db.get_value(doctype, filters):
				return True
