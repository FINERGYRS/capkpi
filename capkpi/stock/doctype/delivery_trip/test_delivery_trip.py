...
from finergy.utils import add_days, flt, now_datetime, nowdate

import capkpi
from capkpi.stock.doctype.delivery_trip.delivery_trip import (
...
class TestDeliveryTrip(FinergyTestCase):
	def setUp(self):
		super().setUp()
	...
	def tearDown(self):
		finergy.db.sql("delete from `tabDriver`")
		finergy.db.sql("delete from `tabVehicle`")
		finergy.db.sql("delete from `tabEmail Template`")
		finergy.db.sql("delete from `tabDelivery Trip`")
...
def create_address(driver):
	if not finergy.db.exists("Address", {"address_title": "_Test Address for Driver"}):
		...
		finergy.db.set_value("Driver", driver.name, "address", address.name)

		return address

	return finergy.get_doc("Address", {"address_title": "_Test Address for Driver"})


def create_driver():
	if not finergy.db.exists("Driver", {"full_name": "Newton Scmander"}):
		driver = finergy.get_doc(
			{
				"doctype": "Driver",
	...
	return finergy.get_doc("Driver", {"full_name": "Newton Scmander"})


def create_delivery_notification():
	if not finergy.db.exists("Email Template", "Delivery Notification"):
		dispatch_template = finergy.get_doc(
			{
	...
	delivery_settings = finergy.get_single("Delivery Settings")
	delivery_settings.dispatch_template = "Delivery Notification"
	delivery_settings.save()


def create_vehicle():
	if not finergy.db.exists("Vehicle", "JB 007"):
