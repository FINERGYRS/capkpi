...
from finergy.utils import flt, get_time

from capkpi.accounts.party import get_party_shipping_address
...
class Shipment(Document):
	...
	def on_submit(self):
		if not self.shipment_parcel:
			finergy.throw(_("Please enter Shipment Parcel information"))
	...
	def validate_weight(self):
		for parcel in self.shipment_parcel:
			if flt(parcel.weight) <= 0:
				finergy.throw(_("Parcel weight cannot be 0"))
	...
	def validate_pickup_time(self):
		if self.pickup_from and self.pickup_to and get_time(self.pickup_to) < get_time(self.pickup_from):
			finergy.throw(_("Pickup To time should be greater than Pickup From time"))
