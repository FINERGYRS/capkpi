...
from capkpi.stock.doctype.delivery_note.delivery_note import make_shipment


class TestShipment(FinergyTestCase):
...
def create_test_shipment(delivery_notes=None):
	...
	shipment = finergy.new_doc("Shipment")
...
def get_shipment_customer_contact(customer_name):
	...
	contacts = finergy.get_all("Contact", fields=["name"], filters={"name": customer_name})
	if len(contacts):
		return contacts[0]
	else:
...
def get_shipment_customer_address(customer_name):
	...
	customer_address = finergy.get_all(
...
def get_shipment_customer():
	...
	customer = finergy.get_all("Customer", fields=["name"], filters={"name": customer_name})
	if len(customer):
		return customer[0]
	else:
...
def get_shipment_company_address(company_name):
	...
	addresses = finergy.get_all("Address", fields=["name"], filters={"address_title": address_title})
	if len(addresses):
		return addresses[0]
	else:
...
def get_shipment_company():
	return finergy.get_doc("Company", "_Test Company")
...
def create_customer_contact(fname, lname):
	customer = finergy.new_doc("Contact")
...
def create_material_receipt(item, company):
	...
	stock = finergy.new_doc("Stock Entry")
