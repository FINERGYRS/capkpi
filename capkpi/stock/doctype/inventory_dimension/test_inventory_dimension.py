...
class TestInventoryDimension(FinergyTestCase):
	def setUp(self):
		prepare_test_data()
	...
	def test_delete_inventory_dimension(self):
		...
		custom_field = finergy.db.get_value(
	...
	def test_inventory_dimension(self):
		...
		sle_shelf = finergy.db.get_value("Stock Ledger Entry", {"voucher_no": outward.name}, "shelf")
...
def prepare_test_data():
	if not finergy.db.exists("DocType", "Shelf"):
		finergy.get_doc(
			{
				"doctype": "DocType",
				"name": "Shelf",
	...
	for shelf in ["Shelf 1", "Shelf 2"]:
		if not finergy.db.exists("Shelf", shelf):
			finergy.get_doc({"doctype": "Shelf", "shelf_name": shelf}).insert(ignore_permissions=True)
...
def create_inventory_dimension(**args):
	...
	if finergy.db.exists("Inventory Dimension", args.dimension_name):
		return finergy.get_doc("Inventory Dimension", args.dimension_name)

	doc = finergy.new_doc("Inventory Dimension")
	doc.update(args)
