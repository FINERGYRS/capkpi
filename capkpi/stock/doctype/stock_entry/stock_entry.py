...
from finergy.query_builder.functions import Sum
from finergy.utils import cint, comma_or, cstr, flt, format_time, formatdate, getdate, nowdate

import capkpi
from capkpi.accounts.general_ledger import process_gl_map
from capkpi.controllers.taxes_and_totals import init_landed_taxes_and_totals
from capkpi.manufacturing.doctype.bom.bom import add_additional_cost, validate_bom_no
from capkpi.setup.doctype.brand.brand import get_brand_defaults
from capkpi.setup.doctype.item_group.item_group import get_item_group_defaults
from capkpi.stock.doctype.batch.batch import get_batch_no, get_batch_qty, set_batch_nos
from capkpi.stock.doctype.item.item import get_item_defaults
from capkpi.stock.doctype.serial_no.serial_no import (
	get_serial_nos,
	update_serial_nos_after_submit,
)
from capkpi.stock.doctype.stock_reconciliation.stock_reconciliation import (
	OpeningEntryAccountError,
)
from capkpi.stock.get_item_details import (
	get_bin_details,
	get_conversion_factor,
	get_default_cost_center,
	get_reserved_qty_for_so,
)
from capkpi.stock.stock_ledger import NegativeStockError, get_previous_sle, get_valuation_rate
from capkpi.stock.utils import get_bin, get_incoming_rate


class FinishedGoodError(finergy.ValidationError):
	pass


class IncorrectValuationRateError(finergy.ValidationError):
	pass


class DuplicateEntryForWorkOrderError(finergy.ValidationError):
	pass


class OperationsNotCompleteError(finergy.ValidationError):
	pass


class MaxSampleAlreadyRetainedError(finergy.ValidationError):
	pass


from capkpi.controllers.stock_controller import StockController
...
class StockEntry(StockController):
	def __init__(self, *args, **kwargs):
		...
		if self.purchase_order:
			self.subcontract_data = finergy._dict(
				{
	...
	def before_validate(self):
		from capkpi.stock.doctype.putaway_rule.putaway_rule import apply_putaway_rule
	...
	def validate(self):
		self.pro_doc = finergy._dict()
		if self.work_order:
			self.pro_doc = finergy.get_doc("Work Order", self.work_order)
	...
	def validate_work_order_status(self):
		pro_doc = finergy.get_doc("Work Order", self.work_order)
		if pro_doc.status == "Completed":
			finergy.throw(_("Cannot cancel transaction for Completed Work Order."))
	...
	def validate_purpose(self):
		...
		if self.purpose not in valid_purposes:
			finergy.throw(_("Purpose must be one of {0}").format(comma_or(valid_purposes)))
	...
	def delete_linked_stock_entry(self):
		if self.purpose == "Send to Warehouse":
			for d in finergy.get_all(
				...
				finergy.delete_doc("Stock Entry", d.name)

	def set_transfer_qty(self):
		for item in self.get("items"):
			if not flt(item.qty):
				finergy.throw(_("Row {0}: Qty is mandatory").format(item.idx), title=_("Zero quantity"))
			if not flt(item.conversion_factor):
				finergy.throw(_("Row {0}: UOM Conversion Factor is mandatory").format(item.idx))
	...
	def update_cost_in_project(self):
		if self.work_order and not finergy.db.get_value(
			"Work Order", self.work_order, "update_consumed_material_cost_in_project"
		):
			return

		if self.project:
			amount = finergy.db.sql(
			...
			additional_costs = finergy.db.sql(
			...
			finergy.db.set_value("Project", self.project, "total_consumed_material_cost", amount)
			...
			if flt(item.qty) and flt(item.qty) < 0:
				finergy.throw(
					_("Row {0}: The item {1}, quantity must be positive number").format(
						item.idx, finergy.bold(item.item_code)
					)
				)

			if item.item_code not in stock_items:
				finergy.throw(_("{0} is not a stock Item").format(item.item_code))
	...
	def validate_qty(self):
		...
		if self.purpose in manufacture_purpose and self.work_order:
			if not finergy.get_value("Work Order", self.work_order, "skip_transfer"):
				item_code = []
				for item in self.items:
					if cstr(item.t_warehouse) == "":
						req_items = finergy.get_all(
							"Work Order Item",
							filters={"parent": self.work_order, "item_code": item.item_code},
							fields=["item_code"],
						)

						transferred_materials = finergy.db.sql(
							"""
									select
										sum(qty) as qty
	...
	def validate_fg_completed_qty(self):
		...
		for item_code, qty_list in item_wise_qty.items():
			total = flt(sum(qty_list), finergy.get_precision("Stock Entry Detail", "qty"))
			if self.fg_completed_qty != total:
				finergy.throw(
					_("The finished product {0} quantity {1} and For Quantity {2} cannot be different").format(
						finergy.bold(item_code), finergy.bold(total), finergy.bold(self.fg_completed_qty)
					)
				)

	def validate_difference_account(self):
		if not cint(capkpi.is_perpetual_inventory_enabled(self.company)):
			return
		...
		for d in self.get("items"):
			if not d.expense_account:
				finergy.throw(
					...
					).format(finergy.bold(self.company))
				)

			elif (
				self.is_opening == "Yes"
				and finergy.db.get_value("Account", d.expense_account, "report_type") == "Profit and Loss"
			):
				finergy.throw(
					_(
		...
		for d in self.get("items"):
			...
			if self.purpose in source_mandatory and not d.s_warehouse:
				if self.from_warehouse:
					...
					finergy.throw(_("Source warehouse is mandatory for row {0}").format(d.idx))

			if self.purpose in target_mandatory and not d.t_warehouse:
				if self.to_warehouse:
					d.t_warehouse = self.to_warehouse
				else:
					finergy.throw(_("Target warehouse is mandatory for row {0}").format(d.idx))
			...
			if cstr(d.s_warehouse) == cstr(d.t_warehouse) and self.purpose not in [
				...
				finergy.throw(_("Source and target warehouse cannot be same for row {0}").format(d.idx))

			if not (d.s_warehouse or d.t_warehouse):
				finergy.throw(_("Atleast one warehouse is mandatory"))
		...
		if self.purpose in (
			...
			if (
				...
				if not self.fg_completed_qty:
					finergy.throw(_("For Quantity (Manufactured Qty) is mandatory"))
	...
	def check_if_operations_completed(self):
		...
		prod_order = finergy.get_doc("Work Order", self.work_order)
		allowance_percentage = flt(
			finergy.db.get_single_value("Manufacturing Settings", "overproduction_percentage_for_work_order")
		)
		...
		for d in prod_order.get("operations"):
			...
			if total_completed_qty > flt(completed_qty):
				job_card = finergy.db.get_value("Job Card", {"operation_id": d.name}, "name")
				if not job_card:
					finergy.throw(
				...
				work_order_link = finergy.utils.get_link_to_form("Work Order", self.work_order)
				job_card_link = finergy.utils.get_link_to_form("Job Card", job_card)
				finergy.throw(
					_(
						...
						d.idx,
						finergy.bold(d.operation),
						finergy.bold(total_completed_qty),
						work_order_link,
						job_card_link,
					),
	...
	def check_duplicate_entry_for_work_order(self):
		other_ste = [
			t[0]
			for t in finergy.db.get_values(
				"Stock Entry",
				{
		...
		if other_ste:
			production_item, qty = finergy.db.get_value(
				"Work Order", self.work_order, ["production_item", "qty"]
			)
			args = other_ste + [production_item]
			fg_qty_already_entered = finergy.db.sql(
	...
	def set_actual_qty(self):
		from capkpi.stock.stock_ledger import is_negative_stock_allowed
			...
			if (
				...
				finergy.throw(
					_(
						...
						d.idx,
						finergy.bold(d.s_warehouse),
						formatdate(self.posting_date),
						format_time(self.posting_time),
						finergy.bold(d.item_code),
					)
					+ "<br><br>"
					+ _("Available quantity is {0}, you need {1}").format(
						finergy.bold(flt(d.actual_qty, d.precision("actual_qty"))), finergy.bold(d.transfer_qty)
					),
					NegativeStockError,
		...
		for d in self.get("items"):
			...
			if not d.basic_rate and not d.allow_zero_valuation_rate:
				d.basic_rate = get_valuation_rate(
					...
					currency=capkpi.get_company_currency(self.company),
					company=self.company,
	...
	def get_basic_rate_for_manufactured_item(self, finished_item_qty, outgoing_items_cost=0) -> float:
		...
		if not outgoing_items_cost and finergy.db.get_single_value(
	...
	def set_stock_entry_type(self):
		if self.purpose:
			self.stock_entry_type = finergy.get_cached_value(
	...
	def set_purpose_for_stock_entry(self):
		if self.stock_entry_type and not self.purpose:
			self.purpose = finergy.get_cached_value("Stock Entry Type", self.stock_entry_type, "purpose")
	...
	def validate_subcontract_order(self):
		...
		backflush_raw_materials_based_on = finergy.db.get_single_value(
			"Buying Settings", "backflush_raw_materials_of_subcontract_based_on"
		)

		qty_allowance = flt(finergy.db.get_single_value("Buying Settings", "over_transfer_allowance"))
		...
		if backflush_raw_materials_based_on == "BOM":
			subcontract_order = finergy.get_doc(
			...
			for se_item in self.items:
				...
				if not required_qty:
					...
					if se_item.allow_alternative_item:
						original_item_code = finergy.get_value(
				...
				parent = finergy.qb.DocType("Stock Entry")
				child = finergy.qb.DocType("Stock Entry Detail")
				...
				if flt(total_supplied, precision) > flt(total_allowed, precision):
					...
					order_rm_detail = finergy.db.get_value(
				...
				if not row.subcontracted_item:
					finergy.throw(
						_("Row {0}: Subcontracted Item is mandatory for the raw material {1}").format(
							row.idx, finergy.bold(row.item_code)
						)
					)
	...
	def get_finished_item(self):
		finished_item = None
		if self.work_order:
			finished_item = finergy.db.get_value("Work Order", self.work_order, "production_item")
		elif self.bom_no:
			finished_item = finergy.db.get_value("BOM", self.bom_no, "item")
	...
	def validate_finished_goods(self):
		...
		wo_details = finergy.db.get_value("Work Order", self.work_order, ["production_item", "qty"])
		if wo_details:
	...
	def update_work_order(self):
		def _validate_work_order(pro_doc):
			if flt(pro_doc.docstatus) != 1:
				finergy.throw(_("Work Order {0} must be submitted").format(self.work_order))
		...
		if self.job_card:
			job_doc = finergy.get_doc("Job Card", self.job_card)
	...
	def get_item_details(self, args=None, for_update=False):
		...
		if self.purpose == "Material Issue":
			ret["expense_account"] = (
				...
				or finergy.get_cached_value("Company", self.company, "default_expense_account")
			)
		...
		for company_field, field in {
			...
			if not ret.get(field):
				ret[field] = finergy.get_cached_value("Company", self.company, company_field)
		...
		if (
			...
			subcontract_items = finergy.get_all(
	...
	def set_items_for_stock_in(self):
		...
		if self.outgoing_stock_entry and self.purpose == "Material Transfer":
			doc = finergy.get_doc("Stock Entry", self.outgoing_stock_entry)

			if doc.per_transferred == 100:
				finergy.throw(_("Goods are already received against the outward entry {0}").format(doc.name))
	...
	def get_items(self):
		...
		if not self.posting_date or not self.posting_time:
			finergy.throw(_("Posting date and posting time is mandatory"))

		self.set_work_order_details()
		self.flags.backflush_based_on = finergy.db.get_single_value(
			"Manufacturing Settings", "backflush_raw_materials_based_on"
		)

		if self.bom_no:

			backflush_based_on = finergy.db.get_single_value(
				"Manufacturing Settings", "backflush_raw_materials_based_on"
			)
			...
			if self.purpose in [
				...
				if self.work_order and self.purpose == "Material Transfer for Manufacture":
					...
					and finergy.db.get_single_value("Manufacturing Settings", "material_consumption") == 1
				):
					self.get_unconsumed_raw_materials()

				else:
					if not self.fg_completed_qty:
						finergy.throw(_("Manufacturing Quantity is mandatory"))
					...
					if self.get(self.subcontract_data.order_field) and self.purpose == "Send to Subcontractor":
						# Get Subcontract Order Supplied Items Details
						parent = finergy.qb.DocType(self.subcontract_data.order_doctype)
						child = finergy.qb.DocType(self.subcontract_data.order_supplied_items_field)
			...
			if self.work_order and self.purpose == "Manufacture":
				work_order = finergy.get_doc("Work Order", self.work_order)
	...
	def load_items_from_bom(self):
		if self.work_order:
			...
			item_code = finergy.db.get_value("BOM", self.bom_no, "item")
	...
	def set_batchwise_finished_goods(self, args, item):
		...
		data = finergy.get_all("Batch", filters=filters, fields=fields, order_by="creation asc")
	...
	def get_bom_raw_materials(self, qty):
		from capkpi.manufacturing.doctype.bom.bom import get_bom_items_as_dict
		...
		for item in item_dict.values():
			...
			if item["allow_alternative_item"]:
				item["allow_alternative_item"] = finergy.db.get_value(
	...
	def get_scrap_items_from_job_card(self):
		...
		job_card_scrap_item = finergy.qb.DocType("Job Card Scrap Item")

		scrap_items = (
			finergy.qb.from_(job_card)
			.select(
		...
		for row in scrap_items:
			...
			if cint(finergy.get_cached_value("UOM", row.stock_uom, "must_be_whole_number")):
				row.stock_qty = finergy.utils.ceil(row.stock_qty)
	...
	def get_unconsumed_raw_materials(self):
		wo = finergy.get_doc("Work Order", self.work_order)
		wo_items = finergy.get_all(
			"Work Order Item",
	...
	def add_transfered_raw_materials_in_items(self) -> None:
		...
		for key, row in available_materials.items():
			...
			if cint(finergy.get_cached_value("UOM", item.stock_uom, "must_be_whole_number")):
				qty = finergy.utils.ceil(qty)
	...
	def get_pending_raw_materials(self, backflush_based_on=None):
		...
		if not len(item_dict):
			finergy.msgprint(_("""All items have already been transferred for this Work Order."""))
	...
	def get_pro_order_required_items(self, backflush_based_on=None):
		...
		"""
		item_dict, job_card_items = finergy._dict(), []
		work_order = finergy.get_doc("Work Order", self.work_order)
		...
		if not finergy.db.get_value("Warehouse", work_order.wip_warehouse, "is_group"):
		...
		for d in work_order.get("required_items"):
			...
			if d.include_item_in_manufacturing:
				...
				if consider_job_card:
					job_card_item = finergy.db.get_value(
						"Job Card Item", {"item_code": d.item_code, "parent": self.get("job_card")}
					)
					item_row["job_card_item"] = job_card_item or None

				if d.source_warehouse and not finergy.db.get_value("Warehouse", d.source_warehouse, "is_group"):
	...
	def get_job_card_item_codes(self, job_card=None):
		...
		job_card_items = finergy.get_all(
	...
	def add_to_stock_entry_detail(self, item_dict, bom_no=None):
		for d in item_dict:
			...
			stock_uom = item_row.get("stock_uom") or finergy.db.get_value("Item", d, "stock_uom")
	...
	def validate_with_material_request(self):
		for item in self.get("items"):
			...
			if material_request:
				mreq_item = finergy.db.get_value(
	...
	def validate_batch(self):
		if self.purpose in [
			...
			for item in self.get("items"):
				if item.batch_no:
					disabled = finergy.db.get_value("Batch", item.batch_no, "disabled")
					if disabled == 0:
						expiry_date = finergy.db.get_value("Batch", item.batch_no, "expiry_date")
						if expiry_date:
							if getdate(self.posting_date) > getdate(expiry_date):
								finergy.throw(_("Batch {0} of Item {1} has expired.").format(item.batch_no, item.item_code))
					else:
						finergy.throw(_("Batch {0} of Item {1} is disabled.").format(item.batch_no, item.item_code))
	...
	def update_subcontract_order_supplied_items(self):
		if self.get(self.subcontract_data.order_field) and (
			...
			# Get Subcontract Order Supplied Items Details
			order_supplied_items = finergy.db.get_all(
				self.subcontract_data.order_supplied_items_field,
			...
			for row in order_supplied_items:
				...
				finergy.db.set_value(self.subcontract_data.order_supplied_items_field, row.name, item)
	...
	def update_so_in_serial_number(self):
		so_name, item_code = finergy.db.get_value(
		...
		if so_name and item_code:
			...
			if qty_to_reserve:
				...
				if qty_to_reserve > 0:
					for item in self.items:
						has_serial_no = finergy.get_cached_value("Item", item.item_code, "has_serial_no")
						if item.item_code == item_code and has_serial_no:
							serial_nos = (item.serial_no).split("\n")
							for serial_no in serial_nos:
								if qty_to_reserve > 0:
									finergy.db.set_value("Serial No", serial_no, "sales_order", so_name)
									qty_to_reserve -= 1
		...
		for item in self.items:
			if item.s_warehouse and not item.t_warehouse and item.serial_no:
				for sr in get_serial_nos(item.serial_no):
					sales_order = finergy.db.get_value("Serial No", sr, "sales_order")
					if sales_order:
						msg = _(
							"(Serial No: {0}) cannot be consumed as it's reserverd to fullfill Sales Order {1}."
						).format(sr, sales_order)

						finergy.throw(_("Item {0} {1}").format(item.item_code, msg))
	...
	def update_transferred_qty(self):
		if self.purpose == "Material Transfer" and self.outgoing_stock_entry:
			...
			for d in self.items:
				...
				transferred_qty = finergy.get_all(
					"Stock Entry Detail",
			...
			for data, transferred_qty in stock_entries.items():
				cond += """ WHEN (parent = %s and name = %s) THEN %s
					""" % (
					finergy.db.escape(data[0]),
					finergy.db.escape(data[1]),
					transferred_qty,
				)
	...
	def set_material_request_transfer_status(self, status):
		...
		if self.outgoing_stock_entry:
			parent_se = finergy.get_value("Stock Entry", self.outgoing_stock_entry, "add_to_transit")
		...
		for item in self.items:
			...
			if self.purpose == "Material Transfer" and material_request not in material_requests:
				if self.outgoing_stock_entry and parent_se:
					material_request = finergy.get_value("Stock Entry Detail", item.ste_detail, "material_request")

			if material_request and material_request not in material_requests:
				material_requests.append(material_request)
				finergy.db.set_value("Material Request", material_request, "transfer_status", status)
	...
	def update_items_for_process_loss(self):
		...
		for d in self.get("items"):
			...
			scrap_warehouse = finergy.db.get_single_value(
	...
	def get_serial_nos_for_fg(self, args):
		...
		stock_entries = finergy.get_all("Stock Entry", fields=fields, filters=filters)
	...
	def update_subcontracting_order_status(self):
		if self.subcontracting_order and self.purpose == "Send to Subcontractor":
			from capkpi.subcontracting.doctype.subcontracting_order.subcontracting_order import (
...
def move_sample_to_retention_warehouse(company, items):
	...
	retention_warehouse = finergy.db.get_single_value("Stock Settings", "sample_retention_warehouse")
	stock_entry = finergy.new_doc("Stock Entry")
...
def make_stock_in_entry(source_name, target_doc=None):
	...
	def update_item(source_doc, target_doc, source_parent):
		...
		if source_doc.material_request_item and source_doc.material_request:
			add_to_transit = finergy.db.get_value("Stock Entry", source_name, "add_to_transit")
			if add_to_transit:
				warehouse = finergy.get_value(
...
def get_work_order_details(work_order, company):
	work_order = finergy.get_doc("Work Order", work_order)
...
def get_operating_cost_per_unit(work_order=None, bom_no=None):
	...
	if not operating_cost_per_unit and bom_no:
		bom = finergy.db.get_value("BOM", bom_no, ["operating_cost", "quantity"], as_dict=1)
		if bom.quantity:
...
def get_valuation_rate_for_finished_good_entry(work_order):
	work_order_qty = flt(
		finergy.get_cached_value("Work Order", work_order, "material_transferred_for_manufacturing")
	)

	field = "(SUM(total_outgoing_value) / %s) as valuation_rate" % (work_order_qty)

	stock_data = finergy.get_all(
		"Stock Entry",
		fields=field,
		filters={
			"docstatus": 1,
...
def validate_sample_quantity(item_code, sample_quantity, qty, batch_no=None):
	...
	max_retain_qty = finergy.get_value("Item", item_code, "sample_quantity")
...
def get_supplied_items(
	...
	for row in finergy.get_all("Stock Entry", fields=fields, filters=filters):
		...
		if key not in supplied_item_details:
			supplied_item_details.setdefault(
				key, finergy._dict({"supplied_qty": 0, "returned_qty": 0, "total_supplied_qty": 0})
			)
...
def get_items_from_subcontracting_order(source_name, target_doc=None):
	sco = finergy.get_doc("Subcontracting Order", source_name)
...
def get_stock_entry_data(work_order):
	stock_entry = finergy.qb.DocType("Stock Entry")
	stock_entry_detail = finergy.qb.DocType("Stock Entry Detail")

	return (
		finergy.qb.from_(stock_entry)
