...
class StockEntry(StockController):
	...
	def set_actual_qty(self):
		allow_negative_stock = cint(finergy.db.get_single_value("Stock Settings", "allow_negative_stock"))
	...
	def set_serial_nos(self, work_order):
		previous_se = finergy.db.get_value(
			"Stock Entry",
		...
		for d in self.get("items"):
			transferred_serial_no = finergy.db.get_value(
	...
	def validate_purchase_order(self):
		...
		if backflush_raw_materials_based_on == "BOM":
			purchase_order = finergy.get_doc("Purchase Order", self.purchase_order)
			...
			for row in self.items:
				if not row.subcontracted_item:
					...
					po_detail = finergy.db.get_value("Purchase Order Item Supplied", filters, "name")
					if po_detail:
	...
	def get_transfered_raw_materials(self):
		...
		materials_already_backflushed = finergy.db.sql(
			"""
			select
		...
		for item in transferred_materials:
			...
			if trans_qty and manufacturing_qty > (produced_qty + flt(self.fg_completed_qty)):
				...
				precision = finergy.get_precision("Stock Entry Detail", "qty")
	...
	def update_purchase_order_supplied_items(self):
		if self.purchase_order and (
			...
			# Get PO Supplied Items Details
			po_supplied_items = finergy.db.get_all(
				"Purchase Order Item Supplied",
			...
			for row in po_supplied_items:
				...
				finergy.db.set_value("Purchase Order Item Supplied", row.name, item)
