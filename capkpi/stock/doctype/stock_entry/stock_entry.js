...
finergy.ui.form.on('Stock Entry Detail', {
	...
	barcode: function(doc, cdt, cdn) {
		...
		if (d.barcode) {
			finergy.call({
				method: "capkpi.stock.get_item_details.get_item_code",
				args: {"barcode": d.barcode },
				callback: function(r) {
					if (!r.exe){
						finergy.model.set_value(cdt, cdn, "item_code", r.message);
					}
				}
			});
		}
...
capkpi.stock.StockEntry = capkpi.stock.StockController.extend({
	setup: function() {
		var me = this;
	...
	scan_barcode: function() {
		let transaction_controller= new capkpi.TransactionController({frm:this.frm});
...
$.extend(cur_frm.cscript, new capkpi.stock.StockEntry({frm: cur_frm}));
