...
if TYPE_CHECKING:
	from capkpi.stock.doctype.stock_entry.stock_entry import StockEntry
...
def make_stock_entry(**args):
	...
	if not args.company:
		if args.source:
			args.company = finergy.db.get_value("Warehouse", args.source, "company")
		elif args.target:
			args.company = finergy.db.get_value("Warehouse", args.target, "company")
	...
	s.company = args.company or capkpi.get_default_company()
	s.purchase_receipt_no = args.purchase_receipt_no
	s.delivery_note_no = args.delivery_note_no
	s.sales_invoice_no = args.sales_invoice_no
	s.is_opening = args.is_opening or "No"
	if not args.cost_center:
		args.cost_center = finergy.get_value("Company", s.company, "cost_center")

	if not args.expense_account and s.is_opening == "No":
		args.expense_account = finergy.get_value("Company", s.company, "stock_adjustment_account")
	...
	if not args.serial_no and args.qty and args.batch_no:
		serial_number_list = finergy.get_list(
