...
import finergy
from finergy.permissions import add_user_permission, remove_user_permission
from finergy.tests.utils import FinergyTestCase, change_settings
from finergy.utils import add_days, flt, nowdate, nowtime, today

from capkpi.accounts.doctype.account.test_account import get_inventory_account
from capkpi.stock.doctype.item.test_item import (
	create_item,
	make_item,
	make_item_variant,
	set_item_variant_settings,
)
from capkpi.stock.doctype.serial_no.serial_no import *  # noqa
...
from capkpi.stock.doctype.stock_ledger_entry.stock_ledger_entry import StockFreezeError
from capkpi.stock.doctype.stock_reconciliation.stock_reconciliation import (
	OpeningEntryAccountError,
)
from capkpi.stock.doctype.stock_reconciliation.test_stock_reconciliation import (
	create_stock_reconciliation,
)
from capkpi.stock.stock_ledger import NegativeStockError, get_previous_sle
...
class TestStockEntry(FinergyTestCase):
	def tearDown(self):
		finergy.db.rollback()
	...
	def test_fifo(self):
		...
		self.assertEqual([[1, 10]], finergy.safe_eval(sle.stock_queue))
		...
		self.assertEqual([[-2, 10]], finergy.safe_eval(sle.stock_queue))

		# move stock to positive
		make_stock_entry(item_code=item_code, target=warehouse, qty=3, basic_rate=20)
		sle = get_sle(item_code=item_code, warehouse=warehouse)[0]
		self.assertEqual([[1, 20]], finergy.safe_eval(sle.stock_queue))

		# incoming entry with diff rate
		make_stock_entry(item_code=item_code, target=warehouse, qty=1, basic_rate=30)
		sle = get_sle(item_code=item_code, warehouse=warehouse)[0]

		self.assertEqual([[1, 20], [1, 30]], finergy.safe_eval(sle.stock_queue))

		finergy.db.set_default("allow_negative_stock", 0)
	...
	def test_auto_material_request_for_variant(self):
		...
		make_item_variant()
		template = finergy.get_doc("Item", "_Test Variant Item")
	...
	def _test_auto_material_request(
		...
		variant = finergy.get_doc("Item", item_code)

		projected_qty, actual_qty = finergy.db.get_value(
		...
		projected_qty = (
			finergy.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "projected_qty")
			or 0
		)

		finergy.db.set_value("Stock Settings", None, "auto_indent", 1)
		...
		from capkpi.stock.reorder_item import reorder_item

		mr_list = reorder_item()

		finergy.db.set_value("Stock Settings", None, "auto_indent", 0)
	...
	def test_material_receipt_gl_entry(self):
		company = finergy.db.get_value("Warehouse", "Stores - TCP1", "company")
	...
	def test_repack_multiple_fg(self):
		...
		repack = finergy.copy_doc(test_records[3])
	...
	def test_repack_with_additional_costs(self):
		...
		expenses_included_in_valuation = finergy.get_value(
	...
	def test_serial_no_not_reqd(self):
		se = finergy.copy_doc(test_records[0])
	...
	def test_serial_no_transfer_in(self):
		...
		self.assertTrue(finergy.db.exists("Serial No", "ABCD"))
		self.assertTrue(finergy.db.exists("Serial No", "EFGH"))

		se.cancel()
		self.assertFalse(finergy.db.get_value("Serial No", "ABCD", "warehouse"))

	def test_serial_no_not_exists(self):
		finergy.db.sql("delete from `tabSerial No` where name in ('ABCD', 'EFGH')")
	...
	def test_serial_by_series(self):
		...
		self.assertTrue(finergy.db.exists("Serial No", serial_nos[0]))
		self.assertTrue(finergy.db.exists("Serial No", serial_nos[1]))

		return se, serial_nos

	def test_serial_item_error(self):
		se, serial_nos = self.test_serial_by_series()
		if not finergy.db.exists("Serial No", "ABCD"):
	...
	def test_serial_move(self):
		...
		se.submit()
		self.assertTrue(
			finergy.db.get_value("Serial No", serial_no, "warehouse"), "_Test Warehouse 1 - _TC"
		)

		se.cancel()
		self.assertTrue(
			finergy.db.get_value("Serial No", serial_no, "warehouse"), "_Test Warehouse - _TC"
		)
	...
	def test_serial_cancel(self):
		...
		serial_no = get_serial_nos(se.get("items")[0].serial_no)[0]
		self.assertFalse(finergy.db.get_value("Serial No", serial_no, "warehouse"))
	...
	def test_serial_batch_item_stock_entry(self):
		...
		"""
		from capkpi.stock.doctype.batch.batch import get_batch_qty

		item = finergy.db.exists("Item", {"item_name": "Batched and Serialised Item"})
		if not item:
			...
			item = finergy.get_doc("Item", {"item_name": "Batched and Serialised Item"})
		...
		batch_in_serial_no = finergy.db.get_value("Serial No", serial_no, "batch_no")
		...
		self.assertEqual(finergy.db.get_value("Serial No", serial_no, "status"), "Inactive")
		self.assertEqual(finergy.db.exists("Batch", batch_no), None)
	...
	def test_serial_batch_item_qty_deduction(self):
		...
		self.assertEqual(finergy.db.get_value("Batch", batch_no, "reference_name"), se1.name)
		...
		self.assertEqual(finergy.db.get_value("Serial No", serial_no1, "batch_no"), batch_no)
		self.assertEqual(finergy.db.get_value("Serial No", serial_no1, "status"), "Active")

		# Check if Serial No from Stock Entry 2 is Unlinked and Inactive
		self.assertEqual(finergy.db.get_value("Serial No", serial_no2, "batch_no"), None)
		self.assertEqual(finergy.db.get_value("Serial No", serial_no2, "status"), "Inactive")

	def test_warehouse_company_validation(self):
		company = finergy.db.get_value("Warehouse", "_Test Warehouse 2 - _TC1", "company")
		finergy.get_doc("User", "test2@example.com").add_roles(
			"Sales User", "Sales Manager", "Stock User", "Stock Manager"
		)
		finergy.set_user("test2@example.com")

		from capkpi.stock.utils import InvalidWarehouseCompany

		st1 = finergy.copy_doc(test_records[0])
	...
	def test_warehouse_user(self):
		...
		test_user = finergy.get_doc("User", "test@example.com")
		...
		st1.get("items")[0].t_warehouse = "_Test Warehouse 2 - _TC1"
		self.assertRaises(finergy.PermissionError, st1.insert)
	...
	def test_freeze_stocks(self):
		finergy.db.set_value("Stock Settings", None, "stock_auth_role", "")

		# test freeze_stocks_upto
		finergy.db.set_value("Stock Settings", None, "stock_frozen_upto", add_days(nowdate(), 5))
		se = finergy.copy_doc(test_records[0]).insert()
		self.assertRaises(StockFreezeError, se.submit)

		finergy.db.set_value("Stock Settings", None, "stock_frozen_upto", "")

		# test freeze_stocks_upto_days
		finergy.db.set_value("Stock Settings", None, "stock_frozen_upto_days", -1)
		se = finergy.copy_doc(test_records[0])
		...
		se.insert()
		self.assertRaises(StockFreezeError, se.submit)
		finergy.db.set_value("Stock Settings", None, "stock_frozen_upto_days", 0)
	...
	def test_work_order(self):
		...
		bom_no, bom_operation_cost = finergy.db.get_value(
			"BOM", {"item": "_Test FG Item 2", "is_default": 1, "docstatus": 1}, ["name", "operating_cost"]
		)

		work_order = finergy.new_doc("Work Order")
		work_order.update(
			{
	...
	def test_work_order_manufacture_with_material_consumption(self):
		...
		bom_no = finergy.db.get_value("BOM", {"item": "_Test FG Item", "is_default": 1, "docstatus": 1})
		...
		s = finergy.get_doc(_make_stock_entry(work_order.name, "Material Transfer for Manufacture", 1))
		...
		s = finergy.get_doc(_make_stock_entry(work_order.name, "Manufacture", 1))
		s.save()
		rm_cost = 0
		for d in s.get("items"):
			if d.s_warehouse:
	...
	def test_variant_work_order(self):
		...
		stock_entry = finergy.get_doc(make_stock_entry(work_order.name, "Manufacture", 1))
		stock_entry.insert()
	...
	def test_retain_sample(self):
		...
		if not finergy.db.exists("Item", test_item_code):
		...
		receipt_entry = finergy.new_doc("Stock Entry")
		...
		retention_entry = finergy.new_doc("Stock Entry")
	...
	def test_quality_check(self):
		...
		repack.insert()
		self.assertRaises(finergy.ValidationError, repack.submit)
	...
	def test_zero_incoming_rate(self):
		...
		value_diff = finergy.db.get_value(
			"Stock Ledger Entry",
	...
	def test_gle_for_opening_stock_entry(self):
		...
		is_opening = finergy.db.get_value(
			"GL Entry",
	...
	def test_conversion_factor_change(self):
		finergy.db.set_value("Stock Settings", None, "allow_negative_stock", 1)
		repack_entry = finergy.copy_doc(test_records[3])
	...
	def test_additional_cost_distribution_manufacture(self):
		se = finergy.get_doc(
			doctype="Stock Entry",
			purpose="Manufacture",
			additional_costs=[finergy._dict(base_amount=100)],
			items=[
				finergy._dict(item_code="RM", basic_amount=10),
				finergy._dict(item_code="FG", basic_amount=20, t_warehouse="X", is_finished_item=1),
				finergy._dict(item_code="scrap", basic_amount=30, t_warehouse="X"),
			],
		)
	...
	def test_additional_cost_distribution_non_manufacture(self):
		se = finergy.get_doc(
			...
			additional_costs=[finergy._dict(base_amount=100)],
			items=[
				finergy._dict(item_code="RECEIVED_1", basic_amount=20, t_warehouse="X"),
				finergy._dict(item_code="RECEIVED_2", basic_amount=30, t_warehouse="X"),
			],
		)
	...
	def test_independent_manufacture_entry(self):
		...
		se = finergy.get_doc(
			...
			items=[
				...
				finergy._dict(item_code="_Test FG Item", qty=4, t_warehouse="_Test Warehouse 1 - _TC"),
			],
		)
	...
	def test_future_negative_sle_batch(self):
		from capkpi.stock.doctype.batch.test_batch import TestBatch
	...
	def test_multi_batch_value_diff(self):
		...
		receipt.append(
			"items", finergy.copy_doc(receipt.items[0], ignore_no_copy=False).update({"basic_rate": 20})
		)
		receipt.save()
		receipt.submit()
		...
		issue.append("items", finergy.copy_doc(issue.items[0], ignore_no_copy=False))
	...
	def test_transfer_qty_validation(self):
		...
		self.assertRaises(finergy.ValidationError, se.save)

	def test_mapped_stock_entry(self):
		"Check if rate and stock details are populated in mapped SE given warehouse."
		from capkpi.stock.doctype.purchase_receipt.purchase_receipt import make_stock_entry
	...
	def test_reposting_for_depedent_warehouse(self):
		from capkpi.stock.doctype.repost_item_valuation.repost_item_valuation import repost_sl_entries
		from capkpi.stock.doctype.warehouse.test_warehouse import create_warehouse
		...
		repost_name = finergy.db.get_value(
			"Repost Item Valuation", {"voucher_no": receipt2.name, "docstatus": 1}, "name"
		)

		doc = finergy.get_doc("Repost Item Valuation", repost_name)
	...
	def test_batch_expiry(self):
		from capkpi.controllers.stock_controller import BatchExpiredError
		from capkpi.stock.doctype.batch.test_batch import make_new_batch
		...
		batch = make_new_batch(
			batch_id=finergy.generate_hash("", 5), item_code=item_doc.name, expiry_date=add_days(today(), -1)
		)
...
test_records = finergy.get_test_records("Stock Entry")


def initialize_records_for_future_negative_sle_test(
	item_code, batch_no, warehouses, opening_qty, posting_date
):
	from capkpi.stock.doctype.batch.test_batch import TestBatch, make_new_batch
	from capkpi.stock.doctype.stock_reconciliation.test_stock_reconciliation import (
		create_stock_reconciliation,
	)
	from capkpi.stock.doctype.warehouse.test_warehouse import create_warehouse
