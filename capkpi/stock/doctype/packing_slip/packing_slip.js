...
cur_frm.fields_dict['items'].grid.get_field('item_code').get_query = function(doc, cdt, cdn) {
	if(!doc.delivery_note) {
		finergy.throw(__("Please select a Delivery Note"));
	} else {
		return {
			query: "capkpi.stock.doctype.packing_slip.packing_slip.item_details",
...
cur_frm.cscript.validate_case_nos = function(doc) {
	...
	if(cint(doc.from_case_no)==0) {
		finergy.msgprint(__("The 'From Package No.' field must neither be empty nor it's value less than 1."));
		...
		finergy.msgprint(__("'To Case No.' cannot be less than 'From Case No.'"));
		finergy.validated = false;
	}
}
	...
	for(var i=0; i<ps_detail.length; i++) {
		for(var j=0; j<ps_detail.length; j++) {
			if(i!=j && ps_detail[i].item_code && ps_detail[i].item_code==ps_detail[j].item_code) {
				finergy.msgprint(__("You have entered duplicate items. Please rectify and try again."));
				finergy.validated = false;
				return;
			}
		}
		if(flt(ps_detail[i].qty)<=0) {
			finergy.msgprint(__("Invalid quantity specified for item {0}. Quantity should be greater than 0.", [ps_detail[i].item_code]));
			finergy.validated = false;
		}
	}
}
...
cur_frm.cscript.calc_net_total_pkg = function(doc, ps_detail) {
	...
	for(var i=0; i<ps_detail.length; i++) {
		...
		if(item.weight_uom != doc.net_weight_uom) {
			finergy.msgprint(__("Different UOM for items will lead to incorrect (Total) Net Weight value. Make sure that Net Weight of each item is in the same UOM."));
			finergy.validated = false;
		}
