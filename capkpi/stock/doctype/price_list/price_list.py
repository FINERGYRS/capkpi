...
class PriceList(Document):
	...
	def set_default_if_missing(self):
		if cint(self.selling):
			if not finergy.db.get_value("Selling Settings", None, "selling_price_list"):
				finergy.set_value("Selling Settings", "Selling Settings", "selling_price_list", self.name)

		elif cint(self.buying):
			if not finergy.db.get_value("Buying Settings", None, "buying_price_list"):
				finergy.set_value("Buying Settings", "Buying Settings", "buying_price_list", self.name)
	...
	def check_impact_on_shopping_cart(self):
		...
		affects_cart = self.name == finergy.get_cached_value("E Commerce Settings", None, "price_list")
	...
	def on_trash(self):
		...
		def _update_default_price_list(module):
			b = finergy.get_doc(module + " Settings")
	...
	def delete_price_list_details_key(self):
		finergy.cache().hdel("price_list_details", self.name)


def get_price_list_details(price_list):
	price_list_details = finergy.cache().hget("price_list_details", price_list)

	if not price_list_details:
		price_list_details = finergy.get_cached_value(
		...
		finergy.cache().hset("price_list_details", price_list, price_list_details)
