...
from finergy.utils import cstr, flt, nowdate, nowtime
...
from capkpi.stock.doctype.delivery_note.delivery_note import (
	make_delivery_trip,
	make_sales_invoice,
)
from capkpi.stock.doctype.item.test_item import make_item
from capkpi.stock.doctype.purchase_receipt.test_purchase_receipt import get_gl_entries
from capkpi.stock.doctype.serial_no.serial_no import SerialNoWarehouseError, get_serial_nos
...
from capkpi.stock.stock_ledger import get_previous_sle


class TestDeliveryNote(FinergyTestCase):
	def test_over_billing_against_dn(self):
		finergy.db.set_value("Stock Settings", None, "allow_negative_stock", 1)

		dn = create_delivery_note(do_not_submit=True)
		self.assertRaises(finergy.ValidationError, make_sales_invoice, dn.name)
		...
		self.assertRaises(finergy.ValidationError, finergy.get_doc(si).insert)

	def test_delivery_note_no_gl_entry(self):
		company = finergy.db.get_value("Warehouse", "_Test Warehouse - _TC", "company")
	...
	def test_sales_return_for_non_bundled_items_partial(self):
		...
		returned = finergy.get_doc("Delivery Note", dn1.name)
	...
	def test_delivery_of_bundled_items_to_target_warehouse(self):
		...
		stock_value_difference1 = finergy.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Delivery Note",
				"voucher_no": dn.name,
	...
	def test_closed_delivery_note(self):
		from capkpi.stock.doctype.delivery_note.delivery_note import update_delivery_note_status
		...
		self.assertEqual(finergy.db.get_value("Delivery Note", dn.name, "Status"), "Closed")
	...
	def test_dn_billing_status_case3(self):
		...
		from capkpi.selling.doctype.sales_order.sales_order import make_delivery_note
		from capkpi.selling.doctype.sales_order.sales_order import (
	...
	def test_dn_billing_status_case4(self):
		# SO -> SI -> DN
		from capkpi.accounts.doctype.sales_invoice.sales_invoice import make_delivery_note
	...
	def test_internal_transfer_precision_gle(self):
		...
		self.assertFalse(
			finergy.db.exists("GL Entry", {"voucher_no": dn.name, "voucher_type": dn.doctype})
		)


def create_delivery_note(**args):
	dn = finergy.new_doc("Delivery Note")
