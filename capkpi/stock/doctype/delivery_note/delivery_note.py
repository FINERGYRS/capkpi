...
from capkpi.stock.doctype.serial_no.serial_no import get_delivery_note_serial_no
...
class DeliveryNote(SellingController):
	...
	def before_print(self, settings=None):
		...
		item_meta = finergy.get_meta("Delivery Note Item")
		print_hide_fields = {
	...
	def so_required(self):
		...
		if finergy.db.get_value("Selling Settings", None, "so_required") == "Yes":
			for d in self.get("items"):
				if not d.against_sales_order:
					finergy.throw(_("Sales Order required for Item {0}").format(d.item_code))
	...
	def check_next_docstatus(self):
		...
		if submit_rv:
			finergy.throw(_("Sales Invoice {0} has already been submitted").format(submit_rv[0][0]))

		submit_in = finergy.db.sql(
			"""select t1.name
		...
		if submit_in:
			finergy.throw(_("Installation Note {0} has already been submitted").format(submit_in[0][0]))
	...
	def cancel_packing_slips(self):
		...
		if res:
			for r in res:
				ps = finergy.get_doc("Packing Slip", r[0])
				ps.cancel()
			finergy.msgprint(_("Packing Slip(s) cancelled"))
	...
	def update_billing_status(self, update_modified=True):
		...
		for dn in set(updated_delivery_notes):
			dn_doc = self if (dn == self.name) else finergy.get_doc("Delivery Note", dn)
	...
	def make_return_invoice(self):
		try:
			...
			credit_note_link = finergy.utils.get_link_to_form("Sales Invoice", return_invoice.name)

			finergy.msgprint(_("Credit Note {0} has been created automatically").format(credit_note_link))
		except Exception:
			finergy.throw(
				_(
...
def update_billed_amount_based_on_so(so_detail, update_modified=True):
	...
	si_item = finergy.qb.DocType("Sales Invoice Item").as_("si_item")
	sum_amount = Sum(si_item.amount).as_("amount")

	billed_against_so = (
		finergy.qb.from_(si_item)
		.select(sum_amount)
		.where(
	...
	dn = finergy.qb.DocType("Delivery Note").as_("dn")
	dn_item = finergy.qb.DocType("Delivery Note Item").as_("dn_item")
	...
	for dnd in dn_details:
		...
		if dnd.si_detail:
			...
			billed_amt_agianst_dn = finergy.db.sql(
...
def get_invoiced_qty_map(delivery_note):
	...
	for dn_detail, qty in finergy.db.sql(
...
def make_sales_invoice(source_name, target_doc=None):
	doc = finergy.get_doc("Delivery Note", source_name)
	...
	def set_missing_values(source, target):
		...
		if len(target.get("items")) == 0:
			finergy.throw(_("All these items have already been Invoiced/Returned"))
...
def make_shipment(source_name, target_doc=None):
	def postprocess(source, target):
		user = finergy.db.get_value(
			"User", finergy.session.user, ["email", "full_name", "phone", "mobile_no"], as_dict=1
		)
		...
		target.pickup_contact_person = finergy.session.user

		contact = finergy.db.get_value(
...
def update_delivery_note_status(docname, status):
	dn = finergy.get_doc("Delivery Note", docname)
...
def on_doctype_update():
	finergy.db.add_index("Delivery Note", ["customer", "is_return", "return_against"])
