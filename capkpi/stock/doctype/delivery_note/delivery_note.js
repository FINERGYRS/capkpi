...
finergy.provide("capkpi.stock.delivery_note");
...
finergy.ui.form.on("Delivery Note", {
	setup: function(frm) {
		...
		capkpi.queries.setup_warehouse_query(frm);
	...
	print_without_amount: function(frm) {
		capkpi.stock.delivery_note.set_print_hide(frm.doc);
	},
		...
		if (frm.doc.docstatus == 1 && !frm.doc.inter_company_reference) {
			...
			if (internal) {
				...
				me.frm.add_custom_button(button_label, function() {
					finergy.model.open_mapped_doc({
						method: 'capkpi.stock.doctype.delivery_note.delivery_note.make_inter_company_purchase_receipt',
						frm: frm,
					});
				}, __('Create'));
			}
...
finergy.ui.form.on("Delivery Note Item", {
...
capkpi.stock.DeliveryNoteController = class DeliveryNoteController extends capkpi.selling.SellingController {
	setup(doc) {
	...
	refresh(doc, dt, dn) {
		...
		if (!doc.is_return && doc.status!="Closed") {
			...
			if(doc.docstatus==0 && !doc.__islocal) {
				this.frm.add_custom_button(__('Packing Slip'), function() {
					finergy.model.open_mapped_doc({
						method: "capkpi.stock.doctype.delivery_note.delivery_note.make_packing_slip",
						frm: me.frm
					}) }, __('Create'));
			}
		...
		capkpi.stock.delivery_note.set_print_hide(doc, dt, dn);
	...
	make_shipment() {
		finergy.model.open_mapped_doc({
			method: "capkpi.stock.doctype.delivery_note.delivery_note.make_shipment",
			frm: this.frm
		})
	}
	...
	make_sales_return() {
		finergy.model.open_mapped_doc({
			method: "capkpi.stock.doctype.delivery_note.delivery_note.make_sales_return",
			frm: this.frm
		})
	}
	...
	make_delivery_trip() {
		finergy.model.open_mapped_doc({
			method: "capkpi.stock.doctype.delivery_note.delivery_note.make_delivery_trip",
			frm: cur_frm
		})
	}
	...
	update_status(status) {
		...
		finergy.call({
			method:"capkpi.stock.doctype.delivery_note.delivery_note.update_delivery_note_status",
...
extend_cscript(cur_frm.cscript, new capkpi.stock.DeliveryNoteController({frm: cur_frm}));
...
finergy.ui.form.on('Delivery Note', {
	...
	unhide_account_head: function(frm) {
		...
		var aii_enabled = capkpi.is_perpetual_inventory_enabled(frm.doc.company)
		frm.fields_dict["items"].grid.set_column_disp(["expense_account", "cost_center"], aii_enabled);
	}
})


capkpi.stock.delivery_note.set_print_hide = function(doc, cdt, cdn){
	var dn_fields = finergy.meta.docfield_map['Delivery Note'];
	var dn_item_fields = finergy.meta.docfield_map['Delivery Note Item'];
