...
from finergy.utils import add_days, add_to_date, flt, now
...
from capkpi.accounts.utils import update_gl_entries_after
from capkpi.assets.doctype.asset.test_asset import create_asset_category, create_fixed_asset_item
...
class TestLandedCostVoucher(FinergyTestCase):
	...
	def test_landed_cost_voucher_stock_impact(self):
		...
		pr1 = make_purchase_receipt(
			...
			qty=500,
			rate=80,
			posting_date=add_days(finergy.utils.nowdate(), -2),
		)
		...
		last_sle = finergy.db.get_value(  # SLE of second PR
			"Stock Ledger Entry",
			{
		...
		last_sle_after_landed_cost = finergy.db.get_value(  # SLE of second PR after LCV's effect
			"Stock Ledger Entry",
			{
				"voucher_type": pr2.doctype,
				"voucher_no": pr2.name,
	...
	def test_landed_cost_voucher_against_purchase_invoice(self):
		...
		pi_lc_value = finergy.db.get_value(
	...
	def test_landed_cost_voucher_for_serialized_item(self):
		...
		serial_no_rate = finergy.db.get_value("Serial No", "SN001", "purchase_rate")

		create_landed_cost_voucher("Purchase Receipt", pr.name, pr.company)

		serial_no = finergy.db.get_value("Serial No", "SN001", ["warehouse", "purchase_rate"], as_dict=1)
	...
	def test_serialized_lcv_delivered(self):
		...
		serial_no_rate = finergy.db.get_value("Serial No", serial_no, "purchase_rate")
		...
		serial_no = finergy.db.get_value(
	...
	def test_asset_lcv(self):
		...
		self.assertEqual(finergy.db.get_value("Asset", assets[0].name, "gross_purchase_amount"), 50080)
...
def make_landed_cost_voucher(**args):
	...
	ref_doc = finergy.get_doc(args.receipt_document_type, args.receipt_document)

	lcv = finergy.new_doc("Landed Cost Voucher")
...
def create_landed_cost_voucher(receipt_document_type, receipt_document, company, charges=50):
	ref_doc = finergy.get_doc(receipt_document_type, receipt_document)
...
test_records = finergy.get_test_records("Landed Cost Voucher")
