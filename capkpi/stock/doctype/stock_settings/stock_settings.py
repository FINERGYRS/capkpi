...
from capkpi.stock.utils import check_pending_reposting
...
class StockSettings(Document):
	def validate(self):
		...
		from capkpi.utilities.naming import set_by_naming_series
	...
	def validate_warehouses(self):
		...
		for field in warehouse_fields:
			if finergy.db.get_value("Warehouse", self.get(field), "is_group"):
				finergy.throw(
	...
	def cant_change_valuation_method(self):
		db_valuation_method = finergy.db.get_single_value("Stock Settings", "valuation_method")
	...
	def validate_clean_description_html(self):
		if int(self.clean_description_html or 0) and not int(self.db_get("clean_description_html") or 0):
			# changed to text
			finergy.enqueue(
				"capkpi.stock.doctype.stock_settings.stock_settings.clean_all_descriptions",
...
def clean_all_descriptions():
	for item in finergy.get_all("Item", ["name", "description"]):
		if item.description:
			clean_description = clean_html(item.description)
		if item.description != clean_description:
			finergy.db.set_value("Item", item.name, "description", clean_description)
