...
from finergy.utils import add_days, cint, cstr, flt, today
from six import iteritems

import capkpi
from capkpi.accounts.doctype.account.test_account import get_inventory_account
from capkpi.controllers.buying_controller import QtyMismatchError
...
from capkpi.stock.doctype.serial_no.serial_no import SerialNoDuplicateError, get_serial_nos
from capkpi.stock.doctype.warehouse.test_warehouse import create_warehouse
from capkpi.stock.stock_ledger import SerialNoExistsInFutureTransaction


class TestPurchaseReceipt(FinergyTestCase):
	def setUp(self):
		finergy.db.set_value("Buying Settings", None, "allow_multiple_items", 1)
	...
	def test_reverse_purchase_receipt_sle(self):
		...
		sl_entry_cancelled = finergy.db.get_all(
			"Stock Ledger Entry",
			{"voucher_type": "Purchase Receipt", "voucher_no": pr.name},
			["actual_qty"],
			order_by="creation",
	...
	def test_make_purchase_invoice(self):
		...
		template = finergy.db.get_value(
			"Payment Terms Template", "_Test Payment Terms Template For Purchase Invoice"
		)
		old_template_in_supplier = finergy.db.get_value("Supplier", "_Test Supplier", "payment_terms")
		finergy.db.set_value("Supplier", "_Test Supplier", "payment_terms", template)

		pr = make_purchase_receipt(do_not_save=True)
		self.assertRaises(finergy.ValidationError, make_purchase_invoice, pr.name)
		pr.submit()
		...
		self.assertRaises(finergy.ValidationError, finergy.get_doc(pi).submit)
		...
		pi.delete()  # draft PI
		pr.cancel()
		finergy.db.set_value("Supplier", "_Test Supplier", "payment_terms", old_template_in_supplier)
		finergy.get_doc(
			"Payment Terms Template", "_Test Payment Terms Template For Purchase Invoice"
		).delete()
	...
	def test_purchase_receipt_no_gl_entry(self):
		...
		existing_bin_qty, existing_bin_stock_value = finergy.db.get_value(
			"Bin",
		...
		stock_value_difference = finergy.db.get_value(
			"Stock Ledger Entry",
			{
		...
		current_bin_stock_value = finergy.db.get_value(
	...
	def test_batched_serial_no_purchase(self):
		item = finergy.db.exists("Item", {"item_name": "Batched Serialized Item"})
		if not item:
			...
			item = finergy.get_doc("Item", {"item_name": "Batched Serialized Item"})

		pr = make_purchase_receipt(item_code=item.name, qty=5, rate=500)

		self.assertTrue(finergy.db.get_value("Batch", {"item": item.name, "reference_name": pr.name}))

		pr.load_from_db()
		batch_no = pr.items[0].batch_no
		pr.cancel()

		self.assertFalse(finergy.db.get_value("Batch", {"item": item.name, "reference_name": pr.name}))
		self.assertFalse(finergy.db.get_all("Serial No", {"batch_no": batch_no}))

	def test_duplicate_serial_nos(self):
		from capkpi.stock.doctype.delivery_note.test_delivery_note import create_delivery_note

		item = finergy.db.exists("Item", {"item_name": "Test Serialized Item 123"})
		if not item:
			...
			item = finergy.get_doc("Item", {"item_name": "Test Serialized Item 123"})

		# First make purchase receipt
		pr = make_purchase_receipt(item_code=item.name, qty=2, rate=500)
		pr.load_from_db()

		serial_nos = finergy.db.get_value(
			"Stock Ledger Entry",
	...
	def test_purchase_receipt_gl_entry(self):
		...
		self.assertEqual(cint(capkpi.is_perpetual_inventory_enabled(pr.company)), 1)
	...
	def test_subcontracting_over_receipt(self):
		...
		from capkpi.buying.doctype.purchase_order.purchase_order import (
			make_rm_stock_entry as make_subcontract_transfer_entry,
		)
		from capkpi.buying.doctype.purchase_order.test_purchase_order import (
		...
		se = finergy.get_doc(make_subcontract_transfer_entry(po.name, rm_item_string))
		...
		pr1.submit()
		self.assertRaises(finergy.ValidationError, pr2.submit)
		...
		if pr2.docstatus == 1 and finergy.db.get_value(
	...
	def test_serial_no_supplier(self):
		...
		self.assertEqual(finergy.db.get_value("Serial No", pr_row_1_serial_no, "supplier"), pr.supplier)

		pr.cancel()
		self.assertFalse(finergy.db.get_value("Serial No", pr_row_1_serial_no, "warehouse"))

	def test_rejected_serial_no(self):
		pr = finergy.copy_doc(test_records[0])
		...
		for serial_no in accepted_serial_nos:
			self.assertEqual(
				finergy.db.get_value("Serial No", serial_no, "warehouse"), pr.get("items")[0].warehouse
			)
		...
		for serial_no in rejected_serial_nos:
			self.assertEqual(
				finergy.db.get_value("Serial No", serial_no, "warehouse"), pr.get("items")[0].rejected_warehouse
			)
	...
	def test_purchase_return_partial(self):
		...
		# check sle
		outgoing_rate = finergy.db.get_value(
			"Stock Ledger Entry",
		...
		returned = finergy.get_doc("Purchase Receipt", return_pr.name)
	...
	def test_purchase_return_for_rejected_qty(self):
		from capkpi.stock.doctype.warehouse.test_warehouse import get_warehouse

		rejected_warehouse = "_Test Rejected Warehouse - TCP1"
		if not finergy.db.exists("Warehouse", rejected_warehouse):
			get_warehouse(
				company="_Test Company with perpetual inventory",
				abbr=" - TCP1",
		...
		actual_qty = finergy.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Purchase Receipt",
	...
	def test_purchase_return_for_serialized_items(self):
		def _check_serial_no_values(serial_no, field_values):
			serial_no = finergy.get_doc("Serial No", serial_no)
	...
	def test_closed_purchase_receipt(self):
		...
		self.assertEqual(finergy.db.get_value("Purchase Receipt", pr.name, "status"), "Closed")
	...
	def test_auto_asset_creation(self):
		...
		if not finergy.db.exists("Item", asset_item):
			asset_category = finergy.get_all("Asset Category")
		...
		assets = finergy.db.get_all("Asset", filters={"purchase_receipt": pr.name})

		self.assertEqual(len(assets), 3)

		location = finergy.db.get_value("Asset", assets[0].name, "location")
		self.assertEqual(location, "Test Location")

		pr.cancel()

	def test_purchase_return_with_submitted_asset(self):
		from capkpi.stock.doctype.purchase_receipt.purchase_receipt import make_purchase_return

		pr = make_purchase_receipt(item_code="Test Asset Item", qty=1)

		asset = finergy.get_doc("Asset", {"purchase_receipt": pr.name})
		asset.available_for_use_date = finergy.utils.nowdate()
		...
		self.assertRaises(finergy.exceptions.ValidationError, pr_return.submit)
	...
	def test_purchase_receipt_cost_center(self):
		...
		if not finergy.db.exists("Location", "Test Location"):
			finergy.get_doc({"doctype": "Location", "location_name": "Test Location"}).insert()
	...
	def test_subcontracted_pr_for_multi_transfer_batches(self):
		...
		rm_item_string = json.dumps(rm_items)
		se = finergy.get_doc(make_rm_stock_entry(po.name, rm_item_string))
		self.assertEqual(len(se.items), 2)
		se.items[0].batch_no = ste1.items[0].batch_no
		se.items[1].batch_no = ste2.items[0].batch_no
		se.submit()

		supplied_qty = finergy.db.get_value(
	...
	def test_po_to_pi_and_po_to_pr_worflow_full(self):
		...
		"""
		from capkpi.buying.doctype.purchase_order import purchase_order, test_purchase_order
	...
	def test_payment_terms_are_fetched_when_creating_purchase_invoice(self):
		...
		from capkpi.accounts.doctype.purchase_invoice.test_purchase_invoice import make_purchase_invoice
		from capkpi.buying.doctype.purchase_order.test_purchase_order import (
			create_purchase_order,
			make_pr_against_po,
		)
		from capkpi.selling.doctype.sales_order.test_sales_order import (
...
def make_purchase_receipt(**args):
	...
	finergy.db.set_value("Buying Settings", None, "allow_multiple_items", 1)
	pr = finergy.new_doc("Purchase Receipt")
	args = finergy._dict(args)
	...
	uom = args.uom or finergy.db.get_value("Item", item_code, "stock_uom") or "_Test UOM"
	pr.append(
		"items",
		{
			"item_code": item_code,
			...
			or finergy.get_cached_value("Company", pr.company, "cost_center"),
	...
	if args.get_multiple_items:
		...
		company_cost_center = finergy.get_cached_value("Company", pr.company, "cost_center")
...
def create_subcontracted_item(**args):
	...
	if not finergy.db.exists("Item", args.item_code):
		make_item(
			args.item_code,
			{
				"is_stock_item": 1,
	...
	if not args.raw_materials:
		if not finergy.db.exists("Item", "Test Extra Item 1"):
			make_item(
				"Test Extra Item 1",
				{
					"is_stock_item": 1,
		...
		if not finergy.db.exists("Item", "Test Extra Item 2"):
			make_item(
				"Test Extra Item 2",
				{
					"is_stock_item": 1,
	...
	if not finergy.db.get_value("BOM", {"item": args.item_code}, "name"):
		make_bom(item=args.item_code, raw_materials=args.get("raw_materials"))


test_dependencies = ["BOM", "Item Price", "Location"]
test_records = finergy.get_test_records("Purchase Receipt")
