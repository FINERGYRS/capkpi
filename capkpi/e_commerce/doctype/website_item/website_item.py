...
if TYPE_CHECKING:
	from capkpi.stock.doctype.item.item import Item

import finergy
from finergy import _
from finergy.utils import cint, cstr, flt, random_string
...
from capkpi.e_commerce.doctype.item_review.item_review import get_item_reviews
from capkpi.e_commerce.redisearch_utils import (
	delete_item_from_index,
	insert_item_to_index,
	update_index_for_item,
)
from capkpi.e_commerce.shopping_cart.cart import _set_price_list
from capkpi.setup.doctype.item_group.item_group import (
	get_parent_item_groups,
	invalidate_cache_for,
)
from capkpi.utilities.product import get_price
...
class WebsiteItem(WebsiteGenerator):
	...
	def autoname(self):
		...
		from finergy.model.naming import get_default_naming_series, make_autoname
	...
	def validate(self):
		...
		if not self.item_code:
			finergy.throw(_("Item Code is required"), title=_("Mandatory"))
		...
		if not self.get("__islocal"):
			wig = finergy.qb.DocType("Website Item Group")
			query = (
				finergy.qb.from_(wig)
	...
	def validate_duplicate_website_item(self):
		existing_web_item = finergy.db.exists("Website Item", {"item_code": self.item_code})
		if existing_web_item and existing_web_item != self.name:
			message = _("Website Item already exists against Item {0}").format(finergy.bold(self.item_code))
			finergy.throw(message, title=_("Already Published"))

	def publish_unpublish_desk_item(self, publish=True):
		if finergy.db.get_value("Item", self.item_code, "published_in_website") and publish:
			return  # if already published don't publish again
		finergy.db.set_value("Item", self.item_code, "published_in_website", publish)

	def make_route(self):
		"""Called from set_route in WebsiteGenerator."""
		if not self.route:
			return (
				cstr(finergy.db.get_value("Item Group", self.item_group, "route"))
				+ "/"
	...
	def update_template_item(self):
		...
		if self.variant_of:
			if self.published:
				# show template
				template_item = finergy.get_doc("Item", self.variant_of)
	...
	def validate_website_image(self):
		...
		if not file_doc:
			...
			finergy.msgprint(_("Website Image should be a public file or website URL"))

			self.website_image = None

	def make_thumbnail(self):
		"""Make a thumbnail of `website_image`"""
		if finergy.flags.in_import or finergy.flags.in_migrate:
			return

		import requests.exceptions

		db_website_image = finergy.db.get_value(self.doctype, self.name, "website_image")
		...
		if self.website_image and not self.thumbnail:
			...
			try:
				...
				finergy.msgprint(_("Warning: Invalid attachment {0}").format(self.website_image))
				self.website_image = None
	...
	def get_context(self, context):
		...
		self.attributes = finergy.get_all(
		...
		if finergy.db.exists(
			"Wishlist Item", {"item_code": self.item_code, "parent": finergy.session.user}
		):
			context.wished = True
	...
	def set_selected_attributes(self, variants, context, attribute_values_available):
		for variant in variants:
			variant.attributes = finergy.get_all(
				"Item Variant Attribute",
			...
			variant.attribute_map = finergy._dict(
	...
	def set_attribute_values(self, attributes, context, attribute_values_available):
		for attr in attributes:
			...
			if cint(finergy.db.get_value("Item Attribute", attr.attribute, "numeric_values")):
				for val in sorted(attribute_values_available.get(attr.attribute, []), key=flt):
					values.append(val)
			else:
				# get list of values defined (for sequence)
				for attr_value in finergy.db.get_all(
	...
	def set_metatags(self, context):
		context.metatags = finergy._dict({})

		safe_description = finergy.utils.to_markdown(self.description)

		context.metatags.url = finergy.utils.get_url() + "/" + context.route

		if context.website_image:
			if context.website_image.startswith("http"):
				url = context.website_image
			else:
				url = finergy.utils.get_url() + context.website_image
		...
		context.metatags["og:site_name"] = "CapKPI"

	def set_shopping_cart_data(self, context):
		from capkpi.e_commerce.shopping_cart.product_info import get_product_info_for_website
	...
	def copy_specification_from_item_group(self):
		...
		if self.item_group:
			for label, desc in finergy.db.get_values(
	...
	def get_tabs(self):
		...
		tab_values["tab_1_content"] = finergy.render_template(
	...
	def get_recommended_items(self, settings):
		ri = finergy.qb.DocType("Recommended Items")
		wi = finergy.qb.DocType("Website Item")
		...
		if settings.show_price:
			is_guest = finergy.session.user == "Guest"
...
def invalidate_cache_for_web_item(doc):
	...
	from capkpi.stock.doctype.item.item import invalidate_item_variants_cache_for_website
...
def on_doctype_update():
	...
	finergy.db.add_index("Website Item", ["route(500)"])

	finergy.db.add_index("Website Item", ["item_group"])
	finergy.db.add_index("Website Item", ["brand"])
...
def make_website_item(doc: "Item", save: bool = True) -> Union["WebsiteItem", List[str]]:
	...
	if finergy.db.exists("Website Item", {"item_code": doc.get("item_code")}):
		message = _("Website Item already exists against {0}").format(finergy.bold(doc.get("item_code")))
		finergy.throw(message, title=_("Already Published"))

	website_item = finergy.new_doc("Website Item")
	...
	if not finergy.flags.in_migrate and (doc.get("image") and not website_item.website_image):
