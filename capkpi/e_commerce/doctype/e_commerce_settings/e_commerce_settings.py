...
from finergy.utils import comma_and, flt, unique
...
class ShoppingCartSetupError(finergy.ValidationError):
	pass


class ECommerceSettings(Document):
	def onload(self):
		self.get("__onload").quotation_series = finergy.get_meta("Quotation").get_options("naming_series")
	...
	def validate(self):
		...
		finergy.clear_document_cache("E Commerce Settings", "E Commerce Settings")

		self.is_redisearch_enabled_pre_save = finergy.db.get_single_value(
	...
	def validate_field_filters(filter_fields, enable_field_filters):
		...
		web_item_meta = finergy.get_meta("Website Item")
		valid_fields = [
		...
		for row in filter_fields:
			if row.fieldname not in valid_fields:
				finergy.throw(
					...
					).format(row.idx, finergy.bold(row.fieldname))
				)
	...
	def validate_search_index_fields(self):
		...
		if not (set(fields).issubset(allowed_indexable_fields)):
			...
			if num_invalid_fields > 1:
				finergy.throw(
					_("{0} are not valid options for Search Index Field.").format(finergy.bold(invalid_fields))
				)
			else:
				finergy.throw(
					_("{0} is not a valid option for Search Index Field.").format(finergy.bold(invalid_fields))
				)
	...
	def validate_price_list_exchange_rate(self):
		...
		price_list_currency = finergy.db.get_value("Price List", self.price_list, "currency")

		if not company_currency:
			msg = f"Please specify currency in Company {self.company}"
			finergy.throw(_(msg), title=_("Missing Currency"), exc=ShoppingCartSetupError)

		if not price_list_currency:
			msg = f"Please specify currency in Price List {finergy.bold(self.price_list)}"
			finergy.throw(_(msg), title=_("Missing Currency"), exc=ShoppingCartSetupError)
		...
		if price_list_currency != company_currency:
			...
			if not flt(exchange_rate):
				...
				finergy.throw(_(msg), title=_("Missing"), exc=ShoppingCartSetupError)

	def validate_tax_rule(self):
		if not finergy.db.get_value("Tax Rule", {"use_for_shopping_cart": 1}, "name"):
			finergy.throw(finergy._("Set Tax Rule for shopping cart"), ShoppingCartSetupError)
...
def validate_cart_settings(doc=None, method=None):
	finergy.get_doc("E Commerce Settings", "E Commerce Settings").run_method("validate")


def get_shopping_cart_settings():
	if not getattr(finergy.local, "shopping_cart_settings", None):
		finergy.local.shopping_cart_settings = finergy.get_doc(
			"E Commerce Settings", "E Commerce Settings"
		)

	return finergy.local.shopping_cart_settings
...
def check_shopping_cart_enabled():
	if not get_shopping_cart_settings().enabled:
		finergy.throw(_("You need to enable Shopping Cart"), ShoppingCartSetupError)
