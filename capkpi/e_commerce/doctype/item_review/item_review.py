...
class UnverifiedReviewer(finergy.ValidationError):
	pass
...
def get_item_reviews(web_item, start=0, end=10, data=None):
	...
	if settings and settings.get("enable_reviews"):
		reviews_cache = finergy.cache().hget("item_reviews", web_item)
...
def get_queried_reviews(web_item, start=0, end=10, data=None):
	...
	data.reviews = finergy.db.get_all(
		"Item Review",
	...
	rating_data = finergy.db.get_all(
		"Item Review",
		filters={"website_item": web_item},
...
def set_reviews_in_cache(web_item, reviews_dict):
	finergy.cache().hset("item_reviews", web_item, reviews_dict)
...
def add_item_review(web_item, title, rating, comment=None):
	...
	if finergy.session.user == "Guest":
		...
		finergy.throw(_("You are not verified to write a review yet."), exc=UnverifiedReviewer)

	if not finergy.db.exists("Item Review", {"user": finergy.session.user, "website_item": web_item}):
		doc = finergy.get_doc(
			{
				"doctype": "Item Review",
				...
				"item": finergy.db.get_value("Website Item", web_item, "item_code"),
				"review_title": title,
				"rating": rating,
				"comment": comment,
			}
...
def get_customer(silent=False):
	...
	if customer:
		return finergy.db.get_value("Customer", customer)
	elif silent:
		return None
	else:
