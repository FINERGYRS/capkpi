...
from finergy.utils import add_months, cint, nowdate

from capkpi.accounts.doctype.tax_rule.tax_rule import ConflictingTaxRule
from capkpi.e_commerce.doctype.website_item.website_item import make_website_item
from capkpi.e_commerce.shopping_cart.cart import (
	_get_cart_quotation,
	get_cart_quotation,
	get_party,
...
class TestShoppingCart(unittest.TestCase):
	...
	def setUp(self):
		...
		if not finergy.db.exists("Website Item", {"item_code": "_Test Item"}):
			make_website_item(finergy.get_cached_doc("Item", "_Test Item"))

		if not finergy.db.exists("Website Item", {"item_code": "_Test Item 2"}):
			make_website_item(finergy.get_cached_doc("Item", "_Test Item 2"))
	...
	def test_get_cart_new_user(self):
		...
		self.assertEqual(
			...
			finergy.db.get_value("Contact", dict(email_id="test_cart_user@example.com")),
		)
		self.assertEqual(quotation.contact_email, finergy.session.user)
	...
	def test_add_item_variant_without_web_item_to_cart(self):
		...
		finergy.render_template("templates/includes/cart/cart_items.html", cart)
	...
	def test_cart_without_checkout_and_draft_quotation(self):
		...
		finergy.local.shopping_cart_settings = None

		# add item to cart
		update_cart("_Test Item", 1)
		quote_name = request_for_quotation()  # Request for Quote
		quote_doctstatus = cint(finergy.db.get_value("Quotation", quote_name, "docstatus"))

		self.assertEqual(quote_doctstatus, 0)

		finergy.db.set_value("E Commerce Settings", None, "save_quotations_as_draft", 0)
		finergy.local.shopping_cart_settings = None
		update_cart("_Test Item", 1)
		quote_name = request_for_quotation()  # Request for Quote
		quote_doctstatus = cint(finergy.db.get_value("Quotation", quote_name, "docstatus"))
	...
	def create_tax_rule(self):
		tax_rule = finergy.get_test_records("Tax Rule")[0]
		try:
			finergy.get_doc(tax_rule).insert(ignore_if_duplicate=True)
		except (finergy.DuplicateEntryError, ConflictingTaxRule):
			pass

	def create_quotation(self):
		quotation = finergy.new_doc("Quotation")

		values = {
			"doctype": "Quotation",
			"quotation_to": "Customer",
			"order_type": "Shopping Cart",
			"party_name": get_party(finergy.session.user).name,
			"docstatus": 0,
			...
			"taxes": finergy.get_doc("Sales Taxes and Charges Template", "_Test Tax 1 - _TC").taxes,
			"company": "_Test Company",
		}
	...
	def enable_shopping_cart(self):
		settings = finergy.get_doc("E Commerce Settings", "E Commerce Settings")
	...
	def login_as_new_user(self):
		...
		finergy.set_user("test_cart_user@example.com")
	...
	def login_as_customer(
		...
		finergy.set_user(email)

	def clear_existing_quotations(self):
		quotations = finergy.get_all(
			"Quotation",
		...
		for quotation in quotations:
			finergy.delete_doc("Quotation", quotation, ignore_permissions=True, force=True)
