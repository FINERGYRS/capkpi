capkpi.ProductView =  class {
	...
	*/
	...
	prepare_toolbar() {
		...
		finergy.require('/assets/js/e-commerce.min.js', function() {
			new capkpi.ProductSearch();
		});
	}
	...
	get_item_filter_data(from_filters=false) {
		...
		finergy.call({
			method: "capkpi.e_commerce.api.get_product_filter_data",
			args: {
				query_args: args
			},
	...
	get_query_filters() {
		const filters = finergy.utils.get_query_params();
	...
	bind_paging_action() {
		...
		$('.btn-prev, .btn-next').click((e) => {
			...
			let path = window.location.pathname + '?' + finergy.utils.get_url_from_dict(query_params);
	...
	bind_filters() {
		...
		$('.filter-lookup-input').on('keydown', finergy.utils.debounce((e) => {
	...
	change_route_with_filters() {
		let route_params = finergy.utils.get_query_params();
	...
	render_no_products_section(error=false) {
		...
		let no_results_section = `
			<div class="cart-empty finergy-card mt-4">
