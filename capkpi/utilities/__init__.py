...
def update_doctypes():
	for d in finergy.db.sql(
		...
		dt = finergy.get_doc("DocType", d.parent)
...
def get_site_info(site_info):
	...
	if not company:
		company = finergy.db.sql("select name from `tabCompany` order by creation asc")
		company = company[0][0] if company else None

	if company:
		domain = finergy.get_cached_value("Company", cstr(company), "domain")
