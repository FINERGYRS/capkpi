...
from finergy.utils import cint, cstr, flt, get_time, now_datetime
from six import string_types

from capkpi.controllers.status_updater import StatusUpdater


class UOMMustBeIntegerError(finergy.ValidationError):
	pass
...
class TransactionBase(StatusUpdater):
	def validate_posting_time(self):
		...
		if finergy.flags.in_import and self.posting_date:
		...
		if not getattr(self, "set_posting_time", None):
			...
			try:
				...
				finergy.throw(_("Invalid Posting Time"))
		...
		if participations:
			for participation in participations:
				...
				if len(total_participants) <= 1:
					finergy.db.sql("delete from `tabEvent` where name='%s'" % participation.parent)

				finergy.db.sql("delete from `tabEvent Participants` where name='%s'" % participation.name)

	def _add_calendar_event(self, opts):
		opts = finergy._dict(opts)
		...
		if self.contact_date:
			...
			if finergy.db.exists("User", self.contact_by):
				finergy.share.add("Event", event.name, self.contact_by, flags={"ignore_share_permission": True})
				...
				if ref_dn:
					if is_child:
						...
						if ref_dn not in item_ref_dn:
							...
							finergy.throw(_("Duplicate row {0} with same {1}").format(d.idx, key))
					elif ref_dn:
	...
	def compare_values(self, ref_doc, fields, doc=None):
		for reference_doctype, ref_dn_list in ref_doc.items():
			for reference_name in ref_dn_list:
				prevdoc_values = finergy.db.get_value(
					reference_doctype, reference_name, [d[0] for d in fields], as_dict=1
				)

				if not prevdoc_values:
					finergy.throw(_("Invalid reference {0} {1}").format(reference_doctype, reference_name))
	...
	def validate_rate_with_reference_doc(self, ref_details):
		...
		if self.doctype in buying_doctypes:
			action = finergy.db.get_single_value("Buying Settings", "maintain_same_rate_action")
			settings_doc = "Buying Settings"
		else:
			action = finergy.db.get_single_value("Selling Settings", "maintain_same_rate_action")
			settings_doc = "Selling Settings"

		for ref_dt, ref_dn_field, ref_link_field in ref_details:
			for d in self.get("items"):
				if d.get(ref_link_field):
					ref_rate = finergy.db.get_value(ref_dt + " Item", d.get(ref_link_field), "rate")

					if abs(flt(d.rate - ref_rate, d.precision("rate"))) >= 0.01:
						if action == "Stop":
							role_allowed_to_override = finergy.db.get_single_value(
								settings_doc, "role_to_override_stop_action"
							)

							if role_allowed_to_override not in finergy.get_roles():
								finergy.throw(
...
def delete_events(ref_type, ref_name):
	...
	if events:
		finergy.delete_doc("Event", events, for_reload=True)
...
def validate_uom_is_integer(doc, uom_field, qty_fields, child_dt=None):
	...
	integer_uoms = list(
		filter(
			lambda uom: finergy.db.get_value("UOM", uom, "must_be_whole_number", cache=True) or None,
			distinct_uoms,
		)
	)
					...
					if abs(cint(qty) - flt(qty)) > 0.0000001:
						finergy.throw(
							_(
								...
								qty, d.idx, finergy.bold(_("Must be Whole Number")), finergy.bold(d.get(uom_field))
							),
