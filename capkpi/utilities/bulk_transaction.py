...
def job(deserialized_data, from_doctype, to_doctype):
	...
	for d in deserialized_data:
		...
		try:
			i += 1
			doc_name = d.get("name")
			finergy.db.savepoint("before_creation_state")
			task(doc_name, from_doctype, to_doctype)

		except Exception as e:
			finergy.db.rollback(save_point="before_creation_state")
...
def task(doc_name, from_doctype, to_doctype):
	from capkpi.accounts.doctype.payment_entry import payment_entry
	from capkpi.accounts.doctype.purchase_invoice import purchase_invoice
	from capkpi.accounts.doctype.sales_invoice import sales_invoice
	from capkpi.buying.doctype.purchase_order import purchase_order
	from capkpi.buying.doctype.supplier_quotation import supplier_quotation
	from capkpi.selling.doctype.quotation import quotation
	from capkpi.selling.doctype.sales_order import sales_order
	from capkpi.stock.doctype.delivery_note import delivery_note
	from capkpi.stock.doctype.purchase_receipt import purchase_receipt
...
def check_logger_doc_exists(log_date):
	return finergy.db.exists("Bulk Transaction Log", log_date)


def get_logger_doc(log_date):
	return finergy.get_doc("Bulk Transaction Log", log_date)


def create_logger_doc():
	log_doc = finergy.new_doc("Bulk Transaction Log")
