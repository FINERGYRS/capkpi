...
capkpi.accounts.bank_reconciliation.NumberCardManager = class NumberCardManager {
	constructor(opts) {
	...
	make_cards() {
		...
		chart_data.forEach((summary) => {
			let number_card = new capkpi.accounts.NumberCard(summary);
...
capkpi.accounts.NumberCard = class NumberCard {
	constructor(options) {
		this.$card = finergy.utils.build_summary_item(options);
	}
