...
capkpi.taxes_and_totals = class TaxesAndTotals extends capkpi.payments {
	setup() {
	...
	apply_pricing_rule_on_item(item) {
		...
		finergy.model.set_value(item.doctype, item.name, "rate", item_rate);
	}
	...
	calculate_discount_amount() {
		if (finergy.meta.get_docfield(this.frm.doc.doctype, "discount_amount")) {
	...
	validate_conversion_rate() {
		...
		var conversion_rate_label = finergy.meta.get_label(this.frm.doc.doctype, "conversion_rate",
			this.frm.doc.name);
	...
	calculate_item_values() {
		...
		if (!this.discount_amount_applied) {
			$.each(this.frm.doc["items"] || [], function(i, item) {
				finergy.model.round_floats_in(item);
	...
	initialize_taxes() {
		...
		$.each(this.frm.doc["taxes"] || [], function(i, tax) {
			...
			finergy.model.round_floats_in(tax);
		});
	}

	fetch_round_off_accounts() {
		let me = this;
		finergy.flags.round_off_applicable_accounts = [];

		if (me.frm.doc.company) {
			return finergy.call({
				"method": "capkpi.controllers.taxes_and_totals.get_round_off_applicable_accounts",
				"args": {
					"company": me.frm.doc.company,
					"account_list": finergy.flags.round_off_applicable_accounts
				},
				callback(r) {
					if (r.message) {
						finergy.flags.round_off_applicable_accounts.push(...r.message);
					}
				}
			});
		}
	...
	calculate_shipping_charges() {
		...
		finergy.model.round_floats_in(this.frm.doc, ["total", "base_total", "net_total", "base_net_total"]);
		if (finergy.meta.get_docfield(this.frm.doc.doctype, "shipping_rule", this.frm.doc.name)) {
	...
	round_off_totals(tax) {
		if (finergy.flags.round_off_applicable_accounts.includes(tax.account_head)) {
	...
	manipulate_grand_total_for_inclusive_tax() {
		...
		if (this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			if (any_inclusive_tax) {
				...
				var non_inclusive_tax_amount = finergy.utils.sum($.map(this.frm.doc.taxes || [],
					function(d) {
	...
	calculate_totals() {
		...
		if(in_list(["Quotation", "Sales Order", "Delivery Note", "Sales Invoice", "POS Invoice"], this.frm.doc.doctype)) {
			...
			if(tax_count) {
				...
				finergy.model.round_floats_in(this.frm.doc,
		...
		finergy.model.round_floats_in(this.frm.doc, ["grand_total", "base_grand_total"]);
	...
	set_rounded_total() {
		...
		if(finergy.meta.get_docfield(this.frm.doc.doctype, "disable_rounded_total", this.frm.doc.name)) {
			disable_rounded_total = this.frm.doc.disable_rounded_total;
		} else if (finergy.sys_defaults.disable_rounded_total) {
			disable_rounded_total = finergy.sys_defaults.disable_rounded_total;
		}
		...
		if(finergy.meta.get_docfield(this.frm.doc.doctype, "rounded_total", this.frm.doc.name)) {
	...
	_cleanup() {
		...
		if(this.frm.doc["items"] && this.frm.doc["items"].length) {
			if(!finergy.meta.get_docfield(this.frm.doc["items"][0].doctype, "item_tax_amount", this.frm.doctype)) {
				$.each(this.frm.doc["items"] || [], function(i, item) {
		...
		if(this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			if(!finergy.meta.get_docfield(this.frm.doc["taxes"][0].doctype, "tax_amount_after_discount_amount", this.frm.doctype)) {
	...
	set_discount_amount() {
		if(this.frm.doc.additional_discount_percentage) {
			this.frm.doc.discount_amount = flt(flt(this.frm.doc[finergy.scrub(this.frm.doc.apply_discount_on)])
	...
	apply_discount_amount() {
		...
		if (this.frm.doc.discount_amount) {
			if(!this.frm.doc.apply_discount_on)
				finergy.throw(__("Please select Apply Discount On"));
	...
	calculate_total_advance(update_paid_amount) {
		var total_allocated_amount = finergy.utils.sum($.map(this.frm.doc["advances"] || [], function(adv) {
	...
	calculate_outstanding_amount(update_paid_amount) {
		...
		finergy.model.round_floats_in(this.frm.doc, ["grand_total", "total_advance", "write_off_amount"]);
		...
		if(in_list(["Sales Invoice", "POS Invoice", "Purchase Invoice"], this.frm.doc.doctype)) {
			...
			finergy.model.round_floats_in(this.frm.doc, ["paid_amount"]);
		...
		if(this.frm.doc.is_pos && (update_paid_amount===undefined || update_paid_amount)) {
			$.each(this.frm.doc['payments'] || [], function(index, data) {
				if(data.default && payment_status && total_amount_to_pay > 0) {
					...
					finergy.model.set_value(data.doctype, data.name, "base_amount", base_amount);
					finergy.model.set_value(data.doctype, data.name, "amount", amount);
					payment_status = false;

				} else if(me.frm.doc.paid_amount) {
					finergy.model.set_value(data.doctype, data.name, "amount", 0.0);
				}
			});
		}
	}
