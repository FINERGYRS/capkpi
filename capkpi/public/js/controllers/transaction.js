...
capkpi.TransactionController = capkpi.taxes_and_totals.extend({
	setup: function() {
		this._super();
		let me = this;
		finergy.flags.hide_serial_batch_dialog = true;
		finergy.ui.form.on(this.frm.doctype + " Item", "rate", function(frm, cdt, cdn) {
			var item = finergy.get_doc(cdt, cdn);
			var has_margin_field = finergy.meta.has_field(cdt, 'margin_type');

			finergy.model.round_floats_in(item, ["rate", "price_list_rate"]);
		...
		finergy.ui.form.on(this.frm.cscript.tax_table, "rate", function(frm, cdt, cdn) {
			cur_frm.cscript.calculate_taxes_and_totals();
		});

		finergy.ui.form.on(this.frm.cscript.tax_table, "tax_amount", function(frm, cdt, cdn) {
			cur_frm.cscript.calculate_taxes_and_totals();
		});

		finergy.ui.form.on(this.frm.cscript.tax_table, "row_id", function(frm, cdt, cdn) {
			cur_frm.cscript.calculate_taxes_and_totals();
		});

		finergy.ui.form.on(this.frm.cscript.tax_table, "included_in_print_rate", function(frm, cdt, cdn) {
			cur_frm.cscript.set_dynamic_labels();
			cur_frm.cscript.calculate_taxes_and_totals();
		});

		finergy.ui.form.on(this.frm.doctype, "apply_discount_on", function(frm) {
		...
		finergy.ui.form.on(this.frm.doctype, "additional_discount_percentage", function(frm) {
			if(!frm.doc.apply_discount_on) {
				finergy.msgprint(__("Please set 'Apply Additional Discount On'"));
				return;
			}
			...
			var total = flt(frm.doc[finergy.model.scrub(frm.doc.apply_discount_on)]);
		...
		finergy.ui.form.on(this.frm.doctype, "discount_amount", function(frm) {
		...
		finergy.ui.form.on(this.frm.doctype + " Item", {
			items_add: function(frm, cdt, cdn) {
				...
				capkpi.accounts.dimensions.copy_dimension_from_first_row(frm, cdt, cdn, 'items');
			}
		});
		...
		if(finergy.meta.get_docfield(this.frm.doc.doctype, "pricing_rules")) {
		...
		if (this.frm.fields_dict["items"].grid.get_field('blanket_order')) {
			this.frm.set_query("blanket_order", "items", function(doc, cdt, cdn) {
				...
				return {
					query: "capkpi.controllers.queries.get_blanket_orders",
					filters: {
						"company": doc.company,
	...
	onload: function() {
		...
		if(this.frm.doc.__islocal) {
			var currency = finergy.defaults.get_user_default("currency");
	...
	make_payment_request: function() {
		...
		finergy.call({
			method:"capkpi.accounts.doctype.payment_request.payment_request.make_payment_request",
			args: {
	...
	onload_post_render: function() {
		if(this.frm.doc.__islocal && !(this.frm.doc.taxes || []).length
			...
			finergy.after_ajax(() => this.apply_default_taxes());
		} else if(this.frm.doc.__islocal && this.frm.doc.company && this.frm.doc["items"]
			&& !this.frm.doc.is_pos) {
			finergy.after_ajax(() => this.calculate_taxes_and_totals());
		}
		if(finergy.meta.get_docfield(this.frm.doc.doctype + " Item", "item_code")) {
	...
	scan_barcode: function() {
		...
		if(this.frm.doc.scan_barcode) {
			finergy.call({
				method: "capkpi.selling.page.point_of_sale.point_of_sale.search_for_serial_or_batch_or_barcode_number",
				args: {
	...
	modify_table_after_scan(data) {
		...
		let batch_no_scan = Boolean(data.batch_no) && finergy.meta.has_field(cur_grid.doctype, "batch_no");
		...
		if (!row_to_modify) {
			...
			row_to_modify = finergy.model.add_child(this.frm.doc, cur_grid.doctype, 'items');
		}
	...
	set_scanned_values(row_to_modify, data, scan_barcode_field) {
		...
		finergy.model.set_value(row_to_modify.doctype, row_to_modify.name, {
			item_code: data.item_code,
			qty: (row_to_modify.qty || 0) + 1
		});

		['serial_no', 'batch_no', 'barcode'].forEach(field => {
			if (data[field] && finergy.meta.has_field(row_to_modify.doctype, field)) {
				...
				finergy.model.set_value(row_to_modify.doctype, row_to_modify.name, field, value);
			}
		});
	...
	apply_default_taxes() {
		var me = this;
		var taxes_and_charges_field = finergy.meta.get_docfield(me.frm.doc.doctype, "taxes_and_charges",
			me.frm.doc.name);
		...
		if (taxes_and_charges_field) {
			return finergy.call({
				method: "capkpi.controllers.accounts_controller.get_default_taxes_and_charges",
				args: {
	...
	send_sms: function() {
		var sms_man = new capkpi.SMSManager(this.frm.doc);
	},
		...
		if(item.item_code || item.barcode || item.serial_no) {
			if(!this.validate_company_and_party()) {
				...
				return this.frm.call({
					method: "capkpi.stock.get_item_details.get_item_details",
					child: item,
					args: {
						doc: me.frm.doc,
						args: {
							...
							finergy.run_serially([
								...
								() => {
									if (show_batch_dialog)
										return finergy.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"])
											.then((r) => {
												if (r.message &&
												(r.message.has_batch_no || r.message.has_serial_no)) {
													finergy.flags.hide_serial_batch_dialog = false;
												}
											});
								},
								() => {
									// check if batch serial selector is disabled or not
									if (show_batch_dialog && !finergy.flags.hide_serial_batch_dialog)
										return finergy.db.get_single_value('Stock Settings', 'disable_serial_no_and_batch_selector')
											.then((value) => {
												if (value) {
													finergy.flags.hide_serial_batch_dialog = true;
												}
								...
								() => {
									if(show_batch_dialog && !finergy.flags.hide_serial_batch_dialog) {
										...
										capkpi.show_serial_batch_selector(me.frm, d, (item) => {
											me.frm.script_manager.trigger('qty', item.doctype, item.name);
											if (!me.frm.doc.set_warehouse)
												me.frm.script_manager.trigger('warehouse', item.doctype, item.name);
											me.apply_price_list(item, true);
										}, undefined, !finergy.flags.hide_serial_batch_dialog);
									}
								},
	...
	price_list_rate: function(doc, cdt, cdn) {
		var item = finergy.get_doc(cdt, cdn);
		finergy.model.round_floats_in(item, ["price_list_rate", "discount_percentage"]);
	...
	margin_type: function(doc, cdt, cdn) {
		...
		let item = finergy.get_doc(cdt, cdn);
		if (!item.margin_type) {
			finergy.model.set_value(cdt, cdn, "margin_rate_or_amount", 0);
		} else {
	...
	get_incoming_rate: function(item, posting_date, posting_time, voucher_type, company) {
		...
		finergy.call({
			...
			callback: function(r) {
				finergy.model.set_value(item.doctype, item.name, 'rate', r.message * item.conversion_factor);
			}
		});
	},
	...
	add_taxes_from_item_tax_template: function(item_tax_map) {
		...
		if (item_tax_map && cint(finergy.defaults.get_default("add_taxes_from_item_tax_template"))) {
			...
			$.each(item_tax_map, function(tax, rate) {
				...
				if (!found) {
					let child = finergy.model.add_child(me.frm.doc, "taxes");
	...
	serial_no: function(doc, cdt, cdn) {
		...
		if (item && item.serial_no) {
			if (!item.item_code) {
				...
				if (!doc.is_return && cint(finergy.user_defaults.set_qty_in_transactions_based_on_serial_no_input)) {
					setTimeout(() => {
	...
	update_qty: function(cdt, cdn) {
		...
		finergy.model.set_value(item.doctype, item.name,
			"qty", valid_serial_nos.length / item.conversion_factor);
		finergy.model.set_value(item.doctype, item.name, "stock_qty", valid_serial_nos.length);
	},
	...
	set_default_internal_warehouse: function() {
		...
		if ((this.frm.doc.doctype === 'Sales Invoice' && me.frm.doc.update_stock)
			...
			if (this.frm.doc.is_internal_customer && this.frm.doc.company === this.frm.doc.represents_company) {
				finergy.db.get_value('Company', this.frm.doc.company, 'default_in_transit_warehouse', function(value) {
	...
	company: function() {
		...
		var set_pricing = function() {
			if(me.frm.doc.company && me.frm.fields_dict.currency) {
				var company_currency = me.get_company_currency();
				var company_doc = finergy.get_doc(":Company", me.frm.doc.company);
				...
				if (company_doc.default_selling_terms && finergy.meta.has_field(me.frm.doc.doctype, "tc_name") &&
				...
				if (company_doc.default_buying_terms && finergy.meta.has_field(me.frm.doc.doctype, "tc_name") &&
		...
		var set_party_account = function(set_pricing) {
			if (in_list(["Sales Invoice", "Purchase Invoice"], me.frm.doc.doctype)) {
				...
				var party = me.frm.doc[finergy.model.scrub(party_type)];
				if(party && me.frm.doc.company) {
					return finergy.call({
						method: "capkpi.accounts.party.get_party_account",
						args: {
							company: me.frm.doc.company,
		...
		if (finergy.meta.get_docfield(this.frm.doctype, "shipping_address") &&
			in_list(['Purchase Order', 'Purchase Receipt', 'Purchase Invoice'], this.frm.doctype)) {
			capkpi.utils.get_shipping_address(this.frm, function() {
		...
		if(this.frm.doc.company) {
			capkpi.last_selected_company = this.frm.doc.company;
		}
	},
	...
	transaction_date: function() {
		if (this.frm.doc.transaction_date) {
			...
			finergy.ui.form.trigger(this.frm.doc.doctype, "currency");
		}
	},
	...
	posting_date: function() {
		...
		if (this.frm.doc.posting_date) {
			...
			if ((this.frm.doc.doctype == "Sales Invoice" && this.frm.doc.customer) ||
				...
				return finergy.call({
					method: "capkpi.accounts.party.get_due_date",
					args: {
					...
					callback: function(r, rt) {
						if(r.message) {
							...
							finergy.ui.form.trigger(me.frm.doc.doctype, "currency");
	...
	due_date: function() {
		...
		if (this.frm.doc.due_date && !this.frm.updating_party_details && !this.frm.doc.is_pos) {
			if (this.frm.doc.payment_terms_template ||
				...
				finergy.msgprint(final_message);
			}
		}
	},
	...
	get_company_currency: function() {
		return capkpi.get_currency(this.frm.doc.company);
	},

	contact_person: function() {
		capkpi.utils.get_contact_details(this.frm);
	},
	...
	conversion_rate: function() {
		...
		this.frm.set_df_property("conversion_rate", "read_only", capkpi.stale_rate_allowed() ? 0 : 1);
	},
		...
		if (in_list(["Quotation", "Sales Order", "Delivery Note", "Sales Invoice", "Purchase Invoice", "Purchase Order", "Purchase Receipt"]), this.frm.doc.doctype) {
			...
			$.each(this.frm.doc.items || [], function(i, d) {
				if(d.margin_type == "Amount") {
					finergy.model.set_value(d.doctype, d.name, "margin_rate_or_amount",
	...
	set_actual_charges_based_on_currency: function(exchange_rate) {
		...
		$.each(this.frm.doc.taxes || [], function(i, d) {
			if(d.charge_type == "Actual") {
				finergy.model.set_value(d.doctype, d.name, "tax_amount",
	...
	get_exchange_rate: function(transaction_date, from_currency, to_currency, callback) {
		...
		return finergy.call({
			method: "capkpi.setup.utils.get_exchange_rate",
			args: {
	...
	uom: function(doc, cdt, cdn) {
		...
		if(item.item_code && item.uom) {
			return this.frm.call({
				method: "capkpi.stock.get_item_details.get_conversion_factor",
				args: {
					item_code: item.item_code,
					uom: item.uom
				},
				callback: function(r) {
					if(!r.exc) {
						finergy.model.set_value(cdt, cdn, 'conversion_factor', r.message.conversion_factor);
					}
				}
			});
		}
	...
	conversion_factor: function(doc, cdt, cdn, dont_fetch_price_list_rate) {
		if(finergy.meta.get_docfield(cdt, "stock_qty", cdn)) {
			var item = finergy.get_doc(cdt, cdn);
			finergy.model.round_floats_in(item, ["qty", "conversion_factor"]);
			...
			if(finergy.flags.dont_fetch_price_list_rate) {
				return
			}

			if (!dont_fetch_price_list_rate &&
				finergy.meta.has_field(doc.doctype, "price_list_currency")) {
	...
	toggle_conversion_factor: function(item) {
		...
		if(this.frm.get_field('items').grid.fields_map.conversion_factor) {
			this.frm.fields_dict.items.grid.toggle_enable("conversion_factor",
				((item.uom != item.stock_uom) && !finergy.meta.get_docfield(cur_frm.fields_dict.items.grid.doctype, "conversion_factor").read_only)? true: false);
		}
	...
	service_stop_date: function(frm, cdt, cdn) {
		...
		if(child.service_stop_date) {
			...
			if(stop_date < start_date) {
				finergy.model.set_value(cdt, cdn, "service_stop_date", "");
				finergy.throw(__("Service Stop Date cannot be before Service Start Date"));
			} else if (stop_date > end_date) {
				finergy.model.set_value(cdt, cdn, "service_stop_date", "");
				finergy.throw(__("Service Stop Date cannot be after Service End Date"));
			}
		}
	},
	...
	service_start_date: function(frm, cdt, cdn) {
		...
		if(child.service_start_date) {
			finergy.call({
				"method": "capkpi.stock.get_item_details.calculate_service_end_date",
				args: {"args": child},
				callback: function(r) {
					finergy.model.set_value(cdt, cdn, "service_end_date", r.message.service_end_date);
				}
			})
		}
	},
	...
	change_form_labels: function(company_currency) {
		...
		if(finergy.meta.get_docfield(cur_frm.doctype, "net_total"))
			cur_frm.toggle_display("net_total", show);

		if(finergy.meta.get_docfield(cur_frm.doctype, "base_net_total"))
	...
	change_grid_labels: function(company_currency) {
		...
		if (this.frm.doc.operations && this.frm.doc.operations.length > 0) {
			...
			$.each(["base_operating_cost", "base_hour_rate"], function(i, fname) {
				if(finergy.meta.get_docfield(item_grid.doctype, fname))
	...
	update_payment_schedule_grid_labels: function(company_currency) {
		...
		if (this.frm.doc.payment_schedule && this.frm.doc.payment_schedule.length > 0) {
			...
			$.each(["base_payment_amount", "base_outstanding", "base_paid_amount"], function(i, fname) {
				if (finergy.meta.get_docfield(schedule_grid.doctype, fname))
	...
	ignore_pricing_rule: function() {
		if(this.frm.doc.ignore_pricing_rule) {
			...
			return this.frm.call({
				method: "capkpi.accounts.doctype.pricing_rule.pricing_rule.remove_pricing_rules",
	...
	apply_pricing_rule: function(item, calculate_taxes_and_totals) {
		...
		return this.frm.call({
			method: "capkpi.accounts.doctype.pricing_rule.pricing_rule.apply_pricing_rule",
	...
	_set_values_for_item_list: function(children) {
		...
		for(var i=0, l=children.length; i<l; i++) {
			var d = children[i] ;
			let item_row = finergy.get_doc(d.doctype, d.name);
			var existing_pricing_rule = finergy.model.get_value(d.doctype, d.name, "pricing_rules");
			for(var k in d) {
				var v = d[k];
			...
			finergy.model.round_floats_in(item_row, ["price_list_rate", "discount_percentage"]);

			// if pricing rule set as blank from an existing value, apply price_list
			if(!me.frm.doc.ignore_pricing_rule && existing_pricing_rule && !d.pricing_rules) {
				me.apply_price_list(finergy.get_doc(d.doctype, d.name));
			} else if(!d.pricing_rules) {
				me.remove_pricing_rule(finergy.get_doc(d.doctype, d.name));
			}
					...
					if (in_list(data.apply_rule_on_other_items, d[data.apply_rule_on])) {
						for(var k in data) {
							if (in_list(fields, k) && data[k] && (data.price_or_product_discount === 'price' || k === 'pricing_rules')) {
								finergy.model.set_value(d.doctype, d.name, k, data[k]);
							}
						}
					}
				});
	...
	apply_product_discount: function(args) {
		...
		args.free_item_data.forEach(pr_row => {
			...
			if (!items || !in_list(exist_items, (pr_row.item_code, pr_row.pricing_rules))) {
				...
				row_to_modify = finergy.model.add_child(this.frm.doc,
	...
	apply_price_list: function(item, reset_plc_conversion) {
		...
		// `capkpi.stock.get_item_details.apply_price_list` is sensitive to its value
		...
		return this.frm.call({
			method: "capkpi.stock.get_item_details.apply_price_list",
			args: {	args: args },
			callback: function(r) {
				if (!r.exc) {
					finergy.run_serially([
	...
	validate_company_and_party: function() {
		...
		$.each(["company", "customer"], function(i, fieldname) {
			if(finergy.meta.has_field(me.frm.doc.doctype, fieldname) && me.frm.doc.doctype != "Purchase Order") {
				if (!me.frm.doc[fieldname]) {
					finergy.msgprint(__("Please specify") + ": " +
						finergy.meta.get_label(me.frm.doc.doctype, fieldname, me.frm.doc.name) +
	...
	get_terms: function() {
		...
		capkpi.utils.get_terms(this.frm.doc.tc_name, this.frm.doc, function(r) {
			if(!r.exc) {
	...
	taxes_and_charges: function() {
		...
		if(this.frm.doc.taxes_and_charges) {
			return this.frm.call({
				method: "capkpi.controllers.accounts_controller.get_taxes_and_charges",
				args: {
					"master_doctype": finergy.meta.get_docfield(this.frm.doc.doctype, "taxes_and_charges",
	...
	tax_category: function() {
		...
		finergy.run_serially([
			...
			() => capkpi.utils.set_taxes(this.frm, "tax_category"),
		]);
	},
	...
	update_item_tax_map: function() {
		...
		if (item_codes.length) {
			return this.frm.call({
				method: "capkpi.stock.get_item_details.get_item_tax_info",
				args: {
					company: me.frm.doc.company,
	...
	item_tax_template: function(doc, cdt, cdn) {
		...
		if(item.item_tax_template) {
			return this.frm.call({
				method: "capkpi.stock.get_item_details.get_item_tax_map",
				args: {
					company: me.frm.doc.company,
					item_tax_template: item.item_tax_template,
					as_json: true
	...
	is_recurring: function() {
		...
		if(this.frm.doc.is_recurring && this.frm.doc.__islocal) {
			finergy.msgprint(__("Please set recurring after saving"));
		...
		if(this.frm.doc.is_recurring) {
			...
			var owner_email = this.frm.doc.owner=="Administrator"
				? finergy.user_info("Administrator").email
				: this.frm.doc.owner;
			...
			this.frm.doc.repeat_on_day_of_month = finergy.datetime.str_to_obj(this.frm.doc.posting_date).getDate();
		}
	...
	from_date: function() {
		...
		if(this.frm.doc.from_date) {
			...
			if(months) {
				var to_date = finergy.datetime.add_months(this.frm.doc.from_date,
					months);
				this.frm.doc.to_date = finergy.datetime.add_days(to_date, -1);
	...
	setup_item_selector: function() {
		...
		// 	this.item_selector = new capkpi.ItemSelector({frm: this.frm});
		// }
	},
	...
	make_quality_inspection: function () {
		...
		const dialog = new finergy.ui.Dialog({
			...
			primary_action: function () {
				...
				finergy.call({
					method: "capkpi.controllers.stock_controller.make_quality_inspections",
					args: {
						doctype: me.frm.doc.doctype,
					...
					callback: function (r) {
						if (r.message.length > 0) {
							if (r.message.length === 1) {
								finergy.set_route("Form", "Quality Inspection", r.message[0]);
							} else {
								finergy.route_options = {
									"reference_type": me.frm.doc.doctype,
									"reference_name": me.frm.doc.name
								};
								finergy.set_route("List", "Quality Inspection");
							}
						}
						dialog.hide();
					}
		...
		if (!data.length) {
			finergy.msgprint(__("All items in this document already have a linked Quality Inspection."));
		} else {
			dialog.show();
		}
	},
	...
	get_method_for_payment: function(){
		var method = "capkpi.accounts.doctype.payment_entry.payment_entry.get_payment_entry";
		if(cur_frm.doc.__onload && cur_frm.doc.__onload.make_payment_via_journal_entry){
			if(in_list(['Sales Invoice', 'Purchase Invoice'],  cur_frm.doc.doctype)){
				method = "capkpi.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_invoice";
			}else {
				method= "capkpi.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_order";
			}
		}
	...
	set_query_for_batch: function(doc, cdt, cdn) {
		...
		if(!item.item_code) {
			finergy.throw(__("Please enter Item Code to get batch no"));
			...
			let filters = {
				'item_code': item.item_code,
				'posting_date': me.frm.doc.posting_date || finergy.datetime.nowdate(),
			}
	...
	set_query_for_item_tax_template: function(doc, cdt, cdn) {
		...
		if(!item.item_code) {
			...
			return {
				query: "capkpi.controllers.queries.get_tax_template",
				filters: filters
			}
		}
	},
	...
	payment_terms_template: function() {
		...
		if(doc.payment_terms_template && doc.doctype !== 'Delivery Note') {
			...
			finergy.call({
				method: "capkpi.controllers.accounts_controller.get_payment_terms",
				args: {
	...
	payment_term: function(doc, cdt, cdn) {
		...
		if(row.payment_term) {
			finergy.call({
				method: "capkpi.controllers.accounts_controller.get_payment_term_details",
				args: {
				...
				callback: function(r) {
					if(r.message && !r.exc) {
						for (var d in r.message) {
							finergy.model.set_value(cdt, cdn, d, r.message[d]);
							const company_currency = me.get_company_currency();
							me.update_payment_schedule_grid_labels(company_currency);
						}
					}
	...
	against_blanket_order: function(doc, cdt, cdn) {
		...
		if(!item.against_blanket_order) {
			finergy.model.set_value(this.frm.doctype + " Item", item.name, "blanket_order", null);
			finergy.model.set_value(this.frm.doctype + " Item", item.name, "blanket_order_rate", 0.00);
		}
	},
	...
	blanket_order: function(doc, cdt, cdn) {
		...
		if (item.blanket_order && (item.parenttype=="Sales Order" || item.parenttype=="Purchase Order")) {
			finergy.call({
				method: "capkpi.stock.get_item_details.get_blanket_order_details",
				args: {
					args:{
						item_code: item.item_code,
				...
				callback: function(r) {
					if (!r.message) {
						finergy.throw(__("Invalid Blanket Order for the selected Customer and Item"));
					} else {
						finergy.run_serially([
							() => finergy.model.set_value(cdt, cdn, "blanket_order_rate", r.message.blanket_order_rate),
							() => me.frm.script_manager.trigger("price_list_rate", cdt, cdn)
						]);
					}
				}
	...
	autofill_warehouse : function (child_table, warehouse_field, warehouse) {
		if (warehouse && child_table && child_table.length) {
			...
			$.each(child_table || [], function(i, item) {
				finergy.model.set_value(doctype, item.name, warehouse_field, warehouse);
			});
		}
	},
...
capkpi.show_serial_batch_selector = function (frm, d, callback, on_close, show_dialog) {
...
capkpi.apply_putaway_rule = (frm, purpose=null) => {
	if (!frm.doc.company) {
		finergy.throw({message: __("Please select a Company first."), title: __("Mandatory")});
	}
	if (!frm.doc.items.length) return;

	finergy.call({
		method: "capkpi.stock.doctype.putaway_rule.putaway_rule.apply_putaway_rule",
		args: {
			doctype: frm.doctype,
			items: frm.doc.items,
