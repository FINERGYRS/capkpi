...
capkpi.TransactionController = class TransactionController extends capkpi.taxes_and_totals {
	setup() {
		super.setup();
		let me = this;
	...
	_set_values_for_item_list(children) {
		...
		for (const child of children) {
			const existing_pricing_rule = finergy.model.get_value(child.doctype, child.name, "pricing_rules");

			for (const [key, value] of Object.entries(child)) {
				if (!["doctype", "name"].includes(key)) {
					if (key === "price_list_rate") {
						finergy.model.set_value(child.doctype, child.name, "rate", value);
					}

					if (key !== "free_item_data") {
						finergy.model.set_value(child.doctype, child.name, key, value);
					}
				}
			}

			finergy.model.round_floats_in(
				finergy.get_doc(child.doctype, child.name),
			...
			if (!this.frm.doc.ignore_pricing_rule && existing_pricing_rule && !child.pricing_rules) {
				this.apply_price_list(finergy.get_doc(child.doctype, child.name));
			} else if (!child.pricing_rules) {
				this.remove_pricing_rule(finergy.get_doc(child.doctype, child.name));
			}
