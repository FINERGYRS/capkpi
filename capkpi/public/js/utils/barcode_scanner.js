capkpi.utils.BarcodeScanner = class BarcodeScanner {
	constructor(opts) {
		this.frm = opts.frm;
		...
		// }
		this.scan_api = opts.scan_api || "capkpi.stock.utils.scan_barcode";
	}
	...
	update_table(data) {
		return new Promise(resolve => {
			...
			if (!row) {
				...
				row = finergy.model.add_child(this.frm.doc, cur_grid.doctype, this.items_table_name);
	...
	set_item(row, item_code) {
		return new Promise(resolve => {
			const increment = async (value = 1) => {
				...
				await finergy.model.set_value(row.doctype, row.name, item_data);
				return value;
			};

			if (this.prompt_qty) {
				finergy.prompt(__("Please enter quantity for item {0}", [item_code]), ({value}) => {
	...
	async set_serial_no(row, serial_no) {
		if (serial_no && finergy.meta.has_field(row.doctype, this.serial_no_field)) {
			...
			await finergy.model.set_value(row.doctype, row.name, this.serial_no_field, new_serial_nos);
		}
	}

	async set_barcode_uom(row, uom) {
		if (uom && finergy.meta.has_field(row.doctype, this.uom_field)) {
			await finergy.model.set_value(row.doctype, row.name, this.uom_field, uom);
		}
	}

	async set_batch_no(row, batch_no) {
		if (batch_no && finergy.meta.has_field(row.doctype, this.batch_no_field)) {
			await finergy.model.set_value(row.doctype, row.name, this.batch_no_field, batch_no);
		}
	}

	async set_barcode(row, barcode) {
		if (barcode && finergy.meta.has_field(row.doctype, this.barcode_field)) {
			await finergy.model.set_value(row.doctype, row.name, this.barcode_field, barcode);
		}
	}
	...
	get_row_to_modify_on_scan(item_code, batch_no, uom) {
		...
		let is_batch_no_scan = batch_no && finergy.meta.has_field(cur_grid.doctype, this.batch_no_field);
		let check_max_qty = this.max_qty_field && finergy.meta.has_field(cur_grid.doctype, this.max_qty_field);
	...
	show_alert(msg, indicator, duration=3) {
		finergy.show_alert({message: msg, indicator: indicator}, duration);
	}
};
