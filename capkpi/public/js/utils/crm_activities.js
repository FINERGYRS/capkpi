capkpi.utils.CRMActivities = class CRMActivities {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	refresh() {
		...
		if (!$(this.all_activities_wrapper).find('.form-footer').length) {
			...
			// remove finergy-control class to avoid absolute position for action-btn
			$(this.all_activities_wrapper).removeClass('finergy-control');
		...
		// open activities
		finergy.call({
			method: "capkpi.crm.utils.get_open_activities",
			args: {
			...
			callback: (r) => {
				if (!r.exc) {
					var activities_html = finergy.render_template('crm_activities', {
	...
	create_task () {
		...
		let _create_task = () => {
			...
			let composer = new finergy.views.InteractionComposer(args);
	...
	async update_status (input_field, doctype) {
		...
		if (completed) {
			await finergy.db.set_value(doctype, docname, "status", "Closed");
			this.refresh();
		}
	}
};

capkpi.utils.CRMNotes = class CRMNotes {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	refresh() {
		...
		let notes_html = finergy.render_template(
			'crm_notes',
			{
				notes: notes
			}
