...
capkpi.SMSManager = function SMSManager(doc) {
	var me = this;
	this.setup = function() {
		var default_msg = {
			'Lead'				: '',
	...
	this.get_contact_number = function(contact, ref_doctype, ref_name) {
		finergy.call({
			method: "finergy.core.doctype.sms_settings.sms_settings.get_contact_number",
			args: {
			...
			callback: function(r) {
				if(r.exc) { finergy.msgprint(r.exc); return; }
				me.number = r.message;
				me.show_dialog();
			}
		});
