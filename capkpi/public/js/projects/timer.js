finergy.provide("capkpi.timesheet");

capkpi.timesheet.timer = function(frm, row, timestamp=0) {
	...
	capkpi.timesheet.control_timer(frm, dialog, row, timestamp);
	dialog.show();
};

capkpi.timesheet.control_timer = function(frm, dialog, row, timestamp=0) {
	...
	$btn_start.click(function(e) {
		if (!initialised) {
			...
			row = finergy.model.add_child(frm.doc, "Timesheet Detail", "time_logs");
			row.activity_type = args.activity_type;
			row.from_time = finergy.datetime.get_datetime_as_string();
			...
			if(row.expected_hours) {
				...
				row.to_time = d.format(finergy.defaultDatetimeFormat);
			}
	...
	$btn_complete.click(function() {
		...
		grid_row.doc.to_time = finergy.datetime.now_datetime();
		grid_row.refresh();
		frm.save();
		reset();
		dialog.hide();
	...
	function updateStopwatch(increment) {
		...
		if(cur_dialog && cur_dialog.get_value('expected_hours') > 0) {
			if(flag && (currentIncrement >= (cur_dialog.get_value('expected_hours') * 3600))) {
				finergy.utils.play_sound("alert");
				finergy.msgprint(__("Timer exceeded the given hours."));
				flag = false;
			}
		}
