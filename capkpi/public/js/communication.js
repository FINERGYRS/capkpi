finergy.ui.form.on("Communication", {
	...
	setup_custom_buttons: (frm) => {
		...
		if(frm.doc.reference_doctype !== "Issue") {
			frm.add_custom_button(__("Issue"), () => {
				finergy.confirm(__(confirm_msg, [__("Issue")]), () => {
		...
		if(!in_list(["Lead", "Opportunity"], frm.doc.reference_doctype)) {
			frm.add_custom_button(__("Lead"), () => {
				finergy.confirm(__(confirm_msg, [__("Lead")]), () => {
					frm.trigger('make_lead_from_communication');
				})
			}, __('Create'));

			frm.add_custom_button(__("Opportunity"), () => {
				finergy.confirm(__(confirm_msg, [__("Opportunity")]), () => {
	...
	make_lead_from_communication: (frm) => {
		return finergy.call({
			method: "capkpi.crm.doctype.lead.lead.make_lead_from_communication",
			args: {
	...
	make_issue_from_communication: (frm) => {
		return finergy.call({
			method: "capkpi.support.doctype.issue.issue.make_issue_from_communication",
			args: {
				communication: frm.doc.name
			},
			freeze: true,
	...
	make_opportunity_from_communication: (frm) => {
		...
		finergy.prompt(fields, data => {
			finergy.call({
				method: "capkpi.crm.doctype.opportunity.opportunity.make_opportunity_from_communication",
				args: {
					communication: frm.doc.name,
