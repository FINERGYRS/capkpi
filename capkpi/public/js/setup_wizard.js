finergy.provide("capkpi.setup");

finergy.pages['setup-wizard'].on_page_load = function(wrapper) {
	if(finergy.sys_defaults.company) {
		finergy.set_route("desk");
		return;
	}
};

finergy.setup.on("before_load", function () {
	capkpi.setup.slides_settings.map(finergy.setup.add_slide);
});
...
capkpi.setup.slides_settings = [
	{
		...
		validate: function () {
			if (this.values.domains.length === 0) {
				finergy.msgprint(__("Please select at least one domain."));
				return false;
			}
			finergy.setup.domains = this.values.domains;
			return true;
		},
	},

	{
		...
		fields: [
			...
			},
			{
				...
				label: finergy.setup.domains.includes('Education') ?
		...
		bind_events: function (slide) {
			...
			}).val(finergy.boot.sysdefaults.company_name || "").trigger("change");

			slide.get_input("company_abbr").on("change", function () {
				if (slide.get_input("company_abbr").val().length > 5) {
					finergy.msgprint(__("Company Abbreviation cannot have more than 5 characters"));
		...
		validate: function() {
			if ((this.values.company_name || "").toLowerCase() == "company") {
				finergy.msgprint(__("Company Name cannot be Company"));
				return false;
			}
	...
	},
	{
		...
		icon: "fa fa-building",
		// help: finergy.setup.domains.includes('Education') ?
		...
		fields: [
			{
				...
				fieldtype: 'Data',
				placeholder: finergy.setup.domains.includes('Education') ?
		...
		validate: function () {
			...
			if(me.values.bank_account) {
				finergy.call({
					async: false,
					method: "capkpi.accounts.doctype.account.chart_of_accounts.chart_of_accounts.validate_bank_account",
					args: {
		...
		validate_fy_dates: function() {
			...
			if (invalid || start_greater_than_end) {
				finergy.msgprint(__("Please enter valid Financial Year Start and End Dates"));
				return false;
			}
		...
		set_fy_dates: function (slide) {
			var country = finergy.wizard.values.country;

			if (country) {
				var fy = capkpi.setup.fiscal_years[country];
				...
				if (year_start_date > finergy.datetime.get_today()) {
					next_year = current_year;
					current_year -= 1;
				}
		...
		bind_events: function (slide) {
			...
			slide.get_input("fy_start_date").on("change", function () {
				...
				var year_end_date =
					finergy.datetime.add_days(finergy.datetime.add_months(start_date, 12), -1);
		...
		charts_modal: function(slide, chart_template) {
			...
			let coa_tree = new finergy.ui.Tree({
				parent: $(dialog.body),
				label: parent,
				expandable: true,
				method: 'capkpi.accounts.utils.get_coa',
				args: {
			...
			const buttons = $(form_container).find('.finergy-control');
