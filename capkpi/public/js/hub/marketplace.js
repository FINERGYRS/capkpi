...
finergy.provide('finergy.route');

finergy.utils.make_event_emitter(finergy.route);
finergy.utils.make_event_emitter(capkpi.hub);

capkpi.hub.Marketplace = class Marketplace {
	constructor({ parent }) {
		this.$parent = $(parent);
	...
	make_body() {
		...
		if (!hub.is_server) {
			capkpi.hub.on('seller-registered', () => {
	...
	show_register_dialog() {
		if(finergy.session.user === 'Administrator') {
			finergy.msgprint(__('You need to be a user other than Administrator with System Manager and Item Manager roles to register on Marketplace.'));
			return;
		}

		if (!is_subset(['System Manager', 'Item Manager'], finergy.user_roles)) {
			finergy.msgprint(__('You need to be a user with System Manager and Item Manager roles to register on Marketplace.'));
			return;
		}
	...
	register_marketplace({company, company_description}) {
		finergy.call({
		    method: 'capkpi.hub_node.api.register_marketplace',
		    args: {
				company,
				company_description
			}
			...
			if (r.message && r.message.ok) {
				...
				this.update_hub_settings()
					.then(() => {
						finergy.set_route('marketplace', 'publish');
						capkpi.hub.trigger('seller-registered');
					});
			}
		});
	}
	...
	show_add_user_dialog() {
		if (!is_subset(['System Manager', 'Item Manager'], finergy.user_roles)) {
			finergy.msgprint(__('You need to be a user with System Manager and Item Manager roles to add users to Marketplace.'));
			return;
		}
			...
			.then(r => {
				...
				const d = new finergy.ui.Dialog({
					...
					primary_action({ users }) {
						...
						finergy.call('capkpi.hub_node.api.register_users', {
						...
						.then(r => {
							...
							if (r.message && r.message.length) {
								finergy.show_alert(__('Added {0} users', [r.message.length]));
							}
						});
					}
				});
	...
	get_unregistered_users() {
		return finergy.call('capkpi.hub_node.api.get_unregistered_users')
	}
...
Object.assign(hub, {
	...
	is_user_registered() {
		return this.is_seller_registered() && hub.settings.users
			.filter(hub_user => hub_user.user === finergy.session.user)
			.length === 1;
	},
	...
	get_settings() {
		...
		return finergy.db.get_doc('Marketplace Settings');
	}
});
