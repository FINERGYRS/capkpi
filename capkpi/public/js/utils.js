...
$.extend(capkpi, {
	get_currency: function(company) {
		...
		if(company)
			return finergy.get_doc(":Company", company).default_currency || finergy.boot.sysdefaults.currency;
		else
			return finergy.boot.sysdefaults.currency;
	},

	get_presentation_currency_list: () => {
		const docs = finergy.boot.docs;
	...
	hide_company: function() {
		if(cur_frm.fields_dict.company) {
			...
			} else if(capkpi.last_selected_company) {
				if(!cur_frm.doc.company) cur_frm.set_value("company", capkpi.last_selected_company);
			}
		}
	},
	...
	is_perpetual_inventory_enabled: function(company) {
		if(company) {
			return finergy.get_doc(":Company", company).enable_perpetual_inventory
		}
	},

	stale_rate_allowed: () => {
		return cint(finergy.boot.sysdefaults.allow_stale);
	},
	...
	setup_serial_or_batch_no: function() {
		...
		finergy.model.get_value('Item', {'name': grid_row.doc.item_code},
	...
	route_to_adjustment_jv: (args) => {
		finergy.model.with_doctype('Journal Entry', () => {
			// route to adjustment Journal Entry to handle Account Balance and Stock Value mismatch
			let journal_entry = finergy.model.get_new_doc('Journal Entry');

			args.accounts.forEach((je_account) => {
				let child_row = finergy.model.add_child(journal_entry, "accounts");
			...
			finergy.set_route('Form','Journal Entry', journal_entry.name);
		});
	},

	route_to_pending_reposts: (args) => {
		finergy.set_route('List', 'Repost Item Valuation', args);
	},
});
...
$.extend(capkpi.utils, {
		...
		if(frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			...
			if(company_wise_info.length > 1) {
				company_wise_info.forEach(function(info) {
					capkpi.utils.add_indicator_for_multicompany(frm, info);
				});
	...
	get_terms: function(tc_name, doc, callback) {
		if(tc_name) {
			return finergy.call({
				method: 'capkpi.setup.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions',
				args: {
	...
	make_bank_account: function(doctype, docname) {
		finergy.call({
			method: "capkpi.accounts.doctype.bank_account.bank_account.make_bank_account",
			args: {
				doctype: doctype,
				docname: docname
			},
	...
	add_dimensions: function(report_name, index) {
		...
		finergy.call({
			method: "capkpi.accounts.doctype.accounting_dimension.accounting_dimension.get_dimensions",
			callback: function(r) {
					...
					if (!found) {
						filters.splice(index, 0, {
							...
							get_data: function(txt) {
								return finergy.db.get_link_options(dimension["document_type"], txt);
							},
						});
					}
				});
	...
	add_inventory_dimensions: function(report_name, index) {
		...
		finergy.call({
			method: "capkpi.stock.doctype.inventory_dimension.inventory_dimension.get_inventory_dimensions",
			callback: function(r) {
					...
					r.message.forEach((dimension) => {
						...
						if (!found) {
							filters.splice(index, 0, {
								...
								"fieldtype": "MultiSelectList",
								get_data: function(txt) {
									return finergy.db.get_link_options(dimension["doctype"], txt);
								},
							});
						}
					});
	...
	make_pricing_rule: function(doctype, docname) {
		finergy.call({
			method: "capkpi.accounts.doctype.pricing_rule.pricing_rule.make_pricing_rule",
			args: {
				doctype: doctype,
				docname: docname
			},
	...
	**/
	...
	get_tree_options: function(option) {
		...
		let unscrub_option = finergy.model.unscrub(option);
		let user_permission = finergy.defaults.get_user_permissions();
		let options;
	...
	get_tree_default: function(option) {
		...
		if(options.includes(finergy.defaults.get_default(option))) {
			return finergy.defaults.get_default(option);
		} else {
			return options[0];
		}
	},
	overrides_parent_value_in_all_rows: function(doc, dt, dn, table_fieldname, fieldname, parent_fieldname) {
		if (doc[parent_fieldname]) {
			...
			finergy.refresh_field(table_fieldname);
		}
	},
	create_new_doc: function (doctype, update_fields) {
		finergy.model.with_doctype(doctype, function() {
			var new_doc = finergy.model.get_new_doc(doctype);
...
capkpi.utils.select_alternate_items = function(opts) {
	const frm = opts.frm;
	...
	const dialog = new finergy.ui.Dialog({
				...
				fields: [{
					...
					onchange: function() {
						...
						if (item_code && warehouse) {
							finergy.call({
								method: "capkpi.stock.utils.get_latest_stock_qty",
								args: {
									item_code: item_code,
									warehouse: warehouse
								},
					...
					get_query: (e) => {
						return {
							query: "capkpi.stock.doctype.item_alternative.item_alternative.get_alternative_items",
							filters: {
		...
		primary_action: function() {
			...
			alternative_items.forEach(d => {
				let row = finergy.get_doc(opts.child_doctype, d.docname);
				let qty = null;
				...
				finergy.model.set_value(row.doctype, row.name, 'qty', qty);
				finergy.model.set_value(row.doctype, row.name, opts.original_item_field, d.item_code);
...
capkpi.utils.update_child_items = function(opts) {
	const frm = opts.frm;
	const cannot_add_row = (typeof opts.cannot_add_row === 'undefined') ? true : opts.cannot_add_row;
	const child_docname = (typeof opts.cannot_add_row === 'undefined') ? "items" : opts.child_docname;
	const child_meta = finergy.get_meta(`${frm.doc.doctype} Item`);
	...
	const dialog = new finergy.ui.Dialog({
		...
		primary_action: function() {
			...
			finergy.call({
				method: 'capkpi.controllers.accounts_controller.update_child_qty_rate',
				freeze: true,
				args: {
...
capkpi.utils.map_current_doc = function(opts) {
	function _map() {
		if($.isArray(cur_frm.doc.items) && cur_frm.doc.items.length > 0) {
			...
			var items_doctype = finergy.meta.get_docfield(cur_frm.doctype, 'items').options;

			// find the link fieldname from items table for the given
			// source_doctype
			var link_fieldname = null;
			finergy.get_meta(items_doctype).fields.forEach(function(d) {
			...
			if(already_set) {
				opts.source_name.forEach(function(src) {
					finergy.model.with_doc(opts.source_doctype, src, function(r) {
						var source_doc = finergy.model.get_doc(opts.source_doctype, src);
					...
					if(already_set) {
						finergy.msgprint(__("You have already selected items from {0} {1}",
		...
		return finergy.call({
			...
			type: "POST",
			method: 'finergy.model.mapper.map_docs',
			args: {
				"method": opts.method,
	...
	if (opts.source_doctype) {
		const d = new finergy.ui.form.MultiSelectDialog({
			...
			action: function(selections, args) {
				...
				if (values.length === 0) {
					finergy.msgprint(__("Please select {0}", [opts.source_doctype]))
					return;
				}
...
finergy.form.link_formatters['Item'] = function(value, doc) {
...
finergy.form.link_formatters['Employee'] = function(value, doc) {
...
finergy.form.link_formatters['Project'] = function(value, doc) {
...
$(document).on('app_ready', function() {
	if(!finergy.datetime.is_timezone_same()) {
		$.each(["Stock Reconciliation", "Stock Entry", "Stock Ledger Entry",
			...
			finergy.ui.form.on(d, "onload", function(frm) {
				cur_frm.set_df_property("posting_time", "description",
					finergy.sys_defaults.time_zone);
			});
		});
	}
});
...
$(document).on('app_ready', function() {
	finergy.call({
		method: 'capkpi.support.doctype.service_level_agreement.service_level_agreement.get_sla_doctypes',
		callback: function(r) {
			if (!r.message)
				return;

			$.each(r.message, function(_i, d) {
				finergy.ui.form.on(d, {
					onload: function(frm) {
						if (!frm.doc.service_level_agreement)
							return;

						finergy.call({
							method: 'capkpi.support.doctype.service_level_agreement.service_level_agreement.get_service_level_agreement_filters',
							args: {
