class Quiz {
	...
	get_quiz() {
		finergy.call('capkpi.education.utils.get_quiz', {
			quiz_name: this.name,
			course: this.course
		}).then(res => {
	...
	submit() {
		...
		this.disable()
		finergy.call('capkpi.education.utils.evaluate_quiz', {
			quiz_name: this.name,
			...
			if (!res.message) {
				finergy.throw(__("Something went wrong while evaluating the quiz."))
			}
