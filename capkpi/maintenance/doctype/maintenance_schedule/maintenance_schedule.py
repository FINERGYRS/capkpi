...
from finergy.utils import add_days, cint, cstr, date_diff, formatdate, getdate

from capkpi.setup.doctype.employee.employee import get_holiday_list_for_employee
from capkpi.stock.doctype.serial_no.serial_no import get_serial_nos
from capkpi.stock.utils import get_valid_serial_nos
from capkpi.utilities.transaction_base import TransactionBase, delete_events
...
class MaintenanceSchedule(TransactionBase):
	...
	def on_submit(self):
		...
		for d in self.get("items"):
			...
			if d.sales_person not in email_map:
				sp = finergy.get_doc("Sales Person", d.sales_person)
				try:
			...
			for key in scheduled_date:
				description = finergy._("Reference: {0}, Item Code: {1} and Customer: {2}").format(
	...
	def validate_schedule_date_for_holiday_list(self, schedule_date, sales_person):
		...
		employee = finergy.db.get_value("Sales Person", sales_person, "employee")
		if employee:
			holiday_list = get_holiday_list_for_employee(employee)
		else:
			holiday_list = finergy.get_cached_value("Company", self.company, "default_holiday_list")

		holidays = finergy.db.sql_list(
	...
	def validate_serial_no(self, item_code, serial_nos, amc_start_date):
		for serial_no in serial_nos:
			...
			if not sr_details:
				finergy.throw(_("Serial No {0} not found").format(serial_no))

			if sr_details.get("item_code") != item_code:
				finergy.throw(
					_("Serial No {0} does not belong to Item {1}").format(
						finergy.bold(serial_no), finergy.bold(item_code)
					),
					title=_("Invalid"),
				)
