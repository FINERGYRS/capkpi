...
finergy.ui.form.on('Maintenance Schedule', {
	setup: function (frm) {
	...
	generate_schedule: function (frm) {
		if (frm.is_new()) {
			finergy.msgprint(__('Please save first'));
		} else {
...
capkpi.maintenance.MaintenanceSchedule = class MaintenanceSchedule extends finergy.ui.form.Controller {
	refresh() {
...
extend_cscript(cur_frm.cscript, new capkpi.maintenance.MaintenanceSchedule({frm: cur_frm}));
