...
def create_stripe_subscription(gateway_controller, data):
	stripe_settings = finergy.get_doc("Stripe Settings", gateway_controller)
	stripe_settings.data = finergy._dict(data)
	...
	try:
		...
		stripe_settings.payment_plans = finergy.get_doc(
...
def create_subscription_on_stripe(stripe_settings):
	...
	for payment_plan in stripe_settings.payment_plans:
		plan = finergy.db.get_value("Subscription Plan", payment_plan.plan, "product_price_id")
	...
	try:
		...
		if subscription.status == "active":
			...
			finergy.log_error("Subscription N°: " + subscription.id, "Stripe Payment not completed")
	except Exception:
		stripe_settings.integration_request.db_set("status", "Failed", update_modified=False)
		finergy.log_error(finergy.get_traceback())
