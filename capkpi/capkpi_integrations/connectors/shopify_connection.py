...
from finergy.utils import cint, cstr, flt, get_datetime, get_request_session, getdate, nowdate

from capkpi import get_company_currency
from capkpi.capkpi_integrations.doctype.shopify_log.shopify_log import (
	dump_request_data,
	make_shopify_log,
)
...
from capkpi.capkpi_integrations.doctype.shopify_settings.sync_customer import create_customer
from capkpi.capkpi_integrations.doctype.shopify_settings.sync_product import (
	sync_item_from_shopify,
)
from capkpi.capkpi_integrations.utils import validate_webhooks_request
from capkpi.selling.doctype.sales_order.sales_order import make_delivery_note, make_sales_invoice
...
def store_request_data(order=None, event=None):
	if finergy.request:
		...
		event = finergy.request.headers.get("X-Shopify-Topic")
...
def sync_sales_order(order, request_id=None, old_order_sync=False):
	...
	finergy.flags.request_id = request_id

	if not finergy.db.get_value("Sales Order", filters={"shopify_order_id": cstr(order["id"])}):
		try:
...
def get_sales_order(shopify_order_id):
	sales_order = finergy.db.get_value("Sales Order", filters={"shopify_order_id": shopify_order_id})
	if sales_order:
		so = finergy.get_doc("Sales Order", sales_order)
		return so
...
def validate_customer(order, shopify_settings):
	...
	if customer_id:
		if not finergy.db.get_value("Customer", {"shopify_customer_id": customer_id}, "name"):
			create_customer(order.get("customer"), shopify_settings)


def validate_item(order, shopify_settings):
	for item in order.get("line_items"):
		if item.get("product_id") and not finergy.db.get_value(
...
def create_sales_order(shopify_order, shopify_settings, company=None):
	product_not_exists = []
	customer = finergy.db.get_value(
		"Customer", {"shopify_customer_id": shopify_order.get("customer", {}).get("id")}, "name"
	)
	so = finergy.db.get_value("Sales Order", {"shopify_order_id": shopify_order.get("id")}, "name")
	...
	if not so:
		...
		so = finergy.get_doc("Sales Order", so)
...
def create_sales_invoice(shopify_order, shopify_settings, so, old_order_sync=False):
	if (
		not finergy.db.get_value("Sales Invoice", {"shopify_order_id": shopify_order.get("id")}, "name")
		and so.docstatus == 1
...
def get_item_code(shopify_item):
	item_code = finergy.db.get_value(
	...
	if not item_code:
		item_code = finergy.db.get_value("Item", {"item_name": shopify_item.get("title")}, "item_code")
...
def get_tax_account_head(tax):
	...
	tax_account = finergy.db.get_value(
		"Shopify Tax Account", {"parent": "Shopify Settings", "shopify_tax": tax_title}, "tax_account"
	)

	if not tax_account:
		finergy.throw(_("Tax Account not specified for Shopify Tax {0}").format(tax.get("title")))
