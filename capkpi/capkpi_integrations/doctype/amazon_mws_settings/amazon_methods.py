# Copyright (c) 2018, Finergy Reporting Solutions SAS (Finergy) and contributors
...
import capkpi.capkpi_integrations.doctype.amazon_mws_settings.amazon_mws_api as mws
...
def get_products_details():
	...
	mws_settings = finergy.get_doc("Amazon MWS Settings")
	...
	for marketplace in market_place_list:
		...
		if report_id:
			...
			# Get ASIN Codes
			string_io = StringIO(finergy.safe_decode(listings_response.original))
...
def call_mws_method(mws_method, *args, **kwargs):
	...
	for x in range(0, max_retries):
		try:
			...
			finergy.log_error(message=e, title=f'Method "{mws_method.__name__}" failed')
			time.sleep(delay)
			continue
	...
	finergy.throw(_("Sync has been temporarily disabled because maximum retries have been exceeded"))


def create_item_code(amazon_item_json, sku):
	if finergy.db.get_value("Item", sku):
		return
	...
	item_group = finergy.db.get_value("Item Group", filters={"item_group_name": temp_item_group})

	if not item_group:
		igroup = finergy.new_doc("Item Group")
...
def create_item_price(amazon_item_json, item_code):
	...
	item_price.price_list = finergy.db.get_value(
...
def get_orders(after_date):
	try:
		...
		finergy.log_error(title="get_orders", message=e)
...
def create_sales_order(order_json, after_date):
	...
	if not so:
		...
		so = finergy.get_doc(
			{
				...
				"items": items,
				"company": finergy.db.get_value("Amazon MWS Settings", "Amazon MWS Settings", "company"),
			}
		)
		...
		try:
			...
			finergy.log_error(message=traceback.format_exc(), title="Create Sales Order")
...
def create_customer(order_json):
	...
	existing_customer_name = finergy.db.get_value(
	...
	if existing_customer_name:
		...
		existing_contacts = finergy.get_list("Contact", filters)

		if existing_contacts:
			pass
		else:
			new_contact = finergy.new_doc("Contact")
		...
		mws_customer_settings = finergy.get_doc("Amazon MWS Settings")
		new_customer = finergy.new_doc("Customer")
...
def create_address(amazon_order_item_json, customer_name):
	...
	existing_address = finergy.get_list("Address", filters)

	if not ("ShippingAddress" in amazon_order_item_json):
		return None
	else:
		make_address = finergy.new_doc("Address")
		...
		for address in existing_address:
			address_doc = finergy.get_doc("Address", address["name"])
			if (
...
def get_order_items(market_place_order_id):
	...
	warehouse = finergy.db.get_value("Amazon MWS Settings", "Amazon MWS Settings", "warehouse")
...
def get_item_code(order_item):
	...
	item_code = finergy.db.get_value("Item", {"item_code": sku}, "item_code")
	if item_code:
		return item_code
...
def get_account(name):
	existing_account = finergy.db.get_value("Account", {"account_name": "Amazon {0}".format(name)})
	account_name = existing_account
	mws_settings = finergy.get_doc("Amazon MWS Settings")

	if not existing_account:
		try:
			new_account = finergy.new_doc("Account")
			...
			finergy.log_error(message=e, title="Create Account")
