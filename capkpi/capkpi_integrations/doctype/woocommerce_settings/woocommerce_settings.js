...
finergy.ui.form.on('Woocommerce Settings', {
	refresh (frm) {
		...
		frm.set_query("tax_account", ()=>{
			return {
				"filters": {
					"company": finergy.defaults.get_default("company"),
					"is_group": 0
				}
			};
		});
		...
		frm.add_custom_button(__('Generate Secret'), () => {
			finergy.confirm(
				...
				() => {
					finergy.call({
						type:"POST",
						method:"capkpi.capkpi_integrations.doctype.woocommerce_settings.woocommerce_settings.generate_secret",
					}).done(() => {
						frm.reload_doc();
					}).fail(() => {
						finergy.msgprint(__("Could not generate Secret"));
					});
				}
			);
		});
...
finergy.ui.form.on("Woocommerce Settings", "onload", function () {
	finergy.call({
		method: "capkpi.capkpi_integrations.doctype.woocommerce_settings.woocommerce_settings.get_series",
		callback: function (r) {
