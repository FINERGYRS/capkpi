...
finergy.ui.form.on('QuickBooks Migrator', {
	...
	onload: function(frm) {
		...
		var domain = finergy.urllib.get_base_url()
		var redirect_url = `${domain}/api/method/capkpi.capkpi_integrations.doctype.quickbooks_migrator.quickbooks_migrator.callback`
		...
		frm.trigger("set_indicator")
 		finergy.realtime.on("quickbooks_progress_update", function (data) {
	...
	refresh: function(frm) {
		...
		if (frm.doc.access_token) {
			...
			// All we need now is a Company from capkpi
