...
from finergy.utils import cint, cstr, get_request_session
...
def create_attribute(shopify_item):
	...
	for attr in shopify_item.get("options"):
		if not finergy.db.get_value("Item Attribute", attr.get("name"), "name"):
			finergy.get_doc(
				{
			...
			item_attr = finergy.get_doc("Item Attribute", attr.get("name"))
...
def create_item(shopify_item, warehouse, has_variant=0, attributes=None, variant_of=None):
	...
	if not is_item_exists(item_dict, attributes, variant_of=variant_of):
		...
		if not item_details:
			new_item = finergy.get_doc(item_dict)
...
def create_item_variants(shopify_item, warehouse, attributes, shopify_variants_attr_list):
	template_item = finergy.db.get_value(
		"Item",
...
def get_attribute_value(variant_attr_val, attribute):
	attribute_value = finergy.db.sql(
...
def get_item_group(product_type=None):
	import finergy.utils.nestedset

	parent_item_group = finergy.utils.nestedset.get_root_of("Item Group")

	if product_type:
		if not finergy.db.get_value("Item Group", product_type, "name"):
			item_group = finergy.get_doc(
				{
					"doctype": "Item Group",
...
def add_to_price_list(item, name):
	shopify_settings = finergy.db.get_value(
		"Shopify Settings", None, ["price_list", "update_price_in_capkpi_price_list"], as_dict=1
	)
	if not shopify_settings.update_price_in_capkpi_price_list:
		return

	item_price_name = finergy.db.get_value(
	...
	if not item_price_name:
		...
		item_rate = finergy.get_doc("Item Price", item_price_name)
...
def get_supplier_group():
	supplier_group = finergy.db.get_value("Supplier Group", _("Shopify Supplier"))
	if not supplier_group:
		supplier_group = finergy.get_doc(
...
def get_item_details(shopify_item):
	...
	item_details = finergy.db.get_value(
		"Item",
		{"shopify_product_id": shopify_item.get("id")},
...
def is_item_exists(shopify_item, attributes=None, variant_of=None):
	if variant_of:
		...
		name = finergy.db.get_value("Item", {"item_name": shopify_item.get("item_name")})

	if name:
		item = finergy.get_doc("Item", name)
		...
		if item.shopify_product_id and attributes and attributes[0].get("attribute_value"):
			if not variant_of:
				variant_of = finergy.db.get_value(
			...
			if parent:
				variant = finergy.get_doc("Item", parent[0][0])
