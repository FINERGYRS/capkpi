...
from capkpi.capkpi_integrations.connectors.shopify_connection import create_order
...
from capkpi.capkpi_integrations.doctype.shopify_settings.sync_product import make_item
...
class ShopifySettings(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		cls.allow_negative_stock = cint(
			finergy.db.get_value("Stock Settings", None, "allow_negative_stock")
		)
		...
		finergy.reload_doctype("Customer")
		finergy.reload_doctype("Sales Order")
		finergy.reload_doctype("Delivery Note")
		finergy.reload_doctype("Sales Invoice")
	...
	def tearDownClass(cls):
		if not cls.allow_negative_stock:
			finergy.db.set_value("Stock Settings", None, "allow_negative_stock", 0)
	...
	def test_order(self):
		...
		sales_order_customer_id = finergy.get_value(
		...
		# Check delivery note
		delivery_note_count = finergy.db.sql(
