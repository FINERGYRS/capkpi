...
finergy.ui.form.on('Mpesa Settings', {
	onload_post_render: function(frm) {
		frm.events.setup_account_balance_html(frm);
	},

	refresh: function(frm) {
		finergy.realtime.on("refresh_mpesa_dashboard", function(){
			frm.reload_doc();
			frm.events.setup_account_balance_html(frm);
		});
	},
	...
	get_account_balance: function(frm) {
		if (!frm.doc.initiator_name && !frm.doc.security_credential) {
			finergy.throw(__("Please set the initiator name and the security credential"));
		}
		finergy.call({
	...
	setup_account_balance_html: function(frm) {
		...
		frm.dashboard.add_section(
			finergy.render_template('account_balance', {
