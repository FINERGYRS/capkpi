...
def create_custom_pos_fields():
	...
	if not finergy.get_meta("POS Invoice").has_field("request_for_payment"):
...
def create_pos_settings(record_dict):
	for record in record_dict:
		if finergy.db.exists("POS Field", {"fieldname": record.get("fieldname")}):
			continue
		finergy.get_doc(record).insert()
