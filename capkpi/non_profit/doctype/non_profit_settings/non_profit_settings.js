...
finergy.ui.form.on("Non Profit Settings", {
	refresh: function(frm) {
		...
		frm.set_intro(__("You can learn more about memberships in the manual. ") + `<a href='${docs_url}'>${__('CapKPI Docs')}</a>`, true);
	...
	setup_buttons_for_membership: function(frm) {
		...
		if (frm.doc.membership_webhook_secret) {
			...
			frm.add_custom_button(__("Copy Webhook URL"), () => {
				finergy.utils.copy_to_clipboard(`https://${finergy.boot.sitename}/api/method/capkpi.non_profit.doctype.membership.membership.trigger_razorpay_subscription`);
			}, __("Memberships"));
	...
	setup_buttons_for_donation: function(frm) {
		...
		if (frm.doc.donation_webhook_secret) {
			...
			frm.add_custom_button(__("Copy Webhook URL"), () => {
				finergy.utils.copy_to_clipboard(`https://${finergy.boot.sitename}/api/method/capkpi.non_profit.doctype.donation.donation.capture_razorpay_donations`);
			}, __("Donations"));
