...
from capkpi.non_profit.doctype.membership_type.membership_type import get_membership_type
...
class Member(Document):
	...
	def setup_subscription(self):
		non_profit_settings = finergy.get_doc("Non Profit Settings")
		...
		plan_id = finergy.get_value("Membership Type", self.membership_type, "razorpay_plan_id")

		if not plan_id:
			finergy.throw(_("Please setup Razorpay Plan ID"))
	...
	def make_customer_and_link(self):
		if self.customer:
			finergy.msgprint(_("A customer is already linked to this Member"))

		customer = create_customer(
			finergy._dict({"fullname": self.member_name, "email": self.email_id, "phone": None})
		)

		self.customer = customer
		self.save()
		finergy.msgprint(_("Customer {0} has been created succesfully.").format(self.customer))


def get_or_create_member(user_details):
	member_list = finergy.get_all(
...
def create_member(user_details):
	user_details = finergy._dict(user_details)
	member = finergy.new_doc("Member")
	member.update(
		{
...
def create_customer(user_details, member=None):
	...
	customer.customer_group = finergy.db.get_single_value("Selling Settings", "customer_group")
	customer.territory = finergy.db.get_single_value("Selling Settings", "territory")
	customer.flags.ignore_mandatory = True
	customer.insert(ignore_permissions=True)

	try:
		finergy.db.savepoint("contact_creation")
		...
		finergy.db.rollback(save_point="contact_creation")
		finergy.log_error(finergy.get_traceback(), _("Contact Creation Failed"))
		pass
...
def register_member(fullname, email, rzpay_plan_id, subscription_id, pan=None, mobile=None):
	...
	member = finergy.db.exists("Member", {"email": email, "subscription_id": subscription_id})
	if member:
		return member
	else:
