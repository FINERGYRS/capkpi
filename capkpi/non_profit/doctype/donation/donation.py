...
from finergy.utils import flt, get_link_to_form, getdate

from capkpi.non_profit.doctype.membership.membership import verify_signature


class Donation(Document):
	def validate(self):
		if not self.donor or not finergy.db.exists("Donor", self.donor):
			# for web forms
	...
	def create_donor_for_website_user(self):
		donor_name = finergy.get_value("Donor", dict(email=finergy.session.user))

		if not donor_name:
			user = finergy.get_doc("User", finergy.session.user)
			donor = finergy.get_doc(
				dict(
					doctype="Donor",
...
def capture_razorpay_donations(*args, **kwargs):
	...
	try:
		...
		log = finergy.log_error(e, "Donation Webhook Verification Error")
		notify_failure(log)
	...
	try:
		...
		log = finergy.log_error(message, _("Error creating donation entry for {0}").format(donor.name))
		notify_failure(log)
		return {"status": "Failed", "reason": e}
...
def create_razorpay_donation(donor, payment):
	if not finergy.db.exists("Mode of Payment", payment.method):
		create_mode_of_payment(payment.method)

	company = get_company_for_donations()
	donation = finergy.get_doc(
		{
			"doctype": "Donation",
			"company": company,
			"donor": donor.name,
...
def get_donor(email):
	donors = finergy.get_all("Donor", filters={"email": email}, order_by="creation desc")

	try:
		return finergy.get_doc("Donor", donors[0]["name"])
	except Exception:
		return None
...
def create_donor(payment):
	donor_details = finergy._dict(payment)
	donor_type = finergy.db.get_single_value("Non Profit Settings", "default_donor_type")

	donor = finergy.new_doc("Donor")
	donor.update(
		{
...
def get_company_for_donations():
	company = finergy.db.get_single_value("Non Profit Settings", "donation_company")
	if not company:
...
def create_mode_of_payment(method):
	finergy.get_doc({"doctype": "Mode of Payment", "mode_of_payment": method}).insert(
...
def notify_failure(log):
	try:
		...
		sendmail_to_system_managers(
			_("[Important] [CapKPI] Razorpay donation webhook failed, please check."), content
		)
	except Exception:
		pass
