...
def verify_request():
	woocommerce_settings = finergy.get_doc("Woocommerce Settings")
	sig = base64.b64encode(
		hmac.new(
			woocommerce_settings.secret.encode("utf8"), finergy.request.data, hashlib.sha256
		).digest()
	)

	if (
		finergy.request.data
		and not sig == finergy.get_request_header("X-Wc-Webhook-Signature", "").encode()
	):
		finergy.throw(_("Unverified Webhook Data"))
	finergy.set_user(woocommerce_settings.creation_user)
...
def order(*args, **kwargs):
	try:
		...
		error_message = (
			finergy.get_traceback() + "\n\n Request Data: \n" + json.loads(finergy.request.data).__str__()
		)
		finergy.log_error("WooCommerce Error", error_message)
		raise


def _order(*args, **kwargs):
	woocommerce_settings = finergy.get_doc("Woocommerce Settings")
	if finergy.flags.woocomm_test_order_data:
		order = finergy.flags.woocomm_test_order_data
		event = "created"

	elif finergy.request and finergy.request.data:
		verify_request()
		try:
			order = json.loads(finergy.request.data)
		except ValueError:
			# woocommerce returns 'webhook_id=value' for the first request which is not JSON
			order = finergy.request.data
		event = finergy.get_request_header("X-Wc-Webhook-Event")

	else:
		return "success"

	if event == "created":
		sys_lang = finergy.get_single("System Settings").language or "en"
...
def link_customer_and_address(raw_billing_data, raw_shipping_data, customer_name):
	...
	customer_exists = finergy.get_value("Customer", {"woocommerce_email": customer_woo_com_email})
	if not customer_exists:
		# Create Customer
		customer = finergy.new_doc("Customer")
	else:
		# Edit Customer
		customer = finergy.get_doc("Customer", {"woocommerce_email": customer_woo_com_email})
	...
	if customer_exists:
		finergy.rename_doc("Customer", old_name, customer_name)
		for address_type in (
			"Billing",
			"Shipping",
		):
...
def create_address(raw_data, customer, address_type):
	...
	address.country = finergy.get_value("Country", {"code": raw_data.get("country", "IN").lower()})
...
def rename_address(address, customer):
	...
	finergy.rename_doc("Address", old_address_title, new_address_title)
...
def link_items(items_list, woocommerce_settings, sys_lang):
	for item_data in items_list:
		...
		if not finergy.db.get_value("Item", {"woocommerce_id": item_woo_com_id}, "name"):
			# Create Item
...
def create_sales_order(order, woocommerce_settings, customer_name, sys_lang):
	new_sales_order = finergy.new_doc("Sales Order")
	...
	new_sales_order.delivery_date = finergy.utils.add_days(created_date[0], delivery_after)
...
def set_items_in_sales_order(new_sales_order, woocommerce_settings, order, sys_lang):
	company_abbr = finergy.db.get_value("Company", woocommerce_settings.company, "abbr")

	default_warehouse = _("Stores - {0}", sys_lang).format(company_abbr)
	if not finergy.db.exists("Warehouse", default_warehouse) and not woocommerce_settings.warehouse:
		finergy.throw(_("Please set Warehouse in Woocommerce Settings"))

	for item in order.get("line_items"):
		woocomm_item_id = item.get("product_id")
		found_item = finergy.get_doc("Item", {"woocommerce_id": cstr(woocomm_item_id)})
