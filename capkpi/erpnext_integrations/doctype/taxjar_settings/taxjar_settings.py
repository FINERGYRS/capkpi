...
from capkpi.capkpi_integrations.taxjar_integration import get_client
...
class TaxJarSettings(Document):
	def on_update(self):
		...
		fields_already_exist = finergy.db.exists(
			"Custom Field",
			{"dt": ("in", ["Item", "Sales Invoice Item"]), "fieldname": "product_tax_category"},
		)
		fields_hidden = finergy.get_value(
		...
		if TAXJAR_CREATE_TRANSACTIONS or TAXJAR_CALCULATE_TAX or TAXJAR_SANDBOX_MODE:
			if not fields_already_exist:
				...
				make_custom_fields()
				add_permissions()
				finergy.enqueue("capkpi.regional.united_states.setup.add_product_tax_categories", now=False)
	...
	def update_nexus_list(self):
		...
		new_nexus_list = [finergy._dict(address) for address in nexus]
...
def create_tax_categories(data):
	for d in data:
		if not finergy.db.exists("Product Tax Category", {"product_tax_code": d.get("product_tax_code")}):
			tax_category = finergy.new_doc("Product Tax Category")
