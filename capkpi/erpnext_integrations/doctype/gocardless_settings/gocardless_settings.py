...
class GoCardlessSettings(Document):
	...
	def on_payment_request_submission(self, data):
		if data.reference_doctype != "Fees":
			customer_data = finergy.db.get_value(
		...
		data = {
			...
			"payer_email": data.email_to or finergy.session.user,
	...
	def check_mandate_validity(self, data):
		...
		if finergy.db.exists("GoCardless Mandate", dict(customer=data.get("payer_name"), disabled=0)):
			registered_mandate = finergy.db.get_value(
	...
	def create_payment_request(self, data):
		...
		try:
			...
			finergy.log_error("Gocardless payment reqeust failed")
			return {
	...
	def create_charge_on_gocardless(self):
		...
		try:
			...
			if (
				...
				finergy.log_error("Gocardless payment cancelled")
				self.integration_request.db_set("error", payment.status, update_modified=False)
			else:
				self.integration_request.db_set("status", "Failed", update_modified=False)
				finergy.log_error("Gocardless payment failed")
				self.integration_request.db_set("error", payment.status, update_modified=False)
		...
		if self.flags.status_changed_to == "Completed":
			...
			if "reference_doctype" in self.data and "reference_docname" in self.data:
				...
				try:
					...
					finergy.log_error("Gocardless redirect failed")
...
def gocardless_initialization(doc):
	...
	settings = finergy.get_doc("GoCardless Settings", gateway_controller)
