...
finergy.ui.form.on('Plaid Settings', {
	...
	refresh: function (frm) {
		if (frm.doc.enabled) {
			frm.add_custom_button(__('Link a new bank account'), () => {
				new capkpi.integrations.plaidLink(frm);
			});
			...
			frm.add_custom_button(__("Sync Now"), () => {
				finergy.call({
					method: "capkpi.capkpi_integrations.doctype.plaid_settings.plaid_settings.enqueue_synchronization",
					freeze: true,
					callback: () => {
...
capkpi.integrations.plaidLink = class plaidLink {
	constructor(parent) {
		this.frm = parent;
	...
	async init_config() {
		...
		this.client_name = finergy.boot.sitename;
	...
	async get_link_token() {
		...
		if (!token) {
			finergy.throw(__('Cannot retrieve link token. Check Error Log for more information'));
		}
		return token;
	}
	...
	plaid_success(token, response) {
		...
		finergy.prompt({
			...
			finergy.xcall('capkpi.capkpi_integrations.doctype.plaid_settings.plaid_settings.add_institution', {
				token: token,
				response: response
			}).then((result) => {
				finergy.xcall('capkpi.capkpi_integrations.doctype.plaid_settings.plaid_settings.add_bank_accounts', {
					response: response,
					bank: result,
					company: me.company
				});
			}).then(() => {
				finergy.show_alert({ message: __("Bank accounts added"), indicator: 'green' });
			});
