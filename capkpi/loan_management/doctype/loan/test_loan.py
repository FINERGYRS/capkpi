...
import finergy
from finergy.utils import add_days, add_months, add_to_date, date_diff, flt, get_datetime, nowdate

from capkpi.loan_management.doctype.loan.loan import (
	make_loan_write_off,
	request_loan_closure,
	unpledge_security,
)
from capkpi.loan_management.doctype.loan_application.loan_application import create_pledge
from capkpi.loan_management.doctype.loan_disbursement.loan_disbursement import (
	get_disbursal_amount,
)
from capkpi.loan_management.doctype.loan_interest_accrual.loan_interest_accrual import (
	days_in_year,
)
...
from capkpi.loan_management.doctype.process_loan_security_shortfall.process_loan_security_shortfall import (
...
from capkpi.selling.doctype.customer.test_customer import get_customer_dict
...
class TestLoan(unittest.TestCase):
	def setUp(self):
		...
		if not finergy.db.exists("Customer", "_Test Loan Customer"):
			finergy.get_doc(get_customer_dict("_Test Loan Customer")).insert(ignore_permissions=True)

		if not finergy.db.exists("Customer", "_Test Loan Customer 1"):
			finergy.get_doc(get_customer_dict("_Test Loan Customer 1")).insert(ignore_permissions=True)

		self.applicant2 = finergy.db.get_value("Customer", {"name": "_Test Loan Customer"}, "name")
		self.applicant3 = finergy.db.get_value("Customer", {"name": "_Test Loan Customer 1"}, "name")

		create_loan(self.applicant1, "Personal Loan", 280000, "Repay Over Number of Periods", 20)

	def test_loan(self):
		loan = finergy.get_doc("Loan", {"applicant": self.applicant1})
	...
	def test_loan_disbursement(self):
		...
		loan = finergy.get_doc("Loan", loan.name)
		gl_entries1 = finergy.db.get_all(
			"GL Entry",
			fields=["name"],
			filters={"voucher_type": "Loan Disbursement", "voucher_no": loan_disbursement_entry1.name},
		)

		gl_entries2 = finergy.db.get_all(
			"GL Entry",
			fields=["name"],
	...
	def test_sanctioned_amount_limit(self):
		...
		finergy.db.sql("DELETE FROM `tabLoan` where applicant = '_Test Loan Customer 1'")
		finergy.db.sql("DELETE FROM `tabLoan Application` where applicant = '_Test Loan Customer 1'")
		finergy.db.sql("DELETE FROM `tabLoan Security Pledge` where applicant = '_Test Loan Customer 1'")
		...
		self.assertRaises(finergy.ValidationError, loan_application.save)
	...
	def test_security_shortfall(self):
		...
		create_process_loan_security_shortfall()
		loan_security_shortfall = finergy.get_doc("Loan Security Shortfall", {"loan": loan.name})
	...
	def test_penalty(self):
		...
		calculated_penalty_amount = finergy.db.get_value(
...
def create_loan_accounts():
	if not finergy.db.exists("Account", "Loans and Advances (Assets) - _TC"):
		finergy.get_doc(
			{
				"doctype": "Account",
	...
	if not finergy.db.exists("Account", "Loan Account - _TC"):
		finergy.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	if not finergy.db.exists("Account", "Payment Account - _TC"):
		finergy.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	if not finergy.db.exists("Account", "Disbursement Account - _TC"):
		finergy.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	if not finergy.db.exists("Account", "Interest Income Account - _TC"):
		finergy.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	if not finergy.db.exists("Account", "Penalty Income Account - _TC"):
		finergy.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
...
def create_loan_type(
	...
	if not finergy.db.exists("Loan Type", loan_name):
		loan_type = finergy.get_doc(
			{
				"doctype": "Loan Type",
				"company": "_Test Company",
				"loan_name": loan_name,
...
def create_loan_security_type():
	if not finergy.db.exists("Loan Security Type", "Stock"):
		finergy.get_doc(
			{
...
def create_loan_security():
	if not finergy.db.exists("Loan Security", "Test Security 1"):
		finergy.get_doc(
			{
	...
	if not finergy.db.exists("Loan Security", "Test Security 2"):
		finergy.get_doc(
			{
				"doctype": "Loan Security",
				"loan_security_type": "Stock",
...
def create_loan_security_pledge(applicant, pledges, loan_application=None, loan=None):
	...
	lsp = finergy.new_doc("Loan Security Pledge")
...
def make_loan_disbursement_entry(loan, amount, disbursement_date=None):
	...
	loan_disbursement_entry = finergy.get_doc(
		{
...
def create_loan_application(
	...
	loan_application = finergy.new_doc("Loan Application")
