...
class LoanSecurityPrice(Document):
	...
	def validate_dates(self):
		...
		if self.valid_from > self.valid_upto:
			finergy.throw(_("Valid From Time must be lesser than Valid Upto Time."))

		existing_loan_security = finergy.db.sql(
		...
		if existing_loan_security:
			finergy.throw(_("Loan Security Price overlapping with {0}").format(existing_loan_security[0][0]))
...
def get_loan_security_price(loan_security, valid_time=None):
	...
	loan_security_price = finergy.db.get_value(
		"Loan Security Price",
		{
	...
	if not loan_security_price:
		finergy.throw(_("No valid Loan Security Price found for {0}").format(finergy.bold(loan_security)))
	else:
