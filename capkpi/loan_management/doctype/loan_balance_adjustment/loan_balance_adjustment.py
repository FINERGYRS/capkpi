...
class LoanBalanceAdjustment(AccountsController):
	...
	def validate(self):
		if self.amount == 0:
			finergy.throw(_("Amount cannot be zero"))
		if self.amount < 0:
			finergy.throw(_("Amount cannot be negative"))
	...
	def set_status_and_amounts(self, cancel=0):
		...
		finergy.db.set_value("Loan", self.loan, {adj_field: adjustment_amount})
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		gle_map = []
		loan_account = finergy.db.get_value("Loan", self.loan, "loan_account")
