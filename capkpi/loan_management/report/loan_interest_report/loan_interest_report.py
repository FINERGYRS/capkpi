...
from finergy.utils import add_days, flt, getdate

import capkpi
from capkpi.loan_management.report.applicant_wise_loan_security_exposure.applicant_wise_loan_security_exposure import (
...
def get_active_loan_details(filters):
	...
	currency = capkpi.get_company_currency(filters.get("company"))
...
def get_interest_accruals(loans):
	...
	interest_accruals = finergy.get_all(
...
def get_penal_interest_rate_map():
	return finergy._dict(
		finergy.get_all("Loan Type", fields=["name", "penalty_interest_rate"], as_list=1)
	)
