...
from finergy.utils import cstr, rounded, time_diff_in_hours
from finergy.utils.formatters import format_value

from capkpi.healthcare.doctype.fee_validity.fee_validity import create_fee_validity
from capkpi.healthcare.doctype.healthcare_settings.healthcare_settings import get_income_account
from capkpi.healthcare.doctype.lab_test.lab_test import create_multiple
...
def validate_customer_created(patient):
	if not finergy.db.get_value("Patient", patient.name, "customer"):
...
def get_appointments_to_invoice(patient, company):
	...
	patient_appointments = finergy.get_list(
		"Patient Appointment",
		fields="*",
		filters={
	...
	for appointment in patient_appointments:
		...
		# Consultation Appointments, should check fee validity
			...
			) and finergy.db.exists("Fee Validity Reference", {"appointment": appointment.name}):
		...
		for encounter in encounters:
			if not encounter.appointment:
				...
				if encounter.practitioner:
					if encounter.inpatient_record and finergy.db.get_single_value(
...
def get_lab_tests_to_invoice(patient, company):
	...
	lab_prescriptions = finergy.db.sql(
		"""
			SELECT
...
def get_clinical_procedures_to_invoice(patient, company):
	...
	procedure_prescriptions = finergy.db.sql(
		"""
			SELECT
...
def get_inpatient_services_to_invoice(patient, company):
	services_to_invoice = []
	inpatient_services = finergy.db.sql(
		"""
			SELECT
				io.*
			FROM
	...
	for inpatient_occupancy in inpatient_services:
		service_unit_type = finergy.db.get_value(
			"Healthcare Service Unit", inpatient_occupancy.service_unit, "service_unit_type"
		)
		service_unit_type = finergy.get_cached_doc("Healthcare Service Unit Type", service_unit_type)
...
def get_therapy_plans_to_invoice(patient, company):
	...
	therapy_plans = finergy.get_list(
		"Therapy Plan",
...
def get_therapy_sessions_to_invoice(patient, company):
	...
	for therapy in therapy_sessions:
		if not therapy.appointment:
			if therapy.therapy_type and finergy.db.get_value(
				...
				therapy_sessions_to_invoice.append(
					{
						...
						"service": finergy.db.get_value("Therapy Type", therapy.therapy_type, "item"),
					}
				)
...
def get_appointment_type_service_item(appointment_type, department, is_inpatient):
	from capkpi.healthcare.doctype.appointment_type.appointment_type import (
...
def throw_config_service_item(is_inpatient):
	...
	finergy.throw(msg, title=_("Missing Configuration"))
...
def get_practitioner_service_item(practitioner, is_inpatient):
	...
	if is_inpatient:
		service_item, practitioner_charge = finergy.db.get_value(
...
def get_healthcare_service_item(is_inpatient):
	...
	if is_inpatient:
		service_item = finergy.db.get_single_value("Healthcare Settings", "inpatient_visit_charge_item")
	else:
		service_item = finergy.db.get_single_value("Healthcare Settings", "op_consulting_charge_item")
...
def get_practitioner_charge(practitioner, is_inpatient):
	if is_inpatient:
		practitioner_charge = finergy.db.get_value(
	...
	if doc.items:
		for item in doc.items:
			if item.get("reference_dt") and item.get("reference_dn"):
				if finergy.get_meta(item.reference_dt).has_field("invoiced"):
					set_invoiced(item, method, doc.name)

	if method == "on_submit" and finergy.db.get_single_value(
...
def set_invoiced(item, method, ref_invoice=None):
	...
	if item.reference_dt == "Clinical Procedure":
		...
		if service_item == item.item_code:
			finergy.db.set_value(item.reference_dt, item.reference_dn, "consumption_invoiced", invoiced)
		else:
			finergy.db.set_value(item.reference_dt, item.reference_dn, "invoiced", invoiced)
	else:
		finergy.db.set_value(item.reference_dt, item.reference_dn, "invoiced", invoiced)

	if item.reference_dt == "Patient Appointment":
		if finergy.db.get_value("Patient Appointment", item.reference_dn, "procedure_template"):
...
def validate_invoiced_on_submit(item):
	if (
		...
		and finergy.db.get_single_value("Healthcare Settings", "clinical_procedure_consumable_item")
		== item.item_code
	):
		is_invoiced = finergy.db.get_value(item.reference_dt, item.reference_dn, "consumption_invoiced")
	else:
		is_invoiced = finergy.db.get_value(item.reference_dt, item.reference_dn, "invoiced")
	if is_invoiced:
		finergy.throw(
...
def manage_prescriptions(invoiced, ref_dt, ref_dn, dt, created_check_field):
	created = finergy.db.get_value(ref_dt, ref_dn, created_check_field)
	if created:
		# Fetch the doc created for the prescription
		doc_created = finergy.db.get_value(dt, {"prescription": ref_dn})
		finergy.db.set_value(dt, doc_created, "invoiced", invoiced)
...
def check_fee_validity(appointment):
	...
	validity = finergy.get_doc("Fee Validity", validity)
	return validity
...
def manage_fee_validity(appointment):
	...
	if fee_validity:
		if appointment.status == "Cancelled" and fee_validity.visited > 0:
			...
			finergy.db.delete("Fee Validity Reference", {"appointment": appointment.name})
...
def manage_doc_for_appointment(dt_from_appointment, appointment, invoiced):
	dn_from_appointment = finergy.db.get_value(
		dt_from_appointment, filters={"appointment": appointment}
	)
	if dn_from_appointment:
		finergy.db.set_value(dt_from_appointment, dn_from_appointment, "invoiced", invoiced)


@finergy.whitelist()
def get_drugs_to_invoice(encounter):
	encounter = finergy.get_doc("Patient Encounter", encounter)
	if encounter:
		patient = finergy.get_doc("Patient", encounter.patient)
		if patient:
			if patient.customer:
				items_to_invoice = []
				for drug_line in encounter.drug_prescription:
					if drug_line.drug_code:
						qty = 1
						if finergy.db.get_value("Item", drug_line.drug_code, "stock_uom") == "Nos":
...
def update_address_links(address, method):
	...
	for link in patient_links:
		customer = finergy.db.get_value("Patient", link.get("link_name"), "customer")
...
def update_patient_email_and_phone_numbers(contact, method):
	...
	"""
	if "Healthcare" not in finergy.get_active_domains() or contact.flags.skip_patient_update:
		return
	...
	if contact.is_primary_contact and (contact.email_id or contact.mobile_no or contact.phone):
		...
		for link in patient_links:
			contact_details = finergy.db.get_value(
			...
			if new_contact_details:
				finergy.db.set_value("Patient", link.get("link_name"), new_contact_details)
