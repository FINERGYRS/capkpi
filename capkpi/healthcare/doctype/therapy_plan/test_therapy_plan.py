...
from capkpi.healthcare.doctype.therapy_plan.therapy_plan import (
	make_sales_invoice,
	make_therapy_session,
)
from capkpi.healthcare.doctype.therapy_type.test_therapy_type import create_therapy_type
...
class TestTherapyPlan(unittest.TestCase):
	def test_creation_on_encounter_submission(self):
		...
		self.assertTrue(finergy.db.exists("Therapy Plan", encounter.therapy_plan))
	...
	def test_status(self):
		...
		finergy.get_doc(session).submit()
		self.assertEqual(finergy.db.get_value("Therapy Plan", plan.name, "status"), "In Progress")

		session = make_therapy_session(plan.name, plan.patient, "Basic Rehab", "_Test Company")
		session.start_date = add_days(getdate(), 1)
		finergy.get_doc(session).submit()
		self.assertEqual(finergy.db.get_value("Therapy Plan", plan.name, "status"), "Completed")
		...
		session = finergy.get_doc(session)
		session.submit()
		self.assertEqual(
	...
	def test_therapy_plan_from_template(self):
		...
		# check linked item
		self.assertTrue(finergy.db.exists("Therapy Plan Template", {"linked_item": "Complete Rehab"}))
		...
		therapy_plan_template_amt = finergy.db.get_value(
...
def create_therapy_plan(template=None):
	...
	plan = finergy.new_doc("Therapy Plan")
...
def create_therapy_plan_template():
	template_name = finergy.db.exists("Therapy Plan Template", "Complete Rehab")
	if not template_name:
		therapy_type = create_therapy_type()
		template = finergy.new_doc("Therapy Plan Template")
		template.plan_name = template.item_code = template.item_name = "Complete Rehab"
		template.item_group = "Services"
		rate = finergy.db.get_value("Therapy Type", therapy_type.name, "rate")
		template.append(
			"therapy_types",
