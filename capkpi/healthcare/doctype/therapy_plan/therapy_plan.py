...
class TherapyPlan(Document):
	...
	def set_therapy_details_from_template(self):
		...
		therapy_plan_template = finergy.get_doc("Therapy Plan Template", self.therapy_plan_template)
...
def make_therapy_session(therapy_plan, patient, therapy_type, company, appointment=None):
	therapy_type = finergy.get_doc("Therapy Type", therapy_type)

	therapy_session = finergy.new_doc("Therapy Session")
...
def make_sales_invoice(reference_name, patient, company, therapy_plan_template):
	...
	si.company = company
	si.patient = patient
	si.customer = finergy.db.get_value("Patient", patient, "customer")
	si.currency = finergy.get_value(
		"Customer", si.customer, "default_currency"
	) or get_company_currency(si.company)

	item = finergy.db.get_value("Therapy Plan Template", therapy_plan_template, "linked_item")
