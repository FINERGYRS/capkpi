...
import finergy
from finergy.utils import getdate, nowtime
...
from capkpi.healthcare.doctype.patient_medical_record.test_patient_medical_record import (
...
class TestLabTest(unittest.TestCase):
	def test_lab_test_item(self):
		...
		self.assertTrue(finergy.db.exists("Item", lab_template.item))
		self.assertEqual(
			finergy.db.get_value("Item Price", {"item_code": lab_template.item}, "price_list_rate"),
			lab_template.lab_test_rate,
		)

		lab_template.disabled = 1
		lab_template.save()
		self.assertEqual(finergy.db.get_value("Item", lab_template.item, "disabled"), 1)
	...
	def test_descriptive_lab_test(self):
		...
		lab_test.save()
		self.assertRaises(finergy.ValidationError, lab_test.submit)
	...
	def test_sample_collection(self):
		...
		self.assertTrue(finergy.db.exists("Sample Collection", {"sample": lab_template.sample}))
...
def create_lab_test_template(test_sensitivity=0, sample_collection=1):
	...
	if finergy.db.exists("Lab Test Template", "Insulin Resistance"):
		return finergy.get_doc("Lab Test Template", "Insulin Resistance")
...
def create_medical_department():
	medical_department = finergy.db.exists("Medical Department", "_Test Medical Department")
...
def create_lab_test_sample():
	blood_sample = finergy.db.exists("Lab Test Sample", "Blood Sample")
	if blood_sample:
		return blood_sample

	sample = finergy.new_doc("Lab Test Sample")
...
def create_patient_encounter():
	...
	patient_encounter = finergy.new_doc("Patient Encounter")
...
def create_practitioner():
	practitioner = finergy.db.exists("Healthcare Practitioner", "_Test Healthcare Practitioner")
