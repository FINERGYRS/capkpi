...
import finergy
from finergy.utils import getdate, strip_html
...
class TestPatientHistorySettings(unittest.TestCase):
	def setUp(self):
		...
		settings = finergy.get_single("Patient History Settings")
		settings.append(
			"custom_doctypes",
			{
	...
	def test_custom_doctype_medical_record(self):
		...
		medical_rec = finergy.get_doc("Patient Medical Record", medical_rec)
		expected_subject = "Date:{0}Rating:3Feedback:Test Patient History Settings".format(
			finergy.utils.format_date(getdate())
		)
...
def create_custom_doctype():
	if not finergy.db.exists("DocType", "Test Patient Feedback"):
		doc = finergy.get_doc(
			{
				"doctype": "DocType",
				"module": "Healthcare",
		...
		return finergy.get_doc("DocType", "Test Patient Feedback")
