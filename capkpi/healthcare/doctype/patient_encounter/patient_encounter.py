...
class PatientEncounter(Document):
	...
	def on_cancel(self):
		if self.appointment:
			finergy.db.set_value("Patient Appointment", self.appointment, "status", "Open")
	...
	def get_applicable_treatment_plans(encounter):
		...
		if diagnosis:
			...
			diagnosis = finergy.get_list("Patient Encounter Diagnosis", filters=filters, fields="*")
		...
		if symptoms:
			...
			symptoms = finergy.get_list("Patient Encounter Symptom", filters=filters, fields="*")
		...
		plans = finergy.get_list("Treatment Plan Template", fields="*", filters=plan_filters)
...
def make_ip_medication_order(source_name, target_doc=None):
	def set_missing_values(source, target):
		...
		for entry in source.drug_prescription:
			if entry.drug_code:
				dosage = finergy.get_doc("Prescription Dosage", entry.dosage)
...
def get_prescription_dates(period, start_date):
	prescription_duration = finergy.get_doc("Prescription Duration", period)
...
def create_therapy_plan(encounter):
	if len(encounter.therapies):
		doc = finergy.new_doc("Therapy Plan")
		...
		if doc.get("name"):
			...
			finergy.msgprint(
				_("Therapy Plan {0} created successfully.").format(finergy.bold(doc.name)), alert=True
			)


def delete_ip_medication_order(encounter):
	record = finergy.db.exists("Inpatient Medication Order", {"patient_encounter": encounter.name})
	if record:
		finergy.delete_doc("Inpatient Medication Order", record, force=1)
