...
finergy.ui.form.on('Patient Encounter', {
	setup: function(frm) {
		...
		if (!frm.doc.__islocal) {
			...
			frm.add_custom_button(__('Patient History'), function() {
				if (frm.doc.patient) {
					...
					finergy.msgprint(__('Please select Patient'));
				}
			},'View');
			...
			if (frm.doc.drug_prescription && frm.doc.inpatient_record && frm.doc.inpatient_status === "Admitted") {
				frm.add_custom_button(__('Inpatient Medication Order'), function() {
					finergy.model.open_mapped_doc({
						method: 'capkpi.healthcare.doctype.patient_encounter.patient_encounter.make_ip_medication_order',
						frm: frm
					});
				}, 'Create');
			}
...
var schedule_inpatient = function(frm) {
	var dialog = new finergy.ui.Dialog({
		...
		primary_action : function() {
			...
			finergy.call({
				method: 'capkpi.healthcare.doctype.inpatient_record.inpatient_record.schedule_inpatient',
				args: {
					args: args
				},
				callback: function(data) {
...
var schedule_discharge = function(frm) {
	var dialog = new finergy.ui.Dialog ({
		...
		fields: [
			{fieldtype: 'Datetime', label: 'Discharge Ordered DateTime', fieldname: 'discharge_ordered_datetime', default: finergy.datetime.now_datetime()},
		...
		primary_action : function() {
			...
			finergy.call ({
				method: 'capkpi.healthcare.doctype.inpatient_record.inpatient_record.schedule_discharge',
				args: {args},
				callback: function(data) {
					if(!data.exc){
						frm.reload_doc();
...
let create_medical_record = function(frm) {
	...
	finergy.new_doc('Patient Medical Record');
};
...
let create_procedure = function(frm) {
	...
	finergy.new_doc('Clinical Procedure');
};

finergy.ui.form.on('Drug Prescription', {
	dosage: function(frm, cdt, cdn){
		finergy.model.set_value(cdt, cdn, 'update_schedule', 1);
		let child = locals[cdt][cdn];
		if (child.dosage) {
			finergy.model.set_value(cdt, cdn, 'interval_uom', 'Day');
			finergy.model.set_value(cdt, cdn, 'interval', 1);
		}
	},
	...
	interval_uom: function(frm, cdt, cdn) {
		...
		if (child.interval_uom == 'Hour') {
			finergy.model.set_value(cdt, cdn, 'dosage', null);
		}
	}
});
