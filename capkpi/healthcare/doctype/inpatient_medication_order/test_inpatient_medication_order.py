...
class TestInpatientMedicationOrder(unittest.TestCase):
	...
	def test_order_creation(self):
		...
		prescription_dosage = finergy.get_doc("Prescription Dosage", "1-1-1")
	...
	def test_inpatient_validation(self):
		...
		self.assertRaises(finergy.ValidationError, ipmo.insert)
...
def create_dosage_form():
	if not finergy.db.exists("Dosage Form", "Tablet"):
		finergy.get_doc({"doctype": "Dosage Form", "dosage_form": "Tablet"}).insert()


def create_drug(item=None):
	if not item:
		item = "Dextromethorphan"
	drug = finergy.db.exists("Item", {"item_code": "Dextromethorphan"})
	if not drug:
		drug = finergy.get_doc(
			{
				"doctype": "Item",
...
def create_ipmo(patient):
	orders = get_orders()
	ipmo = finergy.new_doc("Inpatient Medication Order")
...
def create_ipme(filters, update_stock=0):
	ipme = finergy.new_doc("Inpatient Medication Entry")
