...
from finergy.utils import flt, get_link_to_form, get_time, getdate

from capkpi import get_company_currency
from capkpi.healthcare.doctype.healthcare_settings.healthcare_settings import (
	get_income_account,
	get_receivable_account,
)
from capkpi.healthcare.utils import (
	check_fee_validity,
...
class MaximumCapacityError(finergy.ValidationError):
	pass
...
class PatientAppointment(Document):
	...
	def validate_overlaps(self):
		...
		overlapping_appointments = finergy.db.sql(
			"""
			SELECT
		...
		if self.service_unit:  # validate service unit capacity if overlap enabled
			allow_overlap, service_unit_capacity = finergy.get_value(
				...
				if len(service_unit_appointments) >= (service_unit_capacity or 1):
					finergy.throw(
						_("Not allowed, {} cannot exceed maximum capacity {}").format(
							finergy.bold(self.service_unit), finergy.bold(service_unit_capacity or 1)
						),
		...
		if overlapping_appointments:
			finergy.throw(
				_("Not allowed, cannot overlap appointment {}").format(
					finergy.bold(", ".join([appointment["name"] for appointment in overlapping_appointments]))
				),
				OverlapError,
			)
	...
	def validate_service_unit(self):
		if self.inpatient_record and self.service_unit:
			from capkpi.healthcare.doctype.inpatient_medication_entry.inpatient_medication_entry import (
				get_current_healthcare_service_unit,
			)

			is_inpatient_occupancy_unit = finergy.db.get_value(
			...
			if is_inpatient_occupancy_unit and service_unit != self.service_unit:
				msg = (
					_("Patient {0} is not admitted in the service unit {1}").format(
						finergy.bold(self.patient), finergy.bold(self.service_unit)
					)
					+ "<br>"
				)
				msg += _(
				...
				finergy.throw(msg, title=_("Invalid Healthcare Service Unit"))
	...
	def set_payment_details(self):
		if finergy.db.get_single_value("Healthcare Settings", "automate_appointment_invoicing"):
	...
	def validate_customer_created(self):
		if finergy.db.get_single_value("Healthcare Settings", "automate_appointment_invoicing"):
			if not finergy.db.get_value("Patient", self.patient, "customer"):
				msg = _("Please set a Customer linked to the Patient")
				msg += " <b><a href='/app/Form/Patient/{0}'>{0}</a></b>".format(self.patient)
				finergy.throw(msg, title=_("Customer Not Found"))
	...
	def update_prescription_details(self):
		if self.procedure_prescription:
			...
			if self.procedure_template:
				comments = finergy.db.get_value(
					"Procedure Prescription", self.procedure_prescription, "comments"
				)
				if comments:
					finergy.db.set_value("Patient Appointment", self.name, "notes", comments)

	def update_fee_validity(self):
		if not finergy.db.get_single_value("Healthcare Settings", "enable_free_follow_ups"):
			return
		...
		if fee_validity:
			finergy.msgprint(
				_("{0}: {1} has fee validity till {2}").format(
					self.patient, finergy.bold(self.patient_name), fee_validity.valid_till
				)
			)
	...
	def get_therapy_types(self):
		...
		therapy_types = []
		doc = finergy.get_doc("Therapy Plan", self.therapy_plan)
...
def check_payment_fields_reqd(patient):
	automate_invoicing = finergy.db.get_single_value(
		"Healthcare Settings", "automate_appointment_invoicing"
	)
	free_follow_ups = finergy.db.get_single_value("Healthcare Settings", "enable_free_follow_ups")
	if automate_invoicing:
		if free_follow_ups:
			fee_validity = finergy.db.exists("Fee Validity", {"patient": patient, "status": "Pending"})
			if fee_validity:
...
def invoice_appointment(appointment_doc):
	...
	appointment_invoiced = finergy.db.get_value(
		"Patient Appointment", appointment_doc.name, "invoiced"
	)
	enable_free_follow_ups = finergy.db.get_single_value(
	...
	if enable_free_follow_ups:
		...
		if fee_validity and fee_validity.status == "Completed":
			...
			if finergy.db.exists("Fee Validity Reference", {"appointment": appointment_doc.name}):
				return
	else:
		fee_validity = None
...
def create_sales_invoice(appointment_doc):
	sales_invoice = finergy.new_doc("Sales Invoice")
	sales_invoice.patient = appointment_doc.patient
	sales_invoice.customer = finergy.get_value("Patient", appointment_doc.patient, "customer")
	sales_invoice.currency = finergy.get_value(
	...
	finergy.msgprint(_("Sales Invoice {0} created").format(sales_invoice.name), alert=True)
	finergy.db.set_value(
		"Patient Appointment",
...
def check_is_new_patient(patient, name=None):
	...
	has_previous_appointment = finergy.db.exists("Patient Appointment", filters)
...
def get_appointment_item(appointment_doc, item):
	...
	item.cost_center = finergy.get_cached_value("Company", appointment_doc.company, "cost_center")
	item.rate = charge
	item.amount = charge
	item.qty = 1
...
def cancel_appointment(appointment_id):
	appointment = finergy.get_doc("Patient Appointment", appointment_id)
...
def check_sales_invoice_exists(appointment):
	sales_invoice = finergy.db.get_value(
		"Sales Invoice Item",
	...
	if sales_invoice:
		sales_invoice = finergy.get_doc("Sales Invoice", sales_invoice)
...
def get_availability_data(date, practitioner):
	...
	practitioner_doc = finergy.get_doc("Healthcare Practitioner", practitioner)
...
def check_employee_wise_availability(date, practitioner_doc):
	...
	if practitioner_doc.employee:
		...
		employee = finergy.db.get_value("Employee", {"user_id": practitioner_doc.user_id}, "name")

	if employee:
		# check holiday
		if is_holiday(employee, date):
			finergy.throw(_("{0} is a holiday".format(date)), title=_("Not Available"))

		# check leave status
		leave_record = finergy.db.sql(
...
def get_available_slots(practitioner_doc, date):
	...
	for schedule_entry in practitioner_doc.practitioner_schedules:
		...
		practitioner_schedule = finergy.get_doc("Practitioner Schedule", schedule_entry.schedule)
		...
		if not schedule_entry.service_unit:
			finergy.throw(
				_(
					...
					finergy.bold(schedule_entry.schedule),
				),
...
def update_status(appointment_id, status):
	finergy.db.set_value("Patient Appointment", appointment_id, "status", status)
	appointment_booked = True
	if status == "Cancelled":
		appointment_booked = False
		cancel_appointment(appointment_id)

	procedure_prescription = finergy.db.get_value(
...
def send_confirmation_msg(doc):
	if finergy.db.get_single_value("Healthcare Settings", "send_appointment_confirmation"):
		message = finergy.db.get_single_value("Healthcare Settings", "appointment_confirmation_msg")
		try:
			send_message(doc, message)
		except Exception:
			finergy.log_error(finergy.get_traceback(), _("Appointment Confirmation Message Not Sent"))
			finergy.msgprint(_("Appointment Confirmation Message Not Sent"), indicator="orange")
...
def send_appointment_reminder():
	if finergy.db.get_single_value("Healthcare Settings", "send_appointment_reminder"):
		remind_before = datetime.datetime.strptime(
			finergy.db.get_single_value("Healthcare Settings", "remind_before"), "%H:%M:%S"
		)
		reminder_dt = datetime.datetime.now() + datetime.timedelta(
			hours=remind_before.hour, minutes=remind_before.minute, seconds=remind_before.second
		)

		appointment_list = finergy.db.get_all(
			"Patient Appointment",
			{
		...
		for appointment in appointment_list:
			doc = finergy.get_doc("Patient Appointment", appointment.name)
			message = finergy.db.get_single_value("Healthcare Settings", "appointment_reminder_msg")
			send_message(doc, message)
			finergy.db.set_value("Patient Appointment", doc.name, "reminded", 1)


def send_message(doc, message):
	patient_mobile = finergy.db.get_value("Patient", doc.patient, "mobile")
	if patient_mobile:
		...
		message = finergy.render_template(message, context)
		number = [patient_mobile]
		try:
			send_sms(number, message)
		except Exception as e:
			finergy.msgprint(_("SMS not sent, please check SMS Settings"), alert=True)
...
def update_appointment_status():
	...
	for appointment in appointments:
		finergy.get_doc("Patient Appointment", appointment.name).set_status()
