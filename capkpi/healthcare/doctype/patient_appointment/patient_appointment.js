...
finergy.ui.form.on('Patient Appointment', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		...
		frm.set_query('service_unit', function() {
			return {
				query: 'capkpi.controllers.queries.get_healthcare_service_units',
				filters: {
		...
		if (frm.is_new()) {
			frm.page.set_primary_action(__('Check Availability'), function() {
				if (!frm.doc.patient) {
					...
					finergy.call({
						method: 'capkpi.healthcare.doctype.patient_appointment.patient_appointment.check_payment_fields_reqd',
		...
		if (frm.doc.patient) {
			frm.add_custom_button(__('Patient History'), function() {
				finergy.route_options = { 'patient': frm.doc.patient };
			...
			if (frm.doc.procedure_template) {
				frm.add_custom_button(__('Clinical Procedure'), function() {
					finergy.model.open_mapped_doc({
						method: 'capkpi.healthcare.doctype.clinical_procedure.clinical_procedure.make_procedure',
						frm: frm,
					});
				}, __('Create'));
				...
				frm.add_custom_button(__('Therapy Session'), function() {
					finergy.model.open_mapped_doc({
						method: 'capkpi.healthcare.doctype.therapy_session.therapy_session.create_therapy_session',
						frm: frm,
					})
				}, 'Create');
			} else {
				frm.add_custom_button(__('Patient Encounter'), function() {
					finergy.model.open_mapped_doc({
						method: 'capkpi.healthcare.doctype.patient_appointment.patient_appointment.make_encounter',
						frm: frm,
					});
				}, __('Create'));
			}
	...
	set_payment_details: function(frm) {
		finergy.db.get_single_value('Healthcare Settings', 'automate_appointment_invoicing').then(val => {
			if (val) {
				finergy.call({
					method: 'capkpi.healthcare.utils.get_service_item_and_practitioner_charge',
					args: {
						doc: frm.doc
					},
					callback: function(data) {
						if (data.message) {
							finergy.model.set_value(frm.doctype, frm.docname, 'paid_amount', data.message.practitioner_charge);
							finergy.model.set_value(frm.doctype, frm.docname, 'billing_item', data.message.service_item);
						}
					}
				});
			}
	...
	therapy_type: function(frm) {
		if (frm.doc.therapy_type) {
			finergy.db.get_value('Therapy Type', frm.doc.therapy_type, 'default_duration', (r) => {
	...
	get_prescribed_therapies: function(frm) {
		if (frm.doc.patient) {
			finergy.call({
				method: "capkpi.healthcare.doctype.patient_appointment.patient_appointment.get_prescribed_therapies",
				args: { patient: frm.doc.patient },
				callback: function(r) {
					if (r.message) {
...
let check_and_set_availability = function(frm) {
	...
	function show_slots(d, fd) {
		if (d.get_value('appointment_date') && d.get_value('practitioner')) {
			...
			finergy.call({
				method: 'capkpi.healthcare.doctype.patient_appointment.patient_appointment.get_availability_data',
				args: {
...
let get_prescribed_procedure = function(frm) {
	if (frm.doc.patient) {
		finergy.call({
			method: 'capkpi.healthcare.doctype.patient_appointment.patient_appointment.get_procedure_prescribed',
			args: { patient: frm.doc.patient },
			callback: function(r) {
...
let create_vital_signs = function(frm) {
	if (!frm.doc.patient) {
		finergy.throw(__('Please select patient'));
	}
	...
	finergy.new_doc('Vital Signs');
};

let update_status = function(frm, status) {
	let doc = frm.doc;
	finergy.confirm(__('Are you sure you want to cancel this appointment?'),
		function() {
			finergy.call({
				method: 'capkpi.healthcare.doctype.patient_appointment.patient_appointment.update_status',
