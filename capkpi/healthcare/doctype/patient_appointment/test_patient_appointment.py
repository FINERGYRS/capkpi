...
import finergy
from finergy.utils import add_days, now_datetime, nowdate

from capkpi.accounts.doctype.pos_profile.test_pos_profile import make_pos_profile
from capkpi.healthcare.doctype.patient_appointment.patient_appointment import (
...
class TestPatientAppointment(unittest.TestCase):
	def setUp(self):
		...
		finergy.db.sql("""delete from `tabFee Validity`""")
		finergy.db.sql("""delete from `tabPatient Encounter`""")
		make_pos_profile()
		finergy.db.sql("""delete from `tabHealthcare Service Unit` where name like '_Test %'""")
		finergy.db.sql(
	...
	def test_status(self):
		...
		finergy.db.set_value("Healthcare Settings", None, "automate_appointment_invoicing", 0)
		...
		self.assertEqual(
			finergy.db.get_value("Patient Appointment", appointment.name, "status"), "Closed"
		)
		encounter.cancel()
		self.assertEqual(finergy.db.get_value("Patient Appointment", appointment.name, "status"), "Open")
	...
	def test_start_encounter(self):
		...
		self.assertEqual(
			encounter.invoiced, finergy.db.get_value("Patient Appointment", appointment.name, "invoiced")
		)
	...
	def test_auto_invoicing(self):
		...
		finergy.db.set_value("Healthcare Settings", None, "automate_appointment_invoicing", 0)
		appointment = create_appointment(patient, practitioner, nowdate())
		self.assertEqual(finergy.db.get_value("Patient Appointment", appointment.name, "invoiced"), 0)

		finergy.db.set_value("Healthcare Settings", None, "automate_appointment_invoicing", 1)
		appointment = create_appointment(patient, practitioner, add_days(nowdate(), 2), invoice=1)
		self.assertEqual(finergy.db.get_value("Patient Appointment", appointment.name, "invoiced"), 1)
		sales_invoice_name = finergy.db.get_value(
			"Sales Invoice Item", {"reference_dn": appointment.name}, "parent"
		)
		self.assertTrue(sales_invoice_name)
		self.assertEqual(
			finergy.db.get_value("Sales Invoice", sales_invoice_name, "company"), appointment.company
		)
		self.assertEqual(
			finergy.db.get_value("Sales Invoice", sales_invoice_name, "patient"), appointment.patient
		)
		self.assertEqual(
			finergy.db.get_value("Sales Invoice", sales_invoice_name, "paid_amount"), appointment.paid_amount
		)
	...
	def test_appointment_cancel(self):
		patient, practitioner = create_healthcare_docs()
		finergy.db.set_value("Healthcare Settings", None, "enable_free_follow_ups", 1)
		appointment = create_appointment(patient, practitioner, nowdate())
		fee_validity = finergy.db.get_value(
		...
		self.assertEqual(finergy.db.get_value("Fee Validity", fee_validity, "visited"), 1)

		update_status(appointment.name, "Cancelled")
		# check fee validity updated
		self.assertEqual(finergy.db.get_value("Fee Validity", fee_validity, "visited"), 0)
		...
		self.assertEqual(finergy.db.get_value("Sales Invoice", sales_invoice_name, "status"), "Cancelled")
	...
	def test_appointment_booking_for_admission_service_unit(self):
		...
		# Discharge
		schedule_discharge(
			finergy.as_json({"patient": patient, "discharge_ordered_datetime": now_datetime()})
		)
		ip_record1 = finergy.get_doc("Inpatient Record", ip_record.name)
	...
	def test_invalid_healthcare_service_unit_validation(self):
		...
		self.assertRaises(finergy.exceptions.ValidationError, appointment.save)
	...
	def test_payment_should_be_mandatory_for_new_patient_appointment(self):
		finergy.db.set_value("Healthcare Settings", None, "enable_free_follow_ups", 1)
		finergy.db.set_value("Healthcare Settings", None, "automate_appointment_invoicing", 1)
		finergy.db.set_value("Healthcare Settings", None, "max_visits", 3)
		finergy.db.set_value("Healthcare Settings", None, "valid_days", 30)
	...
	def test_sales_invoice_should_be_generated_for_new_patient_appointment(self):
		patient, practitioner = create_healthcare_docs()
		finergy.db.set_value("Healthcare Settings", None, "automate_appointment_invoicing", 1)
		invoice_count = finergy.db.count("Sales Invoice")

		assert check_is_new_patient(patient)
		create_appointment(patient, practitioner, nowdate())
		new_invoice_count = finergy.db.count("Sales Invoice")

		assert new_invoice_count == invoice_count + 1

	def test_overlap_appointment(self):
		from capkpi.healthcare.doctype.patient_appointment.patient_appointment import OverlapError
	...
	def test_patient_appointment_should_consider_permissions_while_fetching_appointments(self):
		...
		new_practitioner = finergy.get_doc("Healthcare Practitioner", new_practitioner)
		new_practitioner.user_id = user.email
		new_practitioner.save()

		finergy.set_user(user.name)
		appointments = finergy.get_list("Patient Appointment")
...
def create_patient(
	...
	if finergy.db.exists("Patient", {"firstname": f"_Test Patient {str(id)}"}):
		patient = finergy.db.get_value("Patient", {"first_name": f"_Test Patient {str(id)}"}, ["name"])
		return patient

	patient = finergy.new_doc("Patient")
...
def create_medical_department(id=0):
	if finergy.db.exists("Medical Department", f"_Test Medical Department {str(id)}"):
		return f"_Test Medical Department {str(id)}"

	medical_department = finergy.new_doc("Medical Department")
...
def create_practitioner(id=0, medical_department=None):
	if finergy.db.exists(
		...
		practitioner = finergy.db.get_value(
			"Healthcare Practitioner", {"firstname": f"_Test Healthcare Practitioner {str(id)}"}, ["name"]
		)
		return practitioner

	practitioner = finergy.new_doc("Healthcare Practitioner")
...
def create_encounter(appointment):
	if appointment:
		encounter = finergy.new_doc("Patient Encounter")
...
def create_appointment(
	...
	item = create_healthcare_service_items()
	finergy.db.set_value("Healthcare Settings", None, "inpatient_visit_charge_item", item)
	finergy.db.set_value("Healthcare Settings", None, "op_consulting_charge_item", item)
	appointment = finergy.new_doc("Patient Appointment")
...
def create_healthcare_service_items():
	if finergy.db.exists("Item", "HLC-SI-001"):
		return "HLC-SI-001"
...
def create_clinical_procedure_template():
	if finergy.db.exists("Clinical Procedure Template", "Knee Surgery and Rehab"):
		return finergy.get_doc("Clinical Procedure Template", "Knee Surgery and Rehab")

	template = finergy.new_doc("Clinical Procedure Template")
...
def create_appointment_type(args=None):
	...
	if finergy.db.exists("Appointment Type", name):
		return finergy.get_doc("Appointment Type", name)
		...
		return finergy.get_doc(
			{
				...
				"price_list": args.get("price_list") or finergy.db.get_value("Price List", {"selling": 1}),
...
def create_service_unit_type(id=0, allow_appointments=1, overlap_appointments=0):
	if finergy.db.exists("Healthcare Service Unit Type", f"_Test Service Unit Type {str(id)}"):
		return f"_Test Service Unit Type {str(id)}"

	service_unit_type = finergy.new_doc("Healthcare Service Unit Type")
...
def create_service_unit(id=0, service_unit_type=None, service_unit_capacity=0):
	if finergy.db.exists("Healthcare Service Unit", f"_Test Service Unit {str(id)}"):
		return f"_Test service_unit {str(id)}"

	service_unit = finergy.new_doc("Healthcare Service Unit")
...
def create_user(email=None, roles=None):
	if not email:
		email = "{}@finergy-rs.com".format(finergy.utils.random_string(10))
	user = finergy.db.exists("User", email)
	if not user:
