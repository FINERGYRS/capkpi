...
from finergy.utils import flt, get_link_to_form

from capkpi.healthcare.doctype.healthcare_settings.healthcare_settings import get_account
from capkpi.stock.utils import get_latest_stock_qty
...
class InpatientMedicationEntry(Document):
	...
	def get_medication_orders(self):
		...
		if orders:
			...
			finergy.msgprint(_("No pending medication orders found for selected criteria"))
	...
	def update_medication_orders(self, on_cancel=False):
		...
		for order, count in order_entry_map.items():
			medication_order = finergy.get_doc("Inpatient Medication Order", order)
			completed_orders = flt(count)
			current_value = finergy.db.get_value("Inpatient Medication Order", order, "completed_orders")
		...
		if drug_shortage:
			...
			for drug, shortage_qty in drug_shortage.items():
				...
				formatted_item_rows += """
					...
					item_link, finergy.bold(shortage_qty)
				)
			...
			finergy.throw(message, title=_("Insufficient Stock"), is_minimizable=True, wide=True)
	...
	def make_stock_entry(self):
		...
		for entry in self.medication_orders:
			...
			se_child.uom = finergy.db.get_value("Item", entry.drug_code, "stock_uom")
	...
	def cancel_stock_entries(self):
		stock_entries = finergy.get_all("Stock Entry", {"inpatient_medication_entry": self.name})
		for entry in stock_entries:
			doc = finergy.get_doc("Stock Entry", entry.name)
			doc.cancel()
...
def get_current_healthcare_service_unit(inpatient_record):
	ip_record = finergy.get_doc("Inpatient Record", inpatient_record)
...
def make_difference_stock_entry(docname):
	doc = finergy.get_doc("Inpatient Medication Entry", docname)
	...
	cost_center = finergy.get_cached_value("Company", doc.company, "cost_center")
	expense_account = get_account(None, "expense_account", "Healthcare Settings", doc.company)

	for drug, shortage_qty in drug_shortage.items():
		se_child = stock_entry.append("items")
		se_child.item_code = drug
		se_child.item_name = finergy.db.get_value("Item", drug, "stock_uom")
		se_child.uom = finergy.db.get_value("Item", drug, "stock_uom")
		se_child.stock_uom = se_child.uom
