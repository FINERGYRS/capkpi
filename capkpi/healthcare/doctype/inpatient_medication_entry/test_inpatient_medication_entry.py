...
import finergy
from finergy.utils import add_days, getdate, now_datetime
...
class TestInpatientMedicationEntry(unittest.TestCase):
	def setUp(self):
		...
		finergy.db.sql("""delete from `tabInpatient Medication Order`""")
		finergy.db.sql("""delete from `tabInpatient Medication Entry`""")
	...
	def test_ipme_with_stock_update(self):
		...
		is_order_completed = finergy.db.get_value(
			"Inpatient Medication Order Entry", ipme.medication_orders[0].against_imoe, "is_completed"
		)
		self.assertEqual(is_order_completed, 1)

		# test stock entry
		stock_entry = finergy.db.exists("Stock Entry", {"inpatient_medication_entry": ipme.name})
		self.assertTrue(stock_entry)

		# check references
		stock_entry = finergy.get_doc("Stock Entry", stock_entry)
