...
class LabTestTemplate(Document):
	...
	def validate(self):
		if self.is_billable and (not self.lab_test_rate or self.lab_test_rate <= 0.0):
			finergy.throw(_("Standard Selling Rate should be greater than zero."))
		...
		if self.change_in_item and self.is_billable and self.item:
			...
			if not item_price:
				if self.lab_test_rate and self.lab_test_rate > 0.0:
					price_list_name = finergy.db.get_value(
						"Selling Settings", None, "selling_price_list"
					) or finergy.db.get_value("Price List", {"selling": 1})
					make_item_price(self.lab_test_code, price_list_name, self.lab_test_rate)
			else:
				finergy.db.set_value("Item Price", item_price, "price_list_rate", self.lab_test_rate)
	...
	def on_trash(self):
		...
		if self.item:
			try:
				item = self.item
				self.db_set("item", "")
				finergy.delete_doc("Item", item)
			except Exception:
				finergy.throw(_("Not permitted. Please disable the Lab Test Template"))
	...
	def item_price_exists(self):
		item_price = finergy.db.exists({"doctype": "Item Price", "item_code": self.lab_test_code})
		if item_price:
	...
	def validate_conversion_factor(self):
		if self.lab_test_template_type == "Single" and self.secondary_uom and not self.conversion_factor:
			finergy.throw(_("Conversion Factor is mandatory"))
		if self.lab_test_template_type == "Compound":
			for item in self.normal_test_templates:
				if item.secondary_uom and not item.conversion_factor:
					finergy.throw(_("Row #{0}: Conversion Factor is mandatory").format(item.idx))
		if self.lab_test_template_type == "Grouped":
			for group in self.lab_test_groups:
				if (
					...
					finergy.throw(_("Row #{0}: Conversion Factor is mandatory").format(group.idx))
...
def create_item_from_template(doc):
	...
	finergy.db.set_value("Lab Test Template", doc.name, "item", item.name)
...
def change_test_code_from_template(lab_test_code, doc):
	...
	if finergy.db.exists({"doctype": "Item", "item_code": lab_test_code}):
		finergy.throw(_("Lab Test Item {0} already exist").format(lab_test_code))
	else:
		rename_doc("Item", doc.name, lab_test_code, ignore_permissions=True)
		finergy.db.set_value("Lab Test Template", doc.name, "lab_test_code", lab_test_code)
		finergy.db.set_value("Lab Test Template", doc.name, "lab_test_name", lab_test_code)
