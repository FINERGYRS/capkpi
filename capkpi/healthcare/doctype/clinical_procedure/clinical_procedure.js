...
finergy.ui.form.on('Clinical Procedure', {
	setup: function(frm) {
		frm.set_query('batch_no', 'items', function(doc, cdt, cdn) {
			...
			if (!item.item_code) {
				...
				if (frm.doc.status == 'In Progress') {
					filters['posting_date'] = frm.doc.start_date || finergy.datetime.nowdate();
	...
	practitioner: function(frm) {
		if (frm.doc.practitioner) {
			finergy.call({
				...
				callback: function (data) {
					finergy.model.set_value(frm.doctype,frm.docname, 'practitioner_name', data.message.practitioner_name);
				}
			});
		} else {
			finergy.model.set_value(frm.doctype,frm.docname, 'practitioner_name', '');
		}
	},
	...
	set_procedure_consumables: function(frm) {
		finergy.call({
			method: 'capkpi.healthcare.doctype.clinical_procedure.clinical_procedure.get_procedure_consumables',
			args: {
...
finergy.ui.form.on('Clinical Procedure Item', {
	qty: function(frm, cdt, cdn) {
		let d = locals[cdt][cdn];
		finergy.model.set_value(cdt, cdn, 'transfer_qty', d.qty*d.conversion_factor);
	},
	...
	item_code: function(frm, cdt, cdn) {
		...
		if (d.item_code) {
			...
			return finergy.call({
				method: 'capkpi.healthcare.doctype.clinical_procedure_template.clinical_procedure_template.get_item_details',
				args: {args: args},
				callback: function(r) {
					if (r.message) {
						let d = locals[cdt][cdn];
