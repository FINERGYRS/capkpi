...
from finergy.utils import flt, nowdate, nowtime

from capkpi.healthcare.doctype.healthcare_settings.healthcare_settings import get_account
from capkpi.healthcare.doctype.lab_test.lab_test import create_sample_doc
...
class ClinicalProcedure(Document):
	...
	def after_insert(self):
		if self.prescription:
			finergy.db.set_value("Procedure Prescription", self.prescription, "procedure_created", 1)
		if self.appointment:
			finergy.db.set_value("Patient Appointment", self.appointment, "status", "Closed")
		template = finergy.get_doc("Clinical Procedure Template", self.procedure_template)
		if template.sample:
			patient = finergy.get_doc("Patient", self.patient)
			sample_collection = create_sample_doc(template, patient, None, self.company)
			finergy.db.set_value("Clinical Procedure", self.name, "sample", sample_collection.name)
		self.reload()
	...
	def complete_procedure(self):
		...
		if self.items:
			...
			if customer:
				...
				finergy.throw(
					_("Please set Customer in Patient {0}").format(finergy.bold(self.patient)),
...
def make_procedure(source_name, target_doc=None):
	def set_missing_values(source, target):
		consume_stock = finergy.db.get_value(
		...
		if consume_stock:
			...
			if source.service_unit:
				warehouse = finergy.db.get_value("Healthcare Service Unit", source.service_unit, "warehouse")
			if not warehouse:
				warehouse = finergy.db.get_value("Stock Settings", None, "default_warehouse")
			if warehouse:
