...
class TestClinicalProcedure(unittest.TestCase):
	def test_procedure_template_item(self):
		...
		self.assertTrue(finergy.db.exists("Item", procedure_template.item))

		procedure_template.disabled = 1
		procedure_template.save()
		self.assertEqual(finergy.db.get_value("Item", procedure_template.item, "disabled"), 1)
	...
	def test_consumables(self):
		...
		# check consumption
		self.assertTrue(finergy.db.exists("Stock Entry", result))


def create_consumable():
	if finergy.db.exists("Item", "Syringe"):
		return finergy.get_doc("Item", "Syringe")
	consumable = finergy.new_doc("Item")
