...
finergy.ui.form.on('Exercise Type', {
	refresh: function(frm) {
		let wrapper = frm.fields_dict.steps_html.wrapper;

		frm.ExerciseEditor = new capkpi.ExerciseEditor(frm, wrapper);
	}
});

capkpi.ExerciseEditor = Class.extend({
	...
	show_add_card_dialog: function(frm) {
		...
		let d = new finergy.ui.Dialog({
			...
			primary_action: function() {
				...
				let step = finergy.model.add_child(frm.doc, 'Exercise Type Step', 'steps_table');
	...
	show_edit_card_dialog: function(frm, id) {
		let new_dialog = new finergy.ui.Dialog({
