...
finergy.ui.form.on('Inpatient Record', {
	setup: function(frm) {
	...
	let dialog = new finergy.ui.Dialog({
		...
		fields: [
			{fieldtype: 'Datetime', label: 'Discharge Datetime', fieldname: 'check_out',
				reqd: 1, default: finergy.datetime.now_datetime()
			}
		],
	...
	let dialog = new finergy.ui.Dialog({
		...
		fields: [
			...
			{fieldtype: 'Date', label: 'Expected Discharge', fieldname: 'expected_discharge',
				default: frm.doc.expected_length_of_stay ? finergy.datetime.add_days(finergy.datetime.now_datetime(), frm.doc.expected_length_of_stay) : ''
			}
		],
...
let transfer_patient_dialog = function(frm) {
	let dialog = new finergy.ui.Dialog({
		...
		fields: [
			...
			{fieldtype: 'Datetime', label: 'Check In', fieldname: 'check_in', reqd: 1, default: finergy.datetime.now_datetime()}
		],
		...
		primary_action : function() {
			...
			if(check_in > finergy.datetime.now_datetime()){
				finergy.msgprint({
	...
	dialog.fields_dict['leave_from'].get_query = function(){
		return {
			query : 'capkpi.healthcare.doctype.inpatient_record.inpatient_record.get_leave_from',
