...
from finergy.utils import get_datetime, get_link_to_form, getdate, now_datetime, today


class InpatientRecord(Document):
	def after_insert(self):
		finergy.db.set_value("Patient", self.patient, "inpatient_record", self.name)
		finergy.db.set_value("Patient", self.patient, "inpatient_status", self.status)
	...
	def validate(self):
		...
		if self.status == "Discharged":
			finergy.db.set_value("Patient", self.patient, "inpatient_status", None)
			finergy.db.set_value("Patient", self.patient, "inpatient_record", None)
	...
	def validate_dates(self):
		if (getdate(self.expected_discharge) < getdate(self.scheduled_date)) or (
			...
			finergy.throw(_("Expected and Discharge dates cannot be less than Admission Schedule date"))
	...
	def validate_already_scheduled_or_admitted(self):
		...
		ip_record = finergy.db.sql(query, {"name": self.name, "patient": self.patient}, as_dict=1)
	...
	def discharge(self, check_out=None):
		...
		if getdate(check_out) < getdate(self.admitted_datetime):
			finergy.throw(_("Discharge date cannot be less than Admission date"))
...
def schedule_inpatient(args):
	...
	if (
		...
		finergy.throw(_("Missing required details, did not create Inpatient Record"))
	...
	# Patient details
	patient = finergy.get_doc("Patient", admission_order["patient"])
	...
	encounter = finergy.get_doc("Patient Encounter", admission_order["admission_encounter"])
...
def schedule_discharge(args):
	...
	if (
		...
		finergy.throw(_("Missing required details, did not create schedule discharge"))

	inpatient_record_id = finergy.db.get_value(
		"Patient", discharge_order["patient"], "inpatient_record"
	)
	if inpatient_record_id:
		inpatient_record = finergy.get_doc("Inpatient Record", inpatient_record_id)
...
def validate_inpatient_invoicing(inpatient_record):
	if finergy.db.get_single_value("Healthcare Settings", "allow_discharge_despite_unbilled_services"):
		return
	...
	if pending_invoices:
		...
		finergy.throw(message, title=_("Unbilled Services"), is_minimizable=True, wide=True)
...
def admit_patient(inpatient_record, service_unit, check_in, expected_discharge=None):
	...
	finergy.db.set_value("Patient", inpatient_record.patient, "inpatient_status", "Admitted")
	finergy.db.set_value(
...
def transfer_patient(inpatient_record, service_unit, check_in):
	...
	finergy.db.set_value("Healthcare Service Unit", service_unit, "occupancy_status", "Occupied")
