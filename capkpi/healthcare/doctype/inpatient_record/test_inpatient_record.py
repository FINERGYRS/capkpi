...
import finergy
from finergy.utils import now_datetime, today
...
from capkpi.healthcare.doctype.lab_test.test_lab_test import create_patient_encounter
from capkpi.healthcare.utils import get_encounters_to_invoice
...
class TestInpatientRecord(unittest.TestCase):
	def test_admit_and_discharge(self):
		...
		self.assertEqual(ip_record.name, finergy.db.get_value("Patient", patient, "inpatient_record"))
		self.assertEqual(ip_record.status, finergy.db.get_value("Patient", patient, "inpatient_status"))

		# Admit
		service_unit = get_healthcare_service_unit()
		admit_patient(ip_record, service_unit, now_datetime())
		self.assertEqual("Admitted", finergy.db.get_value("Patient", patient, "inpatient_status"))
		self.assertEqual(
			"Occupied", finergy.db.get_value("Healthcare Service Unit", service_unit, "occupancy_status")
		)
		...
		self.assertEqual(
			"Vacant", finergy.db.get_value("Healthcare Service Unit", service_unit, "occupancy_status")
		)

		ip_record1 = finergy.get_doc("Inpatient Record", ip_record.name)
		# Validate Pending Invoices
		self.assertRaises(finergy.ValidationError, ip_record.discharge)
		mark_invoiced_inpatient_occupancy(ip_record1)

		discharge_patient(ip_record1, now_datetime())

		self.assertEqual(None, finergy.db.get_value("Patient", patient, "inpatient_record"))
		self.assertEqual(None, finergy.db.get_value("Patient", patient, "inpatient_status"))
	...
	def test_allow_discharge_despite_unbilled_services(self):
		...
		ip_record = finergy.get_doc("Inpatient Record", ip_record.name)
	...
	def test_validate_overlap_admission(self):
		...
		self.assertRaises(finergy.ValidationError, ip_record_new.save)
...
def create_inpatient(patient):
	patient_obj = finergy.get_doc("Patient", patient)
	inpatient_record = finergy.new_doc("Inpatient Record")
...
def get_healthcare_service_unit(unit_name=None):
	if not unit_name:
		...
		service_unit = finergy.db.exists(
	...
	if not service_unit:
		...
		service_unit_parent_name = finergy.db.exists(
			{
		...
		if not service_unit_parent_name:
			parent_service_unit = finergy.new_doc("Healthcare Service Unit")
...
def create_patient():
	patient = finergy.db.exists("Patient", "_Test IPD Patient")
	if not patient:
