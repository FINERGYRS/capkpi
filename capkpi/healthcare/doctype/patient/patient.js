...
finergy.ui.form.on('Patient', {
	refresh: function (frm) {
		...
		if (finergy.defaults.get_default('patient_name_by') != 'Naming Series') {
			frm.toggle_display('naming_series', false);
		} else {
			capkpi.toggle_naming_series();
		}

		if (finergy.defaults.get_default('collect_registration_fee') && frm.doc.status == 'Disabled') {
			frm.add_custom_button(__('Invoice Patient Registration'), function () {
				invoice_registration(frm);
			});
		}

		if (frm.doc.patient_name && finergy.user.has_role('Physician')) {
			frm.add_custom_button(__('Patient Progress'), function() {
				finergy.route_options = {'patient': frm.doc.name};
				finergy.set_route('patient-progress');
			}, __('View'));
		...
		finergy.dynamic_link = {doc: frm.doc, fieldname: 'name', doctype: 'Patient'};
		frm.toggle_display(['address_html', 'contact_html'], !frm.is_new());

		if (!frm.is_new()) {
			if ((finergy.user.has_role('Nursing User') || finergy.user.has_role('Physician'))) {
...
finergy.ui.form.on('Patient', 'dob', function(frm) {
	if (frm.doc.dob) {
		let today = new Date();
		let birthDate = new Date(frm.doc.dob);
		if (today < birthDate) {
			finergy.msgprint(__('Please select a valid Date'));
			finergy.model.set_value(frm.doctype,frm.docname, 'dob', '');
		} else {
...
finergy.ui.form.on('Patient Relation', {
...
let create_vital_signs = function (frm) {
	if (!frm.doc.name) {
		finergy.throw(__('Please save the patient first'));
	}
	finergy.route_options = {
		'patient': frm.doc.name,
	};
...
let create_encounter = function (frm) {
	...
	finergy.new_doc('Patient Encounter');
};
		...
		callback: function(data) {
			if (!data.exc) {
				if (data.message.invoice) {
					finergy.set_route('Form', 'Sales Invoice', data.message.invoice);
				}
				cur_frm.reload_doc();
			}
		}
