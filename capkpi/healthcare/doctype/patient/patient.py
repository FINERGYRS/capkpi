...
from finergy.utils import cint, cstr, getdate
from finergy.utils.nestedset import get_root_of

from capkpi import get_default_currency
from capkpi.accounts.party import get_dashboard_info
...
class Patient(Document):
	...
	def after_insert(self):
		if finergy.db.get_single_value("Healthcare Settings", "collect_registration_fee"):
			finergy.db.set_value("Patient", self.name, "status", "Disabled")
		else:
			send_registration_sms(self)
		self.reload()

	def on_update(self):
		if finergy.db.get_single_value("Healthcare Settings", "link_customer_to_patient"):
			if self.customer:
				customer = finergy.get_doc("Customer", self.customer)
	...
	def set_missing_customer_details(self):
		if not self.customer_group:
			self.customer_group = finergy.db.get_single_value(
				"Selling Settings", "customer_group"
			) or get_root_of("Customer Group")
		if not self.territory:
			self.territory = finergy.db.get_single_value("Selling Settings", "territory") or get_root_of(
				"Territory"
			)
		if not self.default_price_list:
			self.default_price_list = finergy.db.get_single_value("Selling Settings", "selling_price_list")
		...
		if not self.language:
			self.language = finergy.db.get_single_value("System Settings", "language")

	def create_website_user(self):
		users = finergy.db.get_all(
			"User",
		...
		if users and users[0]:
			finergy.throw(
				...
				).format(finergy.bold(users[0].email), finergy.bold(users[0].mobile_no)),
	...
	def autoname(self):
		patient_name_by = finergy.db.get_single_value("Healthcare Settings", "patient_name_by")
	...
	def get_patient_name(self):
		...
		if finergy.db.get_value("Patient", name):
	...
	def invoice_patient_registration(self):
		if finergy.db.get_single_value("Healthcare Settings", "registration_fee"):
			company = finergy.defaults.get_user_default("company")
			if not company:
				company = finergy.db.get_single_value("Global Defaults", "default_company")

			sales_invoice = make_invoice(self.name, company)
			sales_invoice.save(ignore_permissions=True)
			finergy.db.set_value("Patient", self.name, "status", "Active")
			send_registration_sms(self)
...
def create_customer(doc):
	customer = finergy.get_doc(
		{
			...
			or finergy.db.get_single_value("Selling Settings", "customer_group"),
			"territory": doc.territory or finergy.db.get_single_value("Selling Settings", "territory"),
	...
	finergy.db.set_value("Patient", doc.name, "customer", customer.name)
	finergy.msgprint(_("Customer {0} is created.").format(customer.name), alert=True)


def make_invoice(patient, company):
	uom = finergy.db.exists("UOM", "Nos") or finergy.db.get_single_value("Stock Settings", "stock_uom")
	sales_invoice = finergy.new_doc("Sales Invoice")
	sales_invoice.customer = finergy.db.get_value("Patient", patient, "customer")
	...
	item_line.rate = finergy.db.get_single_value("Healthcare Settings", "registration_fee")
...
def get_patient_detail(patient):
	patient_dict = finergy.db.sql("""select * from tabPatient where name=%s""", (patient), as_dict=1)
	if not patient_dict:
		finergy.throw(_("Patient not found"))
	vital_sign = finergy.db.sql(
...
def get_timeline_data(doctype, name):
	...
	customer = finergy.db.get_value(doctype, name, "customer")
	if customer:
		from capkpi.accounts.party import get_timeline_data
