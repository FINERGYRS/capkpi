...
finergy.treeview_settings['Healthcare Service Unit'] = {
	breadcrumbs: 'Healthcare Service Unit',
	title: __('Service Unit Tree'),
	get_tree_root: false,
	get_tree_nodes: 'capkpi.healthcare.utils.get_children',
	filters: [{
		fieldname: 'company',
		fieldtype: 'Select',
			...
			onchange: () => {
				if (cur_dialog) {
					if (cur_dialog.fields_dict.service_unit_type.value) {
						finergy.db.get_value('Healthcare Service Unit Type',
	...
	post_render: function (treeview) {
		finergy.treeview_settings['Healthcare Service Unit'].treeview = {};
		$.extend(finergy.treeview_settings['Healthcare Service Unit'].treeview, treeview);
	},
	toolbar: [
		{
			...
			click: function (node) {
				const dialog = new finergy.ui.Dialog({
					...
					primary_action: () => {
						...
						return finergy.call({
							method: 'capkpi.healthcare.doctype.healthcare_service_unit.healthcare_service_unit.add_multiple_service_units',
							args: {
							...
							callback: function (r) {
								if (!r.exc && r.message) {
									finergy.treeview_settings['Healthcare Service Unit'].treeview.tree.load_children(node, true);
									...
									finergy.msgprint(__('Could not create Service Units'));
								}
							},
							freeze: true,
