...
class HealthcareServiceUnit(NestedSet):
	...
	def set_service_unit_properties(self):
		if cint(self.is_group):
			...
			service_unit_type = finergy.get_doc("Healthcare Service Unit Type", self.service_unit_type)
...
def add_multiple_service_units(parent, data):
	...
	company = (
		data.get("company")
		or finergy.defaults.get_defaults().get("company")
		or finergy.db.get_single_value("Global Defaults", "default_company")
	)
	...
	for i in range(start_suffix, count + start_suffix):
		...
		service_unit_doc = finergy.get_doc(service_unit)
		try:
