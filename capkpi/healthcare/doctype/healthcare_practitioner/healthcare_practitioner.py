...
from capkpi.accounts.party import validate_party_accounts
...
class HealthcarePractitioner(Document):
	...
	def autoname(self):
		...
		if finergy.db.exists("Healthcare Practitioner", self.name):
	...
	def validate(self):
		...
		if self.user_id:
			...
			existing_user_id = finergy.db.get_value("Healthcare Practitioner", self.name, "user_id")
			if existing_user_id:
	...
	def on_update(self):
		if self.user_id:
			finergy.permissions.add_user_permission("Healthcare Practitioner", self.name, self.user_id)
	...
	def validate_user_id(self):
		if not finergy.db.exists("User", self.user_id):
			finergy.throw(_("User {0} does not exist").format(self.user_id))
		elif not finergy.db.exists("User", self.user_id, "enabled"):
			finergy.throw(_("User {0} is disabled").format(self.user_id))

		# check duplicate
		practitioner = finergy.db.exists(
