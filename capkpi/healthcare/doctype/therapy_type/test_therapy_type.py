...
class TestTherapyType(unittest.TestCase):
	def test_therapy_type_item(self):
		...
		self.assertTrue(finergy.db.exists("Item", therapy_type.item))

		therapy_type.disabled = 1
		therapy_type.save()
		self.assertEqual(finergy.db.get_value("Item", therapy_type.item, "disabled"), 1)


def create_therapy_type():
	exercise = create_exercise_type()
	therapy_type = finergy.db.exists("Therapy Type", "Basic Rehab")
	if not therapy_type:
		therapy_type = finergy.new_doc("Therapy Type")
...
def create_exercise_type():
	exercise_type = finergy.db.exists("Exercise Type", "Sit to Stand")
	if not exercise_type:
		exercise_type = finergy.new_doc("Exercise Type")
		exercise_type.exercise_name = "Sit to Stand"
		exercise_type.append("steps_table", {"title": "Step 1", "description": "Squat and Rise"})
		exercise_type.save()
	else:
		exercise_type = finergy.get_doc("Exercise Type", exercise_type)
