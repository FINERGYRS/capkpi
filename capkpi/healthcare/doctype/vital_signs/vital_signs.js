...
finergy.ui.form.on('Vital Signs', {
	height: function(frm) {
...
let calculate_bmi = function(frm){
	...
	finergy.model.set_value(frm.doctype,frm.docname, 'bmi', bmi);
	finergy.model.set_value(frm.doctype,frm.docname, 'nutrition_note', bmi_note);
};

let set_bp = function(frm){
	let bp = frm.doc.bp_systolic+ '/' + frm.doc.bp_diastolic + ' mmHg';
	finergy.model.set_value(frm.doctype,frm.docname, 'bp', bp);
};
