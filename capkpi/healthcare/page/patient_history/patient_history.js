finergy.provide('finergy.patient_history');
finergy.pages['patient_history'].on_page_load = function(wrapper) {
...
class PatientHistory {
	...
	show() {
		...
		if (finergy.route_options && !this.patient_id) {
	...
	make_patient_profile() {
		...
		this.main_section.empty().append(finergy.render_template('patient_history'));
		this.setup_filters();
	...
	setup_filters() {
		...
		finergy.xcall(
			'capkpi.healthcare.page.patient_history.patient_history.get_patient_history_doctypes'
		).then(document_types => {
			let doctype_filter = finergy.ui.form.make_control({
	...
	setup_documents(document_types="", selected_date_range="") {
		...
		let me = this;
		finergy.call({
			'method': 'capkpi.healthcare.page.patient_history.patient_history.get_feed',
			args: filters,
			callback: function(r) {
				let data = r.message;
				if (data.length) {
	...
	add_to_records(data) {
		...
		for (i=0; i<data.length; i++) {
			if (data[i].reference_doctype) {
				...
				if (finergy.user_info(data[i].owner).image) {
					data[i].imgsrc = finergy.utils.get_file_link(finergy.user_info(data[i].owner).image);
				} else {
					data[i].imgsrc = false;
				}
				...
				time_line_heading += data[i].reference_doctype + " - " +
					`<a onclick="finergy.set_route('Form', '${data[i].reference_doctype}', '${data[i].reference_name}');">
				...
				details += `<div class='d-flex flex-column width-full'>
						...
						<div class='finergy-card p-5 mt-3'>
	...
	add_date_separator(data) {
		let date = finergy.datetime.str_to_obj(data.communication_date);
		let pdate = '';
		let diff = finergy.datetime.get_day_diff(finergy.datetime.get_today(),
			finergy.datetime.obj_to_str(date));
		...
		if (diff < 1) {
			...
			pdate = __('on {0}', [finergy.datetime.global_date_format(date)]);
		}
		data.date_sep = pdate;
		return data;
	}
	...
	show_patient_info() {
		this.get_patient_info().then(() => {
			$('.patient-info').empty().append(finergy.render_template('patient_history_sidebar', {
		...
		this.page.main.on('click', '.btn-more', function() {
			...
			if (me.page.main.find('.'+docname).parent().find('.document-html').attr('data-fetched') == '1') {
				...
				if (doctype && docname) {
					...
					finergy.call({
						method: 'capkpi.healthcare.utils.render_doc_as_html',
						args: {
							doctype: doctype,
							docname: docname,
	...
	show_patient_vital_charts(btn_id, scale_unit, title) {
		...
		finergy.call({
			method: 'capkpi.healthcare.utils.get_patient_vitals',
			args: {
				patient: me.patient_id
			},
			callback: function(r) {
				if (r.message) {
					...
					new finergy.Chart('.patient_vital_charts', {
						data: {
							labels: labels,
							datasets: datasets
						},
