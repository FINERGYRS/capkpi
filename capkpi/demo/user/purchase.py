...
from finergy.utils.make_random import get_random, how_many

import capkpi
from capkpi.accounts.party import get_party_account_currency
from capkpi.buying.doctype.request_for_quotation.request_for_quotation import (
	make_supplier_quotation_from_rfq,
)
from capkpi.exceptions import InvalidCurrency
from capkpi.setup.utils import get_exchange_rate
from capkpi.stock.doctype.material_request.material_request import make_request_for_quotation


def work():
	finergy.set_user(finergy.db.get_global("demo_purchase_user"))
	...
	if random.random() < 0.6:
		for mr in finergy.get_all(
			"Material Request",
			filters={"material_request_type": "Purchase", "status": "Open"},
			limit=random.randint(1, 6),
		):
			if not finergy.get_all("Request for Quotation", filters={"material_request": mr.name}, limit=1):
				rfq = make_request_for_quotation(mr.name)
				rfq.transaction_date = finergy.flags.current_date
				add_suppliers(rfq)
				rfq.save()
				rfq.submit()
	...
	party_account_currency = get_party_account_currency(
		"Supplier", supplier, capkpi.get_default_company()
	)
	...
	if random.random() < 0.5:
		from capkpi.stock.doctype.material_request.material_request import make_supplier_quotation

		report = "Material Requests for which Supplier Quotations are not created"
		for row in query_report.run(report)["result"][: random.randint(1, 3)]:
			if row[0] != "Total":
				sq = finergy.get_doc(make_supplier_quotation(row[0]))
				sq.transaction_date = finergy.flags.current_date
	...
	if random.random() < 0.5:
		...
		for row in query_report.run(report)["result"][: how_many("Purchase Order")]:
			if row[0] != "Total":
				try:
					po = finergy.get_doc(make_purchase_order(row[0]))
					po.supplier = supplier
					po.currency = party_account_currency or company_currency
					po.conversion_rate = exchange_rate
					po.transaction_date = finergy.flags.current_date
					po.insert()
					po.submit()
				except Exception:
					pass
...
def make_material_request(item_code, qty):
	...
	variant_of = finergy.db.get_value("Item", item_code, "variant_of") or item_code

	if finergy.db.get_value("BOM", {"item": variant_of, "is_default": 1, "is_active": 1}):
		mr.material_request_type = "Manufacture"
	else:
		mr.material_request_type = "Purchase"

	mr.transaction_date = finergy.flags.current_date
	mr.schedule_date = finergy.utils.add_days(mr.transaction_date, 7)

	mr.append(
		"items",
		{
			"doctype": "Material Request Item",
			"schedule_date": finergy.utils.add_days(mr.transaction_date, 7),
			"item_code": item_code,
			"qty": qty,
		},
	)
...
def make_subcontract():
	from capkpi.buying.doctype.purchase_order.purchase_order import make_rm_stock_entry
	...
	if item_code:
		...
		po.transaction_date = finergy.flags.current_date  # added
		po.schedule_date = finergy.utils.add_days(finergy.flags.current_date, 7)
		...
		po.append(
			...
			{
				"item_code": item_code,
				"schedule_date": finergy.utils.add_days(finergy.flags.current_date, 7),
		...
		stock_entry = finergy.get_doc(make_rm_stock_entry(po.name, json.dumps([rm_items])))
