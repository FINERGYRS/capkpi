...
import capkpi
from capkpi.stock.doctype.batch.batch import UnableToSelectBatchError
from capkpi.stock.doctype.delivery_note.delivery_note import make_sales_return
from capkpi.stock.doctype.purchase_receipt.purchase_receipt import make_purchase_return
from capkpi.stock.doctype.serial_no.serial_no import SerialNoQtyError, SerialNoRequiredError
...
def make_purchase_receipt():
	if random.random() < 0.6:
		...
		for po in po_list:
			pr = finergy.get_doc(make_purchase_receipt(po))

			if pr.is_subcontracted == "Yes":
				pr.supplier_warehouse = "Supplier - WPL"

			pr.posting_date = finergy.flags.current_date
			pr.insert()
			try:
				pr.submit()
...
def make_delivery_note():
	...
	if random.random() < 0.6:
		...
		for so in list(set([r[0] for r in query_report.run(report)["result"] if r[0] != "Total"]))[
			...
			dn = finergy.get_doc(make_delivery_note(so))
			dn.posting_date = finergy.flags.current_date
			for d in dn.get("items"):
				if not d.expense_account:
					d.expense_account = "Cost of Goods Sold - {0}".format(
						finergy.get_cached_value("Company", dn.company, "abbr")
					)
...
def make_stock_reconciliation():
	...
	if random.random() < 0.4:
		stock_reco = finergy.new_doc("Stock Reconciliation")
		stock_reco.posting_date = finergy.flags.current_date
		stock_reco.company = capkpi.get_default_company()
...
def submit_draft_stock_entries():
	...
	finergy.db.commit()
	for st in finergy.db.get_values("Stock Entry", {"docstatus": 0}, "name"):
		try:
			ste = finergy.get_doc("Stock Entry", st[0])
			ste.posting_date = finergy.flags.current_date
			ste.save()
			ste.submit()
			finergy.db.commit()
		except (
...
def make_sales_return_records():
	if random.random() < 0.1:
		for data in finergy.get_all("Delivery Note", fields=["name"], filters={"docstatus": 1}):
			if random.random() < 0.1:
				try:
...
def make_purchase_return_records():
	if random.random() < 0.1:
		for data in finergy.get_all("Purchase Receipt", fields=["name"], filters={"docstatus": 1}):
			if random.random() < 0.1:
				try:
