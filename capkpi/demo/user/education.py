...
from capkpi.education.api import (
	collect_fees,
	enroll_student,
	get_course,
	get_fee_schedule,
...
def work():
	finergy.set_user(finergy.db.get_global("demo_education_user"))
	for d in range(20):
		approve_random_student_applicant()
		enroll_random_student(finergy.flags.current_date)
	# if finergy.flags.current_date.weekday()== 0:
	# 	make_course_schedule(finergy.flags.current_date, finergy.utils.add_days(finergy.flags.current_date, 5))
	mark_student_attendance(finergy.flags.current_date)
...
def assign_student_group(student, student_name, program, courses, batch):
	...
	for d in finergy.get_list(
		"Student Group",
		fields=("name"),
		filters={"program": program, "course": ("in", course_list), "disabled": 0},
	):
		student_group = finergy.get_doc("Student Group", d.name)
		...
		student_group.save()
	student_batch = finergy.get_list(
		"Student Group",
		fields=("name"),
		filters={"program": program, "group_based_on": "Batch", "batch": batch, "disabled": 0},
	)[0]
	student_batch_doc = finergy.get_doc("Student Group", student_batch.name)
...
def mark_student_attendance(current_date):
	...
	for d in finergy.db.get_list("Student Group", filters={"group_based_on": "Batch", "disabled": 0}):
...
def make_fees():
	for d in range(1, 10):
		...
		collect_fees(random_fee, finergy.db.get_value("Fees", random_fee, "outstanding_amount"))


def make_assessment_plan(date):
	for d in range(1, 4):
		random_group = get_random("Student Group", {"group_based_on": "Course", "disabled": 0}, True)
		doc = finergy.new_doc("Assessment Plan")
...
def make_course_schedule(start_date, end_date):
	for d in finergy.db.get_list("Student Group"):
		cs = finergy.new_doc("Scheduling Tool")
