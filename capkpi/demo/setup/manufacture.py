		...
		finergy.get_doc(
			{
				"doctype": "Workstation",
				"workstation_name": w,
				"holiday_list": finergy.get_all("Holiday List")[0].name,
...
def show_item_groups_in_website():
	...
	products = finergy.get_doc("Item Group", "Products")
...
def setup_asset():
	assets = json.loads(open(finergy.get_app_path("capkpi", "demo", "data", "asset.json")).read())
	for d in assets:
		asset = finergy.new_doc("Asset")
		asset.update(d)
...
def setup_item():
	items = json.loads(open(finergy.get_app_path("capkpi", "demo", "data", "item.json")).read())
	for i in items:
