...
from capkpi.demo.setup.setup_data import import_json
...
def setup_item():
	items = json.loads(
		open(finergy.get_app_path("capkpi", "demo", "data", "item_education.json")).read()
	)
	for i in items:
		item = finergy.new_doc("Item")
		item.update(i)
		item.min_order_qty = random.randint(10, 30)
		item.item_defaults[0].default_warehouse = finergy.get_all(
...
def make_student_applicants():
	...
	with open(file_path, "r") as open_file:
		...
		for idx, d in enumerate(random_student_data):
			student_applicant = finergy.new_doc("Student Applicant")
...
def make_student_group():
	for term in finergy.db.get_list("Academic Term"):
		for program in finergy.db.get_list("Program"):
			sg_tool = finergy.new_doc("Student Group Creation Tool")
			sg_tool.academic_year = "2017-18"
			sg_tool.academic_term = term.name
			sg_tool.program = program.name
			for d in sg_tool.get_courses():
				d = finergy._dict(d)
				student_group = finergy.new_doc("Student Group")
...
def make_fees_category():
	...
	for i in fee_type:
		fee_category = finergy.new_doc("Fee Category")
...
def make_fees_structure():
	for d in finergy.db.get_list("Program"):
		program = finergy.get_doc("Program", d.name)
		for academic_term in ["2017-18 (Semester 1)", "2017-18 (Semester 2)", "2017-18 (Semester 3)"]:
			fee_structure = finergy.new_doc("Fee Structure")
			fee_structure.program = d.name
			fee_structure.academic_term = random.choice(finergy.db.get_list("Academic Term")).name
			for j in range(1, 4):
				temp = {
					"fees_category": random.choice(finergy.db.get_list("Fee Category")).name,
...
def make_assessment_groups():
	for year in finergy.db.get_list("Academic Year"):
		ag = finergy.new_doc("Assessment Group")
		ag.assessment_group_name = year.name
		ag.parent_assessment_group = "All Assessment Groups"
		ag.is_group = 1
		ag.insert()
		for term in finergy.db.get_list("Academic Term", filters={"academic_year": year.name}):
			ag1 = finergy.new_doc("Assessment Group")
			ag1.assessment_group_name = term.name
			ag1.parent_assessment_group = ag.name
			ag1.is_group = 1
			ag1.insert()
			for assessment_group in ["Term I", "Term II"]:
				ag2 = finergy.new_doc("Assessment Group")
...
def get_json_path(doctype):
	return finergy.get_app_path("capkpi", "demo", "data", finergy.scrub(doctype) + ".json")
