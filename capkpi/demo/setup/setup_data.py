...
from finergy.utils import cstr, flt, now_datetime, random_string
from finergy.utils.make_random import add_random_children, get_random
from finergy.utils.nestedset import get_root_of

import capkpi
from capkpi.demo.domains import data


def setup(domain):
	finergy.flags.in_demo = 1
	...
	employees = finergy.get_all("Employee", fields=["name", "date_of_joining"])
...
def complete_setup(domain="Manufacturing"):
	...
	if not finergy.get_all("Company", limit=1):
		setup_complete(
			{
				"full_name": "Test User",
		...
		if company:
			company_doc = finergy.get_doc("Company", company)
			company_doc.db_set(
				"default_payroll_payable_account",
				finergy.db.get_value("Account", dict(account_name="Payroll Payable")),
			)
...
def setup_holiday_list():
	...
	finergy.set_value(
		"Company", capkpi.get_default_company(), "default_holiday_list", holiday_list.name
	)


def setup_user():
	finergy.db.sql('delete from tabUser where name not in ("Guest", "Administrator")')
	for u in json.loads(open(finergy.get_app_path("capkpi", "demo", "data", "user.json")).read()):
...
def setup_employee():
	finergy.db.set_value("HR Settings", None, "emp_created_by", "Naming Series")
	finergy.db.commit()

	for d in finergy.get_all("Salary Component"):
		salary_component = finergy.get_doc("Salary Component", d.name)
		salary_component.append(
			"accounts",
			dict(
				company=capkpi.get_default_company(),
				account=finergy.get_value("Account", dict(account_name=("like", "Salary%"))),
			),
		)
		salary_component.save()

	import_json("Employee")
	holiday_list = finergy.db.get_value(
		"Holiday List", {"holiday_list_name": str(now_datetime().year)}, "name"
	)
	finergy.db.sql("""update tabEmployee set holiday_list={0}""".format(holiday_list))


def setup_salary_structure(employees, salary_slip_based_on_timesheet=0):
	ss = finergy.new_doc("Salary Structure")
	...
	ss.payment_account = finergy.get_value(
		"Account",
	...
	for e in employees:
		sa = finergy.new_doc("Salary Structure Assignment")
		sa.employee = e.name
...
def setup_user_roles(domain):
	user = finergy.get_doc("User", "demo@capkpi.com")
	user.add_roles(
		"HR User",
		"HR Manager",
		"Accounts User",
	...
	if not finergy.db.get_global("demo_hr_user"):
		user = finergy.get_doc("User", "CaitlinSnow@example.com")
		user.add_roles("HR User", "HR Manager", "Accounts User")
		finergy.db.set_global("demo_hr_user", user.name)
		update_employee_department(user.name, "Human Resources")
		for d in finergy.get_all("User Permission", filters={"user": "CaitlinSnow@example.com"}):
			finergy.delete_doc("User Permission", d.name)

	if not finergy.db.get_global("demo_sales_user_1"):
		user = finergy.get_doc("User", "VandalSavage@example.com")
		user.add_roles("Sales User")
		update_employee_department(user.name, "Sales")
		finergy.db.set_global("demo_sales_user_1", user.name)

	if not finergy.db.get_global("demo_sales_user_2"):
		user = finergy.get_doc("User", "GraceChoi@example.com")
		user.add_roles("Sales User", "Sales Manager", "Accounts User")
		update_employee_department(user.name, "Sales")
		finergy.db.set_global("demo_sales_user_2", user.name)

	if not finergy.db.get_global("demo_purchase_user"):
		user = finergy.get_doc("User", "MaxwellLord@example.com")
		user.add_roles("Purchase User", "Purchase Manager", "Accounts User", "Stock User")
		update_employee_department(user.name, "Purchase")
		finergy.db.set_global("demo_purchase_user", user.name)

	if not finergy.db.get_global("demo_manufacturing_user"):
		user = finergy.get_doc("User", "NeptuniaAquaria@example.com")
		user.add_roles(
			"Manufacturing User", "Stock Manager", "Stock User", "Purchase User", "Accounts User"
		)
		update_employee_department(user.name, "Production")
		finergy.db.set_global("demo_manufacturing_user", user.name)

	if not finergy.db.get_global("demo_stock_user"):
		user = finergy.get_doc("User", "HollyGranger@example.com")
		user.add_roles("Manufacturing User", "Stock User", "Purchase User", "Accounts User")
		update_employee_department(user.name, "Production")
		finergy.db.set_global("demo_stock_user", user.name)

	if not finergy.db.get_global("demo_accounts_user"):
		user = finergy.get_doc("User", "BarryAllen@example.com")
		user.add_roles("Accounts User", "Accounts Manager", "Sales User", "Purchase User")
		update_employee_department(user.name, "Accounts")
		finergy.db.set_global("demo_accounts_user", user.name)

	if not finergy.db.get_global("demo_projects_user"):
		user = finergy.get_doc("User", "PeterParker@example.com")
		user.add_roles("HR User", "Projects User")
		update_employee_department(user.name, "Management")
		finergy.db.set_global("demo_projects_user", user.name)

	if domain == "Education":
		if not finergy.db.get_global("demo_education_user"):
			user = finergy.get_doc("User", "ArthurCurry@example.com")
			user.add_roles("Academics User")
			update_employee_department(user.name, "Management")
			finergy.db.set_global("demo_education_user", user.name)

	# Add Expense Approver
	user = finergy.get_doc("User", "ClarkKent@example.com")
	user.add_roles("Expense Approver")


def setup_leave_allocation():
	year = now_datetime().year
	for employee in finergy.get_all("Employee", fields=["name"]):
		leave_types = finergy.get_all("Leave Type", fields=["name", "max_continuous_days_allowed"])
		for leave_type in leave_types:
			if not leave_type.max_continuous_days_allowed:
				leave_type.max_continuous_days_allowed = 10

		leave_allocation = finergy.get_doc(
			{
...
def setup_mode_of_payment():
	company_abbr = finergy.get_cached_value("Company", capkpi.get_default_company(), "abbr")
	account_dict = {"Cash": "Cash - " + company_abbr, "Bank": "National Bank - " + company_abbr}
	for payment_mode in finergy.get_all("Mode of Payment", fields=["name", "type"]):
		if payment_mode.type:
			mop = finergy.get_doc("Mode of Payment", payment_mode.name)
			mop.append(
				"accounts",
				{
					"company": capkpi.get_default_company(),
...
def setup_account():
	...
	data = json.loads(open(finergy.get_app_path("capkpi", "demo", "data", "account.json")).read())
	for d in data:
		doc = finergy.new_doc("Account")
		doc.update(d)
		doc.parent_account = finergy.db.get_value("Account", {"account_name": doc.parent_account})
		doc.insert()
...
def setup_account_to_expense_type():
	...
	for expense_type in expense_types:
		doc = finergy.get_doc("Expense Claim Type", expense_type["name"])
		doc.append(
			"accounts",
			{"company": capkpi.get_default_company(), "default_account": expense_type["account"]},
		)
		doc.save(ignore_permissions=True)


def setup_budget():
	fiscal_years = finergy.get_all("Fiscal Year", order_by="year_start_date")[-2:]

	for fy in fiscal_years:
		budget = finergy.new_doc("Budget")
		budget.cost_center = get_random("Cost Center")
		budget.fiscal_year = fy.name
		budget.action_if_annual_budget_exceeded = "Warn"
		expense_ledger_count = finergy.db.count("Account", {"is_group": "0", "root_type": "Expense"})
...
def setup_pos_profile():
	company_abbr = finergy.get_cached_value("Company", capkpi.get_default_company(), "abbr")
	pos = finergy.new_doc("POS Profile")
	pos.user = finergy.db.get_global("demo_accounts_user")
	...
	pos.append(
		"payments",
		{
			"mode_of_payment": finergy.db.get_value("Mode of Payment", {"type": "Cash"}, "name"),
			"amount": 0.0,
			"default": 1,
		},
	)
...
def setup_role_permissions():
	...
	for doctype, roles in role_permissions.items():
		for role in roles:
			if not finergy.db.get_value("Custom DocPerm", {"parent": doctype, "role": role}):
				finergy.get_doc(
					{
...
def import_json(doctype, submit=False, values=None):
	finergy.flags.in_import = True
	data = json.loads(
		open(finergy.get_app_path("capkpi", "demo", "data", finergy.scrub(doctype) + ".json")).read()
	)
	for d in data:
...
def update_employee_department(user_id, department):
	employee = finergy.db.get_value("Employee", {"user_id": user_id}, "name")
	department = finergy.db.get_value("Department", {"department_name": department}, "name")
	finergy.db.set_value("Employee", employee, "department", department)
