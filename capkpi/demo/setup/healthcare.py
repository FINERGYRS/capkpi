...
from capkpi.healthcare.doctype.lab_test.lab_test import create_test_from_template
...
def make_appointment():
	...
	while i <= 4:
		...
		department = finergy.get_value("Healthcare Practitioner", practitioner, "department")
		patient = get_random("Patient")
		patient_sex = finergy.get_value("Patient", patient, "sex")
...
def consulation_on_appointment():
	for i in range(3):
		...
		appointment = finergy.get_doc("Patient Appointment", appointment)
...
def lab_test_on_encounter():
	...
	while i <= 2:
		...
		test_rx = finergy.get_doc("Lab Prescription", test_rx)
		encounter = finergy.get_doc("Patient Encounter", test_rx.parent)
		set_lab_test(
			encounter.patient,
...
def append_drug_rx(encounter):
	...
	while i <= 3:
		...
		drug = finergy.get_doc("Item", drug)
...
def import_drug():
	...
	data = json.loads(open(finergy.get_app_path("capkpi", "demo", "data", "drug_list.json")).read())
	for d in data:
		doc = finergy.new_doc("Item")
		doc.update(d)
		doc.insert()
