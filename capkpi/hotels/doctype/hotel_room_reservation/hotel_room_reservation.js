...
finergy.ui.form.on('Hotel Room Reservation', {
	...
	recalculate_rates: function(frm) {
		...
		finergy.call({
			"method": "capkpi.hotels.doctype.hotel_room_reservation.hotel_room_reservation.get_room_rate",
	...
	make_invoice: function(frm) {
		finergy.model.with_doc("Hotel Settings", "Hotel Settings", ()=>{
			finergy.model.with_doctype("Sales Invoice", ()=>{
				let hotel_settings = finergy.get_doc("Hotel Settings", "Hotel Settings");
				let invoice = finergy.model.get_new_doc("Sales Invoice");
				...
				for (let d of frm.doc.items){
					let invoice_item = finergy.model.add_child(invoice, "items")
				...
				finergy.set_route("Form", invoice.doctype, invoice.name);
			});
		});
	}
});

finergy.ui.form.on('Hotel Room Reservation Item', {
