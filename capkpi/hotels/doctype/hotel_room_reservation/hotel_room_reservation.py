...
from finergy.utils import add_days, date_diff, flt


class HotelRoomUnavailableError(finergy.ValidationError):
	pass


class HotelRoomPricingNotSetError(finergy.ValidationError):
	pass
	...
	def validate_availability(self):
		for i in range(date_diff(self.to_date, self.from_date)):
			...
			for d in self.items:
				...
				room_type = finergy.db.get_value("Hotel Room Package", d.item, "hotel_room_type")
				rooms_booked = (
				...
				if total_rooms < rooms_booked:
					finergy.throw(
						_("Hotel Rooms of type {0} are unavailable on {1}").format(
							d.item, finergy.format(day, dict(fieldtype="Date"))
						),
		...
		for d in self.items:
			...
			for i in range(date_diff(self.to_date, self.from_date)):
				...
				if day_rate:
					...
					finergy.throw(
						_("Please set Hotel Room Rate on {}").format(finergy.format(day, dict(fieldtype="Date"))),
...
def get_room_rate(hotel_room_reservation):
	...
	doc = finergy.get_doc(json.loads(hotel_room_reservation))
	doc.set_rates()
	return doc.as_dict()
...
def get_rooms_booked(room_type, day, exclude_reservation=None):
	...
	if exclude_reservation:
		exclude_condition = "and reservation.name != {0}".format(finergy.db.escape(exclude_reservation))
