...
from capkpi.projects.report.employee_hours_utilization_based_on_timesheet.employee_hours_utilization_based_on_timesheet import (
	execute,
)
...
class TestEmployeeUtilization(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		finergy.db.set_value("HR Settings", "HR Settings", "standard_working_hours", 9)

	@classmethod
	def create_test_timesheets(cls):
		timesheet1 = finergy.new_doc("Timesheet")
		...
		timesheet2 = finergy.new_doc("Timesheet")
	...
	def tearDownClass(cls):
		...
		finergy.db.sql("DELETE FROM `tabTimesheet` WHERE company='_Test Company'")
		finergy.db.sql(f"DELETE FROM `tabProject` WHERE name='{cls.test_project.name}'")
	...
	def test_utilization_report_for_single_employee(self):
		...
		emp1_data = finergy.get_doc("Employee", self.test_emp1)
		expected_data = [
			{
				"employee": self.test_emp1,
	...
	def test_utilization_report_for_project(self):
		...
		emp2_data = finergy.get_doc("Employee", self.test_emp2)
		expected_data = [
			{
