...
from capkpi.projects.report.project_profitability.project_profitability import execute


class TestProjectProfitability(FinergyTestCase):
	def setUp(self):
		finergy.db.sql("delete from `tabTimesheet`")
		...
		if holidays:
			finergy.db.set_value("Payroll Settings", None, "include_holidays_in_total_working_days", 1)
		...
		finergy.db.set_value("HR Settings", None, "standard_working_hours", 8)
	...
	def test_project_profitability(self):
		...
		row = report[1][0]
		timesheet = finergy.get_doc("Timesheet", self.timesheet.name)
		...
		standard_working_hours = finergy.db.get_single_value("HR Settings", "standard_working_hours")
