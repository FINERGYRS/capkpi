...
from finergy.utils import add_days, flt, get_datetime, get_time, get_url, nowtime, today

from capkpi import get_default_company
from capkpi.controllers.employee_boarding_controller import update_employee_boarding_status
from capkpi.controllers.queries import get_filters_cond
from capkpi.setup.doctype.holiday_list.holiday_list import is_holiday


class Project(Document):
	def get_feed(self):
		return "{0}: {1}".format(_(self.status), finergy.safe_decode(self.project_name))
	...
	def copy_from_template(self):
		...
		"""
		if self.project_template and not finergy.db.get_all("Task", dict(project=self.name), limit=1):
			...
			template = finergy.get_doc("Project Template", self.project_template)
			...
			for task in template.tasks:
				template_task_details = finergy.get_doc("Task", task.task)
	...
	def dependency_mapping(self, template_tasks, project_tasks):
		for template_task in template_tasks:
			...
			project_task = finergy.get_doc("Task", project_task.name)
	...
	def check_depends_on_value(self, template_task, project_task, project_tasks):
		if template_task.get("depends_on") and not project_task.get("depends_on"):
			for child_task in template_task.get("depends_on"):
				child_task_subject = finergy.db.get_value("Task", child_task.task, "subject")
	...
	def check_for_parent_tasks(self, template_task, project_task, project_tasks):
		if template_task.get("parent_task") and not project_task.get("parent_task"):
			parent_task_subject = finergy.db.get_value("Task", template_task.get("parent_task"), "subject")
			corresponding_project_task = list(
	...
	def after_insert(self):
		self.copy_from_template()
		if self.sales_order:
			finergy.db.set_value("Sales Order", self.sales_order, "project", self.name)

	def on_trash(self):
		finergy.db.set_value("Sales Order", {"project": self.name}, "project", "")
	...
	def update_percent_complete(self):
		...
		total = finergy.db.count("Task", dict(project=self.name))
		...
		if not total:
			...
			if self.percent_complete_method == "Task Weight" and total > 0:
				weight_sum = finergy.db.sql(
					"""select sum(task_weight) from tabTask where
					project=%s""",
					self.name,
				)[0][0]
				weighted_progress = finergy.db.sql(
				...
				for row in weighted_progress:
					pct_complete += row["progress"] * finergy.utils.safe_div(row["task_weight"], weight_sum)
	...
	def update_costing(self):
		...
		TimesheetDetail = finergy.qb.DocType("Timesheet Detail")
		from_time_sheet = (
			finergy.qb.from_(TimesheetDetail)
			.select(
	...
	def update_purchase_costing(self):
		total_purchase_cost = finergy.db.sql(
	...
	def update_sales_amount(self):
		total_sales_amount = finergy.db.sql(
	...
	def update_billed_amount(self):
		total_billed_amount = finergy.db.sql(
			"""select sum(base_net_total)
	...
	def after_rename(self, old_name, new_name, merge=False):
		if old_name == self.copied_from:
			finergy.db.set_value("Project", new_name, "copied_from", new_name)
...
def get_cost_center_name(project):
	return finergy.db.get_value("Project", project, "cost_center")
...
def create_duplicate_project(prev_doc, project_name):
	...
	if project_name == prev_doc.get("name"):
		finergy.throw(_("Use a name that is different from previous project name"))

	# change the copied doc name to new project name
	project = finergy.copy_doc(prev_doc)
	...
	task_list = finergy.get_all("Task", filters={"project": prev_doc.get("name")}, fields=["name"])

	# Create duplicate task for all the task
	for task in task_list:
		task = finergy.get_doc("Task", task)
		new_task = finergy.copy_doc(task)
...
def send_project_update_email_to_users(project):
	doc = finergy.get_doc("Project", project)

	if is_holiday(doc.holiday_list) or not doc.users:
		return

	project_update = finergy.get_doc(
		{
	...
	incoming_email_account = finergy.db.get_value(
...
def collect_project_status():
	for data in finergy.get_all("Project Update", {"date": today(), "sent": 0}):
		replies = finergy.get_all(
			"Communication",
		...
		for d in replies:
			doc = finergy.get_doc("Project Update", data.name)
			user_data = finergy.db.get_values(
			...
			doc.append(
				"users",
				{
					...
					"project_status": finergy.utils.md_to_html(
...
def send_project_status_email_to_users():
	...
	for d in finergy.get_all("Project Update", {"date": yesterday, "sent": 0}):
		doc = finergy.get_doc("Project Update", d.name)

		project_doc = finergy.get_doc("Project", doc.project)
...
def update_project_sales_billing():
	sales_update_frequency = finergy.db.get_single_value("Selling Settings", "sales_update_frequency")
	if sales_update_frequency == "Each Transaction":
		return
	elif sales_update_frequency == "Monthly" and finergy.utils.now_datetime().day != 1:
		return
	...
	project_map = {}
	for project_details in finergy.db.sql(
		"""
		...
		project = project_map.setdefault(
			project_details.name, finergy.get_doc("Project", project_details.name)
		)
...
def create_kanban_board_if_not_exists(project):
	from finergy.desk.doctype.kanban_board.kanban_board import quick_kanban_board

	project = finergy.get_doc("Project", project)
	if not finergy.db.exists("Kanban Board", project.project_name):
...
def set_project_status(project, status):
	...
	if not status in ("Completed", "Cancelled"):
		finergy.throw(_("Status must be Cancelled or Completed"))

	project = finergy.get_doc("Project", project)
	finergy.has_permission(doc=project, throw=True)

	for task in finergy.get_all("Task", dict(project=project.name)):
		finergy.db.set_value("Task", task.name, "status", status)
...
def get_holiday_list(company=None):
	if not company:
		company = get_default_company() or finergy.get_all("Company")[0].name

	holiday_list = finergy.get_cached_value("Company", company, "default_holiday_list")
	if not holiday_list:
		finergy.throw(
			_("Please set a default Holiday List for Company {0}").format(
				finergy.bold(get_default_company())
			)
		)
	return holiday_list
...
def get_users_email(doc):
	return [d.email for d in doc.users if finergy.db.get_value("User", d.user, "enabled")]
