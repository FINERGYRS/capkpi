...
from capkpi.projects.doctype.project_template.test_project_template import make_project_template
from capkpi.projects.doctype.task.test_task import create_task
from capkpi.selling.doctype.sales_order.sales_order import make_project as make_project_from_so
from capkpi.selling.doctype.sales_order.test_sales_order import make_sales_order

test_records = finergy.get_test_records("Project")
...
class TestProject(unittest.TestCase):
	def test_project_with_template_having_no_parent_and_depend_tasks(self):
		...
		finergy.db.sql(""" delete from tabTask where project = %s """, project_name)
		finergy.delete_doc("Project", project_name)
	...
	def test_project_template_having_parent_child_tasks(self):
		...
		if finergy.db.get_value("Project", {"project_name": project_name}, "name"):
			project_name = finergy.db.get_value("Project", {"project_name": project_name}, "name")
...
def make_project(args):
	...
	if args.project_name and finergy.db.exists("Project", {"project_name": args.project_name}):
		return finergy.get_doc("Project", {"project_name": args.project_name})
...
def task_exists(subject):
	result = finergy.db.get_list("Task", filters={"subject": subject}, fields=["name"])
	if not len(result):
		return False
	return finergy.get_doc("Task", result[0].name)
