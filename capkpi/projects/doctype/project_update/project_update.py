...
def email_sending(
	...
	msg += "</table>"
	if len(holiday) == 0:
		email = finergy.db.sql("""SELECT user from `tabProject User` WHERE parent = %s;""", project_name)
		for emails in email:
			finergy.sendmail(
				recipients=emails, subject=finergy._(project_name + " " + "Summary"), message=msg
			)
	else:
		pass
