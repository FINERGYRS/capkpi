...
from finergy.utils import add_to_date, flt, get_datetime, getdate, time_diff_in_hours
...
class OverWorkLoggedError(finergy.ValidationError):
	pass
...
class Timesheet(Document):
	...
	def validate_mandatory_fields(self):
		for data in self.time_logs:
			if not data.from_time and not data.to_time:
				finergy.throw(_("Row {0}: From Time and To Time is mandatory.").format(data.idx))

			if not data.activity_type and self.employee:
				finergy.throw(_("Row {0}: Activity Type is mandatory.").format(data.idx))

			if flt(data.hours) == 0.0:
				finergy.throw(_("Row {0}: Hours value must be greater than zero.").format(data.idx))
	...
	def update_task_and_project(self):
		...
		for data in self.time_logs:
			if data.task and data.task not in tasks:
				task = finergy.get_doc("Task", data.task)
				task.update_time_and_costing()
				task.save()
				tasks.append(data.task)

			elif data.project and data.project not in projects:
				finergy.get_doc("Project", data.project).update_project()
	...
	def validate_overlap(self, data):
		settings = finergy.get_single("Projects Settings")
		self.validate_overlap_for("user", data, self.user, settings.ignore_user_time_overlap)
		self.validate_overlap_for("employee", data, self.employee, settings.ignore_employee_time_overlap)

	def set_project(self, data):
		data.project = data.project or finergy.db.get_value("Task", data.task, "project")
	...
	def get_overlap_for(self, fieldname, args, value):
		timesheet = finergy.qb.DocType("Timesheet")
		timelog = finergy.qb.DocType("Timesheet Detail")

		from_time = get_datetime(args.from_time)
		to_time = get_datetime(args.to_time)

		existing = (
			finergy.qb.from_(timesheet)
			.join(timelog)
...
def get_projectwise_timesheet_data(project=None, parent=None, from_time=None, to_time=None):
	...
	return finergy.db.sql(query, filters, as_dict=1)


@finergy.whitelist()
def get_timesheet_detail_rate(timelog, currency):
	timelog_detail = finergy.db.sql(
...
def make_sales_invoice(source_name, item_code=None, customer=None, currency=None):
	target = finergy.new_doc("Sales Invoice")
	timesheet = finergy.get_doc("Timesheet", source_name)

	if not timesheet.total_billable_hours:
		finergy.throw(_("Invoice can't be made for zero billing hour"))

	if timesheet.total_billable_hours == timesheet.total_billed_hours:
		finergy.throw(_("Invoice already created for all billing hours"))
...
def make_salary_slip(source_name, target_doc=None):
	target = finergy.new_doc("Salary Slip")
...
def set_missing_values(time_sheet, target):
	doc = finergy.get_doc("Timesheet", time_sheet)
...
def get_timesheets_list(
	...
	contact = finergy.db.exists("Contact", {"user": user})
	if contact:
		# find customer
	...
	if customer:
		sales_invoices = [
			d.name for d in finergy.get_all("Sales Invoice", filters={"customer": customer})
		] or [None]
		projects = [d.name for d in finergy.get_all("Project", filters={"customer": customer})]
