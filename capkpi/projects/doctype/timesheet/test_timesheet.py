...
import finergy
from finergy.utils import add_months, add_to_date, now_datetime, nowdate
...
class TestTimesheet(unittest.TestCase):
	...
	def test_salary_slip_from_timesheet(self):
		...
		timesheet = finergy.get_doc("Timesheet", timesheet.name)
	...
	def test_timesheet_billing_based_on_project(self):
		...
		ts = finergy.get_doc("Timesheet", timesheet.name)
	...
	def test_timesheet_time_overlap(self):
		...
		timesheet = finergy.new_doc("Timesheet")
		...
		self.assertRaises(finergy.ValidationError, timesheet.save)
...
def make_salary_structure_for_timesheet(employee, company=None):
	...
	if not finergy.db.exists("Salary Component", "Timesheet Component"):
		finergy.get_doc(
	...
	if not finergy.db.get_value("Salary Structure Assignment", {"employee": employee, "docstatus": 1}):
		finergy.db.set_value("Employee", employee, "date_of_joining", add_months(nowdate(), -5))
...
def update_activity_type(activity_type):
	activity_type = finergy.get_doc("Activity Type", activity_type)
