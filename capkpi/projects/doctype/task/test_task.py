...
import finergy
from finergy.utils import add_days, getdate, nowdate

from capkpi.projects.doctype.task.task import CircularReferenceError
...
class TestTask(unittest.TestCase):
	...
	def test_reschedule_dependent_task(self):
		...
		self.assertEqual(
			finergy.db.get_value("Task", task2.name, "exp_start_date"), getdate(add_days(nowdate(), 21))
		)
		self.assertEqual(
			finergy.db.get_value("Task", task2.name, "exp_end_date"), getdate(add_days(nowdate(), 25))
		)

		self.assertEqual(
			finergy.db.get_value("Task", task3.name, "exp_start_date"), getdate(add_days(nowdate(), 26))
		)
		self.assertEqual(
			finergy.db.get_value("Task", task3.name, "exp_end_date"), getdate(add_days(nowdate(), 30))
		)

	def test_close_assignment(self):
		if not finergy.db.exists("Task", "Test Close Assignment"):
			task = finergy.new_doc("Task")
	...
	def test_overdue(self):
		...
		from capkpi.projects.doctype.task.task import set_tasks_as_overdue

		set_tasks_as_overdue()

		self.assertEqual(finergy.db.get_value("Task", task.name, "status"), "Overdue")
...
def create_task(
	...
	if not finergy.db.exists("Task", subject):
		task = finergy.new_doc("Task")
		task.status = "Open"
		...
		task.project = (
			project or None
			if is_template
			else finergy.get_value("Project", {"project_name": "_Test Project"})
		)
		...
		task = finergy.get_doc("Task", subject)
