...
from finergy.utils import get_link_to_form, get_weekdays, getdate, nowdate
...
class ServiceLevelAgreement(Document):
	...
	def check_priorities(self):
		...
		if not len(set(default_priority)) == len(default_priority):
			finergy.throw(_("Select only one Priority as Default."))
	...
	def validate_doc(self):
		if (
			not finergy.db.get_single_value("Support Settings", "track_service_level_agreement")
			and self.enable
		):
			finergy.throw(
		...
		if self.default_service_level_agreement:
			if finergy.db.exists(
				...
				finergy.throw(_("A Default Service Level Agreement already exists."))
		else:
			if self.start_date and self.end_date:
				if getdate(self.start_date) >= getdate(self.end_date):
					finergy.throw(_("Start Date of Agreement can't be greater than or equal to End Date."))

				if getdate(self.end_date) < getdate(finergy.utils.getdate()):
					finergy.throw(_("End Date of Agreement can't be less than today."))
	...
	def validate_condition(self):
		temp_doc = finergy.new_doc("Issue")
		if self.condition:
			try:
...
def check_agreement_status():
	service_level_agreements = finergy.get_list(
		"Service Level Agreement",
	...
	for service_level_agreement in service_level_agreements:
		...
		if doc.end_date and getdate(doc.end_date) < getdate(finergy.utils.getdate()):
			finergy.db.set_value("Service Level Agreement", service_level_agreement.name, "active", 0)
...
def get_customer_group(customer):
	if customer:
		return finergy.db.get_value("Customer", customer, "customer_group")


def get_customer_territory(customer):
	if customer:
		return finergy.db.get_value("Customer", customer, "territory")
...
def get_service_level_agreement_filters(name, customer=None):
	...
	return {
		"priority": [
			priority.priority
			for priority in finergy.get_list(
				"Service Level Priority", filters={"parent": name}, fields=["priority"]
			)
		],
		"service_level_agreements": [
			d.name
			for d in finergy.get_list("Service Level Agreement", filters=filters, or_filters=or_filters)
		],
	}
