...
from finergy.email.inbox import link_communication_to_document
...
from capkpi.support.doctype.service_level_agreement.service_level_agreement import (
...
class Issue(Document):
	...
	def validate(self):
		...
		if not self.raised_by:
			self.raised_by = finergy.session.user
	...
	def set_lead_contact(self, email_id):
		...
		if email_id:
			if not self.lead:
				self.lead = finergy.db.get_value("Lead", {"email_id": email_id})

			if not self.contact and not self.customer:
				self.contact = finergy.db.get_value("Contact", {"email_id": email_id})

				if self.contact:
					contact = finergy.get_doc("Contact", self.contact)
					self.customer = contact.get_link_for("Customer")

			if not self.company:
				self.company = finergy.db.get_value("Lead", self.lead, "company") or finergy.db.get_default(
					"Company"
				)
	...
	def update_status(self):
		status = finergy.db.get_value("Issue", self.name, "status")
		if self.status != "Open" and status == "Open" and not self.first_responded_on:
			self.first_responded_on = finergy.flags.current_time or now_datetime()

		if self.status in ["Closed", "Resolved"] and status not in ["Resolved", "Closed"]:
			self.resolution_date = finergy.flags.current_time or now_datetime()
			if finergy.db.get_value("Issue", self.name, "agreement_status") == "Ongoing":
	...
	def handle_hold_time(self, status):
		if self.service_level_agreement:
			...
			pause_sla_on = finergy.db.get_all(
			...
			if hold_statuses:
				if self.status in hold_statuses and status not in hold_statuses:
					update_values["on_hold_since"] = finergy.flags.current_time or now_datetime()
				...
				if self.status not in hold_statuses and status in hold_statuses:
					...
					now_time = finergy.flags.current_time or now_datetime()
					last_hold_time = 0
	...
	def update_agreement_status(self):
		if self.service_level_agreement and self.agreement_status == "Ongoing":
			if (
				cint(finergy.db.get_value("Issue", self.name, "response_by_variance")) < 0
				or cint(finergy.db.get_value("Issue", self.name, "resolution_by_variance")) < 0
			):
	...
	def split_issue(self, subject, communication_id):
		...
		finergy.get_doc(replicated_issue).insert()

		# Replicate linked Communications
		# TODO: get all communications in timeline before this, and modify them to append them to new doc
		comm_to_split_from = finergy.get_doc("Communication", communication_id)
		...
		for communication in communications:
			doc = finergy.get_doc("Communication", communication.name)
		...
		finergy.get_doc(
			{
				...
				"content": " - Split the Issue from <a href='/app/Form/Issue/{0}'>{1}</a>".format(
					self.name, finergy.bold(self.name)
				),
			}
	...
	def before_insert(self):
		if finergy.db.get_single_value("Support Settings", "track_service_level_agreement"):
	...
	def set_response_and_resolution_time(self, priority=None, service_level_agreement=None):
		...
		if not service_level_agreement:
			if finergy.db.get_value("Issue", self.name, "service_level_agreement"):
				finergy.throw(
...
def get_priority(issue):
	service_level_agreement = finergy.get_doc("Service Level Agreement", issue.service_level_agreement)
...
def set_service_level_agreement_variance(issue=None):
	current_time = finergy.flags.current_time or now_datetime()

	filters = {"status": "Open", "agreement_status": "Ongoing"}
	if issue:
		filters = {"name": issue}

	for issue in finergy.get_list("Issue", filters=filters):
		doc = finergy.get_doc("Issue", issue.name)
...
def get_issue_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by=None):
	...
	contact = finergy.db.get_value("Contact", {"user": user}, "name")
	customer = None

	if contact:
		contact_doc = finergy.get_doc("Contact", contact)
...
def auto_close_tickets():
	...
	auto_close_after_days = (
		finergy.db.get_value("Support Settings", "Support Settings", "close_issue_after_days") or 7
	)

	issues = finergy.db.sql(
	...
	for issue in issues:
		doc = finergy.get_doc("Issue", issue.get("name"))
		doc.status = "Closed"
...
def has_website_permission(doc, ptype, user, verbose=False):
	from capkpi.controllers.website_list_for_contact import has_website_permission
...
def update_issue(contact, method):
	...
	finergy.db.sql("""UPDATE `tabIssue` set contact='' where contact=%s""", contact.name)


def get_holidays(holiday_list_name):
	holiday_list = finergy.get_cached_doc("Holiday List", holiday_list_name)
...
def make_issue_from_communication(communication, ignore_communication_links=False):
	...
	doc = finergy.get_doc("Communication", communication)
	issue = finergy.get_doc(
		{
			"doctype": "Issue",
			"subject": doc.subject,
...
def calculate_first_response_time(issue, first_responded_on):
	...
	support_hours = finergy.get_cached_doc(
...
def get_working_hours(date, support_hours):
	if is_work_day(date, support_hours):
		weekday = finergy.utils.get_weekday(date)
