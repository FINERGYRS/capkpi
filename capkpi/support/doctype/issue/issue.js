finergy.ui.form.on("Issue", {
	onload: function(frm) {
		...
		finergy.db.get_value("Support Settings", {name: "Support Settings"},
		...
		if (frm.doc.status !== "Closed") {
			...
			frm.add_custom_button(__("Task"), function() {
				finergy.model.open_mapped_doc({
					method: "capkpi.support.doctype.issue.issue.make_task",
					frm: frm
				});
			}, __("Create"));
	...
	reset_service_level_agreement: function(frm) {
		let reset_sla = new finergy.ui.Dialog({
			...
			primary_action: (values) => {
				...
				finergy.call("capkpi.support.doctype.service_level_agreement.service_level_agreement.reset_service_level_agreement", {
					reason: values.reason,
					user: finergy.session.user_email
				}, () => {
					reset_sla.enable_primary_action();
					frm.refresh();
					finergy.msgprint(__("Service Level Agreement was reset."));
				});
			}
		});
	...
	timeline_refresh: function(frm) {
					...
					var dialog = new finergy.ui.Dialog({
						...
						primary_action: () => {
							frm.call("split_issue", {
								...
								finergy.msgprint(`New issue created: <a href="/app/issue/${r.message}">${r.message}</a>`);
								frm.reload_doc();
								dialog.hide();
							});
						}
		...
		// if (finergy.model.can_create("Help Article")) {
		...
		// 			${finergy.utils.icon('solid-info', 'sm')}
		// 		</a>
		// 	`);
		...
		// 			const doc = finergy.model.get_new_doc("Help Article");
		// 			doc.title = frm.doc.subject;
		// 			doc.content = content;
		// 			finergy.set_route("Form", "Help Article", doc.name);
		// 		});
		// 	}
