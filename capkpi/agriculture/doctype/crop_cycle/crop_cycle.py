...
class CropCycle(Document):
	...
	def set_missing_values(self):
		crop = finergy.get_doc("Crop", self.crop)
	...
	def import_disease_tasks(self, disease, start_date):
		disease_doc = finergy.get_doc("Disease", disease)
	...
	def reload_linked_analysis(self):
		...
		for doctype in linked_doctypes:
			output[doctype] = finergy.get_all(doctype, fields=required_fields)

		output["Location"] = []

		for location in self.linked_location:
			output["Location"].append(finergy.get_doc("Location", location.location))

		finergy.publish_realtime("List of Linked Docs", output, user=finergy.session.user)
