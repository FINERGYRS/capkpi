...
def get_context(context):
	...
	try:
		...
		content = finergy.form_dict["content"]
		content_type = finergy.form_dict["type"]
	...
	if finergy.session.user == "Guest" or not has_program_access or not has_content_access:
		finergy.local.flags.redirect_location = "/lms"
		raise finergy.Redirect

	# Set context for content to be displayer
	context.content = finergy.get_doc(content_type, content).as_dict()
	context.content_type = content_type
	context.program = program
	context.course = course
	context.topic = topic

	topic = finergy.get_doc("Topic", topic)
	content_list = [
...
def allowed_content_access(program, content, content_type):
	contents_of_program = finergy.db.sql(
