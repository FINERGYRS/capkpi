...
def get_context(context):
	is_enabled = finergy.db.get_single_value("Appointment Booking Settings", "enable_scheduling")
	if is_enabled:
		return context
	else:
...
def get_appointment_settings():
	settings = finergy.get_doc("Appointment Booking Settings")
	settings.holiday_list = finergy.get_doc("Holiday List", settings.holiday_list)
	return settings
...
def get_appointment_slots(date, timezone):
	...
	# Database queries
	settings = finergy.get_doc("Appointment Booking Settings")
	holiday_list = finergy.get_doc("Holiday List", settings.holiday_list)
...
def create_appointment(date, time, tz, contact):
	...
	appointment = finergy.new_doc("Appointment")
...
def convert_to_guest_timezone(guest_tz, datetimeobject):
	...
	local_timezone = pytz.timezone(finergy.utils.get_time_zone())
...
def convert_to_system_timezone(guest_tz, datetimeobject):
	...
	system_tz = pytz.timezone(finergy.utils.get_time_zone())
	datetimeobject = datetimeobject.astimezone(system_tz)
	return datetimeobject


def check_availabilty(timeslot, settings):
	return finergy.db.count("Appointment", {"scheduled_time": timeslot}) < settings.number_of_agents
