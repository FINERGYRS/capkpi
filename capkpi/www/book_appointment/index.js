...
async function get_global_variables() {
    ...
    window.appointment_settings = (await finergy.call({
        method: 'capkpi.www.book_appointment.index.get_appointment_settings'
    })).message;
    window.timezones = (await finergy.call({
        method:'capkpi.www.book_appointment.index.get_timezones'
    })).message;
...
function hide_next_button() {
    ...
    next_button.onclick = () => finergy.msgprint(__("Please select a date and time"));
}
...
function on_date_or_timezone_select() {
    ...
    if (date_picker.value === '') {
        clear_time_slots();
        hide_next_button();
        finergy.throw(__('Please select a date'));
    }
...
async function get_time_slots(date, timezone) {
    let slots = (await finergy.call({
        method: 'capkpi.www.book_appointment.index.get_appointment_slots',
        args: {
            date: date,
            timezone: timezone
        }
...
async function submit() {
    ...
    let appointment =  finergy.call({
        method: 'capkpi.www.book_appointment.index.create_appointment',
        args: {
        ...
        callback: (response)=>{
            if (response.message.status == "Unverified") {
                finergy.show_alert("Please check your email to confirm the appointment")
            } else {
                finergy.show_alert("Appointment Created Successfully");
            }
            setTimeout(()=>{
                let redirect_url = "/";
        ...
        error: (err)=>{
            finergy.show_alert("Something went wrong please try again");
            button.disabled = false;
        }
    });
}
