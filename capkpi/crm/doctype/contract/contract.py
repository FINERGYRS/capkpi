...
class Contract(Document):
	def autoname(self):
		if finergy.db.get_single_value("Selling Settings", "contract_naming_by") == "Naming Series":
			...
			if finergy.db.exists("Contract", name):
	...
	def before_submit(self):
		self.signed_by_company = finergy.session.user
...
def update_status_for_contracts():
	...
	for contract in contracts:
		...
		finergy.db.set_value("Contract", contract.get("name"), "status", status)
