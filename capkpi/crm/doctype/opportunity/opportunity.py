...
class Opportunity(TransactionBase):
	...
	def validate(self):
		self._prev = finergy._dict(
			{
				"contact_date": finergy.db.get_value("Opportunity", self.name, "contact_date")
				if (not cint(self.get("__islocal")))
				else None,
				"contact_by": finergy.db.get_value("Opportunity", self.name, "contact_by")
				if (not cint(self.get("__islocal")))
				else None,
			}
		)
	...
	def map_fields(self):
		for field in self.meta.fields:
			if not self.get(field.fieldname):
				try:
					value = finergy.db.get_value(self.opportunity_from, self.party_name, field.fieldname)
					finergy.db.set(self, field.fieldname, value)
				except Exception:
					continue
	...
	def add_calendar_event(self, opts=None, force=False):
		...
		if self.to_discuss:
			opts.description += f" To Discuss : {finergy.render_template(self.to_discuss, {'doc': self})}"
...
def auto_close_opportunity():
	...
	auto_close_after_days = (
		finergy.db.get_single_value("Selling Settings", "close_opportunity_after_days") or 15
	)

	opportunities = finergy.db.sql(
	...
	for opportunity in opportunities:
		doc = finergy.get_doc("Opportunity", opportunity.get("name"))
		doc.status = "Closed"
