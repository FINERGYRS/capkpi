...
from finergy.query_builder import DocType, Interval
from finergy.query_builder.functions import Now
from finergy.utils import flt, get_fullname

from capkpi.crm.utils import (
	CRMNote,
	copy_comments,
	link_communications,
	link_open_events,
...
class Opportunity(TransactionBase, CRMNote):
	def onload(self):
		ref_doc = finergy.get_doc(self.opportunity_from, self.party_name)
		load_address_and_contact(ref_doc)
		self.set("__onload", ref_doc.get("__onload"))

	def after_insert(self):
		if self.opportunity_from == "Lead":
			finergy.get_doc("Lead", self.party_name).set_status(update=True)
			self.disable_lead()

			link_open_tasks(self.opportunity_from, self.party_name, self)
			link_open_events(self.opportunity_from, self.party_name, self)
			if finergy.db.get_single_value("CRM Settings", "carry_forward_communication_and_comments"):
	...
	def map_fields(self):
		for field in self.meta.get_valid_columns():
			if not self.get(field) and finergy.db.field_exists(self.opportunity_from, field):
				try:
					value = finergy.db.get_value(self.opportunity_from, self.party_name, field)
					finergy.db.set(self, field, value)
				except Exception:
					continue

	def set_exchange_rate(self):
		company_currency = finergy.get_cached_value("Company", self.company, "default_currency")
	...
	def update_prospect(self):
		...
		if self.opportunity_from == "Prospect" and self.party_name:
			...
			prospect_name = finergy.db.get_value("Prospect Lead", {"lead": self.party_name}, "parent")

		if prospect_name:
			prospect = finergy.get_doc("Prospect", prospect_name)
	...
	def disable_lead(self):
		if self.opportunity_from == "Lead":
			finergy.db.set_value("Lead", self.party_name, {"disabled": 1, "docstatus": 1})
	...
	def make_new_lead_if_required(self):
		...
		if (not self.get("party_name")) and self.contact_email:
			...
			lead_name = finergy.db.get_value("Lead", {"email_id": self.contact_email})
			if not lead_name:
	...
	def declare_enquiry_lost(self, lost_reasons_list, competitors, detailed_reason=None):
		if not self.has_active_quotation():
			...
			finergy.throw(_("Cannot declare as lost, because Quotation has been made."))
	...
	def validate_cust_name(self):
		if self.party_name:
			if self.opportunity_from == "Customer":
				self.customer_name = finergy.db.get_value("Customer", self.party_name, "customer_name")
			elif self.opportunity_from == "Lead":
				customer_name = finergy.db.get_value("Prospect Lead", {"lead": self.party_name}, "parent")
				if not customer_name:
					lead_name, company_name = finergy.db.get_value(
	...
	def validate_item_details(self):
		...
		for d in self.items:
			...
			item = finergy.db.get_value("Item", d.item_code, item_fields, as_dict=True)
...
def make_quotation(source_name, target_doc=None):
	def set_missing_values(source, target):
		from capkpi.controllers.accounts_controller import get_default_taxes_and_charges

		quotation = finergy.get_doc(target)

		company_currency = finergy.get_cached_value("Company", quotation.company, "default_currency")
...
def set_multiple_status(names, status):
	...
	for name in names:
		opp = finergy.get_doc("Opportunity", name)
		opp.status = status
		opp.save()
...
def auto_close_opportunity():
	...
	auto_close_after_days = (
		finergy.db.get_single_value("CRM Settings", "close_opportunity_after_days") or 15
	)

	table = finergy.qb.DocType("Opportunity")
	opportunities = (
	...
	for opportunity in opportunities:
		doc = finergy.get_doc("Opportunity", opportunity)
		doc.status = "Closed"
...
def make_opportunity_from_communication(communication, company, ignore_communication_links=False):
	from capkpi.crm.doctype.lead.lead import make_lead_from_communication
