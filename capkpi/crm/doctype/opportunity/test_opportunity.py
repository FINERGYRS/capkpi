...
import finergy
from finergy.utils import add_days, now_datetime, random_string, today

from capkpi.crm.doctype.lead.lead import make_customer
from capkpi.crm.doctype.lead.test_lead import make_lead
from capkpi.crm.doctype.opportunity.opportunity import make_quotation
from capkpi.crm.utils import get_linked_communication_list

test_records = finergy.get_test_records("Opportunity")
...
class TestOpportunity(unittest.TestCase):
	def test_opportunity_status(self):
		...
		doc = finergy.get_doc("Opportunity", doc.name)
	...
	def test_make_new_lead_if_required(self):
		...
		self.assertEqual(
			finergy.db.get_value("Lead", opp_doc.party_name, "email_id"), opp_doc.contact_email
		)
	...
	def test_carry_forward_of_email_and_comments(self):
		...
		opportunity_comment_count = finergy.db.count(
...
def make_opportunity_from_lead():
	...
	opp_doc = finergy.get_doc(args).insert(ignore_permissions=True)
