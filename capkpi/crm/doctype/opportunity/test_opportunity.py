...
class TestOpportunity(unittest.TestCase):
	...
	def test_make_new_lead_if_required(self):
		...
		self.assertEqual(finergy.db.get_value("Lead", opp_doc.party_name, "email_id"), new_lead_email_id)
	...
	def test_render_template_for_to_discuss(self):
		...
		event = finergy.get_all(
			"Event Participants",
			fields=["parent"],
			filters={"reference_doctype": doc.doctype, "reference_docname": doc.name},
		)

		event_description = finergy.db.get_value("Event", event[0].parent, "description")
