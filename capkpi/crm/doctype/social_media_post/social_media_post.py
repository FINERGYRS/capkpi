...
class SocialMediaPost(Document):
	def validate(self):
		if not self.twitter and not self.linkedin:
			finergy.throw(_("Select atleast one Social Media Platform to Share on."))

		if self.scheduled_time:
			current_time = finergy.utils.now_datetime()
			scheduled_time = finergy.utils.get_datetime(self.scheduled_time)
			if scheduled_time < current_time:
				finergy.throw(_("Scheduled Time must be a future time."))

		if self.text and len(self.text) > 280:
			finergy.throw(_("Tweet length must be less than 280."))
	...
	def post(self):
		try:
			...
			finergy.log_error(message=finergy.get_traceback(), title=title)
...
def process_scheduled_social_media_posts():
	...
	start = finergy.utils.now_datetime()
	end = start + datetime.timedelta(minutes=10)
	for post in posts:
		if post.scheduled_time:
			post_time = finergy.utils.get_datetime(post.scheduled_time)
			if post_time > start and post_time <= end:
				sm_post = finergy.get_doc("Social Media Post", post.name)
				sm_post.post()
