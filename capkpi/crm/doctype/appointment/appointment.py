...
class Appointment(Document):
	...
	def find_customer_by_email(self):
		customer_list = finergy.get_list(
	...
	def before_insert(self):
		number_of_appointments_in_same_slot = finergy.db.count(
			"Appointment", filters={"scheduled_time": self.scheduled_time}
		)
		number_of_agents = finergy.db.get_single_value("Appointment Booking Settings", "number_of_agents")
		if not number_of_agents == 0:
			if number_of_appointments_in_same_slot >= number_of_agents:
				finergy.throw(_("Time slot is not available"))
		# Link lead
		if not self.party:
	...
	def send_confirmation_email(self):
		...
		args = {
			"link": verify_url,
			"site_url": finergy.utils.get_url(),
		...
		if finergy.session.user == "Guest":
			finergy.msgprint(_("Please check your email to confirm the appointment"))
		else:
			finergy.msgprint(
	...
	def on_change(self):
		...
		if not self.calendar_event:
			return
		cal_event = finergy.get_doc("Event", self.calendar_event)
		cal_event.starts_on = self.scheduled_time
		cal_event.save(ignore_permissions=True)

	def set_verified(self, email):
		if not email == self.customer_email:
			finergy.throw(_("Email verification failed."))
		# Create new lead
	...
	def auto_assign(self):
		from finergy.desk.form.assign_to import add as add_assignemnt
	...
	def get_assignee_from_latest_opportunity(self):
		if not self.party:
			return None
		if not finergy.db.exists("Lead", self.party):
			return None
		opporutnities = finergy.get_list(
			"Opportunity",
			filters={
		...
		if not opporutnities:
			return None
		latest_opportunity = finergy.get_doc("Opportunity", opporutnities[0].name)
		assignee = latest_opportunity._assign
		if not assignee:
			return None
		assignee = finergy.parse_json(assignee)[0]
		return assignee

	def create_calendar_event(self):
		if self.calendar_event:
			return
		appointment_event = finergy.get_doc(
			{
				"doctype": "Event",
				...
				"status": "Open",
				"type": "Public",
				"send_reminder": finergy.db.get_single_value("Appointment Booking Settings", "email_reminders"),
				"event_participants": [
...
def _get_agents_sorted_by_asc_workload(date):
	appointments = finergy.db.get_list("Appointment", fields="*")
	agent_list = _get_agent_list_as_strings()
	if not appointments:
		return agent_list
	appointment_counter = Counter(agent_list)
	for appointment in appointments:
		assigned_to = finergy.parse_json(appointment._assign)
		if not assigned_to:
			continue
...
def _get_agent_list_as_strings():
	...
	agent_list = finergy.get_doc("Appointment Booking Settings").agent_list
...
def _check_agent_availability(agent_email, scheduled_time):
	appointemnts_at_scheduled_time = finergy.get_list(
...
def _get_employee_from_user(user):
	employee_docname = finergy.db.exists({"doctype": "Employee", "user_id": user})
	if employee_docname:
		# finergy.db.exists returns a tuple of a tuple
		return finergy.get_doc("Employee", employee_docname[0][0])
	return None
