...
def create_test_lead():
	test_lead = finergy.db.exists({"doctype": "Lead", "lead_name": "Test Lead"})
	if test_lead:
		return finergy.get_doc("Lead", test_lead[0][0])
	test_lead = finergy.get_doc(
...
def create_test_appointments():
	test_appointment = finergy.db.exists(
		{
			"doctype": "Appointment",
	...
	if test_appointment:
		return finergy.get_doc("Appointment", test_appointment[0][0])
	test_appointment = finergy.get_doc(
		{
			"doctype": "Appointment",
			"email": "test@example.com",
			"status": "Open",
...
class TestAppointment(unittest.TestCase):
	...
	def test_calendar_event_created(self):
		cal_event = finergy.get_doc("Event", self.test_appointment.calendar_event)
		self.assertEqual(cal_event.starts_on, self.test_appointment.scheduled_time)

	def test_lead_linked(self):
		lead = finergy.get_doc("Lead", self.test_lead.name)
