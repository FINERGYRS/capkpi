...
def get_last_interaction(contact=None, lead=None):
	...
	if contact:
		...
		if query_condition:
			...
			last_communication = finergy.db.sql(
				"""
				SELECT `name`, `content`
	...
	if lead:
		last_communication = finergy.get_all(
			"Communication",
...
def get_scheduled_employees_for_popup(communication_medium):
	...
	now_time = finergy.utils.nowtime()
	weekday = finergy.utils.get_weekday()

	available_employee_groups = finergy.get_all(
