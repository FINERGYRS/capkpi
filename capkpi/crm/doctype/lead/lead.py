...
from finergy.utils import comma_and, get_link_to_form, has_gravatar, validate_email_address

from capkpi.accounts.party import set_taxes
from capkpi.controllers.selling_controller import SellingController
from capkpi.crm.utils import CRMNote, copy_comments, link_communications, link_open_events
...
class Lead(SellingController, CRMNote):
	...
	def onload(self):
		customer = finergy.db.get_value("Customer", {"lead_name": self.name})
	...
	def before_insert(self):
		...
		if finergy.db.get_single_value("CRM Settings", "auto_creation_of_contact"):
	...
	def on_trash(self):
		finergy.db.sql("""update `tabIssue` set lead='' where lead=%s""", self.name)
	...
	def set_lead_name(self):
		if not self.lead_name:
			...
			if not self.company_name and not self.email_id and not self.flags.ignore_mandatory:
				finergy.throw(_("A Lead requires either a person's name or an organization's name"))
	...
	def check_email_id_is_unique(self):
		if self.email_id:
			...
			if not finergy.db.get_single_value("CRM Settings", "allow_lead_duplication_based_on_emails"):
				duplicate_leads = finergy.get_all(
					"Lead", filters={"email_id": self.email_id, "name": ["!=", self.name]}
				)
				duplicate_leads = [
					finergy.bold(get_link_to_form("Lead", lead.name)) for lead in duplicate_leads
				]
	...
	def validate_email_id(self):
		if self.email_id:
			...
			if self.email_id == self.lead_owner:
				finergy.throw(_("Lead Owner cannot be same as the Lead Email Address"))
	...
	def update_prospect(self):
		lead_row_name = finergy.db.get_value(
			"Prospect Lead", filters={"lead": self.name}, fieldname="name"
		)
		if lead_row_name:
			lead_row = finergy.get_doc("Prospect Lead", lead_row_name)
			lead_row.update(
				{
	...
	def unlink_dynamic_links(self):
		...
		for link in links:
			linked_doc = finergy.get_doc(link["parenttype"], link["parent"])
	...
	def remove_link_from_prospect(self):
		...
		for d in prospects:
			prospect = finergy.get_doc("Prospect", d.parent)
	...
	def has_customer(self):
		return finergy.db.get_value("Customer", {"lead_name": self.name})

	def has_opportunity(self):
		return finergy.db.get_value("Opportunity", {"party_name": self.name, "status": ["!=", "Lost"]})
	...
	def create_prospect(self, company_name):
		try:
			prospect = finergy.new_doc("Prospect")
			...
			finergy.throw(_("Prospect {0} already exists").format(company_name or self.company_name))
...
def _make_customer(source_name, target_doc=None, ignore_permissions=False):
	def set_missing_values(source, target):
		...
		target.customer_group = finergy.db.get_default("Customer Group")
...
def get_lead_details(lead, posting_date=None, company=None):
	...
	from capkpi.accounts.party import set_address_details

	out = finergy._dict()

	lead_doc = finergy.get_doc("Lead", lead)
	lead = lead_doc
...
def make_lead_from_communication(communication, ignore_communication_links=False):
	...
	lead_name = None
	if doc.sender:
		lead_name = finergy.db.get_value("Lead", {"email_id": doc.sender})
	if not lead_name and doc.phone_no:
		lead_name = finergy.db.get_value("Lead", {"mobile_no": doc.phone_no})
	if not lead_name:
...
def add_lead_to_prospect(lead, prospect):
	prospect = finergy.get_doc("Prospect", prospect)
	prospect.append("leads", {"lead": lead})
	prospect.save(ignore_permissions=True)

	carry_forward_communication_and_comments = finergy.db.get_single_value(
	...
	finergy.msgprint(
		_("Lead {0} has been added to prospect {1}.").format(
			finergy.bold(lead), finergy.bold(prospect.name)
		),
