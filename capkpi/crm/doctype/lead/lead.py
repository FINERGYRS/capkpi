...
from finergy.utils import comma_and, cstr, getdate, has_gravatar, nowdate, validate_email_address
...
class Lead(SellingController):
	...
	def validate_email_id(self):
		if self.email_id:
			...
			if self.email_id == self.lead_owner:
				finergy.throw(_("Lead Owner cannot be same as the Lead"))

			if self.email_id == self.contact_by:
				finergy.throw(_("Next Contact By cannot be same as the Lead Email Address"))
	...
	def validate_contact_date(self):
		if self.contact_date and getdate(self.contact_date) < getdate(nowdate()):
			finergy.throw(_("Next Contact Date cannot be in the past"))

		if self.ends_on and self.contact_date and (getdate(self.ends_on) < getdate(self.contact_date)):
			finergy.throw(_("Ends On date cannot be before Next Contact Date."))
	...
	def set_prev(self):
		if self.is_new():
			self._prev = finergy._dict({"contact_date": None, "ends_on": None, "contact_by": None})
		else:
			self._prev = finergy.db.get_value(
...
def daily_open_lead():
	leads = finergy.get_all("Lead", filters=[["contact_date", "Between", [nowdate(), nowdate()]]])
	for lead in leads:
		finergy.db.set_value("Lead", lead.name, "status", "Open")
