...
capkpi.LeadController = class LeadController extends finergy.ui.form.Controller {
	setup () {
	...
	refresh () {
		...
		if (!this.frm.is_new()) {
			finergy.contacts.render_address_and_contact(this.frm);
		} else {
			finergy.contacts.clear_address_and_contact(this.frm);
		}
	...
	add_lead_to_prospect () {
		...
		function(data) {
			finergy.call({
				method: 'capkpi.crm.doctype.lead.lead.add_lead_to_prospect',
				args: {
	...
	make_customer () {
		finergy.model.open_mapped_doc({
			method: "capkpi.crm.doctype.lead.lead.make_customer",
			frm: cur_frm
		})
	}
	...
	make_quotation () {
		finergy.model.open_mapped_doc({
			method: "capkpi.crm.doctype.lead.lead.make_quotation",
			frm: cur_frm
		})
	}

	make_prospect () {
		finergy.model.with_doctype("Prospect", function() {
			let prospect = finergy.model.get_new_doc("Prospect");
			...
			let leads_row = finergy.model.add_child(prospect, 'leads');
			leads_row.lead = cur_frm.doc.name;

			finergy.set_route("Form", "Prospect", prospect.name);
		});
	}
...
extend_cscript(cur_frm.cscript, new capkpi.LeadController({ frm: cur_frm }));

finergy.ui.form.on("Lead", {
	make_opportunity: async function(frm) {
		let existing_prospect = (await finergy.db.get_value("Prospect Lead",
			{
				"lead": frm.doc.name
			},
			"name", null, "Prospect"
		...
		let existing_contact = (await finergy.db.get_value("Contact",
			{
