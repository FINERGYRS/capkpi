...
import finergy
from finergy.utils import random_string, today

from capkpi.crm.doctype.lead.lead import make_opportunity
from capkpi.crm.utils import get_linked_prospect
...
class TestLead(unittest.TestCase):
	...
	def test_create_lead_and_unlinking_dynamic_links(self):
		...
		lead_doc.delete()
		address_1.reload()
		self.assertEqual(finergy.db.exists("Lead", lead_doc.name), None)
		self.assertEqual(len(address_1.get("links")), 1)

	def test_prospect_creation_from_lead(self):
		finergy.db.sql("delete from `tabLead` where lead_name='Rahul Tripathi'")
		finergy.db.sql("delete from `tabProspect` where name='Prospect Company'")
	...
	def test_opportunity_from_lead(self):
		finergy.db.sql("delete from `tabLead` where lead_name='Rahul Tripathi'")
		finergy.db.sql("delete from `tabOpportunity` where party_name='Rahul Tripathi'")
