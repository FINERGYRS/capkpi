...
finergy.ui.form.on('Prospect', {
	refresh (frm) {
		finergy.dynamic_link = { doc: frm.doc, fieldname: "name", doctype: frm.doctype };

		if (!frm.is_new() && finergy.boot.user.can_create.includes("Customer")) {
			frm.add_custom_button(__("Customer"), function() {
				finergy.model.open_mapped_doc({
					method: "capkpi.crm.doctype.prospect.prospect.make_customer",
					frm: frm
				});
			}, __("Create"));
		}
		if (!frm.is_new() && finergy.boot.user.can_create.includes("Opportunity")) {
			frm.add_custom_button(__("Opportunity"), function() {
				finergy.model.open_mapped_doc({
					method: "capkpi.crm.doctype.prospect.prospect.make_opportunity",
					frm: frm
				});
			}, __("Create"));
		}
