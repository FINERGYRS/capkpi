...
class LinkedInSettings(Document):
		...
		params = urlencode(
			{
				...
				"redirect_uri": "{0}/api/method/capkpi.crm.doctype.linkedin_settings.linkedin_settings.callback?".format(
	...
	def get_access_token(self, code):
		...
		response = finergy.parse_json(response.content.decode())
	...
	def get_member_profile(self):
		...
		finergy.local.response["location"] = get_url_to_form("LinkedIn Settings", "LinkedIn Settings")
	...
	def post(self, text, title, media=None):
		if not media:
			...
			if media_id:
				...
				finergy.log_error("Failed to upload media.", "LinkedIn Upload Error")
	...
	def api_error(self, response):
		content = finergy.parse_json(response.content.decode())

		if response.status_code == 401:
			self.db_set("session_status", "Expired")
			finergy.db.commit()
			finergy.throw(content["message"], title=_("LinkedIn Error - Unauthorized"))
		elif response.status_code == 403:
			finergy.msgprint(_("You didn't have permission to access this API"))
			finergy.throw(content["message"], title=_("LinkedIn Error - Access Denied"))
		else:
			finergy.throw(response.reason, title=response.status_code)


@finergy.whitelist(allow_guest=True)
def callback(code=None, error=None, error_description=None):
	if not error:
		linkedin_settings = finergy.get_doc("LinkedIn Settings")
