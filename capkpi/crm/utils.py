import finergy
from finergy.desk.notifications import notify_mentions
from finergy.model.document import Document
from finergy.utils import cstr, now, today
...
def link_communications(doctype, docname, doc):
	...
	for communication in communication_list:
		communication_doc = finergy.get_doc("Communication", communication)
...
def get_linked_communication_list(doctype, docname):
	...
	communication_links = finergy.get_all(
		"Communication Link",
...
def get_linked_prospect(reference_doctype, reference_name):
	...
	if reference_doctype == "Lead":
		prospect = finergy.db.get_value("Prospect Lead", {"lead": reference_name}, "parent")

	elif reference_doctype == "Opportunity":
		opportunity_from, party_name = finergy.db.get_value(
			"Opportunity", reference_name, ["opportunity_from", "party_name"]
		)
		if opportunity_from == "Lead":
			prospect = finergy.db.get_value(
...
def link_open_events(ref_doctype, ref_docname, doc):
	...
	for event in events:
		event_doc = finergy.get_doc("Event", event.name)
...
def get_open_events(ref_doctype, ref_docname):
	event = finergy.qb.DocType("Event")
	event_link = finergy.qb.DocType("Event Participants")

	query = (
		finergy.qb.from_(event)
		.join(event_link)
...
def open_leads_opportunities_based_on_todays_event():
	...
	for d in data:
		finergy.db.set_value(d.reference_doctype, d.reference_docname, "status", "Open")


class CRMNote(Document):
	@finergy.whitelist()
	def add_note(self, note):
		self.append("notes", {"note": note, "added_by": finergy.session.user, "added_on": now()})
		self.save()
