...
from capkpi.crm.report.sales_pipeline_analytics.sales_pipeline_analytics import execute


class TestSalesPipelineAnalytics(unittest.TestCase):
	@classmethod
	def setUpClass(self):
		finergy.db.delete("Opportunity")
		create_company()
		create_customer()
		create_opportunity()
...
def create_company():
	doc = finergy.db.exists("Company", "Best Test")
	if not doc:
		doc = finergy.new_doc("Company")
...
def create_customer():
	doc = finergy.db.exists("Customer", "_Test NC")
	if not doc:
		doc = finergy.new_doc("Customer")
		doc.customer_name = "_Test NC"
		doc.insert()


def create_opportunity():
	doc = finergy.db.exists({"doctype": "Opportunity", "party_name": "_Test NC"})
	if not doc:
		doc = finergy.new_doc("Opportunity")
		doc.opportunity_from = "Customer"
		customer_name = finergy.db.get_value("Customer", {"customer_name": "_Test NC"}, ["customer_name"])
