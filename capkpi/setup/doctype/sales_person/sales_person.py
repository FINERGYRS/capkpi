...
class SalesPerson(NestedSet):
	...
	def validate(self):
		...
		for d in self.get("targets") or []:
			if not flt(d.target_qty) and not flt(d.target_amount):
				finergy.throw(_("Either target qty or target amount is mandatory."))
	...
	def load_dashboard_info(self):
		...
		allocated_amount = finergy.db.sql(
			"""
	...
	def get_email_id(self):
		if self.employee:
			user = finergy.db.get_value("Employee", self.employee, "user_id")
			if not user:
				finergy.throw(_("User ID not set for Employee {0}").format(self.employee))
			else:
				return finergy.db.get_value("User", user, "email") or user

	def validate_employee_id(self):
		if self.employee:
			sales_person = finergy.db.get_value("Sales Person", {"employee": self.employee})
...
def on_doctype_update():
	finergy.db.add_index("Sales Person", ["lft", "rgt"])
