...
class TestCompany(unittest.TestCase):
	def test_coa_based_on_existing_company(self):
		...
		finergy.delete_doc("Company", "COA from Existing Company")
			...
			for template in templates:
				try:
					...
					for account_type in account_types:
						...
						has_matching_accounts = finergy.get_all("Account", filters)
					...
					finergy.delete_doc("Company", template)
	...
	def test_basic_tree(self, records=None):
		min_lft = 1
		max_rgt = finergy.db.sql("select max(rgt) from `tabCompany`")[0][0]

		if not records:
			records = test_records[2:]

		for company in records:
			lft, rgt, parent_company = finergy.db.get_value(
				"Company", company["company_name"], ["lft", "rgt", "parent_company"]
			)

			if parent_company:
				parent_lft, parent_rgt = finergy.db.get_value("Company", parent_company, ["lft", "rgt"])
			else:
				# root
	...
	def test_change_parent_company(self):
		child_company = finergy.get_doc("Company", "_Test Company 5")
...
def create_child_company():
	child_company = finergy.db.exists("Company", "Test Company")
	if not child_company:
		child_company = finergy.get_doc(
			{
				"doctype": "Company",
		...
		child_company = finergy.get_doc("Company", child_company)

	return child_company.name


def create_test_lead_in_company(company):
	lead = finergy.db.exists("Lead", "Test Lead in new company")
	if not lead:
		...
		lead = finergy.get_doc("Lead", lead)
