...
finergy.provide("capkpi.company");

finergy.ui.form.on("Company", {
	onload: function(frm) {
		if (frm.doc.__islocal && frm.doc.parent_company) {
			finergy.db.get_value('Company', frm.doc.parent_company, 'is_group', (r) => {
				if (!r.is_group) {
	...
	date_of_commencement: function(frm) {
		...
		{
			finergy.throw(__("Date of Commencement should be greater than Date of Incorporation"));
		}
	...
	refresh: function(frm) {
		...
		if (!frm.is_new()) {
			...
			finergy.dynamic_link = {doc: frm.doc, fieldname: 'name', doctype: 'Company'}

			if (finergy.perm.has_perm("Cost Center", 0, 'read')) {
				frm.add_custom_button(__('Cost Centers'), function() {
					finergy.set_route('Tree', 'Cost Center', {'company': frm.doc.name});
				}, __("View"));
			}

			if (finergy.perm.has_perm("Account", 0, 'read')) {
				frm.add_custom_button(__('Chart of Accounts'), function() {
					finergy.set_route('Tree', 'Account', {'company': frm.doc.name});
				}, __("View"));
			}

			if (finergy.perm.has_perm("Sales Taxes and Charges Template", 0, 'read')) {
				frm.add_custom_button(__('Sales Tax Template'), function() {
					finergy.set_route('List', 'Sales Taxes and Charges Template', {'company': frm.doc.name});
				}, __("View"));
			}

			if (finergy.perm.has_perm("Purchase Taxes and Charges Template", 0, 'read')) {
				frm.add_custom_button(__('Purchase Tax Template'), function() {
					finergy.set_route('List', 'Purchase Taxes and Charges Template', {'company': frm.doc.name});
				}, __("View"));
			}
	...
	make_default_tax_template: function(frm) {
		frm.call({
			...
			freeze: true,
			callback: function() {
				finergy.msgprint(__("Default tax templates for sales, purchase and items are created."));
			}
		})
	},
	...
	delete_company_transactions: function(frm) {
		finergy.verify_password(function() {
			var d = finergy.prompt({
				fieldtype:"Data",
			...
			function(data) {
				if(data.company_name !== frm.doc.name) {
					finergy.msgprint(__("Company name not same"));
					return;
				}
				finergy.call({
					method: "capkpi.setup.doctype.company.company.create_transaction_deletion_request",
					args: {
					...
					callback: function(r, rt) {
						if(!r.exc)
							finergy.msgprint(__("Successfully deleted all transactions related to this company!"));
					},
					onerror: function() {
						finergy.msgprint(__("Wrong Password"));
					}
				});
			},
...
capkpi.company.set_chart_of_accounts_options = function(doc) {
	...
	if(doc.country) {
		return finergy.call({
			method: "capkpi.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
			args: {
				"country": doc.country,
				"with_standard": true
			},
