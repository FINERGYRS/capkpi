...
from finergy.utils.nestedset import NestedSet, rebuild_tree
	...
	def create_default_departments(self):
		records = [
			# Department
			{
				...
				"is_group": 1,
				"parent_department": "",
				"__condition": lambda: not finergy.db.exists("Department", _("All Departments")),
			},
			{
				"doctype": "Department",
...
def install_country_fixtures(company, country):
	try:
		module_name = f"capkpi.regional.{finergy.scrub(country)}.setup.setup"
		finergy.get_attr(module_name)(company, False)
	except ImportError:
		pass
	except Exception:
		finergy.log_error("Unable to set country fixtures")
		finergy.throw(
