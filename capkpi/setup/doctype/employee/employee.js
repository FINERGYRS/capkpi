...
capkpi.setup.EmployeeController = class EmployeeController extends finergy.ui.form.Controller {
	setup() {
		this.frm.fields_dict.user_id.get_query = function(doc, cdt, cdn) {
			return {
				query: "finergy.core.doctype.user.user.user_query",
...
finergy.ui.form.on("Employee", {
	...
	update_contact: function(frm) {
		var prefered_email_fieldname = finergy.model.scrub(frm.doc.prefered_contact_email) || 'user_id';
	...
	create_user: function(frm) {
		if (!frm.doc.prefered_email) {
			finergy.throw(__("Please enter Preferred Contact Email"));
		}
		finergy.call({
			method: "capkpi.setup.doctype.employee.employee.create_user",
			args: {
...
cur_frm.cscript = new capkpi.setup.EmployeeController({
	frm: cur_frm
});


finergy.tour['Employee'] = [
	{
