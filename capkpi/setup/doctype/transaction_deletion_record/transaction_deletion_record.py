...
class TransactionDeletionRecord(Document):
	...
	def delete_lead_addresses(self):
		...
		leads = finergy.get_all("Lead", filters={"company": self.company})
		leads = ["'%s'" % row.get("name") for row in leads]
		addresses = []
		if leads:
			addresses = finergy.db.sql_list(
			...
			if addresses:
				addresses = ["%s" % finergy.db.escape(addr) for addr in addresses]
	...
	def reset_company_values(self):
		company_obj = finergy.get_doc("Company", self.company)
		...
		for docfield in docfields:
			if docfield["parent"] != self.doctype:
				...
				if no_of_docs > 0:
					...
					naming_series = finergy.db.get_value("DocType", docfield["parent"], "autoname")
					if naming_series:
						if "#" in naming_series:
							self.update_naming_series(naming_series, docfield["parent"])

	def get_doctypes_to_be_ignored_list(self):
		singles = finergy.get_all("DocType", filters={"issingle": 1}, pluck="name")
	...
	def get_all_child_doctypes(self):
		return finergy.get_all("DocType", filters={"istable": 1}, pluck="name")

	def get_number_of_docs_linked_with_specified_company(self, doctype, company_fieldname):
		return finergy.db.count(doctype, {company_fieldname: self.company})
	...
	def update_naming_series(self, naming_series, doctype_name):
		...
		finergy.db.sql("""update tabSeries set current = %s where name=%s""", (last, prefix))
	...
	def delete_communications(self, doctype, company_fieldname):
		reference_docs = finergy.get_all(doctype, filters={company_fieldname: self.company})
		...
		finergy.delete_doc("Communication", communication_names, ignore_permissions=True)
