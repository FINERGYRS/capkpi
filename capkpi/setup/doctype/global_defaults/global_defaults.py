...
class GlobalDefaults(Document):
	def on_update(self):
		"""update defaults"""
		for key in keydict:
			finergy.db.set_default(key, self.get(keydict[key], ""))

		# update year start date and year end date from fiscal_year
		year_start_end_date = finergy.db.sql(
		...
		if year_start_end_date:
			...
			if ysd and yed:
				finergy.db.set_default("year_start_date", ysd.strftime("%Y-%m-%d"))
				finergy.db.set_default("year_end_date", yed.strftime("%Y-%m-%d"))
