...
finergy.ui.form.on("Item Group", {
	...
	refresh: function(frm) {
		...
		frm.add_custom_button(__("Item Group Tree"), function() {
			finergy.set_route("Tree", "Item Group");
		});

		if(!frm.is_new()) {
			frm.add_custom_button(__("Items"), function() {
				finergy.set_route("List", "Item", {"item_group": frm.doc.name});
			});
		}

		finergy.model.with_doctype('Website Item', () => {
			const web_item_meta = finergy.get_meta('Website Item');
