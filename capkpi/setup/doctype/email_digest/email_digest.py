...
import finergy
import finergy.desk.notifications
...
from capkpi.accounts.utils import get_balance_on, get_count_on, get_fiscal_year
...
class EmailDigest(Document):
	def __init__(self, *args, **kwargs):
		...
		self.set_dates()
		self._accounts = {}
		self.currency = finergy.db.get_value("Company", self.company, "default_currency")

	@finergy.whitelist()
	def get_users(self):
		"""get list of users"""
		user_list = finergy.db.sql(
			"""
		...
		finergy.response["user_list"] = user_list
	...
	def get_msg_html(self):
		...
		from capkpi.setup.doctype.email_digest.quotes import get_random_quote
		...
		if self.get("purchase_orders_items_overdue"):
			...
			if not context.purchase_order_list:
				finergy.throw(_("No items to be received are overdue"))
		...
		return finergy.render_template(
			"capkpi/setup/doctype/email_digest/templates/default.html", context, is_path=True
		)
	...
	def get_notifications(self):
		...
		notifications = finergy.desk.notifications.get_notifications()
	...
	def get_todo_list(self, user_id=None):
		"""Get to-do list"""
		if not user_id:
			user_id = finergy.session.user

		todo_list = finergy.db.sql(
			"""select *
	...
	def get_issue_list(self, user_id=None):
		...
		meta = finergy.get_meta("Issue")
		role_permissions = finergy.permissions.get_role_permissions(meta, user_id)
		if not role_permissions.get("read"):
			return None

		issue_list = finergy.db.sql(
			"""select *
	...
	def set_accounting_cards(self, context):
		...
		for key in (
			...
			if self.get(key):
				...
				if card:
					card = finergy.safe_eval(card)

				else:
					card = finergy._dict(getattr(self, "get_" + key)())
	...
	def get_income(self):
		...
		income_account = finergy.db.get_all(
			"Account",
			fields=["name"],
	...
	def get_expenses_booked(self):
		...
		expense_account = finergy.db.get_all(
			"Account",
			fields=["name"],
	...
	def get_summary_of_pending(self, doc_type, fieldname, getfield):
		...
		value, count, billed_value, delivered_value = finergy.db.sql(
	...
	def get_purchase_orders_items_overdue_list(self):
		...
		purchase_order_list = finergy.db.sql(sql_po, as_dict=True)
		purchase_order_items_overdue_list = finergy.db.sql(sql_poi, as_dict=True)
...
def send():
	...
	for ed in finergy.db.sql(
		"""select name from `tabEmail Digest`
			where enabled=1 and docstatus<2""",
		as_list=1,
	):
		ed_obj = finergy.get_doc("Email Digest", ed[0])
...
def get_digest_msg(name):
	return finergy.get_doc("Email Digest", name).get_msg_html()
