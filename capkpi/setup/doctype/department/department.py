...
class Department(NestedSet):
	...
	def before_rename(self, old, new, merge=False):
		...
		if not finergy.get_cached_value("Company", self.company, "abbr") in new:
			new = get_abbreviated_name(new, self.company)

		return new

	def on_update(self):
		if not (finergy.local.flags.ignore_update_nsm or finergy.flags.in_setup_wizard):
...
def on_doctype_update():
	finergy.db.add_index("Department", ["lft", "rgt"])


def get_abbreviated_name(name, company):
	abbr = finergy.get_cached_value("Company", company, "abbr")
...
def get_children(doctype, parent=None, company=None, is_root=False):
	...
	return finergy.get_all(doctype, fields=fields, filters=filters, order_by="name")
