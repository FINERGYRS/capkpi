...
from finergy.desk.page.setup_wizard.setup_wizard import make_records
from finergy.utils import cstr, getdate
from finergy.utils.nestedset import rebuild_tree

from capkpi.accounts.doctype.account.account import RootNotEditable
from capkpi.regional.address_template.setup import set_up_address_templates
...
def install(country=None):
	...
	from capkpi.setup.setup_wizard.data.industry_type import get_industry_types
	...
	from capkpi.buying.doctype.supplier_scorecard.supplier_scorecard import make_default_records
...
def update_selling_defaults():
	selling_settings = finergy.get_doc("Selling Settings")
...
def update_buying_defaults():
	buying_settings = finergy.get_doc("Buying Settings")
...
def add_uom_data():
	# add UOMs
	uoms = json.loads(
		open(finergy.get_app_path("capkpi", "setup", "setup_wizard", "data", "uom_data.json")).read()
	)
	for d in uoms:
		if not finergy.db.exists("UOM", _(d.get("uom_name"))):
			uom_doc = finergy.get_doc(
				{
					"doctype": "UOM",
	...
	uom_conversions = json.loads(
		open(
			finergy.get_app_path("capkpi", "setup", "setup_wizard", "data", "uom_conversion_data.json")
		).read()
	)
	for d in uom_conversions:
		if not finergy.db.exists("UOM Category", _(d.get("category"))):
			finergy.get_doc({"doctype": "UOM Category", "category_name": _(d.get("category"))}).db_insert()

		if not finergy.db.exists(
			"UOM Conversion Factor", {"from_uom": _(d.get("from_uom")), "to_uom": _(d.get("to_uom"))}
		):
			uom_conversion = finergy.get_doc(
				{
...
def add_sale_stages():
	...
	for sales_stage in records:
		finergy.get_doc(sales_stage).db_insert()
...
def install_post_company_fixtures(args=None):
	...
	finergy.local.flags.ignore_update_nsm = True
	make_records(records[1:])
	finergy.local.flags.ignore_update_nsm = False
...
def install_defaults(args=None):
	...
	finergy.db.set_value("Currency", args.get("currency"), "enabled", 1)
	finergy.db.set_value("Stock Settings", None, "email_footer_address", args.get("company_name"))
...
def set_global_defaults(args):
	global_defaults = finergy.get_doc("Global Defaults", "Global Defaults")
	current_fiscal_year = finergy.get_all("Fiscal Year")[0]
...
def set_active_domains(args):
	finergy.get_single("Domain Settings").set_active_domains(args.get("domains"))
...
def update_stock_settings():
	...
	stock_settings.default_warehouse = finergy.db.get_value(
...
def create_bank_account(args):
	...
	if bank_account_group:
		...
		try:
			...
			finergy.throw(_("Bank account cannot be named as {0}").format(args.get("bank_account")))
...
def update_shopping_cart_settings(args):
	shopping_cart = finergy.get_doc("E Commerce Settings")
	shopping_cart.update(
		{
			"enabled": 1,
			"company": args.company_name,
			"price_list": finergy.db.get_value("Price List", {"selling": 1}),
