...
import finergy
from finergy import _, qb, query_builder
from finergy.query_builder import Criterion, functions
...
def get_conditions(filters):
	...
	"""
	filters = finergy._dict(filters) if filters else finergy._dict({})
	conditions = finergy._dict({})

	conditions.company = filters.company or finergy.defaults.get_user_default("company")
	conditions.end_date = filters.period_end_date or finergy.utils.today()
	conditions.start_date = filters.period_start_date or finergy.utils.add_months(
...
def set_payment_terms_statuses(sales_orders, invoices, filters):
	...
	for so in sales_orders:
		so.currency = finergy.get_cached_value("Company", filters.get("company"), "default_currency")
		so.invoices = ""
