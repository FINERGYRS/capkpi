...
from capkpi.selling.report.sales_order_analysis.sales_order_analysis import execute
...
class TestSalesOrderAnalysis(FinergyTestCase):
	...
	def test_01_so_to_deliver_and_bill(self):
		...
		expected_value = {
			"status": "To Deliver and Bill",
			"sales_order": so.name,
			"delay_days": finergy.utils.date_diff(finergy.utils.datetime.date.today(), so.delivery_date),
			"qty": 10,
			"delivered_qty": 0,
			"pending_qty": 10,
			"qty_to_bill": 10,
