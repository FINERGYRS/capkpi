capkpi.PointOfSale.ItemDetails = class {
	...
	async toggle_item_details_section(item) {
		...
		if (item && current_item_changed) {
			...
			this.item_meta = finergy.get_meta(this.doctype);
			this.name = item.name;
			this.item_row = item;
		...
		if (!this.hide_images && image) {
			this.$item_image.html(
				`<img
					...
					alt="${finergy.get_abbr(item_name)}"
					style="object-fit: cover;">`
			);
		} else {
			this.$item_image.html(`<div class="item-abbr">${finergy.get_abbr(item_name)}</div>`);
		}
	...
	render_form(item) {
		...
		fields_to_display.forEach((fieldname, idx) => {
			...
			this[`${fieldname}_control`] = finergy.ui.form.make_control({
				df: {
					...field_meta,
					onchange: function() {
	...
	bind_custom_control_change_event() {
			...
			this.rate_control.df.onchange = function() {
				if (this.value || flt(this.value) === 0) {
					me.events.form_updated(me.current_item, 'rate', this.value).then(() => {
						const item_row = finergy.get_doc(me.doctype, me.name);
		...
		finergy.model.on("POS Invoice Item", "*", (fieldname, value, item_row) => {
	...
	async auto_update_batch_no() {
		if (this.serial_no_control && this.batch_no_control) {
			...
			const serials_with_batch_no = await finergy.db.get_list("Serial No", {
