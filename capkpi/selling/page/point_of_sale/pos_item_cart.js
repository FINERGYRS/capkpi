capkpi.PointOfSale.ItemCart = class {
	...
	make_cart_numpad() {
		...
		this.number_pad = new capkpi.PointOfSale.NumberPad({
	...
	bind_events() {
		...
		finergy.ui.form.on("POS Invoice", "paid_amount", frm => {
	...
	attach_shortcuts() {
		for (let row of this.number_pad.keys) {
			for (let btn of row) {
				...
				let shortcut_key = `ctrl+${finergy.scrub(String(btn))[0]}`;
				...
				const fieldname = this.number_pad.fieldnames[btn] ? this.number_pad.fieldnames[btn] :
					typeof btn === 'string' ? finergy.scrub(btn) : btn;

				let shortcut_label = shortcut_key.split('+').map(finergy.utils.to_title_case).join('+');
				shortcut_label = finergy.utils.is_mac() ? shortcut_label.replace('Ctrl', '⌘') : shortcut_label;
				this.$numpad_section.find(`.numpad-btn[data-button-value="${fieldname}"]`).attr("title", shortcut_label);

				finergy.ui.keys.on(`${shortcut_key}`, () => {
		...
		const ctrl_label = finergy.utils.is_mac() ? '⌘' : 'Ctrl';
	...
	make_customer_selector() {
		...
		`);
		const me = this;
		const query = { query: 'capkpi.controllers.queries.customer_query' };
		...
		this.customer_field = finergy.ui.form.make_control({
			df: {
				label: __('Customer'),
				fieldtype: 'Link',
				options: 'Customer',
				...
				onchange: function() {
					if (this.value) {
						...
						finergy.dom.freeze();
						finergy.model.set_value(frm.doc.doctype, frm.doc.name, 'customer', this.value);
	...
	fetch_customer_details(customer) {
		if (customer) {
			return new Promise((resolve) => {
				finergy.db.get_value('Customer', customer, ["email_id", "mobile_no", "image", "loyalty_program"]).then(({ message }) => {
					const { loyalty_program } = message;
					// if loyalty program then fetch loyalty points too
					if (loyalty_program) {
						finergy.call({
							method: "capkpi.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details_with_points",
	...
	show_discount_control() {
		...
		this.discount_field = finergy.ui.form.make_control({
			df: {
				label: __('Discount'),
				fieldtype: 'Data',
				...
				onchange: function() {
					if (flt(this.value) != 0) {
						finergy.model.set_value(frm.doc.doctype, frm.doc.name, 'additional_discount_percentage', flt(this.value));
						me.hide_discount_control(this.value);
					} else {
						finergy.model.set_value(frm.doc.doctype, frm.doc.name, 'additional_discount_percentage', 0);
	...
	get_customer_image() {
		...
		if (image) {
			...
			return `<div class="customer-image customer-abbr">${finergy.get_abbr(customer)}</div>`;
		}
	}
	...
	update_totals_section(frm) {
		...
		const grand_total = cint(finergy.sys_defaults.disable_rounded_total) ? frm.doc.grand_total : frm.doc.rounded_total;
	...
	render_cart_item(item_data, $item_to_update) {
		...
		function get_description_html() {
			if (item_data.description) {
				...
				item_data.description = finergy.ellipsis(item_data.description, 45);
				...
				return `
					<div class="item-image">
						<img
							...
							src="${image}" alt="${finergy.get_abbr(item_name)}"">
					</div>`;
			} else {
				return `<div class="item-image item-abbr">${finergy.get_abbr(item_name)}</div>`;
			}
		}
	}
	...
	render_customer_fields() {
		...
		const me = this;
		dfs.forEach(df => {
			this[`customer_${df.fieldname}_field`] = finergy.ui.form.make_control({
				df: { ...df,
		...
		function handle_customer_field_change() {
			...
			if (this.value && current_value != this.value && this.df.fieldname != 'loyalty_points') {
				finergy.call({
					method: 'capkpi.selling.page.point_of_sale.point_of_sale.set_customer_info',
					args: {
	...
	fetch_customer_transactions() {
		finergy.db.get_list('POS Invoice', {
