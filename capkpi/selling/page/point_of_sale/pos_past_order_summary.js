capkpi.PointOfSale.PastOrderSummary = class {
	...
	init_email_print_dialog() {
		const email_dialog = new finergy.ui.Dialog({
			title: 'Email Receipt',
			fields: [
		...
		const print_dialog = new finergy.ui.Dialog({
			title: 'Print Receipt',
			fields: [
	...
	print_receipt() {
		...
		finergy.utils.print(
			...
			this.doc.language || finergy.boot.lang
		);
	}
	...
	send_email() {
		...
		finergy.call({
			...
			args: {
				...
				send_email: 1,
				print_format,
				sender_full_name: finergy.user.full_name(),
				_lang: doc.language
			},
			callback: r => {
				if (!r.exc) {
	...
	attach_document_info(doc) {
		finergy.db.get_value('Customer', this.doc.customer, 'email_id').then(({ message }) => {
