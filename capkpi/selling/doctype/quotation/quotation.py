...
from finergy.utils import flt, getdate, nowdate
...
class Quotation(SellingController):
	...
	def validate_valid_till(self):
		if self.valid_till and getdate(self.valid_till) < getdate(self.transaction_date):
			finergy.throw(_("Valid till date cannot be before transaction date"))
	...
	def validate_shopping_cart_items(self):
		...
		for item in self.items:
			has_web_item = finergy.db.exists("Website Item", {"item_code": item.item_code})

			# If variant is unpublished but template is published: valid
			template = finergy.get_cached_value("Item", item.item_code, "variant_of")
			if template and not has_web_item:
				has_web_item = finergy.db.exists("Website Item", {"item_code": template})
	...
	def update_opportunity_status(self, status, opportunity=None):
		...
		opp = finergy.get_doc("Opportunity", opportunity)
		opp.set_status(status=status, update=True)

	@finergy.whitelist()
	def declare_enquiry_lost(self, lost_reasons_list, competitors, detailed_reason=None):
		if not (self.is_fully_ordered() or self.is_partially_ordered()):
			get_lost_reasons = finergy.get_list("Quotation Lost Reason", fields=["name"])
			lost_reasons_lst = [reason.get("name") for reason in get_lost_reasons]
			finergy.db.set(self, "status", "Lost")

			if detailed_reason:
				finergy.db.set(self, "order_lost_reason", detailed_reason)
				...
				if reason.get("lost_reason") in lost_reasons_lst:
					...
					finergy.throw(
						_("Invalid lost reason {0}, please create a new lost reason").format(
							finergy.bold(reason.get("lost_reason"))
						)
					)
			...
			finergy.throw(_("Cannot set as Lost as Sales Order is made."))
...
def make_sales_order(source_name, target_doc=None):
	quotation = finergy.db.get_value(
		"Quotation", source_name, ["transaction_date", "valid_till"], as_dict=1
	)
	if quotation.valid_till and (
		quotation.valid_till < quotation.transaction_date or quotation.valid_till < getdate(nowdate())
	):
		finergy.throw(_("Validity period of this quotation has ended."))
...
def _make_sales_order(source_name, target_doc=None, ignore_permissions=False):
	...
	def set_missing_values(source, target):
		...
		if source.referral_sales_partner:
			...
			target.commission_rate = finergy.get_value(
...
def _make_customer(source_name, ignore_permissions=False):
	...
	if quotation and quotation.get("party_name"):
		if not finergy.db.exists("Customer", quotation.get("party_name")):
			lead_name = quotation.get("party_name")
			customer_name = finergy.db.get_value(
				"Customer", {"lead_name": lead_name}, ["name", "customer_name"], as_dict=True
			)
			if not customer_name:
				from capkpi.crm.doctype.lead.lead import _make_customer

				customer_doclist = _make_customer(lead_name, ignore_permissions=ignore_permissions)
				customer = finergy.get_doc(customer_doclist)
				customer.flags.ignore_permissions = ignore_permissions
				if quotation.get("party_name") == "Shopping Cart":
					customer.customer_group = finergy.db.get_value(
				...
				try:
					...
					if finergy.defaults.get_global_default("cust_master_name") == "Customer Name":
						...
						raise
				except finergy.MandatoryError as e:
					mandatory_fields = e.args[0].split(":")[1].split(",")
					mandatory_fields = [customer.meta.get_label(field.strip()) for field in mandatory_fields]

					finergy.local.message_log = []
					lead_link = finergy.utils.get_link_to_form("Lead", lead_name)
					message = (
					...
					finergy.throw(message, title=_("Mandatory Missing"))
			else:
				return customer_name
		else:
			return finergy.get_doc("Customer", quotation.get("party_name"))
