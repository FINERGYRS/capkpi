...
finergy.ui.form.on("Sales Order", {
	setup: function(frm) {
	...
	onload: function(frm) {
		...
		frm.set_query('project', function(doc, cdt, cdn) {
			return {
				query: "capkpi.controllers.queries.get_project_name",
				filters: {
		...
		frm.set_query('warehouse', 'items', function(doc, cdt, cdn) {
			...
			if (row.item_code) {
				query.query = "capkpi.controllers.queries.warehouse_query";
...
finergy.ui.form.on("Sales Order Item", {
	...
	delivery_date: function(frm, cdt, cdn) {
		if(!frm.doc.delivery_date) {
			capkpi.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "delivery_date");
		}
	}
});

capkpi.selling.SalesOrderController = class SalesOrderController extends capkpi.selling.SellingController {
	onload(doc, dt, dn) {
			...
			this.frm.add_custom_button(__('Quotation'),
				function() {
					capkpi.utils.map_current_doc({
						method: "capkpi.selling.doctype.quotation.quotation.make_sales_order",
	...
	make_material_request() {
		finergy.model.open_mapped_doc({
			method: "capkpi.selling.doctype.sales_order.sales_order.make_material_request",
			frm: this.frm
		})
	}
	...
	make_raw_material_request_dialog(r) {
		...
		var d = new finergy.ui.Dialog({
			...
			primary_action: function() {
				...
				me.frm.call({
					method: 'capkpi.selling.doctype.sales_order.sales_order.make_raw_material_request',
					args: {
						items: data,
						company: me.frm.doc.company,
						sales_order: me.frm.docname,
					...
					callback: function(r) {
						if(r.message) {
							finergy.msgprint(__('Material Request {0} submitted.',
					...
					${delivery_dates.map(date => `
						<div class="list-item">
							<div class="list-item__content list-item__content--flex-2">
								...
								${finergy.datetime.str_to_user(date)}
								</label>
							</div>
						</div>
					`).join("")}
	...
	make_delivery_note(delivery_dates) {
		finergy.model.open_mapped_doc({
			method: "capkpi.selling.doctype.sales_order.sales_order.make_delivery_note",
			frm: this.frm,
			args: {
				delivery_dates
			}
	...
	make_maintenance_schedule() {
		finergy.model.open_mapped_doc({
			method: "capkpi.selling.doctype.sales_order.sales_order.make_maintenance_schedule",
			frm: this.frm
		})
	}
	...
	make_project() {
		finergy.model.open_mapped_doc({
			method: "capkpi.selling.doctype.sales_order.sales_order.make_project",
			frm: this.frm
		})
	}
	...
	make_inter_company_order() {
		finergy.model.open_mapped_doc({
			method: "capkpi.selling.doctype.sales_order.sales_order.make_inter_company_purchase_order",
			frm: this.frm
		});
	}
	...
	make_purchase_order(){
		...
		if(!pending_items){
			finergy.throw({message: __("Purchase Order already created for all Sales Order items"), title: __("Note")});
		}
		...
		var dialog = new finergy.ui.Dialog({
			...
			primary_action (args) {
				...
				if(selected_items.length == 0) {
					finergy.throw({message: 'Please select Items from the Table', title: __('Items Required'), indicator:'blue'})
				}
				...
				return finergy.call({
					method: "capkpi.selling.doctype.sales_order.sales_order." + method,
					freeze: true,
					...
					callback: function(r) {
						if(!r.exc) {
							if (!args.against_default_supplier) {
								...
								finergy.set_route("List", "Purchase Order");
							}
						}
					}
				})
	...
	update_status(label, status){
		...
		finergy.call({
			method: "capkpi.selling.doctype.sales_order.sales_order.update_status",
...
extend_cscript(cur_frm.cscript, new capkpi.selling.SalesOrderController({frm: cur_frm}));
