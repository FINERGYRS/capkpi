...
from finergy.utils import add_days, flt, getdate, nowdate, today

from capkpi.controllers.accounts_controller import update_child_qty_rate
from capkpi.maintenance.doctype.maintenance_schedule.test_maintenance_schedule import (
	make_maintenance_schedule,
)
from capkpi.maintenance.doctype.maintenance_visit.test_maintenance_visit import (
	make_maintenance_visit,
)
from capkpi.manufacturing.doctype.blanket_order.test_blanket_order import make_blanket_order
from capkpi.selling.doctype.product_bundle.test_product_bundle import make_product_bundle
from capkpi.selling.doctype.sales_order.sales_order import (
	WarehouseRequired,
	make_delivery_note,
...
class TestSalesOrder(FinergyTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def test_make_material_request(self):
		...
		self.assertRaises(finergy.ValidationError, make_material_request, so.name)
	...
	def test_make_delivery_note(self):
		...
		self.assertRaises(finergy.ValidationError, make_delivery_note, so.name)
	...
	def test_make_sales_invoice(self):
		...
		self.assertRaises(finergy.ValidationError, make_sales_invoice, so.name)
	...
	def test_return_against_sales_order(self):
		...
		from capkpi.accounts.doctype.sales_invoice.test_sales_invoice import create_sales_invoice
	...
	def test_reserved_qty_for_over_delivery(self):
		...
		finergy.db.set_value("Item", "_Test Item", "over_delivery_receipt_allowance", 50)
	...
	def test_reserved_qty_for_over_delivery_via_sales_invoice(self):
		...
		# set over-delivery allowance
		finergy.db.set_value("Item", "_Test Item", "over_delivery_receipt_allowance", 50)
		finergy.db.set_value("Item", "_Test Item", "over_billing_allowance", 20)
	...
	def test_sales_order_on_hold(self):
		...
		si = make_sales_invoice(so.name)
		self.assertRaises(finergy.ValidationError, create_dn_against_so, so.name)
	...
	def test_reserved_qty_for_over_delivery_with_packing_list(self):
		...
		# set over-delivery allowance
		finergy.db.set_value("Item", "_Test Product Bundle Item", "over_delivery_receipt_allowance", 50)
	...
	def test_update_child_removing_item(self):
		...
		self.assertRaises(
			finergy.ValidationError, update_child_qty_rate, "Sales Order", trans_item, so.name
		)
	...
	def test_update_child_with_precision(self):
		...
		precision = get_field_precision(finergy.get_meta("Sales Order Item").get_field("rate"))
	...
	def test_update_child_qty_rate_with_workflow(self):
		from finergy.model.workflow import apply_workflow
		...
		test_user = finergy.get_doc("User", user)
		...
		test_user2 = finergy.get_doc("User", user2)
		test_user2.add_roles("Sales User", "Test Approver")
		finergy.set_user(user2)
	...
	def test_update_child_product_bundle(self):
		...
		if not finergy.db.exists("Item", "_Product Bundle Item"):
	...
	def test_update_child_with_tax_template(self):
		...
		"""
		if not finergy.db.exists("Item", "Test Item with Tax"):
			make_item(
				"Test Item with Tax",
				{
					"is_stock_item": 1,
		...
		if not finergy.db.exists("Item Tax Template", {"title": "Test Update Items Template"}):
			finergy.get_doc(
				{
		...
		new_item_with_tax = finergy.get_doc("Item", "Test Item with Tax")
		...
		if not finergy.db.exists("Item Tax", {"parent": item, "item_tax_template": tax_template}):
		...
		old_stock_settings_value = finergy.db.get_single_value("Stock Settings", "default_warehouse")
		finergy.db.set_value("Stock Settings", None, "default_warehouse", "_Test Warehouse - _TC")
		...
		finergy.get_doc("Item Tax Template", "Test Update Items Template - _TC").delete()
		finergy.db.set_value("Stock Settings", None, "default_warehouse", old_stock_settings_value)

	def test_warehouse_user(self):
		test_user = create_user("test_so_warehouse_user@example.com", "Sales User", "Stock User")

		test_user_2 = finergy.get_doc("User", "test2@example.com")
		test_user_2.add_roles("Sales User", "Stock User")
		test_user_2.remove_roles("Sales Manager")

		finergy.permissions.add_user_permission("Warehouse", "_Test Warehouse 1 - _TC", test_user.name)
		finergy.permissions.add_user_permission("Warehouse", "_Test Warehouse 2 - _TC1", test_user_2.name)
		finergy.permissions.add_user_permission("Company", "_Test Company 1", test_user_2.name)
		...
		self.assertRaises(finergy.PermissionError, so.insert)

		finergy.set_user(test_user_2.name)
		so.insert()

		finergy.set_user("Administrator")
		finergy.permissions.remove_user_permission("Warehouse", "_Test Warehouse 1 - _TC", test_user.name)
		finergy.permissions.remove_user_permission(
			"Warehouse", "_Test Warehouse 2 - _TC1", test_user_2.name
		)
		finergy.permissions.remove_user_permission("Company", "_Test Company 1", test_user_2.name)
	...
	def test_block_delivery_note_against_cancelled_sales_order(self):
		...
		self.assertRaises(finergy.CancelledLinkError, dn.submit)
	...
	def test_auto_insert_price(self):
		...
		finergy.db.set_value("Stock Settings", None, "auto_insert_price_list_rate_if_missing", 1)

		item_price = finergy.db.get_value(
			"Item Price", {"price_list": "_Test Price List", "item_code": "_Test Item for Auto Price List"}
		)
		if item_price:
			finergy.delete_doc("Item Price", item_price)
		...
		finergy.db.set_value("Stock Settings", None, "auto_insert_price_list_rate_if_missing", 0)
	...
	def test_drop_shipping(self):
		from capkpi.buying.doctype.purchase_order.purchase_order import update_status
		from capkpi.selling.doctype.sales_order.sales_order import (
			make_purchase_order_for_default_supplier,
		)
		from capkpi.selling.doctype.sales_order.sales_order import update_status as so_update_status
		...
		if finergy.db.get_value("Item", "_Test Regular Item", "is_stock_item") == 1:
		...
		self.assertEqual(flt(finergy.db.get_value("Sales Order", so.name, "per_delivered"), 2), 100.00)
		po.load_from_db()
	...
	def test_drop_shipping_full_for_default_suppliers(self):
		...
		if not finergy.db.exists("Item", "_Test Item for Drop Shipping 1"):
			make_item("_Test Item for Drop Shipping 1", {"is_stock_item": 1, "delivered_by_supplier": 1})

		if not finergy.db.exists("Item", "_Test Item for Drop Shipping 2"):
	...
	def test_product_bundles_in_so_are_replaced_with_bundle_items_in_po(self):
		...
		"""
		from capkpi.selling.doctype.sales_order.sales_order import make_purchase_order
	...
	def test_serial_no_based_delivery(self):
		finergy.set_value("Stock Settings", None, "automatically_set_serial_nos_based_on_fifo", 1)
		item = make_item(
		...
		finergy.db.sql("""delete from `tabSerial No` where item_code=%s""", (item.item_code))
		make_item(
			"_Test Item A",
			{
				"maintain_stock": 1,
		...
		item_serial_no = finergy.get_doc("Serial No", {"item_code": item.item_code})
		from capkpi.manufacturing.doctype.work_order.work_order import (
			make_stock_entry as make_production_stock_entry,
		)

		se = finergy.get_doc(make_production_stock_entry(work_order.name, "Manufacture", 1))
		se.submit()
		reserved_serial_no = se.get("items")[2].serial_no
		serial_no_so = finergy.get_value("Serial No", reserved_serial_no, "sales_order")
		...
		item_line.serial_no = item_serial_no.name
		self.assertRaises(finergy.ValidationError, dn.submit)
		item_line = si.get("items")[0]
		item_line.serial_no = reserved_serial_no
		...
		si.update_stock = 0
		si.submit()
		from capkpi.accounts.doctype.sales_invoice.sales_invoice import (
		...
		se.load_from_db()
		se.cancel()
		self.assertFalse(finergy.db.exists("Serial No", {"sales_order": so.name}))
	...
	def test_advance_payment_entry_unlink_against_sales_order(self):
		...
		so_doc = finergy.get_doc("Sales Order", so.name)

		self.assertRaises(finergy.LinkExistsError, so_doc.cancel)
	...
	def test_cancel_sales_order_after_cancel_payment_entry(self):
		...
		po_doc = finergy.get_doc("Payment Entry", pe.name)
		po_doc.cancel()
	...
	def test_work_order_pop_up_from_sales_order(self):
		...
		from capkpi.controllers.item_variant import create_variant
		from capkpi.manufacturing.doctype.production_plan.test_production_plan import make_bom
	...
	def test_request_for_raw_materials(self):
		...
		mr_doc = finergy.get_doc("Material Request", mr.get("name"))
	...
	def test_so_optional_blanket_order(self):
		...
		so_doc = finergy.get_doc("Sales Order", so.get("name"))
	...
	def test_so_cancellation_when_si_drafted(self):
		...
		self.assertRaises(finergy.ValidationError, so.cancel)
	...
	def test_so_cancellation_after_si_submission(self):
		...
		so.load_from_db()
		self.assertRaises(finergy.LinkExistsError, so.cancel)
	...
	def test_payment_terms_are_fetched_when_creating_sales_invoice(self):
		from capkpi.accounts.doctype.payment_entry.test_payment_entry import (
	...
	def test_so_back_updated_from_wo_via_mr(self):
		...
		from capkpi.stock.doctype.material_request.material_request import raise_work_orders
		...
		wo = finergy.get_doc("Work Order", wo_name)
		...
		se = finergy.get_doc(make_se_from_wo(wo.name, "Manufacture", 2))
		se.submit()  # Finish WO
...
def automatically_fetch_payment_terms(enable=1):
	accounts_settings = finergy.get_doc("Accounts Settings")
...
def make_sales_order(**args):
	so = finergy.new_doc("Sales Order")
...
def get_reserved_qty(item_code="_Test Item", warehouse="_Test Warehouse - _TC"):
	return flt(
		finergy.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "reserved_qty")
	)
...
def make_sales_order_workflow():
	if finergy.db.exists("Workflow", "SO Test Workflow"):
		doc = finergy.get_doc("Workflow", "SO Test Workflow")
		doc.set("is_active", 1)
		doc.save()
		return doc

	finergy.get_doc(dict(doctype="Role", role_name="Test Junior Approver")).insert(
		ignore_if_duplicate=True
	)
	finergy.get_doc(dict(doctype="Role", role_name="Test Approver")).insert(ignore_if_duplicate=True)
	finergy.cache().hdel("roles", finergy.session.user)

	workflow = finergy.get_doc(
		{
			"doctype": "Workflow",
