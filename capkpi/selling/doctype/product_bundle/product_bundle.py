...
class ProductBundle(Document):
	...
	def validate_main_item(self):
		...
		if finergy.db.get_value("Item", self.new_item_code, "is_stock_item"):
			finergy.throw(_("Parent Item {0} must not be a Stock Item").format(self.new_item_code))

	def validate_child_items(self):
		for item in self.items:
			if finergy.db.exists("Product Bundle", item.item_code):
				finergy.throw(
					_(
						"Row #{0}: Child Item should not be a Product Bundle. Please remove Item {1} and Save"
					).format(item.idx, finergy.bold(item.item_code))
				)
