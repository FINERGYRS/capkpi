...
finergy.ui.form.on("Customer", {
	setup: function(frm) {

		frm.make_methods = {
			'Quotation': () => finergy.model.open_mapped_doc({
				method: "capkpi.selling.doctype.customer.customer.make_quotation",
				frm: cur_frm
			}),
			'Opportunity': () => finergy.model.open_mapped_doc({
				method: "capkpi.selling.doctype.customer.customer.make_opportunity",
				frm: cur_frm
			})
		}
		...
		frm.set_query('customer_primary_contact', function(doc) {
			return {
				query: "capkpi.selling.doctype.customer.customer.get_customer_primary_contact",
				filters: {
					'customer': doc.name
				}
			}
	...
	refresh: function(frm) {
		if(finergy.defaults.get_default("cust_master_name")!="Naming Series") {
			frm.toggle_display("naming_series", false);
		} else {
			capkpi.toggle_naming_series();
		}

		finergy.dynamic_link = {doc: frm.doc, fieldname: 'name', doctype: 'Customer'}
		...
		if(!frm.doc.__islocal) {
			...
			frm.add_custom_button(__('Accounts Receivable'), function () {
				finergy.set_route('query-report', 'Accounts Receivable', {customer:frm.doc.name});
			}, __('View'));
			...
			frm.add_custom_button(__('Pricing Rule'), function () {
				capkpi.utils.make_pricing_rule(frm.doc.doctype, frm.doc.name);
			}, __('Create'));
			...
			if (cint(finergy.defaults.get_default("enable_common_party_accounting"))) {
	...
	validate: function(frm) {
		if(frm.doc.lead_name) finergy.model.clear_doc("Lead", frm.doc.lead_name);
	...
	show_party_link_dialog: function(frm) {
		const dialog = new finergy.ui.Dialog({
			...
			primary_action: function({ supplier }) {
				finergy.call({
					method: 'capkpi.accounts.doctype.party_link.party_link.create_party_link',
					args: {
