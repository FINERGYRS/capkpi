...
from capkpi.accounts.party import get_due_date
from capkpi.exceptions import PartyDisabled, PartyFrozen
from capkpi.selling.doctype.customer.customer import get_credit_limit, get_customer_outstanding
from capkpi.tests.utils import create_test_contact_and_address

test_ignore = ["Price List"]
test_dependencies = ["Payment Term", "Payment Terms Template"]
test_records = finergy.get_test_records("Customer")

from six import iteritems


class TestCustomer(FinergyTestCase):
	def setUp(self):
	...
	def test_get_customer_group_details(self):
		doc = finergy.new_doc("Customer Group")
		...
		c_doc = finergy.new_doc("Customer")
	...
	def test_party_details_tax_category(self):
		...
		finergy.delete_doc_if_exists("Address", "_Test Address With Tax Category-Billing")
		finergy.delete_doc_if_exists("Address", "_Test Address With Tax Category-Shipping")

		# Tax Category without Address
		details = get_party_details("_Test Customer With Tax Category")
		self.assertEqual(details.tax_category, "_Test Tax Category 1")

		billing_address = finergy.get_doc(
			dict(
				doctype="Address",
		...
		shipping_address = finergy.get_doc(
			dict(
				doctype="Address",
				address_title="_Test Address With Tax Category",
		...
		settings = finergy.get_single("Accounts Settings")
	...
	def test_rename(self):
		...
		# add comments
		comment = finergy.get_doc("Customer", "_Test Customer 1").add_comment(
			"Comment", "Test Comment for Rename"
		)

		# rename
		finergy.rename_doc("Customer", "_Test Customer 1", new_name)

		# check if customer renamed
		self.assertTrue(finergy.db.exists("Customer", new_name))
		self.assertFalse(finergy.db.exists("Customer", "_Test Customer 1"))
		...
		finergy.rename_doc("Customer", new_name, "_Test Customer 1")
	...
	def test_freezed_customer(self):
		...
		finergy.db.set_value("Customer", "_Test Customer", "is_frozen", 1)
		...
		finergy.db.set_value("Customer", "_Test Customer", "is_frozen", 0)

		so.save()

	def test_delete_customer_contact(self):
		customer = finergy.get_doc(get_customer_dict("_Test Customer for delete")).insert(
		...
		finergy.delete_doc("Customer", customer.name)

	def test_disabled_customer(self):
		make_test_records("Item")

		finergy.db.set_value("Customer", "_Test Customer", "disabled", 1)
		...
		finergy.db.set_value("Customer", "_Test Customer", "disabled", 0)

		so.save()

	def test_duplicate_customer(self):
		finergy.db.sql("delete from `tabCustomer` where customer_name='_Test Customer 1'")

		if not finergy.db.get_value("Customer", "_Test Customer 1"):
			test_customer_1 = finergy.get_doc(get_customer_dict("_Test Customer 1")).insert(
				ignore_permissions=True
			)
		else:
			test_customer_1 = finergy.get_doc("Customer", "_Test Customer 1")

		duplicate_customer = finergy.get_doc(get_customer_dict("_Test Customer 1")).insert(
			ignore_permissions=True
		)
	...
	def test_customer_credit_limit(self):
		...
		self.assertRaises(finergy.ValidationError, so.submit)
		...
		self.assertRaises(finergy.ValidationError, make_sales_order)

	def test_customer_credit_limit_on_change(self):
		outstanding_amt = self.get_customer_outstanding_amount()
		customer = finergy.get_doc("Customer", "_Test Customer")
		customer.append(
		...
		self.assertRaises(finergy.ValidationError, customer.save)
	...
	def test_customer_payment_terms(self):
		...
		finergy.db.set_value("Customer", "_Test Customer With Template", "payment_terms", "")
...
def create_internal_customer(
	...
	if not finergy.db.exists("Customer", customer_name):
		...
		customer_name = finergy.db.get_value("Customer", customer_name)
