...
from finergy.desk.reportview import build_match_conditions, get_filters_cond
from finergy.model.mapper import get_mapped_doc
from finergy.model.naming import set_name_by_naming_series, set_name_from_naming_options
from finergy.model.rename_doc import update_linked_doctypes
from finergy.utils import cint, cstr, flt, get_formatted_email, today
from finergy.utils.user import get_users_with_role

from capkpi.accounts.party import (  # noqa
	get_dashboard_info,
	get_timeline_data,
...
class Customer(TransactionBase):
	...
	def autoname(self):
		cust_master_name = finergy.defaults.get_global_default("cust_master_name")
		if cust_master_name == "Customer Name":
			self.name = self.get_customer_name()
		elif cust_master_name == "Naming Series":
			set_name_by_naming_series(self)
		else:
			self.name = set_name_from_naming_options(finergy.get_meta(self.doctype).autoname, self)

	def get_customer_name(self):

		if finergy.db.get_value("Customer", self.customer_name) and not finergy.flags.in_import:
	...
	def validate(self):
		...
		if finergy.db.exists("Customer", self.name):
			customer = finergy.get_doc("Customer", self.name)
			if self.loyalty_program == customer.loyalty_program and not self.loyalty_program_tier:
				self.loyalty_program_tier = customer.loyalty_program_tier

		if self.sales_team:
			if sum(member.allocated_percentage or 0 for member in self.sales_team) != 100:
				finergy.throw(_("Total contribution percentage should be equal to 100"))

	@finergy.whitelist()
	def get_customer_group_details(self):
		doc = finergy.get_doc("Customer Group", self.customer_group)
		self.accounts = []
		self.credit_limits = []
	...
	def check_customer_group_change(self):
		finergy.flags.customer_group_changed = False

		if not self.get("__islocal"):
			if self.customer_group != finergy.db.get_value("Customer", self.name, "customer_group"):
				finergy.flags.customer_group_changed = True

	def validate_default_bank_account(self):
		if self.default_bank_account:
			is_company_account = finergy.db.get_value(
				"Bank Account", self.default_bank_account, "is_company_account"
			)
			if not is_company_account:
				finergy.throw(
					_("{0} is not a company bank account").format(finergy.bold(self.default_bank_account))
				)
	...
	def validate_internal_customer(self):
		...
		internal_customer = finergy.db.get_value(
			"Customer",
			{
		...
		if internal_customer:
			finergy.throw(
				_("Internal Customer for company {0} already exists").format(
					finergy.bold(self.represents_company)
				)
			)
	...
	def update_customer_groups(self):
		...
		if finergy.flags.customer_group_changed:
	...
	def update_lead_status(self):
		...
		if self.lead_name:
			finergy.db.set_value("Lead", self.lead_name, "status", "Converted")

	def link_lead_address_and_contact(self):
		if self.lead_name:
			# assign lead address and contact to customer (if already not set)
			linked_contacts_and_addresses = finergy.get_all(
				"Dynamic Link",
				filters=[
			...
			for row in linked_contacts_and_addresses:
				linked_doc = finergy.get_doc(row.doctype, row.name)
	...
	def validate_name_with_customer_group(self):
		if finergy.db.exists("Customer Group", self.name):
			finergy.throw(
				_(
	...
	def on_trash(self):
		...
		if self.lead_name:
			finergy.db.sql("update `tabLead` set status='Interested' where name=%s", self.lead_name)

	def after_rename(self, olddn, newdn, merge=False):
		if finergy.defaults.get_global_default("cust_master_name") == "Customer Name":
			finergy.db.set(self, "customer_name", newdn)
		...
		if len(loyalty_program) == 1:
			...
			finergy.msgprint(
				_("Multiple Loyalty Programs found for Customer {}. Please select manually.").format(
					finergy.bold(self.customer_name)
				)
			)
...
def make_quotation(source_name, target_doc=None):
	...
	price_list, currency = finergy.db.get_value(
...
def get_loyalty_programs(doc):
	...
	lp_details = []
	loyalty_programs = finergy.get_all(
		"Loyalty Program",
...
def get_nested_links(link_doctype, link_name, ignore_permissions=False):
	from finergy.desk.treeview import _get_children
...
def get_customer_list(doctype, txt, searchfield, start, page_len, filters=None):
	from capkpi.controllers.queries import get_fields

	fields = ["name", "customer_name", "customer_group", "territory"]

	if finergy.db.get_default("cust_master_name") == "Customer Name":
...
def check_credit_limit(customer, company, ignore_outstanding_sales_order=False, extra_amount=0):
	...
	if credit_limit > 0 and flt(customer_outstanding) > credit_limit:
		...
		credit_controller_role = finergy.db.get_single_value("Accounts Settings", "credit_controller")
		if not credit_controller_role or credit_controller_role not in finergy.get_roles():
			...
			finergy.msgprint(
				...
				primary_action={
					"label": "Send Email",
					"server_action": "capkpi.selling.doctype.customer.customer.send_emails",
					"args": {
						"customer": customer,
...
def get_customer_outstanding(
	...
	cond = ""
	if cost_center:
		lft, rgt = finergy.get_cached_value("Cost Center", cost_center, ["lft", "rgt"])
	...
	outstanding_based_on_gle = finergy.db.sql(
		"""
	...
	if not ignore_outstanding_sales_order:
		outstanding_based_on_so = finergy.db.sql(
			"""
	...
	unmarked_delivery_note_items = finergy.db.sql(
		"""select
	...
	si_amounts = finergy.db.sql(
		...
		GROUP BY dn_detail""".format(
			", ".join(finergy.db.escape(dn_item.name) for dn_item in unmarked_delivery_note_items)
		)
	)
...
def get_credit_limit(customer, company):
	...
	if customer:
		credit_limit = finergy.db.get_value(
			"Customer Credit Limit",
		...
		if not credit_limit:
			customer_group = finergy.get_cached_value("Customer", customer, "customer_group")
			credit_limit = finergy.db.get_value(
				"Customer Credit Limit",
	...
	if not credit_limit:
		credit_limit = finergy.get_cached_value("Company", company, "credit_limit")
