...
class TestGSTR3BReport(unittest.TestCase):
	def setUp(self):
		...
		finergy.db.sql("delete from `tabSales Invoice` where company='_Test Company GST'")
		finergy.db.sql("delete from `tabPurchase Invoice` where company='_Test Company GST'")
		finergy.db.sql("delete from `tabGSTR 3B Report` where company='_Test Company GST'")
	...
	def test_gstr_3b_report(self):
		...
		if finergy.db.exists("GSTR 3B Report", "GSTR3B-March-2019-_Test Address GST-Billing"):
			report = finergy.get_doc("GSTR 3B Report", "GSTR3B-March-2019-_Test Address GST-Billing")
			report.save()
		else:
	...
	def test_gst_rounding(self):
		...
		current_country = finergy.flags.country
		...
		finergy.flags.country = current_country
		gst_settings.round_off_gst_values = 1
		gst_settings.save()

	def test_gst_category_auto_update(self):
		if not finergy.db.exists("Customer", "_Test GST Customer With GSTIN"):
		...
		if not finergy.db.exists("Address", "_Test GST Category-1-Billing"):
...
def make_suppliers():
	if not finergy.db.exists("Supplier", "_Test Registered Supplier"):
		finergy.get_doc(
			{
	...
	if not finergy.db.exists("Supplier", "_Test Unregistered Supplier"):
		finergy.get_doc(
			{
				"supplier_group": "_Test Supplier Group",
	...
	if not finergy.db.exists("Address", "_Test Supplier GST-1-Billing"):
		address = finergy.get_doc(
			{
				"address_line1": "_Test Address Line 1",
	...
	if not finergy.db.exists("Address", "_Test Supplier GST-2-Billing"):
		address = finergy.get_doc(
			{
				"address_line1": "_Test Address Line 1",
...
def make_customers():
	if not finergy.db.exists("Customer", "_Test GST Customer"):
		finergy.get_doc(
			{
				"customer_group": "_Test Customer Group",
	...
	if not finergy.db.exists("Customer", "_Test GST SEZ Customer"):
		finergy.get_doc(
			{
				"customer_group": "_Test Customer Group",
	...
	if not finergy.db.exists("Customer", "_Test Unregistered Customer"):
		finergy.get_doc(
			{
				"customer_group": "_Test Customer Group",
	...
	if not finergy.db.exists("Address", "_Test GST-1-Billing"):
		address = finergy.get_doc(
			{
				"address_line1": "_Test Address Line 1",
	...
	if not finergy.db.exists("Address", "_Test GST-2-Billing"):
		address = finergy.get_doc(
			{
				"address_line1": "_Test Address Line 1",
	...
	if not finergy.db.exists("Address", "_Test GST-3-Billing"):
		address = finergy.get_doc(
			{
				"address_line1": "_Test Address Line 1",
...
def make_company():
	if finergy.db.exists("Company", "_Test Company GST"):
		return

	company = finergy.new_doc("Company")
	...
	if not finergy.db.exists("Address", "_Test Address GST-Billing"):
		address = finergy.get_doc(
			{
