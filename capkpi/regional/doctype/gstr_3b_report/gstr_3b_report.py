...
from capkpi.regional.india import state_numbers
...
class GSTR3BReport(Document):
	...
	def get_data(self):
		...
		self.json_output = finergy.as_json(self.report_dict)
	...
	def get_itc_reversal_entries(self):
		reversal_entries = finergy.db.sql(
			"""
	...
	def get_inward_nil_exempt(self, state):
		inward_nil_exempt = finergy.db.sql(
			"""
	...
	def get_outward_tax_invoices(self, doctype, reverse_charge=None):
		...
		invoice_details = finergy.db.sql(
			"""
			SELECT
...
def get_address_state_map():
	return finergy._dict(finergy.get_all("Address", fields=["name", "gst_state"], as_list=1))
...
def view_report(name):
	json_data = finergy.get_value("GSTR 3B Report", name, "json_output")
...
def make_json(name):
	...
	finergy.local.response.filename = file_name
	finergy.local.response.filecontent = json_data
