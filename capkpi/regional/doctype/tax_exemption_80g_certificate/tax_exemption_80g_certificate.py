...
class TaxExemption80GCertificate(Document):
	...
	def validate_company_details(self):
		...
		company_details = finergy.db.get_value("Company", self.company, fields, as_dict=True)
		if not company_details.company_80g_number:
			finergy.throw(
				_("Please set the {0} for company {1}").format(
					finergy.bold("80G Number"), get_link_to_form("Company", self.company)
				)
			)
		...
		if not company_details.pan_details:
			finergy.throw(
				_("Please set the {0} for company {1}").format(
					finergy.bold("PAN Number"), get_link_to_form("Company", self.company)
				)
			)
	...
	def get_payments(self):
		if not self.member:
			finergy.throw(_("Please select a Member first."))

		fiscal_year = get_fiscal_year(fiscal_year=self.fiscal_year, as_dict=True)

		memberships = finergy.db.get_all(
			"Membership",
			{
				"member": self.member,
		...
		if not memberships:
			finergy.msgprint(_("No Membership Payments found against the Member {0}").format(self.member))
