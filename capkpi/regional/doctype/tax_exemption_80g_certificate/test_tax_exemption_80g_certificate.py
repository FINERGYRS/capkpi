...
from capkpi.non_profit.doctype.donation.donation import create_razorpay_donation
from capkpi.non_profit.doctype.donation.test_donation import (
	create_donor,
	create_donor_type,
	create_mode_of_payment,
)
from capkpi.non_profit.doctype.member.member import create_member
from capkpi.non_profit.doctype.membership.test_membership import make_membership, setup_membership


class TestTaxExemption80GCertificate(unittest.TestCase):
	def setUp(self):
		finergy.db.sql("delete from `tabTax Exemption 80G Certificate`")
		finergy.db.sql("delete from `tabMembership`")
		create_donor_type()
	...
	def test_duplicate_donation_certificate(self):
		...
		args = finergy._dict({"recipient": "Donor", "donor": donor.name, "donation": donation.name})
		...
		self.assertRaises(finergy.ValidationError, duplicate_certificate.insert)
	...
	def test_membership_80g_certificate(self):
		...
		member_doc = create_member(
			finergy._dict(
				{"fullname": "_Test_Member", "email": "_test_member_capkpi@example.com", "plan_id": plan.name}
			)
		)
