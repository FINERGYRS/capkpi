...
from capkpi.accounts.utils import FiscalYearError, get_fiscal_year
from capkpi.regional.india import states


def setup(company=None, patch=True):
	# Company independent fixtures should be called only once at the first company setup
	if patch or finergy.db.count("Company", {"country": "India"}) <= 1:
...
def setup_company_independent_fixtures(patch=False):
	...
	finergy.enqueue("capkpi.regional.india.setup.add_hsn_sac_codes", now=finergy.flags.in_test)
...
def add_hsn_sac_codes():
	if finergy.flags.in_test and finergy.flags.created_hsn_codes:
		return
	...
	if finergy.flags.in_test:
		finergy.flags.created_hsn_codes = True


def create_hsn_codes(data, code_field):
	for d in data:
		hsn_code = finergy.new_doc("GST HSN Code")
...
def add_print_formats():
	finergy.reload_doc("regional", "print_format", "gst_tax_invoice")
	finergy.reload_doc("accounts", "print_format", "gst_pos_invoice")
	finergy.reload_doc("accounts", "print_format", "GST E-Invoice")

	finergy.db.set_value("Print Format", "GST POS Invoice", "disabled", 0)
	finergy.db.set_value("Print Format", "GST Tax Invoice", "disabled", 0)
	finergy.db.set_value("Print Format", "GST E-Invoice", "disabled", 0)


def make_property_setters(patch=False):
	# GST rules do not allow for an invoice no. bigger than 16 characters
	journal_entry_types = finergy.get_meta("Journal Entry").get_options("voucher_type").split("\n") + [
		"Reversal Of ITC"
	]
	sales_invoice_series = ["SINV-.YY.-", "SRET-.YY.-", ""] + finergy.get_meta(
		"Sales Invoice"
	).get_options("naming_series").split("\n")
	purchase_invoice_series = ["PINV-.YY.-", "PRET-.YY.-", ""] + finergy.get_meta(
		"Purchase Invoice"
	).get_options("naming_series").split("\n")
...
def make_fixtures(company=None):
	docs = []
	company = company or finergy.db.get_value("Global Defaults", None, "default_company")
...
def update_regional_tax_settings(country, company):
	...
	gst_settings = finergy.get_single("GST Settings")
...
def set_tax_withholding_category(company):
	...
	abbr = finergy.get_value("Company", company, "abbr")
	tds_account = finergy.get_value("Account", "TDS Payable - {0}".format(abbr), "name")
	...
	for d in docs:
		if not finergy.db.exists("Tax Withholding Category", d.get("name")):
			doc = finergy.get_doc(d)
			...
			doc = finergy.get_doc("Tax Withholding Category", d.get("name"), for_update=True)
...
def create_gratuity_rule():
	...
	if not finergy.db.exists("Gratuity Rule", "Indian Standard Gratuity Rule"):
...
def update_accounts_settings_for_taxes():
	if finergy.db.count("Company") == 1:
		finergy.db.set_value("Accounts Settings", None, "add_taxes_from_item_tax_template", 0)
