...
from finergy.integrations.utils import make_get_request, make_post_request
...
from capkpi.regional.india.utils import get_gst_accounts, get_place_of_supply
...
def validate_eligibility(doc):
	...
	einvoicing_enabled = cint(finergy.db.get_single_value("E Invoice Settings", "enable"))
	if not einvoicing_enabled:
		return False

	einvoicing_eligible_from = (
		finergy.db.get_single_value("E Invoice Settings", "applicable_from") or "2021-04-01"
	)
	if getdate(doc.get("posting_date")) < getdate(einvoicing_eligible_from):
		return False

	invalid_company = not finergy.db.get_value("E Invoice User", {"company": doc.get("company")})
	invalid_company_gstin = not finergy.db.get_value(
...
def validate_einvoice_fields(doc):
	...
	if doc.docstatus == 0 and doc._action == "save":
		if doc.irn:
			finergy.throw(_("You cannot edit the invoice after generating IRN"), title=_("Edit Not Allowed"))
		if len(doc.name) > 16:
		...
		finergy.throw(_("You must generate IRN before submitting the document."), title=_("Missing IRN"))
...
def get_party_details(address_name, skip_gstin_validation=False):
	addr = finergy.get_doc("Address", address_name)
	...
	party_address_details = finergy._dict(
		dict(
...
def get_overseas_address_details(address_name):
	address_title, address_line1, address_line2, city = finergy.db.get_value(
...
def get_item_list(invoice):
	...
	hide_discount_in_einvoice = cint(
		finergy.db.get_single_value("E Invoice Settings", "dont_show_discounts_in_e_invoice")
	)
...
def get_invoice_value_details(invoice):
	invoice_value_details = finergy._dict(dict())
...
def get_return_doc_reference(invoice):
	invoice_date = finergy.db.get_value("Sales Invoice", invoice.return_against, "posting_date")
	return finergy._dict(
...
def validate_mandatory_fields(invoice):
	...
	if not finergy.db.get_value("Address", invoice.company_address, "gstin"):
		finergy.throw(
			_(
	...
	if invoice.gst_category != "Overseas" and not finergy.db.get_value(
...
def make_einvoice(invoice):
	...
	shipping_details = payment_details = prev_doc_details = eway_bill_details = finergy._dict({})
	...
	dispatch_details = finergy._dict({})
	...
	# not yet implemented
	period_details = export_details = finergy._dict({})
...
def log_error(data=None):
	...
	return finergy.log_error(title=_("E Invoice Request Failed"), message=message)
...
class GSPConnector:
	...
	def set_invoice(self):
		...
		if self.doctype and self.docname:
			self.invoice = finergy.get_cached_doc(self.doctype, self.docname)

	def set_credentials(self):
		self.e_invoice_settings = finergy.get_cached_doc("E Invoice Settings")
	...
	def get_seller_gstin(self):
		gstin = finergy.db.get_value("Address", self.invoice.company_address, "gstin")
		if not gstin:
			finergy.throw(
	...
	def get_client_credentials(self):
		...
		return finergy.conf.einvoice_client_id, finergy.conf.einvoice_client_secret
	...
	def get_gstin_details(gstin):
		...
		if not hasattr(finergy.local, "gstin_cache"):
			finergy.local.gstin_cache = {}

		key = gstin
		gsp_connector = GSPConnector()
		details = gsp_connector.fetch_gstin_details(gstin)

		finergy.local.gstin_cache[key] = details
		finergy.cache().hset("gstin_cache", key, details)
		return details
			...
			if res.get("success"):
				...
				if irn_details:
					...
					raise RequestFailed(
						...
						Contact CapKPI support to resolve the issue."
					)
	...
	def fetch_and_attach_qrcode_from_irn(self):
		is_qrcode_file_attached = self.invoice.qrcode_image and finergy.db.exists(
			"File",
			{
		...
		if not is_qrcode_file_attached:
			if self.invoice.signed_qr_code:
				...
				finergy.msgprint(_("QR Code attached to the invoice."), alert=True)
			else:
				qrcode = self.get_qrcode_from_irn(self.invoice.irn)
				if qrcode:
					qrcode_file = self.create_qr_code_file(qrcode)
					finergy.db.set_value("Sales Invoice", self.invoice.name, "qrcode_image", qrcode_file.file_url)
					finergy.msgprint(_("QR Code attached to the invoice."), alert=True)
				else:
					finergy.msgprint(_("QR Code not found for the IRN"), alert=True)
		else:
			finergy.msgprint(_("QR Code is already Attached"), indicator="green", alert=True)
	...
	def get_ewb_details(self):
		...
		if not irn:
			finergy.throw(_("IRN is mandatory to get E-Waybill Details. Please generate IRN first."))
	...
	def update_ewb_details(self, ewb_details=None):
		...
		if not self.invoice.irn:
			finergy.throw(_("IRN is mandatory to update E-Waybill Details. Please generate IRN first."))
		if not ewb_details:
	...
	def cancel_eway_bill(self):
		...
		if ewb_details:
			...
			if ewb_status == "CNL":
				...
				msg += "<br><br>"
				msg += _(
					"You must first use the portal to cancel the e-way bill and then update the cancelled status in the CapKPI system."
				)
			...
			finergy.msgprint(_("E-Way Bill Details not found for this IRN."))
	...
	def raise_error(self, raise_exception=False, errors=None):
		...
		if errors:
			finergy.throw(errors, title=title, as_list=1)
		else:
...
def get_einvoice(doctype, docname):
	invoice = finergy.get_doc(doctype, docname)
...
def schedule_bulk_generate_irn(docnames):
	...
	finergy.publish_realtime(
		...
		{"user": finergy.session.user, "failures": failures, "invoices": docnames},
	)
...
def show_bulk_action_failure_message(failures):
	for doc in failures:
		...
		finergy.msgprint(message, title=_("Bulk E-Invoice Generation Complete"), indicator="red")
...
def enqueue_bulk_action(job, **kwargs):
	...
	finergy.msgprint(msg, alert=1)


def check_scheduler_status():
	if is_scheduler_inactive() and not finergy.flags.in_test:
		finergy.throw(_("Scheduler is inactive. Cannot enqueue job."), title=_("Scheduler Inactive"))
