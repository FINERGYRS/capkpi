capkpi.setup_einvoice_actions = (doctype) => {
	finergy.ui.form.on(doctype, {
		async refresh(frm) {
			if (frm.doc.docstatus == 2) return;

			const res = await finergy.call({
				method: 'capkpi.regional.india.e_invoice.utils.validate_eligibility',
				args: { doc: frm.doc }
			});
			...
			if (!irn && !__unsaved) {
				const action = () => {
					if (frm.doc.__unsaved) {
						finergy.throw(__('Please save the document to generate IRN.'));
					}
					finergy.call({
						method: 'capkpi.regional.india.e_invoice.utils.get_einvoice',
				...
				const action = () => {
					const d = new finergy.ui.Dialog({
						...
						primary_action: function() {
							...
							finergy.call({
								method: 'capkpi.regional.india.e_invoice.utils.cancel_irn',
								args: {
									doctype,
									docname: name,
									irn: irn,
				...
				const action = () => {
					const d = new finergy.ui.Dialog({
						...
						primary_action: function() {
							const data = d.get_values();
							finergy.call({
								method: 'capkpi.regional.india.e_invoice.utils.generate_eway_bill',
								args: {
									doctype,
									docname: name,
									irn,
					...
					d.fields_dict.transporter.df.onchange = function () {
						...
						if (transporter) {
							finergy.db.get_value('Supplier', transporter, ['gst_transporter_id', 'supplier_name'])
								.then(({ message }) => {
					...
					d.fields_dict.driver.df.onchange = function () {
						...
						if (driver) {
							finergy.db.get_value('Driver', driver, ['full_name'])
								.then(({ message }) => {
			...
			if (irn && ewaybill && !irn_cancelled && !eway_bill_cancelled) {
				const action = () => {
					...
					finergy.confirm(
						...
						() => {
							finergy.call({
								method: "capkpi.regional.india.e_invoice.utils.cancel_eway_bill",
								args: { doctype, docname: name },
								freeze: true,
			...
			if (irn && !irn_cancelled) {
				...
				if (!is_qrcode_attached) {
					const action = () => {
						if (frm.doc.__unsaved) {
							finergy.throw(__('Please save the document to generate QRCode.'));
						}
						const dialog = finergy.msgprint({
							title: __("Generate QRCode"),
							message: __("Generate and attach QR Code using IRN?"),
							primary_action: {
								action: function() {
									finergy.call({
										method: 'capkpi.regional.india.e_invoice.utils.generate_qrcode',
										args: { doctype, docname: name },
										freeze: true,
...
const request_irn_generation = (frm) => {
	finergy.call({
		method: 'capkpi.regional.india.e_invoice.utils.generate_irn',
...
const show_einvoice_preview = (frm, einvoice) => {
	...
	einvoice["Irn"] = einvoice["AckNo"] = ''; einvoice["AckDt"] = finergy.datetime.nowdate();
	...
	finergy.call({
		...
		callback: function (r) {
			if (!r.exc) {
				...
				finergy.dom.set_style(style, "custom-print-style");
				preview_dialog.show();
			}
		}
	});
