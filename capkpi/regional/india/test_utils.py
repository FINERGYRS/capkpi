...
from capkpi.regional.india.utils import validate_document_name


class TestIndiaUtils(unittest.TestCase):
	@patch("finergy.get_cached_value")
	def test_validate_document_name(self, mock_get_cached):
		...
		for name in invalid_names:
			doc = finergy._dict(name=name, posting_date=posting_date)
			self.assertRaises(finergy.ValidationError, validate_document_name, doc)
	...
	def test_validate_document_name_not_india(self, mock_get_cached):
		...
		doc = finergy._dict(name="SI$123", posting_date="2021-05-01")
