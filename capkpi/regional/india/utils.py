...
from capkpi.controllers.accounts_controller import get_taxes_and_charges
from capkpi.controllers.taxes_and_totals import get_itemised_tax, get_itemised_taxable_amount
from capkpi.hr.utils import get_salary_assignments
from capkpi.payroll.doctype.salary_structure.salary_structure import make_salary_slip
from capkpi.regional.india import number_state_mapping, state_numbers, states
...
def validate_gstin_for_india(doc, method):
	...
	if len(doc.links):
		...
		if link_doctype in ["Customer", "Supplier"]:
			gst_category = finergy.db.get_value(link_doctype, {"name": link_name}, ["gst_category"])
	...
	if len(doc.gstin) != 15:
		finergy.throw(_("A GSTIN must have 15 characters."), title=_("Invalid GSTIN"))
	...
	if gst_category and gst_category == "UIN Holders":
		...
		if not doc.gst_state:
			finergy.throw(_("Please enter GST state"), title=_("Invalid State"))
...
def validate_pan_for_india(doc, method):
	...
	if not PAN_NUMBER_FORMAT.match(doc.pan):
		finergy.throw(_("Invalid PAN No. The input you've entered doesn't match the format of PAN."))


def validate_tax_category(doc, method):
	if doc.get("gst_state") and finergy.db.get_value(
		"Tax Category",
		{
...
def update_gst_category(doc, method):
	for link in doc.links:
		if link.link_doctype in ["Customer", "Supplier"]:
			meta = finergy.get_meta(link.link_doctype)
...
def get_itemised_tax_breakup_header(item_doctype, tax_accounts):
	hsn_wise_in_gst_settings = finergy.db.get_single_value("GST Settings", "hsn_wise_tax_breakup")
	if finergy.get_meta(item_doctype).has_field("gst_hsn_code") and hsn_wise_in_gst_settings:
...
def get_itemised_tax_breakup_data(doc, account_wise=False, hsn_wise=False):
	...
	if not finergy.get_meta(doc.doctype + " Item").has_field("gst_hsn_code"):
	...
	for item, taxes in itemised_tax.items():
		...
		hsn_tax.setdefault(item_or_hsn, finergy._dict())
	...
	# set taxable amount
	hsn_taxable_amount = finergy._dict()
...
def validate_document_name(doc, method=None):
	...
	country = finergy.get_cached_value("Company", doc.company, "country")
...
def get_place_of_supply(party_details, doctype):
	if not finergy.get_meta("Address").has_field("gst_state"):
		return
...
def get_regional_address_details(party_details, doctype, company):
	if isinstance(party_details, string_types):
		...
		party_details = finergy._dict(party_details)
...
def get_tax_template_based_on_category(master_doctype, company, party_details):
	...
	default_tax = finergy.db.get_value(
...
def get_tax_template(master_doctype, company, is_inter_state, state_code):
	tax_categories = finergy.get_all(
		"Tax Category",
...
def calculate_annual_eligible_hra_exemption(doc):
	basic_component, hra_component = finergy.db.get_value(
...
def get_end_date_for_assignment(assignment_dates, idx, payroll_period):
	...
	if not end_date:
		end_date = finergy.db.get_value("Payroll Period", payroll_period, "end_date")
...
def validate_house_rent_dates(doc):
	if not doc.rented_to_date or not doc.rented_from_date:
		finergy.throw(_("House rented dates required for exemption calculation"))

	if date_diff(doc.rented_to_date, doc.rented_from_date) < 14:
		finergy.throw(_("House rented dates should be atleast 15 days apart"))

	proofs = finergy.db.sql(
		"""
		select name
	...
	if proofs:
		finergy.throw(_("House rent paid days overlapping with {0}").format(proofs[0][0]))
...
def get_ewb_data(dt, dn):
	...
	ewaybills = []
	for doc_name in dn:
		doc = finergy.get_doc(dt, doc_name)
		...
		if dt == "Delivery Note":
			...
			finergy.throw(_("Unsupported GST Category for E-Way Bill JSON generation"))

		data.docType = "INV"
		data.docDate = finergy.utils.formatdate(doc.posting_date, "dd/mm/yyyy")

		company_address = finergy.get_doc("Address", doc.company_address)
		billing_address = finergy.get_doc("Address", doc.customer_address)

		# added dispatch address
		dispatch_address = (
			finergy.get_doc("Address", doc.dispatch_address_name)
			if doc.dispatch_address_name
			else company_address
		)
		shipping_address = finergy.get_doc("Address", doc.shipping_address_name)
		...
		disable_rounded = finergy.db.get_single_value("Global Defaults", "disable_rounded_total")
...
def download_ewb_json():
	data = json.loads(finergy.local.form_dict.data)
	finergy.local.response.filecontent = json.dumps(data, indent=4, sort_keys=True)
	finergy.local.response.type = "download"

	filename_prefix = "Bulk"
	docname = finergy.local.form_dict.docname
	if docname:
	...
	finergy.local.response.filename = "{0}_e-WayBill_Data_{1}.json".format(
		filename_prefix, finergy.utils.random_string(5)
	)
...
def get_item_list(data, doc, hsn_wise=False):
	...
	for item_or_hsn, taxable_amount in hsn_taxable_amount.items():
		item_data = finergy._dict()
		if not item_or_hsn:
			finergy.throw(_("GST HSN Code does not exist for one or more items"))
...
def validate_doc(doc):
	if doc.docstatus != 1:
		finergy.throw(_("E-Way Bill JSON can only be generated from submitted document"))

	if doc.is_return:
		finergy.throw(_("E-Way Bill JSON cannot be generated for Sales Return as of now"))

	if doc.ewaybill:
		finergy.throw(_("e-Way Bill already exists for this document"))
	...
	if len(doc.company_gstin) < 15:
		finergy.throw(_("You must be a registered supplier to generate e-Way Bill"))


def get_transport_details(data, doc):
	if doc.distance > 4000:
		finergy.throw(_("Distance cannot be greater than 4000 kms"))
	...
	if doc.mode_of_transport == "Road":
		...
		if not doc.gst_vehicle_type:
			finergy.throw(_("Vehicle Type is required if Mode of Transport is Road"))
		else:
			data.vehicleType = vehicle_types.get(doc.gst_vehicle_type)
	else:
		if not doc.lr_no or not doc.lr_date:
			finergy.throw(_("Transport Receipt No and Date are mandatory for your chosen Mode of Transport"))

	if doc.lr_no:
		data.transDocNo = doc.lr_no

	if doc.lr_date:
		data.transDocDate = finergy.utils.formatdate(doc.lr_date, "dd/mm/yyyy")
...
def validate_pincode(pincode, address):
	...
	if not pincode:
		finergy.throw(_(pin_not_found.format(address)))

	pincode = pincode.replace(" ", "")
	if not pincode.isdigit() or len(pincode) != 6:
		finergy.throw(_(incorrect_pin.format(address)))
	else:
		return int(pincode)
...
def validate_state_code(state_code, address):
	...
	if not state_code:
		finergy.throw(_(no_state_code.format(address)))
	else:
...
def get_gst_accounts(
	...
	gst_accounts = finergy._dict()
	gst_settings_accounts = finergy.get_all(
		"GST Account",
		filters=filters,
		fields=["cgst_account", "sgst_account", "igst_account", "cess_account", "utgst_account"],
	)

	if not gst_settings_accounts and not finergy.flags.in_test and not finergy.flags.in_migrate:
		finergy.throw(_("Please set GST Accounts in GST Settings"))
		...
		for tax in doc.get("taxes"):
			for tax in doc.get("taxes"):
				if (
					...
					finergy.throw(_("GST cannot be applied on SEZ or Export invoices without payment of tax"))
...
def get_regional_round_off_accounts(company, account_list):
	country = finergy.get_cached_value("Company", company, "country")
	...
	if not finergy.db.get_single_value("GST Settings", "round_off_gst_values"):
		return
...
def get_depreciation_amount(asset, depreciable_value, row):
	if row.depreciation_method in ("Straight Line", "Manual"):
		...
		if depreciable_value == asset.gross_purchase_amount:
			if row.finance_book and finergy.db.get_value("Finance Book", row.finance_book, "for_income_tax"):
...
def set_item_tax_from_hsn_code(item):
	if not item.taxes and item.gst_hsn_code:
		hsn_doc = finergy.get_doc("GST HSN Code", item.gst_hsn_code)
