...
from finergy.utils import formatdate, get_link_to_form
...
class VATAuditReport(object):
	...
	def get_sa_vat_accounts(self):
		self.sa_vat_accounts = finergy.get_all(
			"South Africa VAT Account", filters={"parent": self.filters.company}, pluck="account"
		)
		if not self.sa_vat_accounts and not finergy.flags.in_test and not finergy.flags.in_migrate:
			link_to_settings = get_link_to_form(
				"South Africa VAT Settings", "", label="South Africa VAT Settings"
			)
			finergy.throw(_("Please set VAT Accounts in {0}").format(link_to_settings))
	...
	def get_items_based_on_tax_rate(self, doctype):
		self.items_based_on_tax_rate = finergy._dict()
	...
	def get_data(self, doctype):
		...
		for rate, section in consolidated_data.items():
			rate = int(rate)
			label = finergy.bold(section_name + "- " + "Rate" + " " + str(rate) + "%")
			...
			total = {
				"posting_date": finergy.bold(_("Total")),
