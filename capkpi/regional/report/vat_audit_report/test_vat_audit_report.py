...
from capkpi.regional.report.vat_audit_report.vat_audit_report import execute
...
class TestVATAuditReport(TestCase):
	...
	def tearDown(self):
		finergy.db.sql("delete from `tabSales Invoice` where company='_Test Company SA VAT'")
		finergy.db.sql("delete from `tabPurchase Invoice` where company='_Test Company SA VAT'")
...
def set_sa_vat_accounts():
	if not finergy.db.exists("South Africa VAT Settings", "_Test Company SA VAT"):
...
def make_customer():
	if not finergy.db.exists("Customer", "_Test SA Customer"):
		finergy.get_doc(
			{
				"doctype": "Customer",
...
def make_supplier():
	if not finergy.db.exists("Supplier", "_Test SA Supplier"):
		finergy.get_doc(
			{
				"doctype": "Supplier",
