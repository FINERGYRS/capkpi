...
import capkpi
from capkpi.regional.india.utils import get_gst_accounts
from capkpi.regional.report.gstr_1.gstr_1 import get_company_gstin_number
...
def _execute(filters=None):
	...
	company_currency = capkpi.get_company_currency(filters.company)
...
def get_output_gst_accounts(company):
	...
	accounts = finergy.qb.DocType("Account")
	gst_accounts = finergy.qb.DocType("GST Account")

	accounts_query = (
		finergy.qb.from_(accounts)
		.select(accounts.name)
		.where((accounts.account_type == "Tax") & (accounts.root_type == "Liability"))
	)

	gst_accounts_query = (
		finergy.qb.from_(gst_accounts)
		.select(
...
def get_items(filters):
	...
	match_conditions = finergy.build_match_conditions("Sales Invoice")
	if match_conditions:
...
def get_tax_accounts(
	...
	tax_amount_precision = (
		get_field_precision(
			finergy.get_meta(tax_doctype).get_field("tax_amount"), currency=company_currency
		)
		or 2
	)
		...
		if item_wise_tax_detail:
			try:
				...
				for item_code, tax_data in item_wise_tax_detail.items():
					if not finergy.db.get_value("Item", item_code, "gst_hsn_code"):
						continue
					...
					for d in item_row_map.get(parent, {}).get(item_code, []):
						...
						if item_tax_amount:
							itemised_tax.setdefault((parent, item_code), {})[account_head] = finergy._dict(
								{
	...
	for account_head in tax_columns:
		if account_head in output_gst_accounts:
			columns.append(
				{
					"label": account_head,
					"fieldname": finergy.scrub(account_head),
					"fieldtype": "Float",
					"width": 110,
				}
			)
...
def get_json(filters, report_name, data):
	...
	if not filters.get("from_date") or not filters.get("to_date"):
		finergy.throw(_("Please enter From Date and To Date to generate JSON"))
...
def download_json_file():
	...
	finergy.response["filename"] = finergy.scrub("{0}".format(data["report_name"])) + ".json"
	finergy.response["filecontent"] = data["data"]
	finergy.response["content_type"] = "application/json"
...
def get_hsn_wise_json_data(filters, report_data):
	...
	for hsn in report_data:
		...
		for account in gst_accounts.get("igst_account"):
			row["iamt"] += flt(hsn.get(finergy.scrub(cstr(account)), 0.0), 2)

		for account in gst_accounts.get("cgst_account"):
			row["camt"] += flt(hsn.get(finergy.scrub(cstr(account)), 0.0), 2)

		for account in gst_accounts.get("sgst_account"):
			row["samt"] += flt(hsn.get(finergy.scrub(cstr(account)), 0.0), 2)

		for account in gst_accounts.get("cess_account"):
			row["csamt"] += flt(hsn.get(finergy.scrub(cstr(account)), 0.0), 2)
