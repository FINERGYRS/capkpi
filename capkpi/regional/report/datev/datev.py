...
from capkpi.regional.germany.utils.datev.datev_csv import get_datev_csv, zip_and_download
...
def execute(filters=None):
	"""Entry point for finergy."""
	data = []
	if filters and validate(filters):
		fn = "temporary_against_account_number"
		filters[fn] = finergy.get_value("DATEV Settings", filters.get("company"), fn)
...
def validate(filters):
	...
	if not company:
		finergy.throw(_("<b>Company</b> is a mandatory filter."))

	from_date = filters.get("from_date")
	if not from_date:
		finergy.throw(_("<b>From Date</b> is a mandatory filter."))

	to_date = filters.get("to_date")
	if not to_date:
		finergy.throw(_("<b>To Date</b> is a mandatory filter."))

	validate_fiscal_year(from_date, to_date, company)

	if not finergy.db.exists("DATEV Settings", filters.get("company")):
		msg = "Please create DATEV Settings for Company {}".format(filters.get("company"))
		finergy.log_error(msg, title="DATEV Settings missing")
		return False
...
def validate_fiscal_year(from_date, to_date, company):
	...
	if from_fiscal_year != to_fiscal_year:
		finergy.throw(_("Dates {} and {} are not in the same fiscal year.").format(from_date, to_date))
...
def run_query(filters, extra_fields, extra_joins, extra_filters, as_dict=1):
	...
	gl_entries = finergy.db.sql(query, filters, as_dict=as_dict)
...
def download_datev_csv(filters):
	...
	GET /api/method/capkpi.regional.report.datev.datev.download_datev_csv
	...
	coa = finergy.get_value("Company", company, "chart_of_accounts")
	filters["skr"] = "04" if "SKR04" in coa else ("03" if "SKR03" in coa else "")

	datev_settings = finergy.get_doc("DATEV Settings", company)
	...
	zip_name = "{} DATEV.zip".format(finergy.utils.datetime.date.today())
	zip_and_download(
		zip_name,
		[
			{
