...
import finergy
from finergy.utils import cstr, now_datetime, today
from six import BytesIO

from capkpi.accounts.doctype.sales_invoice.test_sales_invoice import create_sales_invoice
from capkpi.regional.germany.utils.datev.datev_constants import (
	AccountNames,
	DebtorsCreditors,
	Transactions,
)
from capkpi.regional.germany.utils.datev.datev_csv import get_datev_csv, get_header
from capkpi.regional.report.datev.datev import (
	download_datev_csv,
	get_account_names,
	get_customers,
	get_suppliers,
...
def setup_fiscal_year():
	...
	if not finergy.db.get_value("Fiscal Year", {"year": year}, "name"):
		try:
...
def make_customer_with_account(customer_name, company):
	...
	if not finergy.db.exists("Customer", customer_name):
		...
		customer = finergy.get_doc("Customer", customer_name)

	return customer


def make_item(item_code, company):
	warehouse_name = finergy.db.get_value(
...
def make_datev_settings(company):
	if not finergy.db.exists("DATEV Settings", company.name):
		finergy.get_doc(
			{
...
class TestDatev(TestCase):
	...
	def test_columns(self):
		def is_subset(get_data, allowed_keys):
			...
			allowed_set = set({finergy.as_unicode(key) for key in allowed_keys})
	...
	def test_download(self):
		...
		zip_buffer = BytesIO()
		zip_buffer.write(finergy.response["filecontent"])
