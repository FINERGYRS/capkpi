...
from finergy.utils import flt, formatdate, getdate
...
class Gstr1Report(object):
	...
	def get_invoice_data(self):
		self.invoices = finergy._dict()
		conditions = self.get_conditions()

		invoice_data = finergy.db.sql(
			"""
			select
				{select_columns}
			from `tab{doctype}`
	...
	def get_conditions(self):
		...
		if self.filters.get("type_of_business") in ("B2C Large", "B2C Small"):
			b2c_limit = finergy.db.get_single_value("GST Settings", "b2c_limit")
			if not b2c_limit:
				finergy.throw(_("Please set B2C Limit in GST Settings."))
	...
	def get_invoice_items(self):
		self.invoice_items = finergy._dict()
		self.item_tax_rate = finergy._dict()
		self.item_hsn_map = finergy._dict()
	...
	def get_items_based_on_tax_rate(self):
		...
		self.tax_details = finergy.db.sql(
			"""
			select
		...
		self.invoice_cess = finergy._dict()
		...
		for item in report_data[:-1]:
			if not item.get("place_of_supply"):
				finergy.throw(
					...
					).format(finergy.bold("Place Of Supply"))
				)
			...
			if not invoice[0]["place_of_supply"]:
				finergy.throw(
					...
					Please update and try again"""
						...
						finergy.bold("Place Of Supply"), finergy.bold(invoice[0]["invoice_number"])
					)
				)
...
def get_company_gstin_number(company, address=None, all_gstins=False):
	gstin = ""
	if address:
		gstin = finergy.db.get_value("Address", address, "gstin")
	...
	if not gstin:
		address = finergy.bold(address) if address else ""
		finergy.throw(
			_("Please set valid GSTIN No. in Company Address {} for company {}").format(
				address, finergy.bold(company)
			)
		)
...
def download_json_file():
	...
	finergy.response["filename"] = (
		finergy.scrub("{0} {1}".format(data["report_name"], data["report_type"])) + ".json"
	)
...
def get_company_gstins(company):
	address = finergy.qb.DocType("Address")
	links = finergy.qb.DocType("Dynamic Link")

	addresses = (
		finergy.qb.from_(address)
		.inner_join(links)
...
def get_hsn_wise_tax_rates():
	hsn_wise_tax_rate = {}
	gst_hsn_code = finergy.qb.DocType("GST HSN Code")
	hsn_tax_rates = finergy.qb.DocType("HSN Tax Rate")

	hsn_code_data = (
		finergy.qb.from_(gst_hsn_code)
