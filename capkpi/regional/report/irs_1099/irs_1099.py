...
from finergy.utils import cstr, nowdate
from finergy.utils.data import fmt_money
from finergy.utils.jinja import render_template
from finergy.utils.pdf import get_pdf
from finergy.utils.print_format import read_multi_pdf
...
def execute(filters=None):
	filters = filters if isinstance(filters, finergy._dict) else finergy._dict(filters)
	if not filters:
		filters.setdefault("fiscal_year", get_fiscal_year(nowdate())[0])
		filters.setdefault("company", finergy.db.get_default("company"))

	region = finergy.db.get_value("Company", filters={"name": filters.company}, fieldname=["country"])
	...
	if filters.supplier_group:
		conditions += "AND s.supplier_group = %s" % finergy.db.escape(filters.get("supplier_group"))
...
def irs_1099_print(filters):
	if not filters:
		finergy._dict(
			{
				"company": finergy.db.get_default("Company"),
				"fiscal_year": finergy.db.get_default("Fiscal Year"),
			}
		)
	else:
	...
	company_tin = finergy.db.get_value("Company", filters.company, "tax_id")

	columns, data = execute(filters)
	template = finergy.get_doc("Print Format", "IRS 1099 Form").html
	...
	finergy.local.response.filename = (
...
def get_payer_address_html(company):
	...
	if address_list:
		...
		address_display = finergy.get_doc("Address", company_address).get_display()
...
def get_street_address_html(party_type, party):
	...
	if address_list:
		...
		doc = finergy.get_doc("Address", supplier_address)
