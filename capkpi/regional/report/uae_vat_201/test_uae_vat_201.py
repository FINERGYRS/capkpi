...
from capkpi.regional.report.uae_vat_201.uae_vat_201 import (
	get_exempt_total,
...
from capkpi.stock.doctype.warehouse.test_warehouse import get_warehouse_account
...
class TestUaeVat201(TestCase):
	def setUp(self):
		...
		finergy.db.sql("delete from `tabSales Invoice` where company='_Test Company UAE VAT'")
		finergy.db.sql("delete from `tabPurchase Invoice` where company='_Test Company UAE VAT'")
...
def make_company(company_name, abbr):
	if not finergy.db.exists("Company", company_name):
		...
		company = finergy.get_doc("Company", company_name)

	company.create_default_warehouses()

	if not finergy.db.get_value("Cost Center", {"is_group": 0, "company": company.name}):
...
def set_vat_accounts():
	if not finergy.db.exists("UAE VAT Settings", "_Test Company UAE VAT"):
		vat_accounts = finergy.get_all(
			"Account",
			fields=["name"],
...
def make_customer():
	if not finergy.db.exists("Customer", "_Test UAE Customer"):
		...
		customer = finergy.get_doc("Customer", "_Test UAE Customer")


def make_supplier():
	if not finergy.db.exists("Supplier", "_Test UAE Supplier"):
		finergy.get_doc(
			{
...
def create_warehouse(warehouse_name, properties=None, company=None):
	...
	if not finergy.db.exists("Warehouse", warehouse_id):
		warehouse = finergy.new_doc("Warehouse")
