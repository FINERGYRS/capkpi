...
from capkpi.controllers.taxes_and_totals import get_itemised_tax
...
def export_invoices(filters=None):
	finergy.has_permission("Sales Invoice", throw=True)
	...
	zip_filename = "{0}-einvoices.zip".format(finergy.utils.get_datetime().strftime("%Y%m%d_%H%M%S"))
...
def prepare_invoice(invoice, progressive_number):
	...
	company = finergy.get_doc("Company", invoice.company)
	...
	company_address = finergy.get_doc("Address", invoice.company_address)
	...
	if not invoice.type_of_document:
		if invoice.is_return and invoice.return_against:
			...
			invoice.return_against_unamended = get_unamended_name(
				finergy.get_doc("Sales Invoice", invoice.return_against)
			)
		else:
			invoice.type_of_document = "TD01"  # Sales Invoice (Fattura)

	# set customer information
	invoice.customer_data = finergy.get_doc("Customer", invoice.customer)
	customer_address = finergy.get_doc("Address", invoice.customer_address)
	invoice.customer_address_data = customer_address

	if invoice.shipping_address_name:
		invoice.shipping_address_data = finergy.get_doc("Address", invoice.shipping_address_name)
...
def download_zip(files, output_filename):
	...
	with zipfile.ZipFile(zip_stream, "w", zipfile.ZIP_DEFLATED) as zip_file:
		for file in files:
			file_path = finergy.utils.get_files_path(file.file_name, is_private=file.is_private)

			zip_file.write(file_path, arcname=file.file_name)

	finergy.local.response.filename = output_filename
	finergy.local.response.filecontent = zip_stream.getvalue()
	finergy.local.response.type = "download"
	zip_stream.close()


def get_invoice_summary(items, taxes):
	summary_data = finergy._dict()
	for tax in taxes:
			...
			if reference_row:
				items.append(
					finergy._dict(
						...
						stock_uom=finergy.db.get_single_value("Stock Settings", "stock_uom") or _("Nos"),
						tax_rate=tax.rate,
...
def sales_invoice_validate(doc):
	...
	company_fiscal_regime = finergy.get_cached_value("Company", doc.company, "fiscal_regime")
	...
	doc.company_tax_id = finergy.get_cached_value("Company", doc.company, "tax_id")
	doc.company_fiscal_code = finergy.get_cached_value("Company", doc.company, "fiscal_code")
	...
	customer = finergy.get_doc("Customer", doc.customer)
	...
	if not doc.customer_address:
		finergy.throw(_("Please set the Customer Address"), title=_("E-Invoicing Information Missing"))
	else:
	...
	for schedule in doc.payment_schedule:
		if schedule.mode_of_payment and not schedule.mode_of_payment_code:
			schedule.mode_of_payment_code = finergy.get_cached_value(
...
def sales_invoice_on_submit(doc, method):
	...
	if not len(doc.payment_schedule):
		finergy.throw(_("Please set the Payment Schedule"), title=_("E-Invoicing Information Missing"))
	else:
		for schedule in doc.payment_schedule:
...
def prepare_and_attach_invoice(doc, replace=False):
	...
	item_meta = finergy.get_meta("Sales Invoice Item")

	invoice_xml = finergy.render_template(
		"capkpi/regional/italy/e-invoice.xml",
...
def generate_single_invoice(docname):
	doc = finergy.get_doc("Sales Invoice", docname)
	finergy.has_permission("Sales Invoice", doc=doc, throw=True)
...
def get_company_country(company):
	return finergy.get_cached_value("Company", company, "country")
...
def validate_address(address_name):
	...
	data = finergy.get_cached_value("Address", address_name, fields, as_dict=1) or {}
...
def get_progressive_name_and_number(doc, replace=False):
	...
	progressive_name = finergy.model.naming.make_autoname(company_tax_id + "_.#####")
