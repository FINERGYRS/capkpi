...
def update_address_template(country, html, is_default=0):
	...
	if not finergy.db.exists("Country", country):
		finergy.log_error("Country {} for regional Address Template does not exist.".format(country))
		return

	if finergy.db.exists("Address Template", country):
		finergy.db.set_value("Address Template", country, "template", html)
		finergy.db.set_value("Address Template", country, "is_default", is_default)
	else:
		finergy.get_doc(
