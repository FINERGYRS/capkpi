...
finergy.ui.form.on('Subcontracting Receipt', {
	setup: (frm) => {
		...
		if (!frm.doc.is_return && frm.doc.docstatus == 1 && frm.doc.per_returned < 100) {
			frm.add_custom_button(__('Subcontract Return'), function () {
				finergy.model.open_mapped_doc({
					method: 'capkpi.subcontracting.doctype.subcontracting_receipt.subcontracting_receipt.make_subcontract_return',
					frm: frm
				});
			}, __('Create'));
...
finergy.ui.form.on('Subcontracting Receipt Item', {
	item_code(frm) {
...
finergy.ui.form.on('Subcontracting Receipt Supplied Item', {
	consumed_qty(frm) {
		set_missing_values(frm);
	},
});
