...
class SubcontractingReceipt(SubcontractingController):
	...
	def validate(self):
		...
		if getdate(self.posting_date) > getdate(nowdate()):
			finergy.throw(_("Posting Date cannot be future date"))
	...
	def update_status(self, status=None, update_modified=False):
		if self.docstatus >= 1 and not status:
			if self.docstatus == 1:
				if self.is_return:
					status = "Return"
					return_against = finergy.get_doc("Subcontracting Receipt", self.return_against)
		...
		if status:
			finergy.db.set_value("Subcontracting Receipt", self.name, "status", status, update_modified)
