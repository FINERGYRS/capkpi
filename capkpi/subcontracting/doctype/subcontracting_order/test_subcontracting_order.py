...
from capkpi.buying.doctype.purchase_order.purchase_order import get_mapped_subcontracting_order
...
class TestSubcontractingOrder(FinergyTestCase):
	def setUp(self):
		make_subcontracted_items()
		make_raw_materials()
		make_service_items()
	...
	def test_update_reserved_qty_for_subcontracting(self):
		...
		ste = finergy.get_doc(make_rm_stock_entry(sco.name, rm_items))
	...
	def test_exploded_items(self):
		...
		item_name = finergy.db.get_value("BOM", {"item": item_code}, "name")
		bom = finergy.get_doc("BOM", item_name)
...
def create_subcontracting_order(**args):
	...
	if args.get("warehouse"):
		...
		warehouse = finergy.get_value("Purchase Order", args.po_name, "set_warehouse")
		if warehouse:
			for item in sco.items:
				item.warehouse = warehouse
		else:
			po = finergy.get_doc("Purchase Order", args.po_name)
			warehouses = []
