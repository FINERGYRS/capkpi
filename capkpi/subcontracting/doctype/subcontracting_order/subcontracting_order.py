...
from capkpi.buying.doctype.purchase_order.purchase_order import is_subcontracting_order_created
...
class SubcontractingOrder(SubcontractingController):
	...
	def validate_purchase_order_for_subcontracting(self):
		if self.purchase_order:
			...
			po = finergy.get_doc("Purchase Order", self.purchase_order)

			if not po.is_subcontracted:
				finergy.throw(_("Please select a valid Purchase Order that is configured for Subcontracting."))

			if po.is_old_subcontracting_flow:
				finergy.throw(_("Please select a valid Purchase Order that has Service Items."))
			...
			finergy.throw(_("Please select a Subcontracting Purchase Order."))

	def validate_service_items(self):
		for item in self.service_items:
			if finergy.get_value("Item", item.item_code, "is_stock_item"):
	...
	def update_ordered_qty_for_subcontracting(self, sco_item_rows=None):
		...
		for item in self.get("items"):
			if (
				...
				and finergy.get_cached_value("Item", item.item_code, "is_stock_item")
				and item.warehouse
			):
	...
	def populate_items_table(self):
		...
		for si in self.service_items:
			if si.fg_item:
				item = finergy.get_doc("Item", si.fg_item)
				bom = finergy.db.get_value("BOM", {"item": item.item_code, "is_active": 1, "is_default": 1})
	...
	def update_status(self, status=None, update_modified=False):
		if self.docstatus >= 1 and not status:
			...
			finergy.db.set_value("Subcontracting Order", self.name, "status", status, update_modified)
...
def update_subcontracting_order_status(sco):
	if isinstance(sco, str):
		sco = finergy.get_doc("Subcontracting Order", sco)
