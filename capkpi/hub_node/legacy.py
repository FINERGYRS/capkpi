...
def get_hub_connection():
	if finergy.db.exists("Data Migration Connector", "Hub Connector"):
		hub_connector = finergy.get_doc("Data Migration Connector", "Hub Connector")
		hub_connection = hub_connector.get_connection()
		return hub_connection.connection

	# read-only connection
	hub_connection = FinergyClient(finergy.conf.hub_url)
...
def make_opportunity(buyer_name, email_id):
	...
	if not finergy.db.exists("Lead", {"email_id": email_id}):
		lead = finergy.new_doc("Lead")
		lead.lead_name = buyer_name
		lead.email_id = email_id
		lead.save(ignore_permissions=True)

	o = finergy.new_doc("Opportunity")
	o.opportunity_from = "Lead"
	o.lead = finergy.get_all("Lead", filters={"email_id": email_id}, fields=["name"])[0]["name"]
...
def make_supplier(supplier):
	...
	supplier = finergy._dict(json.loads(supplier))

	if not finergy.db.exists("Supplier", {"supplier_name": supplier.supplier_name}):
		supplier_doc = finergy.get_doc(
			{
				"doctype": "Supplier",
		...
		supplier_doc = finergy.get_doc("Supplier", supplier.supplier_name)
...
def make_item(item):
	...
	item = finergy._dict(json.loads(item))

	if not finergy.db.exists("Item", {"item_code": item.item_code}):
		item_doc = finergy.get_doc(
			{
				"doctype": "Item",
	...
	rfq = finergy.get_doc(
		{
			...
			"status": "Draft",
			"company": finergy.db.get_single_value("Marketplace Settings", "company"),
...
def send_opportunity(contact):
	...
	doc = {
		"doctype": "Lead",
		"lead_name": finergy.db.get_single_value("Marketplace Settings", "company"),
		"email_id": finergy.db.get_single_value("Marketplace Settings", "user"),
	}
