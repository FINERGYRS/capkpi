...
def get_current_student():
	"""Returns current student from finergy.session.user
...
def get_portal_programs():
	...
	"""
	published_programs = finergy.get_all("Program", filters={"is_published": True})
	if not published_programs:
		return None

	program_list = [finergy.get_doc("Program", program) for program in published_programs]
	portal_programs = [
...
def enroll_in_program(program_name, student=None):
	...
	if not student == None:
		student = finergy.get_doc("Student", student)
	else:
		# Check if self enrollment in allowed
		program = finergy.get_doc("Program", program_name)
		if not program.allow_self_enroll:
			return finergy.throw(_("You are not allowed to enroll for this course"))
...
def has_super_access():
	...
	"""
	current_user = finergy.get_doc("User", finergy.session.user)
...
def add_activity(course, content_type, content, program):
	...
	student = get_current_student()
	if not student:
		return finergy.throw(
			_("Student with email {0} does not exist").format(finergy.session.user), finergy.DoesNotExistError
		)
...
def evaluate_quiz(quiz_response, quiz_name, course, program, time_taken):
	...
	quiz = finergy.get_doc("Quiz", quiz_name)
...
def get_quiz(quiz_name, course):
	try:
		...
		finergy.throw(_("Quiz {0} does not exist").format(quiz_name), finergy.DoesNotExistError)
		return None
...
def get_course_progress(course, program):
	...
	for course_topic in course.topics:
		topic = finergy.get_doc("Topic", course_topic.topic)
...
def get_program_progress(program):
	...
	for program_course in program.courses:
		course = finergy.get_doc("Course", program_course.course)
...
def get_program_completion(program):
	...
	progress = []
	for topic in topics:
		topic_doc = finergy.get_doc("Topic", topic[0])
...
def get_or_create_course_enrollment(course, program):
	...
	if not course_enrollment:
		...
		if not program_enrollment:
			finergy.throw(_("You are not enrolled in program {0}").format(program))
			return
		return student.enroll_in_course(
			course_name=course, program_enrollment=get_enrollment("program", program.name, student.name)
		)
	else:
		return finergy.get_doc("Course Enrollment", course_enrollment)
