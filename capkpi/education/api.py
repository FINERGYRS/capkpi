...
from finergy.utils import cstr, flt, getdate
...
def enroll_student(source_name):
	...
	"""
	finergy.publish_realtime("enroll_student_progress", {"progress": [1, 4]}, user=finergy.session.user)
	...
	student_applicant = finergy.db.get_value(
		"Student Applicant", source_name, ["student_category", "program"], as_dict=True
	)
	program_enrollment = finergy.new_doc("Program Enrollment")
	program_enrollment.student = student.name
	program_enrollment.student_category = student_applicant.student_category
	program_enrollment.student_name = student.title
	program_enrollment.program = student_applicant.program
	finergy.publish_realtime("enroll_student_progress", {"progress": [2, 4]}, user=finergy.session.user)
...
def check_attendance_records_exist(course_schedule=None, student_group=None, date=None):
	...
	"""
	if course_schedule:
		return finergy.get_list("Student Attendance", filters={"course_schedule": course_schedule})
	else:
...
def mark_attendance(
	...
	if student_group:
		academic_year = finergy.db.get_value("Student Group", student_group, "academic_year")
		if academic_year:
			year_start_date, year_end_date = finergy.db.get_value(
	...
	finergy.db.commit()
	finergy.msgprint(_("Attendance has been marked successfully."))
...
def make_attendance_records(
	...
	"""
	student_attendance = finergy.get_doc(
		{
	...
	if not student_attendance:
		student_attendance = finergy.new_doc("Student Attendance")
...
def get_student_guardians(student):
	...
	:param student: Student.
	"""
	guardians = finergy.get_all("Student Guardian", fields=["guardian"], filters={"parent": student})
	return guardians
...
def get_fee_structure(program, academic_term=None):
	...
	"""
	fee_structure = finergy.db.get_values(
...
def collect_fees(fees, amt):
	paid_amount = flt(amt) + flt(finergy.db.get_value("Fees", fees, "paid_amount"))
	total_amount = flt(finergy.db.get_value("Fees", fees, "total_amount"))
	finergy.db.set_value("Fees", fees, "paid_amount", paid_amount)
	finergy.db.set_value("Fees", fees, "outstanding_amount", (total_amount - paid_amount))
	return paid_amount
...
def get_result(student, assessment_plan):
	...
	:param Assessment Plan: Assessment Plan
	"""
	results = finergy.get_all(
		"Assessment Result",
		filters={"student": student, "assessment_plan": assessment_plan, "docstatus": ("!=", 2)},
	)
	if results:
		return finergy.get_doc("Assessment Result", results[0])
	else:
		return None
...
def get_grade(grading_scale, percentage):
	...
	if not hasattr(finergy.local, "grading_scale"):
		grading_scale = finergy.get_all(
			"Grading Scale Interval", fields=["grade_code", "threshold"], filters={"parent": grading_scale}
		)
		finergy.local.grading_scale = grading_scale
	for d in finergy.local.grading_scale:
...
def get_assessment_result_doc(student, assessment_plan):
	assessment_result = finergy.get_all(
		"Assessment Result",
		filters={"student": student, "assessment_plan": assessment_plan, "docstatus": ("!=", 2)},
	)
	if assessment_result:
		doc = finergy.get_doc("Assessment Result", assessment_result[0])
		if doc.docstatus == 0:
			return doc
		elif doc.docstatus == 1:
			finergy.msgprint(_("Result already Submitted"))
			return None
	else:
		return finergy.new_doc("Assessment Result")


@finergy.whitelist()
def update_email_group(doctype, name):
	if not finergy.db.exists("Email Group", name):
		email_group = finergy.new_doc("Email Group")
	...
	for stud in students:
		for guard in get_student_guardians(stud.student):
			email = finergy.db.get_value("Guardian", guard.guardian, "email_address")
			if email:
...
def get_current_enrollment(student, academic_year=None):
	current_academic_year = academic_year or finergy.defaults.get_defaults().academic_year
	program_enrollment_list = finergy.db.sql(
		"""
		select
