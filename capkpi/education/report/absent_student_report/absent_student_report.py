...
def execute(filters=None):
	...
	if is_holiday(holiday_list, filters.get("date")):
		msgprint(
			_("No attendance has been marked for {0} as it is a Holiday").format(
				finergy.bold(formatdate(filters.get("date")))
			)
		)
	...
	for student in absent_students:
		if not student.student in leave_applicants:
			...
			stud_details = finergy.db.get_value(
...
def get_absent_students(date):
	absent_students = finergy.db.sql(
		"""
...
def get_transportation_details(date, student_list):
	academic_year = finergy.get_all(
		"Academic Year", filters=[["year_start_date", "<=", date], ["year_end_date", ">=", date]]
	)
	if academic_year:
		academic_year = academic_year[0].name
	elif finergy.defaults.get_defaults().academic_year:
		academic_year = finergy.defaults.get_defaults().academic_year
	else:
		return {}

	transportation_details = finergy.get_all(
		"Program Enrollment",
