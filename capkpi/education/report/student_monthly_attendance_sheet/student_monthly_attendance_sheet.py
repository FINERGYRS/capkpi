...
from finergy.utils import add_days, cstr, date_diff, get_first_day, get_last_day, getdate
...
def execute(filters=None):
	...
	for stud in students:
		...
		student_status = finergy.db.get_value("Student", stud.student, "enabled")
		date = from_date
		total_p = total_a = 0.0
...
def get_attendance_list(from_date, to_date, student_group, students_list):
	attendance_list = finergy.db.sql(
	...
	for d in attendance_list:
		att_map.setdefault(d.student, finergy._dict()).setdefault(d.date, "")
...
def get_students_with_leave_application(from_date, to_date, students_list):
	if not students_list:
		return
	leave_applications = finergy.db.sql(
		"""
	...
	for dt in daterange(getdate(from_date), getdate(to_date)):
		if dt in holidays:
			for student in students_list:
				att_map.setdefault(student, finergy._dict()).setdefault(dt, "Holiday")
