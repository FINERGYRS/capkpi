...
from finergy.utils import add_years, date_diff, getdate, nowdate
...
class StudentApplicant(Document):
	def autoname(self):
		...
		if self.student_admission:
			...
			if self.program:
				...
				finergy.throw(_("Select the program first"))
	...
	def on_update_after_submit(self):
		student = finergy.get_list("Student", filters={"student_applicant": self.name})
		if student:
			finergy.throw(
	...
	def validation_from_student_admission(self):
		...
		if (
			...
			finergy.throw(_("Not eligible for the admission in this program as per Date Of Birth"))
...
def get_student_admission_data(student_admission, program):
	...
	student_admission = finergy.db.sql(
