...
QUnit.test('test student applicant', function(assert){
	...
	finergy.run_serially([
		() => finergy.set_route('List', 'Student Applicant'),
		() => finergy.timeout(0.5),
		...
		() => finergy.timeout(0.5),
		() => finergy.tests.click_button('Submit'),
		() => finergy.tests.click_button('Yes'), // Submitting again after amend
		() => {
			testing_status = $('span.indicator.orange').text();
		...
		() => finergy.tests.click_button('Update'),
		() => {
			testing_status = $('h1.editable-title').text();
		...
		() => finergy.timeout(0.5),
		() => finergy.tests.click_button('Enroll'),
		() => finergy.timeout(0.5),
		() => {
			assert.ok(finergy.get_route()[0] == 'Form'); // Checking if the current page is Program Enrollment page or not
			assert.ok(finergy.get_route()[1] == 'Program Enrollment');
		},

		// Routing to Student List to check if the Applicant's entry has been made or not
		() => finergy.timeout(0.5),
		() => finergy.set_route('List', 'Student'),
		() => finergy.timeout(0.5),
		() => {$(`a:contains("Fname Mname Lname"):visible`)[0].click();},
		() => finergy.timeout(0.5),
