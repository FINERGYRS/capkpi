...
finergy.ui.form.on('Student Attendance Tool', {
	onload: function(frm) {
	...
	refresh: function(frm) {
		if (finergy.route_options) {
			frm.set_value("based_on", finergy.route_options.based_on);
			frm.set_value("student_group", finergy.route_options.student_group);
			frm.set_value("course_schedule", finergy.route_options.course_schedule);
	...
	student_group: function(frm) {
		if ((frm.doc.student_group && frm.doc.date) || frm.doc.course_schedule) {
			var method = "capkpi.education.doctype.student_attendance_tool.student_attendance_tool.get_student_attendance_records";
	...
	date: function(frm) {
		if (frm.doc.date > finergy.datetime.get_today())
			finergy.throw(__("Cannot mark attendance for future dates."));
	...
	make: function(frm, students) {
		...
		student_toolbar.find(".btn-mark-att")
			...
			.on("click", function() {
				...
				finergy.confirm(__("Do you want to update attendance? <br> Present: {0} <br> Absent: {1}",
					[students_present.length, students_absent.length]),
					function() {	//ifyes
						if(!finergy.request.ajax_count) {
							finergy.call({
								method: "capkpi.education.api.mark_attendance",
								freeze: true,
		...
		var htmls = students.map(function(student) {
			return finergy.render_template("student_button", {
