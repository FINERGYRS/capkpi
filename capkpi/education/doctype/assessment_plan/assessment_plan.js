...
finergy.ui.form.on('Assessment Plan', {
	onload: function(frm) {
	...
	refresh: function(frm) {
		if (frm.doc.docstatus == 1) {
			frm.add_custom_button(__('Assessment Result Tool'), function() {
				...
				finergy.set_route('Form', 'Assessment Result Tool');
			}, __('Tools'));
		}
	...
	course: function(frm) {
		if (frm.doc.course && frm.doc.maximum_assessment_score) {
			finergy.call({
				method: 'capkpi.education.api.get_assessment_criteria',
				args: {
				...
				callback: function(r) {
					if (r.message) {
						...
						$.each(r.message, function(i, d) {
							var row = finergy.model.add_child(frm.doc, 'Assessment Plan Criteria', 'assessment_criteria');
