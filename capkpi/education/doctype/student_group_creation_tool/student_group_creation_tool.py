...
from capkpi.education.doctype.student_group.student_group import get_students
...
class StudentGroupCreationTool(Document):
	...
	def get_courses(self):
		...
		batches = finergy.db.sql("""select name as batch from `tabStudent Batch Name`""", as_dict=1)
	...
	def create_student_groups(self):
		if not self.courses:
			finergy.throw(_("""No Student Groups created."""))

		l = len(self.courses)
		for d in self.courses:
			if not d.student_group_name:
				finergy.throw(_("""Student Group Name is mandatory in row {0}""".format(d.idx)))

			if d.group_based_on == "Course" and not d.course:
				finergy.throw(_("""Course is mandatory in row {0}""".format(d.idx)))

			if d.group_based_on == "Batch" and not d.batch:
				finergy.throw(_("""Batch is mandatory in row {0}""".format(d.idx)))

			finergy.publish_realtime(
				"student_group_creation_progress", {"progress": [d.idx, l]}, user=finergy.session.user
			)
		...
		finergy.msgprint(_("{0} Student Groups created.").format(l))
