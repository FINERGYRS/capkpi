...
finergy.ui.form.on("Fees", {
	...
	onload: function(frm) {
		...
		if (!frm.doc.posting_date) {
			frm.doc.posting_date = finergy.datetime.get_today();
		}
	...
	refresh: function(frm) {
		...
		if(frm.doc.docstatus > 0) {
			...
			frm.add_custom_button(__("Payments"), function() {
				finergy.set_route("List", "Payment Entry", {"Payment Entry Reference.reference_name": frm.doc.name});
			}, __("View"));
		}
	...
	student: function(frm) {
		if (frm.doc.student) {
			finergy.call({
				method:"capkpi.education.api.get_current_enrollment",
				args: {
	...
	make_payment_request: function(frm) {
		if (!frm.doc.student_email) {
			finergy.msgprint(__("Please set the Email ID for the Student to send the Payment Request"));
		} else {
			finergy.call({
	...
	academic_term: function() {
		finergy.ui.form.trigger("Fees", "program");
	},
	...
	fee_structure: function(frm) {
		...
		if (frm.doc.fee_structure) {
			finergy.call({
				method: "capkpi.education.api.get_fee_components",
				args: {
				...
				callback: function(r) {
					if (r.message) {
						$.each(r.message, function(i, d) {
							var row = finergy.model.add_child(frm.doc, "Fee Component", "components");
...
finergy.ui.form.on("Fee Component", {
	amount: function(frm) {
		frm.trigger("calculate_total_amount");
	}
});
