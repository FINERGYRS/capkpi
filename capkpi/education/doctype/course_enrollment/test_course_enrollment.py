...
from capkpi.education.doctype.course_activity.test_course_activity import make_course_activity
from capkpi.education.doctype.program.test_program import setup_program
...
class TestCourseEnrollment(unittest.TestCase):
	def setUp(self):
		...
		course_enrollment = finergy.db.get_value(
			"Course Enrollment",
			{
	...
	def test_get_progress(self):
		...
		program_enrollment_name = finergy.get_list(
			"Program Enrollment", filters={"student": student.name, "Program": "_Test Program"}
		)[0].name
		course_enrollment_name = finergy.get_list(
			"Course Enrollment",
			filters={
				"student": student.name,
				"course": "_Test Course 1",
		...
		course_enrollment = finergy.get_doc("Course Enrollment", course_enrollment_name)
