...
finergy.ui.form.on('Assessment Result', {
	refresh: function(frm) {
		...
		frm.set_query('course', function() {
			return {
				query: 'capkpi.education.doctype.program_enrollment.program_enrollment.get_program_courses',
				filters: {
	...
	assessment_plan: function(frm) {
		if (frm.doc.assessment_plan) {
			finergy.call({
				...
				callback: function(r) {
					if (r.message) {
						finergy.model.clear_table(frm.doc, 'details');
...
finergy.ui.form.on('Assessment Result Detail', {
	score: function(frm, cdt, cdn) {
		var d  = locals[cdt][cdn];

		if (!d.maximum_score || !frm.doc.grading_scale) {
			d.score = '';
			finergy.throw(__('Please fill in all the details to generate Assessment Result.'));
		}

		if (d.score > d.maximum_score) {
			finergy.throw(__('Score cannot be greater than Maximum Score'));
		}
		else {
			finergy.call({
				...
				callback: function(r) {
					if (r.message) {
						finergy.model.set_value(cdt, cdn, 'grade', r.message);
					}
				}
			});
		}
