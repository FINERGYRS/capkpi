...
from capkpi.education.api import enroll_student


class ProgramEnrollmentTool(Document):
	def onload(self):
		academic_term_reqd = cint(finergy.db.get_single_value("Education Settings", "academic_term_reqd"))
	...
	def get_students(self):
		...
		if not self.get_students_from:
			finergy.throw(_("Mandatory field - Get Students From"))
		elif not self.program:
			finergy.throw(_("Mandatory field - Program"))
		elif not self.academic_year:
			finergy.throw(_("Mandatory field - Academic Year"))
		else:
			...
			if self.get_students_from == "Student Applicant":
				...
				if student_list:
					inactive_students = finergy.db.sql(
						"""
		...
		if students:
			...
			finergy.throw(_("No students Found"))
	...
	def enroll_students(self):
		...
		for i, stud in enumerate(self.students):
			finergy.publish_realtime(
				"program_enrollment_tool", dict(progress=[i + 1, total]), user=finergy.session.user
			)
			if stud.student:
				prog_enrollment = finergy.new_doc("Program Enrollment")
				...
				prog_enrollment.save()
		finergy.msgprint(_("{0} Students have been enrolled").format(total))
