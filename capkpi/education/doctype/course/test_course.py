...
from capkpi.education.doctype.topic.test_topic import make_topic, make_topic_and_linked_content

# test_records = finergy.get_test_records('Course')
...
class TestCourse(unittest.TestCase):
	...
	def test_get_topics(self):
		course = finergy.get_doc("Course", "_Test Course 1")
...
def make_course(name):
	try:
		course = finergy.get_doc("Course", name)
	except finergy.DoesNotExistError:
		course = finergy.get_doc({"doctype": "Course", "course_name": name, "course_code": name}).insert()
	return course.name
