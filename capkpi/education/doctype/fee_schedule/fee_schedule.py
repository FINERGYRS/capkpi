...
from finergy.utils import cint, cstr, flt, money_in_words
...
class FeeSchedule(Document):
	...
	def calculate_total_and_program(self):
		...
		for d in self.student_groups:
			...
			student_group_program = finergy.db.get_value("Student Group", d.student_group, "program")
	...
	def create_fees(self):
		...
		finergy.publish_realtime(
			"fee_schedule_progress", {"progress": "0", "reload": 1}, user=finergy.session.user
		)
...
def generate_fee(fee_schedule):
	doc = finergy.get_doc("Fee Schedule", fee_schedule)
	error = False
	total_records = sum([int(d.total_students) for d in doc.student_groups])
	created_records = 0

	if not total_records:
		finergy.throw(_("Please setup Students under Student Groups"))
	...
	for d in doc.student_groups:
		...
		for student in students:
			try:
				...
				error = True
				err_msg = finergy.local.message_log and "\n\n".join(finergy.local.message_log) or cstr(e)

	if error:
		finergy.db.rollback()
		finergy.db.set_value("Fee Schedule", fee_schedule, "fee_creation_status", "Failed")
		finergy.db.set_value("Fee Schedule", fee_schedule, "error_log", err_msg)

	else:
		finergy.db.set_value("Fee Schedule", fee_schedule, "fee_creation_status", "Successful")
		finergy.db.set_value("Fee Schedule", fee_schedule, "error_log", None)

	finergy.publish_realtime(
		"fee_schedule_progress", {"progress": "100", "reload": 1}, user=finergy.session.user
	)
...
def get_students(student_group, academic_year, academic_term=None, student_category=None):
	conditions = ""
	if student_category:
		conditions = " and pe.student_category={}".format(finergy.db.escape(student_category))
	if academic_term:
		conditions = " and pe.academic_term={}".format(finergy.db.escape(academic_term))

	students = finergy.db.sql(
		"""
