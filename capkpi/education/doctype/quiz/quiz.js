...
finergy.ui.form.on('Quiz', {
	...
	check_duplicate_question: function(questions_data){
		...
		if (questions.length != questions_set.size) {
			finergy.throw(__('The question cannot be duplicate'));
		}
	},
	...
	add_quiz_to_topics: function(frm) {
		get_topics_without_quiz(frm.doc.name).then(r => {
			if (r.message.length) {
				...
				function(data) {
					finergy.call({
						method: 'capkpi.education.doctype.topic.topic.add_content_to_topics',
						args: {
							'content_type': 'Quiz',
				...
				finergy.msgprint(__('This quiz is already added to the existing topics'));
			}
		});
	}
});
...
let get_topics_without_quiz = function(quiz) {
	return finergy.call({
		type: 'GET',
		method: 'capkpi.education.doctype.quiz.quiz.get_topics_without_quiz',
		args: {'quiz': quiz}
	});
};
