...
import finergy
from finergy.utils import to_timedelta, today
from finergy.utils.data import add_to_date

from capkpi.education.utils import OverlapError

# test_records = finergy.get_test_records('Course Schedule')
...
class TestCourseSchedule(unittest.TestCase):
	def test_student_group_conflict(self):
		...
		cs2 = make_course_schedule_test_record(
			...
			room=finergy.get_all("Room")[1].name,
			do_not_save=1,
		)
...
def make_course_schedule_test_record(**args):
	...
	course_schedule.room = args.room or finergy.get_all("Room")[0].name
