...
from capkpi.education.utils import OverlapError
...
class CourseSchedulingTool(Document):
	...
	def schedule_course(self):
		...
		self.validate_date()
		self.instructor_name = finergy.db.get_value("Instructor", self.instructor, "instructor_name")

		group_based_on, course = finergy.db.get_value(
	...
	def validate_mandatory(self):
		...
		for d in fields:
			if not self.get(d):
				finergy.throw(_("{0} is mandatory").format(self.meta.get_label(d)))

	def validate_date(self):
		"""Validates if Course Start Date is greater than Course End Date"""
		if self.course_start_date > self.course_end_date:
			finergy.throw(_("Course Start Date cannot be greater than Course End Date."))

	def delete_course_schedule(self, rescheduled, reschedule_errors):
		"""Delete all course schedule within the Date range and specified filters"""

		schedules = finergy.get_list(
			"Course Schedule",
		...
		for d in schedules:
			try:
				if self.day == calendar.day_name[getdate(d.schedule_date).weekday()]:
					finergy.delete_doc("Course Schedule", d.name)
	...
	def make_course_schedule(self, date):
		...
		course_schedule = finergy.new_doc("Course Schedule")
