...
from finergy.utils import formatdate, get_link_to_form, getdate

from capkpi import get_default_company
from capkpi.education.api import get_student_group_students
...
class StudentAttendance(Document):
	...
	def set_date(self):
		if self.course_schedule:
			self.date = finergy.db.get_value("Course Schedule", self.course_schedule, "schedule_date")

	def validate_mandatory(self):
		if not (self.student_group or self.course_schedule):
			finergy.throw(
				_("{0} or {1} is mandatory").format(
					finergy.bold("Student Group"), finergy.bold("Course Schedule")
				),
	...
	def validate_date(self):
		if not self.leave_application and getdate(self.date) > getdate():
			finergy.throw(_("Attendance cannot be marked for future dates."))

		if self.student_group:
			academic_year = finergy.db.get_value("Student Group", self.student_group, "academic_year")
			if academic_year:
	...
	def set_student_group(self):
		if self.course_schedule:
			self.student_group = finergy.db.get_value(
				"Course Schedule", self.course_schedule, "student_group"
			)

	def validate_student(self):
		if self.course_schedule:
			student_group = finergy.db.get_value("Course Schedule", self.course_schedule, "student_group")
		else:
		...
		if student_group and self.student not in student_group_students:
			...
			finergy.throw(
				_("Student {0}: {1} does not belong to Student Group {2}").format(
					finergy.bold(self.student), self.student_name, finergy.bold(student_group_doc)
				)
			)
		...
		if attendance_record:
			...
			finergy.throw(
				_("Student Attendance record {0} already exists against the Student {1}").format(
					record, finergy.bold(self.student)
				),
		...
		if is_holiday(holiday_list, self.date):
			finergy.throw(
				_("Attendance cannot be marked for {0} as it is a holiday.").format(
					finergy.bold(formatdate(self.date))
				)
			)
