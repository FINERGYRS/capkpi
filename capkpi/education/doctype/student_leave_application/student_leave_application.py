...
from finergy.utils import date_diff, flt, get_link_to_form, getdate
		...
		if data:
			...
			finergy.throw(
				_("Leave application {0} already exists against the student {1}").format(
					link, finergy.bold(self.student)
				),
	...
	def update_attendance(self):
		...
		for dt in daterange(getdate(self.from_date), getdate(self.to_date)):
			...
			if attendance:
				...
				finergy.db.set_value("Student Attendance", attendance, values)
			else:
				# make a new attendance record
				doc = finergy.new_doc("Student Attendance")
	...
	def cancel_attendance(self):
		if self.docstatus == 2:
			...
			for name in attendance:
				finergy.db.set_value("Student Attendance", name, "docstatus", 2)
