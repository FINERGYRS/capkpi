	...
	add_article_to_topics: function(frm) {
		get_topics_without_article(frm.doc.name).then(r => {
			if (r.message.length) {
				...
				finergy.msgprint(__('This article is already added to the existing topics'));
			}
		});
	}
});
...
let get_topics_without_article = function(article) {
	return finergy.call({
		type: 'GET',
		method: 'capkpi.education.doctype.article.article.get_topics_without_article',
