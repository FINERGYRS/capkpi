...
from finergy.utils import cstr, flt

from capkpi.controllers.tests.test_subcontracting_controller import set_backflush_based_on
from capkpi.manufacturing.doctype.bom.bom import BOMRecursionError, item_query, make_variant_bom
...
class TestBOM(FinergyTestCase):
	...
	def test_get_items_list(self):
		from capkpi.manufacturing.doctype.bom.bom import get_bom_items

		self.assertEqual(len(get_bom_items(bom=get_default_bom(), company="_Test Company")), 3)

	def test_default_bom(self):
		def _get_default_bom_in_item():
			return cstr(finergy.db.get_value("Item", "_Test FG Item 2", "default_bom"))

		bom = finergy.get_doc("BOM", {"item": "_Test FG Item 2", "is_default": 1})
	...
	def test_update_bom_cost_in_all_boms(self):
		...
		bom_rates = finergy.db.get_values(
			"BOM Item",
			{
	...
	def test_bom_cost(self):
		bom = finergy.copy_doc(test_records[2])
		bom.insert()
	...
	def test_bom_cost_multi_uom_multi_currency_based_on_price_list(self):
		finergy.db.set_value("Price List", "_Test Price List", "price_not_uom_dependent", 1)
	...
	def test_bom_cost_multi_uom_based_on_valuation_rate(self):
		...
		reset_item_valuation_rate(
			...
			warehouse_list=finergy.get_all(
	...
	def test_subcontractor_sourced_item(self):
		...
		if not finergy.db.exists("Item", "Test Extra Item 3"):
		...
		from capkpi.controllers.tests.test_subcontracting_controller import (
	...
	def test_bom_recursion_1st_level(self):
		...
		bom.item = item_code
		bom.append("items", finergy._dict(item_code=item_code))
		bom.save()
	...
	def test_bom_recursion_transitive(self):
		...
		bom1 = finergy.new_doc("BOM")
		bom1.item = item1
		bom1.append("items", finergy._dict(item_code=item2))
		bom1.save()

		bom2 = finergy.new_doc("BOM")
		bom2.item = item2
		bom2.append("items", finergy._dict(item_code=item1))
		bom2.save()
	...
	def test_bom_with_process_loss_item(self):
		...
		if not finergy.db.exists("BOM", f"BOM-{fg_item_non_whole.item_code}-001"):
		...
		self.assertRaises(finergy.ValidationError, bom_doc.submit)
	...
	def test_exclude_exploded_items_from_bom(self):
		...
		new_bom = finergy.copy_doc(finergy.get_doc("BOM", bom_no))
	...
	def test_valid_transfer_defaults(self):
		bom_with_op = finergy.db.get_value(
			"BOM", {"item": "_Test FG Item 2", "with_operations": 1, "is_active": 1}
		)
		bom = finergy.copy_doc(finergy.get_doc("BOM", bom_with_op), ignore_no_copy=False)
	...
	def test_bom_versioning(self):
		bom_tree = {finergy.generate_hash(length=10): {finergy.generate_hash(length=10): {}}}
		...
		amendment = finergy.copy_doc(bom)
		...
		version = finergy.copy_doc(amendment)
		version.docstatus = 0
	...
	def test_clear_inpection_quality(self):
		...
		bom = finergy.copy_doc(test_records[2], ignore_no_copy=True)
		bom.docstatus = 0
		bom.is_default = 0
	...
	def test_bom_pricing_based_on_lpp(self):
		...
		parent = finergy.generate_hash(length=10)
		child = finergy.generate_hash(length=10)
	...
	def test_set_default_bom_for_item_having_single_bom(self):
		...
		bom.save()
		bom.submit()
		self.assertEqual(finergy.get_value("Item", fg_item.item_code, "default_bom"), bom.name)

		# Step 2: Uncheck is_active field
		bom.is_active = 0
		bom.save()
		bom.reload()
		self.assertIsNone(finergy.get_value("Item", fg_item.item_code, "default_bom"))
...
def get_default_bom(item_code="_Test FG Item 2"):
	return finergy.db.get_value("BOM", {"item": item_code, "is_active": 1, "is_default": 1})
...
def create_nested_bom(tree, prefix="_Test bom "):
	...
	def create_items(bom_tree):
		for item_code, subtree in bom_tree.items():
			...
			if not finergy.db.exists("Item", bom_item_code):
				finergy.get_doc(doctype="Item", item_code=bom_item_code, item_group="_Test Item Group").insert()
	...
	for item in order_of_creating_bom:
		...
		if child_items:
			...
			bom = finergy.get_doc(doctype="BOM", item=bom_item_code)
...
def reset_item_valuation_rate(item_code, warehouse_list=None, qty=None, rate=None):
	...
	if not warehouse_list:
		warehouse_list = finergy.db.sql_list(
			"""
...
def create_process_loss_bom_item(item_tuple):
	...
	if finergy.db.exists("Item", item_code) is None:
