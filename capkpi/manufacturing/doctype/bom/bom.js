...
finergy.provide("capkpi.bom");

finergy.ui.form.on("BOM", {
	setup: function(frm) {
		...
		frm.set_query("item", function() {
			return {
				query: "capkpi.manufacturing.doctype.bom.bom.item_query",
				filters: {
					"is_stock_item": 1
				}
			};
	...
	refresh: function(frm) {
		...
		if (!frm.is_new() && frm.doc.docstatus<2) {
			...
			frm.add_custom_button(__("Browse BOM"), function() {
				...
				finergy.set_route("Tree", "BOM");
			});
		}

		if (!frm.is_new() && !frm.doc.docstatus == 0) {
			frm.add_custom_button(__("New Version"), function() {
				let new_bom = finergy.model.copy_doc(frm.doc);
				finergy.set_route("Form", "BOM", new_bom.name);
			});
		}
		...
		if (frm.doc.has_variants) {
			...
			frm.$wrapper.find(".variants-intro").on("click", () => {
				finergy.set_route("List", "Item", {"variant_of": frm.doc.item});
			});
		}
	},
	...
	make_work_order: function(frm) {
		frm.events.setup_variant_prompt(frm, "Work Order", (frm, item, data, variant_items) => {
			finergy.call({
				method: "capkpi.manufacturing.doctype.work_order.work_order.make_work_order",
				args: {
					bom_no: frm.doc.name,
					item: item,
					qty: data.qty || 0.0,
	...
	make_variant_bom: function(frm) {
		frm.events.setup_variant_prompt(frm, "Variant BOM", (frm, item, data, variant_items) => {
			finergy.call({
				method: "capkpi.manufacturing.doctype.bom.bom.make_variant_bom",
				args: {
	...
	setup_variant_prompt: function(frm, title, callback, skip_qty_field) {
				...
				fields: [
					...
					},
					{
						...
						get_query: function(data) {
							if (!data.item_code) {
								finergy.throw(__("Select template item"));
							}
		...
		let dialog = finergy.prompt(fields, data => {
			let item = data.item || frm.doc.item;
			let variant_items = data.items || [];

			variant_items.forEach(d => {
				if (!d.variant_item_code) {
					finergy.throw(__("Select variant item code for the template item {0}", [d.item_code]));
				}
			})
	...
	make_quality_inspection: function(frm) {
		finergy.model.open_mapped_doc({
			method: "capkpi.stock.doctype.quality_inspection.quality_inspection.make_quality_inspection",
			frm: frm
		})
	},
		...
		if (frm.doc.routing) {
			finergy.call({
				...
				callback: function(r) {
					if (!r.exc) {
						frm.refresh_fields();
						capkpi.bom.calculate_op_cost(frm.doc);
						capkpi.bom.calculate_total(frm.doc);
					}
				}
			});
		}
...
capkpi.bom.BomController = class BomController extends capkpi.TransactionController {
	conversion_rate(doc) {
		if(this.frm.doc.currency === this.get_company_currency()) {
			this.frm.set_value("conversion_rate", 1.0);
		} else {
			capkpi.bom.update_cost(doc);
		}
	}
...
extend_cscript(cur_frm.cscript, new capkpi.bom.BomController({frm: cur_frm}));

cur_frm.cscript.hour_rate = function(doc) {
	capkpi.bom.calculate_op_cost(doc);
	capkpi.bom.calculate_total(doc);
};
	...
	if (d.item_code) {
		return finergy.call({
			...
			callback: function(r) {
				...
				capkpi.bom.calculate_rm_cost(doc);
				capkpi.bom.calculate_scrap_materials_cost(doc);
				capkpi.bom.calculate_total(doc);
			},
			freeze: true
		});
...
cur_frm.cscript.rate = function(doc, cdt, cdn) {
	...
	if (d.bom_no) {
		finergy.msgprint(__("You cannot change the rate if BOM is mentioned against any Item."));
...
capkpi.bom.update_cost = function(doc) {
	capkpi.bom.calculate_op_cost(doc);
	capkpi.bom.calculate_rm_cost(doc);
	capkpi.bom.calculate_scrap_materials_cost(doc);
	capkpi.bom.calculate_total(doc);
...
capkpi.bom.calculate_op_cost = function(doc) {
	...
	for(var i=0;i<op.length;i++) {
		...
		finergy.model.set_value('BOM Operation',op[i].name, "operating_cost", operating_cost);
		finergy.model.set_value('BOM Operation',op[i].name, "base_operating_cost", base_operating_cost);
...
// rm : raw material
capkpi.bom.calculate_rm_cost = function(doc) {
	var rm = doc.items || [];
	var total_rm_cost = 0;
	...
	for(var i=0;i<rm.length;i++) {
		...
		finergy.model.set_value('BOM Item', rm[i].name, 'base_rate',
			flt(rm[i].rate) * flt(doc.conversion_rate));
		finergy.model.set_value('BOM Item', rm[i].name, 'amount', amount);
		finergy.model.set_value('BOM Item', rm[i].name, 'base_amount', base_amount);
		finergy.model.set_value('BOM Item', rm[i].name,
...
// sm : scrap material
capkpi.bom.calculate_scrap_materials_cost = function(doc) {
	...
	for(var i=0;i<sm.length;i++) {
		...
		finergy.model.set_value('BOM Scrap Item',sm[i].name, 'base_rate', base_rate);
		finergy.model.set_value('BOM Scrap Item',sm[i].name, 'amount', amount);
		finergy.model.set_value('BOM Scrap Item',sm[i].name, 'base_amount', base_amount);
...
capkpi.bom.calculate_total = function(doc) {
...
finergy.ui.form.on("BOM Operation", "operation", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	...
	finergy.call({
		...
		callback: function (data) {
			if(data.message.description) {
				finergy.model.set_value(d.doctype, d.name, "description", data.message.description);
			}
			if(data.message.workstation) {
				finergy.model.set_value(d.doctype, d.name, "workstation", data.message.workstation);
			}
		}
	});
});

finergy.ui.form.on("BOM Operation", "workstation", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	...
	finergy.call({
		...
		callback: function (data) {
			finergy.model.set_value(d.doctype, d.name, "base_hour_rate", data.message.hour_rate);
			finergy.model.set_value(d.doctype, d.name, "hour_rate",
...
finergy.ui.form.on("BOM Item", "qty", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	d.stock_qty = d.qty * d.conversion_factor;
	refresh_field("stock_qty", d.name, d.parentfield);
});

finergy.ui.form.on("BOM Item", "item_code", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	finergy.db.get_value('Item', {name: d.item_code}, 'allow_alternative_item', (r) => {
		d.allow_alternative_item = r.allow_alternative_item
	})
	refresh_field("allow_alternative_item", d.name, d.parentfield);
});

finergy.ui.form.on("BOM Item", "sourced_by_supplier", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
...
finergy.ui.form.on("BOM Item", "rate", function(frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.sourced_by_supplier) {
		d.rate = 0;
		refresh_field("rate", d.name, d.parentfield);
...
finergy.ui.form.on("BOM Operation", "operations_remove", function(frm) {
	capkpi.bom.calculate_op_cost(frm.doc);
	capkpi.bom.calculate_total(frm.doc);
});

finergy.ui.form.on("BOM Item", "items_remove", function(frm) {
	capkpi.bom.calculate_rm_cost(frm.doc);
	capkpi.bom.calculate_total(frm.doc);
});
...
finergy.ui.form.on("BOM Scrap Item", {
