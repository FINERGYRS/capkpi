...
from finergy.utils import cint, cstr, flt, today
from finergy.website.website_generator import WebsiteGenerator

import capkpi
from capkpi.setup.utils import get_exchange_rate
from capkpi.stock.doctype.item.item import get_item_details
from capkpi.stock.get_item_details import get_conversion_factor, get_price_list_rate

form_grid_templates = {"items": "templates/form_grid/item_grid.html"}


class BOMRecursionError(finergy.ValidationError):
	pass
...
class BOMTree:
	...
	def __create_tree(self):
		bom = finergy.get_cached_doc("BOM", self.name)
...
class BOM(WebsiteGenerator):
	...
	def autoname(self):
		...
		if finergy.db.exists("BOM", name):
			conflicting_bom = finergy.get_doc("BOM", name)

			if conflicting_bom.item != self.item:
				msg = _("A BOM with name {0} already exists for item {1}.").format(
					finergy.bold(name), finergy.bold(conflicting_bom.item)
				)
	...
	def validate(self):
		self.route = finergy.scrub(self.name).replace("_", "-")

		if not self.company:
			finergy.throw(_("Please select a Company first."), title=_("Mandatory"))
	...
	def on_update(self):
		finergy.cache().hdel("bom_children", self.name)
	...
	def on_cancel(self):
		finergy.db.set(self, "is_active", 0)
		finergy.db.set(self, "is_default", 0)
	...
	def get_item_det(self, item_code):
		...
		if not item:
			finergy.throw(_("Item: {0} does not exist in the system").format(item_code))
	...
	def get_bom_material_detail(self, args=None):
		...
		if not args:
			args = finergy.form_dict.get("args")
	...
	def validate_bom_currency(self, item):
		if (
			item.get("bom_no")
			and finergy.db.get_value("BOM", item.get("bom_no"), "currency") != self.currency
		):
			finergy.throw(
	...
	def update_cost(self, update_parent=True, from_child_bom=False, update_hour_rate=True, save=True):
		...
		if self.total_cost != existing_bom_cost and update_parent:
			parent_boms = finergy.db.sql_list(
			...
			for bom in parent_boms:
				finergy.get_doc("BOM", bom).update_cost(from_child_bom=True)

		if not from_child_bom:
			finergy.msgprint(_("Cost Updated"), alert=True)
	...
	def manage_default_bom(self):
		...
		if self.is_default and self.is_active:
			from finergy.model.utils import set_default

			set_default(self, "item")
			item = finergy.get_doc("Item", self.item)
			if item.default_bom != self.name:
				finergy.db.set_value("Item", self.item, "default_bom", self.name)
		elif (
			not finergy.db.exists(dict(doctype="BOM", docstatus=1, item=self.item, is_default=1))
			and self.is_active
		):
			finergy.db.set(self, "is_default", 1)
			finergy.db.set_value("Item", self.item, "default_bom", self.name)
		else:
			finergy.db.set(self, "is_default", 0)
			item = finergy.get_doc("Item", self.item)
			if item.default_bom == self.name:
				finergy.db.set_value("Item", self.item, "default_bom", None)
	...
	def validate_main_item(self):
		...
		if not item:
			finergy.throw(_("Item {0} does not exist in the system or has expired").format(self.item))
		else:
			ret = finergy.db.get_value("Item", self.item, ["description", "stock_uom", "item_name"])
			self.description = ret[0]
			self.uom = ret[1]
			self.item_name = ret[2]

		if not self.quantity:
			finergy.throw(_("Quantity should be greater than 0"))

	def validate_currency(self):
		if self.rm_cost_as_per == "Price List":
			price_list_currency = finergy.db.get_value("Price List", self.buying_price_list, "currency")
	...
	def validate_materials(self):
		...
		if not self.get("items"):
			finergy.throw(_("Raw Materials cannot be blank."))
		...
		for m in self.get("items"):
			...
			if flt(m.qty) <= 0:
				finergy.throw(_("Quantity required for Item {0} in row {1}").format(m.item_code, m.idx))
			check_list.append(m)
	...
	def traverse_tree(self, bom_list=None):
		def _get_children(bom_no):
			children = finergy.cache().hget("bom_children", bom_no)
			if children is None:
				children = finergy.db.sql_list(
					"""SELECT `bom_no` FROM `tabBOM Item`
					WHERE `parent`=%s AND `bom_no`!='' AND `parenttype`='BOM'""",
					bom_no,
				)
				finergy.cache().hset("bom_children", bom_no, children)
			return children
	...
	def update_rate_and_time(self, row, update_hour_rate=False):
		if not row.hour_rate or update_hour_rate:
			hour_rate = flt(finergy.get_cached_value("Workstation", row.workstation, "hour_rate"))
	...
	def get_rm_rate_map(self) -> Dict[str, float]:
		...
		for item in self.get("items"):
			if item.bom_no:
				...
				explosion_items = finergy.get_all(
					"BOM Explosion Item",
	...
	def company_currency(self):
		return capkpi.get_company_currency(self.company)
	...
	def add_exploded_items(self, save=True):
		...
		if save:
			finergy.db.sql("""delete from `tabBOM Explosion Item` where parent=%s""", self.name)
	...
	def validate_bom_links(self):
		if not self.is_active:
			...
			if act_pbom and act_pbom[0][0]:
				finergy.throw(_("Cannot deactivate or cancel BOM as it is linked with other BOMs"))
	...
	def validate_operations(self):
		if self.with_operations and not self.get("operations") and self.docstatus == 1:
			finergy.throw(_("Operations cannot be left blank"))

		if self.with_operations:
			for d in self.operations:
				if not d.description:
					d.description = finergy.db.get_value("Operation", d.operation, "description")
	...
	def validate_scrap_items(self):
		for item in self.scrap_items:
			...
			if item.item_code == self.item and not item.is_process_loss:
				...
				).format(finergy.bold(item.item_code))
			...
			must_be_whole_number = finergy.get_value("UOM", item.stock_uom, "must_be_whole_number")
			if item.is_process_loss and must_be_whole_number:
				msg = _(
					"Item: {0} with Stock UOM: {1} cannot be a Scrap/Loss Item as {1} is a whole UOM."
				).format(finergy.bold(item.item_code), finergy.bold(item.stock_uom))
			...
			if msg:
				finergy.throw(msg, title=_("Note"))
...
def get_bom_item_rate(args, bom_doc):
	if bom_doc.rm_cost_as_per == "Valuation Rate":
		...
		rate = (
			...
			or flt(finergy.db.get_value("Item", args["item_code"], "last_purchase_rate"))
		) * (args.get("conversion_factor") or 1)
	elif bom_doc.rm_cost_as_per == "Price List":
		if not bom_doc.buying_price_list:
			finergy.throw(_("Please select Price List"))
		bom_args = finergy._dict(
			{
				"doctype": "BOM",
		...
		item_doc = finergy.get_cached_doc("Item", args.get("item_code"))
...
def get_valuation_rate(data):
	...
	bin_table = finergy.qb.DocType("Bin")
	wh_table = finergy.qb.DocType("Warehouse")
	item_valuation = (
		finergy.qb.from_(bin_table)
		.join(wh_table)
	...
	if (valuation_rate is not None) and valuation_rate <= 0:
		...
		last_val_rate = (
			...
			.orderby(sle.posting_date, order=finergy.qb.desc)
			.orderby(sle.posting_time, order=finergy.qb.desc)
			.orderby(sle.creation, order=finergy.qb.desc)
			.limit(1)
		).run(as_dict=True)
	...
	if not valuation_rate:
		valuation_rate = finergy.db.get_value("Item", item_code, "valuation_rate")
...
def get_bom_items_as_dict(
	...
	if cint(fetch_exploded):
		...
		items = finergy.db.sql(query, {"qty": qty, "bom": bom, "company": company}, as_dict=True)
	else:
		query = query.format(
			table="BOM Item",
			where_conditions="",
	...
	for item, item_details in item_dict.items():
		for d in [
			...
			company_in_record = finergy.db.get_value(d[0], item_details.get(d[1]), "company")
			if not item_details.get(d[1]) or (company_in_record and company != company_in_record):
				item_dict[item][d[1]] = finergy.get_cached_value("Company", company, d[2]) if d[2] else None
...
def validate_bom_no(item, bom_no):
	...
	if not bom.is_active:
		finergy.throw(_("BOM {0} must be active").format(bom_no))
	if bom.docstatus != 1:
		if not getattr(finergy.flags, "in_test", False):
			finergy.throw(_("BOM {0} must be submitted").format(bom_no))
	if item:
		...
		if (
			...
			or bom.item.lower() == cstr(finergy.db.get_value("Item", item, "variant_of")).lower()
		):
			rm_item_exists = True
		if not rm_item_exists:
			finergy.throw(_("BOM {0} does not belong to Item {1}").format(bom_no, item))


@finergy.whitelist()
def get_children(doctype, parent=None, is_root=False, **filters):
	if not parent or parent == "BOM":
		finergy.msgprint(_("Please select a BOM"))
		return

	if parent:
		finergy.form_dict.parent = parent

	if finergy.form_dict.parent:
		bom_doc = finergy.get_cached_doc("BOM", finergy.form_dict.parent)
		finergy.has_permission("BOM", doc=bom_doc, throw=True)

		bom_items = finergy.get_all(
			"BOM Item",
			fields=["item_code", "bom_no as value", "stock_qty"],
			filters=[["parent", "=", finergy.form_dict.parent]],
			order_by="idx",
		)
		...
		for bom_item in bom_items:
			...
			bom_item.image = finergy.db.escape(bom_item.image)
...
def add_additional_cost(stock_entry, work_order):
	...
	expenses_included_in_valuation = finergy.get_cached_value(
...
def add_non_stock_items_cost(stock_entry, work_order, expense_account):
	bom = finergy.get_doc("BOM", work_order.bom_no)
	...
	non_stock_items = finergy.get_all(
		"Item",
		fields="name",
...
def add_operations_cost(stock_entry, work_order=None, expense_account=None):
	from capkpi.stock.doctype.stock_entry.stock_entry import get_operating_cost_per_unit
...
def get_bom_diff(bom1, bom2):
	from finergy.model import table_fields

	if bom1 == bom2:
		finergy.throw(
			_("BOM 1 {0} and BOM 2 {1} should not be same").format(finergy.bold(bom1), finergy.bold(bom2))
		)

	doc1 = finergy.get_doc("BOM", bom1)
	doc2 = finergy.get_doc("BOM", bom2)
...
def item_query(doctype, txt, searchfield, start, page_len, filters):
	meta = finergy.get_meta("Item", cached=True)
	...
	if filters and filters.get("item_code"):
		has_variants = finergy.get_cached_value("Item", filters.get("item_code"), "has_variants")
		if not has_variants:
...
def make_variant_bom(source_name, bom_no, item, variant_items, target_doc=None):
	from capkpi.manufacturing.doctype.work_order.work_order import add_variant_item
