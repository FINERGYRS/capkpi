...
from finergy.utils import add_to_date, flt, now_datetime, nowdate
...
from capkpi.manufacturing.doctype.work_order.work_order import make_stock_entry as make_se_from_wo
...
class TestProductionPlan(FinergyTestCase):
	def setUp(self):
		for item in [
			...
			sr = finergy.db.get_value(
				"Stock Reconciliation Item", {"item_code": item, "docstatus": 1}, "parent"
			)
			if sr:
				sr_doc = finergy.get_doc("Stock Reconciliation", sr)
				sr_doc.cancel()
	...
	def test_production_plan_mr_creation(self):
		...
		material_requests = finergy.get_all(
		...
		for name in material_requests:
			mr = finergy.get_doc("Material Request", name[0])
			if mr.docstatus != 0:
				mr.cancel()

		for name in work_orders:
			mr = finergy.delete_doc("Work Order", name[0])

		pln = finergy.get_doc("Production Plan", pln.name)
		pln.cancel()
	...
	def test_production_plan_start_date(self):
		...
		for wo in work_orders:
			finergy.delete_doc("Work Order", wo.name)
	...
	def test_production_plan_sales_orders(self):
		...
		pln = finergy.new_doc("Production Plan")
		...
		work_order = finergy.db.get_value(
			"Work Order",
			{"sales_order": sales_order, "production_plan": pln.name, "sales_order_item": sales_order_item},
			"name",
		)

		wo_doc = finergy.get_doc("Work Order", work_order)
		wo_doc.update(
			{"wip_warehouse": "Work In Progress - _TC", "fg_warehouse": "Finished Goods - _TC"}
		)
		wo_doc.submit()

		so_wo_qty = finergy.db.get_value("Sales Order Item", sales_order_item, "work_order_qty")
	...
	def test_production_plan_combine_items(self):
		...
		for plan_reference in pln.prod_plan_references:
			...
			so_wo_qty = finergy.db.get_value(
		...
		for so_item in so_items:
			so_wo_qty = finergy.db.get_value("Sales Order Item", so_item, "work_order_qty")
	...
	def test_production_plan_combine_subassembly(self):
		...
		finergy.db.set_value("BOM Item", parent_bom_2.items[0].name, "bom_no", subassembly_bom)
		...
		plan.append(
			...
			{  # Add Green-Car to Prod Plan
				...
				"item_code": "Green-Car",
				"bom_no": finergy.db.get_value("Item", "Green-Car", "default_bom"),
				"planned_qty": 1,
	...
	def test_pp_to_mr_customer_provided(self):
		...
		material_request = finergy.db.get_value(
			"Material Request Item",
			{"production_plan": production_plan.name, "item_code": "CUST-0987"},
			"parent",
		)
		mr = finergy.get_doc("Material Request", material_request)
	...
	def test_production_plan_with_multi_level_bom(self):
		...
		if not finergy.db.get_value("BOM", {"item": "Test BOM 3"}):
			make_bom(item="Test BOM 3", raw_materials=["Test RM BOM 1"], rm_qty=3)

		if not finergy.db.get_value("BOM", {"item": "Test BOM 2"}):
			make_bom(item="Test BOM 2", raw_materials=["Test BOM 3"], rm_qty=3)

		if not finergy.db.get_value("BOM", {"item": "Test BOM 1"}):
		...
		pln.append(
			...
			{
				"item_code": item_code,
				"bom_no": finergy.db.get_value("BOM", {"item": "Test BOM 1"}),
				"planned_qty": 3,
			},
		)
	...
	def test_get_sales_order_with_variant(self):
		...
		if not finergy.db.exists("Item", {"item_code": "PIV"}):
		...
		if not finergy.db.exists("BOM", {"item": "PIV"}):
			parent_bom = make_bom(item="PIV", raw_materials=[rm_item.item_code])
		else:
			parent_bom = finergy.get_doc("BOM", {"item": "PIV"})

		if not finergy.db.exists("Item", {"item_code": "PIV-RED"}):
			variant = create_variant("PIV", {"Colour": "Red"})
			variant.save()
			variant_bom = make_bom(item=variant.item_code, raw_materials=[rm_item.item_code])
		else:
			variant = finergy.get_doc("Item", "PIV-RED")
		if not finergy.db.exists("BOM", {"item": "PIV-RED"}):
			variant_bom = make_bom(item=variant.item_code, raw_materials=[rm_item.item_code])
		...
		variant_bom.cancel()
		finergy.delete_doc("BOM", variant_bom.name)
	...
	def test_subassmebly_sorting(self):
		...
		plan.append(
			"po_items",
			{
				"use_multi_level_bom": 1,
				"item_code": items[1],
				"bom_no": finergy.db.get_value("Item", items[1], "default_bom"),
				"planned_qty": 1,
				"planned_start_date": now_datetime(),
			},
		)
	...
	def test_multiple_work_order_for_production_plan_item(self):
		...
		def create_work_order(item, pln, qty):
			...
			for key, item in items_data.items():
				...
				wo_doc = finergy.get_doc("Work Order", wo_name)
				wo_doc.update(
					{"wip_warehouse": "Work In Progress - _TC", "fg_warehouse": "Finished Goods - _TC"}
				)
				wo_doc.submit()
		...
		wo1 = finergy.get_doc("Work Order", wo_list[0])
		wo1.cancel()
		pln.reload()
		self.assertEqual(pln.po_items[0].ordered_qty, 2)

		# Cancel 2nd Work Order
		wo2 = finergy.get_doc("Work Order", wo_list[1])
		wo2.cancel()
		pln.reload()
	...
	def test_production_plan_pending_qty_with_sales_order(self):
		...
		se = finergy.get_doc(make_se_from_wo(wo.name, "Manufacture", 1))
		se.submit()
	...
	def test_qty_based_status(self):
		pp = finergy.new_doc("Production Plan")
		pp.po_items = [finergy._dict(planned_qty=5, produce_qty=4)]
		self.assertFalse(pp.all_items_completed())

		pp.po_items = [
			finergy._dict(planned_qty=5, produce_qty=10),
			finergy._dict(planned_qty=5, produce_qty=4),
		]
		self.assertFalse(pp.all_items_completed())
	...
	def test_temporary_name_relinking(self):
		...
		for _ in range(10):
			po_item = pp.append(
				"po_items",
				{
					...
					"temporary_name": finergy.generate_hash(length=10),
				},
			)
		...
		pp.append("sub_assembly_items", {"production_plan_item": finergy.generate_hash(length=16)})
		pp._rename_temporary_references()
	...
	def test_produced_qty_for_multi_level_bom_item(self):
		...
		pln.make_work_order()
		work_orders = finergy.get_all("Work Order", filters={"production_plan": pln.name}, pluck="name")
		sa_wo = fg_wo = None
		...
		se = finergy.get_doc(make_se_from_wo(sa_wo, "Manufacture"))
		se.submit()

		se = finergy.get_doc(make_se_from_wo(fg_wo, "Manufacture"))
		se.submit()
	...
	if not args.get("sales_order"):
		pln.append(
			"po_items",
			{
				...
				"bom_no": finergy.db.get_value("Item", args.item_code, "default_bom"),
