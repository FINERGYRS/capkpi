...
from capkpi.manufacturing.doctype.bom.bom import get_children as get_bom_children
from capkpi.manufacturing.doctype.bom.bom import validate_bom_no
from capkpi.manufacturing.doctype.work_order.work_order import get_item_details
...
class ProductionPlan(Document):
	...
	def validate_data(self):
		for d in self.get("po_items"):
			if not d.bom_no:
				finergy.throw(_("Please select BOM for Item in Row {0}").format(d.idx))
			else:
				validate_bom_no(d.item_code, d.bom_no)

			if not flt(d.planned_qty):
				finergy.throw(_("Please enter Planned Qty for Item {0} at row {1}").format(d.item_code, d.idx))
	...
	def get_open_sales_orders(self):
		...
		if open_so:
			...
			finergy.msgprint(_("Sales orders are not available for production"))
	...
	def get_bom_item(self):
		...
		bom_item = None
		has_bom = finergy.db.exists({"doctype": "BOM", "item": self.item_code, "docstatus": 1})
		if not has_bom:
			template_item = finergy.db.get_value("Item", self.item_code, ["variant_of"])
			bom_item = (
				"bom.item = {0}".format(finergy.db.escape(template_item)) if template_item else bom_item
			)
		return bom_item
	...
	def get_so_items(self):
		...
		if not self.get("sales_orders") or not self.get_so_mr_list("sales_order", "sales_orders"):
			finergy.throw(_("Please fill the Sales Orders table"), title=_("Sales Orders Required"))

		so_list = self.get_so_mr_list("sales_order", "sales_orders")

		item_condition = ""
		bom_item = "bom.item = so_item.item_code"
		if self.item_code and finergy.db.exists("Item", self.item_code):
			bom_item = self.get_bom_item() or bom_item
			item_condition = " and so_item.item_code = {0}".format(finergy.db.escape(self.item_code))
	...
	def get_mr_items(self):
		...
		item_condition = ""
		if self.item_code:
			item_condition = " and mr_item.item_code ={0}".format(finergy.db.escape(self.item_code))
	...
	def delete_draft_work_order(self):
		for d in finergy.get_all(
			...
			finergy.delete_doc("Work Order", d.name)
	...
	def get_production_items(self):
		...
		for d in self.po_items:
			...
			if not item_details["project"] and d.sales_order:
				item_details["project"] = finergy.get_cached_value("Sales Order", d.sales_order, "project")
	...
	def make_work_order(self):
		from capkpi.manufacturing.doctype.work_order.work_order import get_default_warehouse
	...
	def show_list_created_message(self, doctype, doc_list=None):
		...
		finergy.flags.mute_messages = False
		if doc_list:
			doc_list = [get_link_to_form(doctype, p) for p in doc_list]
			msgprint(_("{0} created").format(comma_and(doc_list)))

	def create_work_order(self, item):
		from capkpi.manufacturing.doctype.work_order.work_order import OverProductionError

		wo = finergy.new_doc("Work Order")
		wo.update(item)
	...
	def make_material_request(self):
		...
		for item in self.mr_items:
			item_doc = finergy.get_cached_doc("Item", item.item_code)
			...
			if not key in material_request_map:
				...
				material_request_map[key] = finergy.new_doc("Material Request")
			...
			material_request.append(
				"items",
				{
					...
					"project": finergy.db.get_value("Sales Order", item.sales_order, "project")
					if item.sales_order
					else None,
				},
			)
	...
	def get_sub_assembly_items(self, manufacturing_type=None):
		...
		for row in self.po_items:
			if not row.item_code:
				finergy.throw(_("Row #{0}: Please select Item Code in Assembly Items").format(row.idx))
...
def download_raw_materials(doc, warehouses=None):
	...
	doc.warehouse = None
	finergy.flags.show_qty_in_stock_uom = 1
...
def get_material_request_items(
	...
	if row["purchase_uom"] != row["stock_uom"]:
		if not (row["conversion_factor"] or finergy.flags.show_qty_in_stock_uom):
			finergy.throw(
	...
	if finergy.db.get_value("UOM", row["purchase_uom"], "must_be_whole_number"):
...
def get_bin_details(row, company, for_warehouse=None, all_warehouse=False):
	if isinstance(row, str):
		row = finergy._dict(json.loads(row))

	company = finergy.db.escape(company)
	...
	if warehouse:
		lft, rgt = finergy.db.get_value("Warehouse", warehouse, ["lft", "rgt"])
...
def get_warehouse_list(warehouses):
	...
	for row in warehouses:
		child_warehouses = finergy.db.get_descendants("Warehouse", row.get("warehouse"))
		if child_warehouses:
...
def get_items_for_material_requests(doc, warehouses=None, get_parent_warehouse_data=None):
	...
	for data in po_items:
		...
		if data.get("bom") or data.get("bom_no"):
			...
			if not planned_qty:
				finergy.throw(_("For row {0}: Enter Planned Qty").format(data.get("idx")))
		...
		for item_code, details in item_details.items():
			so_item_details.setdefault(sales_order, finergy._dict())
	...
	if not mr_items:
		to_enable = finergy.bold(_("Ignore Existing Projected Quantity"))
		warehouse = finergy.bold(doc.get("for_warehouse"))
		message = (
			_(
		...
		finergy.msgprint(message, title=_("Note"))

	return mr_items


def get_materials_from_other_locations(item, warehouses, new_mr_items, company):
	from capkpi.stock.doctype.pick_list.pick_list import get_available_item_locations
	...
	if required_qty:
		stock_uom, purchase_uom = finergy.db.get_value(
		...
		if purchase_uom != stock_uom and purchase_uom == item["uom"]:
			...
			if not (conversion_factor or finergy.flags.show_qty_in_stock_uom):
				finergy.throw(
					_("UOM Conversion factor ({0} -> {1}) not found for item: {2}").format(
		...
		if finergy.db.get_value("UOM", purchase_uom, "must_be_whole_number"):
			required_qty = ceil(required_qty)
...
def get_sub_assembly_items(bom_no, bom_data, to_produce_qty, indent=0):
	...
	for d in data:
		if d.expandable:
			parent_item_code = finergy.get_cached_value("BOM", bom_no, "item")
