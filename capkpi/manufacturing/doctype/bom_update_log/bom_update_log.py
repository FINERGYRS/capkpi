...
from capkpi.manufacturing.doctype.bom_update_log.bom_updation_utils import (
	get_leaf_boms,
...
class BOMMissingError(finergy.ValidationError):
	pass
...
class BOMUpdateLog(Document):
	...
	def validate_same_bom(self):
		if cstr(self.current_bom) == cstr(self.new_bom):
			finergy.throw(_("Current BOM and New BOM can not be same"))

	def validate_bom_items(self):
		current_bom_item = finergy.db.get_value("BOM", self.current_bom, "item")
		new_bom_item = finergy.db.get_value("BOM", self.new_bom, "item")

		if current_bom_item != new_bom_item:
			finergy.throw(_("The selected BOMs are not for the same item"))

	def validate_bom_cost_update_in_progress(self):
		"If another Cost Updation Log is still in progress, dont make new ones."

		wip_log = finergy.get_all(
			"BOM Update Log",
			{"update_type": "Update Cost", "status": ["in", ["Queued", "In Progress"]]},
			limit_page_length=1,
		)
		if wip_log:
			log_link = finergy.utils.get_link_to_form("BOM Update Log", wip_log[0].name)
			finergy.throw(
	...
	def on_submit(self):
		if self.update_type == "Replace BOM":
			...
			finergy.enqueue(
				method="capkpi.manufacturing.doctype.bom_update_log.bom_update_log.run_replace_bom_job",
				doc=self,
				boms=boms,
				timeout=40000,
			...
			finergy.enqueue(
				method="capkpi.manufacturing.doctype.bom_update_log.bom_update_log.process_boms_cost_level_wise",
				update_doc=self,
				now=finergy.flags.in_test,
			)
...
def queue_bom_cost_jobs(
	...
	while current_boms_list:
		...
		finergy.enqueue(
			method="capkpi.manufacturing.doctype.bom_update_log.bom_updation_utils.update_cost_in_level",
			doc=update_doc,
...
def resume_bom_cost_update_jobs():
	...
	in_progress_logs = finergy.db.get_all(
		"BOM Update Log",
	...
	for log in in_progress_logs:
		...
		bom_batches = finergy.db.get_all(
			"BOM Update Batch",
		...
		if status == "Completed":
			finergy.db.delete("BOM Update Batch", {"parent": log.name})

		if parent_boms:  # there is a next level to process
			process_boms_cost_level_wise(
				update_doc=finergy.get_doc("BOM Update Log", log.name), parent_boms=parent_boms
			)
