...
from finergy.utils.data import add_to_date, now

from capkpi.manufacturing.doctype.job_card.job_card import (
...
from capkpi.manufacturing.doctype.work_order.work_order import WorkOrder
from capkpi.manufacturing.doctype.workstation.test_workstation import make_workstation
from capkpi.stock.doctype.stock_entry.stock_entry_utils import make_stock_entry


class TestJobCard(FinergyTestCase):
	def setUp(self):
	...
	def test_job_card_operations(self):
		...
		if job_cards:
			...
			finergy.db.set_value("Job Card", job_card.name, "operation_row_number", job_card.operation_id)

			doc = finergy.get_doc("Job Card", job_card.name)
	...
	def test_job_card_with_different_work_station(self):
		...
		if job_card:
			workstation = finergy.db.get_value(
			...
			completed_qty = finergy.db.get_value(
	...
	def test_job_card_overlap(self):
		...
		jc1 = finergy.get_last_doc("Job Card", {"work_order": self.work_order.name})
		jc2 = finergy.get_last_doc("Job Card", {"work_order": wo2.name})
	...
	def test_job_card_multiple_materials_transfer(self):
		...
		job_card_name = finergy.db.get_value("Job Card", {"work_order": self.work_order.name})
		job_card = finergy.get_doc("Job Card", job_card_name)
	...
	def test_job_card_excess_material_transfer(self):
		...
		job_card = finergy.get_last_doc("Job Card", {"work_order": self.work_order.name})
	...
	def test_job_card_material_transfer_correctness(self):
		...
		job_card_name = finergy.db.get_value(
	...
	def test_corrective_costing(self):
		...
		corrective_action = finergy.get_doc(
			doctype="Operation", is_corrective_operation=1, name=finergy.generate_hash()
		).insert()
...
def create_bom_with_multiple_operations():
	...
	from capkpi.manufacturing.doctype.operation.test_operation import make_operation

	test_record = finergy.get_test_records("BOM")[2]
	bom_doc = finergy.get_doc(test_record)
