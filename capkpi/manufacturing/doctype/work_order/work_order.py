...
from capkpi.manufacturing.doctype.bom.bom import (
	get_bom_item_rate,
	get_bom_items_as_dict,
	validate_bom_no,
)
from capkpi.manufacturing.doctype.manufacturing_settings.manufacturing_settings import (
	get_mins_between_operations,
)
from capkpi.stock.doctype.batch.batch import make_batch
from capkpi.stock.doctype.item.item import get_item_defaults, validate_end_of_life
...
from capkpi.stock.stock_balance import get_planned_qty, update_bin_qty
from capkpi.stock.utils import get_bin, get_latest_stock_qty, validate_warehouse_company
from capkpi.utilities.transaction_base import validate_uom_is_integer


class OverProductionError(finergy.ValidationError):
	pass


class CapacityError(finergy.ValidationError):
	pass


class StockOverProductionError(finergy.ValidationError):
	pass


class OperationTooLongError(finergy.ValidationError):
	pass


class ItemHasVariantError(finergy.ValidationError):
	pass


class SerialNoQtyError(finergy.ValidationError):
	pass


class WorkOrder(Document):
	def onload(self):
		ms = finergy.get_doc("Manufacturing Settings")
	...
	def validate_sales_order(self):
		if self.sales_order:
			...
			if len(so):
				...
				finergy.throw(_("Sales Order {0} is not valid").format(self.sales_order))

	def check_sales_order_on_hold_or_close(self):
		status = finergy.db.get_value("Sales Order", self.sales_order, "status")
		if status in ("Closed", "On Hold"):
			finergy.throw(_("Sales Order {0} is {1}").format(self.sales_order, status))

	def set_default_warehouse(self):
		if not self.wip_warehouse:
			self.wip_warehouse = finergy.db.get_single_value(
				"Manufacturing Settings", "default_wip_warehouse"
			)
		if not self.fg_warehouse:
			self.fg_warehouse = finergy.db.get_single_value("Manufacturing Settings", "default_fg_warehouse")
	...
	def validate_work_order_against_so(self):
		# already ordered qty
		ordered_qty_against_so = finergy.db.sql(
	...
	def update_work_order_qty(self):
		...
		for purpose, fieldname in (
			...
			from capkpi.selling.doctype.sales_order.sales_order import update_produced_qty_in_so_item
	...
	def update_production_plan_status(self):
		production_plan = finergy.get_doc("Production Plan", self.production_plan)
		produced_qty = 0
		if self.production_plan_item:
			total_qty = finergy.get_all(
				"Work Order",
	...
	def on_submit(self):
		if not self.wip_warehouse and not self.skip_transfer:
			finergy.throw(_("Work-in-Progress Warehouse is required before Submit"))
		if not self.fg_warehouse:
			finergy.throw(_("For Warehouse is required before Submit"))

		if self.production_plan and finergy.db.exists(
	...
	def create_serial_no_batch_no(self):
		...
		if not cint(
			finergy.db.get_single_value("Manufacturing Settings", "make_serial_no_batch_from_work_order")
		):
			return
	...
	def create_batch_for_finished_good(self):
		...
		batch_auto_creation = finergy.get_cached_value("Item", self.production_item, "create_new_batch")
		if not batch_auto_creation:
			finergy.msgprint(
				_("Batch not created for item {} since it does not have a batch series.").format(
					finergy.bold(self.production_item)
				),
				alert=True,
				indicator="orange",
			)
	...
	def delete_auto_created_batch_and_serial_no(self):
		for row in finergy.get_all("Serial No", filters={"work_order": self.name}):
			finergy.delete_doc("Serial No", row.name)
			self.db_set("serial_no", "")

		for row in finergy.get_all("Batch", filters={"reference_name": self.name}):
			finergy.delete_doc("Batch", row.name)

	def make_serial_nos(self, args):
		self.serial_no = clean_serial_no_string(self.serial_no)
		serial_no_series = finergy.get_cached_value("Item", self.production_item, "serial_no_series")
		if serial_no_series:
	...
	def create_job_card(self):
		manufacturing_settings_doc = finergy.get_doc("Manufacturing Settings")
	...
	def set_operation_start_end_time(self, idx, row):
		...
		if row.planned_start_time == row.planned_end_time:
			finergy.throw(_("Capacity Planning Error, planned start time can not be same as end time"))

	def validate_cancel(self):
		if self.status == "Stopped":
			finergy.throw(_("Stopped Work Order cannot be cancelled, Unstop it first to cancel"))

		# Check whether any stock entry exists against this Work Order
		stock_entry = finergy.db.sql(
			"""select name from `tabStock Entry`
		...
		if stock_entry:
			finergy.throw(
				_("Cannot cancel because submitted Stock Entry {0} exists").format(
					finergy.utils.get_link_to_form("Stock Entry", stock_entry[0][0])
				)
			)
	...
	def update_planned_qty(self):
		...
		if self.material_request:
			mr_obj = finergy.get_doc("Material Request", self.material_request)
	...
	def update_ordered_qty(self):
		if (
			...
			qty = finergy.get_value("Production Plan Item", self.production_plan_item, "ordered_qty") or 0.0
			...
			finergy.db.set_value("Production Plan Item", self.production_plan_item, "ordered_qty", qty)

			doc = finergy.get_doc("Production Plan", self.production_plan)
			doc.set_status()
	...
	def update_work_order_qty_in_so(self):
		...
		if self.product_bundle_item:
			total_bundle_qty = finergy.db.sql(
				""" select sum(qty) from
				`tabProduct Bundle Item` where parent = %s""",
				(finergy.db.escape(self.product_bundle_item)),
			)[0][0]
	...
	def update_work_order_qty_in_combined_so(self):
		...
		prod_plan = finergy.get_doc("Production Plan", self.production_plan)
		item_reference = finergy.get_value(
	...
	def update_completed_qty_in_material_request(self):
		if self.material_request:
			finergy.get_doc("Material Request", self.material_request).update_completed_qty(
	...
	def set_work_order_operations(self):
		...
		self.set("operations", [])
		if not self.bom_no or not finergy.get_cached_value("BOM", self.bom_no, "with_operations"):
			return

		operations = []

		if self.use_multi_level_bom:
			bom_tree = finergy.get_doc("BOM", self.bom_no).get_tree_representation()
		...
		bom_qty = finergy.get_cached_value("BOM", self.bom_no, "quantity")
	...
	def get_holidays(self, workstation):
		holiday_list = finergy.db.get_value("Workstation", workstation, "holiday_list")
	...
	def update_operation_status(self):
		...
		for d in self.get("operations"):
			if not d.completed_qty:
				...
				finergy.throw(_("Completed Qty cannot be greater than 'Qty to Manufacture'"))
	...
	def delete_job_card(self):
		for d in finergy.get_all("Job Card", ["name"], {"work_order": self.name}):
			finergy.delete_doc("Job Card", d.name)

	def validate_production_item(self):
		if finergy.get_cached_value("Item", self.production_item, "has_variants"):
			finergy.throw(_("Work Order cannot be raised against a Item Template"), ItemHasVariantError)
	...
	def validate_qty(self):
		if not self.qty > 0:
			finergy.throw(_("Quantity to Manufacture must be greater than 0."))
	...
	def validate_operation_time(self):
		for d in self.operations:
			if not d.time_in_mins > 0:
				finergy.throw(_("Operation Time must be greater than 0 for Operation {0}").format(d.operation))
	...
	def update_transferred_qty_for_required_items(self):
		...
		for d in self.required_items:
			transferred_qty = finergy.db.sql(
				"""select sum(qty)
	...
	def update_batch_produced_qty(self, stock_entry_doc):
		...
		for row in stock_entry_doc.items:
			if row.batch_no and (row.is_finished_item or row.is_scrap_item):
				...
				finergy.db.set_value("Batch", row.batch_no, "produced_qty", flt(qty))
...
def get_bom_operations(doctype, txt, searchfield, start, page_len, filters):
	...
	return finergy.get_all("BOM Operation", filters=filters, fields=["operation"], as_list=1)
...
def get_item_details(item, project=None, skip_bom_info=False):
	...
	res["bom_no"] = finergy.db.get_value("BOM", filters=filters)

	if not res["bom_no"]:
		variant_of = finergy.db.get_value("Item", item, "variant_of")

		if variant_of:
			res["bom_no"] = finergy.db.get_value("BOM", filters={"item": variant_of, "is_default": 1})
	...
	if not res["bom_no"]:
		if project:
			...
			finergy.throw(_("Default BOM for {0} not found").format(item))

	bom_data = finergy.db.get_value(
		"BOM",
		res["bom_no"],
...
def make_work_order(bom_no, item, qty=0, project=None, variant_items=None):
	if not finergy.has_permission("Work Order", "write"):
		finergy.throw(_("Not permitted"), finergy.PermissionError)

	item_details = get_item_details(item, project)

	wo_doc = finergy.new_doc("Work Order")
...
def add_variant_item(variant_items, wo_doc, bom_no, table_name="items"):
	...
	for item in variant_items:
		...
		bom_doc = finergy.get_cached_doc("BOM", bom_no)
...
def check_if_scrap_warehouse_mandatory(bom_no):
	...
	if bom_no:
		bom = finergy.get_doc("BOM", bom_no)
...
def set_work_order_ops(name):
	po = finergy.get_doc("Work Order", name)
...
def make_stock_entry(work_order_id, purpose, qty=None):
	work_order = finergy.get_doc("Work Order", work_order_id)
	...
	if work_order.bom_no:
		stock_entry.inspection_required = finergy.db.get_value(
...
def get_default_warehouse():
	doc = finergy.get_cached_doc("Manufacturing Settings")
...
def stop_unstop(work_order, status):
	...
	pro_order = finergy.get_doc("Work Order", work_order)

	if pro_order.status == "Closed":
		finergy.throw(_("Closed Work Order can not be stopped or Re-opened"))

	pro_order.update_status(status)
	pro_order.update_planned_qty()
	finergy.msgprint(_("Work Order has been {0}").format(status))
...
def split_qty_based_on_batch_size(wo_doc, row, qty):
	if not cint(
		finergy.db.get_value("Operation", row.operation, "create_job_card_based_on_batch_size")
	):
	...
	if row.get("qty") <= 0:
		finergy.throw(
			_("Quantity to Manufacture can not be zero for the operation {0}").format(
				finergy.bold(row.get("operation"))
			)
		)
	...
	if row.get("qty") > row.get("pending_qty"):
		finergy.throw(
			_("For operation {0}: Quantity ({1}) can not be greter than pending quantity({2})").format(
				finergy.bold(row.get("operation")),
				finergy.bold(row.get("qty")),
				finergy.bold(row.get("pending_qty")),
			)
		)


def create_job_card(work_order, row, enable_capacity_planning=False, auto_create=False):
	doc = finergy.new_doc("Job Card")
	doc.update(
		{
...
def create_pick_list(source_name, target_doc=None, for_qty=None):
	...
	max_finished_goods_qty = finergy.db.get_value("Work Order", source_name, "qty")
	...
	def update_item_quantity(source, target, source_parent):
		...
		if qty:
			...
			target.uom = finergy.get_value("Item", source.item_code, "stock_uom")
...
def get_reserved_qty_for_production(item_code: str, warehouse: str) -> float:
	...
	wo = finergy.qb.DocType("Work Order")
	wo_item = finergy.qb.DocType("Work Order Item")
