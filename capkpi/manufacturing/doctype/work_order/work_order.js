...
finergy.ui.form.on("Work Order", {
	setup: function(frm) {
		...
		frm.set_query("bom_no", function() {
			if (frm.doc.production_item) {
				return {
					query: "capkpi.controllers.queries.bom",
					filters: {item: cstr(frm.doc.production_item)}
				};
			} else {
				finergy.msgprint(__("Please enter Production Item first"));
			}
		});
		...
		frm.set_query("operation", "required_items", function() {
			return {
				query: "capkpi.manufacturing.doctype.work_order.work_order.get_bom_operations",
				filters: {
	...
	onload: function(frm) {
		...
		if(frm.doc.__islocal) {
			...
			capkpi.work_order.set_default_warehouse(frm);
		}
	},
	...
	refresh: function(frm) {
		...
		capkpi.work_order.set_custom_buttons(frm);
		frm.set_intro("");
	...
	make_job_card: function(frm) {
		...
		const dialog = finergy.prompt({fieldname: 'operations', fieldtype: 'Table', label: __('Operations'),
			fields: [
				{
					fieldtype: 'Link',
					fieldname: 'operation',
			...
			finergy.call({
				method: "capkpi.manufacturing.doctype.work_order.work_order.make_job_card",
				freeze: true,
				args: {
	...
	production_item: function(frm) {
		if (frm.doc.production_item) {
			finergy.call({
				method: "capkpi.manufacturing.doctype.work_order.work_order.get_item_details",
				args: {
				...
				callback: function(r) {
					if(r.message) {
						...
						capkpi.in_production_item_onchange = true;
						...
						capkpi.in_production_item_onchange = false;
					}
				}
			});
		}
	...
	project: function(frm) {
		if(!capkpi.in_production_item_onchange && !frm.doc.bom_no) {
	...
	set_sales_order: function(frm) {
		if(frm.doc.production_item) {
			finergy.call({
				method: "capkpi.manufacturing.doctype.work_order.work_order.query_sales_order",
	...
	additional_operating_cost: function(frm) {
		capkpi.work_order.calculate_cost(frm.doc);
		capkpi.work_order.calculate_total_cost(frm);
	},
});

finergy.ui.form.on("Work Order Item", {
	source_warehouse: function(frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if(!row.item_code) {
			finergy.throw(__("Please set the Item Code first"));
			...
			finergy.call({
				...
				callback: function (r) {
					finergy.model.set_value(row.doctype, row.name,
	...
	item_code: function(frm, cdt, cdn) {
		...
		if (row.item_code) {
			finergy.call({
				method: "capkpi.stock.doctype.item.item.get_item_details",
				args: {
					item_code: row.item_code,
					company: frm.doc.company
				},
				callback: function(r) {
					if (r.message) {
						finergy.model.set_value(cdt, cdn, {
							"required_qty": 1,
...
finergy.ui.form.on("Work Order Operation", {
		...
		if (d.workstation) {
			finergy.call({
				...
				callback: function (data) {
					finergy.model.set_value(d.doctype, d.name, "hour_rate", data.message.hour_rate);
					capkpi.work_order.calculate_cost(frm.doc);
					capkpi.work_order.calculate_total_cost(frm);
				}
			});
...
capkpi.work_order = {
	set_custom_buttons: function(frm) {
		...
		if (doc.docstatus === 1 && doc.status != "Closed") {
			frm.add_custom_button(__('Close'), function() {
				finergy.confirm(__("Once the Work Order is Closed. It can't be resumed."),
					() => {
						capkpi.work_order.change_work_order_status(frm, "Closed");
					}
				);
			}, __("Status"));
			...
			if (doc.status != 'Stopped' && doc.status != 'Completed') {
				frm.add_custom_button(__('Stop'), function() {
					capkpi.work_order.change_work_order_status(frm, "Stopped");
				}, __("Status"));
			} else if (doc.status == 'Stopped') {
				frm.add_custom_button(__('Re-open'), function() {
					capkpi.work_order.change_work_order_status(frm, "Resumed");
				}, __("Status"));
			}
			...
			if (show_start_btn) {
				...
				if (pending_to_transfer && frm.doc.status != 'Stopped') {
					...
					frm.add_custom_button(__('Create Pick List'), function() {
						capkpi.work_order.create_pick_list(frm);
					});
					var start_btn = frm.add_custom_button(__('Start'), function() {
						capkpi.work_order.make_se(frm, 'Material Transfer for Manufacture');
					});
			...
			if(!frm.doc.skip_transfer){
				...
				&& frm.doc.status != 'Stopped') {
					...
					if (frm.doc.__onload && frm.doc.__onload.material_consumption == 1) {
						...
						if (counter > 0) {
							var consumption_btn = frm.add_custom_button(__('Material Consumption'), function() {
								...
								capkpi.work_order.make_consumption_se(frm, backflush_raw_materials_based_on);
							});
					...
					var finish_btn = frm.add_custom_button(__('Finish'), function() {
						capkpi.work_order.make_se(frm, 'Manufacture');
					});
	...
	calculate_cost: function(doc) {
		if (doc.operations){
			...
			for(var i=0;i<op.length;i++) {
				...
				finergy.model.set_value('Work Order Operation', op[i].name,
	...
	set_default_warehouse: function(frm) {
		if (!(frm.doc.wip_warehouse || frm.doc.fg_warehouse)) {
			finergy.call({
				method: "capkpi.manufacturing.doctype.work_order.work_order.get_default_warehouse",
				callback: function(r) {
					if (!r.exe) {
	...
	make_se: function(frm, purpose) {
		this.show_prompt_for_qty_input(frm, purpose)
			.then(data => {
				return finergy.xcall('capkpi.manufacturing.doctype.work_order.work_order.make_stock_entry', {
				...
				finergy.set_route('Form', stock_entry.doctype, stock_entry.name);
			});
	...
	create_pick_list: function(frm, purpose='Material Transfer for Manufacture') {
		this.show_prompt_for_qty_input(frm, purpose)
			.then(data => {
				return finergy.xcall('capkpi.manufacturing.doctype.work_order.work_order.create_pick_list', {
					'source_name': frm.doc.name,
					'for_qty': data.qty
				});
			}).then(pick_list => {
				finergy.model.sync(pick_list);
				finergy.set_route('Form', pick_list.doctype, pick_list.name);
			});
	},
	...
	make_consumption_se: function(frm, backflush_raw_materials_based_on) {
		...
		finergy.call({
			method:"capkpi.manufacturing.doctype.work_order.work_order.make_stock_entry",
			args: {
				"work_order_id": frm.doc.name,
	...
	change_work_order_status: function(frm, status) {
		...
		finergy.call({
			method: `capkpi.manufacturing.doctype.work_order.work_order.${method_name}`,
			freeze: true,
