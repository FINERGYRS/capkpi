...
class WorkstationHolidayError(finergy.ValidationError):
	pass


class NotInWorkingHoursError(finergy.ValidationError):
	pass
...
class Workstation(Document):
	...
	def validate_workstation_holiday(self, schedule_date, skip_holiday_list_check=False):
		if not skip_holiday_list_check and (
			...
			or cint(finergy.db.get_single_value("Manufacturing Settings", "allow_production_on_holidays"))
		):
...
def get_default_holiday_list():
	return finergy.get_cached_value(
		"Company", finergy.defaults.get_user_default("Company"), "default_holiday_list"
	)
...
def check_if_within_operating_hours(workstation, operation, from_datetime, to_datetime):
	if from_datetime and to_datetime:
		if not cint(
			finergy.db.get_value("Manufacturing Settings", "None", "allow_production_on_holidays")
		):
			check_workstation_for_holiday(workstation, from_datetime, to_datetime)

		if not cint(finergy.db.get_value("Manufacturing Settings", None, "allow_overtime")):
...
def is_within_operating_hours(workstation, operation, from_datetime, to_datetime):
	...
	workstation = finergy.get_doc("Workstation", workstation)
