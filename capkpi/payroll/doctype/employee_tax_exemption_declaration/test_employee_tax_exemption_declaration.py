...
from finergy.utils import add_months, getdate

import capkpi
from capkpi.hr.doctype.employee.test_employee import make_employee
from capkpi.hr.utils import DuplicateDeclarationError


class TestEmployeeTaxExemptionDeclaration(FinergyTestCase):
	def setUp(self):
		...
		finergy.db.delete("Employee Tax Exemption Declaration")
		finergy.db.delete("Salary Structure Assignment")

	def test_duplicate_category_in_declaration(self):
		declaration = finergy.get_doc(
			{
				"doctype": "Employee Tax Exemption Declaration",
				"employee": finergy.get_value("Employee", {"user_id": "employee@taxexemption.com"}, "name"),
		...
		self.assertRaises(finergy.ValidationError, declaration.save)
	...
	def test_duplicate_entry_for_payroll_period(self):
		...
		duplicate_declaration = finergy.get_doc(
			{
				"doctype": "Employee Tax Exemption Declaration",
				"employee": finergy.get_value("Employee", {"user_id": "employee@taxexemption.com"}, "name"),
				"company": capkpi.get_default_company(),
		...
		duplicate_declaration.employee = finergy.get_value(
	...
	def test_india_hra_exemption_with_multiple_salary_structure_assignments(self):
		from capkpi.payroll.doctype.salary_slip.test_salary_slip import create_tax_slab
...
def create_payroll_period(**args):
	...
	if not finergy.db.exists("Payroll Period", name):
		from datetime import date

		payroll_period = finergy.get_doc(
			dict(
				doctype="Payroll Period",
				name=name,
				company=args.company or capkpi.get_default_company(),
		...
		return finergy.get_doc("Payroll Period", name)


def create_exemption_category():
	if not finergy.db.exists("Employee Tax Exemption Category", "_Test Category"):
		category = finergy.get_doc(
			{
	...
	if not finergy.db.exists("Employee Tax Exemption Sub Category", "_Test Sub Category"):
		finergy.get_doc(
			{
	...
	if not finergy.db.exists("Employee Tax Exemption Sub Category", "_Test1 Sub Category"):
		finergy.get_doc(
			{
				"doctype": "Employee Tax Exemption Sub Category",
