...
finergy.ui.form.on("Salary Slip", {
	setup: function(frm) {
	...
	set_end_date: function(frm) {
		finergy.call({
			method: 'capkpi.payroll.doctype.payroll_entry.payroll_entry.get_end_date',
			args: {
	...
	set_dynamic_labels: function(frm) {
		var company_currency = frm.doc.company? capkpi.get_currency(frm.doc.company): finergy.defaults.get_default("currency");
			...
			return finergy.call({
				...
				callback: function(r) {
					if (r.message[1] !== "Leave" && r.message[0]) {
						frm.fields_dict.absent_days.set_description(__("Unmarked Days is treated as {0}. You can can change this in {1}", [r.message, finergy.utils.get_form_link("Payroll Settings", "Payroll Settings", true)]));
					}
					frm.refresh();
				}
			});
...
finergy.ui.form.on('Salary Slip Timesheet', {
