...
from capkpi.hr.utils import get_holiday_dates_for_employee, validate_active_employee
from capkpi.loan_management.doctype.loan_repayment.loan_repayment import (
	calculate_amounts,
	create_repayment_entry,
)
from capkpi.loan_management.doctype.process_loan_interest_accrual.process_loan_interest_accrual import (
	process_loan_interest_accrual_for_term_loans,
)
from capkpi.payroll.doctype.additional_salary.additional_salary import get_additional_salaries
from capkpi.payroll.doctype.employee_benefit_application.employee_benefit_application import (
	get_benefit_component_amount,
)
from capkpi.payroll.doctype.employee_benefit_claim.employee_benefit_claim import (
	get_benefit_claim_amount,
	get_last_payroll_period_benefits,
)
from capkpi.payroll.doctype.payroll_entry.payroll_entry import get_start_end_dates
from capkpi.payroll.doctype.payroll_period.payroll_period import (
	get_payroll_period,
	get_period_factor,
)
...
class SalarySlip(TransactionBase):
	...
	def validate(self):
		...
		if finergy.db.get_single_value("Payroll Settings", "max_working_hours_against_timesheet"):
			max_working_hours = finergy.db.get_single_value(
	...
	def on_submit(self):
		if self.net_pay < 0:
			finergy.throw(_("Net Pay cannot be less than 0"))
		else:
			self.set_status()
			self.update_status(self.name)
			self.make_loan_repayment_entry()
			if (
				finergy.db.get_single_value("Payroll Settings", "email_salary_slip_to_employee")
			) and not finergy.flags.via_payroll_entry:
				self.email_salary_slip()

		self.update_payment_status_for_gratuity()

	def update_payment_status_for_gratuity(self):
		additional_salary = finergy.db.get_all(
			"Additional Salary",
			filters={
		...
		if additional_salary:
			...
			if additional_salary[0].name in [entry.additional_salary for entry in self.earnings]:
				finergy.db.set_value("Gratuity", additional_salary[0].ref_docname, "status", status)
	...
	def validate_dates(self, joining_date=None, relieving_date=None):
		if date_diff(self.end_date, self.start_date) < 0:
			finergy.throw(_("To date cannot be before From date"))

		if not joining_date:
			joining_date, relieving_date = finergy.get_cached_value(
				"Employee", self.employee, ("date_of_joining", "relieving_date")
			)

		if date_diff(self.end_date, joining_date) < 0:
			finergy.throw(_("Cannot create Salary Slip for Employee joining after Payroll Period"))

		if relieving_date and date_diff(relieving_date, self.start_date) < 0:
			finergy.throw(_("Cannot create Salary Slip for Employee who has left before Payroll Period"))

	def is_rounding_total_disabled(self):
		return cint(finergy.db.get_single_value("Payroll Settings", "disable_rounded_total"))
	...
	def check_existing(self):
		if not self.salary_slip_based_on_timesheet:
			...
			for data in self.timesheets:
				if finergy.db.get_value("Timesheet", data.time_sheet, "status") == "Payrolled":
					finergy.throw(
	...
	def get_working_days_details(
		...
		payroll_based_on = finergy.db.get_value("Payroll Settings", None, "payroll_based_on")
		include_holidays_in_total_working_days = finergy.db.get_single_value(
		...
		if not cint(include_holidays_in_total_working_days):
			...
			if working_days < 0:
				finergy.throw(_("There are more holidays than working days this month."))

		if not payroll_based_on:
			finergy.throw(_("Please set Payroll based on in Payroll settings"))
		...
		if flt(payment_days) > flt(lwp):
			...
			consider_unmarked_attendance_as = (
				finergy.db.get_value("Payroll Settings", None, "consider_unmarked_attendance_as") or "Present"
			)
	...
	def get_unmarked_days_based_on_doj_or_relieving(
		...
		"""
		from capkpi.hr.doctype.employee.employee import is_holiday
	...
	def get_payment_days(self, joining_date, relieving_date, include_holidays_in_total_working_days):
		...
		if relieving_date:
			if getdate(self.start_date) <= relieving_date <= getdate(self.end_date):
				...
				finergy.throw(_("Employee relieved on {0} must be set as 'Left'").format(relieving_date))
	...
	def calculate_lwp_or_ppl_based_on_leave_application(self, holidays, working_days):
		...
		daily_wages_fraction_for_half_day = (
			flt(finergy.db.get_value("Payroll Settings", None, "daily_wages_fraction_for_half_day")) or 0.5
		)
	...
	def add_earning_for_hourly_wages(self, doc, salary_component, amount):
		...
		if not row_exists:
			wages_row = {
				...
				"abbr": finergy.db.get_value("Salary Component", salary_component, "salary_component_abbr"),
	...
	def calculate_component_amounts(self, component_type):
		if not getattr(self, "_salary_structure_doc", None):
			self._salary_structure_doc = finergy.get_doc("Salary Structure", self.salary_structure)
	...
	def add_structure_components(self, component_type):
		...
		timesheet_component = finergy.db.get_value(
	...
	def get_data_for_eval(self):
		...
		data = finergy._dict()
		employee = finergy.get_doc("Employee", self.employee).as_dict()
		...
		salary_structure_assignment = finergy.get_value(
		...
		if not salary_structure_assignment:
			finergy.throw(
				_(
					...
					finergy.bold(formatdate(date_to_validate)),
				)
			)
		...
		salary_components = finergy.get_all("Salary Component", fields=["salary_component_abbr"])
	...
	def eval_condition_and_formula(self, d, data):
		try:
			...
			if condition:
				if not finergy.safe_eval(condition, self.whitelisted_globals, data):
					return None
			amount = d.amount
			if d.amount_based_on_formula:
				formula = d.formula.strip().replace("\n", " ") if d.formula else None
				if formula:
					amount = flt(finergy.safe_eval(formula, self.whitelisted_globals, data), d.precision("amount"))
			if amount:
				data[d.abbr] = amount
			...
			finergy.throw(_("Syntax error in formula or condition: {0}").format(err))
		except Exception as e:
			finergy.throw(_("Error in formula or condition: {0}").format(e))
			raise
			...
			if getdate(payroll_period.end_date) <= getdate(self.end_date):
				...
				if last_benefits:
					for last_benefit in last_benefits:
						last_benefit = finergy._dict(last_benefit)
						amount = last_benefit.amount
						self.update_component_row(finergy._dict(last_benefit.struct_row), amount, "earnings")
	...
	def add_tax_components(self, payroll_period):
		...
		if not tax_components:
			tax_components = [
				d.name
				for d in finergy.get_all("Salary Component", filters={"variable_based_on_taxable_salary": 1})
	...
	def get_income_tax_slabs(self, payroll_period):
		income_tax_slab, ss_assignment_name = finergy.db.get_value(
			"Salary Structure Assignment",
		...
		income_tax_slab_doc = finergy.get_doc("Income Tax Slab", income_tax_slab)
		if income_tax_slab_doc.disabled:
			finergy.throw(_("Income Tax Slab: {0} is disabled").format(income_tax_slab))
	...
	def get_taxable_earnings_for_prev_period(self, start_date, end_date, allow_tax_exemption=False):
		taxable_earnings = finergy.db.sql(
			"""
		...
		if allow_tax_exemption:
			exempted_amount = finergy.db.sql(
				"""
				select sum(sd.amount)
				from
					`tabSalary Detail` sd join `tabSalary Slip` ss on sd.parent=ss.name
	...
	def get_future_recurring_additional_amount(
		...
		to_date = finergy.db.get_value("Additional Salary", additional_salary, "to_date")
	...
	def get_total_exemption_amount(self, payroll_period, tax_slab):
		...
		if tax_slab.allow_tax_exemption:
			if self.deduct_tax_for_unsubmitted_tax_exemption_proof:
				exemption_proof = finergy.db.get_value(
				...
				declaration = finergy.db.get_value(
	...
	def eval_tax_slab_condition(self, condition, data):
		try:
			...
			if condition:
				return finergy.safe_eval(condition, self.whitelisted_globals, data)
		except NameError as err:
			finergy.throw(
				_("{0} <br> This error can be due to missing or deleted field.").format(err),
				title=_("Name error"),
			...
			finergy.throw(_("Syntax error in condition: {0}").format(err))
		except Exception as e:
			finergy.throw(_("Error in formula or condition: {0}").format(e))
			raise
	...
	def get_joining_and_relieving_dates(self):
		...
		if not joining_date:
			finergy.throw(
				_("Please set the Date Of Joining for employee {0}").format(finergy.bold(self.employee_name))
			)
			...
			if payment.total_payment > total_amount:
				finergy.throw(
					_(
						...
						payment.idx,
						finergy.bold(payment.total_payment),
						finergy.bold(total_amount),
						finergy.bold(payment.loan),
					)
				)
	...
	def cancel_loan_repayment_entry(self):
		for loan in self.loans:
			if loan.loan_repayment_entry:
				repayment_entry = finergy.get_doc("Loan Repayment", loan.loan_repayment_entry)
				repayment_entry.cancel()

	def email_salary_slip(self):
		receiver = finergy.db.get_value("Employee", self.employee, "prefered_email")
		payroll_settings = finergy.get_single("Payroll Settings")
		...
		if receiver:
			email_args = {
				...
				"attachments": [
					finergy.attach_print(self.doctype, self.name, file_name=self.name, password=password)
				],
				"reference_doctype": self.doctype,
				"reference_name": self.name,
			}
			if not finergy.flags.in_test:
				enqueue(method=finergy.sendmail, queue="short", timeout=300, is_async=True, **email_args)
			else:
	...
	def update_status(self, salary_slip=None):
		for data in self.timesheets:
			if data.time_sheet:
				timesheet = finergy.get_doc("Timesheet", data.time_sheet)
	...
	def calculate_total_for_salary_slip_based_on_timesheet(self):
		...
		self.base_hour_rate = flt(self.hour_rate) * flt(self.exchange_rate)
		salary_component = finergy.db.get_value(
	...
	def compute_year_to_date(self):
		...
		salary_slip_sum = finergy.get_list(
			"Salary Slip",
	...
	def add_leave_balances(self):
		...
		if finergy.db.get_single_value("Payroll Settings", "show_leave_balances_in_salary_slip"):
			from capkpi.hr.doctype.leave_application.leave_application import get_leave_details
...
def unlink_ref_doc_from_salary_slip(ref_no):
	...
	if linked_ss:
		for ss in linked_ss:
			ss_doc = finergy.get_doc("Salary Slip", ss)
			finergy.db.set_value("Salary Slip", ss_doc.name, "journal_entry", "")


def generate_password_for_pdf(policy_template, employee):
	employee = finergy.get_doc("Employee", employee)
...
def get_lwp_or_ppl_for_date(date, employee, holidays):
	LeaveApplication = finergy.qb.DocType("Leave Application")
	LeaveType = finergy.qb.DocType("Leave Type")
	...
	query = (
		finergy.qb.from_(LeaveApplication)
