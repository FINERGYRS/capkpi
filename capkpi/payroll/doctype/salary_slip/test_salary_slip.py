...
from capkpi.hr.doctype.leave_allocation.test_leave_allocation import create_leave_allocation
from capkpi.hr.doctype.leave_type.test_leave_type import create_leave_type
from capkpi.payroll.doctype.employee_tax_exemption_declaration.test_employee_tax_exemption_declaration import (
	create_exemption_category,
	create_payroll_period,
)
from capkpi.payroll.doctype.payroll_entry.payroll_entry import get_month_details
from capkpi.payroll.doctype.salary_structure.salary_structure import make_salary_slip


class TestSalarySlip(FinergyTestCase):
	def setUp(self):
		setup_test()
		finergy.flags.pop("via_payroll_entry", None)

	def tearDown(self):
		finergy.db.set_value("Payroll Settings", None, "include_holidays_in_total_working_days", 0)
	...
	def test_payment_days_based_on_attendance(self):
		...
		finergy.db.set_value("Employee", emp_id, {"relieving_date": None, "status": "Active"})

		finergy.db.set_value("Leave Type", "Leave Without Pay", "include_holiday", 0)

		month_start_date = get_first_day(nowdate())
		month_end_date = get_last_day(nowdate())

		first_sunday = finergy.db.sql(
			"""
	...
	def test_payment_days_for_mid_joinee_including_holidays(self):
		...
		finergy.delete_doc("Salary Slip", new_ss.name, force=True)
		new_ss = make_employee_salary_slip(
			"test_payment_days_based_on_joining_date@salary.com",
			"Monthly",
			"Test Payment Based On Attendence",
	...
	def test_payment_days_in_salary_slip_based_on_timesheet(self):
		from capkpi.hr.doctype.attendance.attendance import mark_attendance
		from capkpi.projects.doctype.timesheet.test_timesheet import (
			make_salary_structure_for_timesheet,
			make_timesheet,
		)
		from capkpi.projects.doctype.timesheet.timesheet import (
		...
		finergy.db.set_value("Employee", emp, {"relieving_date": None, "status": "Active"})
	...
	def test_component_amount_dependent_on_another_payment_days_based_component(self):
		from capkpi.hr.doctype.attendance.attendance import mark_attendance
		from capkpi.payroll.doctype.salary_structure.test_salary_structure import (
	...
	def test_payment_days(self):
		...
		if date_of_joining.day > 1:
			self.assertRaises(finergy.ValidationError, ss.save)
		...
		finergy.db.set_value(
			"Employee",
			finergy.get_value("Employee", {"employee_name": "test_payment_days@salary.com"}, "name"),
			"relieving_date",
			None,
		)
		finergy.db.set_value(
	...
	def test_employee_salary_slip_read_permission(self):
		...
		finergy.set_user("test_employee_salary_slip_read_permission@salary.com")
	...
	def test_email_salary_slip(self):
		...
		email_queue = finergy.db.a_row_exists("Email Queue")
		self.assertTrue(email_queue)

	def test_loan_repayment_salary_slip(self):
		from capkpi.loan_management.doctype.loan.test_loan import (
			create_loan,
	...
	def test_payroll_frequency(self):
		fiscal_year = get_fiscal_year(nowdate(), company=capkpi.get_default_company())[0]
	...
	def test_component_wise_year_to_date_computation(self):
		...
		# clear salary slip for this employee
		finergy.db.sql("DELETE FROM `tabSalary Slip` where employee_name = '%s'" % employee_name)
		...
		year_to_date = dict()
		for slip in salary_slips:
			doc = finergy.get_doc("Salary Slip", slip.name)
	...
	def test_tax_for_payroll_period(self):
		...
		finergy.db.sql("""delete from `tabPayroll Period`""")
		finergy.db.sql("""delete from `tabSalary Component`""")
		...
		for doc in delete_docs:
			finergy.db.sql("delete from `tab%s` where employee='%s'" % (doc, employee))
		...
		try:
			...
			raise
		finergy.db.sql("""delete from `tabSalary Slip` where employee=%s""", (employee))
		...
		try:
			...
			print("\nSalary Slip - Tax calculation failed on following case\n", data, "\n")
			raise
		finergy.db.sql("""delete from `tabAdditional Salary` where employee=%s""", (employee))
	...
	def make_activity_for_employee(self):
		activity_type = finergy.get_doc("Activity Type", "_Test Activity Type")
...
def make_employee_salary_slip(user, payroll_frequency, salary_structure=None, posting_date=None):
	...
	salary_slip_name = finergy.db.get_value(
		"Salary Slip", {"employee": finergy.db.get_value("Employee", {"user_id": user})}
	)
	...
	if not salary_slip_name:
		...
		salary_slip = finergy.get_doc("Salary Slip", salary_slip_name)
...
def make_salary_component(salary_components, test_tax, company_list=None):
	for salary_component in salary_components:
		if finergy.db.exists("Salary Component", salary_component["salary_component"]):
			finergy.delete_doc("Salary Component", salary_component["salary_component"], force=True)
		...
		doc = finergy.new_doc("Salary Component")
...
def get_salary_component_account(sal_comp, company_list=None):
	...
	if not isinstance(sal_comp, Document):
		sal_comp = finergy.get_doc("Salary Component", sal_comp)

	if not sal_comp.get("accounts"):
		for d in company_list:
			company_abbr = finergy.get_cached_value("Company", d, "abbr")
...
def create_account(account_name, company, parent_account, account_type=None):
	...
	account = finergy.db.get_value("Account", account_name + " - " + company_abbr)
	if not account:
		finergy.get_doc(
			{
				"doctype": "Account",
...
def get_tax_paid_in_period(employee):
	tax_paid_amount = finergy.db.sql(
...
def create_proof_submission(employee, payroll_period, amount):
	...
	proof_submission = finergy.get_doc(
		{
...
def create_tax_slab(
	...
	if company:
		currency = capkpi.get_company_currency(company)
	...
	income_tax_slab_name = finergy.db.get_value("Income Tax Slab", {"currency": currency})
	if not income_tax_slab_name:
		income_tax_slab = finergy.new_doc("Income Tax Slab")
...
def make_leave_application(
	...
	leave_application = finergy.get_doc(
		dict(
			...
			company=company or capkpi.get_default_company() or "_Test Company",
			status="Approved",
...
def setup_test():
	...
	for dt in [
		...
		finergy.db.sql("delete from `tab%s`" % dt)

	make_holiday_list()

	finergy.db.set_value(
		"Company", capkpi.get_default_company(), "default_holiday_list", "Salary Slip Test Holiday List"
	)
	finergy.db.set_value("Payroll Settings", None, "email_salary_slip_to_employee", 0)
	finergy.db.set_value("HR Settings", None, "leave_status_notification_template", None)
	finergy.db.set_value("HR Settings", None, "leave_approval_notification_template", None)


def make_holiday_list(list_name=None, from_date=None, to_date=None):
	if not (from_date and to_date):
		fiscal_year = get_fiscal_year(nowdate(), company=capkpi.get_default_company())
	name = list_name or "Salary Slip Test Holiday List"

	finergy.delete_doc_if_exists("Holiday List", name, force=True)
...
def make_salary_structure_for_payment_days_based_component_dependency():
	...
	if finergy.db.exists("Salary Structure", salary_structure):
		finergy.db.delete("Salary Structure", salary_structure)
	...
	salary_structure_doc = finergy.get_doc(details)
...
def make_salary_slip_for_payment_days_dependency_test(employee, salary_structure):
	...
	salary_slip_name = finergy.db.get_value(
		"Salary Slip", {"employee": finergy.db.get_value("Employee", {"user_id": employee})}
	)
