...
finergy.ui.form.on('Salary Structure', {
	onload: function(frm) {
		...
		let help_button = $(`<a class = 'control-label'>
			...
			let message_html = finergy.render_template("condition_and_formula_help")
	...
	refresh: function(frm) {
		...
		if(frm.doc.docstatus==1) {
			frm.add_custom_button(__("Assign Salary Structure"), function() {
				var doc = finergy.model.get_new_doc('Salary Structure Assignment');
				doc.salary_structure = frm.doc.name;
				doc.company = frm.doc.company;
				finergy.set_route('Form', 'Salary Structure Assignment', doc.name);
			});
	...
	preview_salary_slip: function(frm) {
		finergy.call({
			method: "capkpi.payroll.doctype.salary_structure.salary_structure.get_employees",
			args: {
	...
	open_salary_slip: function(frm, employee){
		...
		finergy.call({
			method: "capkpi.payroll.doctype.salary_structure.salary_structure.make_salary_slip",
			args: {
...
var validate_date = function(frm, cdt, cdn) {
	...
	if(doc.to_date && doc.from_date) {
		var from_date = finergy.datetime.str_to_obj(doc.from_date);
		var to_date = finergy.datetime.str_to_obj(doc.to_date);

		if(to_date < from_date) {
			finergy.model.set_value(cdt, cdn, "to_date", "");
			finergy.throw(__("From Date cannot be greater than To Date"));
		}
	}
}
...
finergy.ui.form.on('Salary Detail', {
	amount: function(frm) {
		calculate_totals(frm.doc);
	},
			...
			finergy.call({
				...
				callback: function(data) {
					if(data.message){
						...
						finergy.model.set_value(cdt, cdn, 'condition', result.condition);
						finergy.model.set_value(cdt, cdn, 'amount_based_on_formula', result.amount_based_on_formula);
						if(result.amount_based_on_formula == 1){
							finergy.model.set_value(cdt, cdn, 'formula', result.formula);
						}
						else{
							finergy.model.set_value(cdt, cdn, 'amount', result.amount);
						}
						finergy.model.set_value(cdt, cdn, 'statistical_component', result.statistical_component);
						finergy.model.set_value(cdt, cdn, 'depends_on_payment_days', result.depends_on_payment_days);
						finergy.model.set_value(cdt, cdn, 'do_not_include_in_total', result.do_not_include_in_total);
						finergy.model.set_value(cdt, cdn, 'variable_based_on_taxable_salary', result.variable_based_on_taxable_salary);
						finergy.model.set_value(cdt, cdn, 'is_tax_applicable', result.is_tax_applicable);
						finergy.model.set_value(cdt, cdn, 'is_flexible_benefit', result.is_flexible_benefit);
	...
	amount_based_on_formula: function(frm, cdt, cdn) {
		...
		if(child.amount_based_on_formula == 1){
			finergy.model.set_value(cdt, cdn, 'amount', null);
		}
		else{
			finergy.model.set_value(cdt, cdn, 'formula', null);
		}
	}
})
