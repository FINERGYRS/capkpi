...
class SalaryStructure(Document):
	...
	def set_missing_values(self):
		...
		for table in ["earnings", "deductions"]:
			for d in self.get(table):
				component_default_value = finergy.db.get_value(
					"Salary Component",
	...
	def validate_amount(self):
		if flt(self.net_pay) < 0 and self.salary_slip_based_on_timesheet:
			finergy.throw(_("Net pay cannot be negative"))
			...
			for row in self.get(component_type):
				if (
					...
					message = _("Row #{0}: The {1} Component has the options {2} and {3} enabled.").format(
						row.idx,
						finergy.bold(row.salary_component),
						finergy.bold("Amount based on formula"),
						finergy.bold("Depends On Payment Days"),
					)
					message += "<br><br>" + _(
						...
						finergy.bold("Depends On Payment Days"), finergy.bold(row.salary_component)
					)
					finergy.throw(message, title=_("Payment Days Dependency"))
		...
		for component in self.earnings:
			if component.salary_component == self.salary_component:
				finergy.msgprint(
					...
					).format(self.idx, finergy.bold(self.salary_component)),
					title=_("Warning"),
					indicator="orange",
				)
				break
		...
		if self.earnings:
			...
			for earning_component in self.earnings:
				if earning_component.is_flexible_benefit == 1:
					have_a_flexi = True
					max_of_component = finergy.db.get_value(
			...
			if have_a_flexi and flt(self.max_benefits) == 0:
				finergy.throw(_("Max benefits should be greater than zero to dispense benefits"))
	...
	def assign_salary_structure(
		...
		if employees:
			...
			finergy.msgprint(_("No Employee Found"))
...
def assign_salary_structure_for_employees(
	...
	if salary_structures_assignments:
		finergy.msgprint(_("Structures have been assigned successfully"))
...
def create_salary_structures_assignment(
	...
	if not payroll_payable_account:
		...
		if not payroll_payable_account:
			finergy.throw(_('Please set "Default Payroll Payable Account" in Company Defaults'))
	payroll_payable_account_currency = finergy.db.get_value(
		"Account", payroll_payable_account, "account_currency"
	)
	company_curency = capkpi.get_company_currency(salary_structure.company)
	if (
	...
	assignment = finergy.new_doc("Salary Structure Assignment")
...
def get_existing_assignments(employees, salary_structure, from_date):
	salary_structures_assignments = finergy.db.sql_list(
		"""
...
def make_salary_slip(
	...
	def postprocess(source, target):
		if employee:
			employee_details = finergy.db.get_value(
				"Employee",
				employee,
			...
			if not target.payroll_cost_center and target.department:
				target.payroll_cost_center = finergy.db.get_value(
	...
	if cint(as_print):
		...
		return finergy.get_print(doc.doctype, doc.name, doc=doc, print_format=print_format)
	else:
		return doc
