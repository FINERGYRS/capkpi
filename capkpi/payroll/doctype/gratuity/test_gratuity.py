...
from finergy.utils import add_days, add_months, floor, flt, get_datetime, get_first_day, getdate
...
from capkpi.payroll.doctype.gratuity.gratuity import get_last_salary_slip
...
class TestGratuity(FinergyTestCase):
	def setUp(self):
		finergy.db.delete("Gratuity")
		finergy.db.delete("Salary Slip")
		finergy.db.delete("Additional Salary", {"ref_doctype": "Gratuity"})
	...
	def test_gratuity_based_on_current_slab_via_additional_salary(self):
		...
		# amount calculation
		component_amount = finergy.get_all(
			"Salary Detail",
			filters={
				"docstatus": 1,
				"parent": sal_slip,
		...
		self.assertTrue(finergy.db.exists("Additional Salary", {"ref_docname": gratuity.name}))
...
def get_gratuity_rule(name):
	rule = finergy.db.exists("Gratuity Rule", name)
	if not rule:
		create_gratuity_rule()
	rule = finergy.get_doc("Gratuity Rule", name)
...
def create_gratuity(**args):
	...
	gratuity = finergy.new_doc("Gratuity")
...
def set_mode_of_payment_account():
	...
	mode_of_payment = finergy.get_doc("Mode of Payment", "Cash")
...
def create_salary_slip(employee):
	if not finergy.db.exists("Salary Slip", {"employee": employee}):
