...
from finergy.utils import add_months

import capkpi
from capkpi.accounts.utils import get_fiscal_year, getdate, nowdate
...
from capkpi.payroll.doctype.payroll_entry.payroll_entry import get_end_date, get_start_end_dates
...
class TestPayrollEntry(FinergyTestCase):
	@classmethod
	def setUpClass(cls):
	...
	def test_payroll_entry(self):  # pylint: disable=no-self-use
		...
		for data in finergy.get_all("Salary Component", fields=["name"]):
			if not finergy.db.get_value(
				"Salary Component Account", {"parent": data.name, "company": company}, "name"
			):
				get_salary_component_account(data.name)

		employee = finergy.db.get_value("Employee", {"company": company})
	...
	def test_multi_currency_payroll_entry(self):  # pylint: disable=no-self-use
		...
		finergy.db.sql(
			...
			(finergy.db.get_value("Employee", {"user_id": "test_muti_currency_employee@payroll.com"})),
		)
		...
		if payroll_je:
			payroll_je_doc = finergy.get_doc("Journal Entry", payroll_je)

			self.assertEqual(salary_slip.base_gross_pay, payroll_je_doc.total_debit)
			self.assertEqual(salary_slip.base_gross_pay, payroll_je_doc.total_credit)

		payment_entry = finergy.db.sql(
			"""
	...
	def test_payroll_entry_with_employee_cost_center(self):  # pylint: disable=no-self-use
		...
		if not finergy.db.exists("Department", "cc - _TC"):
			finergy.get_doc(
				{"doctype": "Department", "department_name": "cc", "company": "_Test Company"}
			).insert()

		finergy.db.sql("""delete from `tabEmployee` where employee_name='test_employee1@example.com' """)
		finergy.db.sql("""delete from `tabEmployee` where employee_name='test_employee2@example.com' """)
		finergy.db.sql("""delete from `tabSalary Structure` where name='_Test Salary Structure 1' """)
		finergy.db.sql("""delete from `tabSalary Structure` where name='_Test Salary Structure 2' """)
		...
		if not finergy.db.exists("Account", "_Test Payroll Payable - _TC"):
			create_account(
		...
		if (
			not finergy.db.get_value("Company", "_Test Company", "default_payroll_payable_account")
			or finergy.db.get_value("Company", "_Test Company", "default_payroll_payable_account")
			!= "_Test Payroll Payable - _TC"
		):
			finergy.db.set_value(
				"Company", "_Test Company", "default_payroll_payable_account", "_Test Payroll Payable - _TC"
			)
		currency = finergy.db.get_value("Company", "_Test Company", "default_currency")
		make_salary_structure(
		...
		if not finergy.db.get_value(
			...
			je = finergy.db.get_value("Salary Slip", {"payroll_entry": pe.name}, "journal_entry")
			je_entries = finergy.db.sql(
				"""
	...
	def test_loan(self):
		...
		if not company_doc.default_payroll_payable_account:
			company_doc.default_payroll_payable_account = finergy.db.get_value(
				"Account", {"company": company, "root_type": "Liability", "account_type": ""}, "name"
			)
			company_doc.save()

		if not finergy.db.exists("Branch", branch):
			finergy.get_doc({"doctype": "Branch", "branch": branch}).insert()

		employee_doc = finergy.get_doc("Employee", applicant)
		...
		if not finergy.db.exists("Loan Type", "Car Loan"):
			create_loan_accounts()
			create_loan_type(
				"Car Loan",
				500000,
		...
		salary_slip = finergy.get_doc("Salary Slip", name)
		...
		if salary_slip.docstatus == 0:
			finergy.delete_doc("Salary Slip", name)
...
def make_payroll_entry(**args):
	...
	payroll_entry.company = args.company or capkpi.get_default_company()
...
def make_holiday(holiday_list_name):
	if not finergy.db.exists("Holiday List", holiday_list_name):
