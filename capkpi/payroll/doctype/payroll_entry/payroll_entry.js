...
finergy.ui.form.on('Payroll Entry', {
	onload: function (frm) {
		if (!frm.doc.posting_date) {
			frm.doc.posting_date = finergy.datetime.nowdate();
		}
	...
	refresh: function (frm) {
		if (frm.doc.docstatus == 0) {
			...
			if ((frm.doc.employees || []).length && !finergy.model.has_workflow(frm.doctype)) {
				frm.page.clear_primary_action();
	...
	add_bank_entry_button: function (frm) {
		finergy.call({
			method: 'capkpi.payroll.doctype.payroll_entry.payroll_entry.payroll_entry_has_bank_entries',
			args: {
				'name': frm.doc.name
			},
			callback: function (r) {
	...
	setup: function (frm) {
		...
		frm.set_query('employee', 'employees', () => {
			...
			mandatory_fields.forEach(field => {
				if (!frm.doc[field]) {
					error_fields.push(finergy.unscrub(field));
				}
			});
			...
			return {
				query: "capkpi.payroll.doctype.payroll_entry.payroll_entry.employee_query",
	...
	set_payable_account_and_currency: function (frm) {
		finergy.db.get_value("Company", {"name": frm.doc.company}, "default_currency", (r) => {
			frm.set_value('currency', r.default_currency);
		});
		finergy.db.get_value("Company", {"name": frm.doc.company}, "default_payroll_payable_account", (r) => {
	...
	set_start_end_dates: function (frm) {
		if (!frm.doc.salary_slip_based_on_timesheet) {
			finergy.call({
				method: 'capkpi.payroll.doctype.payroll_entry.payroll_entry.get_start_end_dates',
				args: {
...
const submit_salary_slip = function (frm) {
	finergy.confirm(__('This will submit Salary Slips and create accrual Journal Entry. Do you want to proceed?'),
		function () {
			finergy.call({
...
let make_bank_entry = function (frm) {
	...
	if (doc.payment_account) {
		...
		finergy.msgprint(__("Payment Account is mandatory"));
...
let render_employee_attendance = function (frm, data) {
	frm.fields_dict.attendance_detail_html.html(
		finergy.render_template('employees_to_mark_attendance', {
			data: data
		})
	);
};
