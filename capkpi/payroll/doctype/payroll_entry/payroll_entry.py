...
class PayrollEntry(Document):
	def onload(self):
		...
		entries = finergy.db.count("Salary Slip", {"payroll_entry": self.name, "docstatus": 1}, ["name"])
	...
	def before_submit(self):
		...
		if self.validate_attendance:
			if self.validate_employee_attendance():
				finergy.throw(_("Cannot Submit, Employees left to mark attendance"))
	...
	def validate_employee_details(self):
		...
		if len(emp_with_sal_slip):
			finergy.throw(_("Salary Slip already exists for {0}").format(comma_and(emp_with_sal_slip)))

	def validate_payroll_payable_account(self):
		if finergy.db.get_value("Account", self.payroll_payable_account, "account_type"):
			finergy.throw(
				_(
					"Account type cannot be set for payroll payable account {0}, please remove and try again"
				).format(finergy.bold(get_link_to_form("Account", self.payroll_payable_account)))
			)
	...
	def fill_employee_details(self):
		...
		if not employees:
			error_msg = _(
				...
				finergy.bold(self.currency),
				finergy.bold(self.payroll_payable_account),
			)
			if self.branch:
				error_msg += "<br>" + _("Branch: {0}").format(finergy.bold(self.branch))
			if self.department:
				error_msg += "<br>" + _("Department: {0}").format(finergy.bold(self.department))
			if self.designation:
				error_msg += "<br>" + _("Designation: {0}").format(finergy.bold(self.designation))
			if self.start_date:
				error_msg += "<br>" + _("Start date: {0}").format(finergy.bold(self.start_date))
			if self.end_date:
				error_msg += "<br>" + _("End date: {0}").format(finergy.bold(self.end_date))
			finergy.throw(error_msg, title=_("No employees found"))
	...
	def check_mandatory(self):
		for fieldname in ["company", "start_date", "end_date"]:
			if not self.get(fieldname):
				finergy.throw(_("Please set {0}").format(self.meta.get_label(fieldname)))
	...
	def create_salary_slips(self):
		...
		if employees:
			...
			if len(employees) > 30:
				finergy.enqueue(create_salary_slips_for_employees, timeout=600, employees=employees, args=args)
			else:
	...
	def email_salary_slip(self, submitted_ss):
		if finergy.db.get_single_value("Payroll Settings", "email_salary_slip_to_employee"):
	...
	def get_salary_component_account(self, salary_component):
		...
		if not account:
			finergy.throw(_("Please set account in Salary Component {0}").format(salary_component))
	...
	def get_salary_components(self, component_type):
		...
		if salary_slips:
			salary_components = finergy.db.sql(
				"""
		...
		if salary_components:
			...
			for item in salary_components:
				...
				if component_type == "earnings":
					is_flexible_benefit, only_tax_impact = finergy.db.get_value(
	...
	def make_payment_entry(self):
		...
		salary_slip_name_list = finergy.db.sql(
		...
		if salary_slip_name_list and len(salary_slip_name_list) > 0:
			...
			for salary_slip_name in salary_slip_name_list:
				salary_slip = finergy.get_doc("Salary Slip", salary_slip_name[0])
				...
				for sal_detail in salary_slip.deductions:
					statistical_component = finergy.db.get_value(
	...
	def update_salary_slip_status(self, jv_name=None):
		...
		for ss in ss_list:
			ss_obj = finergy.get_doc("Salary Slip", ss[0])
			finergy.db.set_value("Salary Slip", ss_obj.name, "journal_entry", jv_name)
	...
	def validate_employee_attendance(self):
		...
		for employee_detail in self.employees:
			employee_joining_date = finergy.db.get_value(
...
def get_filter_condition(filters):
	...
	for f in ["company", "branch", "department", "designation"]:
		if filters.get(f):
			cond += " and t1." + f + " = " + finergy.db.escape(filters.get(f))
...
def get_start_end_dates(payroll_frequency, start_date=None, company=None):
	...
	return finergy._dict({"start_date": start_date, "end_date": end_date})
...
def get_month_details(year, month):
	ysd = finergy.db.get_value("Fiscal Year", year, "year_start_date")
	if ysd:
		import calendar
		import datetime
		...
		finergy.throw(_("Fiscal Year {0} not found").format(year))
...
def create_salary_slips_for_employees(employees, args, publish_progress=True):
	...
	for emp in employees:
		if emp not in salary_slips_exists_for:
			...
			ss = finergy.get_doc(args)
			ss.insert()
			count += 1
			if publish_progress:
				finergy.publish_progress(
	...
	payroll_entry = finergy.get_doc("Payroll Entry", args.payroll_entry)
	...
	if salary_slips_not_created:
		finergy.msgprint(
			...
			).format(finergy.bold(", ".join([emp for emp in salary_slips_not_created]))),
			title=_("Message"),
			indicator="orange",
		)
...
def submit_salary_slips_for_employees(payroll_entry, salary_slips, publish_progress=True):
	submitted_ss = []
	not_submitted_ss = []
	finergy.flags.via_payroll_entry = True
	...
	for ss in salary_slips:
		...
		count += 1
		if publish_progress:
			finergy.publish_progress(count * 100 / len(salary_slips), title=_("Submitting Salary Slips..."))
	if submitted_ss:
	...
	if not_submitted_ss:
		finergy.msgprint(_("Could not submit some Salary Slips"))

	finergy.flags.via_payroll_entry = False
...
def employee_query(doctype, txt, searchfield, start, page_len, filters):
	filters = finergy._dict(filters)
	conditions = []
	include_employees = []
	emp_cond = ""

	if not filters.payroll_frequency:
		finergy.throw(_("Select Payroll Frequency."))
