...
from finergy.utils import add_days, add_months, nowdate
...
class TestAdditionalSalary(FinergyTestCase):
	def setUp(self):
		setup_test()
	...
	def test_recurring_additional_salary(self):
		...
		finergy.db.set_value("Employee", emp_id, "relieving_date", add_days(nowdate(), 1800))
	...
	def test_non_recurring_additional_salary(self):
		...
		emp_id = make_employee("test_additional@salary.com")
		finergy.db.set_value("Employee", emp_id, "relieving_date", add_days(date, 1800))
		salary_structure = make_salary_structure(
			"Test Salary Structure Additional Salary", "Monthly", employee=emp_id
		)
...
def get_additional_salary(emp_id, recurring=True, payroll_date=None):
	...
	add_sal = finergy.new_doc("Additional Salary")
	...
	add_sal.amount = 5000
	add_sal.currency = capkpi.get_default_currency()
	add_sal.save()
	add_sal.submit()
