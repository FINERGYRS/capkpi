...
from capkpi.hr.doctype.vehicle_log.test_vehicle_log import get_vehicle, make_vehicle_log
from capkpi.hr.doctype.vehicle_log.vehicle_log import make_expense_claim
from capkpi.hr.report.vehicle_expenses.vehicle_expenses import execute


class TestVehicleExpenses(unittest.TestCase):
	@classmethod
	def setUpClass(self):
		finergy.db.sql("delete from `tabVehicle Log`")

		employee_id = finergy.db.sql(
	...
	def test_vehicle_expenses_based_on_fiscal_year(self):
		...
		# clean up
		vehicle_log.cancel()
		finergy.delete_doc("Expense Claim", expense_claim.name)
		finergy.delete_doc("Vehicle Log", vehicle_log.name)

	def tearDown(self):
		finergy.delete_doc("Vehicle", self.license_plate, force=1)
		finergy.delete_doc("Employee", self.employee_id, force=1)
