...
from capkpi.hr.doctype.leave_allocation.leave_allocation import get_previous_allocation
...
def execute(filters: Optional[Filters] = None) -> Tuple:
	if filters.to_date <= filters.from_date:
		finergy.throw(_('"From Date" can not be greater than or equal to "To Date"'))
...
def get_data(filters: Filters) -> List:
	leave_types = finergy.db.get_list("Leave Type", pluck="name", order_by="name")
	conditions = get_conditions(filters)

	user = finergy.session.user
	department_approver_map = get_department_leave_approver_map(filters.get("department"))

	active_employees = finergy.get_list(
		"Employee",
		filters=conditions,
	...
	for leave_type in leave_types:
		if len(active_employees) > 1:
			...
			row = finergy._dict({"leave_type": leave_type})
		...
		for employee in active_employees:
			...
			if (
				...
				or ("HR Manager" in finergy.get_roles(user))
			):
				if len(active_employees) > 1:
					row = finergy._dict()
...
def get_department_leave_approver_map(department: Optional[str] = None):
	...
	department_list = finergy.get_list(
		"Department",
		filters={"disabled": 0},
	...
	for key, group in groupby(employee_data, lambda x: x["employee_name"]):
		for grp in group:
			if grp.closing_balance:
				leaves.append(
					finergy._dict({"leave_type": grp.leave_type, "closing_balance": grp.closing_balance})
				)
