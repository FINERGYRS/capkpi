...
def execute(filters=None):
	...
	default_holiday_list = finergy.get_cached_value(
	...
	if filters.summarized_view:
		leave_types = finergy.get_all("Leave Type", pluck="name")
...
def add_data(
	...
	for emp in employee_map:
		...
		if filters.summarized_view:
			...
			time_default_counts = finergy.db.sql(
...
def get_attendance_list(conditions, filters):
	...
	for d in attendance_list:
		att_map.setdefault(d.employee, finergy._dict()).setdefault(d.day_of_month, "")
...
def get_employee_details(group_by, company):
	...
	query = """select name, employee_name, designation, department, branch, company,
		holiday_list from `tabEmployee` where company = %s """ % finergy.db.escape(
		company
	)
	...
	employee_details = finergy.db.sql(query, as_dict=1)
