...
import capkpi
from capkpi.hr.doctype.employee.employee import (
	InactiveEmployeeStatusError,
	get_holiday_list_for_employee,
)


class DuplicateDeclarationError(finergy.ValidationError):
	pass
...
class EmployeeBoardingController(Document):
	...
	def on_cancel(self):
		# delete task project
		for task in finergy.get_all("Task", filters={"project": self.project}):
			finergy.delete_doc("Task", task.name, force=1)
		finergy.delete_doc("Project", self.project, force=1)
		self.db_set("project", "")
		for activity in self.activities:
...
def set_employee_name(doc):
	if doc.employee and not doc.employee_name:
		doc.employee_name = finergy.db.get_value("Employee", doc.employee, "employee_name")
...
def update_employee_work_history(employee, details, date=None, cancel=False):
	...
	internal_work_history = {}
	for item in details:
		field = finergy.get_meta("Employee").get_field(item.fieldname)
		if not field:
			continue
...
def delete_employee_work_history(details, employee, date):
	...
	if filters:
		finergy.db.delete("Employee Internal Work History", filters)
		employee.save()
...
def get_employee_fields_label():
	fields = []
	for df in finergy.get_meta("Employee").get("fields"):
		if df.fieldname in [
			"salutation",
			"user_id",
			"employee_number",
...
def get_employee_field_property(employee, fieldname):
	if employee and fieldname:
		field = finergy.get_meta("Employee").get_field(fieldname)
		value = finergy.db.get_value("Employee", employee, fieldname)
...
def validate_dates(doc, from_date, to_date):
	date_of_joining, relieving_date = finergy.db.get_value(
		"Employee", doc.employee, ["date_of_joining", "relieving_date"]
	)
	if getdate(from_date) > getdate(to_date):
		finergy.throw(_("To date can not be less than from date"))
	elif getdate(from_date) > getdate(nowdate()):
		finergy.throw(_("Future dates not allowed"))
	elif date_of_joining and getdate(from_date) < getdate(date_of_joining):
		finergy.throw(_("From date can not be less than employee's joining date"))
	elif relieving_date and getdate(to_date) > getdate(relieving_date):
		finergy.throw(_("To date can not greater than employee's relieving date"))
...
def validate_duplicate_exemption_for_payroll_period(doctype, docname, payroll_period, employee):
	existing_record = finergy.db.exists(
		doctype,
		{
...
def validate_tax_declaration(declarations):
	...
	for d in declarations:
		if d.exemption_sub_category in subcategories:
			finergy.throw(_("More than one selection for {0} not allowed").format(d.exemption_sub_category))
		subcategories.append(d.exemption_sub_category)


def get_total_exemption_amount(declarations):
	exemptions = finergy._dict()
	for d in declarations:
		exemptions.setdefault(d.exemption_category, finergy._dict())
...
def generate_leave_encashment():
	...
	from capkpi.hr.doctype.leave_encashment.leave_encashment import create_leave_encashment

	if finergy.db.get_single_value("HR Settings", "auto_leave_encashment"):
		leave_type = finergy.get_all("Leave Type", filters={"allow_encashment": 1}, fields=["name"])
...
def allocate_earned_leaves():
	...
	for e_leave_type in e_leave_types:
		...
		for allocation in leave_allocations:
			...
			annual_allocation = finergy.db.get_value(
				"Leave Policy Detail",
			...
			if e_leave_type.based_on_date_of_joining:
				from_date = finergy.db.get_value("Employee", allocation.employee, "date_of_joining")
...
def update_previous_leave_allocation(allocation, annual_allocation, e_leave_type):
	...
	allocation = finergy.get_doc("Leave Allocation", allocation.name)
	...
	if new_allocation != allocation.total_leaves_allocated:
		...
		if e_leave_type.based_on_date_of_joining:
			text = _("allocated {0} leave(s) via scheduler on {1} based on the date of joining").format(
				finergy.bold(earned_leaves), finergy.bold(formatdate(today_date))
			)
		else:
...
def is_earned_leave_already_allocated(allocation, annual_allocation):
	...
	date_of_joining = finergy.db.get_value("Employee", allocation.employee, "date_of_joining")

	assignment = finergy.get_doc("Leave Policy Assignment", allocation.leave_policy_assignment)
...
def get_salary_assignments(employee, payroll_period):
	start_date, end_date = finergy.db.get_value(
...
def get_sal_slip_total_benefit_given(employee, payroll_period, component=False):
	...
	sum_of_given_benefit = finergy.db.sql(
		query,
		{
			"employee": employee,
...
def get_holidays_for_employee(
	...
	holidays = finergy.get_all("Holiday", fields=["description", "holiday_date"], filters=filters)
...
def get_previous_claimed_amount(employee, payroll_period, non_pro_rata=False, component=False):
	...
	sum_of_claimed_amount = finergy.db.sql(
		query,
		{
			"employee": employee,
			"start_date": payroll_period.start_date,
...
def share_doc_with_approver(doc, user):
	...
	if not finergy.has_permission(doc=doc, ptype="submit", user=user):
		finergy.share.add(doc.doctype, doc.name, user, submit=1, flags={"ignore_share_permission": True})

		finergy.msgprint(
			_("Shared with the user {0} with {1} access").format(user, finergy.bold("submit"), alert=True)
		)
	...
	if doc_before_save:
		...
		if doc_before_save.get(approver) != doc.get(approver):
			finergy.share.remove(doc.doctype, doc.name, doc_before_save.get(approver))


def validate_active_employee(employee):
	if finergy.db.get_value("Employee", employee, "status") == "Inactive":
		finergy.throw(
