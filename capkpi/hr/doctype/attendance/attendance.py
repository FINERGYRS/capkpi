...
from finergy.utils import cint, cstr, formatdate, get_datetime, getdate, nowdate
...
class Attendance(Document):
	...
	def validate_attendance_date(self):
		...
		if (
			...
			finergy.throw(_("Attendance can not be marked for future dates"))
		elif date_of_joining and getdate(self.attendance_date) < getdate(date_of_joining):
			finergy.throw(_("Attendance date can not be less than employee's joining date"))
		...
		if res:
			finergy.throw(
				_("Attendance for employee {0} is already marked for the date {1}").format(
					finergy.bold(self.employee), finergy.bold(self.attendance_date)
				)
			)

	def validate_employee_status(self):
		if finergy.db.get_value("Employee", self.employee, "status") == "Inactive":
			finergy.throw(_("Cannot mark attendance for an Inactive employee {0}").format(self.employee))
	...
	def validate_employee(self):
		...
		if not emp:
			finergy.throw(_("Employee {0} is not active or does not exist").format(self.employee))

	def unlink_attendance_from_checkins(self):
		from finergy.utils import get_link_to_form

		EmployeeCheckin = finergy.qb.DocType("Employee Checkin")
		linked_logs = (
			finergy.qb.from_(EmployeeCheckin)
		...
		if linked_logs:
			(
				finergy.qb.update(EmployeeCheckin)
				.set("attendance", "")
				.where(EmployeeCheckin.attendance == self.name)
			).run()
...
def get_events(start, end, filters=None):
	...
	employee = finergy.db.get_value("Employee", {"user_id": finergy.session.user})
...
def add_attendance(events, start, end, conditions=None):
	...
	for d in finergy.db.sql(query, {"from_date": start, "to_date": end}, as_dict=True):
		e = {
			"name": d.name,
			"doctype": "Attendance",
...
def mark_attendance(
	...
	if not finergy.db.exists(
		...
		company = finergy.db.get_value("Employee", employee, "company")
...
def mark_bulk_attendance(data):
	...
	company = finergy.get_value("Employee", data.employee, "company")
	if not data.unmarked_days:
		finergy.throw(_("Please select a date."))
		return
	...
	for date in data.unmarked_days:
		...
		attendance = finergy.get_doc(doc_dict).insert()
		attendance.submit()
