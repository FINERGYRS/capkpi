...
class TestLeavePolicy(unittest.TestCase):
	def test_max_leave_allowed(self):
		random_leave_type = finergy.get_all("Leave Type", fields=["name", "max_leaves_allowed"])
		if random_leave_type:
			random_leave_type = random_leave_type[0]
			leave_type = finergy.get_doc("Leave Type", random_leave_type.name)
		...
		self.assertRaises(finergy.ValidationError, leave_policy.insert)
