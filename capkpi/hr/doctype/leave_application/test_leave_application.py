...
import finergy
from finergy.permissions import clear_user_permissions_for_doctype
...
from capkpi.hr.doctype.holiday_list.test_holiday_list import set_holiday_list
...
from capkpi.hr.doctype.leave_policy_assignment.leave_policy_assignment import (
	create_assignment_for_multiple_employees,
)
from capkpi.hr.doctype.leave_type.test_leave_type import create_leave_type
from capkpi.payroll.doctype.salary_slip.test_salary_slip import (
	make_holiday_list,
...
class TestLeaveApplication(unittest.TestCase):
	def setUp(self):
		...
		finergy.db.delete("Attendance", {"employee": "_T-Employee-00001"})
		finergy.db.set_value("Employee", "_T-Employee-00001", "holiday_list", "")

		from_date = get_year_start(getdate())
		to_date = get_year_ending(getdate())
		self.holiday_list = make_holiday_list(from_date=from_date, to_date=to_date)

		if not finergy.db.exists("Leave Type", "_Test Leave Type"):
			finergy.get_doc(
	...
	def _clear_applications(self):
		finergy.db.sql("""delete from `tabLeave Application`""")

	def get_application(self, doc):
		application = finergy.copy_doc(doc)
	...
	def test_validate_application_across_allocations(self):
		...
		finergy.delete_doc_if_exists("Leave Type", "Test Leave Validation", force=1)
		leave_type = finergy.get_doc(
		...
		self.assertRaises(finergy.ValidationError, leave_application.insert)
	...
	def test_insufficient_leave_balance_validation(self):
		...
		finergy.db.set_value("Leave Type", "Test Leave Validation", "allow_negative", True)
		make_leave_application(
	...
	def test_attendance_for_include_holidays(self):
		...
		finergy.delete_doc_if_exists("Leave Type", "Test Include Holidays", force=1)
		leave_type = finergy.get_doc(
		...
		self.assertEqual(finergy.db.count("Attendance", {"leave_application": leave_application.name}), 4)
	...
	def test_attendance_update_for_exclude_holidays(self):
		...
		finergy.delete_doc_if_exists("Leave Type", "Test Do Not Include Holidays", force=1)
		leave_type = finergy.get_doc(
			dict(
		...
		attendance_on_holiday = finergy.get_doc(config)
		...
		attendance = finergy.get_doc(config)
		...
		self.assertEqual(finergy.db.count("Attendance", {"leave_application": leave_application.name}), 3)

		# attendance on holiday deleted
		self.assertFalse(finergy.db.exists("Attendance", attendance_on_holiday.name))

		# attendance on non-holiday updated
		self.assertEqual(finergy.db.get_value("Attendance", attendance.name, "status"), "On Leave")
	...
	def test_optional_leave(self):
		...
		if not finergy.db.exists("Holiday List", holiday_list):
			finergy.get_doc(
				dict(
		...
		finergy.db.set_value("Leave Period", leave_period.name, "optional_holiday_list", holiday_list)
		leave_type = "Test Optional Type"
		if not finergy.db.exists("Leave Type", leave_type):
			finergy.get_doc(
	...
	def test_leaves_allowed(self):
		employee = get_employee()
		leave_period = get_leave_period()
		finergy.delete_doc_if_exists("Leave Type", "Test Leave Type", force=1)
		leave_type = finergy.get_doc(
	...
	def test_applicable_after(self):
		...
		date = add_days(nowdate(), -7)
		finergy.db.set_value("Employee", employee.name, "date_of_joining", date)
		...
		finergy.delete_doc_if_exists("Leave Type", "Test Leave Type 1", force=1)
		leave_type_1 = finergy.get_doc(
		...
		finergy.db.set_value("Employee", employee.name, "date_of_joining", "2010-01-01")
	...
	def test_earned_leaves_creation(self):
		from capkpi.hr.utils import allocate_earned_leaves
		...
		finergy.db.set_value("Leave Type", leave_type, "max_leaves_allowed", 0)
	...
	def test_current_leave_on_submit(self):
		...
		if not finergy.db.exists("Leave Type", leave_type):
			finergy.get_doc(dict(leave_type_name=leave_type, doctype="Leave Type")).insert()

		allocation = finergy.get_doc(
			dict(
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		leave_application.submit()
		leave_ledger_entry = finergy.get_all(
		...
		leave_application.cancel()
		self.assertFalse(
			finergy.db.exists("Leave Ledger Entry", {"transaction_name": leave_application.name})
		)
	...
	def test_leave_approver_perms(self):
		...
		employee.save()
		self.assertTrue("Leave Approver" in finergy.get_roles(user))
		...
		application.insert()
		self.assertTrue(application.name in finergy.share.get_shared("Leave Application", user))

		# check shared doc revoked
		application.reload()
		application.leave_approver = "test@example.com"
		application.save()
		self.assertTrue(application.name not in finergy.share.get_shared("Leave Application", user))
	...
	def test_get_earned_leave_details_for_dashboard(self):
		...
		allocation = finergy.db.get_value(
			"Leave Allocation",
			{"leave_policy_assignment": leave_policy_assignments[0]},
			"name",
		)
		allocation = finergy.get_doc("Leave Allocation", allocation)
...
def get_employee():
	return finergy.get_doc("Employee", "_T-Employee-00001")


def set_leave_approver():
	employee = get_employee()
	dept_doc = finergy.get_doc("Department", employee.department)
	dept_doc.append("leave_approvers", {"approver": "test@example.com"})
	dept_doc.save(ignore_permissions=True)


def get_leave_period():
	leave_period_name = finergy.db.exists({"doctype": "Leave Period", "company": "_Test Company"})
	if leave_period_name:
		return finergy.get_doc("Leave Period", leave_period_name[0][0])
	else:
...
def allocate_leaves(employee, leave_period, leave_type, new_leaves_allocated, eligible_leaves=0):
	allocate_leave = finergy.get_doc(
		{
			"doctype": "Leave Allocation",
			"__islocal": 1,
			"employee": employee.name,
...
def make_policy_assignment(employee, leave_type, leave_period):
	finergy.delete_doc_if_exists("Leave Type", leave_type, force=1)
	finergy.get_doc(
		dict(
	...
	leave_policy_assignments = create_assignment_for_multiple_employees(
		[employee.name], finergy._dict(data)
	)
