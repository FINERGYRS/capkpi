...
from finergy.query_builder.functions import Max, Min, Sum
...
from capkpi.buying.doctype.supplier_scorecard.supplier_scorecard import daterange
from capkpi.hr.doctype.employee.employee import get_holiday_list_for_employee
from capkpi.hr.doctype.leave_block_list.leave_block_list import get_applicable_block_dates
from capkpi.hr.doctype.leave_ledger_entry.leave_ledger_entry import create_leave_ledger_entry
from capkpi.hr.utils import (
...
class LeaveDayBlockedError(finergy.ValidationError):
	pass


class OverlapError(finergy.ValidationError):
	pass


class AttendanceAlreadyMarkedError(finergy.ValidationError):
	pass


class NotAnOptionalHoliday(finergy.ValidationError):
	pass


class InsufficientLeaveBalanceError(finergy.ValidationError):
	pass


class LeaveAcrossAllocationsError(finergy.ValidationError):
	pass
...
class LeaveApplication(Document):
	...
	def validate(self):
		...
		if finergy.db.get_value("Leave Type", self.leave_type, "is_optional_leave"):
	...
	def on_update(self):
		if self.status == "Open" and self.docstatus < 1:
			...
			if finergy.db.get_single_value("HR Settings", "send_leave_notification"):
	...
	def validate_applicable_after(self):
		if self.leave_type:
			leave_type = finergy.get_doc("Leave Type", self.leave_type)
			if leave_type.applicable_after > 0:
				date_of_joining = finergy.db.get_value("Employee", self.employee, "date_of_joining")
				...
				if number_of_days >= 0:
					holidays = 0
					if not finergy.db.get_value("Leave Type", self.leave_type, "include_holiday"):
	...
	def validate_dates(self):
		if finergy.db.get_single_value("HR Settings", "restrict_backdated_leave_application"):
			if self.from_date and getdate(self.from_date) < getdate():
				allowed_role = finergy.db.get_single_value(
				...
				if not allowed_role:
					finergy.throw(
						_("Backdated Leave Application is restricted. Please set the {} in {}").format(
							finergy.bold("Role Allowed to Create Backdated Leave Application"),
		...
		if (
			...
			finergy.throw(_("Half Day Date should be between From Date and To Date"))
	...
	def validate_dates_across_allocation(self):
		if finergy.db.get_value("Leave Type", self.leave_type, "allow_negative"):
			return
		...
		if not (alloc_on_from_date or alloc_on_to_date):
			finergy.throw(_("Application period cannot be outside leave allocation period"))
	...
	def get_allocation_based_on_application_dates(self) -> Tuple[Dict, Dict]:
		...
		def _get_leave_allocation_record(date):
			LeaveAllocation = finergy.qb.DocType("Leave Allocation")
			allocation = (
				finergy.qb.from_(LeaveAllocation)
	...
	def validate_back_dated_application(self):
		future_allocation = finergy.db.sql(
	...
	def update_attendance(self):
		...
		for dt in daterange(getdate(self.from_date), getdate(self.to_date)):
			...
			attendance_name = finergy.db.exists(
			...
			if date in holiday_dates:
				if attendance_name:
					...
					attendance = finergy.get_doc("Attendance", attendance_name)
					attendance.flags.ignore_permissions = True
					if attendance.docstatus == 1:
						attendance.cancel()
					finergy.delete_doc("Attendance", attendance_name, force=1)
				continue
	...
	def cancel_attendance(self):
		if self.docstatus == 2:
			...
			for name in attendance:
				finergy.db.set_value("Attendance", name, "docstatus", 2)

	def validate_salary_processed_days(self):
		if not finergy.db.get_value("Leave Type", self.leave_type, "is_lwp"):
			return

		last_processed_pay_slip = finergy.db.sql(
			"""
	...
	def show_block_day_warning(self):
		...
		if block_dates:
			finergy.msgprint(_("Warning: Leave application contains following block dates") + ":")
			for d in block_dates:
				finergy.msgprint(formatdate(d.block_date) + ": " + d.reason)
	...
	def validate_block_days(self):
		...
		if block_dates and self.status == "Approved":
			finergy.throw(_("You are not authorized to approve leaves on Block Dates"), LeaveDayBlockedError)
	...
	def show_insufficient_balance_message(self, leave_balance_for_consumption: float) -> None:
		...
		if finergy.db.get_value("Leave Type", self.leave_type, "allow_negative"):
			...
			finergy.msgprint(msg, title=_("Warning"), indicator="orange")
		else:
			finergy.throw(
				_("Insufficient leave balance for Leave Type {0}").format(finergy.bold(self.leave_type)),
	...
	def throw_overlap_error(self, d):
		...
		finergy.throw(msg, OverlapError)

	def get_total_leaves_on_half_day(self):
		leave_count_on_half_day_date = finergy.db.sql(
	...
	def validate_max_days(self):
		max_days = finergy.db.get_value("Leave Type", self.leave_type, "max_continuous_days_allowed")
		if max_days and self.total_leave_days > cint(max_days):
			finergy.throw(_("Leave of type {0} cannot be longer than {1}").format(self.leave_type, max_days))
	...
	def validate_optional_leave(self):
		...
		if not leave_period:
			finergy.throw(_("Cannot find active Leave Period"))
		optional_holiday_list = finergy.db.get_value(
	...
	def notify_employee(self):
		employee = finergy.get_doc("Employee", self.employee)
		if not employee.user_id:
			return

		parent_doc = finergy.get_doc("Leave Application", self.name)
		args = parent_doc.as_dict()

		template = finergy.db.get_single_value("HR Settings", "leave_status_notification_template")
		if not template:
			finergy.msgprint(_("Please set default template for Leave Status Notification in HR Settings."))
			return
		email_template = finergy.get_doc("Email Template", template)
		message = finergy.render_template(email_template.response, args)
	...
	def notify_leave_approver(self):
		if self.leave_approver:
			...
			template = finergy.db.get_single_value("HR Settings", "leave_approval_notification_template")
			if not template:
				finergy.msgprint(
		...
		if cint(self.follow_via_email):
			...
			if not isinstance(contact, list):
				if not args.notify == "employee":
					contact = finergy.get_doc("User", contact).email or contact

			sender = dict()
			sender["email"] = finergy.get_doc("User", finergy.session.user).email
			...
			try:
				...
				finergy.msgprint(_("Email sent to {0}").format(contact))
	...
	def create_leave_ledger_entry(self, submit=True):
		...
		lwp = finergy.db.get_value("Leave Type", self.leave_type, "is_lwp")
		...
		if expiry_date:
			...
			if self.is_separate_ledger_entry_required(alloc_on_from_date, alloc_on_to_date):
				...
				raise_exception = False if finergy.flags.in_patch else True
				args = dict(
...
def get_number_of_leave_days(
	...
	if not finergy.db.get_value("Leave Type", leave_type, "include_holiday"):
...
def get_leave_details(employee, date):
	...
	for d in allocation_records:
		allocation = allocation_records.get(d, finergy._dict())
	...
	lwp = finergy.get_list("Leave Type", filters={"is_lwp": 1}, pluck="name")
...
def get_leave_balance_on(
	...
	allocation = allocation_records.get(leave_type, finergy._dict())
...
def get_leave_allocation_records(employee, date, leave_type=None):
	...
	Ledger = finergy.qb.DocType("Leave Ledger Entry")

	cf_leave_case = (
		finergy.qb.terms.Case().when(Ledger.is_carry_forward == "1", Ledger.leaves).else_(0)
	)
	sum_cf_leaves = Sum(cf_leave_case).as_("cf_leaves")

	new_leaves_case = (
		finergy.qb.terms.Case().when(Ledger.is_carry_forward == "0", Ledger.leaves).else_(0)
	)
	sum_new_leaves = Sum(new_leaves_case).as_("new_leaves")

	query = (
		finergy.qb.from_(Ledger)
		.select(
			sum_cf_leaves,
			sum_new_leaves,
	...
	allocated_leaves = finergy._dict()
...
def get_remaining_leaves(
	...
	return finergy._dict(leave_balance=leave_balance, leave_balance_for_consumption=remaining_leaves)
	...
	for leave_entry in leave_entries:
		...
		if inclusive_period and leave_entry.transaction_type == "Leave Encashment":
			...
			if leave_entry.leaves % 1:
				half_day = 1
				half_day_date = finergy.db.get_value(
...
def is_lwp(leave_type):
	lwp = finergy.db.sql("select is_lwp from `tabLeave Type` where name = %s", leave_type)
...
def get_events(start, end, filters=None):
	...
	employee = finergy.db.get_value(
		"Employee", filters={"user_id": finergy.session.user}, fieldname=["name", "company"], as_dict=True
	)
	...
	if "Employee" in finergy.get_roles():
...
def add_department_leaves(events, start, end, employee, company):
	department = finergy.db.get_value("Employee", employee, "department")

	if not department:
		return

	# department leaves
	department_employees = finergy.db.sql_list(
...
def add_leaves(events, start, end, filter_conditions=None):
	...
	if not cint(
		finergy.db.get_value("HR Settings", None, "show_leaves_of_all_department_members_in_calendar")
	):
	...
	for d in finergy.db.sql(query, {"start": start, "end": end}, as_dict=True):
		e = {
			"name": d.name,
...
def get_mandatory_approval(doctype):
	...
	if doctype == "Leave Application":
		mandatory = finergy.db.get_single_value(
...
def get_leave_approver(employee):
	leave_approver, department = finergy.db.get_value(
		"Employee", employee, ["leave_approver", "department"]
	)

	if not leave_approver and department:
		leave_approver = finergy.db.get_value(
			"Department Approver",
