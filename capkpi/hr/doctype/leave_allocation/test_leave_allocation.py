...
from finergy.utils import add_days, add_months, getdate, nowdate

import capkpi
from capkpi.hr.doctype.employee.test_employee import make_employee
from capkpi.hr.doctype.leave_allocation.leave_allocation import (
	BackDatedAllocationError,
	OverAllocationError,
)
from capkpi.hr.doctype.leave_ledger_entry.leave_ledger_entry import process_expired_allocation
from capkpi.hr.doctype.leave_type.test_leave_type import create_leave_type


class TestLeaveAllocation(FinergyTestCase):
	def setUp(self):
		...
		self.employee = finergy.get_doc("Employee", emp_id)
	...
	def test_overlapping_allocation(self):
		...
		finergy.get_doc(leaves[0]).save()
		self.assertRaises(finergy.ValidationError, finergy.get_doc(leaves[1]).save)
	...
	def test_validation_for_over_allocation_based_on_leave_setup(self):
		finergy.delete_doc_if_exists("Leave Period", "Test Allocation Period")
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		self.assertFalse(
			finergy.db.exists("Leave Ledger Entry", {"transaction_name": leave_allocation.name})
		)
	...
	def test_validation_against_leave_application_after_submit(self):
		...
		self.assertRaises(finergy.ValidationError, leave_allocation.submit)
...
def create_leave_allocation(**args):
	...
	employee = finergy.get_doc("Employee", emp_id)
