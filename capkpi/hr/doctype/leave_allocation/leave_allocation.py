...
from finergy.utils import add_days, date_diff, flt, formatdate, getdate

from capkpi.hr.doctype.leave_application.leave_application import get_approved_leaves_for_period
from capkpi.hr.doctype.leave_ledger_entry.leave_ledger_entry import (
	create_leave_ledger_entry,
	expire_allocation,
)
from capkpi.hr.utils import get_leave_period, set_employee_name
...
class BackDatedAllocationError(finergy.ValidationError):
	pass


class OverAllocationError(finergy.ValidationError):
	pass


class LessAllocationError(finergy.ValidationError):
	pass


class ValueMultiplierError(finergy.ValidationError):
	pass
...
class LeaveAllocation(Document):
	...
	def validate_leave_allocation_days(self):
		...
		max_leaves_allowed = flt(
			finergy.db.get_value("Leave Type", self.leave_type, "max_leaves_allowed")
		)
	...
	def get_existing_leave_count(self):
		ledger_entries = finergy.get_all(
			"Leave Ledger Entry",
			filters={
	...
	def update_leave_policy_assignments_when_no_allocations_left(self):
		allocations = finergy.db.get_list(
			"Leave Allocation",
	...
	def set_total_leaves_allocated(self):
		...
		if (
			...
			and not finergy.db.get_value("Leave Type", self.leave_type, "is_earned_leave")
			and not finergy.db.get_value("Leave Type", self.leave_type, "is_compensatory")
		):
			finergy.throw(
	...
	def limit_carry_forward_based_on_max_allowed_leaves(self):
		max_leaves_allowed = finergy.db.get_value("Leave Type", self.leave_type, "max_leaves_allowed")
	...
	def validate_total_leaves_allocated(self):
		...
		if date_difference < self.total_leaves_allocated:
			if finergy.db.get_value("Leave Type", self.leave_type, "allow_over_allocation"):
				finergy.msgprint(
	...
	def create_leave_ledger_entry(self, submit=True):
		if self.unused_leaves:
			expiry_days = finergy.db.get_value(
...
def get_leave_allocation_for_period(
	...
	Allocation = finergy.qb.DocType("Leave Allocation")
	return (
		finergy.qb.from_(Allocation)
...
def get_carry_forwarded_leaves(employee, leave_type, date, carry_forward=None):
	...
	if carry_forward and previous_allocation:
		...
		if unused_leaves:
			max_carry_forwarded_leaves = finergy.db.get_value(
...
def validate_carry_forward(leave_type):
	if not finergy.db.get_value("Leave Type", leave_type, "is_carry_forward"):
		finergy.throw(_("Leave Type {0} cannot be carry-forwarded").format(leave_type))
