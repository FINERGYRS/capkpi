...
from finergy.utils import getdate, nowdate

from capkpi.hr.doctype.leave_application.leave_application import get_leaves_for_period
...
class LeaveEncashment(Document):
	...
	def before_submit(self):
		if self.encashment_amount <= 0:
			finergy.throw(_("You can only submit Leave Encashment for a valid encashment amount"))
	...
	def on_submit(self):
		...
		additional_salary.company = finergy.get_value("Employee", self.employee, "company")
		additional_salary.employee = self.employee
		additional_salary.currency = self.currency
		earning_component = finergy.get_value("Leave Type", self.leave_type, "earning_component")
		if not earning_component:
			finergy.throw(_("Please set Earning Component for Leave type: {0}.").format(self.leave_type))
		...
		finergy.db.set_value(
			...
			finergy.db.get_value("Leave Allocation", self.leave_allocation, "total_leaves_encashed")
	...
	def get_leave_details_for_encashment(self):
		...
		if not finergy.db.get_value("Leave Type", self.leave_type, "allow_encashment"):
			finergy.throw(_("Leave Type {0} is not encashable").format(self.leave_type))
		...
		encashable_days = self.leave_balance - finergy.db.get_value(
			"Leave Type", self.leave_type, "encashment_threshold_days"
		)
		self.encashable_days = encashable_days if encashable_days > 0 else 0

		per_day_encashment = finergy.db.get_value(
...
def create_leave_encashment(leave_allocation):
	...
	for allocation in leave_allocation:
		if not get_assigned_salary_structure(allocation.employee, allocation.to_date):
			continue
		leave_encashment = finergy.get_doc(
			dict(
