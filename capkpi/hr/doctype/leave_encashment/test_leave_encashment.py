...
from finergy.utils import add_days, get_year_ending, get_year_start, getdate
...
from capkpi.hr.doctype.leave_period.test_leave_period import create_leave_period
...
test_records = finergy.get_test_records("Leave Type")


class TestLeaveEncashment(FinergyTestCase):
	def setUp(self):
		finergy.db.delete("Leave Period")
		finergy.db.delete("Leave Policy Assignment")
		finergy.db.delete("Leave Allocation")
		finergy.db.delete("Leave Ledger Entry")
		finergy.db.delete("Additional Salary")
		finergy.db.delete("Leave Encashment")

		if not finergy.db.exists("Leave Type", "_Test Leave Type Encashment"):
			finergy.get_doc(test_records[2]).insert()
		...
		leave_policy_assignments = create_assignment_for_multiple_employees(
			[self.employee], finergy._dict(data)
		)
	...
	def test_leave_balance_value_and_amount(self):
		...
		# assert links
		add_sal = finergy.get_all("Additional Salary", filters={"ref_docname": leave_encashment.name})[0]
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		self.assertFalse(
			finergy.db.exists("Leave Ledger Entry", {"transaction_name": leave_encashment.name})
		)
