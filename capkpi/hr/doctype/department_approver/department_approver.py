...
def get_approvers(doctype, txt, searchfield, start, page_len, filters):
	...
	if not filters.get("employee"):
		finergy.throw(_("Please select Employee first."))

	approvers = []
	department_details = {}
	department_list = []
	employee = finergy.get_value(
		"Employee",
	...
	if employee_department:
		department_details = finergy.db.get_value(
			"Department", {"name": employee_department}, ["lft", "rgt"], as_dict=True
		)
	if department_details:
		department_list = finergy.db.sql(
	...
	if filters.get("doctype") == "Leave Application" and employee.leave_approver:
		approvers.append(
			finergy.db.get_value("User", employee.leave_approver, ["name", "first_name", "last_name"])
		)

	if filters.get("doctype") == "Expense Claim" and employee.expense_approver:
		approvers.append(
			finergy.db.get_value("User", employee.expense_approver, ["name", "first_name", "last_name"])
		)
	...
	if len(approvers) == 0:
		error_msg = _("Please set {0} for the Employee: {1}").format(
			field_name, finergy.bold(employee.employee_name)
		)
		if department_list:
			error_msg += _(" or for Department: {0}").format(finergy.bold(employee_department))
		finergy.throw(error_msg, title=_(field_name + " Missing"))
