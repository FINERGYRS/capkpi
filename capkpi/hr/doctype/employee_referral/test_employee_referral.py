...
from capkpi.hr.doctype.employee_referral.employee_referral import (
...
class TestEmployeeReferral(unittest.TestCase):
	def setUp(self):
		finergy.db.sql("DELETE FROM `tabJob Applicant`")
		finergy.db.sql("DELETE FROM `tabEmployee Referral`")
...
def create_employee_referral():
	emp_ref = finergy.new_doc("Employee Referral")
