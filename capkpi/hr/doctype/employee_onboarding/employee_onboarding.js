...
finergy.ui.form.on('Employee Onboarding', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		if (frm.doc.employee) {
			frm.add_custom_button(__('Employee'), function() {
				finergy.set_route("Form", "Employee", frm.doc.employee);
			},__("View"));
		}
		if (frm.doc.project) {
			frm.add_custom_button(__('Project'), function() {
				finergy.set_route("Form", "Project", frm.doc.project);
			},__("View"));
			frm.add_custom_button(__('Task'), function() {
				finergy.set_route('List', 'Task', {project: frm.doc.project});
			},__("View"));
		}
		if ((!frm.doc.employee) && (frm.doc.docstatus === 1)) {
			frm.add_custom_button(__('Employee'), function () {
				finergy.model.open_mapped_doc({
					method: "capkpi.hr.doctype.employee_onboarding.employee_onboarding.make_employee",
					frm: frm
				});
			}, __('Create'));
		...
		if (frm.doc.docstatus === 1 && frm.doc.project) {
			finergy.call({
				method: "capkpi.hr.utils.get_boarding_status",
				args: {
	...
	employee_onboarding_template: function(frm) {
		...
		if (frm.doc.employee_onboarding_template) {
			finergy.call({
				method: "capkpi.hr.utils.get_onboarding_details",
				args: {
