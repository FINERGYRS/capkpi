...
from finergy.utils import global_date_format
...
class DailyWorkSummary(Document):
	...
	def get_message_details(self):
		...
		dws_group = finergy.get_doc("Daily Work Summary Group", self.daily_work_summary_group)
		...
		for d in replies:
			...
			try:
				if original_image:
					file_name = finergy.get_list("File", {"file_url": original_image})
		...
		did_not_reply = [
			(finergy.db.get_value("User", {"email": email}, "full_name") or email) for email in did_not_reply
		]
...
def get_user_emails_from_group(group):
	...
	if isinstance(group_doc, string_types):
		group_doc = finergy.get_doc("Daily Work Summary Group", group)
