...
# test_records = finergy.get_test_records('Daily Work Summary')
...
class TestDailyWorkSummary(unittest.TestCase):
	...
	def test_email_trigger_failed(self):
		hour = "00:00"
		if finergy.utils.nowtime().split(":")[0] == "00":
			hour = "01:00"
	...
	def test_incoming(self):
		...
		daily_work_summary = finergy.get_doc(
			"Daily Work Summary", finergy.get_all("Daily Work Summary")[0].name
		)
	...
	def setup_and_prepare_test(self, hour=None):
		finergy.db.sql("delete from `tabDaily Work Summary`")
		finergy.db.sql("delete from `tabEmail Queue`")
		finergy.db.sql("delete from `tabEmail Queue Recipient`")
		finergy.db.sql("delete from `tabCommunication`")
		finergy.db.sql("delete from `tabDaily Work Summary Group`")

		self.users = finergy.get_all(
			"User", fields=["email"], filters=dict(email=("!=", "test@example.com"))
		)
		self.setup_groups(hour)

		from capkpi.hr.doctype.daily_work_summary_group.daily_work_summary_group import trigger_emails

		trigger_emails()

		# check if emails are created

		self.emails = finergy.db.sql(
	...
	def setup_groups(self, hour=None):
		...
		if not hour:
			hour = finergy.utils.nowtime().split(":")[0]
			hour = hour + ":00"

		groups = finergy.get_doc(
			dict(
