...
import finergy
from finergy import _, scrub, throw
...
from finergy.utils import add_days, add_years, cstr, getdate, today, validate_email_address
from finergy.utils.nestedset import NestedSet

from capkpi.utilities.transaction_base import delete_events


class EmployeeUserDisabledError(finergy.ValidationError):
	pass


class InactiveEmployeeStatusError(finergy.ValidationError):
	pass
...
class Employee(NestedSet):
	...
	def autoname(self):
		naming_method = finergy.db.get_value("HR Settings", None, "emp_created_by")
	...
	def validate(self):
		from capkpi.controllers.status_updater import validate_status
		...
		if self.user_id:
			...
			existing_user_id = finergy.db.get_value("Employee", self.name, "user_id")
			if existing_user_id:
	...
	def validate_user_details(self):
		if self.user_id:
			data = finergy.db.get_value("User", self.user_id, ["enabled", "user_image"], as_dict=1)
	...
	def update_user_permissions(self):
		...
		employee_user_permission_exists = finergy.db.exists(
	...
	def update_user(self):
		...
		user = finergy.get_doc("User", self.user_id)
	...
	def update_approver_role(self):
		if self.leave_approver:
			user = finergy.get_doc("User", self.leave_approver)
			user.flags.ignore_permissions = True
			user.add_roles("Leave Approver")

		if self.expense_approver:
			user = finergy.get_doc("User", self.expense_approver)
			user.flags.ignore_permissions = True
	...
	def set_preferred_email(self):
		preferred_email_field = finergy.scrub(self.prefered_contact_email)
	...
	def validate_status(self):
		if self.status == "Left":
			...
			if reports_to:
				link_to_employees = [
					finergy.utils.get_link_to_form("Employee", employee.name, label=employee.employee_name)
	...
	def validate_for_enabled_user_id(self, enabled):
		...
		if enabled is None:
			finergy.throw(_("User {0} does not exist").format(self.user_id))
		if enabled == 0:
			finergy.throw(_("User {0} is disabled").format(self.user_id), EmployeeUserDisabledError)

	def validate_duplicate_user_id(self):
		employee = finergy.db.sql_list(
	...
	def on_trash(self):
		...
		if finergy.db.exists("Employee Transfer", {"new_employee_id": self.name, "docstatus": 1}):
			emp_transfer = finergy.get_doc(
	...
	def validate_preferred_email(self):
		if self.prefered_contact_email and not self.get(scrub(self.prefered_contact_email)):
			finergy.msgprint(_("Please enter {0}").format(self.prefered_contact_email))

	def validate_onboarding_process(self):
		employee_onboarding = finergy.get_all(
			"Employee Onboarding",
			filters={
		...
		if employee_onboarding:
			doc = finergy.get_doc("Employee Onboarding", employee_onboarding[0].name)
	...
	def reset_employee_emails_cache(self):
		...
		if cell_number != prev_number or self.get("user_id") != prev_doc.get("user_id"):
			finergy.cache().hdel("employees_with_number", cell_number)
			finergy.cache().hdel("employees_with_number", prev_number)
...
def get_retirement_date(date_of_birth=None):
	...
	if date_of_birth:
		try:
			retirement_age = int(finergy.db.get_single_value("HR Settings", "retirement_age") or 60)
...
def validate_employee_role(doc, method):
	...
	if "Employee" in [d.role for d in doc.get("roles")]:
		if not finergy.db.get_value("Employee", {"user_id": doc.name}):
			finergy.msgprint(_("Please set User ID field in an Employee record to set Employee Role"))
...
def update_user_permissions(doc, method):
	...
	if "Employee" in [d.role for d in doc.get("roles")]:
		if not has_permission("User Permission", ptype="write", raise_exception=False):
			return
		employee = finergy.get_doc("Employee", {"user_id": doc.name})
...
def get_holiday_list_for_employee(employee, raise_exception=True):
	if employee:
		holiday_list, company = finergy.get_cached_value(
...
def is_holiday(
	...
	holidays = finergy.get_all("Holiday", fields=["description"], filters=filters, pluck="description")
...
def deactivate_sales_person(status=None, employee=None):
	if status == "Left":
		sales_person = finergy.db.get_value("Sales Person", {"Employee": employee})
		if sales_person:
			finergy.db.set_value("Sales Person", sales_person, "enabled", 0)


@finergy.whitelist()
def create_user(employee, user=None, email=None):
	emp = finergy.get_doc("Employee", employee)
...
def get_all_employee_emails(company):
	...
	for employee in employee_list:
		if not employee:
			continue
		user, company_email, personal_email = finergy.db.get_value(
...
def get_children(doctype, parent=None, company=None, is_root=False, is_tree=False):
	...
	employees = finergy.get_list(doctype, fields=fields, filters=filters, order_by="name")

	for employee in employees:
		is_expandable = finergy.get_all(doctype, filters=[["reports_to", "=", employee.get("value")]])
...
def on_doctype_update():
	finergy.db.add_index("Employee", ["lft", "rgt"])
