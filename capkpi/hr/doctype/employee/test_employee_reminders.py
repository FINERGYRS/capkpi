...
from capkpi.hr.doctype.employee.employee_reminders import send_holidays_reminder_in_advance
from capkpi.hr.doctype.employee.test_employee import make_employee
from capkpi.hr.doctype.hr_settings.hr_settings import set_proceed_with_frequency_change
from capkpi.hr.utils import get_holidays_for_employee


class TestEmployeeReminders(unittest.TestCase):
	@classmethod
	def setUpClass(cls):
		from capkpi.hr.doctype.holiday_list.test_holiday_list import make_holiday_list
		...
		test_employee_2 = finergy.get_doc("Employee", test_employee_2)
	...
	def test_birthday_reminders(self):
		employee = finergy.get_doc(
			"Employee", finergy.db.sql_list("select name from tabEmployee limit 1")[0]
		)
		employee.date_of_birth = "1992" + finergy.utils.nowdate()[4:]
		employee.company_email = "test@example.com"
		employee.company = "_Test Company"
		employee.save()

		from capkpi.hr.doctype.employee.employee_reminders import (
		...
		hr_settings = finergy.get_doc("HR Settings", "HR Settings")
		hr_settings.send_birthday_reminders = 1
		hr_settings.save()

		send_birthday_reminders()

		email_queue = finergy.db.sql("""select * from `tabEmail Queue`""", as_dict=True)
		self.assertTrue("Subject: Birthday Reminder" in email_queue[0].message)

	def test_work_anniversary_reminders(self):
		make_employee(
			"test_work_anniversary@gmail.com",
			date_of_joining="1998" + finergy.utils.nowdate()[4:],
	...
	def test_work_anniversary_reminder_not_sent_for_0_years(self):
		...
		from capkpi.hr.doctype.employee.employee_reminders import get_employees_having_an_event_today
	...
	def test_advance_holiday_reminders_monthly(self):
		from capkpi.hr.doctype.employee.employee_reminders import send_reminders_in_advance_monthly
		...
		recipients = finergy.db.get_all("Email Queue Recipient", pluck="recipient")
	...
	def test_advance_holiday_reminders_weekly(self):
		from capkpi.hr.doctype.employee.employee_reminders import send_reminders_in_advance_weekly
