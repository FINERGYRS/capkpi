...
class TestEmployeeTransfer(unittest.TestCase):
	...
	def test_submit_before_transfer_date(self):
		...
		transfer_obj = finergy.get_doc(
			{
				"doctype": "Employee Transfer",
				"employee": finergy.get_value("Employee", {"user_id": "employee2@transfers.com"}, "name"),
				"transfer_details": [
					{
		...
		transfer_obj.save()
		self.assertRaises(finergy.DocstatusTransitionError, transfer_obj.submit)
		transfer = finergy.get_doc("Employee Transfer", transfer_obj.name)
	...
	def test_new_employee_creation(self):
		...
		transfer = finergy.get_doc(
			{
				"doctype": "Employee Transfer",
				"employee": finergy.get_value("Employee", {"user_id": "employee3@transfers.com"}, "name"),
		...
		self.assertEqual(finergy.get_value("Employee", transfer.new_employee_id, "status"), "Active")
		self.assertEqual(finergy.get_value("Employee", transfer.employee, "status"), "Left")
...
def create_company():
	if not finergy.db.exists("Company", "Test Company"):
		finergy.get_doc(
			{
				"doctype": "Company",
