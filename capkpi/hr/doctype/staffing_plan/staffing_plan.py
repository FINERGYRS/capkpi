...
class SubsidiaryCompanyError(finergy.ValidationError):
	pass


class ParentCompanyError(finergy.ValidationError):
	pass
...
class StaffingPlan(Document):
	...
	def validate_with_parent_plan(self, staffing_plan_detail):
		if not finergy.get_cached_value("Company", self.company, "parent_company"):
		...
		parent_company = finergy.db.get_value("Staffing Plan", parent_plan_details[0].name, "company")
		...
		if cint(staffing_plan_detail.vacancies) > cint(parent_plan_details[0].vacancies) or flt(
			...
			finergy.throw(
				...
				for {2} as per staffing plan {3} for parent company {4}."
					...
					finergy.bold(staffing_plan_detail.designation),
		...
		lft, rgt = finergy.get_cached_value("Company", parent_company, ["lft", "rgt"])
		all_sibling_details = finergy.db.sql(
	...
	def validate_with_subsidiary_plans(self, staffing_plan_detail):
		...
		children_details = finergy.db.sql(
			"""select sum(spd.vacancies) as vacancies,
			sum(spd.total_estimated_cost) as total_estimated_cost
			from `tabStaffing Plan Detail` spd join `tabStaffing Plan` sp on spd.parent=sp.name
			where spd.designation=%s and sp.docstatus=1
...
def get_designation_counts(designation, company, job_opening=None):
	...
	employee_count = finergy.db.count(
	...
	job_openings = finergy.db.count("Job Opening", filters)
...
def get_active_staffing_plan_details(company, designation, from_date=None, to_date=None):
	...
	if not company or not designation:
		finergy.throw(_("Please select Company and Designation"))

	staffing_plan = finergy.db.sql(
		"""
	...
	if not staffing_plan:
		parent_company = finergy.get_cached_value("Company", company, "parent_company")
		if parent_company:
