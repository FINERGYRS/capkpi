...
finergy.ui.form.on('Staffing Plan', {
	setup: function(frm) {
...
finergy.ui.form.on('Staffing Plan Detail', {
	...
	vacancies: function(frm, cdt, cdn) {
		...
		if(child.vacancies < child.current_openings) {
			finergy.throw(__("Vacancies cannot be lower than the current openings"));
		}
		set_number_of_positions(frm, cdt, cdn);
	},
...
var set_number_of_positions = function(frm, cdt, cdn) {
	let child = locals[cdt][cdn];
	if (!child.designation) finergy.throw(__("Please enter the designation"));
	finergy.call({
		"method": "capkpi.hr.doctype.staffing_plan.staffing_plan.get_designation_counts",
		args: {
		...
		callback: function (data) {
			if(data.message){
				finergy.model.set_value(cdt, cdn, 'current_count', data.message.employee_count);
				finergy.model.set_value(cdt, cdn, 'current_openings', data.message.job_openings);
				...
				if (cint(child.number_of_positions) < total_positions){
					finergy.model.set_value(cdt, cdn, 'number_of_positions', total_positions);
				}
			}
			else{ // No employees for this designation
				finergy.model.set_value(cdt, cdn, 'current_count', 0);
				finergy.model.set_value(cdt, cdn, 'current_openings', 0);
			}
		}
	});
...
var set_total_estimated_cost = function(frm, cdt, cdn) {
	...
	if(child.vacancies > 0 && child.estimated_cost_per_position) {
		finergy.model.set_value(cdt, cdn, 'total_estimated_cost', child.vacancies * child.estimated_cost_per_position);
	}
	else {
		finergy.model.set_value(cdt, cdn, 'total_estimated_cost', 0);
	}
