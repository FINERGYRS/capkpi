...
class TestStaffingPlan(unittest.TestCase):
	def test_staffing_plan(self):
		_set_up()
		finergy.db.set_value("Company", "_Test Company 3", "is_group", 1)
		if finergy.db.exists("Staffing Plan", "Test"):
			return
		staffing_plan = finergy.new_doc("Staffing Plan")
	...
	def test_staffing_plan_subsidiary_company(self):
		...
		if finergy.db.exists("Staffing Plan", "Test 1"):
			return
		staffing_plan = finergy.new_doc("Staffing Plan")
...
def make_company(name=None, abbr=None):
	...
	if finergy.db.exists("Company", name):
		return
