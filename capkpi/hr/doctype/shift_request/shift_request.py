...
from finergy.utils import formatdate, getdate

from capkpi.hr.utils import share_doc_with_approver, validate_active_employee
...
class ShiftRequest(Document):
	...
	def on_submit(self):
		if self.status not in ["Approved", "Rejected"]:
			finergy.throw(_("Only Shift Request with status 'Approved' and 'Rejected' can be submitted"))
		if self.status == "Approved":
			assignment_doc = finergy.new_doc("Shift Assignment")
			...
			finergy.msgprint(
				_("Shift Assignment: {0} created for Employee: {1}").format(
					finergy.bold(assignment_doc.name), finergy.bold(self.employee)
				)
			)

	def on_cancel(self):
		shift_assignment_list = finergy.get_list(
		...
		if shift_assignment_list:
			for shift in shift_assignment_list:
				shift_assignment_doc = finergy.get_doc("Shift Assignment", shift["name"])
				shift_assignment_doc.cancel()

	def validate_default_shift(self):
		default_shift = finergy.get_value("Employee", self.employee, "default_shift")
		if self.shift_type == default_shift:
			finergy.throw(
				_("You can not request for your Default Shift: {0}").format(finergy.bold(self.shift_type))
			)

	def validate_approver(self):
		department = finergy.get_value("Employee", self.employee, "department")
		shift_approver = finergy.get_value("Employee", self.employee, "shift_request_approver")
		...
		if self.approver not in approvers:
			finergy.throw(_("Only Approvers can Approve this Request."))
