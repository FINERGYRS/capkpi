...
class TestShiftRequest(unittest.TestCase):
	...
	def test_make_shift_request(self):
		...
		department = finergy.get_value("Employee", "_T-Employee-00001", "department")
		set_shift_approver(department)
		approver = finergy.db.sql(
		...
		shift_assignment = finergy.db.get_value(
			"Shift Assignment",
		...
		shift_assignment_docstatus = finergy.db.get_value(
	...
	def test_shift_request_approver_perms(self):
		employee = finergy.get_doc("Employee", "_T-Employee-00001")
		...
		self.assertTrue(shift_request.name in finergy.share.get_shared("Shift Request", user))

		# check shared doc revoked
		shift_request.reload()
		department = finergy.get_value("Employee", "_T-Employee-00001", "department")
		set_shift_approver(department)
		department_approver = finergy.db.sql(
			"""select approver from `tabDepartment Approver` where parent= %s and parentfield = 'shift_request_approver'""",
			(department),
		)[0][0]
		shift_request.approver = department_approver
		shift_request.save()
		self.assertTrue(shift_request.name not in finergy.share.get_shared("Shift Request", user))
...
def set_shift_approver(department):
	department_doc = finergy.get_doc("Department", department)
