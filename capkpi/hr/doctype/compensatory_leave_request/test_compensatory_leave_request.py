...
import finergy
from finergy.utils import add_days, add_months, today

from capkpi.hr.doctype.attendance_request.test_attendance_request import get_employee
from capkpi.hr.doctype.leave_application.leave_application import get_leave_balance_on
...
class TestCompensatoryLeaveRequest(unittest.TestCase):
	def setUp(self):
		finergy.db.sql(""" delete from `tabCompensatory Leave Request`""")
		finergy.db.sql(""" delete from `tabLeave Ledger Entry`""")
		finergy.db.sql(""" delete from `tabLeave Allocation`""")
		finergy.db.sql(
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		leave_ledger_entry = finergy.get_all("Leave Ledger Entry", fields="*", filters=filters)
...
def get_compensatory_leave_request(employee, leave_date=today()):
	prev_comp_leave_req = finergy.db.get_value(
	...
	if prev_comp_leave_req:
		return finergy.get_doc("Compensatory Leave Request", prev_comp_leave_req)
...
def create_holiday_list():
	if finergy.db.exists("Holiday List", "_Test Compensatory Leave"):
		return
