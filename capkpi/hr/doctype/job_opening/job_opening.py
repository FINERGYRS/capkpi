...
from capkpi.hr.doctype.staffing_plan.staffing_plan import (
...
class JobOpening(WebsiteGenerator):
	...
	def validate(self):
		if not self.route:
			self.route = finergy.scrub(self.job_title).replace("_", "-")
	...
	def validate_current_vacancies(self):
		if not self.staffing_plan:
			...
			self.planned_vacancies = finergy.db.get_value(
		...
		if self.staffing_plan and self.planned_vacancies:
			staffing_plan_company = finergy.db.get_value("Staffing Plan", self.staffing_plan, "company")

			designation_counts = get_designation_counts(self.designation, self.company, self.name)
			current_count = designation_counts["employee_count"] + designation_counts["job_openings"]

			number_of_positions = finergy.db.get_value(
				"Staffing Plan Detail",
				{"parent": self.staffing_plan, "designation": self.designation},
				"number_of_positions",
			)
			...
			if number_of_positions <= current_count:
				finergy.throw(
					_(
						...
						finergy.bold(self.designation), get_link_to_form("Staffing Plan", self.staffing_plan)
					),
