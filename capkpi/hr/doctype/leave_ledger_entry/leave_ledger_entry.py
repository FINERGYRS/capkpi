...
from finergy.utils import DATE_FORMAT, flt, getdate, today


class LeaveLedgerEntry(Document):
	def validate(self):
		if getdate(self.from_date) > getdate(self.to_date):
			finergy.throw(_("To date needs to be before from date"))

	def on_cancel(self):
		# allow cancellation of expiry leaves
		if self.is_expired:
			finergy.db.set_value("Leave Allocation", self.transaction_name, "expired", 0)
		else:
			finergy.throw(_("Only expired allocation can be cancelled"))
...
def validate_leave_allocation_against_leave_application(ledger):
	...
	leave_application_records = finergy.db.sql_list(
		"""
...
def get_previous_expiry_ledger_entry(ledger):
	...
	creation_date = finergy.db.get_value(
		"Leave Ledger Entry",
		filters={
...
def process_expired_allocation():
	...
	leave_type_records = finergy.db.get_values(
	...
	expire_allocation = finergy.db.sql(
		"""
		SELECT
...
def expire_allocation(allocation, expiry_date=None):
	...
	finergy.db.set_value("Leave Allocation", allocation.name, "expired", 1)
