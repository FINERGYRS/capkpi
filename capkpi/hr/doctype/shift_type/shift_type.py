...
from finergy.utils import cint, get_datetime, getdate
...
from capkpi.hr.doctype.employee_checkin.employee_checkin import (
	calculate_working_hours,
	mark_attendance_and_link_log,
)
from capkpi.hr.doctype.shift_assignment.shift_assignment import (
...
class ShiftType(Document):
	...
	def mark_absent_for_dates_with_no_attendance(self, employee):
		...
		"""
		date_of_joining, relieving_date, employee_creation = finergy.get_cached_value(
					...
					finergy.get_doc(
						{
							...
							"content": finergy._("Employee was marked Absent due to missing Employee Checkins."),
						}
	...
	def get_assigned_employee(self, from_date=None, consider_default_shift=False):
		...
		assigned_employees = finergy.get_all("Shift Assignment", filters, pluck="employee")

		if consider_default_shift:
			filters = {"default_shift": self.name, "status": ["!=", "Inactive"]}
			default_shift_employees = finergy.get_all("Employee", filters, pluck="name")
			return list(set(assigned_employees + default_shift_employees))
		return assigned_employees


def process_auto_attendance_for_all_shifts():
	shift_list = finergy.get_all("Shift Type", filters={"enable_auto_attendance": "1"}, pluck="name")
	for shift in shift_list:
		doc = finergy.get_cached_doc("Shift Type", shift)
