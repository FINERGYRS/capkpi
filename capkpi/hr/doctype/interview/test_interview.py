...
from finergy.utils import add_days, get_time, getdate, nowtime

from capkpi.hr.doctype.designation.test_designation import create_designation
from capkpi.hr.doctype.interview.interview import DuplicateInterviewRoundError
...
class TestInterview(unittest.TestCase):
	...
	def test_notification_for_scheduling(self):
		from capkpi.hr.doctype.interview.interview import send_interview_reminder
	...
	def test_notification_for_feedback_submission(self):
		from capkpi.hr.doctype.interview.interview import send_daily_feedback_reminder
...
def create_interview_round(name, skill_set, interviewers=[], designation=None, save=True):
	...
	interview_round = finergy.new_doc("Interview Round")
...
def create_skill_set(skill_set):
	for skill in skill_set:
		if not finergy.db.exists("Skill", skill):
			doc = finergy.new_doc("Skill")
			doc.skill_name = skill
			doc.save()


def create_interview_type(name="test_interview_type"):
	if finergy.db.exists("Interview Type", name):
		return finergy.get_doc("Interview Type", name).name
	else:
		doc = finergy.new_doc("Interview Type")
		doc.name = name
