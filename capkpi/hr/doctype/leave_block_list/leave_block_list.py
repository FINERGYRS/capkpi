	...
	def validate(self):
		...
		for d in self.get("leave_block_list_dates"):
			...
			if d.block_date in dates:
				finergy.msgprint(_("Date is repeated") + ":" + d.block_date, raise_exception=1)
...
def get_applicable_block_lists(employee=None, company=None, all_lists=False):
	...
	if department:
		block_list = finergy.db.get_value("Department", department, "leave_block_list")
		add_block_list(block_list)

	# global
	for block_list in finergy.db.sql_list(
...
def is_user_in_allow_list(block_list):
	return finergy.session.user in finergy.db.sql_list(
		"""select allow_user
