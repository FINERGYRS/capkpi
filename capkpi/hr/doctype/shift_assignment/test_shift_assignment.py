...
from capkpi.hr.doctype.shift_assignment.shift_assignment import get_events

test_dependencies = ["Shift Type"]


class TestShiftAssignment(FinergyTestCase):
	def setUp(self):
		finergy.db.delete("Shift Assignment")
		if not finergy.db.exists("Shift Type", "Day Shift"):
			finergy.get_doc(
				{"doctype": "Shift Type", "name": "Day Shift", "start_time": "9:00:00", "end_time": "18:00:00"}
			).insert()

	def test_make_shift_assignment(self):
		shift_assignment = finergy.get_doc(
			{
	...
	def test_overlapping_for_ongoing_shift(self):
		...
		shift_assignment_1 = finergy.get_doc(
			{
				"doctype": "Shift Assignment",
				"shift_type": "Day Shift",
				"company": "_Test Company",
		...
		self.assertRaises(finergy.ValidationError, shift_assignment.save)
	...
	def test_overlapping_for_fixed_period_shift(self):
		...
		shift_assignment_3 = finergy.get_doc(
			{
				"doctype": "Shift Assignment",
				"shift_type": "Day Shift",
				"company": "_Test Company",
		...
		self.assertRaises(finergy.ValidationError, shift_assignment_3.save)
