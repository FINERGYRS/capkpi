...
from finergy.utils import cstr, getdate, now_datetime, nowdate
...
class ShiftAssignment(Document):
	...
	def validate_overlapping_dates(self):
		...
		assigned_shifts = finergy.db.sql(
			"""
	...
	def throw_overlap_error(self, shift_details):
		shift_details = finergy._dict(shift_details)
		if shift_details.docstatus == 1 and shift_details.status == "Active":
			msg = _("Employee {0} already has Active Shift {1}: {2}").format(
				finergy.bold(self.employee), finergy.bold(self.shift_type), finergy.bold(shift_details.name)
			)
...
def get_events(start, end, filters=None):
	...
	employee = finergy.db.get_value(
		"Employee", {"user_id": finergy.session.user}, ["name", "company"], as_dict=True
	)
	if employee:
...
def add_assignments(start, end, conditions=None):
	...
	records = finergy.db.sql(query, {"start_date": start, "end_date": end}, as_dict=True)
	...
	for d in records:
		...
		while daily_event_start <= daily_event_end:
			start_timing = (
				finergy.utils.get_datetime(daily_event_start) + shift_timing_map[d.shift_type]["start_time"]
			)
			end_timing = (
				finergy.utils.get_datetime(daily_event_start) + shift_timing_map[d.shift_type]["end_time"]
			)
...
def get_employee_shift(
	...
	if for_date is None:
		for_date = nowdate()
	default_shift = finergy.get_cached_value("Employee", employee, "default_shift")
	shift_type_name = None
	shift_assignment_details = finergy.db.get_value(
		"Shift Assignment",
	...
	if shift_type_name:
		holiday_list_name = finergy.get_cached_value("Shift Type", shift_type_name, "holiday_list")
...
def get_shift_details(shift_type_name, for_date=None):
	...
	if not for_date:
		for_date = nowdate()
	shift_type = finergy.get_cached_value(
		"Shift Type",
		shift_type_name,
		[
			"name",
