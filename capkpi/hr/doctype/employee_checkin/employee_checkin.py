...
class EmployeeCheckin(Document):
	...
	def validate_duplicate_log(self):
		doc = finergy.db.exists(
			"Employee Checkin", {"employee": self.employee, "time": self.time, "name": ["!=", self.name]}
		)
		if doc:
			doc_link = finergy.get_desk_link("Employee Checkin", doc)
			finergy.throw(
...
def add_log_based_on_employee_field(
	...
	if not employee_field_value or not timestamp:
		finergy.throw(_("'employee_field_value' and 'timestamp' are required."))

	employee = finergy.db.get_values(
		"Employee",
	...
	doc = finergy.new_doc("Employee Checkin")
...
def mark_attendance_and_link_log(
	...
	if attendance_status == "Skip":
		...
		company = finergy.get_cached_value("Employee", employee, "company")
		duplicate = finergy.db.exists(
			"Attendance",
		...
		finergy.throw(_("{} is an invalid Attendance Status.").format(attendance_status))
