...
class TestEmployeeCheckin(unittest.TestCase):
	...
	def test_mark_attendance_and_link_log(self):
		...
		attendance_count = finergy.db.count(
			"Attendance",
	...
	def test_unlink_attendance_on_cancellation(self):
		...
		linked_logs = finergy.db.get_all("Employee Checkin", {"attendance": attendance.name})
	...
	def test_calculate_working_hours(self):
		...
		logs_type_1 = [finergy._dict(x) for x in logs_type_1]
		logs_type_2 = [finergy._dict(x) for x in logs_type_2]
