...
finergy.ui.form.on('Expense Claim', {
	onload: function(frm) {
	...
	company: function(frm) {
		...
		for (var i = 0; i < expenses.length; i++) {
			...
			finergy.call({
				method: "capkpi.hr.doctype.expense_claim.expense_claim.get_expense_claim_account_and_cost_center",
				args: {
...
finergy.ui.form.on('Expense Claim Detail', {
	expense_type: function(frm, cdt, cdn) {
		var d = locals[cdt][cdn];
		if (!frm.doc.company) {
			d.expense_type = "";
			finergy.msgprint(__("Please set the Company"));
...
cur_frm.cscript.onload = function(doc) {
	if (doc.__islocal) {
		cur_frm.set_value("posting_date", finergy.datetime.get_today());
...
cur_frm.cscript.refresh = function(doc) {
	...
	if(!doc.__islocal) {
		...
		if (doc.docstatus===1) {
			...
			if (cint(doc.total_amount_reimbursed) > 0 && finergy.model.can_read(entry_doctype)) {
				cur_frm.add_custom_button(__('Bank Entries'), function() {
					...
					finergy.set_route("List", entry_doctype);
				}, __("View"));
			}
			/* eslint-enable */
		}
...
cur_frm.cscript.set_help = function(doc) {
	cur_frm.set_intro("");
	if(doc.__islocal && !in_list(finergy.user_roles, "HR User")) {
...
finergy.ui.form.on("Expense Claim", {
	...
	onload: function(frm) {
		if (frm.doc.docstatus == 0) {
			return finergy.call({
				method: "capkpi.hr.doctype.leave_application.leave_application.get_mandatory_approval",
				args: {
					doctype: frm.doc.doctype,
				},
				callback: function(r) {
	...
	refresh: function(frm) {
		...
		if (
			frm.doc.docstatus === 1
			&& frm.doc.status !== "Paid"
			&& finergy.model.can_create("Payment Entry")
		) {
	...
	make_payment_entry: function(frm) {
		...
		if(frm.doc.__onload && frm.doc.__onload.make_payment_via_journal_entry) {
			method = "capkpi.hr.doctype.expense_claim.expense_claim.make_bank_entry";
		}
		return finergy.call({
			method: method,
			args: {
	...
	employee_name: function(frm) {
		capkpi.expense_claim.set_title(frm);
	},
	...
	get_advances: function(frm) {
		finergy.model.clear_table(frm.doc, "advances");
		if (frm.doc.employee) {
			return finergy.call({
				method: "capkpi.hr.doctype.expense_claim.expense_claim.get_advances",
				args: {
					employee: frm.doc.employee
				},
				callback: function(r, rt) {
					...
					if(r.message) {
						$.each(r.message, function(i, d) {
							var row = finergy.model.add_child(frm.doc, "Expense Claim Advance", "advances");
...
finergy.ui.form.on("Expense Claim Detail", {
	amount: function(frm, cdt, cdn) {
		...
		finergy.model.set_value(cdt, cdn, 'sanctioned_amount', child.amount);
	},
	...
	cost_center: function(frm, cdt, cdn) {
		capkpi.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "expenses", "cost_center");
	}
});

finergy.ui.form.on("Expense Claim Advance", {
	employee_advance: function(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		if(!frm.doc.employee){
			finergy.msgprint(__('Select an employee to get the employee advance.'));
			frm.doc.advances = [];
			refresh_field("advances");
		}
		else {
...
finergy.ui.form.on("Expense Taxes and Charges", {
