...
class InterviewFeedback(Document):
		...
		if self.interviewer not in applicable_interviewers:
			finergy.throw(
				_("{0} is not allowed to submit Interview Feedback for the Interview: {1}").format(
					finergy.bold(self.interviewer), finergy.bold(self.interview)
				)
			)

	def validate_interview_date(self):
		scheduled_date = finergy.db.get_value("Interview", self.interview, "scheduled_on")

		if getdate() < getdate(scheduled_date) and self.docstatus == 1:
			finergy.throw(
				_("{0} submission before {1} is not allowed").format(
					finergy.bold("Interview Feedback"), finergy.bold("Interview Scheduled Date")
				)
			)

	def validate_duplicate(self):
		duplicate_feedback = finergy.db.exists(
			"Interview Feedback",
	...
	def update_interview_details(self):
		doc = finergy.get_doc("Interview", self.interview)
		total_rating = 0
...
def get_applicable_interviewers(interview):
	data = finergy.get_all("Interview Detail", filters={"parent": interview}, fields=["interviewer"])
