...
from capkpi.hr.doctype.interview.test_interview import (
...
class TestInterviewFeedback(unittest.TestCase):
	def test_validation_for_skill_set(self):
		...
		finergy.set_user(interviewer)

		self.assertRaises(finergy.ValidationError, interview_feedback.save)
	...
	def test_average_ratings_on_feedback_submission_and_cancellation(self):
		...
		avg_on_interview_detail = finergy.db.get_value(
			"Interview Detail",
			{
