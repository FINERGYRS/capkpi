finergy.ui.form.on("Employee Attendance Tool", {
	refresh: function(frm) {
		frm.disable_save();
	},

	onload: function(frm) {
		frm.set_value("date", finergy.datetime.get_today());
		capkpi.employee_attendance_tool.load_employees(frm);
	},
...
capkpi.employee_attendance_tool = {
	load_employees: function(frm) {
		if(frm.doc.date) {
			finergy.call({
				method: "capkpi.hr.doctype.employee_attendance_tool.employee_attendance_tool.get_employees",
				args: {
					date: frm.doc.date,
				...
				callback: function(r) {
					if(r.message['unmarked'].length > 0) {
						...
						frm.EmployeeSelector = new capkpi.EmployeeSelector(frm, frm.employee_area, r.message['unmarked'])
					}
					else{
					...
					if(r.message['marked'].length > 0) {
						...
						frm.marked_employee = new capkpi.MarkedEmployee(frm, frm.marked_employee_area, r.message['marked'])
					}
					else{
...
capkpi.MarkedEmployee = Class.extend({
...
capkpi.EmployeeSelector = Class.extend({
	init: function(frm, wrapper, employee) {
		this.wrapper = wrapper;
		this.frm = frm;
		this.make(frm, employee);
	...
	make: function(frm, employee) {
		...
		mark_employee_toolbar.find(".btn-mark-present")
			...
			.on("click", function() {
				...
				finergy.call({
					method: "capkpi.hr.doctype.employee_attendance_tool.employee_attendance_tool.mark_employee_attendance",
					args:{
