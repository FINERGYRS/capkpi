...
class JobOffer(Document):
	def onload(self):
		employee = finergy.db.get_value("Employee", {"job_applicant": self.job_applicant}, "name") or ""
		self.set_onload("employee", employee)

	def validate(self):
		self.validate_vacancies()
		job_offer = finergy.db.exists(
		...
		if job_offer and job_offer != self.name:
			finergy.throw(
				_("Job Offer: {0} is already for Job Applicant: {1}").format(
					finergy.bold(job_offer), finergy.bold(self.job_applicant)
				)
			)

	def validate_vacancies(self):
		staffing_plan = get_staffing_plan_detail(self.designation, self.company, self.offer_date)
		check_vacancies = finergy.get_single("HR Settings").check_vacancies
		if staffing_plan and check_vacancies:
			...
			if not staffing_plan.get("vacancies") or cint(staffing_plan.vacancies) - len(job_offers) <= 0:
				error_variable = "for " + finergy.bold(self.designation)
				if staffing_plan.get("parent"):
					error_variable = finergy.bold(get_link_to_form("Staffing Plan", staffing_plan.parent))

				finergy.throw(_("There are no vacancies under staffing plan {0}").format(error_variable))
...
def update_job_applicant(status, job_applicant):
	if status in ("Accepted", "Rejected"):
		finergy.set_value("Job Applicant", job_applicant, "status", status)


def get_staffing_plan_detail(designation, company, offer_date):
	detail = finergy.db.sql(
		"""
	...
	return finergy._dict(detail[0]) if (detail and detail[0].parent) else None


@finergy.whitelist()
def make_employee(source_name, target_doc=None):
	def set_missing_values(source, target):
		target.personal_email, target.first_name = finergy.db.get_value(
