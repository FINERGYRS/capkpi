...
from finergy.utils import add_days, cstr, date_diff, getdate
...
from capkpi.hr.utils import get_holiday_dates_for_employee
...
@finergy.whitelist()
def get_template():
	if not finergy.has_permission("Attendance", "create"):
		raise finergy.PermissionError

	args = finergy.local.form_dict

	if getdate(args.from_date) > getdate(args.to_date):
		finergy.throw(_("To Date should be greater than From Date"))
	...
	try:
		...
		finergy.respond_as_web_page("Holiday List Missing", html=e)
		return

	# write out response as a type csv
	finergy.response["result"] = cstr(w.getvalue())
	finergy.response["type"] = "csv"
	finergy.response["doctype"] = "Attendance"


def add_header(w):
	status = ", ".join(
		(finergy.get_meta("Attendance").get_field("status").options or "").strip().split("\n")
	)
	w.writerow(["Notes:"])
...
def get_naming_series():
	series = finergy.get_meta("Attendance").get_field("naming_series").options.strip().split("\n")
	if not series:
		finergy.throw(_("Please setup numbering series for Attendance via Setup > Numbering Series"))
	return series[0]
...
def upload():
	...
	rows = read_csv_content(finergy.local.uploaded_file)
	if not rows:
		finergy.throw(_("Please select a csv file"))
	finergy.enqueue(import_attendances, rows=rows, now=True if len(rows) < 200 else False)
...
def import_attendances(rows):
	...
	from finergy.utils.csvutils import check_record, import_doc

	for i, row in enumerate(rows):
		if not row:
			continue
		row_idx = i + 5
		d = finergy._dict(zip(columns, row))

		d["doctype"] = "Attendance"
		if d.name:
			d["docstatus"] = finergy.db.get_value("Attendance", d.name, "docstatus")

		try:
			check_record(d)
			ret.append(import_doc(d, "Attendance", 1, row_idx, submit=True))
			finergy.publish_realtime("import_attendance", dict(progress=i, total=len(rows)))
		except AttributeError:
			pass
		except Exception as e:
			error = True
			ret.append("Error for row (#%d) %s : %s" % (row_idx, len(row) > 1 and row[1] or "", cstr(e)))
			finergy.errprint(finergy.get_traceback())
	...
	finergy.publish_realtime("import_attendance", dict(messages=ret, error=error))
