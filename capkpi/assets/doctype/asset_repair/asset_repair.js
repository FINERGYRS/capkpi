...
finergy.ui.form.on('Asset Repair', {
	setup: function(frm) {
	...
	repair_status: (frm) => {
		if (frm.doc.completion_date && frm.doc.repair_status == "Completed") {
			finergy.call ({
				method: "capkpi.assets.doctype.asset_repair.asset_repair.get_downtime",
				args: {
		...
		if (frm.doc.repair_status == "Completed") {
			frm.set_value('completion_date', finergy.datetime.now_datetime());
		}
	},
...
finergy.ui.form.on('Asset Repair Consumed Item', {
	item_code: function(frm, cdt, cdn) {
		...
		finergy.call({
			...
			callback: function(r) {
				finergy.model.set_value(cdt, cdn, 'valuation_rate', r.message);
			}
		});
	},
	...
	consumed_quantity: function(frm, cdt, cdn) {
		...
		finergy.model.set_value(cdt, cdn, 'total_value', row.consumed_quantity * row.valuation_rate);
	},
});
