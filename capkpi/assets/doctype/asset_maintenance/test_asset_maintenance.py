...
import finergy
from finergy.utils import add_days, get_last_day, nowdate

from capkpi.assets.doctype.asset_maintenance.asset_maintenance import calculate_next_due_date
...
class TestAssetMaintenance(unittest.TestCase):
	...
	def test_create_asset_maintenance(self):
		...
		asset_doc = finergy.get_doc("Asset", asset_name)
		...
		if not finergy.db.exists("Asset Maintenance", "Photocopier"):
			asset_maintenance = finergy.get_doc(
				{
	...
	def test_create_asset_maintenance_log(self):
		if not finergy.db.exists("Asset Maintenance Log", "Photocopier"):
			asset_maintenance_log = finergy.get_doc(
				{
...
def create_maintenance_team():
	...
	if not finergy.db.exists("Role", "Technician"):
		finergy.get_doc({"doctype": "Role", "role_name": "Technician"}).insert()
	...
	if not finergy.db.exists("Asset Maintenance Team", "Team Awesome"):
		finergy.get_doc(
			{
