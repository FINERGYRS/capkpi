...
import finergy
from finergy.utils import add_days, add_months, cstr, flt, get_last_day, getdate, nowdate

from capkpi.accounts.doctype.purchase_invoice.test_purchase_invoice import make_purchase_invoice
from capkpi.assets.doctype.asset.asset import make_sales_invoice
...
class AssetSetup(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		finergy.db.sql("delete from `tabTax Rule`")
...
class TestAsset(AssetSetup):
	...
	def test_gross_purchase_amount_is_mandatory(self):
		...
		self.assertRaises(finergy.MandatoryError, asset.save)
	...
	def test_pr_or_pi_mandatory_if_not_existing_asset(self):
		...
		self.assertRaises(finergy.ValidationError, asset.save)
	...
	def test_validate_item(self):
		asset = create_asset(item_code="MacBook Pro", do_not_save=1)
		item = finergy.get_doc("Item", "MacBook Pro")
	...
	def test_purchase_asset(self):
		...
		asset_name = finergy.db.get_value("Asset", {"purchase_receipt": pr.name}, "name")
	...
	def test_is_fixed_asset_set(self):
		...
		doc = finergy.new_doc("Purchase Invoice")
	...
	def test_gle_made_by_asset_sale(self):
		...
		self.assertEqual(finergy.db.get_value("Asset", asset.name, "status"), "Sold")
		...
		self.assertEqual(finergy.db.get_value("Asset", asset.name, "status"), "Partially Depreciated")
	...
	def test_cwip_accounting(self):
		...
		asset = finergy.db.get_value("Asset", {"purchase_receipt": pr.name, "docstatus": 0}, "name")

		asset_doc = finergy.get_doc("Asset", asset)
	...
	def test_asset_cwip_toggling_cases(self):
		cwip = finergy.db.get_value("Asset Category", "Computers", "enable_cwip_accounting")
		name = finergy.db.get_value(
			"Asset Category Account", filters={"parent": "Computers"}, fieldname=["name"]
		)
		cwip_acc = "CWIP Account - _TC"

		finergy.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", 0)
		finergy.db.set_value("Asset Category Account", name, "capital_work_in_progress_account", "")
		finergy.db.get_value("Company", "_Test Company", "capital_work_in_progress_account", "")
		...
		asset = finergy.db.get_value("Asset", {"purchase_invoice": pi.name, "docstatus": 0}, "name")
		asset_doc = finergy.get_doc("Asset", asset)
		...
		finergy.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", 1)
		finergy.db.set_value("Asset Category Account", name, "capital_work_in_progress_account", cwip_acc)
		asset = finergy.db.get_value("Asset", {"purchase_receipt": pr.name, "docstatus": 0}, "name")
		asset_doc = finergy.get_doc("Asset", asset)
		asset_doc.available_for_use_date = nowdate()
		asset_doc.calculate_depreciation = 0
		...
		finergy.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", cwip)
		finergy.db.set_value("Asset Category Account", name, "capital_work_in_progress_account", cwip_acc)
		finergy.db.get_value("Company", "_Test Company", "capital_work_in_progress_account", cwip_acc)
...
class TestDepreciationMethods(AssetSetup):
	...
	def test_discounted_wdv_depreciation_rate_for_indian_region(self):
		# set indian company
		company_flag = finergy.flags.company
		finergy.flags.company = "_Test Company"

		finance_book = finergy.new_doc("Finance Book")
		...
		finergy.flags.company = company_flag
	...
	def test_monthly_depreciation_by_wdv_method(self):
		existing_precision = finergy.db.get_default("float_precision")
		finergy.db.set_default("float_precision", 3)
		asset = create_asset(
			calculate_depreciation=1,
		...
		self.assertEqual(schedules, expected_schedules)
		finergy.db.set_default("float_precision", existing_precision)
...
class TestDepreciationBasics(AssetSetup):
	...
	def test_get_depreciation_amount(self):
		...
		from capkpi.assets.doctype.asset.asset import get_depreciation_amount
	...
	def test_number_of_depreciations(self):
		...
		self.assertRaises(finergy.ValidationError, asset_2.save)
	...
	def test_depr_entry_posting_when_depr_expense_account_is_an_expense_account(self):
		...
		je = finergy.get_doc("Journal Entry", asset.schedules[0].journal_entry)
		accounting_entries = [
	...
	def test_depr_entry_posting_when_depr_expense_account_is_an_income_account(self):
		...
		depr_expense_account = finergy.get_doc("Account", "_Test Depreciations - _TC")
	...
	def test_depreciation_entry_cancellation(self):
		...
		finergy.get_doc("Journal Entry", depr_entry).cancel()
	...
	def test_gle_made_by_depreciation_entries(self):
		...
		finergy.db.set_value("Company", "_Test Company", "series_for_depreciation_entry", "DEPR-")
		post_depreciation_entries(date="2021-01-01")
		asset.load_from_db()
	...
	def test_asset_cost_center(self):
		...
		self.assertRaises(finergy.ValidationError, asset.submit)
...
def create_asset_data():
	if not finergy.db.exists("Asset Category", "Computers"):
		create_asset_category()

	if not finergy.db.exists("Item", "Macbook Pro"):
...
def create_asset_category():
	asset_category = finergy.new_doc("Asset Category")
...
def set_depreciation_settings_in_company():
	...
	finergy.db.set_value("Accounts Settings", None, "book_asset_depreciation_entry_automatically", 1)


def enable_cwip_accounting(asset_category, enable=1):
	finergy.db.set_value("Asset Category", asset_category, "enable_cwip_accounting", enable)
